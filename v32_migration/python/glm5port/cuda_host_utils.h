...
template<typename T>
inline void check_impl_shim(T result, const char* const file, int const line) {
    if (result != T(0)) {
        char b[256];
        snprintf(b, sizeof(b), "cuda error %d at %s:%d", (int)result, file, line);
        throw std::runtime_error(b);
    }
}
#define check_cuda_value(val) rtp_llm::check_impl_shim((val), __FILE__, __LINE__)
