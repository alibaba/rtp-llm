...
def pip_deps():
    ...
    # XPU lockfile is generated with Python 3.10, matching the CUDA/ROCm build
    # infra. pip_parse only declares the hub repo and parses the hashed
    # lockfile; it does not download wheels, so declaring it in every container
    # is cheap. The actual whl_library fetches pull XPU-specific torch wheels
    # from the XPU extra-index-url, so they are gated by xpu_pip_gate below on
    # TF_NEED_XPU, so `bazel sync` / non-XPU builds never resolve the XPU
    # wheels.
    pip_parse(
        name = "pip_xpu_torch",
    ...
    # .bazelrc). Without this, `bazel sync` on a non-XPU build eagerly fetches
    # every pip_xpu_torch_* whl_library (XPU-specific torch wheels from the XPU
    # extra-index-url) that non-XPU builds do not need.
