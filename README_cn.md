...
# Install rtp-llm
cd rtp-llm
# For cuda12_9 (CUDA 12.9) environment, please use requirements_torch_gpu_cuda12_9.txt
pip3 install -r ./deps/requirements_torch_gpu_cuda12_9.txt
...
3. Bazel build time out
    ...
    **Resolution**:
     1. 修改pip源，在`deps/pip.bzl`里添加 `extra_pip_args=["--index-url=xxx", ...]` 配置（或使用 `PIP_EXTRA_ARGS`）
