...
# Install rtp-llm
cd rtp-llm
# For cuda12 environment, please use the resolved lock file
pip3 install -r ./deps/requirements_lock_torch_gpu_cuda12.txt
...
3. Bazel build time out
    ...
    **Resolution**:
     1. 修改pip源，在`MODULE.bazel`对应 hub 的 `pip.parse(...)` 里改 `experimental_index_url` / 追加 `extra_pip_args=["--index-url=xxx", ...]`
