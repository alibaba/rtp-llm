...
如果对 residual deficit 有贡献的 blocker 缺少可信 priority 来源，Master 不能声称
higher/same 或纯资源因果，返回 `8432 ADMISSION_UNAVAILABLE + UNSPECIFIED`。只有所有
相关 priority 来源可信、但没有可证明的 higher/same 前序阻塞时，才返回
`8431 RESOURCE_EXHAUSTED + RESOURCE_EXHAUSTED`。这里的 resource 表示所选路径未能在
admission budget 内提供容量，包含 KV/slot/token、dispatch/engine ACK 和 backpressure，
并不只表示 CUDA OOM。

### 8.3 多 endpoint

每个 endpoint 返回类型化 failure。任一 endpoint 的真实 residual blocker 缺少可信 priority 来源时，cluster 归因同样不可证明，优先返回 `8432 ADMISSION_UNAVAILABLE`。排除该情形后，只有 endpoint 对 higher/same 归因一致时，Scheduler 才公开该优先级原因；其他已知原因不一致时统一返回 `RESOURCE_EXHAUSTED`。禁止在多个 endpoint 的自由文本中任意挑一个原因。
...
只有测量事实能证明对应优先级原因时，才能填写 higher/same。真实 residual blocker
缺少可信 priority 来源时返回 `ADMISSION_UNAVAILABLE`；owner missing、Cancel unsupported、
commit conflict 等其他 subcause 保留在内部事实和指标中，并在没有 higher/same 证据时
折叠为 broadened `RESOURCE_EXHAUSTED`，不声称发生了物理 OOM。
...
| 8432 | `ADMISSION_UNAVAILABLE` | 相关占用缺少可信优先级来源，无法做 higher/same/resource 归因 |
...
|---|---:|---|---|---|
| `8429` 且有有效 QoS header | 429 | `Throttling.Aborted` | `Too many requests.` | 禁止 |
...
| `8429/8430/8431` 且无有效 QoS header | 503 | `ServiceUnavailable` | `Service unavailable.` | 禁止 |
| `8432 + UNSPECIFIED` | 503 | `ServiceUnavailable` | `Service unavailable.` | 禁止 |
| 其他非法 code/reason 组合 | 503 | `ServiceUnavailable` | `Service unavailable.` | 禁止，并上报 protocol error |
| 无健康 worker/路由不可用 | 503 | `ServiceUnavailable` | `Service unavailable.` | 沿用有限 route retry |
| 单请求超过 hard limit | 400/413 | 参数/容量专用名称 | 稳定校验信息 | 禁止 |

约束：

- 8429 仅在存在有效 QoS header 时映射为 `429 / Throttling.Aborted / Too many requests.`；无有效 header 时沿用 503 合同。
- 只读取显式 `x-dashscope-inner-qos-level`；边界固定为 `<50` / `>=50`。
- reason 缺失、未知或与 code 不匹配时，禁止从 `error_message` 猜测，固定回落为 `503 / ServiceUnavailable / Service unavailable.`，并上报 protocol-error 指标。
...
| 8432 | `UNSPECIFIED`（缺少可信优先级归因） |
| 8400 等既有非 admission 专用错误 | `UNSPECIFIED` |

### 10.1 Retry owner

- Master 内部的 version conflict、victim gone 等控制冲突，只能由 Master 在返回 Schedule response 前有限重试；
- 8429、8430、8431、8432 和 legacy 8511 一旦返回 Python，Backend 不得通过生成新 request ID 透明重试；
- 客户端是否根据 429 退避重试属于客户端策略，不属于服务端透明重试；
- Cancel transport unknown 只能针对同一 Prefill、同一 request ID 做有界幂等重试，不能释放容量、切换 victim 或生成新请求。

对于流式响应，如果 HTTP headers 或 token 已经发出，HTTP transport 可能保持 200；
终止 Dash frame 必须按显式 QoS 合同携带 `status_code=429` 或 `503`，以及对应的
`status_name` 和 `status_message`。
...
- **Python/Dash**：保留 8429 与 typed admission reason，禁止透明重试；有显式有效 QoS
  时按合同映射 429，无显式有效 QoS 或缺少可信 priority 归因时映射 503。
...
- 8429 永不透明重试；
- 8429 在 QoS 49/50 时输出 429 Aborted，无有效 QoS header 时输出历史 503；
- 8430/8431 在显式 QoS 49/50 时分别输出 ServiceOverloaded/ResourceExhausted；
- 无有效 QoS header 时统一沿用 `503 / ServiceUnavailable` 历史公开映射；
- 8432 的缺少可信 priority attribution 不解析 `error_message`，固定输出 503；非法
  code/reason 组合同样输出 503，并上报 protocol-error metric；
- legacy `error_msg` JSON 与 standalone `status_code/status_name/status_message` 一致；
- 已输出 token 后，terminal frame 仍保留与 header 合同一致的 429/503 字段。
...
8. victim 携带有效 QoS header 时，验证最终 Dash 状态为 `429 / Throttling.Aborted / Too many requests.`；无有效 header 时验证 `503 / ServiceUnavailable / Service unavailable.`。
...
- Dash 只在显式 QoS 有效时公开 Auto-TPM 429：8430/8431 按 `<50` / `>=50`
  区分名称，8429 使用 `Throttling.Aborted`；无有效 QoS 时三者均返回 503；
