...
A pre-service template initializes model weights, warmup, the core GPU KV allocation, executor and scheduler before the checkpoint barrier. It deliberately does not initialize the memory-cache, remote-cache or P2P connector coordinator. Those connectors, the system prompt, Torch preallocation, the normal engine computation loop, CacheStore and network listeners start only after template release, through `LocalRpcServer::startDeferredServices`. This applies to every rank, including ranks without a TP broadcaster.
...
Normal startup keeps its existing immediate connector and loop start. Repeated release must not create another connector coordinator or computation thread, and aborting a prepared template must safely stop an engine whose loop never started.
