...
## 15. RTP-LLM 与 vLLM EPD 对比复测（2026-08-21）

本节替代此前复用同一张图片、以及图片文件编号不完整时得到的对比结论。旧结果
可能命中 RTP-LLM 多模态缓存，或因缺少输入文件产生非性能类失败，不应继续作为
框架性能依据。

### 15.1 测试约束

- 模型：MiniMax-M3-MXFP8；
- 部署：Encoder/ViT 使用 GPU 5 单卡，Prefill 使用 GPU 1-2 TP2，Decode 使用
  GPU 3-4 TP2，P、D、E 为独立进程且不共卡；
- RTP-LLM：`47e05783fc78390725dcb58ea8c6e34940212449`；
- vLLM 源码：`bb233626caa31602728f7ee4625f3d2a4d1a3ad5`，运行时版本
  `0.27.2rc1.dev154+g5fd7a8883`；
- 输入：每个请求一张 1920x1080 JPEG，共生成 13200 张。每张图片的解码像素中
  都写入不同的 128x128 二进制块并校验 SHA256，不使用硬链接或相同内容副本；
- 请求：streaming，`max_tokens=1`，并发 64/96/128/160/192，对应每轮请求数
  192/288/384/320/384；
- 每个并发点运行三轮，并采集 QPS、TTFT、RT、错误数和 NVML 利用率/显存；
- 测试窗口持续检查同卡进程，发现外部任务时丢弃该轮。表中 RTP-LLM 指标为三轮
  中位数；vLLM 指标只统计该并发下完全无请求错误的轮次。

由于只生成一个 token，完整请求 RT 与 TTFT 基本相同，差值通常只有数毫秒。本节
表格保留 RT p50/p95，评估多模态首 token 时可直接将其视为 TTFT。

### 15.2 结果

| 并发 | RTP QPS | RTP RT p50/p95 | vLLM QPS | vLLM RT p50/p95 | vLLM 无错误轮数 | vLLM 错误 |
| ---: | ---: | ---: | ---: | ---: | ---: | ---: |
...
vLLM 在 C160 和 C192 只有一轮完全无错误，不能据此建立稳定的高并发性能结论。
失败类型包括 Encoder 连接被重置、服务端断开，以及 HTTP 200 后没有收到首个
streaming body。失败请求提前结束会抬高按成功数/总耗时计算的 QPS，因此含错误
轮次不进入上表的 vLLM 性能中位数。

三轮原始 QPS 波动如下：

| 并发 | RTP QPS 范围 / CV | vLLM QPS 范围 / CV |
| ---: | ---: | ---: |
...
资源侧观测：RTP-LLM 的 Prefill 平均 GPU 利用率中位数约为 60%-71%，ViT 为
15%-20%；vLLM 的 Prefill 约为 63%-80%，Encoder 为 21%-28%。每张 Prefill
GPU 的峰值显存约为 RTP-LLM 239.5 GiB、vLLM 264.2 GiB；每张 Decode GPU
约为 243.0 GiB 和 248.8 GiB。RTP-LLM ViT 峰值显存随唯一图片请求累计，从
首轮早期的 6.6 GiB 增长到第三轮约 37.0 GiB，而 vLLM Encoder 稳定在约
20.6 GiB。该增长需要后续区分 embedding cache 驻留和 CUDA allocator 保留。

### 15.3 结论和结果位置

- 修正后的数据不支持“vLLM 明显快于 RTP-LLM”的旧结论；C64-C128 整体接近，
  C96 的 RTP-LLM 吞吐更高；
- RTP-LLM 共 4704 个请求全部成功，并且 C192 吞吐最稳定；
- vLLM 在低并发仍有明显的轮间波动，高并发还存在 Proxy 到 Encoder 的连接稳定性
  问题，修复前不应使用含错误轮次比较峰值吞吐；
- RTP-LLM 的高并发 p95 仍明显波动，需要结合 batching/queue timeline 继续定位；
- RTP-LLM 结果：
...
## 16. RTP-LLM 与 vLLM EPD 稳态复测（2026-08-24）

本节使用更严格的 streaming 正确性校验重新测试第 15 节的相同负载。HTTP 200
只有在 SSE 中至少出现一个非空 `choices` 事件时才计为成功，空响应和连接重置均
计入错误。输入仍为每请求一张像素内容不同的 1920x1080 JPEG，输出一个 token，
E/P/D 拓扑和模型版本保持不变。

RTP-LLM 运行五轮。由于 `warm_up=0`，第一轮包含明显的模型和 kernel 冷启动，稳态
结果取第 2-5 轮中位数；这四轮共 6272 个正式请求，全部成功。RTP-LLM 开启
4096 项多模态和 URL cache，但正式请求的图片内容均不同，不存在跨请求 cache hit；
ViT batch 配置为 `max_requests=64`、`max_images=64`、`wait=10 ms`。

vLLM 运行三轮。Proxy 到 Encoder 的 stale keep-alive 复用已修复，C64-C160 三轮
全部成功。第三轮 C192 的压测客户端到 Proxy 仍有 7 次同类 stale connection
错误，因此 C192 只取两轮零错误结果；任何含错误轮次都不参与 QPS 和 RT 中位数。

### 16.1 完整链路结果

| 并发 | RTP QPS | vLLM QPS | RTP 吞吐差异 | RTP RT p50/p95 | vLLM RT p50/p95 | 有效轮数 RTP/vLLM |
| ---: | ---: | ---: | ---: | ---: | ---: | ---: |
...
RTP-LLM 在所有并发点的稳态吞吐都更高，稳态中位数峰值为 C160 的
44.206 QPS；vLLM 峰值为 C128 的 40.437 QPS。RTP-LLM 的 RT p50 低
8.5%-17.0%，但 C96-C192 的 p95 高 13.5%-56.5%。因此当前主要差距不是平均
吞吐，而是 RTP-LLM 高并发队列和批处理产生的尾延迟。今天的四个稳态轮次中，
RTP-LLM C128 QPS 为 42.953/42.503/42.223/48.839，不存在固定的 C128 吞吐断崖。

### 16.2 分段和资源证据

RTP-LLM ViT 请求的 p50/p95 从 C64 的 0.466/0.918 秒增长到 C192 的
2.017/3.595 秒。Prefill access log 中的 `wait_time` p95 同期从 1.367 秒增长到
4.868 秒，而非等待部分 p95 为 0.475-0.994 秒，说明高并发尾延迟主要在等待和
批调度阶段形成。vLLM Proxy 的 Encoder p50 从 1.138 秒增长到 3.945 秒，而
Prefill+Decode TTFB p50 保持在 0.337-0.378 秒，vLLM 的完整 RT 主要受 Encoder
排队控制。

RTP-LLM 在 C64/96/128/160/192 的实际平均 ViT batch 分别为
13.7/17.5/30.7/33.7/43.9，满 64 的批次比例为 0%/3.0%/8.0%/21.1%/48.6%。
该分布不支持直接增大 batch 上限：除 C192 外大部分批次没有触顶，而当前需要优先
改善的是 p95。下一项 A/B 固定 `wait=10 ms`，只将两个 batch 上限从 64 降到 32。

NVML 粗粒度采样显示，RTP-LLM Prefill 平均利用率中位数约 69%-71%，ViT 为
20%-26%；vLLM Prefill 为 74%-84%，Encoder 为 27%-29%。每张 Prefill GPU 的
峰值显存约为 RTP-LLM 239.7 GiB、vLLM 258.0 GiB。RTP-LLM ViT 显存随唯一图片
写入 4096 项 cache 增长到约 37.7 GiB，vLLM Encoder 稳定在约 16.4 GiB；该值
反映当前 cache 配置，不能直接当作模型固有显存差异。
...
### 16.4 ViT batch 64 降至 32 的 A/B

保持第 16 节 RTP-LLM 基线的全部条件不变，只将 `gpu_max_batch_size` 和
`gpu_max_batch_images` 从 64 改为 32，`gpu_batch_wait_ms` 保持 10。两组均运行
五轮、剔除第一轮冷启动，并取第 2-5 轮中位数；每组 7840 个正式请求全部成功。

| 并发 | QPS 64 -> 32 | QPS 差异 | RT p50 64 -> 32 | p50 差异 | RT p95 64 -> 32 | p95 差异 |
| ---: | ---: | ---: | ---: | ---: | ---: | ---: |
...
batch 32 在 C96-C192 的平均实际 batch 为 25.6-27.9，78%-80% 的 C128-C192
批次达到上限，说明新上限确实约束了调度。相比 batch 64，每图 ViT forward 时间
在五个并发点分别增加 8.3%/11.4%/21.8%/5.0%/8.3%；ViT 请求 p50 在 C96-C192
增加 34%-79%。更小的 batch 减少了单批工作量，但需要更多批次且 kernel 效率更低。

batch 32 的稳态 QPS CV 在 C64/96/128/160/192 为
12.1%/8.4%/21.4%/10.4%/13.1%，batch 64 为
8.2%/5.7%/7.1%/10.3%/1.5%。C192 的端到端中位数改善与更差的 ViT 自身延迟、
更大的轮间波动同时出现，更像批调度和 Prefill 交接的时序效应，不能据此全局降低
batch。结论是保留默认上限 64；若继续优化尾延迟，下一项应固定上限 64，仅 A/B
更短的 5 ms 等待，或在生产队列深度下验证自适应等待。

batch 32 结果：
`/data2/xieshui.yyx/m3vl_profile_artifacts/rtpllm_rtp_rt_batch32_wait10/20260824_112024`。

## 17. Prefill/Decode 全自动 KV 复测（2026-08-25）

本轮将 Prefill 和 Decode 的 `kv_cache_mem_mb` 均改为自动计算，其他主要负载条件
延续第 16 节：单请求一张内容不同的 1920x1080 JPEG、输出一个 token，E/ViT 单卡、
Prefill TP2、Decode TP2，ViT batch 上限 64、等待 10 ms。C64/96/128/160/192
各运行五轮，第一轮作为冷启动观察值，稳态统计取第 2-5 轮中位数；7840 个正式请求
全部成功。最终采用的数据窗口均通过外部 GPU 占用检查，受干扰的尝试未计入结果。

当前 RTP-LLM 版本为 `def9bf5b63835db869e6d152beec38f045e27f48`
（`fix(multimodal): bound ViT async compute concurrency`）。第 16 节 RTP-LLM 基线为
`47e05783fc78390725dcb58ea8c6e34940212449`，因此本轮与第 16 节不是只改变 KV 配置的
严格 A/B，结果用于确认当前版本的容量和性能，不单独归因于自动 KV。

### 17.1 自动 KV 容量

- Prefill rank 0 自动分配 16364 MiB、969 blocks、124032 tokens；rank 1 自动分配
  16486 MiB、976 blocks、124928 tokens。集群可用容量受较小 rank 限制；
- Decode 自动分配 728 MiB、43 blocks、5504 tokens；
- Prefill 和 Decode 仍保留 `reserver_runtime_mem_mb=32768` 及 5% block reserve；
- 本轮每请求只输出一个 token，Decode GPU 3/4 的采样利用率中位数为 0%。因此当前
  结果验证了以多模态 Prefill 为主的链路，但没有验证 Decode 自动 KV 在正常长输出下
  的容量和吞吐。Decode 需要另用至少 32 个输出 token 的负载复测。

### 17.2 稳态中位数

RTP-LLM 取第 2-5 轮中位数；vLLM 沿用第 16 节相同 EPD 配置的有效轮次中位数。

| 并发 | RTP QPS | vLLM QPS | RTP QPS 差异 | RTP RT p50 | vLLM RT p50 | RTP RT p95 | vLLM RT p95 |
| ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 64 | 40.019 | 38.147 | +4.9% | 1.287 s | 1.515 s | 2.554 s | 1.908 s |
| 96 | 41.967 | 39.689 | +5.7% | 1.991 s | 2.209 s | 3.113 s | 2.526 s |
| 128 | 41.095 | 40.437 | +1.6% | 2.699 s | 2.949 s | 4.302 s | 3.342 s |
| 160 | 39.732 | 39.609 | +0.3% | 3.507 s | 3.613 s | 4.780 s | 4.165 s |
| 192 | 45.196 | 39.871 | +13.4% | 3.659 s | 4.334 s | 4.734 s | 4.974 s |

RTP-LLM 的 QPS 和 p50 在全部并发点占优，但 p95 在 C64-C160 仍高于 vLLM；只有
C192 的 p95 更低。第 2-5 轮 RTP QPS 范围分别为 18.309-45.986、
24.362-46.015、31.549-47.574、27.874-46.060、41.793-47.753，C64-C160
仍存在明显轮间波动，不能只依据单轮峰值判断稳定吞吐。

### 17.3 最优有效轮次

为同时展示当前实现可达到的峰值状态，下面按每个并发点选择 p95 最低的零错误轮次，
QPS 和 p50 均取同一轮；该表不能替代上面的多轮稳定性统计。

| 并发 | RTP 轮次 | RTP QPS | RTP p50/p95 | vLLM 轮次 | vLLM QPS | vLLM p50/p95 | QPS 差异 | p50 差异 | p95 差异 |
| ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 64 | 5 | 45.599 | 1.208 / 1.883 s | 2 | 38.147 | 1.515 / 1.846 s | +19.5% | -20.3% | +2.0% |
| 96 | 5 | 46.015 | 1.724 / 2.817 s | 2 | 40.392 | 2.209 / 2.475 s | +13.9% | -22.0% | +13.8% |
| 128 | 5 | 47.574 | 2.295 / 3.411 s | 3 | 40.437 | 2.949 / 3.209 s | +17.7% | -22.2% | +6.3% |
| 160 | 4 | 46.060 | 2.873 / 4.053 s | 3 | 39.751 | 3.582 / 4.141 s | +15.9% | -19.8% | -2.1% |
| 192 | 5 | 47.753 | 3.682 / 4.439 s | 2 | 40.025 | 4.352 / 4.895 s | +19.3% | -15.4% | -9.3% |

峰值有效轮次中，RTP-LLM 在全部并发点的 QPS 高 13.9%-19.5%，p50 低
15.4%-22.2%；p95 在 C64/C96/C128 分别高 2.0%/13.8%/6.3%，在
C160/C192 分别低 2.1%/9.3%。这说明实现具有更高的峰值吞吐和较低的主体延迟，
但 C64-C160 的尾延迟稳定性尚未解决。

### 17.4 瓶颈和进程拓扑结论

按正式请求的图片 ID 范围重新关联第 2-5 轮后，C128/C160/C192 的 `wait_time`
p95 中位数分别为 3.34/4.15/3.81 秒，去除等待后的链路 p95 为
0.70/0.62/0.62 秒。对应 ViT 每图 forward 时间为 21.7/21.4/18.9 ms，实际平均
batch 为 26.5/29.1/29.5。该结果排除了外部 GPU 占用检查后被丢弃的尝试，说明当前
较差轮次主要由 FIFO admission 等待和流水线调度波动造成，不是 ViT kernel 或
Decode 计算退化，也没有证据支持继续增大 ViT batch 上限。

本轮日志中的 Prefill 配置为 `VIT_SEPARATION=2`，通过
`REMOTE_VIT_SERVER_IP=127.0.0.1:18634` 调用独立 ViT；Prefill 日志中没有
`ROLE_TYPE=VIT`、`start_vit_server` 或 `[PROCESS_SPAWN] Start vit server process`
记录。对应记录只出现在 `VIT_SEPARATION=1`、`ROLE_TYPE=VIT` 的独立 ViT launcher
日志中。因此现有日志不支持“Prefill 额外启动了一个 ViT 子进程”的判断；观察到的
ViT 子进程属于独立 ViT 服务。由于服务停止前没有保存实时 `pstree`，若后续监控仍将
该进程归到 Prefill，需要在运行时同时保存 PID、PPID 和完整命令行做最终确认。

本轮结果目录：
`/data2/xieshui.yyx/m3vl_profile_artifacts/rtpllm_rtp_rt_cache_on_recheck/20260825_143433`。

### 17.5 C128 以上的吞吐变化

最新 RTP-LLM 稳态中位数并不存在 C160、C192 相对 C128 同时下降：C160 下降
3.3%，C192 则提高 10.0%。

| 并发 | QPS 中位数 | 相对 C128 | QPS 轮次范围 |
| ---: | ---: | ---: | ---: |
| 128 | 41.095 | - | 31.549-47.574 |
| 160 | 39.732 | -3.3% | 27.874-46.060 |
| 192 | 45.196 | +10.0% | 41.793-47.753 |

#### Prefill KV admission 容量拐点

本轮 Prefill 较小 rank 有 969 个 KV blocks，allocator 报告 968 个可用 blocks，
其中 `reserve_block_ratio=5` 保留 48 个，因此正常 admission 最多可占用约 920 个
blocks。每个请求 `input_len=763`，`seq_size_per_block=128`，且正式请求
`reuse_len=0`，所以每个请求至少需要 `ceil(763 / 128) = 6` 个 blocks。当前负载
最多只能同时 admission `floor(920 / 6) = 153` 个请求。

| 并发 | 请求所需 blocks | 相对约 920 个可分配 blocks | admission 状态 |
| ---: | ---: | ---: | --- |
| 128 | 768 | -152 | 可以同时进入 |
| 160 | 960 | +40 | 至少 7 个请求等待 |
| 192 | 1152 | +232 | 至少 39 个请求等待 |

`max_batch_tokens_size=196608` 在这里不是限制：即使 C192 全部同时进入，输入 token
总数也只有 146496；`max_context_batch_size=192` 和 `concurrency_limit=192` 同样
没有在 C160 提前截断。当前首先触发的是 KV allocator 保留 blocks 后的 admission
容量。

#### 阶段证据

下表均为第 2-5 轮逐轮统计的中位数。`wait_time` 在 C++ `FIFOScheduler` 真正
admission 请求时记录，反映请求从开始到首次进入执行 batch 前的等待。

| 指标 | C128 | C160 | C192 |
| --- | ---: | ---: | ---: |
| QPS | 41.10 | 39.73 | 45.20 |
| admission wait p50 | 2.05 s | 2.84 s | 2.87 s |
| admission wait p95 | 3.34 s | 4.15 s | 3.81 s |
| 非等待部分 p95 | 0.70 s | 0.62 s | 0.62 s |
| ViT 请求 p50 | 1.14 s | 1.13 s | 1.10 s |
| ViT 请求 p95 | 1.67 s | 1.83 s | 1.58 s |
| ViT 平均实际 batch | 26.5 | 29.1 | 29.5 |
| ViT forward/图 | 21.7 ms | 21.4 ms | 18.9 ms |
| Prefill 平均 GPU 利用率 | 74.5% | 68.6% | 74.4% |

C128 到 C160 时，ViT 的实际 batch 变大且每图 forward 基本不变，非等待部分也没有
恶化；真正增加的是 admission 等待。C160 只比约 153 的同时 admission 容量多 7
个请求，KV 释放、FIFO admission、ViT 完成和 Prefill 执行之间容易形成短暂空洞，
Prefill 平均利用率也从 74.5% 降到 68.6%。C192 有约 39 个请求持续等待 admission，
反而能够重新填满 Prefill，利用率恢复到 74.4%，所以 QPS 高于 C128。该行为说明当前
是容量边界附近的调度和流水线填充问题，不是并发越高、kernel 越慢的单调退化。

#### 测量边界和后续验证

当前轮间波动较大，C128 和 C160 的 QPS 范围明显重叠，不能把 3.3% 的中位数差异
视为稳定回归。压测还使用了不同的请求波数：C128 为 384 个请求、约三轮并发，C160
为 320 个请求、约两轮并发，C192 为 384 个请求、约两轮并发；有限负载的填充和排空
开销没有等比例摊销。后续应在预热后按固定持续时间统计，并重点测试
C144/C152/C160/C176/C192，同时记录实时 KV free blocks、admitted streams、
Prefill batch 和队列长度。

若关注的是第 16 节 vLLM 数据，其 C160/C192 相对 C128 仅下降 2.0%/1.4%，Encoder
耗时随并发增加而 Prefill+Decode TTFB 基本稳定，属于约 40 QPS 饱和平台上的小幅
波动，不支持存在新的高并发性能断崖。
