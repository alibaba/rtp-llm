...
| A5. placement 证实健康 Prefill 容量、Decode KV／slot 或 Prefill 队列容量不足，分类结果为纯容量原因 | 8431 / RESOURCE_EXHAUSTED(3) | 5 / 429 / Throttling.ServiceOverloaded | 5 / 429 / Throttling.ResourceExhausted | 5 / 503 / ServiceUnavailable | 同 A1 的外部固定消息；有可信高优／同优或未知优先级归因时分别走 A1/A2/A9 |
...
| A12. 已取得 delivery claim 后到期 | 8431 / RESOURCE_EXHAUSTED(3) | 5 / 429 / Throttling.ServiceOverloaded | 5 / 429 / Throttling.ResourceExhausted | 5 / 503 / ServiceUnavailable | 仍读取 queue 已记录的等待原因；没有记录时明确未知，不按交付阶段覆盖为 8511 |
...
| A14d. Decode 抢占成功，后续 Prefill 选择失败 | 保留实际失败：容量 8431；无健康候选或 endpoint 已注销 8402；选择异常 8510 | 8431 同 A5；其余 5 / 503 | 8431 同 A5；其余 5 / 503 | 5 / 503 / ServiceUnavailable | 释放 incoming Decode reservation；容量失败使用标准消息，诊断写入 PV |
...
8431 标准 message 为 `admission capacity is temporarily exhausted`，决策诊断使用 `; trigger=…`，响应出口的补充信息使用 `; context=…`。8430/8432 保持固定 message，不能用重试诊断覆盖标准描述。`Response.error(...)` 统一校验 code/reason 并规范消息，调度链路直接传递失败响应；两个 scheduler 不再分别维护规范消息规则，也不再保留独立失败包装类。响应按请求创建，不共享可变实例。同一次失败沿调用链透传，不在终态发布时重新采集并覆盖已记录的诊断；真正发生下一次选路／入队决策时，才使用新决策产生的结果。
...
6. Schedule 已完成不发布第二个终态；到期取得终态处理权时读取 queue 状态并固定响应和 PV，异步 Cancel 确认只清理和发布，不重新读取已变化的 queue。delivery claim 不决定失败原因。
...
| 目标 group 内无健康 Prefill worker，或选中 endpoint 已注销 | 8402 MASTER_NO_PREFILL_WORKER | 5 | 503 / ServiceUnavailable | 路由诊断透传；健康但容量已满返回 8431 |
| 目标 group 内无健康 Decode worker | 8403 MASTER_NO_DECODE_WORKER | 5 | 503 / ServiceUnavailable | 路由诊断透传；容量拒绝按失败快照返回 8430/8431/8432，见第 4 节 |
...
| Batch/dispatch 或本地调度异常 | 8510 BATCH_DISPATCH_FAILED | 5 | 503 / ServiceUnavailable | 动态诊断透传 |
...
| `DefaultRouter` | 保留实际选路失败；按角色/group 收集失败诊断，Decode 容量失败时归因，并完成选路资源回滚 |
| `RouteService` | 只分发 Direct / Queue，不依赖 EndpointRegistry |
| `DecodeEndpoint.admissionSnapshot` | 按现有 admission version 懒生成不可变的优先级占用摘要；同版本复用，不复制请求明细 |
| `ClusterSnapshot.captureDecode` | 只在抢占规划时采集指定 group 的健康 Decode victim 明细，不组装错误或 PV |
| `PriorityAdmissionScheduler` | 使用 Router 的失败结果；仅在实际进入抢占规划时获取新的规划快照 |
| `AdmissionFailureClassifier` | 根据 Decode 容量拒绝时的快照解释优先级；水位拒绝或快照前容量已恢复保留 8431 |
...
| `PriorityScheduler.buildExpirationResponse` | 直接读取 queue 最近一次等待原因；首次超时决策固定响应与诊断，后续取消确认不重新归因 |
...
Master 转发结果未知或已被旧 Master 接收时保留终态码 8511，阻止 frontend 更换 request_id 重试；这不表示所有此类失败都是时间耗尽。8510 会触发 frontend 重试，不能用于这个分支。对本地调度异常，真实 TimeoutException（含 CompletionException / ExecutionException 包装）及 gRPC DEADLINE_EXCEEDED 映射为 8511，其余为 8510；诊断字符串不参与错误码选择。
...
| `prefill` | 始终为列表；队列拒绝、超时或交付失败时含单 worker 摘要：endpoint、queueDepth/capacity/version、ordering、higher/same/lower/unknownPriorityCount、readyCount、pendingCount、waitReason、inflightRequests/Batches |
| `prefill[].aheadCountUpperBound` | 前序请求数量的保守上界；不是精确排名。计数包含仍占位的当前请求和后到的同优请求，ready/pending 请求也计入上界 |
| `decode` | 始终为列表；关联 Decode 的 reservedRequests、totalLoad、engineLoad、kvAvailable/Total、hard/expectedKvReserved；地址缺失时省略 endpoint |
| `role` / `group` | 路由失败的角色与候选组；路由失败时 `prefill` / `decode` 为该组的 endpoint 计数摘要列表，包含健康状态、排队数量、engineLoad 和 KV 用量 |
...
Router 在选路失败后才收集同组 P/D 的标量摘要，PDFusion 使用其独立注册表。Decode 容量失败读取 endpoint 的不可变占用摘要，PV 复用同一份数值。同一 admission version 和 KV 总容量下重复失败复用摘要；首次读取或状态变更后，在已有 admission lock 内遍历账目重建固定 101 档的槽位／KV 汇总，不复制请求明细。排队转移也更新版本。只有实际进入抢占规划时才采集 victim 明细，提交时继续校验版本和所有权。成功路由不收集故障快照。过滤阶段直接记录已有 StrategyErrorType，诊断 Map 的字符串只用于日志，不参与错误码选择。
