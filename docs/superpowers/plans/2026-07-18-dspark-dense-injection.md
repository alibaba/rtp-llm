...
## 后续待办:DFlash/DSpark 拆分对齐 vLLM(push 前做)
现状:DFlash 与 DSpark 被合进一个 `Qwen3DSparkModel`(`qwen3_dspark.py`),
靠 `markov_rank` 开关退化——`markov_rank==0` 走 DFlash(stage D 直接 argmax),
`>0` 走 DSpark(Markov 低秩修正链)。两个模型类型都注册到同一个类
(`register_model("qwen_3_dflash", Qwen3DSpark, ...)` /
`register_model("qwen_3_dspark", Qwen3DSpark, ...)`),C++ 侧只有一个
`SP_TYPE_DSPARK`。stage A/B/C 逐字相同,差异只有 stage D 那几行。

问题两点:①命名藏血缘——`Qwen3DSparkModel` 同时服务 `qwen_3_dflash`,读代码
的人会懵;②DFlash 退化路径(`markov_rank=0`)没有独立 golden/E2E,我们跑绿
的 11 UT + 9 prompt E2E 全是 DSpark(ckpt `markov_rank=256`)。

上游 vLLM 的分层(见 `dspark_reference.py` 头部锚定):
- `vllm/.../qwen3_dflash.py` = backbone + fc + 特征 KV 注入(stage A/B/C),底座
- `vllm/.../qwen3_dspark.py` = Markov 头(stage D),在底座上扩展

**方向(user 拍板):push 前先重构对齐 vLLM 这个"DFlash 基座 + DSpark 扩展"的
分层,给个合适的命名,再 push。** 重构注意:①stage D 差异只有几行,拆继承要
权衡是否过度设计——最小改动可能是 DFlash 基类持 A/B/C + `propose` 骨架、
DSpark 子类只覆盖 stage D 与 `markov_correct`;②拆分后补一条 `markov_rank=0`
的 DFlash golden/E2E,补上现在缺的验证;③保持既有 11 UT + E2E gate 全绿、
dspark 输出与 1a 存档逐字一致(重构不应改数学)。

在这之前分支不 push、名字不定(候选 `feature/dspark-markov-draft` /
`feature/dflash-dspark` 待重构后再敲定)。
