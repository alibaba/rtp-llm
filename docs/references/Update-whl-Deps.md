...
Lock files are generated by `uv`. The recommended entry point is still the Bazel update target, e.g. `bazel run //deps:requirements_torch_gpu_cuda12_9.update`. When opening any dependency file starting with requirements_lock, you will find the exact `uv` generation command at the top of the file:
