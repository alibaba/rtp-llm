...
| `--use_new_loader` | Select the loader explicitly. When unset, registered models use NewLoader when the requested loading features are supported and otherwise fall back to the legacy loader; set true to require NewLoader or false to force rollback. (`USE_NEW_LOADER`) | Auto |
| `--keep_mla_checkpoint_weights` | Requires NewLoader. For DeepSeek MLA only: retain checkpoint weights after conversion to the runtime layout. This increases device memory usage and reduces available KV-cache blocks; it has no effect on other models and is intended for debugging only. (`KEEP_MLA_CHECKPOINT_WEIGHTS`) | False |
