...
| `--use_new_loader` | Select the loader explicitly. When unset, routing preserves legacy until `--require_weight_update false` confirms that online updates are not needed; compatible registered models then select NewLoader automatically. Set true to require NewLoader or false to force legacy. (`USE_NEW_LOADER`) | Undeclared (legacy) |
