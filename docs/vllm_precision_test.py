#!/usr/bin/env python3
"""vLLM precision test for GLM-5 4-layer model."""
import json
import os
import sys
...
MODEL_PATH = "/data1/zw193905/models/GLM-5-BF16-4layer"
OUTPUT_FILE = "/home/zw193905/RTP-LLM/github-opensource/docs/vllm_precision_output.json"
...
def main():
    print(f"Loading vLLM model from {MODEL_PATH}...", flush=True)
    llm = LLM(
        model=MODEL_PATH,
    ...
    # Test 2: Long prompt (~4k tokens)
    print("\n=== Test 2: Long prompt (~4k tokens) ===", flush=True)
    long_prompt = build_long_prompt(4096)
    input_ids = tokenizer.encode(long_prompt)
    print(f"  Input tokens: {len(input_ids)}", flush=True)
    long_params = SamplingParams(temperature=0.0, top_k=1, max_tokens=50)
    ...
    print("vLLM precision test COMPLETE", flush=True)
