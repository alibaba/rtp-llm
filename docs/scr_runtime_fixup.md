...
- Backend：准备模型、核心 GPU KV 和 executor → checkpoint → 初始化 memory/remote/P2P cache connector，随后启动 system prompt、Torch 预分配、engine loop、CacheStore 和 RPC/HTTP 服务。模板阶段不会申请 memory cache/remote cache 等外部缓存资源。
