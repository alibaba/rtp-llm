...
from maga_transformer.model_loader.weight_module import MMAtomicWeight
...
class MultiModalMixin:
    ...
    def load_mm_weight(self, ctype: str, tp_size: int, tp_rank: int, device: str):
        ...
        if os.environ.get("VIT_TRT", "0") == "1":
            self.init_mm_trt(self.config.ckpt_path,
                self.config.mm_related_params,
                tp_size, tp_rank, device)
            return 
