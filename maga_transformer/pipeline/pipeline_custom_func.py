...
def get_custom_func(model: BaseModel, func_name: str, default_func: Callable[..., Any]) -> Callable[..., Any]:
    ...
    if isinstance(model, AsyncModel):
        if getattr(model.model, func_name, None) is not None:
            logging.info(f"using {func_name} implement in model")
            return getattr(model.model, func_name)
    elif getattr(model, func_name, None) is not None:
        logging.info(f"using {func_name} implement in model")
...
def get_piple_custom_func(model):
    funcs = PipelineCustomFunc()
    funcs.modify_prompt_func = get_custom_func(model, "modify_prompt_plugin", DefaultPlugin.modify_prompt_func)
    funcs.multimodal_modify_prompt_func = get_custom_func(model, "multimodal_modify_prompt_plugin", DefaultPlugin.multimodal_modify_prompt_func)
    funcs.process_encode_func = get_custom_func(model, "process_encode_plugin", DefaultPlugin.process_encode_func)
    funcs.process_decode_func = get_custom_func(model, "process_decode_plugin", DefaultPlugin.tokenids_decode_func)
    funcs.modify_response_func = get_custom_func(model, "modify_response_plugin", DefaultPlugin.modify_response_func)
    funcs.stop_generate_func = get_custom_func(model, "stop_generate_plugin", DefaultPlugin.stop_generate_func)
    return funcs