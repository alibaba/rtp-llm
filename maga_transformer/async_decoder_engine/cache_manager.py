...
class CacheManager:
    ...
    def __init_free_block(self, config: CacheConfig, nccl_op: Any):
        ...
        if g_parallel_info.tp_size > 1:
            block_nums_t = torch.tensor([block_nums], dtype=torch.int32, device=self.device)
    ...
    def __init_kv_cache(self, config: CacheConfig):
        ...
        self.k_blocks = torch.zeros((config.layer_num, self.block_nums, config.local_head_num_kv,
                                config.seq_size_per_block, config.size_per_head), dtype=config.dtype, device=self.device)
        self.v_blocks = torch.zeros((config.layer_num, self.block_nums, config.local_head_num_kv, config.seq_size_per_block, config.size_per_head), dtype=config.dtype, device=self.device)
        if config.dtype is torch.int8:
            self.k_scale = torch.zeros((config.layer_num, self.block_nums, config.local_head_num_kv,
                                config.seq_size_per_block), dtype=torch.float32, device=self.device)
            self.v_scale = torch.zeros((config.layer_num, self.block_nums, config.local_head_num_kv, config.seq_size_per_block), dtype=torch.float32, device=self.device)
        else:
            self.k_scale = None
            self.v_scale = None
