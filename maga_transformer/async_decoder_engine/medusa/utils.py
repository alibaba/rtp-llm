...
from maga_transformer.async_decoder_engine.medusa.medusa_config import MedusaBuffer

def pad_path(path: List[int], length: int, pad_value: int=-2):
    """
...
def generate_medusa_buffers(medusa_choices: List[List[int]], top_k: int):
    """
    ...
    medusa_choices_new: List[List[int]] = []
    for value in medusa_choices:
        if len(value)<=3:
            medusa_choices_new.append(value)
    sorted_medusa_choices = sorted(medusa_choices_new, key=lambda x: (len(x), x))
    ...
    for i in range(len(depth_counts)):
        for j in range(depth_counts[i]):
            cur_medusa_choice = sorted_medusa_choices[start + j]
            medusa_tree_indices[start + j + 1] = cur_medusa_choice[-1] + top_k * i + 1
        start += depth_counts[i]
...
def generate_candidates(medusa_logits, logits, tree_indices, retrieve_indices, top_k):
    """
    ...
    candidates_medusa_logits = torch.topk(medusa_logits[:, 0, -1], top_k, dim = -1).indices
