...
{                                                                                               \
    auto request_cost_time_ms =                                                                 \
        (currentTimeUs() - generate_context.request_begin_time_us) / 1000;                      \
    if (generate_context.request_timeout_ms > 0 &&                                              \
        request_cost_time_ms >= generate_context.request_timeout_ms) {                          \
        generate_context.error_info = ErrorInfo(ErrorCode::GENERATE_TIMEOUT,                    \
                "request cost time is " + std::to_string(request_cost_time_ms)                  \
