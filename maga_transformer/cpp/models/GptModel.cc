...
void GptModelInputs::tpSync(ft::NcclParam tensor_para, ft::NcclParam pipeline_para, ft::DeviceBase* device) {
    ...
    shape_hints_ptr[0] = combo_tokens.get() ? combo_tokens->size() : 0; // combo_token_size
    shape_hints_ptr[1] = combo_tokens.get() ? input_lengths->size() : 0; // total_batch_size
    shape_hints_ptr[2] = combo_tokens.get() ? sequence_lengths->size() : 0; // generate_batch_size
    shape_hints_ptr[3] = combo_tokens.get() ? kv_cache_blocks->shape()[0] : 0; // layer_num
    shape_hints_ptr[4] = combo_tokens.get() ? kv_cache_blocks->shape()[3] : 0; // block_size
    shape_hints_ptr[5] = kv_cache_scales.get() != nullptr; // use_block_scale
    ...
    if (tensor_para.rank_) {
        combo_tokens = device->allocateBuffer({ft::DataType::TYPE_INT32, {(size_t)shape_hints_ptr[0]}, ft::AllocationType::HOST});
        input_lengths = device->allocateBuffer({ft::DataType::TYPE_INT32, {(size_t)shape_hints_ptr[1]}, ft::AllocationType::HOST});
        sequence_lengths = device->allocateBuffer({ft::DataType::TYPE_INT32, {(size_t)shape_hints_ptr[2]}, ft::AllocationType::HOST});
        kv_cache_blocks = device->allocateBuffer({ft::DataType::TYPE_UINT64, {(size_t)shape_hints_ptr[3], (size_t)shape_hints_ptr[1], 2, (size_t)shape_hints_ptr[4]}, ft::AllocationType::HOST});
        if (shape_hints_ptr[5]) {
            kv_cache_scales = device->allocateBuffer({ft::DataType::TYPE_INT32, {(size_t)shape_hints_ptr[3], (size_t)shape_hints_ptr[0], 2, (size_t)shape_hints_ptr[4]}, ft::AllocationType::HOST});
        }
    }
    std::vector<ft::Buffer *> buffers;
    buffers.emplace_back(combo_tokens.get());
    buffers.emplace_back(input_lengths.get());
    buffers.emplace_back(sequence_lengths.get());
    buffers.emplace_back(kv_cache_blocks.get());
    if (shape_hints_ptr[5]) {
        buffers.emplace_back(kv_cache_scales.get());
    }
    ft::ftNcclGroupStart();
