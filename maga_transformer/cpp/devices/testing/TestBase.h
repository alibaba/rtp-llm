...
protected:
    ...
    rtp_llm::BufferPtr createBuffer(const std::vector<size_t>& shape, rtp_llm::DataType type,
    ...
    template <typename T>
    rtp_llm::BufferPtr createBuffer(const std::vector<size_t>& shape, const std::vector<T>& data,
                               rtp_llm::AllocationType alloc_type = rtp_llm::AllocationType::DEVICE)
    {
    ...
    rtp_llm::BufferPtr createDeviceBuffer(const std::vector<size_t>& shape, const void* data) {
        ...
        auto buffer = device_->allocateBuffer({rtp_llm::getTensorType<T>(), shape, rtp_llm::AllocationType::DEVICE}, {});
        if (data && (buffer->size() > 0)) {
    ...
    void assertBufferValueEqual(const rtp_llm::Buffer& buffer, const std::vector<T>& expected) {
        ...
        auto comp_buffer = device_->allocateBuffer(
            {buffer.type(), buffer.shape(), rtp_llm::AllocationType::HOST}
        );
                                 ...
                                 rtp_llm::AllocationType alloc_type = rtp_llm::AllocationType::DEVICE)
    {
        ...
        if (alloc_type == rtp_llm::AllocationType::DEVICE) {
            auto device_buffer = device_->allocateBuffer(
                    {buffer->type(), buffer->shape(), rtp_llm::AllocationType::DEVICE}
            );
