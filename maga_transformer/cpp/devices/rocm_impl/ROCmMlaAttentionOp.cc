...
#include "maga_transformer/cpp/kernels/mla_kernels_rocm/page_hip.h"
#include "maga_transformer/cpp/kernels/mla_kernels_rocm/rope_hip.h"
...
void ROCmDevice::mlaRotaryWriteKVCache(const MlaRotaryWriteKVCacheParams& params) {
    ...
    apply_rope_pos_ids_cos_sin_cache(q_rope_t, k_rope_t.unsqueeze(1), dest_q, k_rope_t.unsqueeze(1), cos_sin_cache_t, f.positions_t, false, (int64_t)stream_);
    auto append_ckv_t
...
void ROCmDevice::DecoderOutputGemmWrapper(torch::Tensor& qkv_output_t, const torch::Tensor& mla_out_t, const MlaAttentionModuleParams& params) {
...
void ROCmDevice::mlaDecoderSelfAttention(const MlaAttentionModuleParams& params) {
    ...
    int max_seq_len = *((kv_indptr.slice(0, 1, batch_size + 1, 1) - kv_indptr.slice(0, 0, batch_size, 1)).max().data_ptr<int>());
    ...
    for (int i = 0; i < batch_size; i++) {
        ...
        k_vector.push_back(F::pad(tmp_t, F::PadFuncOptions({0, 0, 0, max_seq_len - tmp_t.size(1)}).mode(torch::kConstant)));
    ...
    auto before_softmax = torch::bmm(fused_q_input_t.to(torch::kFloat), k.transpose(-1, -2).to(torch::kFloat)) * softmax_scale;
