...
struct LoraWeights {
    ...
    LoraWeights(ConstBufferPtr lora_a, ConstBufferPtr lora_b) {
        RTP_LLM_CHECK_WITH_INFO((lora_a != nullptr && lora_b != nullptr),
            "lora lora_a and lora b cannot be empty.");
        RTP_LLM_CHECK_WITH_INFO((lora_a->dim() == lora_b->dim()),
            "lora lora_a and lora b need have same dim.");
        RTP_LLM_CHECK_WITH_INFO((lora_a->dim() >= 2),
        ...
        RTP_LLM_CHECK_WITH_INFO((lora_a->type() == lora_b->type()),
            "lora lora_a and lora b need have same type.");
        RTP_LLM_CHECK_WITH_INFO((lora_a->where() == lora_b->where()),
...
struct LoraModel {
    ...
    LoraModel(loraLayerWeightsMap lora_a, loraLayerWeightsMap lora_b) {
        RTP_LLM_CHECK_WITH_INFO((lora_a.size() == lora_b.size()),
...
struct LoraOpInput {
    ...
    LoraOpInput(BufferPtr lora_input_length,
                ...
                bool use_same_lora = false)
    {
        RTP_LLM_CHECK_WITH_INFO((lora_input_length->dim() == 1),
...
struct LoraModelInput {
    ...
    LoraModelInput(BufferPtr lora_input_lengths,
                   ...
                   bool use_same_lora = false) {
        RTP_LLM_CHECK_WITH_INFO((lora_input_lengths != nullptr),
            "lora_input_lengths can not be empty");

        RTP_LLM_CHECK_WITH_INFO((lora_input_lengths->dim() == 1),
