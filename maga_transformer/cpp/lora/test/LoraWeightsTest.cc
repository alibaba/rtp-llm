...
TEST_F(LoraWeightsTest, testLoraWeightsConstruct) {
    ...
    EXPECT_ANY_THROW(ft::lora::LoraWeights(lora_a_buffer_ptr, lora_b_buffer_ptr));
    // lora_a, nullptr
    ...
    lora_a_buffer_ptr = tensorToBuffer(lora_a);
    lora_b_buffer_ptr = tensorToBuffer(lora_b);
    EXPECT_NO_THROW(ft::lora::LoraWeights(lora_a_buffer_ptr, lora_b_buffer_ptr));
    auto result = ft::lora::LoraWeights(lora_a_buffer_ptr, lora_b_buffer_ptr);
    ...
    lora_a = torch::rand({10, 8});
    lora_b = torch::rand({8, 10});
    lora_a_buffer_ptr = tensorToBuffer(lora_a, ft::AllocationType::HOST);
    lora_b_buffer_ptr = tensorToBuffer(lora_b);
    EXPECT_ANY_THROW(ft::lora::LoraWeights(lora_a_buffer_ptr, lora_b_buffer_ptr));
