...
void ChatService::generateStreamingResponse(const std::shared_ptr<GenerateConfig>& config,
    ...
    while (!stream->finished()) {
        ...
        if (index == 0) {
            std::string json_response = ctx->render_stream_response_first(num_return_sequences);
