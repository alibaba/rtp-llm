...
from maga_transformer.cpp.eplb.test.libth_eplb_py_wrapper_test import EplbPyWrapperOP
...
class FakeResult:
    def __init__(self, config: TestMoeConfig):
        ...
        self.logic_expert_cnt = torch.randint(0, config.layer_num, (config.log_exp_num,), dtype=torch.int32)
        max_exp_num = config.phy_exp_num - config.log_exp_num + 1
        exp_per_ep = config.phy_exp_num // config.ep_size
        self.log2phy = torch.randint(0, config.phy_exp_num, (config.log_exp_num, max_exp_num), dtype=torch.int32)
        self.phy2log = torch.randint(0, config.log_exp_num, (config.phy_exp_num,), dtype=torch.int32)
...
class FakeExpertBalancer:
    ...
    def load_moe_weight(
        ...
        return layer_id, self.res.moe_w1, self.res.moe_w2, self.res.moe_w1_s, self.res.moe_w2_s
...
class TestEplbPyWrapper(unittest.TestCase):
    def __init__(self, methodName: str = "runTest") -> None:
        ...
        sys.path.append(
            ...
            + "/maga_transformer/maga_transformer/cpp/eplb/test"
        )
