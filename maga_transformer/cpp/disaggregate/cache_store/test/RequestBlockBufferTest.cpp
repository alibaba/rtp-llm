...
TEST_F(RequestBlockBufferTest, testWatchFunc_SetWatchFunc) {
    {
        ...
        // set twice
        auto watch_func2 = [](bool success, const std::vector<std::shared_ptr<BlockBuffer>>& blocks) {};
        ASSERT_FALSE(request_block_buffer->setWatchFunc(std::move(watch_func2)));
...
TEST_F(RequestBlockBufferTest, testWatchFunc_AddBlock) {
    ...
    ASSERT_TRUE(watched_success);
    ASSERT_EQ(1, watched_blocks.size());
    ASSERT_EQ("b2", watched_blocks[0]->key);
    ...
    ASSERT_TRUE(watched_success);
    ASSERT_EQ(2, watched_blocks.size());
    ASSERT_EQ("b3", watched_blocks[0]->key);
    ASSERT_EQ("b4", watched_blocks[1]->key);
