...
AUTIL_LOG_SETUP(rtp_llm, MessagerServer);
...
bool MessagerServer::init(uint32_t listen_port, bool enable_metric) {
    if (!initTcpServer(listen_port, enable_metric)) {
        AUTIL_LOG(INFO, "messager server init failed, tcp server init failed");
        return false;
    }
    return true;
}
...
bool MessagerServer::initTcpServer(uint32_t listen_port, bool enable_metric) {
    rpc_service_ = std::move(createCacheStoreServiceImpl(memory_util_, request_block_buffer_store_, metrics_reporter_));
    ...
    rpc_server_->RegisterService(rpc_service_.get());

    std::string listen_spec = "tcp:0.0.0.0:" + std::to_string(listen_port);
    if (!rpc_server_->Listen(listen_spec)) {
        AUTIL_LOG(WARN, "messager server init failed, listen %s failed", listen_spec.c_str());
        return false;
    }

    AUTIL_LOG(INFO, "messager server init success, listen on %s", listen_spec.c_str());
    return true;
}
