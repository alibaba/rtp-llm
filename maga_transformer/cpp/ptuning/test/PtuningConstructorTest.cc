...
TEST_F(PtuningConstructorTest, testPtuningConstruct) {
    CustomConfig config;
    auto engine = createMockEngine(device_, config); 
    auto cache_config = engine->resourceContext().cache_manager->cacheConfig();
    ...
    PtuningConstructor constructor;
    auto result = constructor.createPtuningV2(*params.gpt_init_parameter, engine->resourceContext().cache_manager.get(), prefix_prompt);
