#include "maga_transformer/cpp/openai/ApiDataType.h"
...
bool RoleEnum::contains(const std::string&  role) {
    ...
    if (role == RoleEnum::assistant) return true;
    if (role == RoleEnum::system) return true;
    if (role == RoleEnum::function) return true;
    if (role == RoleEnum::tool) return true;
    if (role == RoleEnum::observation) return true;
    return false;
}

bool ContentPartTypeEnum::contains(const std::string& type) {
    if (type == ContentPartTypeEnum::text) return true;
    if (type == ContentPartTypeEnum::image_url) return true;
    if (type == ContentPartTypeEnum::video_url) return true;
    if (type == ContentPartTypeEnum::audio_url) return true;
    return false;
}
