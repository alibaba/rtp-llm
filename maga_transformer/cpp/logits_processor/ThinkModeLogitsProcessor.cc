...
void ThinkModeLogitsProcessor::process(const SamplerInputs& inputs, size_t start_idx, size_t finish_idx) {
    ...
    for (size_t i = 0; i < size(); ++i) {
        ...
        if (!info.in_think_mode) continue;
        ...
        bool enforce = (sequence_lengths[i + start_idx] + num_new_tokens >= info.max_thinking_tokens + input_lengths[i + start_idx]);
        setVocabMask(info.dfa_ptr, inputs.logits->index(i + start_idx), num_new_tokens, info.end_think_token_ids, inputs.vocab_size, enforce);
    }
}
...
void ThinkModeLogitsProcessor::updateLogitProcessorStatus(const rtp_llm::BufferPtr& new_tokens, int32_t num_new_tokens) {
...
ThinkModeLogitsProcessorPtr ThinkModeLogitsProcessor::fromGenerateInput(
    rtp_llm::DeviceBase* device, std::shared_ptr<GenerateInput> generate_input, int32_t num)
{
    ...
    for (size_t i = 0; i < num; i++) {
        StreamThinkInfo think_info(generate_input->generate_config->in_think_mode,
