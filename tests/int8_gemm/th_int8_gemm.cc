    ...
    const int64_t timing_iterations, float& avg_time, bool select_config)
{
    ...
    if (select_config)
    {
        ...
        std::vector<tc::CutlassGemmConfig> configs = runner->getValidConfigs(input_act_ptr, weight_ptr, quant_mode,
            alpha_col_ptr, alpha_row_ptr, output_tensor_ptr, m, n, k, ws_ptr, ws_bytes, stream);
    ...
    {
        tc::CutlassGemmConfig config = runner->getChosenConfig(input_act_ptr, weight_ptr, quant_mode, alpha_col_ptr,
            alpha_row_ptr, output_tensor_ptr, m, n, k, ws_ptr, ws_bytes, stream);
