...
class FlashYaRNRotaryEmbedding(torch.nn.Module):
    ...
    def forward(self, q: torch.Tensor, seqlen_offset: int = 0) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        ...
        token in the batch.
        """
        self._update_cos_sin_cache(q.shape[2] + seqlen_offset, device=q.device, dtype=q.dtype)
