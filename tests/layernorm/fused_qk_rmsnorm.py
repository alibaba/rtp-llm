...
class TestLayerNorm(unittest.TestCase):
    ...
    def test_fused_qk_rmsnorm(self):
        ...
        for batch_size in [2, 32, 4096, 512]:
            for q_head, kvhead, hidden_units in [(1, 1, 384),(64, 4, 9216), (32, 2, 4608), (16, 1, 2304)]:
                ...
                normed_q = q_rms(input_ref.reshape(batch_size, -1, norm_size)[:, :q_head, :].contiguous())

                normed_k = k_rms(input_ref.reshape(batch_size, -1, norm_size)[:, q_head:q_head+kvhead, :].contiguous())

                fused_qk_rmsnorm = torch.classes.unittest.FusedQkRmsNormOp(eps)
                fused_qk_rmsnorm.forward(input, q_gamma, None, k_gamma, None, q_head, kvhead, norm_size)

                fused_q_norm = input.reshape(batch_size, -1, norm_size)[:, :q_head, :]
                fused_k_norm = input.reshape(batch_size, -1, norm_size)[:, q_head:q_head+kvhead, :]
