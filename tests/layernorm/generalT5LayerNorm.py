...
class TestT5LayerNorm(unittest.TestCase):
    ...
    def test_stride_norm(self):
        ...
        for batch_size in [1, 2, 12, 192]:
            for (hidden_units_1, hidden_units_2) in [(128, 128), (1024, 256), (4096, 512), (1024, 128)]:
                size_per_head = 128
                ...
                expect_result_1 = self.T5LayerNorm1(hidden_states_1.reshape([batch_size, -1, size_per_head])).reshape([batch_size, -1]).cpu()
                expect_result_2 = self.T5LayerNorm2(hidden_states_2.reshape([batch_size, -1, size_per_head])).reshape([batch_size, -1]).cpu()

                stride = hidden_units_1 + hidden_units_2*2
                torch.set_printoptions(threshold=float('inf'))

                actual_1 = self.T5LayerNormOp.stride_forward(hidden_states, self.gamm1,
                                                           hidden_units_1, 0, stride).reshape([batch_size, -1]).cpu()
                actual_2 = self.T5LayerNormOp.stride_forward(hidden_states, self.gamm2,
