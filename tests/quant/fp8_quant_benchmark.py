from time import time

import torch
import torch.nn.functional as F

from rtp_llm.models_py.kernels import QuantizeFp8Tensor, atex_minmax_pertensor_quant_fp8

M = [1, 3, 9, 15, 32, 256, 512, 1039]
N = [256, 512, 768, 1024, 1536, 2048, 4096, 8192, 16384]
ITER = 100


def tensor_size(tensor: torch.Tensor) -> int:
    return tensor.numel() * tensor.element_size()


f = open("/root/projects/rtp-llm/quant.csv", "w")
for m in M:
    for n in N:
        x = torch.randn(size=[m, n], device="cuda", dtype=torch.float16)
        torch.cuda.synchronize()

        tik = time()
        for iter in range(ITER):
            o: QuantizeFp8Tensor = atex_minmax_pertensor_quant_fp8(x)
        torch.cuda.synchronize()
        tok = time()

        bandwidth = (
            tensor_size(x)
            + tensor_size(o.quantized_scale)
            + tensor_size(o.quantized_value)
        )
        bandwidth = bandwidth / 1024 / 1024 / 1024 * ITER
        bandwidth = bandwidth / (tok - tik)
        print(f"FP8 Quant Benchmark: M={m}, N={n}, bandwidth={bandwidth:.2f} GB/s")
        f.write(f"{m},{n},{bandwidth}\n")

for m in M:
    for n in N:
        x = torch.randn(size=[m, n], device="cuda", dtype=torch.bfloat16)
        torch.cuda.synchronize()

        tik = time()
        for iter in range(ITER):
            o: QuantizeFp8Tensor = atex_minmax_pertensor_quant_fp8(x)
        torch.cuda.synchronize()
        tok = time()

        bandwidth = (
            tensor_size(x)
            + tensor_size(o.quantized_scale)
            + tensor_size(o.quantized_value)
        )
        bandwidth = bandwidth / 1024 / 1024 / 1024 * ITER
        bandwidth = bandwidth / (tok - tik)
        print(f"FP8 Quant Benchmark: M={m}, N={n}, bandwidth={bandwidth:.2f} GB/s")
        f.write(f"{m},{n},{bandwidth}\n")
