...
def per_token_cast_to_fp8(x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    return (x_view * (240.0 / x_amax.unsqueeze(2))).to(torch.float8_e4m3fnuz).view(m, n), (x_amax / 240.0).view(m, -1).to(torch.float32)
...
def per_block_cast_to_fp8(x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    return x_scaled.view_as(x_padded)[:m, :n].contiguous(), (x_amax / 240.0).view(x_view.size(0), x_view.size(2)).to(torch.float32)
...
def dequantize_per_block(fp8_tensor: torch.Tensor, scale: torch.Tensor) -> torch.Tensor:
    ...
    fp8_padded = torch.zeros(padded_shape, dtype=torch.float8_e4m3fnuz, device=fp8_tensor.device)
