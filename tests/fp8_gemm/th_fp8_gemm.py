...
class TestFp8Gemm(unittest.TestCase):
    ...
    def quantize_weight_to_fp8(weight: torch.Tensor):
        ...
        # 量化操作
        quantized_weight = (weight / scaling_factor).to(torch.float8_e4m3fn).view(torch.int8)
        return quantized_weight, scaling_factor

    @staticmethod
    def dequantize_weight_from_fp8(quantized_weight: torch.Tensor, scaling_factor: float):
        # 反量化操作
        dequantized_weight = (quantized_weight.view(torch.float8_e4m3fn).to(torch.float32) * scaling_factor).to(torch.float32)

        return dequantized_weight

    
    def gt_fp8_gem(self, mat1_quant, mat2_quant, scale_a_torch, scale_b_torch):
        mat_res = torch.mm(mat1_quant.view(torch.float8_e4m3fn).to(torch.float32), mat2_quant.view(torch.float8_e4m3fn).to(torch.float32))
    ...
    def fp8_gemm_helper(self, m, n, k):
        ...
        ref = self.gt_fp8_gem(mat1_quant,
                              mat2_quant,
                              scale_a_torch,
                              scale_b_torch)
        ...
        msg = f"fp8 quant gemm Failed on m={m}, n={n}, k={k}, output={output}, ref={ref}"
