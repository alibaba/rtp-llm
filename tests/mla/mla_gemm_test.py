    ...
    def test_mla_qkv_gemm(self):
        ...
        for batch_size in batch_sizes:
            for seq_len in seq_lens:
                ...
                q_b_weight = torch.randn(head_num * (nope_dim + rope_dim), config.q_lora_rank)
                kv_a_weight = torch.randn(config.kv_lora_rank + rope_dim, config.hidden_size)
                kv_b_weight = torch.randn(head_num * (nope_dim + vhead_dim), config.kv_lora_rank)
                kv_a_layernorm_weight = torch.randn(config.kv_lora_rank)
                q_a_layernorm_weight = torch.randn(config.q_lora_rank)

                gemm.set_weight(q_a_weight, q_b_weight, kv_a_weight, kv_b_weight, kv_a_layernorm_weight, q_a_layernorm_weight)

                hidden_states = torch.randn(batch_size, seq_len, hidden_size).contiguous().cuda()
                gemm.cuda()
                ...
                kv_a, k_rope = torch.split(
                    kv_a_weight.transpose(0, 1), [config.kv_lora_rank, config.qk_rope_head_dim], dim=-1
                )
                kv_a = kv_a.contiguous().cuda()
                k_rope = k_rope.contiguous().cuda()

                k_nope, v = torch.split(
                    kv_b_weight.transpose(0, 1).reshape(kv_lora, head_num, nope_dim + vhead_dim), [nope_dim, vhead_dim], dim=-1
                )

                k_nope = k_nope.reshape(kv_lora, head_num * nope_dim).contiguous().cuda()
