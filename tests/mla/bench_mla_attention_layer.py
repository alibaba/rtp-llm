...
class TestMlaAttentionLayer(unittest.TestCase):
    ...
    def _test_one_case(self, sequence_lengths: List[int], head_num: int, page_size: int):
        ...
        hidden = torch.randn([total_token_num, self.config.hidden_size], dtype=torch.bfloat16, device=torch.device("cuda"))
        
        # 构造权重列表
        weights = [
            # 0. kc_t_weight
            torch.randn([self.config.head_num, self.config.nope_head_size, self.config.kv_lora], dtype=torch.bfloat16, device=torch.device("cuda")),
            # 1, vc_t_weight
            torch.randn([self.config.head_num, self.config.kv_lora, self.config.v_head_size], dtype=torch.bfloat16, device=torch.device("cuda")),
        ...
        for seq_len in [4096, 8192, 16384]:
            for batch_size in [128, 256, 512]:
                for tp_size in [1, 2, 4]:
                    ...
                    with torch.cuda.nvtx.range(f"b_{batch_size}_s_{seq_len}_tp_{tp_size}_lbs_{local_batch_size}"):
                        self._test_one_case([seq_len] * local_batch_size, local_head_num, 64)    
