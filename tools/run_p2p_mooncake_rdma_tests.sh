...
MODEL_ALIAS_PATH="${MODEL_ALIAS_PATH:-/mnt/nas1/hf/Qwen2.5-0.5B-Instruct}"
...
host_precheck() {
  ...
  if [[ -n "$MODEL_SOURCE_PATH" ]]; then
    [[ -d "$MODEL_SOURCE_PATH" ]] || die "不存在模型源目录: $MODEL_SOURCE_PATH"
  else
...
start_container() {
  ...
  local -a model_mount_args=()
  if [[ -n "$MODEL_ROOT" && "$MODEL_ROOT" != "$HOST_WORK_ROOT" ]]; then
    model_mount_args=(-v "$MODEL_ROOT":"$MODEL_ROOT")
  fi

  docker run -d --name "$CONTAINER_NAME"     --privileged     --network host     --ipc host     --gpus all     --ulimit memlock=-1     --ulimit nofile=655350:655350     -v "$HOST_WORK_ROOT":"$HOST_WORK_ROOT"     "${model_mount_args[@]}"     -v /dev/infiniband:/dev/infiniband     -v /sys/class/infiniband:/sys/class/infiniband     -v /sys/class/net:/sys/class/net     -v "$BAZEL_CACHE_DIR":/root/.cache/bazel     -w "$OPEN_SOURCE_REPO"     "$IMAGE"     bash -lc 'sleep infinity' >/dev/null
}
...
prepare_model_alias() {
  ...
  local alias_parent
  alias_parent="$(dirname "$MODEL_ALIAS_PATH")"
  log "配置模型别名: $MODEL_SOURCE_PATH -> $MODEL_ALIAS_PATH"
  docker exec "$CONTAINER_NAME" bash -lc "mkdir -p '$alias_parent' && ln -sfn '$MODEL_SOURCE_PATH' '$MODEL_ALIAS_PATH'"
  docker exec "$CONTAINER_NAME" bash -lc "ls -l '$MODEL_ALIAS_PATH/config.json'"
}
...
usage() {
  cat <<USAGE
用法:
  $(basename "$0") [test|smoke-tcp|smoke-rdma|all]

环境变量:
  BRANCH                     默认: develop/vin/p2p-connector-3
  WORKSPACE_ROOT             默认: /data0/qiongshi.gb/RTP-LLM
  HOST_WORK_ROOT             默认: WORKSPACE_ROOT 的父目录
  MODEL_ROOT                 默认: /mnt/nas1；为空或与 HOST_WORK_ROOT 相同时不会额外挂载
  MODEL_SOURCE_PATH          可选：自定义模型源目录，例如 /ssd/1/vin.gb/qwen2.5-0.5b
  MODEL_ALIAS_PATH           默认: /mnt/nas1/hf/Qwen2.5-0.5B-Instruct
