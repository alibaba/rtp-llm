...
def check_answer(text: str) -> None:
    ...
    try:
        actual = json.loads(answer, object_pairs_hook=unique_keys)
...
def check_prefix_reuse(reuse: int, common: int, reuse_unit: int) -> None:
    ...
    require(
        reuse <= common, "cache reuse exceeds the seed's actual common token prefix"
    )
    ...
    require(
        reuse >= minimum, f"long prefix hit reused too little history: {reuse}/{common}"
    )
...
def expanded_bytes_per_token(checkpoint: pathlib.Path | None, tp_size: int) -> int:
    ...
    if checkpoint is not None:
        ...
        config.update(checkpoint_config.get("text_config", checkpoint_config))
...
class LongPrefixCase:
    def __init__(
        ...
        *,
        ...
        require(
            self.reuse_unit_tokens % page_size == 0,
            "reuse unit must contain whole physical pages",
        )
    ...
    def request(
        ...
        # Persist the exact wire response (including malformed JSON/HTTP errors).
        request = urllib.request.Request(
        ...
        try:
            ...
            raw = exc.read()
            (self.output / f"{name}-response.raw").write_bytes(raw)
            save(self.output / f"{name}-http-error.json", {"status": exc.code})
            raise
        (self.output / f"{name}-response.raw").write_bytes(raw)
        ...
        if selected is not None:
            require(
                selected in record["observed_decode_role_addrs"],
    ...
    def run(self) -> dict:
        ...
        try:
            ...
            require(
                self.target_tokens - 200 <= len(seed_ids) <= self.target_tokens + 100,
