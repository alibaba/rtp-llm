...
def test_throughput(
    prompts: List[Tuple[str, int]], max_concurrent_num: int, **kwargs: Any
):
    query_time = 0.0
    total_time = 0.0
    exception = None
        ...
        try:
            ...
            for prompt, output_len in prompts:
                ...
                new_generate_config.update(
                    {"max_new_tokens": output_len, "min_new_tokens": output_len}
                )
...
def wait_server_ready(proc):
    ...
    while proc.is_alive():
        try:
            ...
            if response.status_code == 200 and response.json().get("status") == "home":
...
def run_test(
    ...
    prompts = [
        (get_prompt(tokenizer, origin_prompt, input_len), output_len)
...
def report_throughput_test_res(
    model_type: str,
    model_size: float,
    prec: str,
    device: str,
    ...
    table_name = os.environ.get("THROUGHPUT_ODPS_TABLE", "perf_test_throughput")
    fields = [
        "model_type",
        "model_size",
        "weight_type",
