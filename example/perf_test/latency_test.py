...
def run_latency_test(
    model_args: Dict[str, Any], test_args: Dict[str, Any], pipeline, tokenizer
):
    state = True

    test_batchsize = test_args["test_batchsize"]
    test_input_len = test_args["test_input_len"]
    prompts = {x: get_prompt(tokenizer, origin_prompt, x) for x in set(test_input_len)}
    res = []
...
def run_test(model_args: Dict[str, Any], test_args: Dict[str, Any]):
    ...
    for i in range(1, tp_size):
        ...
        proc = multiprocessing.Process(target=pipeline_proc, args=(model_args, barrier))
        proc.start()
        procs.append(proc)
...
def report_latency_test_res(
    model_type, model_size, prec, device, framework, results: List[LatencyTestResult]
):
    lantency_records = []
    for res in results:
...
if __name__ == "__main__":
    ...
    while True:
        ...
        report_latency_test_res(
            model_type, args.model_size, args.prec, device_name, "rtp_llm", lantency_res
        )
