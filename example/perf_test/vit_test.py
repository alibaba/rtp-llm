...
class TestCase:
    ...
    def get_args(self):
        return {
            "model_args": {"model_type": self.model_type, "ckpt_path": self.ckpt_path},
            "test_args": {"image_size": self.image_size},
        }


def create_model(test_config: TestCase):
    model_config = ModelConfig(
        test_config.model_type, test_config.ckpt_path, test_config.ckpt_path
    )
...
def run_test(testcase: TestCase):
    ...
    for image in testcase.images:
        with tempfile.NamedTemporaryFile(suffix=".png") as temp_file:
            ...
            model.mm_part.mm_embedding(
                temp_file.name, MMUrlType.IMAGE, configs=MMPreprocessConfig()
            )
            end_time = time.time()
...
if __name__ == "__main__":
    ...
    t_case = TestCase(
        args.model_type, args.ckpt_path, [int(x) for x in args.image_size.split(",")]
    )
