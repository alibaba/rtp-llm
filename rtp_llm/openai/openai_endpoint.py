...
class OpenaiEndpoint(object):
    ...
    async def _collect_complete_response(
        ...
        if debug_info is not None and extra_outputs is not None and extra_outputs.output_ids is not None:
            debug_info.output_ids = extra_outputs.output_ids
            debug_info.raw_output = [
                self.tokenizer.decode(output_ids) for output_ids in extra_outputs.output_ids
            ]
