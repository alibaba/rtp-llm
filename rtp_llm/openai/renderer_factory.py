    ...
    def get_renderer(
        ...
        if model_template_type:
            ...
            if renderer_instance:
                pass  # Fall through to the end for potential wrapping
                ...
                renderer_instance = _renderer_factory[model_template_type](
                    tokenizer,
                    new_params,
                    generate_env_config,
                    render_config,
        ...
        if renderer_instance is None:
            # Check dedicated renderer factory
            if params.model_type in _renderer_factory:
                ...
                renderer_instance = _renderer_factory[params.model_type](
                    tokenizer,
                    params,
                    generate_env_config,
                    render_config,
        ...
        if renderer_instance is None:
            try:
                if tokenizer.chat_template is not None:
                    ...
                    renderer_instance = BasicRenderer(
                        tokenizer,
                        params,
                        generate_env_config,
                        render_config,
        ...
        if params.custom_modal_config:
            logging.info(
                f"Wrapping renderer {type(renderer_instance).__name__} with CustomModalProxyRenderer for custom modal support."
            )
            return CustomModalProxyRenderer(
                tokenizer, params, generate_env_config, render_config,
                ckpt_path, misc_config, vit_config, renderer_instance
            )
