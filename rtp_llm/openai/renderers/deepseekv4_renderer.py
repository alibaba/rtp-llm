...
class DeepseekV4Renderer(ReasoningToolBaseRenderer):
    ...
    def in_think_mode(self, request: ChatCompletionRequest) -> bool:
        ...
        # Honor both the top-level OpenAI-compatible field and the legacy
        # chat_template_kwargs form.
        if request.enable_thinking_requested():
    ...
    def _reasoning_effort_for_encoding(self, effort: Any) -> Optional[str]:
        normalized = self._normalize_reasoning_effort(effort)
        if normalized is None:
            return None

        render_message = getattr(self.encoding_module, "render_message", None)
        if not callable(render_message):
            return normalized

        code = getattr(render_message, "__code__", None)
        if code is not None:
            supported_efforts = next(
                (
                    set(constant)
                    for constant in code.co_consts
                    if isinstance(constant, tuple)
                    and None in constant
                    and "high" in constant
                ),
                None,
            )
            if supported_efforts is not None:
                if normalized in supported_efforts:
                    return normalized
                if normalized == "xhigh" and "max" in supported_efforts:
                    return "max"
                raise ValueError(
                    f"reasoning_effort '{normalized}' is not supported by the "
                    "installed DeepSeek-V4 encoding"
                )

        probe_messages = [{"role": "user", "content": ""}]
    ...
    def _build_prompt(self, request: ChatCompletionRequest) -> str:
        ...
        encode_config["reasoning_effort"] = self._reasoning_effort_for_encoding(
