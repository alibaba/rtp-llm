"""Logical ragged BERT masks: QI/vision see QI/vision; user tokens see all."""
...
def derive_bert_uqi_segment_ids(
    ...
    seq_of_tok = torch.repeat_interleave(
        torch.arange(num_seq, device=device), lengths, output_size=total
...
def build_bert_uqi_flashinfer_mask(
    ...
    # Allocating the variable-size mask requires one host scalar read.
    total_mask = int(sizes.sum())
    if total_mask == 0:
        return torch.empty(0, dtype=torch.bool, device=device)
    seq_of_pos = torch.repeat_interleave(
        torch.arange(num_seq, device=device), sizes, output_size=total_mask
    )  # [total_mask] 每个 mask 元素属于哪条序列
    block_start = torch.cumsum(sizes, 0) - sizes  # 各序列 mask 块在输出里的起点
    within = (
        torch.arange(total_mask, device=device) - block_start[seq_of_pos]
    )  # 块内 0..n^2-1
    ...
    return (seg_i == 1) | (
        seg_j == 0
    )  # [total_mask] bool, row-major [q,kv] per-req 拼接
