...
def convert_qwen_name(orig_name):
    ...
    per_layer_weights = {
        ...
        "transformer.attn.c_attn.weight": "attn.c_attn.weight",
        "transformer.attn.c_attn.bias": "attn.c_attn.bias",
        "transformer.attn.c_proj.weight": "attn.c_proj.weight",
    ...
    return f"transformer.h.{layer_id}.{per_layer_weights[weight_name]}"
...
def run_conversion(args: ProgArgs):
    ...
    print("========================================")
    ret = {}
    ret = hf_qwen_converter(args, ret)
    saved_dir = Path(args.out_dir)
    saved_dir.mkdir(parents=True, exist_ok=True)
    torch.save(ret, os.path.join(str(saved_dir), "sq_qwen.pt"))
