...
It also logs a ``[SleepMem]`` segment breakdown (``torch.cuda.memory_snapshot``)
...
from rtp_llm.utils.sleep_timing import timed_sleep_phase
...
def _cuda_graph_baked() -> bool:
    ...
    try:
        ...
        # If the optional Python state cannot be imported, fail closed for
        # pointer-sensitive cache destruction.  allocator empty_cache below is
        # still attempted: it can release ordinary fully-free segments even
        # when graph-private allocations remain live.
        graph_baked = True
    if graph_baked:
        return True
    try:
...
def _pool_id(seg: dict) -> tuple:
    ...
    ``empty_cache()`` returns fully-free segments from pools that are eligible
    for release; free blocks trapped in a still-referenced private MemPool are
    physically resident until that pool's owner is gone. This split attributes
    the sleep residual to default-vs-MemPool without pretending reserved bytes
    are live tensors.
    """
...
def release_and_trim(device: object, reason: str = "sleep") -> None:
    ...
    try:
        with torch.cuda.device(device):
            operation = "wake" if reason.startswith("wake") else "sleep"
            graph_baked = _cuda_graph_baked()
            free_before, total = torch.cuda.mem_get_info(device)
            before_snapshot = _snapshot_summary(device).replace("\n", " | ")
            logging.info(
                "[SleepMem] op=%s phase=reclaim_before graph_baked=%s driver_free=%.0fMiB total=%.0fMiB snapshot=%s",
                operation,
                graph_baked,
                free_before / _MiB,
                total / _MiB,
            ...
            with timed_sleep_phase(operation, "clear_runtime_caches"):
                notes = _clear_module_device_caches()
            with timed_sleep_phase(operation, "allocator_trim"):
                gc.collect()
                torch.cuda.synchronize()
                # Do this on every role, including graph-baked roles.  empty_cache
                # only returns fully-free segments; it does not free live graph
                # pointers, so no recapture is needed and pointer stability is
                # preserved.  Some private pools may reject trimming under TMS;
                # report that condition and keep sleep best-effort.
                try:
                    torch.cuda.empty_cache()
                    logging.info(
                        "[SleepMem] op=%s phase=allocator_trim status=ok graph_baked=%s",
                        operation,
                        graph_baked,
                    )
                except Exception as e:  # noqa: BLE001 - best-effort teardown
                    logging.warning(
                        "[SleepMem] op=%s phase=allocator_trim status=error graph_baked=%s detail=%s",
                        operation,
                        graph_baked,
                        str(e).replace("\n", " "),
                    )
            free_after = torch.cuda.mem_get_info(device)[0]
            after_snapshot = _snapshot_summary(device).replace("\n", " | ")
            logging.info(
                "[SleepMem] op=%s phase=reclaim_after graph_baked=%s driver_free=%.0fMiB reclaimed=%.0fMiB notes=%s snapshot=%s",
                operation,
                graph_baked,
                free_after / _MiB,
