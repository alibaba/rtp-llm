...
def _manager(local: Path, remote: Path):
    ...
    with contextlib.ExitStack() as stack:
        ...
        for name, value in (
            ...
            ("_toolkit_scope", "nvcc-test"),
            ("_cpp_runtime_scope", "cxx-test"),
        ):
            stack.enter_context(mock.patch.object(jit, name, return_value=value))
        stack.enter_context(
            mock.patch.object(jit.importlib.metadata, "version", return_value="1_0")
        )
        manager = jit.start_from_config(
            SimpleNamespace(
                local_jit_dir=str(local),
    ...
    if manager is None:
        raise AssertionError("remote cache manager did not start")
    return manager
...
class RemoteJitCacheE2ETest(unittest.TestCase):
    def round_trip(self, directory=None):
        with tempfile.TemporaryDirectory(
            prefix="jit-e2e-", dir=directory
        ) as tmp, tempfile.TemporaryDirectory(prefix="jit-remote-") as remote:
            local = Path(tmp) / "local"

            def run(mode):
                return subprocess.Popen(
                    [sys.executable, __file__, mode, str(local), remote],
            ...
            # Only this test's stopped producer tree is removed. A fresh consumer
            # uses the same configured path, as it would in another container.
            shutil.rmtree(local)
