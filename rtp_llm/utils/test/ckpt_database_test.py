...
class CkptDataBaseTest(unittest.TestCase):
    ...
    def test_collect_ckpt_file(self):
        ...
        self.assertEqual(
            path + "/pytorch_model.bin", database.PretrainFileList[0].file_name
        )
        ...
        self.assertEqual(1, len(database.PretrainFileList))
        self.assertEqual(
            path + "/test.safetensors", database.PretrainFileList[0].file_name
        )
        ...
        self.assertEqual(
            ["c_proj", "w2", "c_attn", "w1"],
            list(database.LoraCkpt.LoraFileList)[0].target_modules,
        )
        self.assertEqual(1, len(list(database.LoraCkpt.LoraFileList.values())[0]))
        self.assertEqual(
            12,
            len(list(database.LoraCkpt.LoraFileList.values())[0][0].get_tensor_names()),
        )
