...
class KimiK3MtpCheckpointTest(unittest.TestCase):
    def test_real_checkpoint_headers_when_provided(self):
        checkpoint = os.environ.get("K3_MTP_CKPT_PATH")
        if not checkpoint:
            self.skipTest("K3_MTP_CKPT_PATH is not set")
        root = Path(checkpoint)
        text = json.loads((root / "config.json").read_text())["text_config"]
        result = validate_checkpoint(root)
        self.assertEqual(result["required_tensors"], len(expected_tensors(text)))
