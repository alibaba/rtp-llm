...
class ScrAdvertiseIpTest(unittest.TestCase):
    def setUp(self):
        ...
        identity = patch.object(runtime, "_runtime_identity", None)
        identity.start()
    ...
    def test_resolves_each_restore_without_using_seed_identity(self):
        with patch("socket.gethostname", return_value="restored-pod"), patch(
            ...
            self.assertEqual(
                cache_store_advertise_ip(self.world, self.pc), "192.0.2.20"
            )
            self.assertEqual(
                cache_store_advertise_ip(self.world, self.pc), "192.0.2.21"
            )
    ...
    def test_manifest_host_wins_over_restored_runtime_identity(self):
        for member in self.world.members:
            member.ip = "192.0.2.50"
        identity = runtime.RestoreRuntimeIdentity("g1", "192.0.2.20", ())
        with patch.object(runtime, "_runtime_identity", identity), patch(
            "socket.gethostbyname"
        ) as resolve:
            self.assertIsNone(cache_store_advertise_ip(self.world, self.pc))
        resolve.assert_not_called()
