...
def get_token_at_first_id_from_combo_tokens(
    ...
    seq_of_tok = torch.repeat_interleave(
        torch.arange(num_seq, device=device), lengths_l, output_size=total
...
def bert_dual_head_scores(
    hidden_states: torch.Tensor,
    input_ids: torch.Tensor,
    input_lengths: torch.Tensor,
    w_qi: torch.nn.Linear,
    w_uqi: torch.nn.Linear,
    ...
    return torch.cat(
        (
            ...
            torch.softmax(w_uqi(uqi_hidden), dim=-1),
        ),
        dim=-1,
    )
