...
class MultiModalModifyPromptCallable(Protocol):
    def __call__(
        self, prompt: str, urls: List[str], mm_token: str, **kwargs: Any
    ) -> Tuple[str, List[Any]]: ...
...
class Plugin(object):
    def __init__(self, plugin_cls: Optional[type] = None):
        ...
        self.modify_prompt_plugin: Optional[ModifyPromptCallable] = self.get_if_exists(
            self._plugin, "modify_prompt_plugin"
        )
        self.process_encode_plugin: Optional[EncodeCallable] = self.get_if_exists(
            self._plugin, "process_encode_plugin"
        )
        self.process_decode_plugin: Optional[DecodeCallable] = self.get_if_exists(
            self._plugin, "process_decode_plugin"
        )
        self.modify_response_plugin: Optional[ModifyResponseCallable] = (
            self.get_if_exists(self._plugin, "modify_response_plugin")
        )
        self.stop_generate_plugin: Optional[StopGenerateCallable] = self.get_if_exists(
            self._plugin, "stop_generate_plugin"
        )

    def get_if_exists(
        self, instance: Any, plugin_name: str
