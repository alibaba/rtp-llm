    ...
    def load(self):
        ...
        while True:
            ...
            if key[0] == GLOBAL[0]:
                ...
                if func.__name__ in ['_rebuild_tensor_v2', '_rebuild_tensor', '_rebuild_from_type_v2']:
                    if func.__name__ == '_rebuild_from_type_v2':
                        new_obj = tuple(args[2])
                    else:
                        new_obj = tuple(args)
                else:
                    new_obj = func(*args)
                if isinstance(new_obj, torch.Tensor):
                    new_obj = new_obj.cuda()
                self.stack[-1] = new_obj
            elif key[0] == BUILD[0]:
