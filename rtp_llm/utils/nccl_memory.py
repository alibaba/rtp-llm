...
from rtp_llm.utils.nccl_memory_policy import NCCL_IN_PROGRESS as _NCCL_IN_PROGRESS
from rtp_llm.utils.nccl_memory_policy import canonical_key as _canonical_key
from rtp_llm.utils.nccl_memory_policy import fingerprint as _fingerprint
from rtp_llm.utils.nccl_memory_policy import rc_detail as _rc_detail
...
def suspend_for_sleep(device: object, reason: str = "sleep") -> None:
    ...
    with _lock:
        ...
        log_sleep_timing(
            ...
            elapsed * 1000.0,
            fields={
                "comm_count": len(found),
                "suspended_count": len(_suspended),
                "suspendable_mib": f"{suspendable / _MiB:.1f}",
                "driver_free_before_mib": f"{free_pre / _MiB:.0f}",
                "driver_free_after_mib": f"{free_post / _MiB:.0f}",
                "released_mib": f"{released / _MiB:.1f}",
    ...
    with _lock:
        ...
        log_sleep_timing(
            ...
            fields={
                ...
                "driver_free_before_mib": f"{free_pre / _MiB:.0f}",
                "driver_free_after_mib": f"{free_post / _MiB:.0f}",
                "restored_mib": f"{restored / _MiB:.1f}",
                "failed_count": len(failures),
            },
        )
        if failures:
