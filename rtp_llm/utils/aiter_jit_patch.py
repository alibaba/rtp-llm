...
def _bootstrap_aiter_core() -> types.ModuleType:
    ...
    try:
        ...
        # AITER 0.1.17 patches inspect.BlockFinder during import on Python
        # 3.10.9. The AOT bootstrap above imports that patch once; restore the
        # original hook before reload so AITER does not wrap its own wrapper and
        # recurse forever while Triton inspects a JIT function.
        inspect_module = sys.modules.get("inspect")
        original_tokeneater = getattr(aiter_module, "_orig_te", None)
        patched_tokeneater = getattr(aiter_module, "_patched_te", None)
        if (
            inspect_module is not None
            and original_tokeneater is not None
            and patched_tokeneater is not None
            and getattr(inspect_module.BlockFinder, "tokeneater", None)
            is patched_tokeneater
        ):
            inspect_module.BlockFinder.tokeneater = original_tokeneater
        with _temporary_stdlib_hooks():
