...
AUX_OSS_REGION_ENV = "AUX_HIDDEN_STATES_OSS_REGION"
...
def infer_oss_region(endpoint: str) -> str:
    ...
    if not host.startswith("oss-"):
        return ""
    region = host[len("oss-") :]
    for suffix in ("-internal.aliyuncs.com", ".aliyuncs.com"):
        if region.endswith(suffix):
            return region[: -len(suffix)]
    return ""


def build_ossutil_config(work_dir: Path, endpoint: str, region: str) -> List[str]:
...
def build_config_from_env() -> UploaderConfig:
    ...
    region = (
        os.environ.get(AUX_OSS_REGION_ENV, "").strip()
        or os.environ.get(OSS_REGION_ENV, "").strip()
        or infer_oss_region(endpoint)
    )
    ossutil_bin = (
...
def run_uploader(config: UploaderConfig) -> int:
    ...
    config_args = build_ossutil_config(config.work_dir, config.endpoint, config.region)
    if not config_args:
        log(
...
def parse_args(argv: Optional[Sequence[str]] = None) -> UploaderConfig:
    ...
    parser.add_argument("--region", default=env_config.region)
    ...
    return UploaderConfig(
        ...
        region=args.region or infer_oss_region(args.endpoint),
