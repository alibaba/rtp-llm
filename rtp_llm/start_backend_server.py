...
def _create_rank_processes(global_controller: ConcurrencyController, pipe_writer=None):
...
def multi_rank_start(global_controller: ConcurrencyController, pipe_writer=None):
    ...
    processes, rank_pipe_readers = _create_rank_processes(
        global_controller, pipe_writer
    )
    local_world_size = len(processes)

    # Check for fake gang environment
    distribute_config = DistributeConfig()
    distribute_config.update_from_env()
    if distribute_config.fake_gang_env:
        return processes
...
def start_backend_server(global_controller: ConcurrencyController, pipe_writer=None):
