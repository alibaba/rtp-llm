...
class HCNormGraphTest(unittest.TestCase):
    ...
    def test_analytic_rounding_and_empty(self):
        ...
        with self.assertRaisesRegex(ValueError, "replicated contiguous BF16"):
            unit.pre_norm(
        ...
        # The fused path consumes replicated full-width inputs on every TP rank.
        for tp_size in (2, 4):
            for tp_rank in range(tp_size):
                with self.subTest(tp_size=tp_size, tp_rank=tp_rank):
                    actual = unit.pre_norm(x, norm, tp_size=tp_size, tp_rank=tp_rank)
                    for value, reference in zip(actual, (y, post, comb)):
                        torch.testing.assert_close(value, reference, rtol=0, atol=0)
        for tp_size, tp_rank in ((0, 0), (4, -1), (4, 4)):
            with self.assertRaisesRegex(ValueError, "replicated contiguous BF16"):
                unit.pre_norm(x, norm, tp_size=tp_size, tp_rank=tp_rank)
