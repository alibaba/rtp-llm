...
double readScalar(const std::unordered_map<std::string, torch::Tensor>& weights,
...
void LengthPredictor::processPacket(StepPacket& packet) {
    ...
    if (hidden.is_cuda()) {
        ...
        if (slot.capacity < batch) {
            slot.pinned = torch::empty(
                {batch, config_.feature_dim},
        ...
        switch (hidden.scalar_type()) {
            case torch::kFloat32:
                invokeLengthEncoderForward<float>(
                    hidden.data_ptr<float>(), weight_t, bias, out, b, h, f, static_cast<float>(config_.layernorm_eps), stream.stream());
                break;
            case torch::kHalf:
...
void LengthPredictor::processRow(LengthPredictorState& state, const float* feature, int64_t decode_step) const {
    ...
    if (is_prediction) {
        ...
        if (alpha <= 0.0) {
            ...
            const double history_remaining = headExpectedLength(scratch_.modulated.data(),
                                                                static_cast<double>(decode_step));
