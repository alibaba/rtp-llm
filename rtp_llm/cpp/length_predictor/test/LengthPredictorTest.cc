...
TEST(LengthPredictorTest, PredictionIsCausalAgainstSameStepObservation) {
    ...
    auto modulated =
        feature_t21 * (1.0 + config.scale_limit * torch::tanh(chunks[0])) + config.shift_limit * torch::tanh(chunks[1]);
    const double t            = 21.0;
    auto         time_feature = torch::tensor(
        {{static_cast<float>(t / config.t_cap), static_cast<float>(std::log1p(t) / std::log1p(config.t_cap))}});
    auto encoded_time = torch::nn::functional::gelu(
        torch::nn::functional::linear(time_feature, weights.at("time_linear_weight"), weights.at("time_linear_bias")));
