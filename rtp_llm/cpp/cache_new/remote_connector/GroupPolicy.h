...
public:
    ...
    virtual bool filterNeedLoadLocations(const kv_cache_manager::Locations& locations,
                                         LocationsView&                     locations_view) const = 0;
...
public:
    ...
    virtual bool filterNeedLoadLocations(const kv_cache_manager::Locations& locations,
                                         LocationsView&                     locations_view) const override;
...
public:
    FullLinearLayerGroupPolicy(std::shared_ptr<KVCacheAllocator> allocator,
        ...
        DefaultLayerGroupPolicy(allocator, full_group_ids, other_group_ids),
        linear_attention_write_interval_(linear_attention_write_interval) {}
    bool init() override;
...
private:
    /*
        interval == 0 :         only write last key's linear attention
        interval == n (n > 0) : every n keys, write a linear attention
    */
    uint32_t linear_attention_write_interval_ = 0;
...
class FullSWLayerGroupPolicy: public DefaultLayerGroupPolicy {
public:
    FullSWLayerGroupPolicy(std::shared_ptr<KVCacheAllocator> allocator,
                           ...
                           size_t                            sink_size,
                           size_t                            sw_size):
        DefaultLayerGroupPolicy(allocator, full_group_ids, other_group_ids), sink_size_(sink_size), sw_size_(sw_size) {}
    bool init() override;
