...
struct ReadMetricsHelper {
    ReadMetricsHelper(const kmonitor::MetricsReporterPtr& metrics_reporter):
        begin_us(rtp_llm::currentTimeUs()), metrics_reporter(metrics_reporter) {}
    ~ReadMetricsHelper() {
        ...
        if (metrics_reporter) {
            ...
            RTP_LLM_LOG_INFO(
                "ReadMetrics : {fail[%d], reuse_block_num[%ld], read_task_cost_time_us[%ld], match_time_us[%ld], broadcast_time_us[%ld]}",
...
struct WriteMetricsHelper {
    WriteMetricsHelper(const kmonitor::MetricsReporterPtr& metrics_reporter):
        begin_us(rtp_llm::currentTimeUs()), metrics_reporter(metrics_reporter) {}
    ~WriteMetricsHelper() {
        ...
        if (metrics_reporter) {
            ...
            RTP_LLM_LOG_INFO(
                "WriteMetrics : {fail[%d], write_cache_block_num[%ld], write_task_cost_time_us[%ld], get_write_location_time_us[%ld], broadcast_time_us[%ld], finish_write_time_us[%ld]}",
...
struct SdkMetricsHelper {
    SdkMetricsHelper(bool is_read, const kmonitor::MetricsReporterPtr& metrics_reporter):
        is_read(is_read), begin_us(rtp_llm::currentTimeUs()), metrics_reporter(metrics_reporter) {}
    ~SdkMetricsHelper() {
        ...
        if (metrics_reporter) {
            ...
            RTP_LLM_LOG_DEBUG("SDKMetrics : {mode[%s], fail[%d], sdk_block_num[%ld], sdk_cost_time_us[%ld]}",
                              is_read ? "read" : "write",
...
void RemoteConnector::asyncReadTask(const std::shared_ptr<KVCacheResourceV1>&           resource,
    ...
    RTP_LLM_LOG_DEBUG("asyncReadTask, [%d] [%zu]", resource->reuse_block_num, resource->cache_keys.size());
    ...
    kv_cache_manager::BlockMaskOffset block_mask     = resource->reuse_block_num;
    const std::vector<int64_t>&       keys           = resource->cache_keys;
    ReadMetricsHelper                 helper(metrics_reporter_);
    ...
    resource->reuse_block_num               = block_mask + new_reuse_block_num;
    RTP_LLM_LOG_DEBUG(
        "asyncReadTask old reuse_block_num[%zu], new reuse_block_num [%zu]", block_mask, resource->reuse_block_num);
    async_context->setState(RemoteConnectorAsyncContext::State::RCS_SUCCESS);
}
...
void RemoteConnector::asyncWriteTask(const std::shared_ptr<KVCacheResourceV1>&           resource,
                                     const std::shared_ptr<RemoteConnectorMeta>&         meta,
                                     const std::shared_ptr<RemoteConnectorAsyncContext>& async_context) {
    RTP_LLM_LOG_DEBUG("asyncWriteTask, [%d] [%zu]", resource->reuse_block_num, resource->cache_keys.size());
    WriteMetricsHelper          helper(metrics_reporter_);
    const std::string&          unique_id            = meta->unique_id;
...
bool RemoteConnector::genReadRequest(size_t                                    tp_size,
    ...
    for (const auto& location_view : locations_view) {
        for (const auto& location_spec : location_view) {
            ...
            const auto& block_indices = resource->group_block_ids.at(spec_info.group_id)->block_indices;
...
bool RemoteConnector::Read(const std::string&                 trace_id,
    ...
    SdkMetricsHelper helper(true, metrics_reporter_);
...
bool RemoteConnector::Write(const std::string&                 trace_id,
    ...
    // for transfer client
    // TODO : support finish partially
    SdkMetricsHelper helper(false, metrics_reporter_);
    helper.collector.remote_sdk_block_num = block_ids.size();
    kv_cache_manager::BlockBuffers block_buffers;
    if (!group_policy_->genBlockBuffers(group_ids, block_ids, block_buffers)) {
        return false;
