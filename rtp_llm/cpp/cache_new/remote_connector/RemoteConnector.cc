...
RemoteConnectorAsyncContext::RemoteConnectorAsyncContext(std::future<void> future): future_(std::move(future)) {}

bool RemoteConnectorAsyncContext::success() const {
    return state_.load(std::memory_order_acquire) == RemoteConnectorAsyncContext::State::RCS_SUCCESS;
}
...
RemoteConnector::RemoteConnector(const CacheConfig&                        cache_config,
    ...
    RemoteConnector::InitParams init_params{
        cache_config, model_parameter, device, register_buffer_addr, register_buffer_size, lora_info_map, tp_addrs};
...
RemoteConnector::genLocationSpecInfoMapAndGroups(int64_t tp_size) {
    ...
    size_t                   byte_size_per_block = init_params_->cache_config.layer_type_params[0]->block_size();
...
bool RemoteConnector::init() {
    ...
    broadcaster_           = std::make_shared<TpBroadcastManager>(init_params_->tp_addrs);
...
RemoteConnector::asyncRead(const std::shared_ptr<KVCacheResourceV1>&      resource,
    ...
    if (const auto& remote_connector_meta = std::dynamic_pointer_cast<RemoteConnectorMeta>(meta);
        ...
        auto async_context =
            std::shared_ptr<RemoteConnectorAsyncContext>(new RemoteConnectorAsyncContext(promise->get_future()));
...
std::shared_ptr<KVCacheConnector::AsyncContext>
RemoteConnector::asyncWrite(const std::shared_ptr<KVCacheResourceV1>&      resource,
                            const std::shared_ptr<KVCacheConnector::Meta>& meta) {
    if (const auto& remote_connector_meta = std::dynamic_pointer_cast<RemoteConnectorMeta>(meta);
        remote_connector_meta != nullptr) {
        auto promise = std::make_shared<std::promise<void>>();
...
std::shared_ptr<KVCacheConnector::AsyncContext>
RemoteConnector::asyncWriteByLayer(int                                            layer_id,
...
::grpc::Status RemoteConnector::broadcastTp(::grpc::ServerContext*        context,
                                            const ::BroadcastTpRequestPB* request,
                                            ::BroadcastTpResponsePB*      response) {
    if (!request->has_remote_request()) {
        RTP_LLM_LOG_WARNING("request has no remote_request");
        return grpc::Status(grpc::StatusCode::INTERNAL, "request has no remote_request");
    }
    const auto&                 remote_request = request->remote_request();
    const auto&                 trace_id       = remote_request.trace_id();
    std::vector<int32_t>        group_ids(remote_request.group_ids().begin(), remote_request.group_ids().end());
    std::vector<int32_t>        block_ids(remote_request.block_ids().begin(), remote_request.block_ids().end());
    kv_cache_manager::UriStrVec uris(remote_request.uris().begin(), remote_request.uris().end());
    auto                        remote_response = response->mutable_remote_response();
    switch (remote_request.op()) {
        case ::RemoteBroadcastOp::REMOTE_BROADCAST_READ: {
            if (!Read(trace_id, group_ids, block_ids, uris)) {
                remote_response->set_success(false);
                RTP_LLM_LOG_WARNING("broadcastTp Read failed");
                return grpc::Status::OK;
            }
            break;
        }
        case ::RemoteBroadcastOp::REMOTE_BROADCAST_WRITE: {
            ...
            if (!out_uris.empty()) {
                auto mutable_actual_uris = remote_response->mutable_actual_uris();
        ...
        default: {
            remote_response->set_success(false);
            RTP_LLM_LOG_WARNING("invalid operation [%d]", remote_request.op());
            return grpc::Status::OK;
        }
    }
    remote_response->set_success(true);
    return grpc::Status::OK;
}
...
#define CHECK_AND_LOG(condition, format, args...)                                                                      \
    do {                                                                                                               \
        if (!(condition)) {                                                                                            \
...
void RemoteConnector::asyncReadTask(const std::shared_ptr<KVCacheResourceV1>&           resource,
    ...
    kv_cache_manager::BlockMaskOffset block_mask     = resource->reuse_len;
    const std::vector<int64_t>&       keys           = resource->cache_keys;
    RETURN_IF(block_mask >= keys.size());
    ...
    CHECK_AND_LOG(match_result.first, "asyncGet match failed, [%s]", match_trace_id.c_str());
    const auto& new_match_locations = match_result.second;
    RETURN_IF(new_match_locations.empty());
    // 2. sync call all rank get read
    std::vector<BroadcastTpRequestPB> requests;
    size_t                            new_reuse_len = 0;
    ...
    RETURN_IF(new_reuse_len == 0);

    async_context->setState(RemoteConnectorAsyncContext::State::RCS_READ_BROADCAST);
    auto broadcast_result = broadcaster_->broadcast(requests, get_broadcast_timeout_);
    broadcast_result->waitDone();
    CHECK_AND_LOG(broadcast_result->success(), "Read failed for grpc status, trace_id [%s]", match_trace_id.c_str());
    auto responses = broadcast_result->responses();
    for (int i = 0; i < responses.size(); i++) {
        CHECK_AND_LOG(
            responses[i].remote_response().success(), "Read rank[%d] failed, trace_id [%s]", i, match_trace_id.c_str());
    }
    // TODO : maybe not all locations are loaded successfuly
    resource->reuse_len = block_mask + new_reuse_len;
    async_context->setState(RemoteConnectorAsyncContext::State::RCS_SUCCESS);
}
...
void RemoteConnector::asyncWriteTask(const std::shared_ptr<KVCacheResourceV1>&           resource,
    ...
    CHECK_AND_LOG(start_result, "asyncPut getWriteLocation failed, [%s]", start_write_trace_id.c_str());
    ...
    auto                               finish_write_task = [&, this](bool success = false) {
        ...
        CHECK_AND_LOG(finish_result, "asyncPut finishWrite failed, [%s]", finish_write_trace_id.c_str());
        async_context->setState(success ? RemoteConnectorAsyncContext::State::RCS_SUCCESS :
                                          RemoteConnectorAsyncContext::State::RCS_ERROR);
    };

    // 2. sync call all rank write
    std::vector<BroadcastTpRequestPB> requests;
    ActualUriGather                   actual_uri_gather;
    CHECK_AND_LOG(genWriteRequest(broadcaster_->workerNum(),
    ...
    auto broadcast_result = broadcaster_->broadcast(requests, put_broadcast_timeout_);
    broadcast_result->waitDone();
    ...
    auto responses = broadcast_result->responses();
    for (int i = 0; i < responses.size(); i++) {
        CHECK_AND_LOG_WITH_DEFER(responses[i].remote_response().success(),
                                 finish_write_task,
                                 "Write rank[%d] failed, trace_id [%s]",
                                 i,
                                 meta->trace_id.c_str());
    }
...
bool RemoteConnector::genReadRequest(size_t                                    tp_size,
                                     ...
                                     const std::shared_ptr<KVCacheResourceV1>& resource,
                                     std::vector<BroadcastTpRequestPB>&        requests,
                                     size_t&                                   new_reuse_len) const {

    requests.resize(tp_size, {});
    for (size_t i = 0; i < tp_size; ++i) {
        requests[i].mutable_remote_request()->set_op(::RemoteBroadcastOp::REMOTE_BROADCAST_READ);
    ...
    if (!group_policy_->filterNeedLoadLocations(locations, locations_view)) {
        RTP_LLM_LOG_WARNING("trace_id [%s], filterNeedLoadLocations failed");
        return false;
    }
    new_reuse_len                 = locations_view.size();
    ...
    for (const auto& location_view : locations_view) {
        for (const auto& location_spec : location_view) {
            ...
            auto block_id = resource->group_block_ids.at(spec_info.group_id)->block_indices.at(cache_key_idx);
...
bool RemoteConnector::genWriteRequest(size_t                                    tp_size,
    ...
    for (size_t i = 0; i < tp_size; ++i) {
        requests[i].mutable_remote_request()->set_op(::RemoteBroadcastOp::REMOTE_BROADCAST_WRITE);
        requests[i].mutable_remote_request()->set_trace_id(trace_id);
