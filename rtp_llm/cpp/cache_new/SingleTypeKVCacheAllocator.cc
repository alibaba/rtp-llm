...
MallocResult SingleTypeKVCacheAllocator::malloc(const MallocInfo& malloc_info) {
    ...
    int seq_len = (malloc_info.total_seq_len >= 0) ? malloc_info.total_seq_len 
    ...
    if (common_blocks_need > 0) {
        ...
        if (free_blocks_num < static_cast<size_t>(common_blocks_need)) {
            RTP_LLM_LOG_WARNING("Insufficient free blocks for common part: need %d, have %zu", common_blocks_need, free_blocks_num);
        }
        
        // Allocate common blocks
        full_kv_cache_group_->malloc(cache_keys_0, block_indices_0, common_seq_len);
        ...
        if (newly_allocated_blocks > 0 && batch_size > 1) {
            std::vector<int> new_common_blocks(
                block_indices_0.end() - newly_allocated_blocks,
                block_indices_0.end()
            );
    ...
    if (has_common_extra_split) {
        ...
        if (extra_blocks_need_per_batch > 0) {
            ...
            if (free_blocks_num < static_cast<size_t>(total_extra_blocks)) {
                RTP_LLM_LOG_WARNING("Insufficient free blocks for extra part: need %d, have %zu", total_extra_blocks, free_blocks_num);
...
InsertResult SingleTypeKVCacheAllocator::insertIntoCache(const InsertInfo& insert_info) {
    ...
    for (int batch_id = 0; batch_id < batch_size; ++batch_id) {
        ...
        // Note: loss parameter removed from insertIntoCache signature
