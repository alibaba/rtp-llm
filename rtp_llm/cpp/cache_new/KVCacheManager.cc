...
InsertResult KVCacheManager::insertIntoCache(const InsertInfo& insert_info) {
    ...
    if (insert_info.enable_memory_cache || params_.kv_cache_config.enable_remote_cache) {
        ...
        InsertInfo deep_copy_insert_info = insert_info;
        deep_copy_insert_info.batch_kv_cache_resource =
            std::make_shared<BatchKVCacheResource>(*insert_info.batch_kv_cache_resource);
        auto deleter = [deep_copy_insert_info, allocator = allocator_](KVCacheResourceV1* resource) {
            FreeInfo free_info(deep_copy_insert_info.batch_kv_cache_resource, deep_copy_insert_info.complete_token_ids);
