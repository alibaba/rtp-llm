...
static void registerRuntimeBindings(py::module& m) {
    ...
    m.def(
        ...
        [](std::size_t device_id, bool trace_memory, bool enable_comm_overlap, int mla_ops_type) {
            // Python initializes process state but does not consume the resolved
            // MLA type; C++ engine construction obtains that value from initRuntime.
    ...
    RTP_LLM_CHECK_WITH_INFO(py::hasattr(m, "init_exec_ctx") && py::hasattr(m, "register_comm_ops")
                                && py::hasattr(m, "clear_comm_ops"),
                            "librtp_compute_ops runtime bindings are incomplete; check th_compute_lib GPU deps");
}
#endif
