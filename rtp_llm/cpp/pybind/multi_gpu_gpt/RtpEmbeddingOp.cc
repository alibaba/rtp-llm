...
void RtpEmbeddingOp::init(py::object model,
    ...
    try {
        ...
        startRpcServer(parallelism_config.arpc_server_port,
        ...
        grpc_server_thread_ = std::thread(&RtpEmbeddingOp::initGrpcServer,
                                          this,
                                          parallelism_config.grpc_server_port,
                                          grpc_config,
                                          embedding_engine_,
                                          py_render,
                                          py_handler,
...
void RtpEmbeddingOp::startRpcServer(int64_t                              arpc_server_port,
                                    ...
                                    std::shared_ptr<MultimodalProcessor> mm_processor) {
    auto arpc_service = std::move(createEmbeddingArpcService(arpc_server_port,
                                                             arpc_thread_num,
                                                             arpc_queue_num,
                                                             arpc_io_thread_num,
                                                             py_render,
    ...
    if (arpc_service) {
        ...
        embedding_rpc_service_.reset(new ArpcServerWrapper(
            std::move(arpc_service), arpc_thread_num, arpc_queue_num, arpc_io_thread_num, arpc_server_port));
