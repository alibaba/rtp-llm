...
std::unique_ptr<ProposeModelEngineInitParams> prepareMTPEngineInitParams(size_t model_id, py::object propose_model, py::object engine_config, py::object vit_config) {
    auto        sp_model           = propose_model.attr("model");
    std::string sp_type_str        = propose_model.attr("sp_type").cast<std::string>();
    SpeculativeType sp_type         = SpeculativeExecutionConfig::from_string(sp_type_str);
...
void RtpLLMOp::init(py::object model,
    ...
    std::unique_ptr<ProposeModelEngineInitParams> propose_params = initProposeModel(propose_model, engine_config, vit_config);
...
std::unique_ptr<ProposeModelEngineInitParams> RtpLLMOp::initProposeModel(py::object propose_model, py::object engine_config, py::object vit_config) {
    try {
        ...
        if (sp_type == SP_TYPE_VANILLA) {
            ...
            params                             = std::make_unique<ProposeModelEngineInitParams>(
                ...
                std::move(*gpt_weight));
            model_id_++;
        } else if (sp_type == SP_TYPE_MTP) {
            params = prepareMTPEngineInitParams(model_id_, propose_model, engine_config, vit_config);
            // Get gen_num_per_cycle from sp_config
            auto sp_config = engine_config.attr("sp_config").cast<SpeculativeExecutionConfig>();
            size_t gen_num_per_cycle = sp_config.gen_num_per_cycle;
            model_id_ += gen_num_per_cycle;
        } else if (sp_type == SP_TYPE_EAGLE || sp_type == SP_TYPE_EAGLE3) {
            params = prepareMTPEngineInitParams(model_id_, propose_model, engine_config, vit_config);
            model_id_++;
            ...
            RTP_LLM_FAIL("sp_type %s not support", sp_type_str.c_str());
        }
        return params;
    } catch (const std::exception& e) {
