    ...
    py::class_<SpeculativeExecutionConfig>(m, "SpeculativeExecutionConfig")
        ...
        .def_property("type",
            [](const SpeculativeExecutionConfig& self) {
                return SpeculativeExecutionConfig::to_string(self.type);
            },
        ...
        .def("to_string", [](const SpeculativeExecutionConfig& self) {
            return self.to_string();
        })
        .def(py::pickle(
            [](const SpeculativeExecutionConfig& self) {
                return py::make_tuple(
                    self.model_type, SpeculativeExecutionConfig::to_string(self.type), self.sp_min_token_match,
                    ...
                    self.quantization, self.checkpoint_path
                );
            },
            [](py::tuple t) {
                if (t.size() != 10) throw std::runtime_error("Invalid state!");
