...
PYBIND11_MODULE(libth_transformer_config, m) {
    ...
    // Register WorkerPortsInfo (worker RPC/HTTP ports)
    py::class_<WorkerPortsInfo>(m, "WorkerPortsInfo")
        .def(py::init<>())
        .def_readwrite("model_rpc_port", &WorkerPortsInfo::model_rpc_port)
        .def_readwrite("embedding_rpc_server_port", &WorkerPortsInfo::embedding_rpc_server_port)
        .def_readwrite("http_port", &WorkerPortsInfo::http_port);
