...
PYBIND11_MODULE(libth_transformer_config, m) {
    ...
    py::class_<HWKernelConfig>(m, "HWKernelConfig")
        ...
        .def_readwrite("disable_sp_draft_prefill_cuda_graph", &HWKernelConfig::disable_sp_draft_prefill_cuda_graph)
        .def_readwrite("disable_sp_draft_decode_cuda_graph", &HWKernelConfig::disable_sp_draft_decode_cuda_graph)
        .def_readwrite("disable_sp_target_verify_cuda_graph", &HWKernelConfig::disable_sp_target_verify_cuda_graph)
        ...
        .def(py::pickle(
            [](const HWKernelConfig& self) {
                return py::make_tuple(self.deep_gemm_num_sm,
                                      ...
                                      self.disable_sp_draft_prefill_cuda_graph,
                                      self.disable_sp_draft_decode_cuda_graph,
                                      self.disable_sp_target_verify_cuda_graph);
            },
            [](py::tuple t) {
                if (t.size() != 20)
                    throw std::runtime_error("Invalid state!");
                ...
                try {
                    ...
                    c.disable_sp_draft_prefill_cuda_graph = t[17].cast<bool>();
                    c.disable_sp_draft_decode_cuda_graph  = t[18].cast<bool>();
                    c.disable_sp_target_verify_cuda_graph = t[19].cast<bool>();
                } catch (const std::exception& e) {
