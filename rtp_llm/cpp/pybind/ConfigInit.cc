...
PYBIND11_MODULE(libth_transformer_config, m) {
    ...
    py::class_<KVCacheConfig>(m, "KVCacheConfig")
        ...
        .def(py::pickle(
            ...
            [](py::tuple t) {
                if (t.size() != 43 && t.size() != 54 && t.size() != 57 && t.size() != 68)
                    throw std::runtime_error("Invalid state!");
                KVCacheConfig c;
                try {
                    ...
                    if (t.size() >= 68) {
                        ...
                        c.enable_memory_cache_remote_eviction     = t[57].cast<bool>();
                        c.device_cache_high_watermark_ratio       = t[58].cast<int>();
                        c.memory_cache_high_watermark_ratio       = t[59].cast<int>();
                        c.memory_cache_remote_eviction_timeout_ms = t[60].cast<int>();
                        c.memory_cache_remote_eviction_max_blocks = t[61].cast<int>();
                        c.memory_cache_h2d_copy_mode              = t[62].cast<std::string>();
                        c.memory_cache_h2d_copy_strict            = t[63].cast<bool>();
                        c.enable_memory_cache_h2d_3d_batch_auto   = t[64].cast<bool>();
                        c.memory_cache_d2h_copy_mode              = t[65].cast<std::string>();
                        c.memory_cache_d2h_copy_strict            = t[66].cast<bool>();
                        c.enable_memory_cache_d2h_3d_batch_auto   = t[67].cast<bool>();
                    }
                } catch (const std::exception& e) {
