...
PYBIND11_MODULE(libth_transformer_config, m) {
        ...
        .def(py::pickle(
            [](const FMHAConfig& self) {
                return py::make_tuple(self.enable_fmha,
                                      ...
                                      self.fmha_perf_instrument,
                                      self.fmha_show_params,
        ...
        .def(py::pickle(
            [](const KVCacheConfig& self) {
                return py::make_tuple(self.reuse_cache,
                                      ...
                                      self.blockwise_use_fp8_kv_cache);
            },
            [](py::tuple t) {
                if (t.size() != 22)
                    throw std::runtime_error("Invalid state!");
        ...
        .def(py::pickle(
            [](const ProfilingDebugLoggingConfig& self) {
                return py::make_tuple(self.trace_memory,
                                      ...
                                      self.nccl_debug_file,
                                      self.debug_load_server,
                                      self.hack_layer_num,
                                      self.debug_start_fake_process,
                                      self.dg_print_reg_reuse,
                                      self.qwen_agent_debug,
    ...
    py::class_<SpeculativeExecutionConfig>(m, "SpeculativeExecutionConfig")
        ...
        .def_property(
            "type",
            [](const SpeculativeExecutionConfig& self) { return SpeculativeExecutionConfig::to_string(self.type); },
            [](SpeculativeExecutionConfig& self, const std::string& str) {
                self.type = SpeculativeExecutionConfig::from_string(str);
            })
