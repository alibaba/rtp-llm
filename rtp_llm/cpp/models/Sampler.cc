...
SamplerOutput Sampler::forward(const SamplerInputs& inputs) {
    ...
    auto beam_width_in = inputs.num_beams_in->data<uint64_t>();
    auto beam_width_out = inputs.num_beams_out->data<uint64_t>();
    ...
    do {
        ...
        const auto sample_batch_size = sample_to_batch_idx - from_batch_idx;
...
#define MAY_GET_BUFFER_VIEW(buffer_ptr)                                                                                \
    (buffer_ptr.get() ? buffer_ptr->view(from_batch_idx, sample_batch_size) : Buffer::emptyBuffer())
        ...
        if (current_beam_size == 1 && current_beam_width_out == 1) {
            ...
            size_t beam_batch_size = (size_t)(sample_batch_size / current_beam_width_in);
            RTP_LLM_LOG_DEBUG("current_beam_width_in is %d", current_beam_width_in);
            RTP_LLM_LOG_DEBUG("current_beam_width_out is %d", current_beam_width_out);
            ...
            auto output = device_->sampleBeamSearch({*sample_logits_device,
                                                     sample_tokens_device,
                                                     input_lengths_device,
                                                     beam_search_sequence_lengths_device,
                                                     sample_cum_log_probs_device,
                                                     current_beam_width_out});

            device_->copy({sample_tokens, *output.token_ids});
            device_->copy({input_lengths, *output.input_lengths});
            device_->copy({beam_search_sequence_lengths, *output.sequence_lengths});
            device_->copy({*sample_cum_log_probs, *output.cum_log_probs});
            device_->copy({beam_index, *output.beam_indices});
        ...
        from_batch_idx = sample_to_batch_idx;
        ++sample_to_batch_idx;
