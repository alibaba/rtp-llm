...
class PyWrappedModelCacheStoreIntegrationTest(unittest.TestCase):
    ...
    def test_bert_weights_alone_do_not_disable_layer_micro_batch(self) -> None:
        model = CacheStoreForwardModel()
        result = run_scenario(
            model,
            "bert_without_request_owned_multimodal_inputs",
        )

        self.assertTrue(result["micro_batch_plan_enabled"])

    def test_existing_degenerate_micro_batch_fallback_is_unchanged(
        self,
    ) -> None:
    ...
    def test_bert_graph_prepare_and_update_receive_request_ids(self) -> None:
        result = run_scenario(CacheStoreForwardModel(), "bert_graph_wiring")
        ...
        self.assertEqual(
            ...
            [
                ...
                "canRun",
                "updateKVCacheKernelBlockId",
            ],
        )
        self.assertEqual(result["graph_position_id_numels"], [4, 4, 4, 4])
        self.assertEqual(result["graph_token_type_id_numels"], [4, 4, 4, 4])

    def test_request_owned_mask_bypasses_graph_prepare_and_update(self) -> None:
        result = run_scenario(CacheStoreForwardModel(), "bert_graph_request_owned_mask")

        self.assertEqual(result["graph_calls"], [])
        self.assertEqual(result["graph_position_id_numels"], [])
        self.assertEqual(result["graph_token_type_id_numels"], [])

    def test_graph_rejection_skips_prepare_and_update(self) -> None:
        result = run_scenario(CacheStoreForwardModel(), "bert_graph_can_run_false")

        self.assertEqual(result["graph_calls"], ["canRun", "canRun"])
        self.assertEqual(result["graph_position_id_numels"], [4, 4])
        self.assertEqual(result["graph_token_type_id_numels"], [4, 4])
