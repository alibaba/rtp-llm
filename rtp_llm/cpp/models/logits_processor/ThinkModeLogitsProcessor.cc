...
void maskThinkBeginToken(const torch::Tensor& new_tokens_logits, size_t vocab_size, const StreamThinkInfo& info) {
...
void applyThinkSpecRowMask(int32_t* row, size_t words, StreamThinkInfo& info) {
    std::fill_n(row, words, SpecLogitsProcessorRequest::kBitmaskAllowAll);
...
MtpProcessorCapability ThinkModeLogitsProcessor::mtpCapability() const {
    return {MtpProcessorMode::SPEC_VERIFY, {}};
}

ErrorResult<int> ThinkModeLogitsProcessor::prepareSpeculative(const SpecLogitsProcessorRequest& request) {
    return ErrorResult<int>(tryAcceptAndFillBitmask(request));
}

std::optional<int64_t> ThinkModeLogitsProcessor::committedOutputLen() const {
    if (!isSpecVerifyEligible()) {
        return std::nullopt;
    }
    return acceptedTokenLen();
}
...
ThinkModeLogitsProcessorPtr ThinkModeLogitsProcessor::fromGenerateInput(std::shared_ptr<GenerateInput> generate_input,
    ...
    // Preserve main's admission gate while allowing adaptive thinking, whose
    // legacy in_think_mode flag is false until the model emits a begin tag.
    const bool uses_think_processor = thinking_mode == ThinkingMode::ENABLED || thinking_mode == ThinkingMode::ADAPTIVE;
    if (!generate_config->enable_think_logits_processor || !uses_think_processor
    ...
    const bool has_think_budget = !end_think_token_ids.empty();
