...
// Canonical kimi_k3 structural tags generated by upstream xgrammar's own
// get_model_structural_tag("kimi_k3", ...) at commit ba00e8bd.
constexpr const char* kKimiK3ReasoningAutoTag = R"KTAG({"type":"structural_tag","format":{"type":"sequence","elements":[{"type":"tag","begin":"","content":{"type":"any_text","excludes":["<|open|>","<|close|>"]},"end":"<|close|>think<|sep|>"},{"type":"tag","begin":"<|open|>response<|sep|>","content":{"type":"any_text","excludes":["<|open|>","<|close|>"]},"end":"<|close|>response<|sep|>"},{"type":"optional","content":{"type":"tag","begin":"<|open|>tools<|sep|>","content":{"type":"tags_with_separator","tags":[{"type":"tag","begin":"<|open|>call tool=\"get_weather\" index=\"","content":{"type":"sequence","elements":[{"type":"regex","pattern":"\\d+"},{"type":"const_string","value":"\"<|sep|>"},{"type":"json_schema","json_schema":{"type":"object","properties":{"q":{"type":"string"}}},"style":"kimi_k3_xml","any_order":false,"max_whitespace_cnt":null}]},"end":"<|close|>call<|sep|>"}],"separator":"","at_least_one":true,"stop_after_first":false},"end":"<|close|>tools<|sep|>"}},{"type":"const_string","value":"<|close|>message<|sep|>"}]}})KTAG";

constexpr const char* kKimiK3NoReasoningAutoTag = R"KTAG({"type":"structural_tag","format":{"type":"sequence","elements":[{"type":"tag","begin":"","content":{"type":"any_text","excludes":["<|open|>","<|close|>"]},"end":"<|close|>response<|sep|>"},{"type":"optional","content":{"type":"tag","begin":"<|open|>tools<|sep|>","content":{"type":"tags_with_separator","tags":[{"type":"tag","begin":"<|open|>call tool=\"get_weather\" index=\"","content":{"type":"sequence","elements":[{"type":"regex","pattern":"\\d+"},{"type":"const_string","value":"\"<|sep|>"},{"type":"json_schema","json_schema":{"type":"object","properties":{"q":{"type":"string"}}},"style":"kimi_k3_xml","any_order":false,"max_whitespace_cnt":null}]},"end":"<|close|>call<|sep|>"}],"separator":"","at_least_one":true,"stop_after_first":false},"end":"<|close|>tools<|sep|>"}},{"type":"const_string","value":"<|close|>message<|sep|>"}]}})KTAG";

constexpr const char* kKimiK3ReasoningNoToolsTag = R"KTAG({"type":"structural_tag","format":{"type":"sequence","elements":[{"type":"tag","begin":"","content":{"type":"any_text","excludes":["<|open|>","<|close|>"]},"end":"<|close|>think<|sep|>"},{"type":"tag","begin":"<|open|>response<|sep|>","content":{"type":"any_text","excludes":["<|open|>","<|close|>"]},"end":"<|close|>response<|sep|>"},{"type":"const_string","value":"<|close|>message<|sep|>"}]}})KTAG";

// Feeds text through the matcher using the RAW single-byte vocab: every char is one token.
bool driveString(RtpGrammarMatcher& matcher, const std::string& text) {
    bool all_accepted = true;
    for (char c : text) {
        all_accepted = matcher.acceptToken(static_cast<uint8_t>(c)) && all_accepted;
    }
    return all_accepted;
}
...
TEST(GrammarLogitsProcessorTest, KimiK3StructuralTagAcceptsReasoningResponseTurn) {
    auto backend  = makeBackend();
    auto compiled = backend.compileNow({"structural_tag", kKimiK3ReasoningNoToolsTag}).compiled;
    ASSERT_TRUE(compiled);

    auto matcher = backend.createMatcher(compiled, false, std::nullopt, /*terminate_without_stop_token=*/true);
    EXPECT_TRUE(driveString(*matcher,
...
TEST(GrammarLogitsProcessorTest, KimiK3StructuralTagAcceptsSingleToolCall) {
    auto backend  = makeBackend();
    auto compiled = backend.compileNow({"structural_tag", kKimiK3ReasoningAutoTag}).compiled;
    ASSERT_TRUE(compiled);
    ...
    EXPECT_TRUE(driveString(*matcher,
                            ...
                            "<|open|>argument key=\"q\" type=\"string\"<|sep|>weather in SF"
                            "<|close|>argument<|sep|>"
                            "<|close|>call<|sep|>"
                            "<|close|>tools<|sep|>"
                            "<|close|>message<|sep|>"));
...
TEST(GrammarLogitsProcessorTest, KimiK3StructuralTagAcceptsParallelToolCalls) {
    auto backend  = makeBackend();
    auto compiled = backend.compileNow({"structural_tag", kKimiK3ReasoningAutoTag}).compiled;
    ASSERT_TRUE(compiled);
...
TEST(GrammarLogitsProcessorTest, KimiK3StructuralTagRejectsMissingThinkClose) {
    auto backend  = makeBackend();
    auto compiled = backend.compileNow({"structural_tag", kKimiK3ReasoningAutoTag}).compiled;
    ASSERT_TRUE(compiled);

    auto matcher = backend.createMatcher(compiled, false, std::nullopt);
    EXPECT_FALSE(driveString(*matcher,
                             "<|open|>response<|sep|>Hello!<|close|>response<|sep|>"
                             "<|close|>message<|sep|>"));
}

TEST(GrammarLogitsProcessorTest, KimiK3StructuralTagRejectsUnknownToolName) {
    auto backend  = makeBackend();
    auto compiled = backend.compileNow({"structural_tag", kKimiK3ReasoningAutoTag}).compiled;
    ASSERT_TRUE(compiled);
...
TEST(GrammarLogitsProcessorTest, KimiK3NoReasoningStartsInsideResponseBlock) {
    auto backend  = makeBackend();
    auto compiled = backend.compileNow({"structural_tag", kKimiK3NoReasoningAutoTag}).compiled;
    ASSERT_TRUE(compiled);
    ...
    auto reopened = backend.createMatcher(compiled, false, std::nullopt);
    EXPECT_FALSE(driveString(*reopened,
                             "<|open|>response<|sep|>Hello!<|close|>response<|sep|>"
                             "<|close|>message<|sep|>"));
}

TEST(GrammarLogitsProcessorTest, KimiK3UnknownXmlStyleIsRejected) {
    auto backend = makeBackend();
    auto result  = backend.compileNow(
        {"structural_tag",
         R"KTAG({"type":"structural_tag","format":{"type":"sequence","elements":[{"type":"tag","begin":"","content":{"type":"json_schema","json_schema":{"type":"object","properties":{"q":{"type":"string"}}},"style":"bogus_xml","any_order":false,"max_whitespace_cnt":null},"end":"<|close|>message<|sep|>"}]}})KTAG"});
    EXPECT_TRUE(result.is_invalid);
    EXPECT_FALSE(result.compiled);
    EXPECT_NE(result.error_message.find("style"), std::string::npos);
}
