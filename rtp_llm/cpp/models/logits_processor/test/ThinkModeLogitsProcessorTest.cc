...
TEST_F(SamplerTest, testNoThinkingMasksThinkBoundaryTokensBeforeSampling) {
    ...
    // Main #1006 admission gate: non-think requests never get a ThinkMode
    // processor, so think boundary tokens stay unmasked for them.
...
TEST_F(SamplerTest, testZeroThinkBudgetMasksThinkBoundaryTokensBeforeSampling) {
    ...
    // Main #1006 admission gate: a zero thinking budget rejects the processor
    // outright instead of masking boundary tokens for the whole request.
    auto processor = ThinkModeLogitsProcessor::fromGenerateInput(generate_input, 1);
    EXPECT_EQ(processor, nullptr);
}
...
TEST_F(SamplerTest, testNoThinkingMasksThinkEndTokenWithoutBeginTokenConfig) {
    ...
    // Main #1006 admission gate: non-think requests never get a processor.
    auto processor = ThinkModeLogitsProcessor::fromGenerateInput(generate_input, 1);
    EXPECT_EQ(processor, nullptr);
}
...
TEST_F(SamplerTest, testZeroThinkBudgetMasksThinkEndTokenWithoutBeginTokenConfig) {
    ...
    // Main #1006 admission gate: zero thinking budget rejects the processor.
    auto processor = ThinkModeLogitsProcessor::fromGenerateInput(generate_input, 1);
    EXPECT_EQ(processor, nullptr);
}
...
TEST_F(SamplerTest, testSpecForceMismatchCapsWithoutMutatingParent) {
    ...
    const int            P     = 3;
    const size_t         W     = SpecLogitsProcessorRequest::bitmaskWordCount(16);
    std::vector<int32_t> draft = {5, 8, 9};
    std::vector<int32_t> bitmask((P + 1) * W, SpecLogitsProcessorRequest::kBitmaskAllowAll);
    ...
    EXPECT_EQ(processor.mtpCapability().mode, MtpProcessorMode::SPEC_VERIFY);
    auto cap_or = processor.prepareSpeculative(request);
    ASSERT_TRUE(cap_or.ok());
    EXPECT_EQ(cap_or.value(), 0);
...
TEST_F(SamplerTest, testSpecAfterThinkMasksThinkBeginAndAllowsThinkEnd) {
    ...
    auto accepted_tokens = [&](int32_t boundary_token) {
        ...
        std::vector<int32_t> bitmask(2 * W, SpecLogitsProcessorRequest::kBitmaskAllowAll);
        ...
        auto result = processor.prepareSpeculative(request);
