...
void TreeLogitsProcessorCSR::process(const SamplerInputs& inputs,
    ...
    auto d_mask_out = device_->allocateBuffer(
        {DataType::TYPE_UINT8,
    ...
    invokeCsrBuildMask<cudaStream_t>(
        ref_info->d_indptr->data<int32_t>(),
...
void TreeLogitsProcessorCSR::updateStatus(const rtp_llm::BufferPtr& new_tokens,
    ...
    for (int j = 0; j < num_new_tokens; ++j) {
        ...
        for (int i = 0; i < batch_size; ++i) {
            ...
            col_offsets_cpu[i] = info.is_beam_search
        ...
        invokeCsrGatherTokens<cudaStream_t>(
            new_tokens->data<int32_t>(),
        ...
        invokeCsrUpdateStates<cudaStream_t>(
            ref_info->d_indptr->data<int32_t>(),
            ref_info->d_packed_csr_tokens->data<int32_t>(),
        ...
        auto updated_cpu = buffer2vector<int32_t>(
            *device_->clone({*d_states_batch_, AllocationType::HOST}));
        for (int i = 0; i < batch_size; ++i) {
            if (!tree_infos_[i].in_tree_mode) {
                continue;
            }
...
TreeLogitsProcessorCSR::fromGenerateInput(rtp_llm::DeviceBase*           device,
    ...
    auto processor_ptr =
        std::make_shared<TreeLogitsProcessorCSR>(DeviceFactory::getDefaultDevice());
    ...
    processor_ptr->d_states_batch_ =
        device->clone({*cpu_states_buf, AllocationType::DEVICE});
