...
void PyWrappedModel::padTensorParallelInputs(torch_ext::PyModelInputs& inputs) {
    ...
    if (attention.linear_replay) {
        RTP_LLM_CHECK_WITH_INFO(attention.linear_replay->slot_ids.numel() == plan.logical_requests,
                                "TP padding requires one LINEAR replay row per logical request");
        attention.linear_replay = attention.linear_replay->padToBatch(plan.physical_requests);
    }
