...
GptModelOutputs PyWrappedModel::forwardMicroBatched(const GptModelInputs& inputs) {
    ...
    for (size_t i = 0; i < split_inputs.size(); ++i) {
        ...
        PyModelInputs py_model_input{token_ids, input_hiddens, py_attn_inputs, bert_embedding_inputs};
        // tbstars: extra_input_ids is batch-level data, shared across all micro batches,
        // so use the full-batch inputs rather than the per-micro-batch slice.
        if (inputs.combo_extra_input_ids.defined() && inputs.combo_extra_input_ids.numel() > 0) {
            py_model_input.extra_input_ids.combo_extra_input_ids   = inputs.combo_extra_input_ids.cuda();
            py_model_input.extra_input_ids.extra_input_ids_lengths = inputs.extra_input_ids_lengths.cuda();
            py_model_input.extra_input_ids.extra_input_ids_locs =
                inputs.extra_input_ids_locs.defined() ? inputs.extra_input_ids_locs.cuda() :
...
GptModelOutputs PyWrappedModel::forward(const GptModelInputs& inputs) {
    ...
    try {
        ...
        // tbstars: forward gathered extra_input_ids (torch::Tensor on GptModelInputs) to the python model.
        if (inputs.combo_extra_input_ids.defined() && inputs.combo_extra_input_ids.numel() > 0) {
            py_model_inputs.extra_input_ids.combo_extra_input_ids   = inputs.combo_extra_input_ids.cuda();
            py_model_inputs.extra_input_ids.extra_input_ids_lengths = inputs.extra_input_ids_lengths.cuda();
