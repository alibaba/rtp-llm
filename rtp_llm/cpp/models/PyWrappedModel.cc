...
bool hasRequestOwnedMultimodalInputs(const GptModelInputs& inputs) {
    ...
    return hasRequestOwnedMultimodalSignals(
        RequestOwnedMultimodalSignals{.multimodal_features = has_multimodal_features,
                                      .multimodal_locs     = has_multimodal_locs,
                                      .multimodal_extra    = has_multimodal_extra,
                                      .text_tokens_mask    = has_text_tokens_mask});
}

bool hasBertEmbeddingTables(const Weights& weights) {
    return weights.position_encoding && weights.token_type_embedding
           && hasBothBertEmbeddingTables(weights.position_encoding->kernel, weights.token_type_embedding->kernel);
}

bool hasBertRequestOwnedMultimodalInputs(const GptModelInputs& inputs, const Weights& weights) {
    return hasBertEmbeddingTables(weights) && hasRequestOwnedMultimodalInputs(inputs);
}
...
void PyWrappedModel::prepareAttentionInputs(const GptModelInputs& inputs, bool skip_forward_event_sync) {
    ...
    if (enable_cuda_graph_ && !hasRequestOwnedMultimodalInputs(inputs)) {
        ...
        auto bert_embedding_inputs =
            hasBertEmbeddingTables(weights_) ? buildBertEmbeddingInputs(inputs) : torch_ext::BertEmbeddingInputs();
...
MicroBatchPlan PyWrappedModel::planMicroBatches(const GptModelInputs& inputs) {
    ...
    // Intentionally Bert-only: request-owned multimodal metadata on this new
    // path is a whole-request payload and is not partitioned by the two-slot
    // layer-micro-batch protocol. Disable a real two-slot plan and use the
    // regular fallback, which reuses the full request for the fake slot. Keep
    // historical non-Bert micro-batch behavior outside this PR's scope.
