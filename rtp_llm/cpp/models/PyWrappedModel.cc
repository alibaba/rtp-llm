...
void PyWrappedModel::setMtpChunkPrefillRoundHook(std::function<void(py::object, bool)> hook) {
    mtp_chunk_prefill_round_hook_fn_ = std::move(hook);
}
...
std::optional<PyCacheStoreInputs> PyWrappedModel::prepareWriteCacheParams(const GptModelInputs& inputs) {
    ...
    if (!inputs.warmup && inputs.pd_separation) {
        ...
        const bool has_cache_store_length_override = inputs.cache_store_input_lengths_host.defined()
                                                     || inputs.cache_store_prefix_lengths_host.defined();
        RTP_LLM_CHECK_WITH_INFO(!has_cache_store_length_override
                                    || (inputs.cache_store_input_lengths_host.defined()
                                        && inputs.cache_store_prefix_lengths_host.defined()),
                                "cache-store input/prefix length overrides must be defined together");
        auto input_lengths_host = has_cache_store_length_override ?
                                      inputs.cache_store_input_lengths_host :
                                      (inputs.input_lengths_host_for_log.defined() ?
                                           inputs.input_lengths_host_for_log :
                                           async_to_pinned_host(inputs.input_lengths));
        auto prefix_lengths_host = has_cache_store_length_override ?
                                       inputs.cache_store_prefix_lengths_host :
                                       (inputs.prefix_lengths_host_for_log.defined() ?
        ...
        RTP_LLM_CHECK_WITH_INFO(input_lengths_host.numel() == inputs.input_lengths.numel()
                                ...
                                "cache-store length override size mismatch: input=%ld/%ld prefix=%ld/%ld",
...
GptModelOutputs PyWrappedModel::forward(const GptModelInputs& inputs) {
    ...
    try {
        ...
        if (enable_cuda_graph_ && graph_runner_->canRun(py_model_inputs, graph_state_)) {
            ...
            auto outputs          = mtp_chunk_prefill_round_hook_fn_ ?
                                        py_model_forward(py_model_inputs,
                                                         held_attn_pyobj_,
                                                         py::cpp_function(mtp_chunk_prefill_round_hook_fn_)) :
