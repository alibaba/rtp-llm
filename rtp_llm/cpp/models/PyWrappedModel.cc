...
struct TensorParallelPaddingPlan {
    ...
    int64_t coordinated_requests{0};
    int64_t physical_requests{0};
    int64_t logical_tokens{0};
    int64_t coordinated_tokens{0};
...
buildTensorParallelPaddingPlan(const torch_ext::PyModelInputs& inputs, int64_t tp_size) {
    ...
    plan.coordinated_requests = attention.input_lengths.defined() ? attention.input_lengths.numel() : 0;
    plan.coordinated_tokens = inputs.input_ids.defined() && inputs.input_ids.numel() > 0 ?
                                  inputs.input_ids.numel() :
                                  (inputs.input_hiddens.defined() && inputs.input_hiddens.dim() > 0 ?
                                       inputs.input_hiddens.size(0) :
                                       0);
    RTP_LLM_CHECK_WITH_INFO(
        plan.coordinated_requests > 0 && plan.coordinated_tokens > 0,
        "TP padding requires non-empty coordinated requests and tokens: requests=%ld tokens=%ld",
        plan.coordinated_requests,
        plan.coordinated_tokens);

    // A DP/KTP planner may have padded the request domain first. Preserve its
    // real counts instead of reclassifying coordinated dummy rows as logical.
    const bool coordinated_layout_published = attention.coordinated_request_count > 0;
    if (coordinated_layout_published) {
        RTP_LLM_CHECK_WITH_INFO(
            attention.coordinated_request_count == plan.coordinated_requests
                && attention.coordinated_token_count == plan.coordinated_tokens,
            "published coordinated layout does not match model inputs: "
            "requests=%d/%ld tokens=%d/%ld",
            attention.coordinated_request_count,
            plan.coordinated_requests,
            attention.coordinated_token_count,
            plan.coordinated_tokens);
    }
    plan.logical_requests = coordinated_layout_published ? attention.logical_request_count : plan.coordinated_requests;
    plan.logical_tokens   = coordinated_layout_published ? attention.logical_token_count : plan.coordinated_tokens;
    RTP_LLM_CHECK_WITH_INFO(
        plan.logical_requests >= 0 && plan.logical_requests <= plan.coordinated_requests
            && plan.logical_tokens >= 0 && plan.logical_tokens <= plan.coordinated_tokens,
        "invalid coordinated layout: logical_requests=%ld coordinated_requests=%ld "
        "logical_tokens=%ld coordinated_tokens=%ld",
    ...
    if (plan.prefill) {
        plan.physical_tokens      = alignUp(plan.coordinated_tokens, tp_size);
        plan.padding_tokens       = plan.physical_tokens - plan.coordinated_tokens;
        plan.padding_requests     = plan.padding_tokens > 0 ? 1 : 0;
        plan.physical_requests    = plan.coordinated_requests + plan.padding_requests;
        plan.dummy_request_tokens = plan.padding_tokens;
    } else {
        RTP_LLM_CHECK_WITH_INFO(
            plan.coordinated_tokens % plan.coordinated_requests == 0,
            "decode-like TP padding requires a fixed token width: requests=%ld tokens=%ld",
            plan.coordinated_requests,
            plan.coordinated_tokens);
        const int64_t tokens_per_request = plan.coordinated_tokens / plan.coordinated_requests;
        const int64_t request_alignment  = tp_size / std::gcd(tp_size, tokens_per_request);
        plan.physical_requests           = alignUp(plan.coordinated_requests, request_alignment);
        plan.padding_requests            = plan.physical_requests - plan.coordinated_requests;
        plan.dummy_request_tokens        = tokens_per_request;
        plan.padding_tokens              = plan.padding_requests * tokens_per_request;
        plan.physical_tokens             = plan.coordinated_tokens + plan.padding_tokens;
    }
...
void PyWrappedModel::padTensorParallelInputs(torch_ext::PyModelInputs& inputs) {
    ...
    inputs.combo_position_ids =
        appendTokenValues(inputs.combo_position_ids, plan.coordinated_tokens, plan.padding_tokens, 0);
    ...
    inputs.embedding_inputs.combo_tokens_type_ids = appendTokenValues(
        inputs.embedding_inputs.combo_tokens_type_ids, plan.coordinated_tokens, plan.padding_tokens, 0);
    ...
    inputs.embedding_inputs.text_tokens_mask =
        appendTokenValues(inputs.embedding_inputs.text_tokens_mask, plan.coordinated_tokens, plan.padding_tokens, 1);
    inputs.bert_embedding_inputs.combo_position_ids = appendTokenValues(
        inputs.bert_embedding_inputs.combo_position_ids, plan.coordinated_tokens, plan.padding_tokens, 0);
    inputs.bert_embedding_inputs.combo_tokens_type_ids = appendTokenValues(
        inputs.bert_embedding_inputs.combo_tokens_type_ids, plan.coordinated_tokens, plan.padding_tokens, 0);
    ...
    RTP_LLM_LOG_DEBUG("tensor-parallel boundary padding: prefill=%d target_verify=%d "
                      "requests=%ld/%ld/%ld tokens=%ld/%ld/%ld",
                      plan.prefill,
