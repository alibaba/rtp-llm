...
torch::Tensor PyWrappedModel::customOutputIndexes(const GptModelInputs& inputs) {
    ...
    TORCH_CHECK(indexes.defined() && indexes.dim() == 1 && indexes.size(0) <= context_batch_size,
                "custom output indexes must contain at most one row per context request");
...
void PyWrappedModel::initializeCustomOutput() {
    ...
    const auto stage = py::hasattr(handler, "hidden_state_stage") ?
                           py::cast<std::string>(handler.attr("hidden_state_stage")()) :
                           "post_final_norm";
...
torch::Tensor PyWrappedModel::runCustomOutput(const torch::Tensor& rows) {
    ...
    TORCH_CHECK(output.size(0) == rows.size(0), "custom output must return one row per context request");
...
GptModelOutputs PyWrappedModel::forwardPostLayers(torch::Tensor         hidden,
    ...
    const bool run_custom = custom_output_enabled_ && has_context_request
