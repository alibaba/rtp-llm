...
torch::Tensor PositionIdsGenerator::generatePositionIds(int32_t                         input_len,
                                                        PositionIdsStyle                style,
                                                        optional<torch::Tensor>         mm_locs,
                                                        optional<vector<torch::Tensor>> mm_position_ids) {
    torch::Tensor res;
    int32_t*      position_ids;
    switch (style) {
        case PositionIdsStyle::DEFAULT: {
            res          = torch::empty({(int64_t)input_len}, torch::kInt32);
            position_ids = res.data_ptr<int32_t>();
        ...
        case PositionIdsStyle::MMWITHTAG: {
            ...
            if (!mm_locs && !mm_position_ids) {
                ...
                int32_t* mm_loc_data = mm_locs.value().data_ptr<int32_t>();
                auto     mm_pos_ids  = mm_position_ids.value();
                while (now_pos < input_len) {
                    if (mm_idx >= mm_locs.value().numel() || now_pos < mm_loc_data[mm_idx]) {
                        position_ids[now_pos++] = now_id++;
                    } else {
                        auto* mm_pos_data = mm_pos_ids[mm_idx].data_ptr<int32_t>();
                        for (int32_t i = 0; i < mm_pos_ids[mm_idx].numel(); ++i) {
                            position_ids[now_pos + i] = mm_pos_data[i] + now_id;
                        }
                        now_pos += mm_pos_ids[mm_idx].numel();
        ...
        case PositionIdsStyle::MROPE: {
            res             = torch::empty({(int64_t)input_len, 3}, torch::kInt32);
            position_ids    = res.data_ptr<int32_t>();
            int32_t now_pos = 0, now_id = 0, mm_idx = 0;
            ...
            if (!mm_locs && !mm_position_ids) {
                ...
                while (now_pos < input_len) {
                    if (mm_idx >= mm_locs.value().numel() || now_pos < mm_loc_data[mm_idx]) {
                        ...
                        int32_t feature_len = mm_pos_ids[mm_idx].size(0);
                        for (int32_t i = 0; i < feature_len; ++i) {
                            int32_t  now_pos_tmp              = now_pos + i;
                            int32_t* mm_pos_id                = mm_pos_ids[mm_idx].data_ptr<int32_t>();
...
void PositionIdsGenerator::generateNextPositionId(int32_t*         now_pos,
    ...
    switch (style) {
        case PositionIdsStyle::DEFAULT:
            ...
            break;
        case PositionIdsStyle::MMWITHTAG:
            context_len = context_pos_ids.numel();
            now_pos[0]  = context_pos_ids.data_ptr<int32_t>()[context_len - 1] + now_len - context_len;
            break;
        case PositionIdsStyle::MROPE: {
            context_len              = context_pos_ids.numel();
            int32_t* context_pos     = context_pos_ids.data_ptr<int32_t>();
