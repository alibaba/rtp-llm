...
void IContextParallelProcessor::handleInputs(GptModelInputs&                     model_input,
    ...
    auto sequence_lengths = model_input.sequence_lengths.is_cuda() ? model_input.sequence_lengths.cpu().pin_memory() :
    ...
    if (need_token_remap) {
        ...
        auto remap_token_field = [&](torch::Tensor& field) {
            ...
            field = out.is_pinned() ? out : out.pin_memory();
        };
        ...
        if (model_input.multimodal_features.has_value() && !model_input.multimodal_features.value().empty()
            ...
            if (new_locs.empty()) {
                ...
                auto locs_cpu = torch::from_blob(new_locs.data(),
                                    ...
                                    .clone()
                                    .pin_memory();
                model_input.mm_features_locs = locs_cpu;
            }
        }
    }
    ...
    model_input.sequence_lengths = sequence_lengths;
