...
void remapAlignedInputs(GptModelInputs&             model_input,
    ...
    if (model_input.v41_token_types.defined()) {
        auto remap_v41 = [&](torch::Tensor& field, int64_t width, int64_t padding, c10::ScalarType dtype) {
            RTP_LLM_CHECK_WITH_INFO(field.defined() && field.is_contiguous() && field.scalar_type() == dtype
                                        && field.dim() == (width == 1 ? 1 : 2)
                                        && field.size(0) == remap.global_token_num
                                        && field.numel() == remap.global_token_num * width,
                                    "V4.1 CP metadata must align with global canonical rows");
            auto source = field.is_cuda() ? field.cpu() : field;
            auto shape =
                width == 1 ? std::vector<int64_t>{local_token_num} : std::vector<int64_t>{local_token_num, width};
            auto output = torch::full(shape, padding, source.options());
            if (global_token_num > 0 && local_token_num > 0) {
                auto indices = remap.select_indices.clone();
                indices.masked_fill_(remap.valid_mask.logical_not(), 0);
                output    = source.index_select(0, indices).contiguous();
                auto mask = width == 1 ? remap.valid_mask.logical_not() : remap.valid_mask.logical_not().unsqueeze(1);
                output.masked_fill_(mask, padding);
            }
            field = output.is_pinned() ? output : output.pin_memory();
        };
        remap_v41(model_input.v41_token_types, 1, -1, torch::kInt32);
        remap_v41(model_input.v41_token_valid, 1, 0, torch::kBool);
        remap_v41(model_input.engram_history_ids, 3, 0, torch::kInt32);
        remap_v41(model_input.engram_history_valid, 3, 0, torch::kBool);
    }
...
void IContextParallelProcessor::handleInputs(GptModelInputs&                     model_input,
    ...
    const bool need_token_remap = model_input.v41_token_types.defined() || model_input.text_tokens_mask.defined()
