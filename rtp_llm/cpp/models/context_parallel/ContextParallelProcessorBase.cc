...
void IContextParallelProcessor::handleInputs(GptModelInputs&                     model_input,
    ...
    // Do not infer validity from divisibility: an exactly divisible stale
    // tensor is just as wrong as a non-divisible one.  Multimodal requests
    // retain their explicit position-id semantics and stay strict.
    // Keep multi-axis (M-ROPE) position ids even for a text-only request: the
    // model configuration can require three axes without any multimodal
    // features in this particular request.  A one-axis tensor is the only
    // form that can be safely regenerated by the prefix-aware synthesis below.
    const bool has_one_axis_position_ids = model_input.combo_position_ids.defined()
        && global_token_num > 0 && model_input.combo_position_ids.numel() == global_token_num;
    if (!has_multimodal_input && has_prefix_reuse && has_one_axis_position_ids) {
        RTP_LLM_LOG_WARNING(
