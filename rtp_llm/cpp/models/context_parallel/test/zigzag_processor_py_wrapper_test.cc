...
torch::Tensor zigzagRestoreRankChunks(const torch::Tensor& rank_chunks,
    ...
    RTP_LLM_CHECK_WITH_INFO(
        rank_chunks.dim() == 3, "rank_chunks must be [cp_size, local_tokens, hidden], got dim=%ld", rank_chunks.dim());
    ...
    auto gather_to_output =
        ZigZagProcessorTestWrapper::buildGatherToOutputIndices(restore_indices, padding_mask, num_valid_tokens);
    ...
    for (int64_t chunk_offset = 0; chunk_offset < local_token_num; chunk_offset += chunk_rows) {
        ...
        ZigZagProcessorTestWrapper::restoreGatheredChunk(
            restored_padded, gathered_chunk, gather_to_output, local_token_num, chunk_offset, cp_size);
    }
