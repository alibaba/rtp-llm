...
public:
    explicit RoundRobinProcessorTestWrapper(int page_size = 1): RoundRobinProcessor(page_size) {}
...
roundRobinProcessorPlanWrapper(const std::vector<int>& total_input_tokens,
    ...
    RoundRobinProcessorTestWrapper processor(page_size);
...
torch::Tensor
roundRobinGenerateQKVRestoreIndices(const torch::Tensor& prefill_cp_chunk_lengths, int cp_size, int page_size = 1) {
    RoundRobinProcessorTestWrapper processor(page_size);
...
PYBIND11_MODULE(libth_round_robin_py_wrapper_test, m) {
    m.def("round_robin_plan",
          ...
          py::arg("page_size") = 1,
          "Distribute input tokens across context parallel ranks with page-level round-robin pattern");
