...
GptModelInputShapeHints getModelInputShapeHints(const GptModelInputs& inputs) {
    ...
    // Both hints travel: pdSeparation gated cache_keys in the packed tpSync
    // broadcast (PP root/non-root plans differ in size), isTargetVerify is the
    // official MTP speculation flag. Neither may default on the receiver.
    shape_hints[GptModelInputIndex::pdSeparation]  = inputs.pd_separation ? 1 : 0;
...
void tpSyncModelInputs(GptModelInputs& inputs, const ParallelismConfig& parallelism_config) {
    ...
    if (kLogTpsync) {
        RTP_LLM_LOG_INFO("[TPSYNC] rank=%d skip=%d pd_sep=%d tensors=%ld cpu_bytes=%lld gpu_bytes=%lld "
