...
void tpSyncModelInputs(GptModelInputs& inputs, const ParallelismConfig& parallelism_config) {
    ...
    // All ranks configure the same tag set before entering request collectives.
    // Only its order may differ; the numeric wire format cannot compare identities.
    ...
    // Receiver checks must happen after the agreed broadcasts, not between them.
