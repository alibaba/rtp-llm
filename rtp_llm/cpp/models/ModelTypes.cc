...
GptModelInputShapeHints getModelInputShapeHints(const GptModelInputs& inputs) {
    ...
    shape_hints[GptModelInputIndex::mmFeaturesSpans] =
        inputs.mm_features_spans.defined() ? inputs.mm_features_spans.numel() : 0;
...
void tpSyncModelInputs(GptModelInputs& inputs, const ParallelismConfig& parallelism_config) {
    ...
    if (is_non_root) {
        ...
        if (combo_position_ids_size) {
            ...
            // Discard positions generated by the previous CP target forward.
        ...
        if (mm_features_spans_size) {
            RTP_LLM_CHECK_WITH_INFO(mm_features_spans_size % 3 == 0, "multimodal spans must have three columns");
            inputs.mm_features_spans = allocBuf(rtp_llm::DataType::TYPE_INT64, {mm_features_spans_size / 3, 3});
        } else {
            inputs.mm_features_spans = torch::Tensor();
        }
        if (mm_features_num) {
    ...
    if (has_v41) {
        collect(inputs.v41_token_types);
