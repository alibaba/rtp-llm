...
public:
    ...
    // The caller must keep the Python interpreter alive and stop publication
    // before shutdown. These methods acquire the GIL for their Python calls.
    // The publisher destructor intentionally performs no Python calls.
    // Waits for the current store work without closing the store.
    bool flushPending();
...
private:
    ...
    static decltype(auto) perform(FailureStage       failure_stage,
    ...
    bool        observeAsyncErrors(FailureStage failure_stage, const char* phase);
