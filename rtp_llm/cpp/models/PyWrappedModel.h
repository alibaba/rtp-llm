...
private:
    ...
    // DSpark/DFlash draft: its CUDA graph captures the full forward including
    // the lm_head + Markov + softmax tail (vLLM FULL-graph boundary).  Draft
    // gate = is_dspark_ && !use_spec_decoding_ (not the target-verify graph).
