...
fp8x4_e4m3_to_bfloat2(__nv_bfloat162* out1, __nv_bfloat162* out2, __nv_fp8x4_e4m3 const* in) {
...
template<typename T_OUT, typename T_S, typename T_IN>
void invokeDequantizeMatrix(T_OUT*       output,
                            T_S const*   input_qua_amax_ptr,
                            T_IN const*  input,
                            int64_t      numel,
                            int64_t      lda,
...
void invokeComputeFP8QuantizeScale(T_S*          quant_ptr,
                                   const T_W*    weights,
                                   const int64_t k,
                                   const int64_t lda,
                                   QuantizeMode  quantize_mode,
...
template<typename T_OUT, typename T_S, typename T_IN>
void invokeComputeScalesAndQuantizeMatrix(T_OUT*        output,
                                          T_S*          quant_ptr,
                                          const T_IN*   weights,
                                          const int64_t numel,
                                          const int64_t lda,
