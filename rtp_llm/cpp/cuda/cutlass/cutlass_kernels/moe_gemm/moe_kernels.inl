...
__global__ void expandInputRowsKernel(T const*       unpermuted_input,
                                      T*             permuted_output,
...
template<typename T>
void expandInputRowsKernelLauncher(T const*       unpermuted_input,
                                   T*             permuted_output,
                                   float*         unpermuted_scales,
                                   float*         permuted_scales,
                                   int const*     expanded_dest_row_to_expanded_source_row,
...
__global__ void doGatedActivationKernel(T*                output,
                                        OutputType const* gemm_result,
                                        int64_t const*    num_valid_tokens_ptr,
                                        int64_t           inter_size) {
    ...
    for (int64_t elem_index = start_offset; elem_index < num_elems_in_col; elem_index += stride) {
        // gemm_result layout: [gate | up], gate is first half, up is second half
        ...
        auto up_value          = arrayConvert<DataElem, ComputeElem>(gemm_result_vec[elem_index + inter_size_vec]);
        auto gate_act          = fn(gate_value);
        output_vec[elem_index] = arrayConvert<ComputeElem, DataElem>(up_value * gate_act);
    }
}

template<typename T, typename OutputType>
void doGatedActivation(T*                output,
                       OutputType const* gemm_result,
                       int64_t const*    num_valid_tokens_ptr,
                       int64_t           inter_size,
                       int64_t           num_tokens,
...
__global__ void doActivationKernel(T*                    output,
...
void doActivation(T*                    output,
    ...
    fn<<<blocks, threads, 0, stream>>>(output,
                                       gemm_result,
                                       fp8_quant,
                                       bias,
                                       bias_is_broadcast,
...
__global__ void loraAddBiasKernel(ScaleBiasType*       output,
                                  LoraType const*      lora_result,
                                  ScaleBiasType const* bias,
                                  int64_t const*       num_valid_tokens_ptr,
...
void loraAddBias(ScaleBiasType*       output,
                 LoraType const*      lora_result,
                 ScaleBiasType const* bias,
                 int64_t const*       num_valid_tokens_ptr,
                 int64_t              inter_size,
...
__device__ void computeHopperInputPointers(HopperGroupedGemmInput layout_info,
                                           int64_t                gemm_m,
                                           int64_t                gemm_n,
                                           int64_t                gemm_k,
                                           int                    num_tokens_before_expert,
                                           int64_t                expert,
                                           T const*               in,
...
computeHopperInputStrides(HopperGroupedGemmInput layout_info, int gemm_m, int gemm_n, int gemm_k, int64_t out_idx) {
...
template<class T, class WeightType, class OutputType>
__global__ void computeStridesHopperKernel(int64_t const*         expert_first_token_offset,
                                           HopperGroupedGemmInput layout_info,
                                           int64_t                gemm_n,
                                           int64_t                gemm_k,
...
template<class T,
         ...
         class ScaleBiasType,
         class Enable>
void CutlassMoeFCRunner<T, WeightType, QuantOp, OutputType, ScaleBiasType, Enable>::configureWsPtrs(
    char*                           ws_ptr,
    int64_t const                   num_rows,
    int64_t const                   hidden_size,
    int64_t const                   inter_size,
...
void CutlassMoeFCRunner<T, WeightType, QuantOp, OutputType, ScaleBiasType, Enable>::gemm1(
    MoeGemmRunner<T, WeightType, QuantOp, OutputType, ScaleBiasType>& gemm_runner,
    T const* const                                                    input,
    T* const                                                          output,
    void* const                                                       intermediate_result,
    int64_t const* const                                              expert_first_token_offset,
    HopperGroupedGemmInput const                                      hopper_input_template,
    ...
    ScaleBiasType const* const                                        fc1_int_scales,
    ScaleBiasType const* const                                        fc1_int_zeros,
    int                                                               group_size,
    float const* const                                                fc1_fp8_dequant,
    ...
    float const**                                                     alpha_scale_ptr_array,
    bool                                                              bias_is_broadcast,
    cudaStream_t                                                      stream,
    ...
    if (using_hopper_gemm1) {
        ...
        hopper_input        = computeStridesHopper(expert_first_token_offset,
                                            hopper_input,
                                            fc1_out_size,
                                            hidden_size,
                                            ...
                                            fc1_fp8_dequant,
                                            nullptr,
                                            static_cast<UnfusedGemmOutputType*>(gemm_output),
                                            stream);
        check_cuda_error();

        gemm_runner.moeGemm(input,
                            nullptr,
                            nullptr,
                            nullptr,
                            group_size,
        ...
        doActivation<T, UnfusedGemmOutputType>(output,
        ...
        TLLM_CHECK(!use_ampere_activation_fusion);
        TLLM_CHECK(!config.is_sm90);
        gemm_runner.moeGemmBiasAct(input,
                                   fc1_expert_weights,
                                   fc1_int_scales,
                                   fc1_int_zeros,
                                   group_size,
...
void CutlassMoeFCRunner<T, WeightType, QuantOp, OutputType, ScaleBiasType, Enable>::gemm2(
    MoeGemmRunner<T, WeightType, QuantOp, OutputType, ScaleBiasType>& gemm_runner,
    T const* const                                                    input,
    void* const                                                       gemm_output,
    OutputType* const                                                 final_output,
    int64_t const* const                                              expert_first_token_offset,
    HopperGroupedGemmInput const                                      hopper_input_template,
    WeightType const* const                                           fc2_expert_weights,
    ScaleBiasType const* const                                        fc2_expert_biases,
    ScaleBiasType const* const                                        fc2_int_scales,
    ScaleBiasType const* const                                        fc2_int_zeros,
    int                                                               group_size,
    float const* const                                                fc2_fp8_dequant,
    ...
    if (using_hopper_gemm2) {
        ...
        if (using_hopper_fused_finalize) {
            ...
            hopper_input.setFinalizeFusionParams(final_output,
                                                 token_topk_permuted_scales,
                                                 expert_first_token_offset,
                                                 expanded_dest_row_to_expanded_source_row,
                                                 apply_bias ? fc2_expert_biases : nullptr,
                                                 hidden_size,
                                                 num_rows);
    ...
    gemm_runner.moeGemmBiasAct(input,
                               ...
                               fc2_int_zeros,
                               group_size,
                               fuse_lora_bias ? static_cast<ScaleBiasType const*>(fc2_lora) : nullptr,
                               false,
                               gemm_output,
                               total_tokens_including_expert,
                               hopper_input,
    ...
    if (use_lora && !fuse_lora_bias) {
        ...
        loraBiasApplyFunc(static_cast<UnfusedGemmOutputType*>(gemm_output),
                          static_cast<UnfusedGemmOutputType const*>(gemm_output),
                          nullptr,
                          static_cast<ScaleBiasType const*>(fc2_lora),
                          false,
                          expert_first_token_offset,
                          num_experts_per_node,
                          hidden_size,
    ...
    if (has_different_output_type_ampere || has_different_output_type_hopper) {
        ...
        finalizeMoeRoutingKernelLauncher<T, OutputType, T>(static_cast<T const*>(gemm_output),
                                                           final_output,
                                                           fc2_expert_biases,
                                                           token_topk_unpermuted_scales,
                                                           expanded_source_row_to_expanded_dest_row,
...
void CutlassMoeFCRunner<T, WeightType, QuantOp, OutputType, ScaleBiasType, Enable>::runMoe(
    ...
    TLLM_CHECK_WITH_INFO(num_rows == active_rows,
    ...
    TLLM_CHECK_WITH_INFO(num_rows * num_experts <= std::numeric_limits<int>::max(),
    ...
    sortAndScanSoftmaxOutput(expert_for_source_row,
                             source_rows_,
                             permuted_experts_,
                             permuted_rows_,
    ...
    if (use_lora) {
        ...
        TLLM_CUDA_CHECK(cudaMemcpyAsync(host_permuted_rows.data(),
                                        permuted_rows_,
                                        expanded_num_rows * sizeof(int),
                                        cudaMemcpyDeviceToHost,
                                        stream));
        host_expert_first_token_offset.resize(num_experts_per_node + 1);
        TLLM_CUDA_CHECK(cudaMemcpyAsync(host_expert_first_token_offset.data(),
                                        expert_first_token_offset_,
