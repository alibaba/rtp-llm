...
int get_bits_in_quant_type(QuantType quant_type)
{
    ...
    default: RTP_LLM_FAIL("Invalid quant_type"); return -1;
    }
}
...
LayoutDetails getLayoutDetailsForTransform(QuantType quant_type, const int arch)
{
    ...
    else if (arch >= 80 && arch <= 90)
    {
        ...
        RTP_LLM_FAIL("Unsupported Arch: %d", arch);
    ...
    const std::vector<size_t>& shape, QuantType quant_type)
{
    ...
    {
        RTP_LLM_FAIL("Invalid quant_tye");
    }
}
...
void add_bias_and_interleave_int8s_inplace(int8_t* int8_tensor, const size_t num_elts)
{
    ...
    RTP_LLM_CHECK_WITH_INFO(num_elts % 4 == 0, "Dimensions of int8 tensor must be a multiple of 4 for register relayout");
...
void add_bias_and_interleave_int4s_inplace(int8_t* packed_int4_tensor, const size_t num_elts)
{
    ...
    RTP_LLM_CHECK_WITH_INFO(num_bytes % 4 == 0, "Dimensions of int4 tensor must be a multiple of 8 for register relayout");
...
void add_bias_and_interleave_quantized_tensor_inplace(int8_t* tensor, const size_t num_elts, QuantType quant_type)
{
    ...
    {
        RTP_LLM_FAIL("Invalid quantization type for interleaving.");
    }
}
    ...
    const std::vector<size_t>& shape, QuantType quant_type, LayoutDetails details)
{
    ...
    RTP_LLM_CHECK_WITH_INFO(!(num_rows % elts_in_int32),
    ...
    RTP_LLM_CHECK_WITH_INFO(!(num_rows % rows_per_tile),
...
void symmetric_quantize(int8_t* processed_quantized_weight, int8_t* unprocessed_quantized_weight,
    ComputeType* scale_ptr, const WeightType* input_weight_ptr, const std::vector<size_t>& shape, QuantType quant_type, const int arch)
{
...
template void symmetric_quantize<half, float>(
    int8_t*, int8_t*, half*, const float*, const std::vector<size_t>&, QuantType, const int arch);

template void symmetric_quantize<half, half>(
    int8_t*, int8_t*, half*, const half*, const std::vector<size_t>&, QuantType, const int arch);

#ifdef ENABLE_BF16
template void symmetric_quantize<__nv_bfloat16, __nv_bfloat16>(
    int8_t*, int8_t*, __nv_bfloat16*, const __nv_bfloat16*, const std::vector<size_t>&, QuantType, const int arch);

template void symmetric_quantize<__nv_bfloat16, float>(
    int8_t*, int8_t*, __nv_bfloat16*, const float*, const std::vector<size_t>&, QuantType, const int arch);
#endif
