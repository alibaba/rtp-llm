...
struct GemmCfg {
    ...
    GemmCfg(CutlassGemmConfig gemm_config, float time): gemm_config(gemm_config), time(time) {}
...
inline std::string convert_config_to_string(int block_m, int block_n, int block_k, int warp_m, int warp_n, int warp_k) {
    std::string ctashape =
        "CtaShape" + std::to_string(block_m) + "x" + std::to_string(block_n) + "x" + std::to_string(block_k);
    std::string wrapshape =
        "WarpShape" + std::to_string(warp_m) + "x" + std::to_string(warp_n) + "x" + std::to_string(warp_k);
...
inline void print_config_file(
    CutlassGemmConfig config, int m, int n, int k, float time, const char* file, std::ios::openmode open_mode) {
    TileConfig    tile_config = get_tile_config_from_config(config.tile_config);
    std::ofstream outFile(file, open_mode);
    std::string   config_to_string = convert_config_to_string(tile_config.block_m,
                                                            tile_config.block_n,
                                                            tile_config.block_k,
                                                            tile_config.warp_m,
                                                            tile_config.warp_n,
                                                            tile_config.warp_k);
    if (outFile) {
        outFile << "{{" << m << "," << n << "," << k << "}, {CutlassGemmConfig(" << config_to_string << ","
                << config.split_k_factor << "," << config.stages << "), " << time << "}}," << std::endl;
    } else {
...
inline void print_config_file(
    CutlassGemmConfig config, int m, int n, int k, int e, float time, const char* file, std::ios::openmode open_mode) {
    TileConfig    tile_config = get_tile_config_from_config(config.tile_config);
    std::ofstream outFile(file, open_mode);
    std::string   config_to_string = convert_config_to_string(tile_config.block_m,
                                                            tile_config.block_n,
                                                            ...
                                                            tile_config.warp_k);
    if (outFile) {
        outFile << "{{" << m << "," << n << "," << k << "," << e << "}, {CutlassGemmConfig(" << config_to_string << ","
                << config.split_k_factor << "," << config.stages << "), " << time << "}}," << std::endl;
    } else {
        throw std::runtime_error(std::string("[ERROR] Cannot open file: ") + file + "\n");
    }
