...
TEST_F(CudaCheckAndResetNANTest, TestFP8_KVCachePrefill_RemainderTailDetection) {
    ...
    ASSERT_EQ(cudaMalloc(&d_block_ids, h_block_ids.size() * sizeof(int32_t)), cudaSuccess);
    ...
    ASSERT_EQ(cudaMalloc(&d_prefix_lengths, batch_size * sizeof(int32_t)), cudaSuccess);
    ASSERT_EQ(cudaMalloc(&d_input_lengths, batch_size * sizeof(int32_t)), cudaSuccess);
...
TEST_F(CudaCheckAndResetNANTest, TestFP8_ExpFNotNan_NotReset_And_NaN_Reset) {
    ...
    ASSERT_EQ(cudaMalloc(&d_kv_cache_block_id, batch_size * max_blocks_per_batch * sizeof(int32_t)), cudaSuccess);
    cudaMemcpy(d_kv_cache_block_id,
               h_kv_cache_block_id.data(),
               batch_size * max_blocks_per_batch * sizeof(int32_t),
               cudaMemcpyHostToDevice);
    ...
    ASSERT_EQ(cudaMalloc(&d_seq_len_cu, batch_size * sizeof(int32_t)), cudaSuccess);
    cudaMemcpy(d_seq_len_cu, seq_len_cu.data(), batch_size * sizeof(int32_t), cudaMemcpyHostToDevice);
    float* d_nan_flag = nullptr;
    ASSERT_EQ(cudaMalloc(&d_nan_flag, batch_size * sizeof(float)), cudaSuccess);
    ...
    ASSERT_EQ(cudaMalloc(&d_sequence_lengths, batch_size * sizeof(int32_t)), cudaSuccess);
    cudaMemcpy(d_sequence_lengths, sequence_lengths.data(), batch_size * sizeof(int32_t), cudaMemcpyHostToDevice);
    cudaMemset(d_nan_flag, 0, batch_size * sizeof(float));
...
TEST_F(CudaCheckAndResetNANTest, TestFloat_KVCacheDecode_MultiWarpNanDetection) {
    ...
    ASSERT_EQ(cudaMalloc(&d_seq_lens, batch_size * sizeof(int32_t)), cudaSuccess);
