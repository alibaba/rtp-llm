...
void cublasAlgoMap::loadGemmConfig() {
    ...
    if (fd == NULL) {
        if (!log_flag) {
            RTP_LLM_LOG_WARNING("%s is not found; using default GEMM algo", config_filename_.c_str());
            log_flag = true;
        }
        return;
    }
    ...
    if (!fgets(tmp, 1024, fd)) {
        RTP_LLM_LOG_ERROR("fgets fail at %s:%d \n", __FILE__, __LINE__);
        exit(-1);
    }
    while (fscanf(fd,
        ...
        if (dataType != FLOAT_DATATYPE && dataType != HALF_DATATYPE && dataType != BFLOAT16_DATATYPE
            ...
            RTP_LLM_LOG_WARNING("[readAlgoFromConfig] wrong dataType %d!\n", dataType);
            continue;
        }
...
void cublasAlgoMap::loadSpGemmConfig() {
    ...
    if (fd == NULL) {
        RTP_LLM_LOG_WARNING("%s is not found; using SPGEMM algo id 0\n", sp_config_filename_.c_str());
        return;
    }
    sp_algo_map_.clear();
