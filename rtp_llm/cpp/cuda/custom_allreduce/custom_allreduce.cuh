// Adapted from vllm custom_all_reduce.cuh
// Self-contained: no external dependencies beyond CUDA + standard library.
#pragma once
...
class CustomAllreduce {
 public:
  ...
  void check_rank_data_capacity(size_t num = 1) {
    if (d_rank_data_base_ + num > d_rank_data_end_)
      throw std::runtime_error(
          "Rank data buffer overflowed by " + std::to_string(d_rank_data_base_ + num - d_rank_data_end_));
  }
  ...
  void allreduce(cudaStream_t stream, T* input, T* output, int size, int threads = 512, int block_limit = 36) {
    ...
    if (size % d != 0)
      throw std::runtime_error("custom allreduce requires input length to be multiple of " + std::to_string(d));
...
#define REDUCE_CASE(ngpus)                                                                          \
  case ngpus: {                                                                                     \
    ...
    switch (world_size_) {
      ...
      REDUCE_CASE(8)
      default:
        throw std::runtime_error("custom allreduce only supports (2,4,6,8) gpus, got " + std::to_string(world_size_));
    }
#undef REDUCE_CASE
#undef KL
  }
