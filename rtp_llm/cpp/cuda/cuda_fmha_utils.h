...
public:
    ...
    static bool UseTrtFMHA(const FMHAConfig& fmha_config,
        ...
        if (!tensorrt_llm::kernels::MHARunner::fmha_supported(model_config.size_per_head_, rtp_llm::get_sm())) {
    ...
    static bool UseOldTrtFMHA(const FMHAConfig& fmha_config,
        ...
        if (model_config.head_num_ != model_config.head_num_kv_) {
            RTP_LLM_LOG_INFO("OLD TRT not support head_num != head_num_kv");
            return false;
        }
        auto testRunner = FusedMHARunnerFP16v2(
            model_config.head_num_, model_config.size_per_head_, get_sm(), model_config.q_scaling_);
        if (!testRunner.fmha_supported(model_config.is_causal_)) {
...
protected:
    ...
    static bool CheckQKVLengthEqual(const ModelConfig& model_config,
        ...
        bool               not_prefix                = model_config.pre_seq_len_ == 0 && !reuse_cache_env;
        ...
        bool enable_partial_fallback_env = runtime_config.enable_partial_fallback;
        if (enable_partial_fallback_env) {
            RTP_LLM_LOG_INFO("QKV length not equal: enable part fallback");
            return false;
        }

        if (runtime_config.enable_fast_gen) {
