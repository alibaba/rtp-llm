...
void cufmha::runTrtV1Fmha(void*  input,
    ...
    if (mtype_ == AttentionMaskType::causalMask) {
        ...
        trtv1_fmha_runner_->run_causal_masked_fmha(input, cu_seqlens, output, true, stream_);
    } else {
        DISPATCH_CUDA_FUNCTION_DATA_TYPE(
            dtype_, invokeTransposeAxis12, qkv_buf_temp, input, token_num, 3, head_num_, size_per_head_, stream_);

        auto max_length = trtv1_fmha_runner_->getSFromMaxSeqLen(seq_len);
        trtv1_fmha_runner_->setup(max_length, batch_size);
        trtv1_fmha_runner_->run(qkv_buf_temp, nullptr, cu_seqlens, nullptr, output, stream_);
    }
#else
    return;
#endif
...
bool cufmha::initTrtV2FmhaAndCheckSupport() {
    ...
    trtv2_fmha_runner_.reset(new tensorrt_llm::kernels::FusedMHARunnerV2(
...
bool cufmha::initTrtV2FmhaPagedAndCheckSupport() {
    ...
    trtv2_paged_fmha_runner_.reset(new tensorrt_llm::kernels::FusedMHARunnerV2(
...
void cufmha::runTrtV2FmhaPaged(void*        input,
    ...
    trtv2_paged_fmha_runner_->setup_flags(
        mFMHAForceFP32Acc, mRemovePadding, (mtype_ == AttentionMaskType::causalMask), kv_head_num_);
...
void cufmha::runTrtV2Fmha(void*        input,
    ...
    if (trtv2_fmha_runner_) {
        trtv2_fmha_runner_->setup_flags(
            mFMHAForceFP32Acc, mRemovePadding, (mtype_ == AttentionMaskType::causalMask), kv_head_num_);
        ...
        trtv2_sm70_fmha_runner_->setup(batch_size, seq_len, seq_len, token_num, is_alibi, is_alibi_with_sacle, 1, 0);
