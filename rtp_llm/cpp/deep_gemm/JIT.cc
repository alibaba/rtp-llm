...
string JIT::getKernelStr(uint32_t     n,
    ...
    const string func_name_str = to_string(n) + "_" + to_string(k) + "_" + to_string(bm) + "_" + to_string(bn) + "_"
                                 + to_string(bk) + "_" + to_string(num_groups) + "_" + to_string(num_stages) + "_"
                                 + to_string(num_tma_multicast) + "_" + to_string(uint32_t(gemm_type)) + "_"
                                 + to_string(uint32_t(swap_ab));
...
runDeepGemmFunc JIT::compileAndLoadKernel(uint32_t     n,
    ...
    const string params_str = "_" + to_string(n) + "_" + to_string(k) + "_" + to_string(bm) + "_" + to_string(bn) + "_"
                              + to_string(bk) + "_" + to_string(num_groups) + "_" + to_string(num_stages) + "_"
                              + to_string(num_tma_multicast) + "_" + to_string(uint32_t(gemm_type)) + "_"
                              + to_string(uint32_t(swap_ab));
    ...
    if (!filesystem::exists(so_name)) {
        ...
        string command =
            "/usr/local/cuda/bin/nvcc " + cu_filename + " -o " + so_name
...
runDeepGemmFunc JIT::searchKernel(uint32_t     n,
                    ...
                    for (auto& num_tma_multicast_ : num_tma_multicast_list) {
                        if (gemm_type == DeepGemmType::Normal || gemm_type == DeepGemmType::GroupedMasked) {
                            threads.emplace_back([&]() {
                                compileAndLoadKernel(
                                    n, k, bm_, bn_, bk_, num_groups, num_stages_, num_tma_multicast_, gemm_type, true);
                            });
                        }
                        threads.emplace_back([&]() {
                            compileAndLoadKernel(
                                n, k, bm_, bn_, bk_, num_groups, num_stages_, num_tma_multicast_, gemm_type, false);
                        });
                    }
                }
            }
    ...
    if (kernel_value) {
        ...
        string params_str = to_string(n) + ", " + to_string(k) + ", " + to_string(bm) + ", " + to_string(bn) + ", "
                            + to_string(bk) + ", " + to_string(num_groups) + ", " + to_string(num_stages) + ", "
                            + to_string(num_tma_multicast) + ", " + to_string(uint32_t(gemm_type)) + ", "
                            + to_string(uint32_t(swap_ab));
