...
TEST_F(InferenceServiceTest, InferResponseFailed_ControllerIncrementFailed) {
    ...
    EXPECT_CALL(*mock_metric_reporter_, reportQpsMetric(StrEq("unknown"), _));
...
TEST_F(InferenceServiceTest, InferResponseSuccess) {
    ...
    // metric
    EXPECT_CALL(*mock_metric_reporter_, reportQpsMetric(Eq("test_source"), _));
    EXPECT_CALL(*mock_metric_reporter_, reportSuccessQpsMetric(Eq("test_source"), _));
...
TEST_F(InferenceServiceTest, InferResponseRejectsPartialEnqueueAndCancelsAdmittedStreams) {
    ...
    try {
        ...
        EXPECT_EQ(e.getType(), HttpApiServerException::MALLOC_ERROR);
        EXPECT_NE(e.getMessage().find("scheduler rejected request"), std::string::npos);
    }
...
TEST_F(InferenceServiceTest, iterateStreams_CancelError) {
    ...
    EXPECT_CALL(*mock_metric_reporter_, reportCancelQpsMetric(StrEq("unknown"), _));
...
TEST_F(InferenceServiceTest, InferResponseStreamingErrorAfterStart_WritesSseError) {
    ...
    std::vector<GenerateStreamPtr> streams({stream});
    EXPECT_CALL(*mock_engine_, enqueueMultiple(Matcher<const std::vector<std::shared_ptr<GenerateInput>>&>(_)))
        .WillOnce(Return(std::make_pair(std::vector<bool>(streams.size(), true), streams)));
