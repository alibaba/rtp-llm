...
TEST_F(TelemetryTest, UsageTokenAttributesFiveKeyDoubleWrite) {
    // Per-hop usage contract (the Python schema in rtp_llm/telemetry/attributes.py
    // is aligned in a follow-up PR): semconv input/output + legacy
    // prompt/completion aliases + total.
    auto span_data = startInMemoryRuntime();
    {
