...
// Centralized C++ span attribute registry. Within this PR this header is the
// authoritative source for the attribute schema; the Python-side schema in
// rtp_llm/telemetry/attributes.py is added and aligned in a follow-up PR (not
// part of this change), so keep the two in sync when it lands. Grouping follows the same
