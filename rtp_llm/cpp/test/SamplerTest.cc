...
TEST_F(SamplerTest, JointTopKTopPReturnedProbabilitySupport) {
    auto makeInputs = [](bool return_probs) {
        SamplerInputs input;
        input.logits =
            torch::tensor({0.30f, 0.28f, 0.22f, 0.20f}, torch::kFloat32).log().repeat({32, 1}).to(torch::kCUDA);
    ...
    EXPECT_TRUE(
        torch::allclose(result.cum_log_probs.cpu(), expected.gather(1, selected).log().reshape({32}), 1e-5, 1e-6));
}
