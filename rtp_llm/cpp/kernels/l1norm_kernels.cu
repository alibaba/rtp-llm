...
#include "rtp_llm/cpp/kernels/l1norm_kernels.h"
...
template<typename T>
void dispatch_l1norm_type_square_method(T*           output,
                                        const T*     input,
                                        const float  eps,
                                        int          tokens,
                                        int          hidden_dim,
    ...
    if (shmem_size >= (48 << 10)) {
        cudaError_t ret =
            cudaFuncSetAttribute(generall1Norm<T>, cudaFuncAttributeMaxDynamicSharedMemorySize, shmem_size);
    }
    generall1Norm<T><<<grid, block, shmem_size, stream>>>(output, input, eps, tokens, hidden_dim);
}

template<typename T>
void invokeGeneralL1Norm(
    T* out, const T* input, const float eps, const int tokens, const int hidden_dim, cudaStream_t stream) {
    dim3 grid(tokens);
    ...
    if (use_vec_type) {
        ...
        dispatch_l1norm_type_square_method(reinterpret_cast<Tp*>(out),
...
#define INSTANTIATE_GENERAL_L1NORM(T)                                                                                  \
    template void invokeGeneralL1Norm(                                                                                 \
        T* out, const T* input, const float eps, const int tokens, const int hidden_dim, cudaStream_t stream);
