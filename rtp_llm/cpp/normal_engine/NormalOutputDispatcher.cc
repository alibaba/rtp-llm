...
absl::Status NormalOutputDispatcher::dispatch(const StreamGroups& stream_groups,
    ...
    for (auto& stream : stream_groups.allStreams()) {
        ...
        // allStreams() appends contextStreams() in gather order; prefill dispatch is synchronous.
        // Reuse that order and selection so custom output rows stay aligned with their streams.
