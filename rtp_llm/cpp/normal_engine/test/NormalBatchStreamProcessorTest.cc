...
TEST_F(NormalBatchStreamProcessorTest, testCustomOutputDispatch) {
    ...
    for (const auto& [score_rows, cached_first] :
         {std::pair{2, false}, std::pair{2, true}, std::pair{0, false}, std::pair{0, true}}) {
        ...
        if (score_rows < 2) {
            ...
            EXPECT_EQ(context->statusInfo().ToString(), "custom output processor failed: handler failure");
        } else {
