...
TEST_F(NormalBatchStreamProcessorTest, testDecodeRejectsOutOfRangeEmbeddingId) {
    for (const int invalid_token : {-1, 2048}) {
        ...
        complete_token_ids.data_ptr<int32_t>()[stream->seqLength() - 1] = invalid_token;
        stream->generate_status_->status                                = StreamState::RUNNING;

        StreamGroups stream_groups({stream});
        TensorHolder holder;
        const auto   gathered = processor.gatherModelInput(stream_groups, holder);
        ASSERT_FALSE(gathered.ok());
        EXPECT_EQ(gathered.status().code(), absl::StatusCode::kInvalidArgument);
        EXPECT_NE(std::string(gathered.status().message()).find(std::to_string(stream->streamId())), std::string::npos);
        EXPECT_NE(std::string(gathered.status().message()).find("exceed vocab_size 2048"), std::string::npos);
    }
}
