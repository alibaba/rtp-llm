...
TEST_F(NormalBatchStreamProcessorTest, gatherExecutionModesPreserveFlagsAndErrors) {
    ...
    const std::vector<TestCase> cases = {
        ...
        {"existing-error-prefill-still-defines-mode", {{true, 0}, {true, 1}}, true, true, true},
        {"existing-error-in-cross-container-batch", {{true, 0}, {false, 1}}, false, true, true},
    ...
    for (const auto& test_case : cases) {
        ...
        TensorHolder holder;
        auto         model_input = processor.gatherModelInput(stream_groups, holder);
        // Mixed batches still reach TP sync; flags follow the first decode stream,
        // or the first context stream when there are no decode streams.
        ASSERT_TRUE(model_input.ok());
