...
rtp_llm::EngineInitParams createEngineInitParams(DeviceBase*             device,
    ...
    auto word_embeddings = make_unique<const Buffer>(mem_type,
                                                     data_type,
                                                     vector<size_t>{(size_t)model_config.vocab_size, hidden_units},
                                                     data->data(),
                                                     [data](Buffer*) {});
    ...
    for (int i = 0; i < model_config.num_layers; ++i) {
        ...
        weights.emplace(W::ffn_up, std::move(ffn_weight));
        weights.emplace(W::ffn_up_b, std::move(ffn_weight_beta));
...
std::shared_ptr<NormalEngine> createMockEngine(DeviceBase* device, const CustomConfig& config) {
    ...
    rtp_llm::RuntimeConfig runtime_config;
    rtp_llm::KVCacheConfig kv_cache_config;
    EngineInitParams       rtp_llm_params =
        createEngineInitParams(device, config, model_config, runtime_config, kv_cache_config);
