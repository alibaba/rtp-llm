...
TEST_F(PPBatchStreamProcessorTest, DraftDecodeInputUsesOnlyAcceptedRowsWithoutMutatingVerifyInput) {
    ...
    auto draft_input =
        executor.prepareDraftInputForDecode(verify_input, target_output, accepted_tokens, intTensor({1, 3}));
    ...
    auto expected_hidden =
        torch::arange(32, torch::kFloat32).reshape({8, 4}).index_select(0, torch::tensor({0, 4, 5, 6}, torch::kLong));
