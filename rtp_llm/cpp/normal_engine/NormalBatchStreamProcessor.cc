...
NormalBatchStreamProcessor::NormalBatchStreamProcessor(
    ...
    model_input_gatherer_config_.max_kernel_blocks_per_kv_block = 1;
    ...
    if (cache_config.groupNums() > 0) {
        ...
        for (const auto& group : groups) {
            ...
            model_input_gatherer_config_.max_kernel_blocks_per_kv_block = std::max(
                model_input_gatherer_config_.max_kernel_blocks_per_kv_block, group.storedKernelBlocksPerKvBlock());
        }
    }
