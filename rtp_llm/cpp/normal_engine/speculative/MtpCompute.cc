...
void applySpecLogitsAcceptLenCap(const SpecLogitsVerifyRunner::LaunchResult& verify_result,
    ...
    RTP_LLM_CHECK_WITH_INFO(accept_len.defined() && accept_len.is_cuda(), "spec logits cap requires CUDA accept_len");
    ...
    RTP_LLM_CHECK_WITH_INFO(target_token_ids.defined(), "spec logits cap requires target sampler token_ids");
    ...
    auto cols =
        torch::arange(propose_step + 1, torch::TensorOptions().device(accept_tokens.device()).dtype(torch::kLong))
            .unsqueeze(0)
            .expand({batch_size, propose_step + 1});
    auto replace_mask =
        (cap_gpu < propose_step).unsqueeze(1) & (accept_len > cap_gpu).unsqueeze(1) & (cols == cap_index.unsqueeze(1));
    accept_tokens = torch::where(replace_mask, replacement.expand({batch_size, propose_step + 1}), accept_tokens);
}
