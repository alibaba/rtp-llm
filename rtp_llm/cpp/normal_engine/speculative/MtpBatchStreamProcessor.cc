...
void MtpBatchStreamProcessor::updateDecodePostDraftModelInput(
    ...
    // The post-rejection draft forward always keeps the fixed-width MTP layout.
    // Acceptance changes only which row supplies logits; it never changes the
    // tokens or hidden-state rows processed by the draft model.
    const int tokens_per_batch = propose_step_ + 1;
    const int total_tokens     = tokens_per_batch * batch_size;
    ...
    RTP_LLM_CHECK_WITH_INFO(accept_len_d.numel() == static_cast<int64_t>(batch_size),
                            "accept_len batch mismatch: %ld != %zu",
                            accept_len_d.numel(),
                            batch_size);
    model_input.lm_output_indexes =
        torch::arange(0, total_tokens, tokens_per_batch, cudaInt32Options()) + (accept_len_d - 1);
    model_input.last_hidden_states = model_output.all_hidden_states;
}
...
void MtpBatchStreamProcessor::prepareDecodeSpecUpdateInfo(
    ...
    for (auto& stream : stream_groups.allStreams()) {
        ...
        // Checked here rather than in the dense decode path, which never leaves
        // the device and would silently index a neighbouring batch row.
        RTP_LLM_CHECK_WITH_INFO(cur_accept_len >= 1 && cur_accept_len <= propose_step_ + 1,
                                "invalid accept_len[%d]=%d for propose_step=%d",
                                batch_idx_out,
                                cur_accept_len,
                                propose_step_);
