...
static constexpr int32_t kDsv4BeginThinkToken = 128821;
static constexpr int32_t kDsv4EndThinkToken   = 128822;
static constexpr int32_t kDsv4DsmlToken       = 128825;
...
public:
    ...
    BaseLogitsProcessorPtr createDsv4ThinkingProcessor(size_t input_length, size_t output_length = 0) {
        std::vector<int> end_think_token_ids = {kDsv4EndThinkToken};
        StreamThinkInfo  think_info(/*think_mode=*/true,
                                   /*max_thinking_tokens=*/32,
                                   /*begin_think_token_ids=*/{kDsv4BeginThinkToken},
                                   end_think_token_ids,
                                   static_cast<int32_t>(input_length),
                                   static_cast<int32_t>(output_length),
                                   /*is_beam_search=*/false,
                                   std::make_shared<StringContainDFA<size_t, int>>(end_think_token_ids));
        think_info.excluded_token_ids = {kDsv4DsmlToken};
        return std::make_shared<ThinkModeLogitsProcessor>(std::vector<StreamThinkInfo>{think_info});
    }
...
TEST_F(MtpExecutorTest, testDecodeRewritesDsv4ToolCallTokenSampledDuringThinking) {
    size_t propose_step = 1;
    size_t vocab_size   = static_cast<size_t>(kDsv4DsmlToken + 1);
    ...
    test_config.vocab_size_override    = vocab_size;
    auto components                    = createMtpExecutorComponents(test_config);
    components.model_config.vocab_size = static_cast<int64_t>(vocab_size);

    auto stream_new_tokens                      = torch::tensor({{42}}, torch::kInt32);
    auto stream_hidden_states                   = torch::tensor({{0.03f, 0.04f}});
    auto stream_draft_token_probs               = torch::zeros({1, static_cast<int64_t>(vocab_size)}, torch::kFloat32);
    stream_draft_token_probs[0][kDsv4DsmlToken] = 1.0f;
    StreamSpecUpdateInfo spec_update_info{
        stream_new_tokens, 1, kDsv4DsmlToken, stream_hidden_states, stream_draft_token_probs};

    GenerateStreamPtr stream =
        createContextStream(components.model_config, components.runtime_config, components.resource_context, {0, 1});
    stream->logits_processor_list_.push_back(createDsv4ThinkingProcessor(/*input_length=*/2));
    ...
    auto target_input              = GptModelInputs{};
    auto target_output             = GptModelOutputs{};
    target_input.combo_tokens      = torch::tensor({42, kDsv4DsmlToken}, torch::kInt32);
    target_input.input_lengths     = torch::tensor({2}, torch::kInt32);
    target_input.prefix_lengths    = torch::tensor({2}, torch::kInt32);
    target_input.lm_output_indexes = torch::tensor({0, 1}, torch::kInt32);
    target_output.logits =
        torch::zeros({2, static_cast<int64_t>(vocab_size)}, torch::TensorOptions().dtype(torch::kFloat32))
            .to(torch::kCUDA);
    target_output.all_hidden_states = torch::tensor({0.01f, 0.02f, 0.03f, 0.04f}).reshape({2, 2});

    auto next_draft_input               = GptModelInputs{};
    auto next_draft_output              = GptModelOutputs{};
    next_draft_input.combo_tokens       = torch::tensor({kDsv4EndThinkToken, 7}, torch::kInt32);
    next_draft_input.input_lengths      = torch::tensor({2}, torch::kInt32);
    next_draft_input.prefix_lengths     = torch::tensor({2}, torch::kInt32);
    next_draft_input.lm_output_indexes  = torch::tensor({0}, torch::kInt32);
    next_draft_input.last_hidden_states = target_output.all_hidden_states;
    next_draft_output.logits =
        torch::zeros({1, static_cast<int64_t>(vocab_size)}, torch::TensorOptions().dtype(torch::kFloat32));
    next_draft_output.logits[0][7]      = 1.0f;
    next_draft_output.all_hidden_states = torch::tensor({0.21f, 0.22f, 0.23f, 0.24f}).reshape({2, 2});
    ...
    auto next_draft_probs  = torch::zeros({1, static_cast<int64_t>(vocab_size)}, torch::kFloat32);
    next_draft_probs[0][7] = 1.0f;
    auto next_draft_sampler_output =
        spec::FastTopKSamplerOutput{next_draft_probs, torch::tensor({7}, torch::kInt32).reshape({1, 1})};
    components.fake_fast_topk_sampler->setInputs({next_draft_output.logits});
    components.fake_fast_topk_sampler->setOutputs({next_draft_sampler_output});

    auto sampler_input                            = SamplerInputs{target_output.logits.clone()};
    sampler_input.logits[0][kDsv4BeginThinkToken] = BaseLogitsProcessor::neg_inf;
    auto target_sampler_output = SamplerOutput{torch::tensor({kDsv4DsmlToken, 7}, torch::kInt32).reshape({2, 1})};
    target_sampler_output.all_probs =
        torch::zeros({2, static_cast<int64_t>(vocab_size)}, torch::TensorOptions().dtype(torch::kFloat32));
    target_sampler_output.all_probs[0][kDsv4DsmlToken] = 1.0f;
    target_sampler_output.all_probs[1][7]              = 1.0f;
    components.fake_sampler->setInputs({sampler_input});
    components.fake_sampler->setOutputs({target_sampler_output});

    auto forced_accept_tokens                    = torch::tensor({{kDsv4DsmlToken, 7}}, torch::kInt32);
    auto speculative_sampler_output              = spec::SpeculativeSamplerOutput();
    speculative_sampler_output.accept_tokens_cpu = forced_accept_tokens;
    speculative_sampler_output.accept_tokens     = forced_accept_tokens.to(torch::kCUDA);
    speculative_sampler_output.accept_len_cpu    = torch::tensor({2}, torch::kInt32);
    ...
    auto token_ids = stream->getCompleteTokenIds()->completeTokenIdsVec(0);
    EXPECT_EQ((std::vector<int>{0, 1, 42, kDsv4EndThinkToken}), token_ids);

    auto sp_output_buffer = stream->getSPOutputBuffer();
    auto tokens_h         = sp_output_buffer->tokens.cpu().clone();
    EXPECT_EQ((std::vector<int>{kDsv4EndThinkToken, 7}), toVec<int>(tokens_h));
}
