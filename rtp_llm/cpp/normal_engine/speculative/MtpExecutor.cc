...
MtpExecutor::MtpExecutor(const EngineInitParams&                        params,
    ...
    const auto replay_prepare_policy = (useStreamAsync() || useAsyncPrepare()) ?
                                           CudaGraphReplayPreparePolicy::OVERLAP_WITH_PREVIOUS_REPLAY :
                                           CudaGraphReplayPreparePolicy::WAIT_FOR_PREVIOUS_REPLAY;
    const auto model_input_placement = useDeviceInput() ? ModelInputPlacement::DEVICE : ModelInputPlacement::HOST;

    if (!params.py_model.is_none()) {
        RTP_LLM_LOG_INFO("init executor with python model");
        PyWrappedModelOptions target_model_options;
        target_model_options.use_spec_decoding     = true;
        target_model_options.replay_prepare_policy = replay_prepare_policy;
        model_.reset(new PyWrappedModel(model_init_params, params.py_model, target_model_options));
    }
    ...
    for (auto& mtp_params : *propose_params->mtp_model_params_) {
        ...
        if (!params.py_sp_model.is_none()) {
            ...
            PyWrappedModelOptions draft_model_options;
            draft_model_options.dspark_model_role     = is_dspark_ ? DSparkModelRole::PROPOSE : DSparkModelRole::NONE;
            draft_model_options.allow_cuda_graph      = draft_graph_allowed;
            draft_model_options.replay_prepare_policy = replay_prepare_policy;
            draft_model_.reset(new PyWrappedModel(model_params, params.py_sp_model, draft_model_options));
            ...
            if (enable_cuda_graph || is_dspark_) {
                ...
                PyWrappedModelOptions draft_prefill_model_options;
                draft_prefill_model_options.is_prefill_cuda_graph_mode = !is_dspark_;
                draft_prefill_model_options.dspark_model_role =
                    is_dspark_ ? DSparkModelRole::COMMIT : DSparkModelRole::NONE;
                draft_prefill_model_options.allow_cuda_graph      = draft_graph_allowed;
                draft_prefill_model_options.replay_prepare_policy = replay_prepare_policy;
                sp_prefill_draft_model_.reset(
                    new PyWrappedModel(model_params, params.py_sp_model, draft_prefill_model_options));
            }
        }
