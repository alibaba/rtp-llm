...
GptModelInputs MtpExecutor::makePrefillRoundInput(const GptModelInputs& full_inputs,
    ...
    chunk.cache_store_input_lengths_host  = torch::Tensor();
    chunk.cache_store_prefix_lengths_host = torch::Tensor();
...
void MtpExecutor::setPrefillChunkCacheStoreRange(GptModelInputs&          chunk_input,
                                                  const KimiK3ChunkRound& round,
    ...
    std::vector<int32_t> cache_store_input_lengths;
    std::vector<int32_t> cache_store_prefix_lengths;
    cache_store_input_lengths.reserve(round.slices.size());
    cache_store_prefix_lengths.reserve(round.slices.size());
    for (const auto& slice : round.slices) {
        RTP_LLM_CHECK_WITH_INFO(slice.absolute_start >= 0 && slice.absolute_end > slice.absolute_start,
                                ...
                                slice.absolute_end);
        const int64_t store_start = static_cast<int64_t>(slice.absolute_start) / block_size * block_size;
        const int64_t store_end   = complete_blocks_only ?
                                        static_cast<int64_t>(slice.absolute_end) / block_size * block_size :
                                        static_cast<int64_t>(slice.absolute_end);
        RTP_LLM_CHECK_WITH_INFO(store_end >= store_start && store_end <= std::numeric_limits<int32_t>::max(),
                                "K3 MTP chunk cache-store produced invalid aligned range [%ld, %ld)",
                                store_start,
                                store_end);
        cache_store_prefix_lengths.push_back(static_cast<int32_t>(store_start));
        cache_store_input_lengths.push_back(static_cast<int32_t>(store_end - store_start));
    }

    const auto host_i32 = torch::TensorOptions().dtype(torch::kInt32).device(torch::kCPU);
    chunk_input.cache_store_input_lengths_host  = torch::tensor(cache_store_input_lengths, host_i32);
    chunk_input.cache_store_prefix_lengths_host = torch::tensor(cache_store_prefix_lengths, host_i32);
}
...
void MtpExecutor::runK3ChunkPrefillRound(K3ChunkPrefillHook& hook, const KimiK3ChunkRound& round, bool is_last) {
    ...
    setPrefillChunkCacheStoreRange(
...
absl::Status MtpExecutor::prefillStep(const std::list<GenerateStreamPtr>& streams,
    ...
    if (chunked_mtp_prefill) {
        ...
        setPrefillChunkCacheStoreRange(final_chunk_input,
        ...
        // Preserve the unchunked ordering: EPLB consumes target statistics
        // before target sampling and the draft forward.
        if (expert_balancer_) {
    ...
    // Chunked EPLB observes the complete target Prefill, including every chunk.
    if (chunked_mtp_prefill && expert_balancer_) {
        RTP_LLM_PROFILE_SCOPE("executor.mtp.prefill_step(eplb_step_forward)");
        int64_t start_time_us = autil::TimeUtility::currentTimeInMicroSeconds();
        expert_balancer_->stepForward(*model_, executor_collector);
        executor_collector.eplb_step_latency_us = autil::TimeUtility::currentTimeInMicroSeconds() - start_time_us;
