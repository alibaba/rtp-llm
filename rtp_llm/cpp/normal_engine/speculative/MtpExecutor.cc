...
absl::Status MtpExecutor::prefillStep(const std::list<GenerateStreamPtr>& streams,
    ...
    if (stream_groups.needFrontendMetricStreaming()) {
        ...
        if (frontend_execute_time_us <= 0) {
            frontend_execute_time_us = model_forward_us;
        }
...
absl::Status MtpExecutor::decodeStep(const std::list<GenerateStreamPtr>& streams,
    ...
    const bool need_frontend_metrics = stream_groups.needFrontendMetricStreaming();
    if (metrics_reporter_ || need_frontend_metrics) {
        collectDecodeMetrics(
            stream_groups, accept_len_ready_event, speculative_sampler_output, metrics_collector, model_forward_us);
    } else {
        // A frontend-metric request may leave between decode steps. Drain the
        // delayed snapshot when the gate closes so a later request cannot
        // consume accept lengths from a non-adjacent batch.
        consumePendingAcceptLenMetrics();
    }

    if (need_frontend_metrics) {
        const int64_t generate_execute_time_us  = autil::TimeUtility::currentTimeInMicroSeconds() - schedule_time_us;
        ...
        if (engine_generate_token_num > 0) {
            // Keep the private frontend cumulative token counter on the same
            // one-step-delayed accepted-token numerator used by rtp_llm_generate_tps.
            stream_groups.addFrontendGenerateExecuteMetrics(generate_execute_time_us, engine_generate_token_num);
        }
    }
...
absl::Status MtpExecutor::process(const std::list<GenerateStreamPtr>& streams, int64_t schedule_time_us) {
    ...
    if (role_type_ == RoleType::DECODE || role_type_ == RoleType::PDFUSION) {
        THROW_IF_STATUS_ERROR(decodeStep(decode_streams, metrics_collector, schedule_time_us));
    }
