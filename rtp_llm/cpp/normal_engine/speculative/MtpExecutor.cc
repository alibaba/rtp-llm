...
absl::Status MtpExecutor::prefillStep(const std::list<GenerateStreamPtr>& streams,
    ...
    // target model prefill
    {
        ...
        maybeOverrideLastHiddenWithMtpBuffer(model_output, *model_, -1);
    ...
    if (cp_enabled && !is_dspark_) {
        ...
        maybeOverrideLastHiddenWithMtpBuffer(draft_model_output, *draft_model_, -1);
    }
    ...
    if (isTpRank0()) {
        ...
        if (is_dspark_) {
            ...
            batch_stream_processor_->updateDecodePostDraftModelInput(
                model_input, model_output, speculative_sampler_output, batch_size, buffer_holder_);
        }
        if (metrics_reporter_) {
...
void MtpExecutor::broadcastPostRejectionInputs(GptModelInputs& model_input) {
    ...
    if (parallelism_config_.tp_size > 1 && !is_dspark_) {
        // Every rank retains the dense [batch, propose_step + 1] layout. Only
        // rejection-updated values need synchronization; tensor shapes and
        // input_lengths remain identical to the target-verify input.
