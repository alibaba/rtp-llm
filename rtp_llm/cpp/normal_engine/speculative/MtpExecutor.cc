...
absl::Status MtpExecutor::decodeStep(const std::list<GenerateStreamPtr>& streams,
    ...
    if (isTpRank0()) {
        ...
        if (exact_host_prefix_available && !model_input.is_fake_stream && model_input.prefix_lengths.numel() > 0) {
            static const bool prefix_mirror_logged = []() {
                RTP_LLM_LOG_INFO(
                    "[mtp-sync-free] draft-prefill prefix host mirror uses completed stream seqLength-1 metadata");
                return true;
            }();
            (void)prefix_mirror_logged;
