        ...
        if (!params.py_sp_model.is_none()) {
            ...
            if (enable_cuda_graph || is_dspark_) {
                ...
                if (py::hasattr(params.py_sp_model, "clone_for_cuda_graph")) {
                    ...
                    RTP_LLM_LOG_WARNING(
                        "[speculative decoding] py_sp_model has no clone_for_cuda_graph(); sp_prefill CUDA graph "
                        "will share Python runtime state with eager draft model");
                }
...
absl::Status MtpExecutor::decodeStep(const std::list<GenerateStreamPtr>& streams,
    ...
    if (use_cp_local_decode_hidden) {
    ...
    } else if (!cp_context_request && !is_dspark_) {
        ...
        // above, so re-reading mutable Python model state is both redundant and
...
void MtpExecutor::publishSyncMtpDeviceState(const StreamGroups&                          stream_groups,
    ...
    for (auto& stream : all_streams) {
        ...
        // stream by accept_len. Keep the next bound aligned with the committed
        // host length; adding accept_len here would reserve/rollback against a
        // future position that has not been verified yet.
