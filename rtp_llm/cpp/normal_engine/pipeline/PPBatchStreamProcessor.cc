...
absl::Status PPBatchStreamProcessor::dispatchExecutionResult(const StreamGroups& stream_groups,
    ...
    for (const auto& stream : stream_groups.allStreams()) {
        snapshot.push_back({static_cast<int64_t>(stream->currentBatchSize()),
...
absl::Status PPBatchStreamProcessor::dispatchExecutionResult(
    const StreamGroups& stream_groups, const PPExecutionResult& result,
...
absl::Status PPBatchStreamProcessor::dispatchNormalExecutionResult(const StreamGroups& stream_groups,
    ...
    for (const auto& stream : all_streams) {
        ...
        RTP_LLM_CHECK_WITH_INFO(stream_batch_size > 0 && token_size >= 0
                                    && token_size % stream_batch_size == 0,
