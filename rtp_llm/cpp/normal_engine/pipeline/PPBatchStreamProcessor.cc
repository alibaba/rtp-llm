...
SamplerInputs PPBatchStreamProcessor::allocateSamplerInputs(const PPSamplingPlan& sampling_plan,
    ...
    sampler_inputs.token_ids = torch::empty({batch_size, static_cast<int64_t>(sampler_inputs.step + 1)}, pinned_i32);
...
absl::StatusOr<SamplingState> PPBatchStreamProcessor::createSamplingState(const PPSamplingPlan& sampling_plan,
    ...
    if (!processors_result.ok()) {
        return absl::InvalidArgumentError("failed to initialize sampling state for request_id="
                                          + std::to_string(request_id) + ": " + processors_result.status().ToString());
    }
...
PPBatchStreamProcessor::validateExecutionResult(const std::list<GenerateStreamPtr>& all_streams,
    ...
    RTP_LLM_CHECK_WITH_INFO(
        result.new_token_ids.defined() && result.new_token_ids.device().is_cpu()
            ...
            && result.new_token_ids.size(0) == layout.batch_size && result.sample_success.defined()
            && result.sample_success.device().is_cpu() && result.sample_success.scalar_type() == torch::kBool
            && result.sample_success.dim() == 1 && result.sample_success.size(0) == layout.batch_size
...
absl::Status PPBatchStreamProcessor::dispatchNormalExecutionResult(const StreamGroups&      stream_groups,
    ...
    RTP_LLM_CHECK_WITH_INFO(result.new_token_ids.size(1) == 1 && !result.accept_len.defined()
                                && !result.propose_token_ids.defined(),
...
void PPBatchStreamProcessor::validateMtpExecutionResult(const std::list<GenerateStreamPtr>& all_streams,
    ...
    RTP_LLM_CHECK_WITH_INFO(
        layout.batch_size == static_cast<int64_t>(all_streams.size()) && result.accept_len.defined()
            && result.accept_len.device().is_cpu() && result.accept_len.scalar_type() == torch::kInt32
            && result.accept_len.dim() == 1 && result.accept_len.size(0) == layout.batch_size
    ...
    for (const auto& stream : all_streams) {
        ...
        if (success[row] && !result.processor_errors[row].has_value()) {
            RTP_LLM_CHECK_WITH_INFO(accept_lengths[row] >= 1 && accept_lengths[row] <= result.new_token_ids.size(1)
