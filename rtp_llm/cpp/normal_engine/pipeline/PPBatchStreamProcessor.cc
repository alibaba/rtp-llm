...
PPSamplingPlan PPBatchStreamProcessor::gatherSamplingPlan(const StreamGroups& stream_groups) const {
    ...
    sampling_plan.random_seeds.reserve(batch_size);
    sampling_plan.logits_processor_configs.reserve(batch_size);
    sampling_plan.request_ids = torch::empty({static_cast<int64_t>(batch_size)}, pinned_i64);
    sampling_plan.token_ids =
        torch::empty({sampling_plan.request_ids.size(0), static_cast<int64_t>(token_width)}, pinned_i32);
    sampling_plan.input_lengths        = torch::empty(sampling_plan.request_ids.sizes(), pinned_i32);
    sampling_plan.sequence_lengths     = torch::empty(sampling_plan.request_ids.sizes(), pinned_i32);
    sampling_plan.top_k                = torch::empty(sampling_plan.request_ids.sizes(), pinned_i32);
    sampling_plan.top_p                = torch::empty(sampling_plan.request_ids.sizes(), pinned_f32);
    sampling_plan.temperature          = torch::empty(sampling_plan.request_ids.sizes(), pinned_f32);
    sampling_plan.repetition_penalty   = torch::empty(sampling_plan.request_ids.sizes(), pinned_f32);
    sampling_plan.presence_penalty     = torch::empty(sampling_plan.request_ids.sizes(), pinned_f32);
    sampling_plan.frequency_penalty    = torch::empty(sampling_plan.request_ids.sizes(), pinned_f32);
    sampling_plan.no_repeat_ngram_size = torch::empty(sampling_plan.request_ids.sizes(), pinned_i32);
    sampling_plan.do_sample            = torch::empty(sampling_plan.request_ids.sizes(), pinned_bool);
    sampling_plan.finished_mask        = torch::empty(sampling_plan.request_ids.sizes(), pinned_bool);
...
absl::StatusOr<PPExecutionResult> PPBatchStreamProcessor::makeExecutionResult(
    ...
    if (!sampler_output.token_ids.defined() || sampler_output.token_ids.dim() != 2
        || sampler_output.token_ids.size(0) != batch_size || sampler_output.token_ids.size(1) == 0
        || !sampler_output.success.defined() || sampler_output.success.dim() != 1
        || sampler_output.success.size(0) != batch_size) {
...
void PPBatchStreamProcessor::dispatchSingleStream(const GenerateStreamPtr& stream,
    ...
    const auto token_size = static_cast<int64_t>(stream->currentExecuteTokenSize());
    const auto loss_size  = std::max<int64_t>(token_size - 1, 0);

    torch::Tensor hidden_states;
    if (stream->generateConfig()->return_hidden_states) {
        hidden_states = result.hidden_states.narrow(0, index, 1).clone();
    }
    torch::Tensor logits;
    if (stream->returnLogits()) {
        logits = result.logits.narrow(0, index, 1).clone();
    }
    torch::Tensor softmax_probs;
    if (stream->calculateSoftmaxProbs()) {
        softmax_probs = result.softmax_probs.narrow(0, index, 1).clone();
    }
    torch::Tensor cum_log_probs;
    if (stream->returnCumLogProbs()) {
        cum_log_probs = result.cum_log_probs.narrow(0, index, 1).clone();
    }
    torch::Tensor all_probs;
    if (stream->getReturnAllProbs() != ReturnAllProbsMode::NONE) {
        all_probs = result.all_probs.narrow(0, index, 1).clone();
    }
    torch::Tensor loss;
    ...
    stream->updateFromPP({result.new_token_ids.narrow(0, index, 1),
                          1,
