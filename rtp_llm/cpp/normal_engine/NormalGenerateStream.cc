    ...
    for (int i = 0; i < currentBatchSize(); i++) {
        ...
        if (generate_input_->generate_config->return_all_hidden_states) {
            ...
            if (update_info.all_hidden_states.defined() && iter_count_ == 1) {
                ...
                shared_all_hidden_states_length = update_info.shared_all_hidden_states_length;
            } else if (!isStreaming() && generate_output.finished && all_hidden_states_.defined()) {
                all_hidden_states               = &all_hidden_states_;
                shared_all_hidden_states_length = shared_all_hidden_states_length_;
            }
            if (all_hidden_states != nullptr) {
                ...
                generate_output.shared_all_hidden_states_length = shared_all_hidden_states_length;
            }
        }
        if (loss_.defined()) {
        ...
        if (generate_input_->generate_config->aux_info) {
            ...
            generate_output.aux_info.memory_reuse_len = host_reuse_length_;
            generate_output.aux_info.disk_reuse_len   = disk_reuse_length_;
...
void NormalGenerateStream::updateOutput(const StreamUpdateInfo& update_info) {
    ...
    // Decode handoff may consume the first iteration without prompt hidden states.
