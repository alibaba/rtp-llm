...
absl::Status ContextParallelBatchStreamProcessor::dispatch(const StreamGroups& stream_groups,
...
ContextParallelBatchStreamProcessor::gatherModelInput(const StreamGroups& stream_groups) const {
    ...
    model_input.prefill_shuffle_indices = CACHED_HOST_BUF(TYPE_INT32, {current_tokens_size});  // reserve for later use
    ...
    int* prefill_cp_padding_lengths = (int*)model_input.prefill_cp_padding_lengths->data();
    int* prefill_cp_chunk_lengths   = (int*)model_input.prefill_cp_chunk_lengths->data();
...
absl::StatusOr<SamplerInputs> ContextParallelBatchStreamProcessor::gatherSamplerInput(
