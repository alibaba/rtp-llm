...
void NormalEngine::mayAddFakeStream(std::list<GenerateStreamPtr>& streams) {
    ...
    if (isMTPEagle()) {
        ...
        const auto& mtp_model_config = propose_params_->getEngineInitParams().model_config_;
            ...
            case RoleType::DECODE:
                if (streams.empty()) {
                    streams.emplace_back(MtpExecutor::createMinFakeDecodeStream(
                        propose_step, model_config_, runtime_config, resource_context_, mtp_model_config));
                }
                break;
