...
NormalEngine::NormalEngine(const EngineInitParams& params):
    ...
    if (runtime_config.warm_up && (!mm_model_config_.is_multimodal_) && !ffn_disaggregate_config.enable_ffn_disaggregate) {
        // warm up
        RTP_LLM_LOG_INFO("warm up (max_context_batch_size %d, max_seq_len %d calculate_loss %d) query begin",
                         runtime_config.max_context_batch_size,
...
WarmUpResult NormalEngine::warmUp(const EngineInitParams& params) {
    if (runtime_config.use_batch_decode_scheduler) {
        if (runtime_config.batch_decode_scheduler_warmup_type == 0) {
...
std::shared_ptr<GenerateInput> NormalEngine::makeFakeInput(size_t seq_len) {
    ...
    size_t                     token_size =
        model_config_.embedding_size_ ? std::min(model_config_.embedding_size_, model_config_.vocab_size_) : model_config_.vocab_size_;
...
WarmUpResult NormalEngine::prefillWarmUp(const EngineInitParams& params) {
    auto fake_input                                   = makeFakeInput((size_t)model_config_.max_seq_len_ - 1);
    fake_input->generate_config->num_return_sequences = runtime_config.max_context_batch_size;
...
WarmUpResult NormalEngine::decodeWarmUp(const EngineInitParams& params) {
    ...
    cache_config.seq_size_per_block = model_config_.seq_size_per_block_;
...
void NormalEngine::initCacheManager(std::optional<WarmUpResult> warm_up_result) {
    auto result = CacheConfigCreator::createConfig(model_config_, mm_model_config_, parallelism_config, runtime_config, warm_up_result);
    RTP_LLM_LOG_INFO(
...
absl::Status NormalEngine::initSystemPrompt() {
    resource_context_.reuse_cache               = runtime_config.reuse_cache;
