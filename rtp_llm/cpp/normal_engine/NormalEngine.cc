...
std::shared_ptr<KVCacheManager> NormalEngine::createWarmupCacheManager() {
    ...
    // Match the production no-split default: 0 means kernel block size equals physical block size.
    const size_t kernel_block_size = model_config_.attn_config.kernel_tokens_per_block > 0 ?
                                         model_config_.attn_config.kernel_tokens_per_block :
                                         cache_config.seq_size_per_block;
    RTP_LLM_CHECK_WITH_INFO(kernel_block_size > 0 && cache_config.seq_size_per_block % kernel_block_size == 0,
                            "seq_size_per_block(%zu) must be divisible by kernel_seq_size_per_block(%zu)",
                            cache_config.seq_size_per_block,
                            kernel_block_size);
    cache_config.kernel_seq_size_per_block = kernel_block_size;
    auto cache_manager = make_shared<KVCacheManager>(cache_config, true);
...
WarmUpResult NormalEngine::prefillWarmUp(const EngineInitParams& params) {
    ...
    const int64_t token_heavy_seq_len = std::max<int64_t>(1, model_config_.max_seq_len - 1);
