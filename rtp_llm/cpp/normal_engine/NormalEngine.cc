...
#if USING_CUDA
    // Cross-file contract: this condition set is restated in the user-visible --warm_up help
    // (rtp_llm/server/server_args/engine_group_args.py) as the "skip warmup" list, and nothing
    // synchronizes the two. Adding or removing a term here requires updating that help in the same
    // commit -- same registration convention as PYTHON_LOG_SENTINEL in _log_runtime_tuning_summary.
