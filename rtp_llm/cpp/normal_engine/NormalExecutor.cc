...
NormalExecutor::NormalExecutor(const EngineInitParams&                   params,
    ...
    warm_up_(warm_up),
    use_all_gather_(params.model_specific_config.use_all_gather_),
    ...
    if (params.eplb_config.enable_eplb && params.model_config_.moe_style_ != 0) {
        // use first moe layer weight as moe weight type
        int  first_moe_layer = params.model_config_.moe_layer_index_.front();
        auto moe_weight_type = params.gpt_weights.layers[first_moe_layer].ffn_weights.moe_gate_weight->kernel->type();

        expert_balancer_ = make_shared<ExpertBalancer>(params.model_config_.expert_num_,
                                                       params.eplb_config.phy_exp_num,
                                                       params.model_config_.num_layers_,
                                                       params.model_config_.moe_inter_padding_size_,
                                                       params.model_config_.hidden_size_,
                                                       params.eplb_config.eplb_update_time,
                                                       params.parallelism_config.ep_rank,
                                                       params.parallelism_config.ep_size,
                                                       params.eplb_config.py_eplb,
                                                       moe_weight_type,
                                                       device_,
                                                       params.eplb_config.eplb_mode,
                                                       params.model_config_.quant_algo_,
                                                       metrics_reporter_);
    }
    ...
    GptModelInitParams model_init_params(
        {device_,
         params.gpt_weights,
         genModelDescription(params.model_config_, params.parallelism_config, params.eplb_config),
    ...
    PrefixToCandidateTokens::instance()->reloadPrefixDictWithPrefix(
        params.model_config_.ckpt_path_, params.sp_config.tree_decode_config);
