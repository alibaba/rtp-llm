...
void NormalExecutor::publishNormalDeviceState(const StreamGroups& stream_groups, const SamplerOutput& sampler_output) {
    ...
    for (const auto& stream : all_streams) {
        ...
        valid_device_seq_lens += prev_state.hasNextSeqLen();
        if (prev_state.hasNextSeqLen()) {
            checkRuntimeCudaDevice(prev_state.next_seq_len_gpu.defined() ? prev_state.next_seq_len_gpu :
                                                                           prev_state.batched_next_seq_lens_gpu,
                                   "publish_normal_device_state",
                                   "previous sequence lengths");
        }

        const auto& batched_seq_lens = prev_state.batched_next_seq_lens_gpu;
        if (!prev_state.hasBatchedState() || prev_state.device_batch_index != prev_batch_idx
    ...
    if (can_reuse_batched_seq_lens && shared_prev_next_seq_lens_gpu.defined()) {
        ...
        for (const auto& stream : all_streams) {
            const auto& prev_state = stream->getNormalAsyncDeviceState();
            if (prev_state.hasNextSeqLen()) {
                cur_seq_len_slices_gpu.push_back(prev_state.nextSeqLenView());
            } else {
