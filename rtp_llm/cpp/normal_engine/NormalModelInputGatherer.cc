...
absl::StatusOr<GptModelInputs> NormalModelInputGatherer::gather(const StreamGroups& stream_groups,
    ...
    const auto& first_streams   = decode_streams.empty() ? context_streams : decode_streams;
    const bool  prefill_only    = !first_streams.empty() && isPrefillOnly(first_streams.front());
    if (hasMixedExecutionModes(decode_streams) || hasMixedExecutionModes(context_streams)
        || (!decode_streams.empty() && !context_streams.empty()
            && isPrefillOnly(decode_streams.front()) != isPrefillOnly(context_streams.front()))) {
