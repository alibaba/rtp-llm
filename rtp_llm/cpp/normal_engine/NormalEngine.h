...
private:
    ...
    void                            loop();
    // Waits out the shared cudacore collection window before the engine exits on
    // a fatal CUDA incident; no-op when no incident was recorded.
    void                            joinFatalCudacoreCollectionWindow(const char* reason) const;
