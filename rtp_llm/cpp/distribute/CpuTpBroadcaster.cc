    ...
    if (tp_rank == 0) {
        try {
            ...
            for (int i = 1; i < tp_size; ++i) {
                ...
                try {
                    ...
                    RTP_LLM_CHECK_WITH_INFO(peer_rank > 0 && peer_rank < tp_size,
                                            ...
                                            peer_rank,
                                            tp_size);
                    RTP_LLM_CHECK_WITH_INFO(peer_fds_[peer_rank] < 0,
                                            "CpuTpBroadcaster duplicate peer rank: %d",
                                            peer_rank);
            ...
            for (int peer_rank = 1; peer_rank < tp_size; ++peer_rank) {
                ssize_t n =
                    writeAllWithTimeout(peer_fds_[peer_rank], &kLinkProbeToken, sizeof(kLinkProbeToken), kInitTimeoutMs);
                int saved = errno;
