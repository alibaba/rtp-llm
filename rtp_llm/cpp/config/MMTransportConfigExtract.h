...
inline MMRdmaConfig extractMMRdmaConfig(const py::object& rdma_config) {
    MMRdmaConfig cfg;
    ...
    cfg.max_inflight_bytes = rdma_config.attr("max_inflight_bytes").cast<int64_t>();
...
inline MMTransportConfig extractMMTransportConfig(const py::object& transport_config) {
    ...
    cfg.rdma    = extractMMRdmaConfig(transport_config.attr("rdma"));
    return cfg;
}
