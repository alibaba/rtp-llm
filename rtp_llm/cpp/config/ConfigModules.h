...
struct MoeConfig {
    ...
    // 1/ep_size mean share. Must be finite and >= 1.0; exactly 1.0 disables the
    // skew (hot share equals the mean, no id rewrite) and is the documented
    // rollback value. This struct does not enforce that -- validation lives in
    // MoeWarmupDiagnostics.reload_runtime_settings (warmup_diagnostics.py) and
    // the argparse layer (--moe_skew_mult in moe_group_args.py).
