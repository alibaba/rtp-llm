...
FIFOScheduler::FIFOScheduler(const RuntimeConfig&                   runtime_config,
    ...
    RTP_LLM_LOG_INFO("max_generate_batch_size is [%zu], max_batch_tokens_size is [%zu], "
                     "max_batch_tokens_without_cache is [%zu], max_batch_kv_len is [%zu], prefill_cp_size is [%zu], "
...
bool FIFOScheduler::evaluateRunningBatch(const ScheduleRuntime&   schedule_runtime,
    ...
    return fitsPrefillTokenLimits(admitted_running_stream_count,
                                  ...
                                  schedule_runtime.admitted_prefill_token_size_without_cache,
...
bool FIFOScheduler::evaluateRunningBatch(const std::list<GenerateStreamPtr>& streams,
    ...
    return fitsPrefillTokenLimits(admitted_count,
                                  admitted_tokens,
                                  admitted_tokens_without_cache,
                                  admitted_max_seq_len,
                                  admitted_sequence_count,
                                  new_stream);
}
...
bool FIFOScheduler::fitsPrefillTokenLimits(size_t                   admitted_stream_count,
    ...
    if (max_batch_kv_len_ > 0) {
        ...
        const auto fits_strict_sum = [running_token_reserve](size_t admitted, size_t candidate, size_t limit) {
            if (running_token_reserve >= limit || admitted >= limit - running_token_reserve) {
                return false;
            }
    ...
    while (!stop_.load(std::memory_order_acquire)) {
        ...
        size_t                         inited_streams = 0;
        {
            ...
            cond_.wait(lock, [this]() {
                ...
                const auto needs_prepare = [this](const GenerateStreamPtr& stream) {
                    if (cache_prepare_blocked_stream_ && stream == cache_prepare_blocked_stream_) {
                        return stream->hasError();
                    }
                    return stream && !stream->hasEvent(StreamEvents::CachePrepared);
                };
                for (const auto& stream : waiting_streams_) {
                    if (needs_prepare(stream)) {
                        return true;
                    }
                }
                for (const auto& group : waiting_group_queue_) {
...
void FIFOScheduler::evaluateWaitingGroupQueue() {
    ...
    const bool has_unprepared_stream =
        async_cache_prepare_enabled_ && std::any_of(group.begin(), group.end(), [](const auto& stream) {
            return !stream->hasEvent(StreamEvents::CachePrepared);
        });
    const bool blocked_by_kv_shortage =
        cache_prepare_blocked_stream_
        && std::find(group.begin(), group.end(), cache_prepare_blocked_stream_) != group.end();
    if (has_unprepared_stream && !blocked_by_kv_shortage) {
        // enqueueGroup is an atomic admission boundary. Cache preparation may
        // overlap the current GPU round, but no member is admitted until every
        // surviving member has completed connector/cache preparation. A KV
        // shortage is handled below by splitting off only prepared members;
        // otherwise prepared members would hold the blocks needed by the
        // residual group forever.
        return;
    }
