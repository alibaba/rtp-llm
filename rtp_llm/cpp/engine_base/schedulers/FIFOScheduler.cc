...
bool FIFOScheduler::hasAvailableKVForPrefill(const list<GenerateStreamPtr>& streams,
    ...
    auto   accumulate_blocks = [&needed_blocks](const GenerateStreamPtr& stream) {
        const int remaining = std::max(0, stream->generateConfig()->max_new_tokens
                                        - static_cast<int>(stream->outputTokenLen()));
        needed_blocks += std::max(stream->estimatePeakNeedBlocks(remaining), 0);
    };
    for (const auto& stream : running_streams_) {
        accumulate_blocks(stream);
    }
    for (const auto& stream : streams) {
        accumulate_blocks(stream);
    }
    accumulate_blocks(new_stream);
    ...
    return needed_blocks <= effective;
}
...
void FIFOScheduler::evaluateWaitingStreams(list<GenerateStreamPtr>& waiting_streams) {
    ...
    const bool prefill_overrides_decode = hasRunningDecode() && !hasRunningPrefill();
