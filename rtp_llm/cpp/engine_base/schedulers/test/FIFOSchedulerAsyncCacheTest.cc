...
protected:
    ...
    void installReadinessAllocator(ContextSelector selector) {
        ...
        mock_allocator_ =
            std::make_shared<testing::NiceMock<MockKVCacheAllocator>>(cache_manager_->config_);
        ...
        ON_CALL(*mock_allocator_, incrMalloc(testing::_))
            .WillByDefault(
                testing::Invoke([this](const MallocInfo& info) { return real_allocator_->incrMalloc(info); }));
        ON_CALL(*mock_allocator_, free(testing::_))
            .WillByDefault(testing::Invoke([this](const FreeInfo& info) {
                ++free_calls_;
        ...
        ON_CALL(*mock_allocator_, singleBatchNeedBlocks(testing::_, testing::_, testing::_))
            .WillByDefault(testing::Invoke([this](const BatchKVCacheResourcePtr& resource,
                                                  int                            seq_len,
                                                  int                            reserve_step) {
                return real_allocator_->singleBatchNeedBlocks(resource, seq_len, reserve_step);
            }));
        ON_CALL(*mock_allocator_, seqSizePerBlock())
            .WillByDefault(testing::Invoke([this]() { return real_allocator_->seqSizePerBlock(); }));
...
TEST_F(FIFOSchedulerAsyncCacheTest,
       testScheduleNew_ReturningFromLoadingCache_SkipsDuplicateInitialReadiness) {
    auto scheduler = createScheduler();
    auto stream    = createStream({1, 2, 3}, /*reuse_cache=*/true);
    auto context   = std::make_shared<ControlledAllocatorContext>();
    ASSERT_TRUE(scheduler->enqueue(stream).ok());
