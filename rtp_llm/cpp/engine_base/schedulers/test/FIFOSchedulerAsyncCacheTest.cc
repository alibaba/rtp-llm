...
TEST_F(FIFOSchedulerAsyncCacheTest, testEmptyRunningDoesNotRetryBlockedPrepareWithoutResourceChange) {
...
TEST_F(FIFOSchedulerAsyncCacheTest, testAsyncPrepareGroupStartsAtEnqueueAndWaitsForEveryMember) {
    ...
    // Both preparations start from enqueueGroup(), before schedule() is called.
    ASSERT_TRUE(waitUntil([&]() {
...
TEST_F(FIFOSchedulerAsyncCacheTest, testAsyncPrepareGroupSplitsAtKvShortageWithoutDeadlock) {
    ...
    // The prepared prefix is a complete sub-batch. Running it is required to
    // release the KV blocks that the residual member needs.
...
TEST_F(FIFOSchedulerAsyncCacheTest, testStopWithoutAsyncWorkerCancelsWaitingGroup) {
    autil::EnvGuard disable_async_prepare("RTP_LLM_ASYNC_PREPARE_CACHE", "0");
    auto            scheduler = createScheduler();
    auto            first     = createStream();
    auto            second    = createStream();

    ASSERT_EQ(scheduler->enqueueGroup({first, second}).first, std::vector<bool>({true, true}));
    ...
    for (const auto& stream : {first, second}) {
        ...
        EXPECT_EQ(stream->stopReason(), "scheduler stopped");
    }
}

TEST_F(FIFOSchedulerAsyncCacheTest, testStopWithoutAsyncWorkerCancelsLoadingGroup) {
    autil::EnvGuard disable_async_prepare("RTP_LLM_ASYNC_PREPARE_CACHE", "0");
    setupMockCoordinator();
    auto pending_ctx = createPendingAsyncContext();
    EXPECT_CALL(*mock_coord_, asyncRead(_)).WillOnce(Return(std::static_pointer_cast<AsyncContext>(pending_ctx)));
    ...
    EXPECT_TRUE(scheduler->empty());
    EXPECT_EQ(scheduler->onflightStreams(), 0);
    for (const auto& stream : {direct_stream, loading_stream}) {
        EXPECT_TRUE(stream->hasError());
        EXPECT_EQ(stream->statusInfo().code(), ErrorCode::CANCELLED);
        EXPECT_EQ(stream->stopReason(), "scheduler stopped");
    }
