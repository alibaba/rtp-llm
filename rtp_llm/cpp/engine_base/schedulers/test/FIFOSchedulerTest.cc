...
TEST_F(FIFOSchedulerTest, testMaxBatchKvLenUsesSumQAndKvLengthAndIsOptIn) {
    ...
    // Uneven requests: both sum(KV)=90 and sum(q)=90 fit. The default padded
    // rectangle rule would reject this pair as max_len(70) * batch(2) = 140.
    std::list<GenerateStreamPtr> admitted{make_stream(70)};
    EXPECT_TRUE(scheduler.evaluateRunningBatch(admitted, make_stream(20)));

    // MAX_BATCH_TOKENS_SIZE remains the strict sum(q) limit while the KV limit
    // is enabled: KV 70 + 20 fits 100, q 70 + 20 exceeds 80.
    ...
    // Disabled by default: retain the auto-TPM full-token/rectangle limit.
    runtime_config.fifo_scheduler_config.max_batch_kv_len = 0;
    FIFOScheduler default_scheduler(
        runtime_config, model_config, pd_sep_config, parallelism_config, model_specific_config, cache_manager);
    EXPECT_FALSE(default_scheduler.evaluateRunningBatch(admitted, make_stream(20)));

    // The KV cap uses the full visible sequence, not post-reuse q length.
    // Both requests have q_len=10, but their KV sum is 60 + 50 = 110.
    std::list<GenerateStreamPtr> reuse_admitted{make_stream(60, 50)};
    EXPECT_FALSE(scheduler.evaluateRunningBatch(reuse_admitted, make_stream(50, 40)));

    // Preserve the strict upper bound: a sum equal to the limit does not fit.
    admitted.push_back(make_stream(20));
    EXPECT_FALSE(scheduler.evaluateRunningBatch(admitted, make_stream(10)));
}
