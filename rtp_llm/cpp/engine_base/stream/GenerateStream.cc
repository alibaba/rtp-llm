    ...
    *is_context_stream_ = false;
    ...
    if (update_info.draft_tokens_cpu.defined()) {
        ...
        RTP_LLM_CHECK_WITH_INFO(sp_output_buffer_->tokens.numel() >= proposal.numel() + 1,
                                ...
                                proposal.numel() + 1);
        std::copy(proposal.data_ptr<int32_t>(), proposal.data_ptr<int32_t>() + proposal.numel(), spec_tokens + 1);
        propose_token_.resize(1 + proposal.numel());
        propose_token_[0] = target_last_token;
        std::copy(
            proposal.data_ptr<int32_t>(), proposal.data_ptr<int32_t>() + proposal.numel(), propose_token_.begin() + 1);
    } else {
