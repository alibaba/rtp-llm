...
StreamState GenerateStream::moveToNext() {
    ...
    bool        should_report_metric = false;
    {
        ...
        // Timeout can become an error here, after the scheduler's earlier
        // check. Keep KV ownership until every dispatched PP chunk drains.
        if (ppOutstandingResults() > 0 && hasErrorWithoutLock()) {
            return getStatus();
        }
...
void GenerateStream::updateFromPP(const StreamUpdateInfo& update_info) {
    ...
    if (update_info.intermediate_chunk) {
        // Use dispatch-time chunk state: overlapping cursors have already advanced.
        // An intermediate result must not append tokens or end context mode.
        return;
    }
    *is_context_stream_ = false;
    if ((hasErrorWithoutLock() || isFinished()) && !update_info.force_update_info) {
