...
absl::Status GenerateStream::initKVBlock() {
    ...
    if (!ret.ok()) {
        RTP_LLM_LOG_WARNING("GenerateStream::initKVBlock: initKVBlock failed, stream_id: %llu",
...
uint64_t GenerateStream::streamId() const {
...
bool GenerateStream::waitForRemoteGenerate() {
    ...
    if (hasError()) {
        RTP_LLM_LOG_WARNING("waitForRemoteGenerate exits due to stream [%llu] error: %s",
                            (unsigned long long)streamId(),
...
void GenerateStream::specUpdate(const StreamSpecUpdateInfo& update_info) {
    RTP_LLM_PROFILE_FUNCTION();
    std::lock_guard<std::mutex> lock(*mutex_);
    RTP_LLM_LOG_DEBUG("stream [%llu] spec update", (unsigned long long)streamId());
    *is_context_stream_ = false;
    ...
    if (accept_token_num > 1 && stream_cache_resource_) {
        ...
        RTP_LLM_LOG_DEBUG("[stream %llu (%d -> %d)] swap cache blocks: %d -> %d, %d -> %d",
                          (unsigned long long)streamId(),
                          cur_cached_len + 1,
                          nxt_cached_len + 1,
        ...
        RTP_LLM_LOG_DEBUG("[stream %llu (%d -> %d)] no swap cache blocks",
                          (unsigned long long)streamId(),
                          cur_cached_len + 1,
                          nxt_cached_len + 1);
    }
...
void GenerateStream::update(const StreamUpdateInfo& update_info) {
    RTP_LLM_PROFILE_FUNCTION();
    std::lock_guard<std::mutex> lock(*mutex_);
    RTP_LLM_LOG_DEBUG("stream [%llu] update", (unsigned long long)streamId());
    *is_context_stream_ = false;
    if (hasError() && !update_info.force_update_info) {
        return;
    }
...
void GenerateStream::reportStreamMetrics() {
    ...
    if (metrics_reporter_) {
        ...
        if (getStatus() == StreamState::FINISHED || cancelled || timeout) {
            ...
            RTP_LLM_LOG_DEBUG("stream [%llu] report first latency us = %ld",
                              (unsigned long long)streamId(),
