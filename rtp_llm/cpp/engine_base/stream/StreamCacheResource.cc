...
static torch::Tensor buildFirstTokenUpdateTokens(GenerateStream* stream, int32_t first_token_id) {
    const auto next_batch_size = static_cast<int64_t>(stream->nextBatchSize());
    if (!stream->hasNumBeams()) {
        auto new_tokens = torch::empty({next_batch_size, 1}, torch::kInt32);
        std::fill(new_tokens.data_ptr<int32_t>(), new_tokens.data_ptr<int32_t>() + next_batch_size, first_token_id);
        return new_tokens;
    }

    const auto seq_len          = static_cast<int64_t>(stream->seqLength());
    auto       new_tokens       = torch::zeros({next_batch_size, seq_len + 1}, torch::kInt32);
    const auto current_batches  = std::max<int64_t>(1, static_cast<int64_t>(stream->currentBatchSize()));
    const auto token_stride     = new_tokens.size(1);
    auto*      new_tokens_ptr   = new_tokens.data_ptr<int32_t>();
    for (int64_t batch_idx = 0; batch_idx < next_batch_size; ++batch_idx) {
        const auto source_batch_idx = static_cast<int>(std::min(batch_idx, current_batches - 1));
        const auto prefix_tokens    = stream->completeTokenIdsVec(source_batch_idx);
        auto*      row              = new_tokens_ptr + batch_idx * token_stride;
        std::copy(prefix_tokens.begin(), prefix_tokens.end(), row);
...
static bool applyP2PSideChannelToStream(const std::shared_ptr<FusedAsyncReadContext>& read_context,
    ...
    if (payload->has_first_token) {
        ...
        stream->step();
        auto new_tokens = buildFirstTokenUpdateTokens(stream, static_cast<int32_t>(payload->first_token_id));
