...
bool StreamCacheResource::updateKVBlock(const std::vector<int>& block_src_batch, int cached_sequence_length) {
    return resource_context_.cache_manager->updateKVBlock(
        batch_kv_cache_resource_, block_src_batch, cached_sequence_length, block_update_mapping_);
}
...
void StreamCacheResource::adjustLinearBlocksForAcceptedTokens(int32_t batch_id,
                                                              int     cur_cached_len,
                                                              int     nxt_cached_len) {
    ...
    for (const auto& group : cache_config.topology().groups()) {
        ...
        auto [cached_src, cached_dst] = getCachedTokenBlockSwapIdx(cur_cached_len, nxt_cached_len, span);
        if (cached_src != cached_dst) {
            batch_kv_cache_resource_->swapBlocks(batch_id, group.tag, cached_src, cached_dst);
        }
        auto [final_src, final_dst] = getFinalTokenBlockSwapIdx(cur_cached_len, nxt_cached_len, span);
        if (final_src != final_dst) {
            batch_kv_cache_resource_->swapBlocks(batch_id, group.tag, final_src, final_dst);
        }
        RTP_LLM_LOG_DEBUG("adjust speculative LINEAR cache tag=%s span=%d: %d -> %d, %d -> %d",
                          group.tag.c_str(),
                          span,
                          cached_src,
                          cached_dst,
