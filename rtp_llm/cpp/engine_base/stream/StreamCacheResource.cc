...
void StreamCacheResource::init(int batch_size) {
    ...
    std::vector<std::vector<int>> layer_attn_to_group = {};
    ...
    if (resource_context_.cache_manager) {  // cache manager is null when warmup
        ...
        layer_attn_to_group      = cache_config.layer_attn_to_group_id;
    ...
    batch_kv_cache_resource_->initGroups(
        group_nums, layer_all_num, layer_to_group, kernel_blocks_per_kv_block, group_types, layer_attn_to_group);
