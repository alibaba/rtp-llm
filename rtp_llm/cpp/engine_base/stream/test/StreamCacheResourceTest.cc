...
TEST_F(StreamCacheResourceTest, SpeculativeLinearSwapUsesTagLocalPhysicalSpan) {
    auto        config = test::makeSimpleHybridMhaCacheConfig(/*layer_num=*/4,
                                                       /*block_num=*/9,
                                                       /*tokens_per_block=*/4,
                                                       rtp_llm::DataType::TYPE_FP16,
                                                       /*group_layer_num=*/2);
    auto        groups = config.topology().groups();
    std::string linear_tag;
    std::string full_tag;
    for (auto& group : groups) {
        if (group.policy.group_type == CacheGroupType::LINEAR) {
            linear_tag                      = group.tag;
            group.seq_size_per_block        = 2;
            group.kernel_seq_size_per_block = 2;
        } else {
            full_tag                        = group.tag;
            group.seq_size_per_block        = 4;
            group.kernel_seq_size_per_block = 4;
        }
    }
    config.setTopology(std::move(groups), config.topology().layers());
    prepareResourceWithCacheConfig(
        config, /*input_tokens=*/{1, 2, 3}, /*reuse_cache=*/false, RoleType::PDFUSION, /*initial free=*/12);

    auto& cache_resource = stream_->streamCacheResource();
    EXPECT_EQ(cache_resource.seqSizePerBlock(), 4);
    auto& batch_resource = cache_resource.kvCacheMutable();
    auto  linear_blocks  = cache_manager_->allocator_->blockPool(linear_tag)->malloc(4);
    auto  full_blocks    = cache_manager_->allocator_->blockPool(full_tag)->malloc(2);
    ASSERT_EQ(linear_blocks.size(), 4u);
    ASSERT_EQ(full_blocks.size(), 2u);
    batch_resource.mutableBlockIds(0, linear_tag).assign(linear_blocks);
    batch_resource.mutableBlockIds(0, full_tag).assign(full_blocks);

    cache_resource.adjustLinearBlocksForAcceptedTokens(
        /*batch_id=*/0, /*cur_cached_len=*/2, /*nxt_cached_len=*/5);

    EXPECT_EQ(batch_resource.blocks(0, linear_tag),
              (BlockIndicesType{linear_blocks[0], linear_blocks[2], linear_blocks[3], linear_blocks[1]}));
    EXPECT_EQ(batch_resource.blocks(0, full_tag), full_blocks);
}
