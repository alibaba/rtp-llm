...
class DelayedCallbackCacheStore: public MemoryBackedCacheStore {
public:
    ~DelayedCallbackCacheStore() override {
    ...
    void store(const std::shared_ptr<RequestBlockBuffer>&, CacheStoreStoreDoneCallback callback) override {
        callback_thread_ = std::thread([callback = std::move(callback)]() mutable {
    ...
    storeBuffers(const std::vector<std::shared_ptr<RequestBlockBuffer>>& request_block_buffers,
        ...
        observed_timeout_ms_ = timeout_ms;
        return NormalCacheStore::storeBuffers(request_block_buffers, timeout_ms);
    }

    int64_t observed_timeout_ms_ = 0;
...
TEST_F(PdSepKVCacheReleaseTest, testChunkPrefillWaitsForCacheStoreStagingCallback) {
    const int kv_stride = 64;
    auto      inputs = makeSingleBlockWriteInputs("chunk-key", 5252, 8, kv_stride, 0, true, KVCacheRegionName::CSA_STATE);
    inputs.wait_cache_store_done       = true;
    inputs.cache_store_wait_timeout_ms = 3210;
    ...
    auto cache_store = std::make_shared<DelayedCallbackCacheStore>();
    auto started     = std::chrono::steady_clock::now();
    runtimeWriteCacheStore(inputs, kv_cache_info, /*mla_kvcache=*/false, cache_store);
    auto elapsed = std::chrono::steady_clock::now() - started;
    EXPECT_GE(elapsed, std::chrono::milliseconds(90));
    EXPECT_EQ(cache_store->observed_timeout_ms_, 3210);
}
