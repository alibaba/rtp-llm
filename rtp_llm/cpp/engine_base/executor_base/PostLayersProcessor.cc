...
PostLayersProcessor::~PostLayersProcessor() {
    ...
    } catch (const std::exception& e) { RTP_LLM_LOG_ERROR("error releasing post-layers handler: %s", e.what()); }
}
...
void PostLayersProcessor::setHandler(py::object handler) {
    ...
    handler_args_ =
        HandlerArgs::parse(py::cast<std::vector<std::string>>(handler_.attr("extend_forward_args")()), &unknown);
    if (!unknown.empty()) {
    ...
    if (wants_selected) {
        if (has_position == has_token_id) {
            throw std::runtime_error("selected_hidden_states requires exactly one of CUSTOM_OUTPUT_TOKEN_POSITION or "
...
void PostLayersProcessor::warmup(int64_t hidden_size, c10::ScalarType dtype) const {
    ...
    for (const int64_t batch_size : {int64_t(1), int64_t(8)}) {
        auto dummy = torch::zeros({batch_size, hidden_size}, torch::TensorOptions().dtype(dtype).device(torch::kCUDA));
        invokeHandler(dummy);
    }
