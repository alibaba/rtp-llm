...
PyObject* serializeGrammarTokenizerInfo(PyObject*, PyObject* args) {
    ...
    if (!PyArg_ParseTuple(args,
                          "Os:serialize_grammar_tokenizer_info",
                          &encoded_vocab_obj,
                          &tokenizer_metadata_json)) {
        return nullptr;
    }

    PyObject* encoded_vocab_seq = PySequence_Fast(encoded_vocab_obj,
                                                  "encoded_vocab must be a sequence of str");
    ...
    for (Py_ssize_t i = 0; i < vocab_size; ++i) {
        ...
        const char* token      = PyUnicode_AsUTF8AndSize(items[i], &token_size);
        if (token == nullptr) {
            Py_DECREF(encoded_vocab_seq);
            PyErr_SetString(PyExc_TypeError, "encoded_vocab must be a sequence of str");
            return nullptr;
        }
    ...
    try {
        const auto result = rtp_llm::xgrammar_impl::serializeTokenizerInfo(
            encoded_vocab, tokenizer_metadata_json);
        return PyUnicode_FromStringAndSize(result.data(),
