...
enum class MMRdmaReadStatus {
    ...
    // slot. The transport owns deferred cleanup until completion is confirmed;
    // unconfirmed reads retain their leases until transport teardown.
    IN_FLIGHT,
};
...
public:
    ...
    // On IN_FLIGHT (or an exception after submission), the transport invokes
    // release_after_completion once completion is confirmed, off the NIC callback
    // thread. The callback must own its captures, including the remote handle.
    // For any other return status the caller remains responsible for release.
    // release_deferred is initialized false, then set true only after cleanup
    // bookkeeping is prepared and immediately before submission. On exceptions
    // it tells the caller whether cleanup belongs to the transport. The pointer
    // is used only during this call and is never captured by async callbacks.
    virtual MMRdmaReadStatus
    readEmbedding(const MMRdmaDescPB& desc,
                  std::vector<torch::Tensor>* out,
                  int64_t timeout_ms,
                  std::function<void()> release_after_completion = {},
                  bool* release_deferred = nullptr) = 0;
};
