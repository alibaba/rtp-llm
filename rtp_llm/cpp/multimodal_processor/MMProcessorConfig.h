...
inline const char* roleTypeName(RoleType role_type) {
    switch (role_type) {
...
// Keep this ownership rule aligned with mm_ingress.py.
...
inline std::string mmProcessorConfigError(VitSeparation vit_separation,
    ...
    return "invalid multimodal processor config: vit_separation=" + std::string(vitSeparationName(vit_separation))
           ...
           + roleTypeName(role_type) + ", tp_rank=" + std::to_string(tp_rank) + ", model_type=" + model_type
           + " (most likely cause: the Python side did not create mm_process_engine for this"
             " process -- see should_create_local_mm_process_engine in"
             " rtp_llm/multimodal/mm_ingress.py)";
}
...
inline MMProcessorDecision resolveAndLogMMProcessorKind(bool               is_multimodal,
    ...
    const std::string described = "entry=" + entry + ", vit_separation=" + vitSeparationName(vit_separation)
                                  + ", role_type=" + roleTypeName(role_type) + ", tp_rank=" + std::to_string(tp_rank)
