...
int64_t resolveRpcTimeoutMs(const MultimodalInputsPB& request) {
    ...
    for (const auto& mm_input : request.multimodal_inputs()) {
        max_timeout_ms = std::max<int64_t>(max_timeout_ms, mm_input.mm_preprocess_config().mm_timeout_ms());
    }
    return max_timeout_ms > 0 ? max_timeout_ms : kDefaultVitRpcTimeoutMs;
}
...
void MMTransportMetrics::reportCircuitState(const std::string& backend,
    ...
    tags.AddTag("source", kMetricSourceInferenceClient);
    tags.AddTag("target", endpoint);
    const std::string metric = "rtp_llm_vit_" + backend + "_circuit_open";
    reporter_->report(open ? 1 : 0, metric, kmonitor::MetricType::GAUGE, &tags, true);
}
...
ErrorResult<MultimodalOutput> MMRemoteOutputTransport::fetch(const std::string&  endpoint,
    ...
    DeadlineBudget  budget(resolveRpcTimeoutMs(request_pb));
    ...
    if (matched != nullptr) {
        ...
        return degradeToTerminal(endpoint, request_pb, context, result.reason());
    }
...
ErrorResult<MultimodalOutput> MMRemoteOutputTransport::degradeToTerminal(const std::string&  endpoint,
                                                                        ...
                                                                        const std::string&  reason) {
    metrics_->reportRpcClientError(endpoint, reason);
    for (auto& reader : readers_) {
        reader->withdraw(request_pb);
    }
    if (context.budget.exhausted()) {
        return ErrorInfo(ErrorCode::MM_PROCESS_ERROR, "data-plane fallback exhausted request deadline");
    }
    auto receipt = control_->request(endpoint, request_pb, context.budget);
    if (!receipt.ok()) {
        return ErrorInfo(ErrorCode::MM_PROCESS_ERROR,
                         "data plane failed and grpc-inline fallback also failed: " + receipt.status().ToString());
    }
    // A fallback response must be inline.
    if (auto* matched = matchReader(receipt.value()); matched != nullptr) {
        matched->discard(receipt.value(), context);
        return ErrorInfo(ErrorCode::MM_PROCESS_ERROR,
                         std::string("data plane failed and fallback response unexpectedly carried a '")
                             + matched->name() + "' receipt");
    }
    return terminal_->consumeTerminal(receipt.value(), context);
}
