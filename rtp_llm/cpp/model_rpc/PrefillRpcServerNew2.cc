...
bool PrefillRpcServerNew2::isValidGrpcPort(int64_t port) {
    return rtp_llm::isValidGrpcPort(port);
}

std::string PrefillRpcServerNew2::formatHostPort(const std::string& host, int64_t port) {
    return formatGrpcHostPort(host, port);
}
...
bool PrefillRpcServerNew2::parseP2PWorkerGrpcAddr(const std::string& entry, std::string* grpc_addr) {
    ...
    *grpc_addr = formatHostPort(host, grpc_port);
    return !grpc_addr->empty();
}

bool PrefillRpcServerNew2::isContextCancelled(grpc::ServerContext* context) const {
    return context && context->IsCancelled();
}

bool PrefillRpcServerNew2::hasStartLoadEngine() const {
    return engine_ != nullptr;
}

bool PrefillRpcServerNew2::hasStartLoadCacheManager() const {
    return engine_ && engine_->getCacheManager() != nullptr;
}

void PrefillRpcServerNew2::handleStartLoadRead(const P2PConnectorStartLoadRequestPB& request,
                                               P2PConnectorStartLoadResponsePB&      response,
                                               std::function<bool()>                 is_cancelled) {
    engine_->getCacheManager()->handleRead(request, response, std::move(is_cancelled));
}

grpc::Status PrefillRpcServerNew2::callLocalGenerateStream(grpc::ServerContext*                   context,
                                                           const GenerateInputPB*                 request,
                                                           grpc::ServerWriter<GenerateOutputsPB>* writer) {
    return LocalRpcServer::GenerateStreamCall(context, request, writer);
}

bool PrefillRpcServerNew2::engineIsMtpEagle() const {
    return engine_ && engine_->isMTPEagle();
}

std::shared_ptr<GenerateStream> PrefillRpcServerNew2::enqueuePrefillStream(const std::shared_ptr<GenerateInput>& input) {
    return engine_->enqueue(input);
}

grpc::Status PrefillRpcServerNew2::pollPrefillStreamOutput(grpc::ServerContext*             context,
                                                           const std::string&               request_key,
                                                           WriterInterface*                 writer,
                                                           std::shared_ptr<GenerateStream>& stream) {
    return pollStreamOutput(context, request_key, writer, stream);
}
...
grpc::Status PrefillRpcServerNew2::GenerateStreamCall(grpc::ServerContext*                   server_context,
    ...
    if (!pd_separation) {
        ...
        return callLocalGenerateStream(server_context, request, response_writer);
    }
    ...
    auto stream     = enqueuePrefillStream(input);
    ...
    generate_context.error_status =
        pollPrefillStreamOutput(server_context, generate_context.request_key, response_writer, generate_context.getStream());
...
::grpc::Status PrefillRpcServerNew2::StartLoad(::grpc::ServerContext*                context,
    ...
    auto    is_cancelled      = [this, context]() { return isContextCancelled(context); };
    int64_t handle_read_start = currentTimeUs();
    handleStartLoadRead(*request, *response, is_cancelled);
...
::grpc::Status PrefillRpcServerNew2::GetPeerInfo(::grpc::ServerContext*      context,
    ...
    if (!dp_grpc_addrs_.empty()) {
        ...
        for (int64_t i = 0; i < pc.dp_size; ++i) {
            ...
            auto addr = formatHostPort(bind_ip, port);
            if (addr.empty()) {
