...
void PrefillRpcServer::pollRemoteOutput(PrefillGenerateContext& prefill_context) {
    ...
    while (prefill_context.client_stream->Read(&response)) {
        ...
        if (response.generate_outputs().aux_info_size() == 0) {
            RTP_LLM_LOG_ERROR("request [%ld] generate output size is 0", request_id);
            break;
        }
        for (size_t i = 0; i < response.generate_outputs().aux_info_size(); i++) {
            response.mutable_generate_outputs()->mutable_aux_info(i)->set_pd_sep(true);
        }
        int64_t cost_time_us = currentTimeUs() - prefill_context.request_begin_time_us;
        for (size_t i = 0; i < response.generate_outputs().aux_info_size(); i++) {
            response.mutable_generate_outputs()->mutable_aux_info(i)->set_first_token_cost_time_us(first_token_rt_us);
            response.mutable_generate_outputs()->mutable_aux_info(i)->set_cost_time_us(cost_time_us);

            response.mutable_generate_outputs()->mutable_aux_info(i)->set_total_reuse_len(prefill_total_reuse_len);
            response.mutable_generate_outputs()->mutable_aux_info(i)->set_local_reuse_len(prefill_local_reuse_len);
            response.mutable_generate_outputs()->mutable_aux_info(i)->set_remote_reuse_len(prefill_remote_reuse_len);

            response.mutable_generate_outputs()->mutable_aux_info(i)->set_prefill_total_reuse_len(
                prefill_total_reuse_len);
            response.mutable_generate_outputs()->mutable_aux_info(i)->set_prefill_local_reuse_len(
                prefill_local_reuse_len);
            response.mutable_generate_outputs()->mutable_aux_info(i)->set_prefill_remote_reuse_len(
                prefill_remote_reuse_len);

            response.mutable_generate_outputs()->mutable_aux_info(i)->set_decode_total_reuse_len(
                response.generate_outputs().aux_info(i).total_reuse_len());
            response.mutable_generate_outputs()->mutable_aux_info(i)->set_decode_local_reuse_len(
                response.generate_outputs().aux_info(i).local_reuse_len());
            response.mutable_generate_outputs()->mutable_aux_info(i)->set_decode_remote_reuse_len(
                response.generate_outputs().aux_info(i).remote_reuse_len());
        }
