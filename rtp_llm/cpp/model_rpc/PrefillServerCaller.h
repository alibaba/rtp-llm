...
public:
    ...
    // 等待 Prefill 完成
    grpc::Status waitPrefillDone(const std::shared_ptr<GenerateStream>& stream,
...
private:
    // 响应第一个 token
    grpc::Status responseFirstToken(const std::shared_ptr<GenerateStream>& stream,
                                    grpc::ServerContext*                   server_context,
                                    grpc::ServerWriter<GenerateOutputsPB>* response_writer);
...
public:
    PrefillServerCaller(const std::string& process_id, int64_t decode_polling_call_prefill_ms, bool is_mtp_eagle);
