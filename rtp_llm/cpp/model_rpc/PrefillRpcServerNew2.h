...
protected:
    virtual bool isContextCancelled(grpc::ServerContext* context) const;
    virtual bool hasStartLoadEngine() const;
    virtual bool hasStartLoadCacheManager() const;
    virtual void handleStartLoadRead(const P2PConnectorStartLoadRequestPB& request,
                                     P2PConnectorStartLoadResponsePB&      response,
                                     std::function<bool()>                 is_cancelled);

    virtual grpc::Status callLocalGenerateStream(grpc::ServerContext*                   context,
                                                 const GenerateInputPB*                 request,
                                                 grpc::ServerWriter<GenerateOutputsPB>* writer);
    virtual bool engineIsMtpEagle() const;
    virtual std::shared_ptr<GenerateStream> enqueuePrefillStream(const std::shared_ptr<GenerateInput>& input);
    virtual grpc::Status pollPrefillStreamOutput(grpc::ServerContext*             context,
...
private:
    ...
    static bool        isValidGrpcPort(int64_t port);
    static std::string formatHostPort(const std::string& host, int64_t port);
