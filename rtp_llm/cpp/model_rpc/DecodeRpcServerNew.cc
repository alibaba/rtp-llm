...
ErrorInfo DecodeRpcServerNew::writeAppendFirstToken(DecodeGenerateContextNew& decode_context) {
    ...
    for (size_t i = 0; i < response_output->generate_outputs_size(); i++) {
        ...
        response_output->mutable_generate_outputs(i)->mutable_aux_info()->set_prefill_gpu_reuse_len(
            response_output->generate_outputs(i).aux_info().gpu_reuse_len());
        response_output->mutable_generate_outputs(i)->mutable_aux_info()->set_prefill_memory_reuse_len(
            response_output->generate_outputs(i).aux_info().memory_reuse_len());
