...
ErrorInfo DecodeRpcServerNew::writeAppendFirstToken(DecodeGenerateContextNew& decode_context) {
    ...
    for (size_t i = 0; i < response_output->generate_outputs().aux_info_size(); i++) {
        response_output->mutable_generate_outputs()->mutable_aux_info(i)->set_first_token_cost_time_us(
            first_token_rt_us);
        response_output->mutable_generate_outputs()->mutable_aux_info(i)->set_cost_time_us(cost_time_us);

        response_output->mutable_generate_outputs()->mutable_aux_info(i)->set_prefill_total_reuse_len(
            response_output->generate_outputs().aux_info(i).total_reuse_len());
        response_output->mutable_generate_outputs()->mutable_aux_info(i)->set_prefill_local_reuse_len(
            response_output->generate_outputs().aux_info(i).local_reuse_len());
        response_output->mutable_generate_outputs()->mutable_aux_info(i)->set_prefill_remote_reuse_len(
            response_output->generate_outputs().aux_info(i).remote_reuse_len());

        response_output->mutable_generate_outputs()->mutable_aux_info(i)->set_decode_total_reuse_len(
            decode_total_reuse_len);
        response_output->mutable_generate_outputs()->mutable_aux_info(i)->set_decode_local_reuse_len(
            decode_local_reuse_len);
        response_output->mutable_generate_outputs()->mutable_aux_info(i)->set_decode_remote_reuse_len(
