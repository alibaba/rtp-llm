...
TEST(DSV41RpcStateTest, BoundedProgressPreservesDecoderBoundaryAndRequiresProtectedCopyBeforeSuffix) {
    auto identity = dsv41CacheIdentity(cacheConfig(RoleType::DECODE, true));
    identity.replay_mode = DSV41ReplayMode::BOUNDED_CHECKPOINT_V1;
    DSV41CacheState state(identity);
    const auto metadata = publication(1024, 3).checkpoint(identity, 1024);
    state.restore(metadata, 1024);
    EXPECT_EQ(state.view().target_ready_end, 1024);
    state.requireProtectedPrefix(1024, 1281);
    EXPECT_THROW(state.publishProgress({19, 1152, 1024}), std::logic_error);
    EXPECT_EQ(state.view().encoder_materialized_end, 1024);
    state.protect(std::make_shared<DSV41CheckpointSnapshot>(metadata));
    state.publishProgress({19, 1152, 1024});
    EXPECT_EQ(state.view().encoder_materialized_end, 1152);
    EXPECT_EQ(state.view().decoder_checkpoint_end, 1024);
    EXPECT_EQ(state.view().target_ready_end, 0);
    EXPECT_FALSE(state.view().execution.has_value());
    EXPECT_THROW(state.publishProgress({19, 1281, 1152}), std::invalid_argument);
    EXPECT_THROW(state.finish(1281), std::logic_error);
    state.publishExecution(publication(1281, 3), 3);
    state.finish(1281);
    EXPECT_TRUE(state.view().finished);
    EXPECT_EQ(state.view().target_ready_end, 1281);
    EXPECT_THROW(state.publishProgress({19, 1281, 1281}), std::logic_error);
}

TEST(DSV41RpcStateTest, FinalAlignedBoundaryNeedsNoProtectionWhenThereIsNoSuffix) {
    auto identity = dsv41CacheIdentity(cacheConfig(RoleType::DECODE, true));
    identity.replay_mode = DSV41ReplayMode::BOUNDED_CHECKPOINT_V1;
    DSV41CacheState state(identity);
    state.requireProtectedPrefix(1024, 1024);
    state.publishExecution(publication(1024, 3), 3);
    state.completeDecoder(publication(1024, 3).checkpoint(identity, 1024), 1024);
    EXPECT_NO_THROW(state.finish(1024));
    EXPECT_TRUE(state.view().finished);
}

TEST(DSV41RpcStateTest, ExecutionContextKeepsRequestAndWorkerMappingsExact) {
    DSV41ExecutionContext context;
    context.request_id = 19;
    bool active = true;
    context.is_active = [&] { return active; };
    context.report_progress = [](const DSV41ExecutionProgress& value) {
        EXPECT_EQ(value.encoder_materialized_end, 1024);
    };
    const DSV41ExecutionContext::WorkerBlockIds workers{{{11, 13}}, {{23, 29}}};
    context.protect_checkpoint = [&](const auto& value, const auto& local, const auto& actual_workers) {
        EXPECT_EQ(value.materialized_end, 1024);
        EXPECT_EQ(local, workers.front());
        EXPECT_EQ(actual_workers, workers);
        return true;
    };
    EXPECT_TRUE(context.isActive());
    EXPECT_NO_THROW(context.reportProgress({19, 1024, 0}));
    EXPECT_THROW(context.reportProgress({20, 1024, 0}), std::invalid_argument);
    EXPECT_TRUE(context.protectCheckpoint(publication(1024, 3), workers.front(), workers));
    auto other = publication(1024, 3);
    other.request_id = 20;
    EXPECT_THROW(context.protectCheckpoint(other, workers.front(), workers), std::invalid_argument);
    active = false;
