...
TEST(DecodeRpcServerTest, EmbeddingRemoteHandlerReportsDeterministicErrorsOnceWithoutRetryOrAllocation) {
    for (const bool malformed : {true, false}) {
        ...
        // Validation fails before the first KV allocation attempt and is reported once.
        const auto logs        = log_capture.content();
        const auto failure_pos = logs.find("allocate resource failed after 0 attempts");
        ASSERT_NE(failure_pos, std::string::npos);
        EXPECT_EQ(logs.find("allocate resource failed after", failure_pos + 1), std::string::npos);
