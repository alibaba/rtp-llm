...
class ModelRpcClientTest(TestCase):
    ...
    def test_enqueue_cancels_generate_stream_on_early_close(self):
        ...
        # returned generator early -- what the frontend does when a client disconnects -- must cancel
        # the RPC, because that gRPC cancellation is what makes the backend release the engine stream,
        # its KV blocks and its per-rank admission slot (GenerateContext::stopStream on the server).
        # GeneratorExit is a BaseException, so it bypasses both except clauses but still runs the
        # finally that performs the cancel.
        async def run_and_close():
            gen = client.enqueue(input_py)
            await gen.__anext__()
            await gen.aclose()
