...
TEST(LocalRpcServerTest, DirectAllocatorDumpRpcIsSingleFlightButInternalFanoutRemainsAllowed) {
    TestLocalRpcServer server;
    server.configureAllocatorDump(true, "secret");
    ...
    grpc::ServerContext        internal_context;
    TorchAllocatorDumpResultPB internal_response;
    EXPECT_TRUE(server.DumpTorchAllocatorInternal(&internal_context, &first_request, &internal_response).ok());
    EXPECT_EQ(internal_response.dump_id(), first_request.dump_id());

    release_dump.set_value();
    ASSERT_EQ(first_call.wait_for(std::chrono::seconds(5)), std::future_status::ready);
    EXPECT_TRUE(first_call.get().ok());
    EXPECT_EQ(dump_calls.load(), 2);
}
