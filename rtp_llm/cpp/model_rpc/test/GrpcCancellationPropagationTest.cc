...
public:
    ::grpc::Status
    RemoteGenerate(::grpc::ServerContext*                                                context,
    ...
    void setEmitBeforeStall(int n) {
        emit_before_stall_ = n;
    }
...
public:
    ...
    void setDownstreamDeadlineMs(int64_t ms) {
        downstream_deadline_ms_ = ms;
    }
...
public:
    ...
    ::grpc::Status upstreamStatus() const {
        return upstream_status_;
    }

    FakePrefillService* prefill() {
        return prefill_.get();
    }
    SilentDecodeService* decode() {
        return &decode_;
    }

    // A constructor cannot use ASSERT_*, so startup failures are reported here instead.
    bool ok() const {
        return init_error_.empty();
    }
...
TEST(CancellationPropagation, UpstreamCancelReleasesASilentDownstreamWithinBudget) {
    ...
    ASSERT_TRUE(pair.prefill()->waitForReturn(Ms(kReleaseBudgetMs)))
        << "prefill stayed blocked in Read() after the upstream cancel (detail: " << pair.prefill()->returnDetail()
        << ")";
...
TEST(CancellationPropagation, ExplicitDownstreamDeadlineStillAppliesWithoutAClientCancel) {
    ...
    ASSERT_TRUE(pair.prefill()->waitForReturn(std::chrono::seconds(5)))
        << "the explicit downstream deadline did not end the call (detail: " << pair.prefill()->returnDetail() << ")";
...
TEST(CancellationPropagation, NormalCompletionIsUnaffected) {
    ...
    ASSERT_TRUE(pair.prefill()->waitForReturn(std::chrono::seconds(5)));
    EXPECT_FALSE(pair.decode()->sawCancellation()) << "a normal shutdown was reported as a cancellation";
    pair.joinUpstream(client);
...
TEST(CancellationPropagation, UpstreamDeadlineEarlierThanDownstreamStillReleasesTheChild) {
    ...
    ASSERT_TRUE(pair.prefill()->waitForReturn(std::chrono::seconds(10)))
        << "an expired upstream call did not release the downstream (detail: " << pair.prefill()->returnDetail() << ")";
    const int64_t elapsed = msSince(t0);
    ...
    EXPECT_TRUE(pair.decode()->sawCancellation()) << "the downstream did not observe the cancellation that released it";
...
TEST(CancellationPropagation, DownstreamDeadlineEarlierThanUpstreamEndsTheChildFirst) {
    ...
    const auto t0 = Clock::now();
    ASSERT_TRUE(pair.prefill()->waitForReturn(std::chrono::seconds(10)))
        << "the tighter downstream deadline did not end the call (detail: " << pair.prefill()->returnDetail() << ")";
    const int64_t elapsed = msSince(t0);
...
TEST(CancellationPropagation, NullParentFallbackPathStillCompletesNormally) {
    ...
    ASSERT_TRUE(pair.prefill()->waitForReturn(std::chrono::seconds(10)))
        << "the fallback context did not complete a normal request (detail: " << pair.prefill()->returnDetail() << ")";
    EXPECT_FALSE(pair.decode()->sawCancellation())
