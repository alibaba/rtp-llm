...
bool destroyWithoutSchedulerProgress(std::unique_ptr<GenerateContext>            context,
    ...
    auto               destroyed = std::async(std::launch::async, [context = std::move(context), &started]() mutable {
        started.set_value();
        context.reset();
    });
    const bool         entered   = started_future.wait_for(std::chrono::seconds(5)) == std::future_status::ready;
    const bool         ready     = entered && destroyed.wait_for(std::chrono::seconds(5)) == std::future_status::ready;
    if (!ready) {
...
TEST(GenerateContextLifecycleTest, PrefillSuccessDoesNotWaitForScheduler) {
    RemoteServerResource resource;
    GenerateInputPB      request;
    request.set_request_id(1005);
    request.mutable_group_id()->set_value(17);
    RPCContext                   rpc_context{&request, nullptr};
    kmonitor::MetricsReporterPtr reporter;
    auto                         meta = std::make_shared<RpcServerRuntimeMeta>();
    auto context = std::make_unique<PrefillGenerateContext>(&resource, rpc_context, 0, nullptr, reporter, meta);
...
TEST(GenerateContextLifecycleTest, PrefillAsyncCancellationIsPreservedAtTeardown) {
    RemoteServerResource resource;
    GenerateInputPB      request;
    request.set_request_id(1006);
    RPCContext                   rpc_context{&request, nullptr};
    kmonitor::MetricsReporterPtr reporter;
    auto                         meta = std::make_shared<RpcServerRuntimeMeta>();
    auto context = std::make_unique<PrefillGenerateContext>(&resource, rpc_context, 0, nullptr, reporter, meta);
