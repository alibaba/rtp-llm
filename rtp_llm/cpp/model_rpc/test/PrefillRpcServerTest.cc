...
namespace rtp_llm::prefill_rpc_server_test {

void setGenerateStreamCallSetupFailpoint(void (*failpoint)());
...
void throwSetupStdException() {
    throw std::runtime_error("injected GenerateStreamCall setup failure");
}

void throwSetupUnknownException() {
    throw 42;
}

void throwSetupInvalidArgument() {
    throw std::invalid_argument("injected invalid setup argument");
}
...
TEST_F(PrefillRpcServerTest, rawGrpcPromptScoringWithPositiveMaxNewTokensRoutesLocally) {
    ...
    grpc::ServerContext context;
    prefill_rpc_server_test::setGenerateStreamCallSetupFailpoint(throwSetupStdException);
    const auto status = server.GenerateStreamCall(&context, &request, nullptr);
    prefill_rpc_server_test::setGenerateStreamCallSetupFailpoint(nullptr);
    ...
    EXPECT_EQ(error_details.error_message().find("injected GenerateStreamCall setup failure"), std::string::npos);
}
...
TEST_F(PrefillRpcServerTest, GenerateStreamCallCatchesExceptionsBeforeContextAwareTry) {
    struct SetupFailureCase {
        const char* name;
        void (*failpoint)();
        grpc::StatusCode grpc_status_code;
        ErrorCode        error_code;
        const char*      error_message;
    };
    const SetupFailureCase cases[] = {
        {"std::exception",
         throwSetupStdException,
         grpc::StatusCode::INTERNAL,
         ErrorCode::EXECUTION_EXCEPTION,
         "injected GenerateStreamCall setup failure"},
        {"unknown exception",
         throwSetupUnknownException,
         grpc::StatusCode::INTERNAL,
         ErrorCode::EXECUTION_EXCEPTION,
         "unknown exception"},
        {"invalid_argument",
         throwSetupInvalidArgument,
         grpc::StatusCode::INVALID_ARGUMENT,
         ErrorCode::INVALID_PARAMS,
         "injected invalid setup argument"},
    };
    ...
    for (const auto& test_case : cases) {
        ...
        request.add_token_ids(0);
        request.mutable_generate_config()->set_max_new_tokens(2);
        request.mutable_generate_config()->set_can_use_pd_separation(true);

        TestPrefillRpcServer server;
        grpc::ServerContext  context;
        grpc::Status         status;
        prefill_rpc_server_test::setGenerateStreamCallSetupFailpoint(test_case.failpoint);
        try {
            status = server.GenerateStreamCall(&context, &request, nullptr);
        } catch (...) {
            prefill_rpc_server_test::setGenerateStreamCallSetupFailpoint(nullptr);
            FAIL() << "GenerateStreamCall leaked an injected setup exception";
        }
        prefill_rpc_server_test::setGenerateStreamCallSetupFailpoint(nullptr);

        EXPECT_EQ(server.onflightRequestNum(), 0);
        EXPECT_EQ(status.error_code(), test_case.grpc_status_code);
        ErrorDetailsPB error_details;
        ASSERT_TRUE(error_details.ParseFromString(status.error_details()));
        EXPECT_EQ(error_details.error_code(), static_cast<int>(test_case.error_code));
        EXPECT_NE(error_details.error_message().find(test_case.error_message), std::string::npos);
    }
}
