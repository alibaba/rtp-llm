    ...
    for (int session = 0; session < sessions; ++session) {
        ...
        auto report = [&](bool diagnostics = false) {
            ...
            for (const auto& rank : ranks) {
                ...
                out.source_markers += buffers->removed_request_ids_.size();
                for (const auto& [request_id, expiry] : buffers->removed_request_ids_) {
            ...
            if (command.op == 'f') {
                ...
                for (int r = 0; r < kRanks; ++r) {
                    for (int l = 0; l < (command.op == '1' ? 1 : kLayers); ++l) {
                        check(ranks[r]->connector->writeByLayerTag(l, ranks[r]->config.topologyPtr()->groups()[0].tag,
                            source.resources[r], command.id, nullptr, source.deadline), "batch publication failed");
                    }
                }
                ...
                for (const auto& rank : ranks) {
                    for (int l = 0; l < kLayers; ++l) {
                        check(rank->connector->writeByLayerTag(l, rank->config.topologyPtr()->groups()[0].tag,
                            rank->resource, id, nullptr, deadline), "Prefill publication failed");
                    }
                }
            } else {
