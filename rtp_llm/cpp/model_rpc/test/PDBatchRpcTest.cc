...
public:
    ...
    std::pair<std::vector<bool>, std::vector<GenerateStreamPtr>> enqueueMultiple(const std::vector<std::shared_ptr<GenerateInput>>& inputs) override {
        std::vector<GenerateStreamPtr> streams;
        for (const auto& input : inputs)
            streams.push_back(makeStream(input));
        return {std::vector<bool>(streams.size(), true), batchEnqueue(streams)};
    }
...
TEST_F(PDBatchRpcTest, OneRpcAndOneEnqueuePerSidePreserveIdentityKeysAndOutputOrder) {
    ...
    for (auto& item : *batch.mutable_inputs()) {
        item.set_group_size(2);
        item.mutable_group_id()->set_value(77);
    }
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        EXPECT_EQ(stream->generateInput()->group_size, 2);
        EXPECT_EQ(stream->generateInput()->group_id, 77);
        EXPECT_EQ(sent.group_size(), 2);
        EXPECT_EQ(sent.group_id().value(), 77);
