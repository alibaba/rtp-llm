...
void EmbeddingRpcServiceImpl::PostProcess(std::shared_ptr<EmbeddingOutput> embedding_output) {
    ...
    }
    {
        ...
        py::object  engine_outputs = EngineOutputs(py::arg("outputs") = py_batch_output, py::arg("input_length") = 0);
...
void EmbeddingRpcServiceImpl::ProcessOutputs(py::object py_outputs, std::shared_ptr<EmbeddingOutput> embedding_output) {
    if (py_outputs.is_none()) {
        embedding_output->setError(ErrorCode::NONE_ERROR, "embedding python post process outputs is None");
    } else if (py::isinstance<torch::Tensor>(py_outputs)) {
        torch::Tensor tensor = py::cast<torch::Tensor>(py_outputs);
        embedding_output->setTensorOutput(tensor);

    } else if (py::isinstance<py::list>(py_outputs)) {
        ...
        embedding_output->setError(ErrorCode::INVALID_PARAMS, "embedding python post process outputs type unsupported");
    }
}
...
std::shared_ptr<EmbeddingOutput> EmbeddingRpcServiceImpl::ConvertEngineOutputs(py::object result) {
    ...
    try {
        ...
        ProcessOutputs(py_outputs, embedding_output);
    } catch (const py::error_already_set& e) {
        embedding_output->setError(ErrorCode::UNKNOWN_ERROR, e.what());
        PyErr_Clear();
    }
