...
void logSleepMemorySnapshotForRank(const std::string& phase, const ParallelismConfig& parallelism, int64_t epoch) {
    ...
    try {
        OptionalSleepDeviceGuard device_guard(parallelism.local_rank);
...
void LocalRpcServer::logSleepMemorySnapshot(const std::string& phase, int64_t epoch) const {
    logSleepMemorySnapshotForRank(phase, maga_init_params_.parallelism_config, epoch);
}
...
void LocalRpcServer::installSleepHooks() {
    ...
    hooks.releaseRestorableGpuMemory = [this, vmm_backend, local_rank](const SleepOptions& opt) {
        ...
        if (!vmm_backend->isAvailable()) {
            ...
            trimSleepAllocator(local_rank, epoch, world_rank, graph_enabled, "allocator_trim");
            return true;
        }
        ...
        trimSleepAllocator(local_rank, epoch, world_rank, graph_enabled, "allocator_trim_pre");
