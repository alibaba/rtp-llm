...
grpc::Status LocalRpcServer::init(const EngineInitParams&                       maga_init_params,
    ...
    if (propose_params) {
        ...
        if (!mm_process_engine.is_none()) {
            ...
            if (vit_separation == 2) {
                mm_processor_.reset(
                    new RemoteMultimodalProcessor(mm_process_engine, maga_init_params.mm_model_config_, maga_init_params.model_config_.max_seq_len_));
            } else if (vit_separation == 0) {
                mm_processor_.reset(
                    new LocalMultimodalProcessor(mm_process_engine, maga_init_params.mm_model_config_, maga_init_params.model_config_.max_seq_len_));
            } else {
...
WorkerStatusInfo LocalRpcServer::getWorkerStatusInfo(int64_t latest_finished_version) {
    ...
    auto quant_method          = maga_init_params_.model_config_.quant_algo_.getQuantMethod();
