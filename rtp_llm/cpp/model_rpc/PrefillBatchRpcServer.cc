...
DeferredPrefillContext::~DeferredPrefillContext() {
    // Rejected, expired and priority-preempted requests can terminate without
    // ever attaching FetchResponse. Their owner has handled the terminal path.
...
grpc::Status PrefillBatchRpcServer::FetchResponse(grpc::ServerContext*                   context,
    ...
    // Fetch owns these borrowed pointers only until this handler returns.
    // Detach and latch cancellation BEFORE finishOperation can publish the
    // context to an asynchronous priority finalizer on another thread.
