...
public:
    ...
    bool markPriorityPreemptionCanceled(int64_t request_id, int64_t error_code, const std::string& error_message) {
        ...
        if (running != running_streams_.end()) {
            ...
            if (stream) {
                // Snapshot the coherent TimeInfo BEFORE sampling the wall clock:
                // begin_time_us and wait_time_us must come from the same locked
                // TimeInfo, and `current` must be read afterwards so a concurrent
                // resetBeginTime can never advance begin past it and drive
                // execution_time_ms negative across the epoch boundary.
                ...
                task_info.waiting_time_ms = time_info.wait_time_us / 1000;
                task_info.iterate_count   = stream->iterCount();
                task_info.execution_time_ms =
                    computeExecutionTimeMs(current, time_info.begin_time_us, task_info.waiting_time_ms);
            }
    ...
    void dequeue(int64_t request_id, const GenerateStreamPtr& stream) {
        ...
        // Snapshot TimeInfo before the wall clock (see markPriorityPreemptionCanceled):
        // `current` must be >= the sampled begin epoch or execution_time_ms goes
        // negative when resetBeginTime races this teardown.
        const auto time_info      = stream->getTimeInfo();
