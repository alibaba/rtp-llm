...
ErrorInfo DecodeRpcServer::loadCache(const LoadKVCacheContext& load_context) {
    ...
    // Receive-side fan-in for a head-sharded (segmented) linear cache.  Reachable when
    // non-CP K3 runs Prefill TP > Decode TP (the sender adds the P/D contiguous peers
    // whose KDA head shards this rank owns), or when CP prefill has a peer count other
    // than prefill_cp_size.  Equal P/D TP adds one peer, making the split below a no-op.
