...
ErrorInfo DecodeRpcServer::validateMTPTransfer(const CacheConfig&         main_config,
    ...
    const bool main_whole_block = remoteWholeBlock(main_config);
    for (const size_t module_index : active_modules) {
        ...
        if (main_whole_block == remoteWholeBlock(sub_config)) {
            continue;
        }
        return ErrorInfo(ErrorCode::LOAD_KV_CACHE_FAILED,
...
ErrorInfo DecodeRpcServer::loadCacheAsyncForTp(DecodeGenerateContext& decode_context,
    ...
    active_modules.reserve(mtp_load_plan.size());
    for (const auto& module : mtp_load_plan) {
        active_modules.push_back(module.module_index);
    }
    const auto mtp_transfer_error = validateMTPTransfer(load_cache_config, active_modules);
                    ...
                    for (const auto& group_ref : mtp_layer_groups.at(mtp_model_id)[layer_id]) {
                        ...
                        for (size_t block_pos : block_pos_list) {
                            ...
                            const bool mtp_use_kv_key_prefix =
                                mtp_use_mla || mtp_use_opaque_kv_store || mtp_use_hybrid_attention;
...
grpc::Status DecodeRpcServer::RemoteLoad(grpc::ServerContext*          server_context,
    ...
    BlockIdsByTag             group_block_ids = decodeBlockIdsByTag(*request, cache_config.topology());
