...
BroadcastLoadRequestPB DecodeRpcServer::constructRemoteLoadRequestForMla(
    ...
    if (!load_context.block_ids_by_group.empty()) {
        RTP_LLM_CHECK_WITH_INFO(load_context.block_ids_by_group.size() == load_context.group_tags.size(),
                                "cache group block size=%zu differs from tag size=%zu",
                                load_context.block_ids_by_group.size(),
                                load_context.group_tags.size());
        for (size_t group_index = 0; group_index < load_context.block_ids_by_group.size(); ++group_index) {
            const auto& group_block = load_context.block_ids_by_group[group_index];
            ...
            tagged_row->set_tag(load_context.group_tags[group_index]);
...
ErrorInfo DecodeRpcServer::loadCacheForAllRank(DecodeGenerateContext& decode_context) {
    ...
    const auto& group_tags         = cache_resource.groupTags();
...
ErrorInfo DecodeRpcServer::loadCache(const LoadKVCacheContext& load_context) {
    ...
    auto layerGroupTags = [](const CacheConfig& cfg, size_t layer_id) {
        const auto& layer = cfg.topology().layer(static_cast<int>(layer_id));
        RTP_LLM_CHECK_WITH_INFO(!layer.group_tags.empty(), "cache layer %zu has empty group_tags", layer_id);
        return layer.group_tags;
    };
    auto groupType = [](const CacheConfig& cfg, const std::string& tag) { return cfg.group(tag).policy.group_type; };
    auto cpMapperForGroup = [&](const CacheConfig& cfg, const std::string& tag) {
        return CPSlotMapper(load_context.prefill_cp_size - 1,
                            ...
                            static_cast<int>(cfg.group(tag).seq_size_per_block));
    };
    ...
    for (int i = 0; i < load_context.peer_addrs.size(); i++) {
        ...
        for (size_t layer_id = 0; layer_id < layer_num; layer_id++) {
            ...
            // multiple groups. Iterate every tag the layer owns.
            const auto layer_group_tags = layerGroupTags(cache_config, layer_id);

            for (const auto& tag : layer_group_tags) {
            ...
            if (propose_maga_init_params_ && propose_maga_init_params_->mtp_model_params_
                ...
                for (const auto& module_plan : mtp_load_plan) {
                    ...
                    for (size_t layer_id = 0; layer_id < layer_num; layer_id++) {
                        ...
                        const auto mtp_layer_group_tags = layerGroupTags(mtp_cache_cfg, layer_id);
                        ...
                        for (const auto& tag : mtp_layer_group_tags) {
                            auto request_key      = makeTaggedRequestKey(load_context.request_id, layer_id, tag);
                            ...
                            CacheGroupType group_type = groupType(mtp_cache_cfg, tag);
                            auto           block_pos_list =
...
grpc::Status DecodeRpcServer::RemoteLoad(grpc::ServerContext*          server_context,
    ...
    const auto&               group_tags         = topology.groupTagsSnapshot();
...
GroupBlockIds DecodeRpcServer::decodeGroupBlockIds(const BroadcastLoadRequestPB& request,
    ...
    GroupBlockIds block_ids_by_group(topology.groups().size(), nullptr);
    for (const auto& tagged_row : request.tagged_group_block_ids()) {
        const size_t group_index = topology.groupIndex(tagged_row.tag());
        RTP_LLM_CHECK_WITH_INFO(
            block_ids_by_group[group_index] == nullptr, "duplicate RPC cache tag=%s", tagged_row.tag().c_str());
        auto holder = std::make_shared<BlockIds>();
        holder->assign(BlockIndicesType(tagged_row.block_ids().begin(), tagged_row.block_ids().end()));
        block_ids_by_group[group_index] = std::move(holder);
    }
    RTP_LLM_CHECK_WITH_INFO(request.tagged_group_block_ids_size() == static_cast<int>(topology.groups().size()),
                            "RPC cache tag set size=%d does not match local topology size=%zu",
                            request.tagged_group_block_ids_size(),
                            topology.groups().size());
    for (size_t group_index = 0; group_index < block_ids_by_group.size(); ++group_index) {
        RTP_LLM_CHECK_WITH_INFO(block_ids_by_group[group_index] != nullptr,
                                "RPC cache tag set is missing local tag=%s",
                                topology.groups()[group_index].tag.c_str());
    }
