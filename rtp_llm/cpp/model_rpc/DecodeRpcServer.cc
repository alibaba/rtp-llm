...
ErrorInfo DecodeRpcServer::loadCache(const LoadKVCacheContext& load_context) {
    ...
    const auto& spec    = cache_config.cache_specs.empty() ? nullptr : cache_config.cache_specs[0];
    RTP_LLM_CHECK_WITH_INFO(use_mla || spec != nullptr, "cache_specs is empty for non-MLA cache");
    const size_t k_total_bytes = (use_mla || !spec) ? 0 : spec->k_block_size_bytes();
    const size_t v_total_bytes = (use_mla || !spec) ? 0 : spec->v_block_size_bytes();
    ...
    for (int i = 0; i < load_context.peer_addrs.size(); i++) {
        ...
        for (size_t layer_id = 0; layer_id < layer_num; layer_id++) {
            ...
            for (size_t block_pos = load_context.reuse_block_size; block_pos < block_num; block_pos++) {
                ...
                RTP_LLM_CHECK_WITH_INFO(addr_info.kv_addr != nullptr,
                                        "convertIndexToAddr returned null kv_addr (layer=%zu block=%d)",
                                        layer_id,
                                        block_id);
                ...
                for (size_t mtp_model_id = 0; mtp_model_id < propose_maga_init_params_->mtp_model_params_->size();
                    ...
                    for (size_t layer_id = 0; layer_id < layer_num; layer_id++) {
                        ...
                        for (size_t block_pos = load_context.reuse_block_size; block_pos < block_num; block_pos++) {
                            ...
                            RTP_LLM_CHECK_WITH_INFO(addr_info.kv_addr != nullptr,
                                                    "convertIndexToAddr returned null kv_addr (mtp layer=%zu block=%d)",
                                                    layer_id,
                                                    block_id);

                            const auto& mtp_spec =
                                mtp_cache_cfg.cache_specs.empty() ? nullptr : mtp_cache_cfg.cache_specs[0];
                            const bool mtp_use_mla          = mtp_cache_cfg.use_mla;
                            const bool mtp_need_kv_separate = !mtp_use_mla;
                            RTP_LLM_CHECK_WITH_INFO(mtp_use_mla || mtp_spec != nullptr,
                                                    "mtp cache_specs is empty for non-MLA cache");

                            if (!mtp_need_kv_separate) {
                                std::shared_ptr<void> kv_block_addr(addr_info.kv_addr, [](void* p) {});
                                load_layer_cache->addBlock(
