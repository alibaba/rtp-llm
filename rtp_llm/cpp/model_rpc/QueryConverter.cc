...
void QueryConverter::mergeAndPadBuffersToTensorPB(TensorPB*                                   target_pb,
    ...
    std::vector<size_t> final_shape = {batch_size, max_len};
...
void QueryConverter::transResponse(GenerateOutputsPB*     outputs,
    ...
    stackBuffersToTensorPB(
        merged_output->mutable_loss(), source_outputs, [](const auto& r) { return r.hidden_states; });

    stackBuffersToTensorPB(
        merged_output->mutable_logits(), source_outputs, [](const auto& r) { return r.hidden_states; });

    stackBuffersToTensorPB(
        merged_output->mutable_all_hidden_states(), source_outputs, [](const auto& r) { return r.hidden_states; });
