...
void RtpLLMStreamMetrics::report(const kmonitor::MetricsTags* tags, RtpLLMStreamMetricsCollector* collector) {
    ...
    if (!collector->cache_probe_target) {
    ...
    } else if (collector->cache_probe) {
        reportCacheScheduleProbe(tags, *collector->cache_probe);
    }
...
void RtpLLMStreamMetrics::reportCacheScheduleProbe(const kmonitor::MetricsTags* parent_tags,
                                                   const CacheScheduleSnapshot& snapshot) {
    kmonitor::MetricsTags probe_tags = parent_tags ? *parent_tags : kmonitor::MetricsTags();
    probe_tags.AddTag("cache_probe", "v1");
    probe_tags.AddTag("cache_dependency", cacheDependencyName(snapshot.dependency));
    probe_tags.AddTag("loading_entered", snapshot.loading_entered ? "true" : "false");
    probe_tags.AddTag("sample_status", cacheSampleStatusName(snapshot.sample_status));
    const auto* tags = &probe_tags;
    REPORT_MUTABLE_METRIC(cache_probe_qps_metric, 1);
    if (snapshot.numeric_valid && snapshot.sample_status == CacheSampleStatus::SINGLE_PASS) {
        // All six values share the probe denominator, including legitimate zeros.
