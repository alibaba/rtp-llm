...
LinearSoftmaxHandlerImpl::LinearSoftmaxHandlerImpl(const GptInitParameter& params):
    IHandlerImpl(params), is_initalized_(false) {
...
void getStateIndexes(int32_t* select_indexes, const int32_t* sequence_length, const int32_t batch_size) {
...
th::Tensor LinearSoftmaxHandlerImpl::forward(th::Tensor hidden_states, th::Tensor input_lengths) {
    ...
    auto indexes_cpu = device_->allocateBuffer({DataType::TYPE_INT32, {batch_size}, AllocationType::HOST});
