...
absl::Status EmbeddingEngine::step() {
    ...
    // If any stream requested gen_timeline and no session is active, configure
    // before tick() so the profiler is up before this step's work (this
    // branch's StepWindowProfiler has tick(), not main's stepScope()).
