...
EmbeddingScheduler::EmbeddingScheduler(const ModelConfig& model_config,
    ...
    model_config_(model_config), concurrency_config_(concurrency_config), metrics_reporter_(metrics_reporter) {}
...
absl::StatusOr<list<EmbeddingStreamPtr>> EmbeddingScheduler::scheduleNew() {
    ...
    while (it != waiting_streams_.end()) {
        ...
        if (total_len + stream->inputLength() > concurrency_config_.max_context_batch_size * model_config_.max_seq_len_) {
            break;
        }
        stream->setStart();
