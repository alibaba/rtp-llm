...
absl::StatusOr<GptModelInputs> EmbeddingExecutor::gatherModelInput(const std::list<EmbeddingStreamPtr>& streams) const {
    ...
    if (HandlerArgs::has_arg(handler_args_, HandlerArgs::Arg::ALL_MOE_GATING)) {
...
absl::StatusOr<py::object> EmbeddingExecutor::postProcess(const ModelRequest&    model_request,
    ...
    try {
        ...
        if (has_arg(handler_args_, Arg::ALL_HIDDEN_STATES)) {
            kwargs[get_name(Arg::ALL_HIDDEN_STATES)] = Buffer2torchTensor(gpu_outputs.all_hidden_states, false);
        }
        ...
        if (has_arg(handler_args_, Arg::ALL_MOE_GATING)) {
            py::list moe_gating;
            ...
            kwargs[get_name(Arg::ALL_MOE_GATING)] = moe_gating;
        }
