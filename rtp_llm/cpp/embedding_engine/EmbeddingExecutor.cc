...
absl::StatusOr<GptModelInputs> EmbeddingExecutor::gatherModelInput(const std::list<EmbeddingStreamPtr>& streams) const {
    ...
    if (model_config_.bert_uqi_config.enabled) {
        model_input.bert_uqi = buildBertUqiInputs(model_input.combo_tokens,
                                                     model_input.input_lengths,
                                                     model_config_.bert_uqi_config.segment_token_id,
                                                     model_config_.bert_uqi_config.separator_token_id);
    }
...
ModelRequest EmbeddingExecutor::generateOldModelRequest(GptModelInputs& model_input) {
    ...
    if (HandlerArgs::has_arg(handler_args_, HandlerArgs::Arg::UQI_POOLING_POSITIONS)) {
        // UQI heads consume known CLS rows, not a downloaded copy of every token.
        RTP_LLM_CHECK_WITH_INFO(model_config_.bert_uqi_config.enabled,
                                "uqi_pooling_positions requires BERT UQI attention");
        model_request.uqi_pooling_positions =
            model_input.bert_uqi.pooling_positions
                .to(model_input.combo_tokens.device(), torch::kInt64, /*non_blocking=*/true);
        // Preserve explicitly requested legacy arguments for a mixed consumer.
        if (HandlerArgs::has_arg(handler_args_, HandlerArgs::Arg::INPUT_IDS)) {
            model_request.combo_tokens = to_cpu(model_input.combo_tokens);
        }
        if (HandlerArgs::has_arg(handler_args_, HandlerArgs::Arg::INPUT_LENGTHS)) {
            model_request.input_lengths = to_cpu(model_input.input_lengths);
        }
    } else {
        // Keep the existing metadata and device contract for every other handler.
        model_request.combo_tokens         = to_cpu(model_input.combo_tokens);
...
absl::StatusOr<py::object> EmbeddingExecutor::postProcess(const ModelRequest&    model_request,
    ...
    try {
        ...
        if (has_arg(handler_args_, Arg::UQI_POOLING_POSITIONS)) {
            kwargs[get_name(Arg::UQI_POOLING_POSITIONS)] = model_request.uqi_pooling_positions;
        }
