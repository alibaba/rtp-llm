...
// A [W, K], B [N, K]
void GemmParams::check_mk_nk_style() const {
    ...
    if (dim > 2) {
        throw std::runtime_error("Batch Gemm op is not supported for check_mk_nk_style()");
    }

    auto m_a = (transA == TransposeOperation::NONE) ? A.shape()[dim - 2] : A.shape()[dim - 1];
    auto k_a = (transA == TransposeOperation::NONE) ? A.shape()[dim - 1] : A.shape()[dim - 2];

    auto n_b = (transB == TransposeOperation::NONE) ? B.shape()[dim - 2] : B.shape()[dim - 1];
    auto k_b = (transB == TransposeOperation::NONE) ? B.shape()[dim - 1] : B.shape()[dim - 2];
