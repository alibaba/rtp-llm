...
public:
    HipGraphRunner(const DeviceInitParams& params,
        ...
        options_hip_int32_    = torch::TensorOptions().dtype(torch::kInt32).device(torch::kCUDA).requires_grad(false);
        options_cpu_int32_    = torch::TensorOptions().dtype(torch::kInt32).device(torch::kCPU).requires_grad(false);
        options_hip_float_    = torch::TensorOptions().dtype(model_data_type).device(torch::kCUDA).requires_grad(false);
        RTP_LLM_LOG_INFO("Initialize HipGraphRunner with parameters below: \n \
            enable_hip_graph_: %d, max_bs_: %d, enable_hip_graph_debug_mode_: %d, max_seq_len_: %d, seq_size_per_block_: %d, \
            hidden_size_: %d, num_tokens_per_bs_: %d, is_prefill_hip_graph_mode_: %d",
                         enable_hip_graph_,
                         max_bs_,
...
private:
    ...
    int                max_prefill_hip_graph_len_{160};
    int                max_seq_len_{0};
    int                seq_size_per_block_{0};
    int                hidden_size_{0};
    HipGraphState      state_;
    ...
    at::TensorOptions                      options_hip_int32_;
