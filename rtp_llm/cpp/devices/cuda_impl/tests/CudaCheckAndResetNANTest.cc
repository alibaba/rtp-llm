...
protected:
    ...
    testKVCachePrefillCheckAndResetNAN(size_t                      batch_size,
                                       ...
                                       const std::vector<std::tuple<size_t, size_t, int32_t, bool>>&
                                           positions) {  // (batch_id, layer_id, token_idx, is_inf)
        ...
        T* d_kv_cache;
        cudaMalloc(&d_kv_cache, kv_cache_buffer_size);
        ...
        cudaMalloc(&d_layer_base_addr, layer_num * sizeof(void*));
        ...
        cudaMalloc(&d_kv_cache_block_id, batch_size * max_blocks_per_batch * sizeof(int32_t));
        ...
        for (const auto& pos : positions) {
            ...
            size_t token_offset_bytes = offset_in_block * token_bytes;
            size_t total_offset       = layer_offset_bytes + block_offset_bytes + token_offset_bytes;

            if (total_offset >= kv_cache_buffer_size)
                continue;

            token_offsets.push_back(total_offset);
            is_nan_flags.push_back(!is_inf);  // false for NaN, true for Inf -> !is_inf for is_nan
        }
        ...
        if (err != cudaSuccess) {
            RTP_LLM_LOG_ERROR("CUDA error after kernel launch: %s", cudaGetErrorString(err));
        }
        ...
        for (const auto& pos : positions) {
            ...
            size_t block_offset_bytes = physical_block_id * block_size_bytes;
            size_t token_offset_bytes = offset_in_block * token_bytes;
            size_t total_offset_bytes = layer_offset_bytes + block_offset_bytes + token_offset_bytes;

            verify_positions.push_back({batch_id, layer_id, token_idx, total_offset_bytes, in_checked_region});
        }
    ...
    testKVCacheDecodeCheckAndResetNAN(size_t                      batch_size,
        ...
        cudaMalloc(&d_sequence_lengths, batch_size * sizeof(int32_t));
        ...
        for (const auto& pos : positions) {
            ...
            verify_positions.push_back({batch_id, layer_id, token_idx, total_offset_bytes, is_last_token});
        }
...
TEST_F(CudaCheckAndResetNANTest, TestFloat_KVCachePrefill_Basic) {
    ...
    std::vector<std::tuple<size_t, size_t, int32_t, bool>> positions = {
        {0, 0, 100, false},  // batch 0, layer 0, token 100, NaN
        {1, 0, 50, false},   // batch 1, layer 0, token 50 (before prefix_length, should not be reset), NaN
        {1, 6, 100, false}   // batch 1, layer 6, token 100, NaN
    };
...
TEST_F(CudaCheckAndResetNANTest, TestFloat_KVCachePrefill_MultiLayer) {
    ...
    std::vector<std::tuple<size_t, size_t, int32_t, bool>> positions = {
        {0, 0, 6, false},               // layer 0, token 6, NaN
        {0, 1, 8, false},               // layer 1, token 8, NaN
        {0, 2, 10, false},              // layer 2, token 10, NaN
        {0, 6, 1024 * 1023, false},     // layer 6, token 1024*1023, NaN
        {0, 6, 1024 * 1024 - 1, true},  // layer 6, token 1024*1023, NaN
    };
...
TEST_F(CudaCheckAndResetNANTest, TestFloat_KVCachePrefill_NoNewTokens) {
    ...
    std::vector<std::tuple<size_t, size_t, int32_t, bool>> positions = {
        {0, 0, 15, false},  // token 15, in range but prefix_length == seq_len, NaN
    };
...
TEST_F(CudaCheckAndResetNANTest, TestFloat_KVCachePrefill_CrossBlockBoundary) {
    ...
    std::vector<std::tuple<size_t, size_t, int32_t, bool>> positions = {
        {0, 0, 7, false},   // block 0, last token, NaN
        {0, 0, 8, false},   // block 1, first token, NaN
        {0, 0, 15, false},  // block 1, last token, NaN
        {0, 0, 16, false},  // block 2, first token, NaN
    };
...
TEST_F(CudaCheckAndResetNANTest, TestFloat_KVCachePrefill_NullBlockId) {
    ...
    std::vector<std::tuple<size_t, size_t, int32_t, bool>> positions = {
        {0, 0, 8, false},   // token 8 is in block 1 (null), should be skipped, NaN
        {0, 0, 15, false},  // token 15 is in block 2, should be checked, NaN
        {1, 0, 8, false}    // token 8 is in block 4, should be checked, NaN
    };
...
TEST_F(CudaCheckAndResetNANTest, TestFloat_KVCacheDecode_Basic) {
    ...
    std::vector<std::tuple<size_t, size_t, int32_t, bool>> positions = {
        {0, 0, 24, false},  // batch 0, last token, should be reset, NaN
        {0, 0, 20, false},  // batch 0, non-last token, should remain, NaN
        {1, 0, 34, false},  // batch 1, last token, should be reset, NaN
    };
...
TEST_F(CudaCheckAndResetNANTest, TestFloat_KVCacheDecode_MultiLayer) {
    ...
    std::vector<std::tuple<size_t, size_t, int32_t, bool>> positions = {
        {0, 0, 14, false},  // layer 0, last token, NaN
        {0, 1, 14, false},  // layer 1, last token, NaN
        {0, 2, 14, false},  // layer 2, last token, NaN
    };
...
TEST_F(CudaCheckAndResetNANTest, TestFloat_KVCacheDecode_SeqLenZero) {
    ...
    std::vector<std::tuple<size_t, size_t, int32_t, bool>> positions = {
        {0, 0, 0, false},  // Should be skipped, NaN
    };
...
TEST_F(CudaCheckAndResetNANTest, TestFloat_KVCacheDecode_SeqLenOne) {
    ...
    std::vector<std::tuple<size_t, size_t, int32_t, bool>> positions = {
        {0, 0, 0, false},  // Should be reset, NaN
    };
...
TEST_F(CudaCheckAndResetNANTest, TestFloat_KVCacheDecode_LastTokenInNewBlock) {
    ...
    std::vector<std::tuple<size_t, size_t, int32_t, bool>> positions = {
        {0, 0, 15, false},  // Should be reset, NaN
    };
...
TEST_F(CudaCheckAndResetNANTest, TestFloat_KVCacheDecode_NullBlockId) {
    ...
    std::vector<std::tuple<size_t, size_t, int32_t, bool>> positions = {
        {0, 0, 16, false},  // Should be reset, NaN
    };
...
TEST_F(CudaCheckAndResetNANTest, TestHalf_KVCacheDecode_Basic) {
    ...
    std::vector<std::tuple<size_t, size_t, int32_t, bool>> positions = {
        {0, 0, 13, true}, {0, 0, 19, false},  // Last token, NaN
    };
...
TEST_F(CudaCheckAndResetNANTest, TestBFloat16_KVCacheDecode_Basic) {
    ...
    std::vector<std::tuple<size_t, size_t, int32_t, bool>> positions(
        1024, std::make_tuple(static_cast<size_t>(0), static_cast<size_t>(0), static_cast<int32_t>(19), false));
