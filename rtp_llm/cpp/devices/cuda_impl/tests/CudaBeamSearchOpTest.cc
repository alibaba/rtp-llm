...
TEST_F(CudaBeamSearchOpTest, simpleTest) {
    ...
    std::vector<int> beam_widths = {1, 2, 4, 5, 8, 16, 32, 64, 70, 128, 256, 500, 1024};
...
TEST_F(CudaBeamSearchOpTest, variableBeamWidthTest) {
    std::vector<int> batch_sizes = {1, 2, 4, 8, 16, 32};
    std::vector<int> beam_widths = {1, 2, 5, 8, 32, 70, 256, 500, 1024};
    std::vector<int> vocab_sizes = {3000};
    std::vector<int> max_seq_len = {10, 100, 1000};
    ...
    for (auto batch_size : batch_sizes) {
        for (auto beam_width_in: beam_widths) {
            for (auto beam_width_out: beam_widths) {
                if (beam_width_in == beam_width_out) continue;

                for (auto vocab_size : vocab_sizes) {
                    for (auto seq_len : max_seq_len) {
                        std::cout << "batch_size: " << batch_size << ", beam_width_in: " << beam_width_in << ", beam_width_out: " << beam_width_out << ", vocab_size: " << vocab_size << ", seq_len: " << seq_len << std::endl;
