...
void CudaDevice::commBarrier(const NcclParam& nccl_param) {
    ...
    constexpr size_t element_len  = 32;
    constexpr size_t element_size = sizeof(float);
    check_cuda_value(cudaMalloc(&tmpBuffer, element_len * element_size));
    check_cuda_value(cudaMemset(tmpBuffer, 0, element_len * element_size));
    ftNcclAllReduceSum((float*)tmpBuffer, (float*)tmpBuffer, element_len, nccl_param, stream_);
