...
NcclParam CudaDevice::getNcclParam(ParallelMode mode) {
    switch (mode) {
        ...
        case ParallelMode::AFD:
            return afd_nccl_param_;
        default:
...
void CudaDevice::batchSendRecv(const BatchSendRecvParams& params, const ParallelMode& mode) {
    RTP_LLM_CHECK_WITH_INFO(mode == ParallelMode::DP_AND_TP || mode == ParallelMode::AFD,
                            "batch send recv just support ParallelMode::DP_AND_TP or ParallelMode::AFD but got [%d]",
                            int(mode));
    RTP_LLM_CHECK_WITH_INFO(params.p2p_params.size() > 0, "send_params is empty");
    NcclParam used_nccl_param = mode == ParallelMode::DP_AND_TP ? dp_tp_nccl_param_ : afd_nccl_param_;
    NCCLCHECK(ncclGroupStart());
    for (const auto& params : params.p2p_params) {
        RTP_LLM_CHECK_WITH_INFO(params.dest_rank >= 0 && params.dest_rank < used_nccl_param.world_size_,
                                "dest_rank [%d] must be in range [0, %d)",
                                params.dest_rank,
                                used_nccl_param.world_size_);
        if (params.type == SendRecvType::kSend) {
            NCCLCHECK(ncclSend(params.buffer->data(),
                               params.buffer->size(),
                               getNcclDataType(params.buffer->type()),
                               params.dest_rank,
                               used_nccl_param.nccl_comm_,
                               stream_));
