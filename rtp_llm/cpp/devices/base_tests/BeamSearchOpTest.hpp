...
public:
    ...
    prepareInput(int batch_size, int beam_width_in, int beam_width_out, int vocab_size, int max_seq_len) {
        ...
        if (rand_sequence_length == rand_input_length) {
            logits = torch::rand({batch_size, 1, vocab_size}, float_options).repeat({1, beam_width_in, 1});
            token_ids =
            ...
            logits        = torch::rand({batch_size, beam_width_in, vocab_size}, float_options);
            token_ids     = torch::randint(0, vocab_size, {batch_size, beam_width_in, max_seq_len}, int_options);
            cum_log_probs = torch::rand({batch_size, beam_width_in}, float_options);
        }
    ...
    TestBeamSearchOutput opRun(TestBeamSearchInput& input) {
        ...
        auto beam_index_t = torch::zeros_like(input.input_lengths, int_options);
        auto beam_index   = tensorToBuffer(beam_index_t);

        device_->sampleBeamSearch(
            {*logits, *token_ids, *input_lengths, *sequence_lengths, *cum_log_probs, *beam_index});

        return TestBeamSearchOutput({bufferToTensor(*token_ids),
                                     bufferToTensor(*input_lengths),
                                     bufferToTensor(*sequence_lengths),
                                     bufferToTensor(*cum_log_probs),
                                     bufferToTensor(*beam_index)});
    };
