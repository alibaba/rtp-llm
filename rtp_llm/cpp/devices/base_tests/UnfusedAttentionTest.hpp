...
void UnfusedAttentionTest::prefillAddFusedQKVBiasTransposeTest(size_t batch_size,
    ...
    auto query_states_host = torch::rand(
        {(int)batch_size, (int)seq_len, (int)num_heads, (int)head_dim}, bf16_tensor_options);

    auto key_states_host = torch::rand(
        {(int)batch_size, (int)seq_len, (int)num_key_value_heads, (int)head_dim}, bf16_tensor_options);
    ...
    auto kvcache_pad = torch::zeros(
        {1, (int)batch_size, 2, (int)padding_seq_len, (int)num_key_value_heads * (int)head_dim},
        bf16_tensor_options);
    ...
    rtp_llm::CacheConfig cache_conf(
        rtp_llm::KVCacheParam({1, (uint)block_num, (uint)num_key_value_heads, (uint)head_dim, (uint)tokens_per_block, DataType::TYPE_BF16})
    );
    ...
    auto k_output = device->allocateBuffer({params.input.type(),
                                           {batch_size, num_key_value_heads, seq_len, head_dim},
                                           AllocationType::DEVICE},
                                           {"k_output"});

    auto v_output = device->allocateBuffer({params.input.type(),
                                           {batch_size, num_key_value_heads, seq_len, head_dim},
                                           AllocationType::DEVICE},
                                           {"v_output"});
    ...
    auto qkv_buf_fp8 = device->allocateBuffer({DataType::TYPE_FP8_E4M3,
    ...
    if (is_perf) {
        ...
        std::cout << "prefillAddFusedQKVBiasTransposeTest perf time: " << time << ", batch_size: " << batch_size << ", seq_q: "
                  << seq_len << ", head_q: " << num_heads << ", head_kv: " << num_key_value_heads << ", head_dim: " << head_dim
...
void UnfusedAttentionTest::decodeAddFusedQKVBiasTransposeTest(size_t batch_size,
    ...
    auto cos_sin_cache = device->allocateBuffer({DataType::TYPE_FP32,
                                                {(uint)(rope_dim * max_position_embeddings)},
                                                AllocationType::DEVICE},
                                                {"cos_sin_cache"});
    ...
    if (is_perf) {
        ...
        float time = total_time / iters;
        std::cout << "decodeAddFusedQKVBiasTransposeTest perf time: " << time << ", batch_size: " << batch_size << ", seq_q: "
                  << seq_len << ", kv_seq_len: " << kv_seq_len << ", head_q: " << num_heads << ", head_kv: " << num_key_value_heads
