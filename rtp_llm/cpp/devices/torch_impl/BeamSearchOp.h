...
struct BeamSearchOpOutput {
    ...
    void zeros(int batch_size, int beam_num, int max_seq_len) {
        ...
        beam_index =
            torch::zeros({batch_size, beam_num}, torch::TensorOptions(torch::kInt).device(torch::Device(torch::kCPU)));
    };
...
struct BeamSearchOpImpl: torch::nn::Module {
    ...
    BeamSearchOpOutput oneBatchForward(BeamSearchOpInput input) {
        ...
        auto new_token_ids =
            torch::zeros({beam_width}, torch::TensorOptions(torch::kInt).device(torch::Device(torch::kCPU)));
        auto beam_index =
            torch::zeros({beam_width}, torch::TensorOptions(torch::kInt).device(torch::Device(torch::kCPU)));
        // first beam search
