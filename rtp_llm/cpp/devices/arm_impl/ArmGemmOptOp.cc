...
BufferPtr ArmCpuDevice::gemm_opt(const GemmParams& params) {
...
#ifdef GEMM_DEBUG
    timer_recorder_.record(std::string("gemm_prepare_data, ") + "m=" + std::to_string(m) + ", n=" + std::to_string(n)
                               + ", k=" + std::to_string(k),
                           timer.elapsed_nano());
    timer.reset();
#endif
    ...
    BufferPtr workspace =
        allocateBuffer({DataType::TYPE_BF16, workspace_shape, AllocationType::DEVICE}, {"gemm_workspace"});
    ...
    if (params.B.type() == DataType::TYPE_FP32 || params.B.type() == DataType::TYPE_FP16 ||
        ...
        params.B.type() == DataType::TYPE_BF16 || params.B.type() == DataType::TYPE_QINT4X2) {
...
#ifdef GEMM_DEBUG
    timer_recorder_.record(std::string("gemm_prepare_workspace(packing)") + ", m=" + std::to_string(m)
    ...
    for (size_t batch = 0; batch < batch_size; ++batch) {
        ...
        hie::bfloat16* B_bf16_ptr =
            reinterpret_cast<hie::bfloat16*>(weight_workspace_ptr->dataWithOffset(batch * height * width));
        ...
        if (params.A.type() == DataType::TYPE_FP32) {
...
#ifdef GEMM_DEBUG
            timer_recorder_.record(std::string("gemm_prepare_batch_data") + ", m=" + std::to_string(m)
                                       + ", n=" + std::to_string(n) + ", k=" + std::to_string(k),
                                   timer.elapsed_nano());
            timer.reset();
#endif
            ...
            if (data_type == DataType::TYPE_FP32) {
                gemm_kernel_.gemm_kernel_arm<float, float>(
                    m, n, k, k_pack, lda, A_fp32_ptr, B_bf16_ptr, C_fp32_ptr, nullptr, 0, workspace->data());
            } else if (data_type == DataType::TYPE_FP16) {
                gemm_kernel_.gemm_kernel_arm<float, float16_t>(
                    m, n, k, k_pack, lda, A_fp32_ptr, B_bf16_ptr, C_fp16_ptr, nullptr, 0, workspace->data());
            } else {
...
#ifdef GEMM_DEBUG
            timer_recorder_.record(std::string("gemm_kernel") + ", m=" + std::to_string(m) + ", n=" + std::to_string(n)
                                       + ", k=" + std::to_string(k),
                                   timer.elapsed_nano());
#endif
            ...
            timer.reset();
#endif
            // gemm_kernel_.gemm_kernel_arm(m, n, k, k_pack, lda, A_fp16_ptr, B_bf16_ptr, C_fp32_ptr, nullptr, 0,
            // workspace->data());
            if (data_type == DataType::TYPE_FP32) {
                gemm_kernel_.gemm_kernel_arm<float16_t, float>(
                    m, n, k, k_pack, lda, A_fp16_ptr, B_bf16_ptr, C_fp32_ptr, nullptr, 0, workspace->data());
            } else if (data_type == DataType::TYPE_FP16) {
                gemm_kernel_.gemm_kernel_arm<float16_t, float16_t>(
                    m, n, k, k_pack, lda, A_fp16_ptr, B_bf16_ptr, C_fp16_ptr, nullptr, 0, workspace->data());
            } else {
                std::cerr << "Unsupported data type for compute" << std::endl;
                return nullptr;
            }
