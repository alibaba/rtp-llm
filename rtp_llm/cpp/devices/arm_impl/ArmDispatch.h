...
#define DISPATCH_FOR_EACH_COMPUTE_TYPE(MACRO, ...)         \
    ...
    MACRO(DataType::TYPE_FP16, __fp16, __VA_ARGS__)          \
    MACRO(DataType::TYPE_BF16, hie::bfloat16, __VA_ARGS__) \
    default: \
        RTP_LLM_CHECK(false);
...
#define DISPATCH_ARM_FUNCTION_DATA_TYPE(data_type, function, ...)                              \
    do {                                                                                        \
        switch (data_type) {                                                                    \
...
#define GENERAL_OUTER_TYPE_CASE(dtype1, T1, dtype2, function, ...)                           \
    case dtype1: {                                                                           \
        switch (dtype2) {                                                                    \
            DISPATCH_FOR_EACH_COMPUTE_TYPE(DP_TWO_TYPE_FUNCTION_CALL_CASE, T1, function, __VA_ARGS__);  \
        }                                                                                    \
        break;                                                                               \
    }

#define DISPATCH_ARM_FUNCTION_TWO_DATA_TYPES(dtype1, dtype2, function, ...)                      \
    switch (dtype1) {                                                                                \
        GENERAL_OUTER_TYPE_CASE(DataType::TYPE_FP16, __fp16, dtype2, function, __VA_ARGS__)            \
        GENERAL_OUTER_TYPE_CASE(DataType::TYPE_BF16, hie::bfloat16, dtype2, function, __VA_ARGS__)   \
