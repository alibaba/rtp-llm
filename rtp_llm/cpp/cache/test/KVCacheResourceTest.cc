...
TEST(KVCacheResourceTest, InitGroups_RespectsGroupTypesAndBlocksPerKvBlock) {
    ...
    ASSERT_EQ(resource.blockIdsForLayer(0).size(), 1u);
    EXPECT_EQ(&resource.blockIdsForLayer(0).at("group0"), &resource.blockIds("group0"));
    ASSERT_EQ(resource.blockIdsForLayer(1).size(), 1u);
    EXPECT_EQ(&resource.blockIdsForLayer(1).at("group1"), &resource.blockIds("group1"));
    ASSERT_EQ(resource.blockIdsForLayer(2).size(), 1u);
    EXPECT_EQ(&resource.blockIdsForLayer(2).at("group0"), &resource.blockIds("group0"));
    ...
    for (int layer_id = 0; layer_id < 3; ++layer_id) {
        const auto layer_blocks = single_group_resource.blockIdsForLayer(layer_id);
        ASSERT_EQ(layer_blocks.size(), 1u);
        EXPECT_EQ(&layer_blocks.at("group0"), &single_group_resource.blockIds("group0"));
    }
...
TEST(KVCacheResourceTest, LayerViewReturnsAllGroupsForOneLayer) {
    KVCacheResource resource;
    ...
    const auto layer_blocks = resource.blockIdsForLayer(0);
    EXPECT_EQ(layer_blocks.size(), 2u);
    EXPECT_TRUE(layer_blocks.contains("group0"));
    EXPECT_TRUE(layer_blocks.contains("group1"));
}
...
TEST(KVCacheResourceTest, TagAccessKeepsSameLayerGroupsIndependent) {
    ...
    const auto entries = resource.blockIdsForLayer(0);
    EXPECT_EQ(entries.size(), 2u);
    const auto first = *entries.begin();
    EXPECT_TRUE(first.tag == "full" || first.tag == "linear");
    EXPECT_EQ(&first.value.get(), &resource.blockIds(first.tag));
}
