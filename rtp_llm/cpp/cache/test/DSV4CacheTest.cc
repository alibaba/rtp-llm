...
TEST_F(DSV4AllocatorTest, PrefixCacheReusePagedGroupsOnly) {
    ...
    EXPECT_GT(result.reuse_tokens, 0) << "Prefix cache reuse should work with paged DSV4 groups";
...
TEST_F(DSV4AllocatorTest, PrefixCacheReuseRequiresSWATailHit) {
    ...
    EXPECT_EQ(result.reuse_tokens, 0) << "SWA tail miss should veto paged prefix reuse";
...
TEST_F(DSV4AllocatorTest, PrefixCacheReuseDoesNotRequireHCAStateHit) {
    ...
    EXPECT_GT(result.reuse_tokens, 0) << "HCA_STATE miss should not veto DSV4 prefix reuse";
...
TEST_F(DSV4AllocatorTest, PrefixCacheReuseAcceptsSingleLatestSWATailHit) {
    ...
    EXPECT_GT(result.reuse_tokens, 0) << "latest SWA tail hit should allow paged prefix reuse";
...
TEST_F(DSV4AllocatorTest, FlashPrefixCacheReusePagedGroupsOnly) {
    ...
    EXPECT_GT(result.reuse_tokens, 0) << "Flash prefix cache reuse should work for paged groups";
...
TEST_F(DSV4AllocatorTest, SWAPrefixCacheRestoresTailReuse) {
    ...
    auto result              = allocator->malloc(info);
    ASSERT_TRUE(result.success);
    EXPECT_GT(result.reuse_tokens, 0);
