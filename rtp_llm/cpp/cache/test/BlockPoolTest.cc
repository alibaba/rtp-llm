...
TEST_F(BlockPoolTest, PinnedMlaKeepsIndexerOnGpuAndVersionsRecycledBlocks) {
    ...
    EXPECT_TRUE(indexer[2].is_cuda());
    EXPECT_TRUE(indexer[3].is_cuda());
#if USING_CUDA
...
TEST_F(BlockPoolTest, PinnedMlaPhysicalBlocksUseCompactedHostOffsets) {
    ...
    config.dsa_mla_hbm_blocks          = 3;
    config.dsa_mla_resident_tokens     = 256;
    auto pool_config                   = BlockPoolConfigHelper::createConfig(config);
    pool_config.memory_layouts.front().kernel_blocks_per_kv_block = 2;
    BlockPool pool(pool_config);
    ASSERT_TRUE(pool.init());
    const auto host = pool.allLayerCacheBase().front();
    const auto hbm  = pool.allLayerHbmCacheBase().front();
    for (int id = 0; id < 7; ++id) {
        const auto buffers = pool.convertIndexToBuffer(0, id);
        ASSERT_GE(buffers.size(), 1u);
        EXPECT_EQ(buffers[0].is_cuda, id < 3);
        EXPECT_EQ(buffers[0].addr, id < 3 ? hbm[id].data_ptr() : host[id - 3].data_ptr());
    }
}
