...
TEST_F(KVCacheAllocatorCPShardTest, FullPromptHitRetainsOneCPLogicalBlockForPrefill) {
    auto config    = makeCPHybridConfig();
    auto allocator = std::make_shared<KVCacheAllocator>(config, AllocationType::DEVICE);
    allocator->setSharedBlockCache(std::make_shared<SharedBlockCache>());
    ASSERT_TRUE(allocator->init());
...
TEST_F(KVCacheAllocatorCPShardTest, ConnectorRefPairsCanonicalKeysWithCompactLocalBlocks) {
    auto config    = makeCPHybridConfig();
    ...
    auto selected =
        allocator->incrKVCacheRef(resource, CacheKeysByGroup{{"full", CacheKeysType{101, 103}}}, /*is_connector=*/true);
...
TEST_F(KVCacheAllocatorCPShardTest, ReportedTokenCapacityUsesCPLogicalSpan) {
    auto config    = makeCPHybridConfig();
    auto allocator = std::make_shared<KVCacheAllocator>(config, AllocationType::HOST);
    ASSERT_TRUE(allocator->init());
    setCPMapper(allocator, /*cp_rank=*/0, /*cp_size=*/2);
