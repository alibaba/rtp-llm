...
// Cache (key, block) pair into the SharedBlockCache and drop request ref so the block is reusable.
// M03 unified the legacy per-pool BlockCache into a single SharedBlockCache owned by the allocator;
// SharedBlockCache::put() merges per-group slots and bumps the CACHE refcount on every pool, so the
// callers no longer issue an explicit blockCacheReference.
std::vector<BlockIdxType> seedCache(BlockPoolPtr         block_pool,
    ...
    for (size_t i = 0; i < keys.size(); ++i) {
        ...
        shared_cache->put(keys[i], group_slots, /*is_resident=*/true);
    }
...
TEST_F(HybridKVCacheAllocatorCPShardTest, ReuseHitOnLastRankCanonicalKey) {
    ...
    seedCache(block_pool, shared_cache, gid_full, group_num, CacheKeysType{101});
    seedCache(block_pool, shared_cache, gid_linear, group_num, CacheKeysType{101});
