...
TEST(PPTopologyValidatorTest, SwaGroupPassesWhenEveryStageOwnsIt) {
    // DSV4 state rings use SWA groups; per-tag minima reconcile stage capacities.
    ...
    EXPECT_FALSE(validatePPTopology({make(50, 25), make(50, 25)}).ok);
    auto identical = validatePPTopology({make(50, 25), make(50, 25)}, 1.5, true);
    ...
    auto skewed = validatePPTopology({make(50, 30), make(50, 25)}, 1.5, true);
    ...
    auto missing = validatePPTopology({makeFullSnapshot(50), make(50, 25)}, 1.5, true);
