...
void expectGeometry(const CacheLayerLayout& layout,
                    const CacheConfig&     physical_config,
                    int                    page_tokens,
                    int                    local_shards,
                    int                    upstream_shards) {
    ...
    for (size_t group = 0; group < layout.group_types.size(); ++group) {
        ...
        EXPECT_EQ(layout.group_seq_size_per_block[group],
                  static_cast<size_t>(kind == CacheGroupType::LINEAR ? page_tokens * upstream_shards : page_tokens));
    }
    ...
    for (const int page_tokens : {128, 256}) {
        for (const int upstream_shards : {2, 4, 8, 16}) {
            for (const auto topology :
                ...
                ASSERT_TRUE(manager.init());
                const auto main = manager.getMainModelCacheLayerLayout();
                expectGeometry(main, config, page_tokens, topology.local_shards, upstream_shards);
                expectReplayMetadata(main, 2, 3, 96 / topology.attention_tp);
                ...
                for (int module = 0; module < 2; ++module) {
                    ...
                    expectGeometry(mtp, config, page_tokens, topology.local_shards, upstream_shards);
...
TEST_F(K3CacheGeometryManagerTest, Eagle3SwaDraftRetainsPhysicalGeometryAndTargetReplay) {
    ...
    for (const bool decode : {false, true}) {
        ...
        ASSERT_TRUE(manager.init());
        const auto main = manager.getMainModelCacheLayerLayout();
        expectGeometry(main, config, page_tokens, decode ? 1 : shards, shards);
        ...
        const auto mtp = manager.getMTPModuleCacheLayerLayout(0);
        expectGeometry(mtp, config, page_tokens, decode ? 1 : shards, shards);
        EXPECT_FALSE(mtp.linear_replay.has_value());
