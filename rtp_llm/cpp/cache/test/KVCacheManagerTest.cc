...
TEST_F(KVCacheManagerTest, WriteP2PLayer_HoldsAllocatorReferenceUntilLayerCleanup) {
    ...
    ASSERT_TRUE(manager->writeP2PLayer(
        0, 0, cache_config.tagForGroup(0), {7001}, blocks, request_id, nullptr, deadline_ms));
    EXPECT_EQ(pool->refCount(blocks.front()), 2u);
    EXPECT_NE(worker->getComputedBuffersStore()->getBuffer(request_id), nullptr);
    ...
    EXPECT_EQ(worker->getComputedBuffersStore()->getBuffer(request_id), nullptr);
