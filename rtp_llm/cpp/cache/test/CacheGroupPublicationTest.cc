...
// KVCacheAllocator::reuseParticipatingGroupIds(), which delegates to
// reuseParticipatingGroupIdsFromPolicies() for every allocator variant.
...
TEST(CacheGroupPublicationTest, HbmProtocolRejectsAnyHostPlacedRequiredGroup) {
    auto host_policy                             = defaultCacheGroupPolicy(CacheGroupType::FULL);
    host_policy.memory_placement                 = CacheMemoryPlacement::HOST_PINNED;
    const std::vector<CacheGroupPolicy> policies = {
        defaultCacheGroupPolicy(CacheGroupType::FULL),
        host_policy,
    };

    EXPECT_TRUE(kvCacheEventGroupsUseDeviceMemory(policies, {0}));
    EXPECT_FALSE(kvCacheEventGroupsUseDeviceMemory(policies, {0, 1}));
    EXPECT_FALSE(kvCacheEventGroupsUseDeviceMemory(policies, {0, 2}));
}
