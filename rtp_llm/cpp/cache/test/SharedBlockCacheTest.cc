...
GroupBlock groupBlock(std::string_view tag, BlockIdxType block_id, bool matchable = true) {
...
BlockIdxType evictedBlockForTag(const SharedBlockCache::EvictResult& result, CacheKeyType key, std::string_view tag) {
    const auto& blocks = result.evicted_blocks_by_group.at(key);
...
TEST(SharedBlockCacheTest, NonMatchableSlotStillEvictsButDoesNotMatchGroup) {
    TestSharedBlockCache cache;
    cache.put(1,
              {groupBlock(kFullTag, 101), groupBlock(kLinearTag, 201, /*matchable=*/false)},
              /*is_resident=*/false,
