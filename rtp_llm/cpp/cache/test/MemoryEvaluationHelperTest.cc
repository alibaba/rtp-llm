...
protected:
    void SetUp() override {
        ...
        // Every case depends on the executable-local stub winning over the production shared
        // symbol. Check that invariant before any test can obtain false confidence from the stub.
        ASSERT_EQ(getGpuExecStatus().device_memory_status.total_bytes, 0u)
            << "host getGpuExecStatus stub is not active; check the test target's link order";
    }

    void TearDown() override {
        test::stubbedDeviceMemoryStatus()            = MemoryStatus{};
...
TEST_F(MemoryEvaluationHelperTest, ExplicitKvCacheMemShortCircuitsEverything) {
    ...
    EXPECT_NE(capture.content().find("[KV_ALLOC] warm_up=0 source=explicit total=4096 MiB"), std::string::npos);
}
...
// pool_shrink exceeding the measured growth means the warmup left more resident than it grew: the
// difference comes out of the safety margin, which must WARN. The budget itself is untouched as long
// as the cap does not bind, i.e. while pool_shrink stays within max(configured, growth) -- 512 MiB
// shrink against a 256 MiB growth is inside the 1 GiB configured term.
TEST_F(MemoryEvaluationHelperTest, UnaccountedPoolShrinkWarnsButKeepsTheBudget) {
    ...
    // Budget = 40 GiB + 512 MiB - 5 GiB = 35 GiB + 512 MiB, which is under the
    // available(40 GiB) - safety(4 GiB) = 36 GiB cap, so the cap does not bind.
    EXPECT_EQ(size(warm_up), 40 * GiB + 512 * MiB - 5 * GiB);
    EXPECT_NE(capture.content().find("[KV_ALLOC_POOL_SHRINK]"), std::string::npos);
}

// The cap is the only thing keeping the trusted path's budget inside real free memory, since its
// base (pre_warmup) is deliberately larger than the pool free at cache-config time. What it
// guarantees is that the safety margin survives -- capping to available_bytes alone would leave
// nothing free and move the failure from startup to the first serving forward.
TEST_F(MemoryEvaluationHelperTest, BudgetExceedingRealFreeMemoryIsCappedAndKeepsTheSafetyMargin) {
    ...
    EXPECT_EQ(capped, 40 * GiB - 4 * GiB);
    EXPECT_NE(capture.content().find("[KV_ALLOC_CAPPED]"), std::string::npos);
    // The safety margin still fits alongside the cache in the real pool -- the point of the cap.
    EXPECT_LE(capped + 4 * GiB, status_.available_bytes);
}

// Same trusted path, but the real pool cannot even hold the safety margin: that must abort at
// startup rather than hand back a budget with no headroom at all.
TEST_F(MemoryEvaluationHelperTest, AbortsWhenRealFreeMemoryCannotCoverTheSafetyMargin) {
    status_.available_bytes = 3 * GiB;  // below the 4 GiB safety term (5% of 80 GiB)
    WarmUpResult warm_up;
    warm_up.measurement_trusted         = true;
    warm_up.available_bytes_pre_warmup  = 60 * GiB;
    warm_up.device_reserved_bytes       = status_.available_bytes;
...
// Every other case here drives the MemoryStatus overload, which means the overload production
// actually calls -- the one that samples the device itself -- is otherwise never executed. Gate it
// by driving the stub's reported status and asserting the two overloads agree: that is what catches
// a field the sampling path forgets to forward.
TEST_F(MemoryEvaluationHelperTest, SamplingOverloadMatchesTheInjectedOverload) {
    test::stubbedDeviceMemoryStatus() = status_;
    WarmUpResult warm_up;
    warm_up.measurement_trusted         = true;
    warm_up.available_bytes_pre_warmup  = 42 * GiB;
    warm_up.device_reserved_bytes       = status_.available_bytes;
    ...
    const size_t sampled = MemoryEvaluationHelper::getKVCacheMemorySize(
        runtime_config_, kv_cache_config_, model_config_, warm_up, std::nullopt);
    EXPECT_EQ(sampled, size(warm_up));
}
