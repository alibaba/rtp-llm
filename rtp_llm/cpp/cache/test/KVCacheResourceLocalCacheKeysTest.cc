...
protected:
    KVCacheResource make(const CacheKeysType& keys) {
        KVCacheResource r;
        r.initGroups(makeTestCacheTopology(/*group_num=*/1, /*layer_num=*/1, {{0}}));
        r.cacheKeys("group0") = keys;
        return r;
    }
};
...
TEST_F(LocalCacheKeysTest, CpSize1Passthrough) {
    ...
    auto out = r.localCacheKeys("group0", 0, 1);
...
TEST_F(LocalCacheKeysTest, CpSize2EvenLengthLastRank) {
    ...
    auto out = r.localCacheKeys("group0", 1, 2);
    ASSERT_EQ(out.size(), 4u);
    EXPECT_EQ(out[0], 101);
    EXPECT_EQ(out[1], 201);
    EXPECT_EQ(out[2], 301);
...
TEST_F(LocalCacheKeysTest, CpSize2Rank0) {
    ...
    auto out = r.localCacheKeys("group0", 0, 2);
...
TEST_F(LocalCacheKeysTest, CpSize4NonDivisibleLastRankShorter) {
    ...
    auto out = r.localCacheKeys("group0", 3, 4);
    ASSERT_EQ(out.size(), 2u);
    EXPECT_EQ(out[0], 3);
    EXPECT_EQ(out[1], 7);
}
...
TEST_F(LocalCacheKeysTest, EmptyKeys) {
    auto r   = make({});
    auto out = r.localCacheKeys("group0", 0, 4);
    EXPECT_TRUE(out.empty());
}

TEST_F(LocalCacheKeysTest, KeysShorterThanCpSizeReturnsEmptyForLastRank) {
    auto r   = make({42});
    auto out = r.localCacheKeys("group0", 3, 4);  // last-rank starts at idx 3, but only 1 key
    EXPECT_TRUE(out.empty());
}
