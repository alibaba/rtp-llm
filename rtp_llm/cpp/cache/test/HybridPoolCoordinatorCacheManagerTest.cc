...
TEST_F(CoordinatorCacheManagerTest, ConvertIndexToAddrAndBufferByGroup) {
    ...
    auto        addr_default   = coordinator_manager->convertIndexToAddr(/*layer_id=*/0, group_tag, /*block_id=*/1);
    ...
    auto bufs_default = coordinator_manager->convertIndexToBuffer(/*layer_id=*/0, group_tag, /*block_id=*/1);
...
TEST_F(CoordinatorCacheManagerTest, ReserveShareDoesNotCollapseForDepletedPool) {
    ...
    coordinator_manager->setReserveBlocksNum(reserve_blocks);
    const auto before = coordinator_manager->poolMetricsSnapshots();
    ...
    const auto after = coordinator_manager->poolMetricsSnapshots();
...
TEST_F(CoordinatorCacheManagerTest, ConcurrentInitMallocCannotOverrunReserve) {
    ...
    for (int i = 0; i < thread_count; ++i) {
        threads.emplace_back([&, i] {
            ...
            info.verbose                      = false;
            successes[static_cast<size_t>(i)] = coordinator_manager->malloc(info).success ? 1 : 0;
        });
    }
...
TEST_F(CoordinatorCacheManagerTest, DSV4ConvertIndexToAddrByTagRoutesToCorrectPool) {
    ...
    auto addr_csa = coordinator_manager->convertIndexToAddr(csa_layer, "csa_kv", 1);
    EXPECT_NE(addr_csa.kv_addr, nullptr);
    EXPECT_EQ(addr_csa.kv_addr, coordinator_manager->convertIndexToBuffer(csa_layer, "csa_kv", 1).front().addr);

    auto addr_swa = coordinator_manager->convertIndexToAddr(csa_layer, "swa_kv", 1);
    ...
    EXPECT_THROW((void)coordinator_manager->convertIndexToAddr(csa_layer, "missing", 1), std::exception);
    EXPECT_THROW((void)coordinator_manager->convertIndexToAddr(csa_layer, "hca_kv", 1), std::exception);
...
TEST_F(CoordinatorCacheManagerTest, DSV4ConvertIndexToBufferByTagAndPartition) {
    ...
    auto buf = coordinator_manager->convertIndexToBuffer(csa_layer, "csa_kv", /*block_id=*/1);
    ASSERT_FALSE(buf.empty());
    EXPECT_NE(buf[0].addr, nullptr);

    auto buf_part = coordinator_manager->convertIndexToBuffer(
