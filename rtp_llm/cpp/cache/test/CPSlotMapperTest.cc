...
TEST_F(CPSlotMapperTest, PhysicalBlockPositionProjectsCompactLastRankKeys) {
    ...
    EXPECT_EQ(mapper.localPhysicalBlockPosition(policy, 0, 5), std::nullopt);
    EXPECT_EQ(mapper.localPhysicalBlockPosition(policy, 1, 5), std::make_optional<size_t>(0));
    EXPECT_EQ(mapper.localPhysicalBlockPosition(policy, 3, 5), std::make_optional<size_t>(1));
    EXPECT_EQ(mapper.localPhysicalBlockPosition(policy, 4, 5), std::make_optional<size_t>(2));
}
