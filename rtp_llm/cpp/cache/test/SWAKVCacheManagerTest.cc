...
GroupTopology makeSwaGroupTopology(const KVCacheSpecPtr& spec,
                                   CacheGroupPolicy      policy = defaultCacheGroupPolicy(CacheGroupType::SWA)) {
    return makeTestGroupTopology(spec, policy);
}
...
class SWAKVCacheManagerTest: public ::testing::Test {
protected:
    void SetUp() override {
    ...
    SWAKVCacheManager makeGroup(int seq_size_per_block) {
        auto spec = makeSwaSpec(seq_size_per_block);
        return SWAKVCacheManager(makeSwaGroupTopology(spec), block_pool_, /*linear_step=*/0, shared_cache_.get());
    }

    SWAKVCacheManager makeGroupWithStep(int seq_size_per_block, int linear_step) {
        auto spec = makeSwaSpec(seq_size_per_block);
        return SWAKVCacheManager(makeSwaGroupTopology(spec, makePolicy(/*skip_prefix_reuse=*/false)),
                                 block_pool_,
                                 linear_step,
                                 shared_cache_.get());
    }
...
TEST_F(SWAKVCacheManagerTest, DefaultPolicyDrivesBehaviorInterfaces) {
    auto              spec = makeSwaSpec(4);
    SWAKVCacheManager group(makeSwaGroupTopology(spec), block_pool_, /*linear_step=*/0, shared_cache_.get());
...
TEST_F(SWAKVCacheManagerTest, NeedBlocksNum_Basic) {
...
TEST_F(SWAKVCacheManagerTest, NeedBlocksNum_WithCurrentBlocks) {
    auto group = makeGroup(4);
...
TEST_F(SWAKVCacheManagerTest, NeedBlocksNum_WithReserveStep) {
    auto group = makeGroup(4);
...
TEST_F(SWAKVCacheManagerTest, GetNeedBlocks_SeqLenZero) {
    auto group = makeGroup(4);
...
TEST_F(SWAKVCacheManagerTest, GetNeedBlocks_ReuseDisabledCountsActiveTail) {
...
TEST_F(SWAKVCacheManagerTest, GetNeedBlocks_ReuseEnabledUsesSparse) {
    auto group = makeGroupWithStep(4, 2);
...
TEST_F(SWAKVCacheManagerTest, GetNeedBlocks_HCAStateReuseEnabledCountsTailOnly) {
    auto spec  = makeDsv4StateSpec("hca_state", 4);
    auto group = SWAKVCacheManager(
        makeSwaGroupTopology(spec, makePolicy(true)), block_pool_, /*linear_step=*/3, shared_cache_.get());
...
TEST_F(SWAKVCacheManagerTest, GetNeedBlocks_CSAStateReuseEnabledStillUsesSparse) {
    auto spec  = makeDsv4StateSpec("csa_state", 4);
    auto group = SWAKVCacheManager(
        makeSwaGroupTopology(spec, makePolicy()), block_pool_, /*linear_step=*/3, shared_cache_.get());
...
TEST_F(SWAKVCacheManagerTest, GetNeedBlocks_WithReserveStep) {
    auto group = makeGroupWithStep(4, 2);
...
TEST_F(SWAKVCacheManagerTest, GetNeedBlocks_ReusePartialOverlap) {
    auto group = makeGroupWithStep(4, 2);
    // seq_len=12 => seq_slots=3
...
TEST_F(SWAKVCacheManagerTest, GetNeedBlocks_ReuseFullOverlap) {
    auto group = makeGroupWithStep(4, 2);
    // seq_len=12 => seq_slots=3
...
TEST_F(SWAKVCacheManagerTest, GetNeedBlocks_CommonSeqLenIgnored) {
    auto group = makeGroup(4);
...
TEST_F(SWAKVCacheManagerTest, MatchAlwaysThrows) {
    auto group = makeGroup(4);
    EXPECT_THROW(group.match({101, 102, 103}), std::exception);
}

TEST_F(SWAKVCacheManagerTest, MatchSingleKey_NotFound) {
    auto group  = makeGroup(4);
    auto result = group.matchSingleKey(999);
    EXPECT_TRUE(result.block_indices.empty());
}

TEST_F(SWAKVCacheManagerTest, MatchSingleKey_Found) {
    auto group = makeGroup(4);
...
TEST_F(SWAKVCacheManagerTest, Malloc_ShortSeq_OnlyOneBlock) {
    auto     group = makeGroup(4);
...
TEST_F(SWAKVCacheManagerTest, Malloc_ManyBlocks_LastTwoActiveBlocksReal) {
    auto     group = makeGroup(4);
    BlockIds block_ids(1);
...
TEST_F(SWAKVCacheManagerTest, Malloc_DSV4PromptTailKeepsPenultimateBlock) {
...
TEST_F(SWAKVCacheManagerTest, Malloc_NoOpWhenEnoughBlocks) {
    auto     group = makeGroup(4);
    BlockIds block_ids(1);
...
TEST_F(SWAKVCacheManagerTest, Malloc_SkipsNullTailCheckWhenPolicyDisablesValidation) {
    auto spec  = makeSwaSpec(4);
    auto group = SWAKVCacheManager(
        makeSwaGroupTopology(spec, makePolicy(true)), block_pool_, /*linear_step=*/0, shared_cache_.get());
    BlockIds block_ids(1);
...
TEST_F(SWAKVCacheManagerTest, Malloc_HCAStateReuseEnabledAllocatesTailOnly) {
    auto spec  = makeDsv4StateSpec("hca_state", 4);
    auto group = SWAKVCacheManager(
        makeSwaGroupTopology(spec, makePolicy(true)), block_pool_, /*linear_step=*/3, shared_cache_.get());
    BlockIds block_ids(1);
...
TEST_F(SWAKVCacheManagerTest, Malloc_CSAStateReuseEnabledKeepsSparseBlocks) {
    auto spec  = makeDsv4StateSpec("csa_state", 4);
    auto group = SWAKVCacheManager(
        makeSwaGroupTopology(spec, makePolicy()), block_pool_, /*linear_step=*/3, shared_cache_.get());
    BlockIds block_ids(1);
...
TEST_F(SWAKVCacheManagerTest, Malloc_RejectsNullTailWhenValidationEnabled) {
    auto spec  = makeSwaSpec(4);
    auto group = SWAKVCacheManager(
        makeSwaGroupTopology(spec, makePolicy()), block_pool_, /*linear_step=*/0, shared_cache_.get());
    BlockIds block_ids(1);
    block_ids.assign(BlockIndicesType{NULL_BLOCK_IDX, NULL_BLOCK_IDX, NULL_BLOCK_IDX});
...
TEST_F(SWAKVCacheManagerTest, Malloc_RejectsNullPenultimateBlockWhenTwoTailBlocksAreActive) {
    auto spec  = makeSwaSpec(4);
    auto group = SWAKVCacheManager(
        makeSwaGroupTopology(spec, makePolicy()), block_pool_, /*linear_step=*/0, shared_cache_.get());
    BlockIds block_ids(1);
...
TEST_F(SWAKVCacheManagerTest, Malloc_WithReserveStep) {
    auto     group = makeGroup(4);
    BlockIds block_ids(1);
...
TEST_F(SWAKVCacheManagerTest, Malloc_FailsWhenPoolExhausted) {
    auto                  group = makeGroup(4);
...
TEST_F(SWAKVCacheManagerTest, Malloc_WithStep_ReuseEnabled) {
    auto     group = makeGroupWithStep(4, 2);
    BlockIds block_ids(1);
...
TEST_F(SWAKVCacheManagerTest, Malloc_WithStep_ReuseDisabled) {
    auto     group = makeGroupWithStep(4, 2);
    BlockIds block_ids(1);
...
TEST_F(SWAKVCacheManagerTest, Malloc_WithStep_ReserveAllocated) {
    auto     group = makeGroupWithStep(4, 2);
    BlockIds block_ids(1);
...
TEST_F(SWAKVCacheManagerTest, RemoveSkippedBlocks_TwoOrFewer_NoOp) {
    auto     group = makeGroup(4);
    BlockIds block_ids(1);
...
TEST_F(SWAKVCacheManagerTest, RemoveSkippedBlocks_FreesNonTailReal) {
    auto     group = makeGroupWithStep(4, 2);
    BlockIds block_ids(1);
...
TEST_F(SWAKVCacheManagerTest, RemoveSkippedBlocks_WithStep_FreesNonStepBlocks) {
    auto block_pool = createBlockPool();
    ASSERT_TRUE(block_pool->init());
    ASSERT_EQ(block_pool->freeBlocksNum(), 9u);

    auto              spec = makeSwaSpec(4);
    SWAKVCacheManager group(
        makeSwaGroupTopology(spec, makePolicy(/*skip_prefix_reuse=*/false)), block_pool, /*linear_step=*/2);
...
TEST_F(SWAKVCacheManagerTest, RemoveSkippedBlocks_HCAStateReuseEnabledKeepsTailOnly) {
    auto block_pool = createBlockPool();
    ASSERT_TRUE(block_pool->init());
    ASSERT_EQ(block_pool->freeBlocksNum(), 9u);

    auto spec  = makeDsv4StateSpec("hca_state", 4);
    auto group = SWAKVCacheManager(makeSwaGroupTopology(spec, makePolicy(true)), block_pool, /*linear_step=*/2);
...
TEST_F(SWAKVCacheManagerTest, RemoveSkippedBlocks_WithReserveStep) {
    auto block_pool = createBlockPool();
    ASSERT_TRUE(block_pool->init());
    ASSERT_EQ(block_pool->freeBlocksNum(), 9u);
...
TEST_F(SWAKVCacheManagerTest, Free_ReleasesRealBlocks) {
    auto     group = makeGroup(4);
    BlockIds block_ids(1);
    ASSERT_TRUE(group.malloc(block_ids, 20));
...
TEST_F(SWAKVCacheManagerTest, Free_Empty) {
    auto group = makeGroup(4);
    group.free({});
    EXPECT_EQ(block_pool_->freeBlocksNum(), total_blocks_);
}

TEST_F(SWAKVCacheManagerTest, Free_SkipsNullBlocks) {
    auto     group = makeGroup(4);
    BlockIds block_ids(1);
    ASSERT_TRUE(group.malloc(block_ids, 20));
    EXPECT_LT(block_pool_->freeBlocksNum(), total_blocks_);
...
TEST_F(SWAKVCacheManagerTest, Reference_AddsAndRefsBlocks) {
    auto     group = makeGroup(4);
    BlockIds block_ids(1);
    ASSERT_TRUE(group.malloc(block_ids, 5));
...
TEST_F(SWAKVCacheManagerTest, Reference_NullBlocksNotReffed) {
    auto     group = makeGroup(4);
    BlockIds block_ids(1);
    ASSERT_TRUE(group.malloc(block_ids, 20));
    auto original = block_ids.blocks();
...
TEST_F(SWAKVCacheManagerTest, PutIntoCache_SkipsNullBlocks) {
    auto     group = makeGroup(4);
    BlockIds block_ids(1);
    ASSERT_TRUE(group.malloc(block_ids, 20));
...
// Reproduces the historical bug where SWAKVCacheManager::malloc called block_pool_->malloc(1)
...
// rollback in HybridPoolCoordinatorKVCacheManager::initMallocForCommonLen could not see them.
//
// After the fix, SWAKVCacheManager::malloc performs a single atomic batch malloc on the pool,
// so a failed allocation must leave the pool's free counter unchanged.
TEST_F(SWAKVCacheManagerTest, Malloc_FailsAtomicallyWithoutLeak) {
    auto group = makeGroupWithStep(4, 2);
...
// Verifies the new behavior: SWAKVCacheManager::malloc reserves all required physical blocks
// via a single batch BlockPool::malloc(N) call instead of N individual malloc(1) calls.
TEST_F(SWAKVCacheManagerTest, Malloc_AllocatesAtomicallyAsBatch) {
    auto         group       = makeGroupWithStep(4, 2);
    const size_t free_before = block_pool_->freeBlocksNum();
...
TEST_F(SWAKVCacheManagerTest, Malloc_BatchPlacementMatchesShouldAllocate) {
    auto         group       = makeGroupWithStep(4, 2);
    const size_t free_before = block_pool_->freeBlocksNum();
