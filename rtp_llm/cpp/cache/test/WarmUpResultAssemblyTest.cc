...
TEST(WarmUpResultAssemblyTest, PairsThePreWarmupPoolWithTheTotalGrowth) {
    const auto peak = makeStatus(/*torch_peak=*/12 * GiB, /*torch_current=*/12 * GiB, /*non_torch=*/70 * MiB, 0);
    const auto post_teardown =
        makeStatus(/*torch_peak=*/0, /*torch_current=*/200 * MiB, /*non_torch=*/70 * MiB, /*available=*/60 * GiB);
...
// The growth term is read entirely from the peak sample, so the post-teardown readings cannot move
// it -- not even when they exceed the peak, which an unrelated process on the same GPU can cause.
TEST(WarmUpResultAssemblyTest, GrowthTermIgnoresThePostTeardownReadings) {
    const auto peak          = makeStatus(/*torch_peak=*/100 * MiB, /*torch_current=*/100 * MiB, 70 * MiB, 0);
    const auto post_teardown = makeStatus(0, /*torch_current=*/300 * MiB, /*non_torch=*/128 * MiB, 40 * GiB);

    const auto result = assembleWarmUpResult(/*pre_warmup_available_bytes=*/41 * GiB, peak, post_teardown, true);

    EXPECT_EQ(result.measured_total_growth_bytes, 100 * MiB + 70 * MiB);
