...
void CacheManager::clearIncompleteBlocks(const std::vector<int>& block_indices) {
    // All TP ranks need to clear incomplete blocks to prevent NaN pollution
    // tpSyncModelInputs synchronizes block IDs from TP0 to all ranks,
    // but each TP rank has its own KV cache memory space, so all ranks should clear
    if (!block_indices.empty()) {
        allocator_->clearIncompleteBlocks(block_indices);
    }
}
