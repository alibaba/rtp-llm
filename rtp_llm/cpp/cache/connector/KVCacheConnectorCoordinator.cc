...
KVCacheConnectorCoordinator::asyncRead(const std::shared_ptr<KVCacheConnectorReadWriteContext>& connector_context) {
    ...
    RTP_LLM_LOG_INFO("yemu_debug match_keys size [%zu]", match_keys.size());
    ...
    for (const auto& [type, connector] : connectors_) {
        ...
        if (type == KVCacheConnector::ConnectorType::Remote && connector_context->enableRemoteCache()) {
            ...
            std::shared_ptr<KVCacheConnector::Meta> remote_connector_meta =
                std::make_shared<RemoteConnectorMeta>(unique_id, connector_context->trace_id(), tokens);
