...
namespace {
std::vector<std::vector<int>> makeLayerGroupIds(int layer_num, int group_id = 0) {
    return std::vector<std::vector<int>>(static_cast<size_t>(layer_num), {group_id});
}
}  // namespace
...
protected:
    void SetUp() override {
        ...
        {
            // Use the 4-arg overload to create a minimal BlockPool config.
...
TEST_F(KVCacheConnectorCoordinatorTest, AsyncRead_ReturnNull_WhenCacheKeysEmpty) {
    ...
    resource.initGroups(1, cache_config_.layer_all_num, makeLayerGroupIds(cache_config_.layer_all_num));
...
TEST_F(KVCacheConnectorCoordinatorTest, AsyncWrite_ReturnNull_WhenIncrKVCacheRefReturnsNull) {
    ...
    ctx_resource->initGroups(1, cache_config_.layer_all_num, makeLayerGroupIds(cache_config_.layer_all_num));
