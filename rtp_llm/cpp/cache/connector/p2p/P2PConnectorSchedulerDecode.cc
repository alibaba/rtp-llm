...
P2PConnectorSchedulerDecode::AsyncReadResult P2PConnectorSchedulerDecode::asyncRead(
    ...
    if (!no_transfer && config_.cp_size > 1 && prefill_tp_size != config_.cp_size) {
        RTP_LLM_LOG_WARNING("asyncRead: source/target CP layout mismatch, prefill_tp_size=%d, decode_cp_size=%d",
                            prefill_tp_size,
                            config_.cp_size);
        collector->success = false;
        return {nullptr,
