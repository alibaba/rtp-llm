...
std::shared_ptr<P2PConnectorResourceEntry> P2PConnectorStreamStore::waitAndStealResource(const std::string& unique_key,
                                                                                         int64_t deadline_ms) {
    std::unique_lock<std::mutex> lock(resource_map_mutex_);

    // 使用条件变量等待，直到资源可用或超时
    auto wait_result = resource_cv_.wait_until(
        lock, std::chrono::system_clock::time_point(std::chrono::milliseconds(deadline_ms)), [this, &unique_key]() {
    ...
    if (it == resource_map_.end()) {
        RTP_LLM_LOG_WARNING("P2PConnectorStreamStore::waitAndStealResource failed: resource not found, unique_key: %s",
                            unique_key.c_str());
        return nullptr;
    }
