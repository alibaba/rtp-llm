...
bool P2PConnectorPrefill::init() {
    ...
    if (config_.tp_rank == 0) {
        ...
        stream_store_->setOnRequestReleased(
            [computed_buffers = worker_->getComputedBuffersStore()](int64_t request_id, int64_t request_deadline_ms) {
                if (computed_buffers) {
...
std::shared_ptr<AsyncContext> P2PConnectorPrefill::registerResource(const KVCacheResourcePtr&    resource,
    ...
    if (metrics_reporter_) {
        ...
        collector.prefill_scheduler_resource_register_time_us = currentTimeUs() - register_start_us;
...
void P2PConnectorPrefill::processRead(const P2PConnectorStartLoadRequestPB& request,
    ...
    auto                         report_process_metrics = [&] {
        collector.prefill_scheduler_process_read_time_us = currentTimeUs() - process_start_us;
        if (metrics_reporter_) {
            metrics_reporter_->report<P2PConnectorMetrics, P2PConnectorMetricsCollector>(nullptr, &collector);
        }
    };
    ...
    if (!request.no_transfer()) {
        ...
        collector.prefill_scheduler_check_plan_time_us = currentTimeUs() - check_plan_start_us;
    ...
    collector.prefill_scheduler_resource_wait_time_us = wait_resource_cost_us;
    if (!wait_status.ok()) {
    ...
    transfer_deadline_ms = resource_entry->deadline_ms;
...
bool P2PConnectorPrefill::processNoTransferPerRank(int64_t request_id,
                                                   const std::string& /* unique_key */,
                                                   int64_t                                 deadline_ms,
                                                   const P2PConnectorBroadcastTpRequestPB& p2p_request,
                                                   FunctionResponsePB&                     response) {
...
grpc::Status P2PConnectorPrefill::waitForResourceEntry(
    ...
    resource_entry = stream_store_->waitAndStealResource(unique_key, transfer_deadline_ms, request_deadline_ms, is_cancelled);
    if (resource_entry) {
