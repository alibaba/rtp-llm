...
/// @brief P2P Connector 资源条目，存储 prefill 传输时需要的数据
...
public:
    ...
    /// @brief 等待并获取资源条目（阻塞直到资源可用或超时）
    /// @param unique_key 资源唯一标识
    /// @param deadline_ms 超时时间（毫秒时间戳）
    /// @return 资源条目，如果超时则返回 nullptr
    std::shared_ptr<P2PConnectorResourceEntry> waitAndStealResource(const std::string& unique_key, int64_t deadline_ms);
