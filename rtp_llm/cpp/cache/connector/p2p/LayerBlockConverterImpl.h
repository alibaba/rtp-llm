...
public:
    explicit LayerBlockConverterImpl(const std::shared_ptr<KVCacheAllocator>& allocator):
    ...
    std::vector<BlockInfo> convertIndexToBuffer(
        ...
        return filterValid(
            allocator_->convertIndexToBuffer(layer_id, tag, block_id, partition_count, partition_id));
    }
