...
public:
    explicit LayerBlockConverterImpl(const std::shared_ptr<CoordinatorCacheManager>& coordinator_manager):
    ...
    std::vector<BlockInfo> convertIndexToBufferByTag(
        ...
        return filterValid(
            coordinator_manager_->convertIndexToBufferByTag(layer_id, tag, block_id, partition_count, partition_id));
    }
