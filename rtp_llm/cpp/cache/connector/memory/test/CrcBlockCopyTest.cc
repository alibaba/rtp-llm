...
TEST(CrcBlockCopyTest, StandardCrcAndRaggedRoundtrip) {
    ...
    for (const auto& sizes :
        ...
        for (size_t gap : {size_t(0), size_t(3)}) {
            ...
            ASSERT_TRUE(copyBlock(*f.copy, f.host, f.bytes, f.tiles, false).success);
            ...
            ASSERT_EQ(cudaStreamSynchronize(nullptr), cudaSuccess);
            ASSERT_TRUE(copyBlock(*f.copy, f.host, f.bytes, f.tiles, true).success);
...
TEST(CrcBlockCopyTest, WholeBlockAlignmentKeepsPaddingOutsideCrc) {
    if (!CrcBlockCopy::supported()) {
        GTEST_SKIP() << "CRC backend unavailable";
    }
    ...
    ASSERT_GT(static_cast<size_t>(page_size), CrcBlockCopy::storageBytes(f.bytes));
    ...
    auto*                                  transferred = boundary - CrcBlockCopy::storageBytes(f.bytes);
    std::memcpy(transferred, f.host, CrcBlockCopy::storageBytes(f.bytes));
...
TEST(CrcBlockCopyTest, InheritVerifiedBlockAndRejectDirtySourceWithoutChangingCandidate) {
    ...
    ASSERT_TRUE(copyBlock(*source.copy, source.host, source.bytes, source.tiles, false).success);
    ...
    ASSERT_TRUE(
        copyBlock(*candidate.copy, candidate.host, candidate.bytes, tail, false, source.host)
            .success);
    ...
    const auto rejected = copyBlock(
        *candidate.copy, candidate.host, candidate.bytes, tail, false, source.host, [] {
            return true;
        });
    EXPECT_FALSE(rejected.success);
...
TEST(CrcBlockCopyTest, PageableBounceRoundtripAndMixedSource) {
    ...
    for (bool source_pinned : {false, true}) {
        ...
        ASSERT_TRUE(
            copyBlock(
                *target.copy, target.host, target.bytes, target.tiles, true, nullptr, {}, false)
                .success);
...
TEST(CrcBlockCopyTest, ReusedWorkspaceClearsUnwrittenSlots) {
    ...
    for (const auto& indices : std::vector<std::vector<size_t>>{{2}, {0, 2}, {0}}) {
        ...
        ASSERT_TRUE(copyBlock(*f.copy, f.host, f.bytes, partial, false).success);
    ...
    ASSERT_TRUE(copyBlock(*f.copy, f.host, f.bytes, f.tiles, false).success);
    ASSERT_TRUE(copyBlock(*f.copy, f.host, 9, {{f.device, 0, 9}}, false).success);
    ...
    for (int w = 0; w < 8; ++w)
        threads.emplace_back([&, w] {
            ...
            for (int run = 0; run < 20; ++run) {
                barrier.wait();
                EXPECT_TRUE(copyBlock(*f.copy, f.host, f.bytes, f.tiles, false).success);
                EXPECT_EQ(f.footer().crc32c, crc32c(f.host, f.bytes));
                EXPECT_TRUE(copyBlock(*f.copy, f.host, f.bytes, f.tiles, true).success);
