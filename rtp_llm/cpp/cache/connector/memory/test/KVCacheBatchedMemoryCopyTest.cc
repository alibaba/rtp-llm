...
CacheConfig makeCompactDsv4TypedMemoryCopyConfig(bool use_flash) {
    ...
    config.fromGroupedSpecs(specs, layers_by_group, group_types, group_tags, group_policies);
...
void runDsv4TypedStagedCopyRoundTrip(const std::set<std::string>& host_tags) {
    ...
    std::vector<std::string> server_addrs = {"127.0.0.1:1"};
    auto connector = std::make_shared<KVCacheMemoryConnector>(config, kv_config, coordinator_manager, server_addrs);
    ...
    for (size_t block_idx = 0; block_idx < request_mem_blocks.size(); ++block_idx) {
        ...
        for (size_t i = 0; i < slots.size(); ++i) {
            ...
            const auto  gpu_bufs =
                coordinator_manager->convertIndexToBuffer(slot.layer_id, slot.tag, gpu_block_sets[block_idx][i]);
...
TEST(KVCacheBatchedMemoryCopyTest, PrefixTreeReadRejectsCompressedOnlyWhenStateSwaRequired) {
    ...
    auto coordinator_manager = std::make_shared<FakeTypedCoordinatorCacheManager>(config);
...
TEST(KVCacheBatchedMemoryCopyTest, PrefixTreeBlockZeroAndNullSlotsAreNotCopiedForD2HAndH2D) {
    ...
    setBlockInfosContent(
        coordinator_manager->convertIndexToBuffer(valid_slot.layer_id, valid_slot.tag, valid_gpu_block), 'V');
    setBlockInfosContent(coordinator_manager->convertIndexToBuffer(slots[state_slots[1]].layer_id,
    ...
    setBlockInfosContent(
        coordinator_manager->convertIndexToBuffer(valid_slot.layer_id, valid_slot.tag, valid_gpu_block), 'x');
    setBlockInfosContent(coordinator_manager->convertIndexToBuffer(slots[state_slots[1]].layer_id,
                                                                   slots[state_slots[1]].tag,
                                                                   /*block_id=*/0),
                         'z');
    ...
    verifyBlockInfosContent(
        coordinator_manager->convertIndexToBuffer(valid_slot.layer_id, valid_slot.tag, valid_gpu_block), 'A');
    verifyBlockInfosContent(
        coordinator_manager->convertIndexToBuffer(slots[state_slots[1]].layer_id, slots[state_slots[1]].tag, 0), 'z');
}
...
TEST(KVCacheBatchedMemoryCopyTest, PrefixTreeD2HMergeSourceKeepsOldSlotsAndOverlaysNewSlots) {
    ...
    setBlockInfosContent(coordinator_manager->convertIndexToBuffer(new_slot.layer_id, new_slot.tag, new_gpu_block),
                         'N');
