...
TEST(KVCacheBatchedMemoryCopyTest, EndpointCopyPlanMapsPhysicalOrdinalToCompactLocalIndex) {
    auto config = makeCompactDsv4TypedMemoryCopyConfig(/*use_flash=*/true);
    ...
    auto                     connector    = std::make_shared<KVCacheMemoryConnector>(
        config, kv_config, parallelism_config, std::shared_ptr<KVCacheAllocator>(), server_addrs);
    ASSERT_TRUE(connector->init());

    KVCacheResource resource;
    initResourceGroupsForConfig(resource, config);
    std::vector<int32_t> tokens(768, 1);
    resource.requestPrefix().rebuild(tokens.data(), tokens.size());
    resource.resizeBlocks(/*reserver_blocks=*/3, NULL_BLOCK_IDX);

    CacheKeyType next_key = 1000;
    for (const auto& group : config.topology().groups()) {
        if (!group.policy.enable_prefix_reuse) {
            continue;
        }
        if (group.policy.group_type == CacheGroupType::FULL) {
            resource.cacheKeys(group.tag)         = {next_key++, group.tag == "csa_kv" ? 902 : next_key++};
            resource.blockDependencies(group.tag) = {rootDep(/*ordinal=*/0), rootDep(/*ordinal=*/2)};
            resource.mutableBlockIds(group.tag).assign({10, 11});
        } else {
            resource.cacheKeys(group.tag)         = {next_key++, next_key++, next_key++};
            resource.blockDependencies(group.tag) = {
                rootDep(/*ordinal=*/0), rootDep(/*ordinal=*/1), rootDep(/*ordinal=*/2)};
            resource.mutableBlockIds(group.tag).assign({20, 21, 22});
        }
    }
    ...
    const auto item = std::find_if(plan->copy_infos.begin(), plan->copy_infos.end(), [](const auto& copy_info) {
        return copy_info.cache_key == 902;
    });
    ASSERT_NE(item, plan->copy_infos.end());
    EXPECT_EQ(item->dependency.ordinal, 2u);
    EXPECT_NE(std::find(item->gpu_blocks.begin(), item->gpu_blocks.end(), 11), item->gpu_blocks.end());
}
