...
public:
    MockKVCacheMemoryConnector(const CacheConfig&                              cache_config,
        ...
        KVCacheMemoryConnector(
            cache_config, kv_cache_config, parallelism_config, coordinator_manager, worker_addrs, metrics_reporter) {}
    MockKVCacheMemoryConnector(const CacheConfig&                              cache_config,
                               const KVCacheConfig&                            kv_cache_config,
                               const std::shared_ptr<CoordinatorCacheManager>& coordinator_manager,
                               const std::vector<std::string>&                 worker_addrs,
                               const kmonitor::MetricsReporterPtr&             metrics_reporter):
        KVCacheMemoryConnector(cache_config, kv_cache_config, coordinator_manager, worker_addrs, metrics_reporter) {}
