...
TEST(KVCacheCrcMetricsTest, OnlyRankZeroReportsClassifiedCopyErrorsWithOrdinaryTags) {
    ...
    autil::EnvGuard hippo_role("HIPPO_ROLE", "crc-role");
    autil::EnvGuard hippo_app("HIPPO_APP", "crc-app");
    autil::EnvGuard hippo_group("HIPPO_SERVICE_NAME", "crc-group");
    autil::EnvGuard host_ip("HIPPO_SLAVE_IP", "192.0.2.1");
    autil::EnvGuard container_ip("RequestedIP", "192.0.2.2");
    autil::EnvGuard business_tags("kmonitorTags", "biz^crc-biz");
    auto            expect_common_tags = [](const kmonitor::MetricsTags& tags) {
        EXPECT_EQ(tags.FindTag("hippo_slave_ip"), "192.0.2.1");
        EXPECT_EQ(tags.FindTag("host_ip"), "192.0.2.1");
        EXPECT_EQ(tags.FindTag("container_ip"), "192.0.2.2");
        EXPECT_EQ(tags.FindTag("hippo_role"), "crc-role");
        EXPECT_EQ(tags.FindTag("hippo_app"), "crc-app");
        EXPECT_EQ(tags.FindTag("hippo_group"), "crc-group");
        EXPECT_EQ(tags.FindTag("biz"), "crc-biz");
    };
    ...
    parallel.world_rank               = 15;
    parallel.tp_rank                  = 1;
    parallel.dp_rank                  = 7;
    parallel.tp_size                  = 2;
    parallel.dp_size                  = 8;
    parallel.world_size               = 16;
    ...
    connector->reportCopyError(MemoryOperationResponsePB::CRC_MISMATCH, KVCacheMemoryConnector::CopyDirection::H2D);
    ...
    tags.AddTag("model", "crc-test-model");
    auto ordinary = std::make_shared<kmonitor::MetricsReporter>("", "", tags);
    // Even a supplied reporter must not cause a nonzero rank to report this metric.
    connector->metrics_reporter_ = ordinary;
    connector->reportCopyError(MemoryOperationResponsePB::CRC_MISMATCH, KVCacheMemoryConnector::CopyDirection::H2D);
    kmonitor::MetricsCollector worker_snapshot;
    ordinary->_monitor->GetMetrics(
        &worker_snapshot, {kmonitor::NORMAL}, autil::TimeUtility::currentTimeInMilliSeconds());
    EXPECT_TRUE(worker_snapshot.GetRecords().getRecords().empty());
    connector->metrics_reporter_.reset();

    parallel.world_rank = 14;
    parallel.tp_rank    = 0;
    ...
    for (auto direction : {KVCacheMemoryConnector::CopyDirection::H2D, KVCacheMemoryConnector::CopyDirection::D2H}) {
        for (auto error : {Response::NONE,
                           Response::RPC_FAILED,
                           Response::COPY_FAILED,
                           Response::INVALID_REQUEST,
                           Response::IO_FAILED,
                           Response::CRC_COMPUTE_FAILED,
                           Response::CRC_MISMATCH}) {
            leader.reportCopyError(error, direction);
        }
    }
    kmonitor::MetricsCollector snapshot;
    ordinary->_monitor->GetMetrics(&snapshot, {kmonitor::NORMAL}, autil::TimeUtility::currentTimeInMilliSeconds());
    int error_series = 0;
    int crc_series   = 0;
    for (const auto* record : snapshot.GetRecords().getRecords()) {
        for (const auto* value : record->Values()) {
            if (value->Name().find("rtp_llm_kv_cache_memory_cache_copy_error_qps") == std::string::npos)
                continue;
            expect_common_tags(*record->Tags());
            EXPECT_EQ(record->Tags()->FindTag("dp_rank"), "7");
            EXPECT_EQ(record->Tags()->FindTag("model"), "crc-test-model");
            const auto direction = record->Tags()->FindTag("copy_direction");
            EXPECT_TRUE(direction == "TO_GPU" || direction == "FROM_GPU");
            const auto error = record->Tags()->FindTag("error_type");
            EXPECT_NE(error, "NONE");
            EXPECT_GT(std::stod(value->Value()), 0);
            ++error_series;
            crc_series += error == "CRC_MISMATCH";
        }
    }
    EXPECT_EQ(error_series, 12);
    EXPECT_EQ(crc_series, 2);
    EXPECT_TRUE(ordinary->getTags().FindTag("copy_direction").empty());
    EXPECT_TRUE(ordinary->getTags().FindTag("error_type").empty());
    ...
    for (auto direction : {MemoryOperationRequestPB::H2D, MemoryOperationRequestPB::D2H}) {
        ...
        EXPECT_EQ(response.error_code(), Response::COPY_FAILED);
    }
...
protected:
    void SetUp() override {
        ...
        request_.add_copy_items()->set_mem_block(17);
    }
    ...
    std::vector<uint8_t> block(const std::vector<uint8_t>& payload) {
...
TEST_F(KVCacheCrcDiagnosticsTest, DumpCompletesBeforeBackingReuse) {
...
TEST_F(KVCacheCrcDiagnosticsTest, StoredCrcFailureKeepsBlockLocationAndLayout) {
