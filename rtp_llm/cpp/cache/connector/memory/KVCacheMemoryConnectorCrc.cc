...
void KVCacheMemoryConnector::initCrcCopy() {
    ...
    if (usePrefixTreeMemoryCache())
        sizes = {compressed_block_size_, state_swa_block_size_};
    else if (isDualPool()) {
    ...
    crc_dump_pool_ = std::make_shared<autil::LockFreeThreadPool>(1, 2, nullptr, "CacheCrcDump");
    RTP_LLM_CHECK_WITH_INFO(crc_dump_pool_->start(), "start cache CRC dump worker failed");
    initCrcMetrics();
...
void KVCacheMemoryConnector::initCrcMetrics() {
    crc_metrics_reporter_ = metrics_reporter_;
    if (crc_metrics_reporter_)
        return;

    // The engine only supplies its ordinary reporter on TP rank 0. Every CRC
    // worker rank needs its own reporter without enabling ordinary cache metrics.
    static std::mutex           init_mutex;
    std::lock_guard<std::mutex> lock(init_mutex);
    if (!kmonitor::KMonitorFactory::IsStarted())
        RTP_LLM_CHECK_WITH_INFO(initKmonitorFactory(), "initialize rank-local cache CRC metrics failed");
    kmonitor::MetricsTags tags;
    tags.AddTag("dp_rank", std::to_string(parallelism_config_.dp_rank));
    crc_metrics_reporter_ = std::make_shared<kmonitor::MetricsReporter>("", "", tags);
}
...
bool KVCacheMemoryConnector::reserveCrcDump() {
    ...
    if (crc_dump_pending_ >= 2 || crc_dump_count_ >= 2)
        return false;
    ++crc_dump_pending_;
    ++crc_dump_count_;
    return true;
...
void KVCacheMemoryConnector::invalidateCrcPlan(const CopyPlan& plan, bool sources) {
    for (const auto& info : plan.copy_infos) {
        if (sources && !info.src_generation)
            continue;
        if (info.kind == CacheBlockKind::COMPRESSED_KV || info.kind == CacheBlockKind::STATE_SWA_KV) {
            auto retired = prefix_block_cache_->detachIfMatch(info.cache_key,
                                                              info.kind,
                                                              sources ? info.src_backing_type : info.backing_type,
                                                              sources ? info.src_mem_block : info.mem_block,
                                                              sources ? info.src_disk_slot : info.disk_slot,
                                                              sources ? info.src_generation : info.generation);
            if (retired)
                releasePrefixCacheBacking(*retired);
        } else if (!sources) {
            auto removed = block_cache_->removeIfMatch(
                info.cache_key, info.backing_type, info.mem_block, info.disk_slot, info.crc_epoch);
            if (removed)
...
bool KVCacheMemoryConnector::copyCacheWithCrc(const MemoryOperationRequestPB&     request,
                                              const std::vector<LayerRegionSlot>& slots) {
    const bool to_device = request.copy_direction() == MemoryOperationRequestPB::H2D;
    auto       reject    = [&](const char* message, bool metadata = true) {
        RTP_LLM_LOG_ERROR("memory cache CRC rejected: rank=%ld direction=%d metadata=%d items=%d trace=%s error=%s",
                          ...
                          message);
        reportCrcMetrics(to_device, 0, 0, 1, metadata ? 1 : 0);
        return false;
    };
    ...
    size_t slot_index = 0;
    {
        ...
        if (stop_.load()) {
            return reject("CRC connector stopping", false);
        }
        if (crc_copy_slots_.empty())
            return reject("CRC workspace not initialized");
    ...
    for (int item_index = 0; item_index < request.copy_items_size(); ++item_index) {
        ...
        if (!host || (has_source && !inherited)) {
            return reject("CRC backing read/allocation failed", false);
        }
        ...
        const int64_t      start = currentTimeUs();
        ...
        try {
            if (to_device) {
                ...
                if (!inherited || result.success) {
                    ...
                    result = workspace->store(host, bytes, request.request_id(), host_pinned, capture_failure);
                }
            }
        } catch (const std::invalid_argument& error) {
            return reject(error.what());
        }
        reportCrcMetrics(to_device, bytes, currentTimeUs() - start, !result.success, 0);
        if (!result.success) {
            CrcBlockHostMetadata provenance;
            const auto*          source = static_cast<const unsigned char*>(inherited ? inherited : host);
            std::memcpy(&provenance, source + CrcBlockCopy::transferBytes(bytes), sizeof(provenance));
            RTP_LLM_LOG_ERROR(
                "memory cache CRC rejected: key=%lu kind=%s rank=%ld "
                "expected=%u actual=%u gpu_status=%u reader_request_id=%ld writer_request_id=%ld trace=%s",
                item.cache_key(),
            ...
            if (dump_reserved) {
                try {
                    ...
                    RTP_LLM_LOG_ERROR(
                        "CRC dump preparation failed: rank=%ld error=%s", parallelism_config_.world_rank, error.what());
                    reportCrcMetrics(to_device, 0, 0, 0, 0, 0, 1);
                }
            } else {
                RTP_LLM_LOG_WARNING("CRC dump dropped: rank=%ld bytes=%zu reason=%s",
                                    parallelism_config_.world_rank,
                                    bytes,
                                    fullCrcDumpFits(bytes) ? "rate_or_pending_limit" : "full_block_exceeds_quota");
                reportCrcMetrics(to_device, 0, 0, 0, 0, 0, 1);
            }
            return false;
        }
        if (!to_device && item.backing_type() == MemoryOperationRequestPB::DISK
            ...
            return reject("CRC disk write failed", false);
        }
    }
    return true;
}
...
void KVCacheMemoryConnector::dumpCrcFailure(const MemoryOperationRequestPB& request,
    ...
    // CopyPlan still owns the source reference here (disk buffers are local to
    // copyCacheWithCrc). The worker must never retain a raw backing pointer.
    ...
    CrcBlockHostMetadata provenance;
    std::memcpy(&provenance, cpu.data() + CrcBlockCopy::transferBytes(bytes), sizeof(provenance));
    ...
    std::map<std::string, std::string> manifest{
        ...
        {"cache_key", std::to_string(item.cache_key())},
        {"reader_request_id", std::to_string(request.request_id())},
        {"writer_request_id", std::to_string(provenance.writer_request_id)},
    ...
    if (!cpu_output.empty()) {
        ...
        CrcBlockHostMetadata output_provenance;
        std::memcpy(
            &output_provenance, cpu_output.data() + CrcBlockCopy::transferBytes(bytes), sizeof(output_provenance));
        manifest["output_writer_request_id"] = std::to_string(output_provenance.writer_request_id);
    }
    ...
    const auto sequence    = crc_dump_sequence_.fetch_add(1);
    ...
    const auto code        = crc_dump_pool_->pushTask([this,
                                                path,
                                                prefix,
                                                name,
                                                to_device,
                                                ...
                                                source_gpu,
                                                cpu        = std::move(cpu),
                                                cpu_output = std::move(cpu_output),
                                                result     = std::move(result),
                                                manifest   = std::move(manifest)]() mutable {
        bool written = false;
        try {
            // CPU reference hashing only sees owned, immutable snapshots, on the
            // rate-limited failure worker. No CPU hash is added to successful copies.
            ...
            RTP_LLM_LOG_ERROR("CRC diagnostic: rank=%s direction=%s key=%s reader_request_id=%s writer_request_id=%s "
                                     "stage=%s diagnosis=%s checked_crc=%s cpu_crc=%u gpu_crc=%s gpu_status=%s "
                                     "different_bytes=%s cpu_role=%s gpu_role=%s trace=%s "
                                     "temporal_cause=undetermined dump=%s/%s",
                              manifest["rank"].c_str(),
                              manifest["direction"].c_str(),
                              manifest["cache_key"].c_str(),
                              manifest["reader_request_id"].c_str(),
                              manifest["writer_request_id"].c_str(),
            ...
            RTP_LLM_LOG_ERROR("CRC dump failed: rank=%ld error=%s", parallelism_config_.world_rank, error.what());
        }
        {
            std::lock_guard<std::mutex> lock(crc_mutex_);
            --crc_dump_pending_;
        }
        reportCrcMetrics(to_device, 0, 0, 0, 0, written ? 1 : 0, written ? 0 : 1);
    });
...
void KVCacheMemoryConnector::reportCrcMetrics(bool    to_device,
                                              int64_t bytes,
                                              int64_t latency_us,
                                              int64_t failed,
                                              int64_t metadata_failed,
                                              int64_t dump_written,
                                              int64_t dump_dropped) {
    if (!crc_metrics_reporter_)
        return;
    try {
        RtpLLMMemoryCacheCrcMetricsCollector collector;
        collector.to_device       = to_device;
        collector.bytes           = bytes;
        collector.latency_us      = latency_us;
        collector.failed          = failed;
        collector.metadata_failed = metadata_failed;
        collector.dump_written    = dump_written;
        collector.dump_dropped    = dump_dropped;
        kmonitor::MetricsTags tags("rank", std::to_string(parallelism_config_.world_rank));
        crc_metrics_reporter_->report<RtpLLMMemoryCacheMetrics, RtpLLMMemoryCacheCrcMetricsCollector>(&tags,
                                                                                                      &collector);
    } catch (const std::exception& error) {
        RTP_LLM_LOG_WARNING(
            "CRC metric report failed: rank=%ld error=%s", parallelism_config_.world_rank, error.what());
    }
}
