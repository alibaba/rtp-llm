...
void MemoryConnectorAsyncContext::waitDone() {
    ...
    std::call_once(once_flag_, [this]() {
        std::unique_lock<std::mutex> lock(broadcast_result_mutex_);
        broadcast_result_cv_.wait(lock, [this]() { return broadcast_result_ready_; });
...
void MemoryConnectorAsyncContext::setBroadcastResult(
    ...
    broadcast_result_cv_.notify_all();
}
...
std::shared_ptr<AsyncContext> KVCacheMemoryConnector::asyncWrite(const std::shared_ptr<KVCacheResource>& resource,
    ...
    auto context = std::make_shared<MemoryConnectorAsyncContext>(write_done);
...
bool KVCacheMemoryConnector::startCopyAsync(const std::shared_ptr<MemoryConnectorAsyncContext>& context,
                                            const std::vector<CopyInfoPerKey>&                  copy_infos,
                                            CopyDirection                                       direction) {
    if (!context) {
        RTP_LLM_LOG_WARNING("start copy plan async failed, context is null");
        return false;
    }
    if (!wait_done_thread_pool_) {
        RTP_LLM_LOG_WARNING("start copy plan async failed, thread pool is null");
        return false;
    }
...
bool KVCacheMemoryConnector::prepareCopyBuffers(const std::vector<LayerBlock>& gpu_layer_blocks,
    ...
    if (mem_buffer.kv_addr->sizeBytes() < mem_block_size) {
        RTP_LLM_LOG_WARNING("prepare copy buffers failed, mem buffer too small: have=%zu need=%zu",
                            mem_buffer.kv_addr->sizeBytes(),
                            mem_block_size);
        return false;
    }
    auto* mem_base = static_cast<char*>(mem_buffer.kv_addr->data());
    if (!mem_base) {
        RTP_LLM_LOG_WARNING("prepare copy buffers failed, mem buffer base is null, block_idx=%d", mem_block_index);
        return false;
    }

    size_t byte_off           = 0;
    auto   append_copy_buffer = [&](const BufferPtr& gpu_src, size_t bytes) -> bool {
        if (!gpu_src || bytes == 0) {
            return true;
        }
        if (byte_off + bytes > mem_block_size) {
            RTP_LLM_LOG_WARNING("prepare copy buffers failed, mem block overflow: offset=%zu bytes=%zu mem_size=%zu",
                                byte_off,
                                bytes,
                                mem_block_size);
            return false;
        }
        auto mem_slice = std::make_shared<Buffer>(mem_buffer.kv_addr->where(),
                                                  rtp_llm::DataType::TYPE_INT8,
                                                  std::vector<size_t>{bytes},
                                                  static_cast<void*>(mem_base + byte_off));
        if (direction == CopyDirection::H2D) {
            src.push_back(mem_slice);
            dst.push_back(gpu_src);
        } else {
            src.push_back(gpu_src);
    ...
    for (const auto& lb : gpu_layer_blocks) {
        ...
        if (gpu_buffer.kv_addr) {
            if (!append_copy_buffer(gpu_buffer.kv_addr, gpu_buffer.kv_addr->sizeBytes())) {
                return false;
            }
        }
        if (gpu_buffer.kv_scale_addr) {
            if (!append_copy_buffer(gpu_buffer.kv_scale_addr, gpu_buffer.kv_scale_addr->sizeBytes())) {
                return false;
            }
        }
    }
