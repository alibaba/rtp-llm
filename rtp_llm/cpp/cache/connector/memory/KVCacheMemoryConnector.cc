...
KVCacheMemoryConnector::buildEndpointCopyPlanForRead(const KVCacheResource&             resource,
    ...
    for (const auto& group : cache_config_.topology().groups()) {
        ...
        for (const auto ordinal : selection.owned_global_positions) {
            const auto tag_it = ordinal_indices.find(group.tag);
            if (tag_it == ordinal_indices.end()) {
                success = false;
                break;
            }
            const auto ordinal_it = tag_it->second.find(ordinal);
            if (ordinal_it == tag_it->second.end()) {
                success = false;
                break;
            }
            const size_t local_index  = ordinal_it->second;
            const auto&  cache_keys   = resource.cacheKeys(group.tag);
            const auto&  dependencies = resource.blockDependencies(group.tag);
            if (local_index >= cache_keys.size() || local_index >= dependencies.size()) {
                success = false;
                break;
            }
            const auto                cache_key = cache_keys[local_index];
            const auto                kind = full_item ? CacheBlockKind::COMPRESSED_KV : CacheBlockKind::STATE_SWA_KV;
            std::vector<BlockIdxType> gpu_blocks;
            const auto                required_mask = nativeItemSlotValidMask(
                resource, group.tag, local_index, full_item, layer_attn_block_ids, slots, kind, &gpu_blocks);
            if (std::none_of(required_mask.begin(), required_mask.end(), [](uint8_t value) { return value != 0; })) {
                success = false;
                break;
            }
            const auto match = prefix_block_cache_->matchAndMarkInFlight(cache_key, kind, required_mask);
            if (!match.found) {
                success = false;
                break;
            }
            const auto release_match = [&]() {
                const auto retired = prefix_block_cache_->releaseInFlight(
                    cache_key, kind, match.backing_type, match.block_index, match.disk_slot, match.generation);
    ...
    for (size_t endpoint = prefix.matchSpanTokens(); endpoint <= prefix.writeLimitTokens();
        ...
        for (const auto& group : cache_config_.topology().groups()) {
            ...
            for (const auto ordinal : selection.owned_global_positions) {
                ...
                const auto cache_key  = cache_keys[local_index];
                const auto dependency = dependencies[local_index];
                const auto kind       = full_item ? CacheBlockKind::COMPRESSED_KV : CacheBlockKind::STATE_SWA_KV;
                std::vector<BlockIdxType> gpu_blocks;
                const auto                slot_mask = nativeItemSlotValidMask(
                    resource, group.tag, local_index, full_item, layer_attn_block_ids, slots, kind, &gpu_blocks);
                if (std::none_of(slot_mask.begin(), slot_mask.end(), [](uint8_t value) { return value != 0; })) {
                    continue;
                }

                const auto plan_key   = std::make_pair(cache_key, kind);
                const auto planned_it = planned.find(plan_key);
                if (planned_it != planned.end()) {
                    auto& existing = copy_infos[planned_it->second];
                    if (existing.dependency.has_parent != dependency.has_parent
                        || existing.dependency.parent_key != dependency.parent_key
                        || existing.dependency.ordinal != dependency.ordinal) {
                        return nullptr;
                    }
        ...
        for (const auto& group : cache_config_.topology().groups()) {
            ...
            for (const auto ordinal : selection.owned_global_positions) {
                ...
                const auto mask = nativeItemSlotValidMask(
                    resource, group.tag, local_index, full_item, blocks, layerGroupSlots(), kind, nullptr);
                if (std::none_of(mask.begin(), mask.end(), [](uint8_t value) { return value != 0; })
                    || !prefix_block_cache_->contains(cache_keys[local_index], kind, mask)) {
                    complete = false;
                    break;
                }
            }
