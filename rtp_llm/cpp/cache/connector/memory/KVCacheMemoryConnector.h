...
private:
    ...
    using NativeOrdinalLookup = std::unordered_map<size_t, size_t>;
    using NativeOrdinalIndex  = std::unordered_map<std::string, NativeOrdinalLookup>;
    ...
    std::shared_ptr<CopyPlan> buildEndpointCopyPlanForWrite(const KVCacheResource&             resource,
                                                            ...
                                                            const std::vector<LayerGroupSlot>& slots,
                                                            bool&                              no_need_write);
    NativeOrdinalIndex        buildNativeOrdinalIndex(const KVCacheResource& resource) const;
    std::optional<NativeItemProjection> nativeItemForOrdinal(const KVCacheResource&    resource,
