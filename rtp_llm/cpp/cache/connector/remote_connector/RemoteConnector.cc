...
RemoteConnector::RemoteConnector(const CacheConfig&                        cache_config,
    ...
    RemoteConnector::InitParams init_params{
        cache_config, kv_cache_config, runtime_config, parallelism_config, device, register_buffer_addr, register_buffer_size, lora_info_map};
...
remote_connector::ClientWrapper::ConfigMap RemoteConnector::genClientConfig() {
    ...
    const auto& dtype_str = getDataTypeStr(init_params_->cache_config.cache_specs[0]->dtype); // TODO(zhoushipei.zsp) is this right?  
...
void RemoteConnector::asyncMatchTask(const std::shared_ptr<KVCacheResourceV1>&       resource,
    ...
    async_context->set_matched_block_count(async_context->locations_ptr()->size() + async_context->prev_reuse_blocks_num()); // gpu + remote
    async_context->setState(RemoteConnectorAsyncContext::State::RCS_SUCCESS);
}
...
void RemoteConnector::asyncReadTask(const std::shared_ptr<KVCacheResourceV1>&           resource,
    ...
    auto broadcast_result =
        broadcaster_->broadcast<BroadcastTpRequestPB, BroadcastTpResponsePB>(requests, get_broadcast_timeout_, rpc_call);
...
void RemoteConnector::asyncWriteTask(const std::shared_ptr<KVCacheResourceV1>&           resource,
    ...
    auto    broadcast_result =
        broadcaster_->broadcast<BroadcastTpRequestPB, BroadcastTpResponsePB>(requests, put_broadcast_timeout_, rpc_call);
    broadcast_result->waitDone();
