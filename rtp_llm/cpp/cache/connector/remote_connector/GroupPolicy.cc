...
bool DefaultLayerGroupPolicy::isRingBufferGroup(int32_t group_id) const {
    ...
    // For non-DSV4 models, group_to_region_name_ is empty, so we return false.
    if (group_to_region_name_.empty()) {
        return false;
    }
    // If the group has an region_name registered, it's a DSV4 model.
