...
bool DefaultLayerGroupPolicy::init() {
    ...
    if (!l2g.empty()) {
        ...
        // Build group_id -> attn_type mapping for DSV4 multi-pool support
        const auto& group_attn_types = layer_layout.group_attn_types;
        for (const auto& entry : groups_) {
            int32_t gid = entry.first;
            if (gid < static_cast<int32_t>(group_attn_types.size())) {
                group_to_attn_type_[gid] = group_attn_types[static_cast<size_t>(gid)];
            } else {
                group_to_attn_type_[gid] = KVCacheAttnType::DEFAULT;
            }
        }
        return true;
    }
...
bool DefaultLayerGroupPolicy::genBlockBuffers(const std::vector<int32_t>&     group_ids,
    ...
    for (size_t i = 0; i < block_ids.size(); ++i) {
        ...
        // DSV4: use attn_type-aware convertIndexToBuffer to route to correct pool
        auto            it        = group_to_attn_type_.find(group_ids[i]);
        KVCacheAttnType attn_type = (it != group_to_attn_type_.end()) ? it->second : KVCacheAttnType::DEFAULT;
        for (size_t j = 0; j < layer_ids.size(); ++j) {
            // if support scale, block_infos: {kv_info, scale_info}
            const auto& block_infos = allocator_->convertIndexToBuffer(layer_ids[j], attn_type, block_ids[i]);
...
bool DefaultLayerGroupPolicy::isRingBufferGroup(int32_t group_id) const {
    // DSV4 uses 7 pools: 0-2 are FULL (CSA_KV, HCA_KV, INDEXER_KV),
    // 3-6 are LINEAR (INDEXER_STATE, CSA_STATE, HCA_STATE, SWA_KV).
    // This matches DSV4_REMOTE_FULL_POOL_NUM = 3.
    // For non-DSV4 models, group_to_attn_type_ is empty, so we return false.
    if (group_to_attn_type_.empty()) {
        return false;
    }
    // If the group has an attn_type registered, it's a DSV4 model.
    // LINEAR groups are those with group_id >= 3.
    return group_id >= DSV4_REMOTE_FULL_POOL_NUM;
}
