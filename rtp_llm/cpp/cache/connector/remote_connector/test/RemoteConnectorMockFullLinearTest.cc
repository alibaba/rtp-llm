...
//     int start_read_block_index = gpu_reuse_num;
//     int read_block_num = matched_num - gpu_reuse_num;
//     auto read_context = remote_connectors_[tp_rank]->asyncRead(kv_cache_resouce, meta, match_context,
//     start_read_block_index, read_block_num); ASSERT_FALSE(read_context->success()); auto context =
//     std::dynamic_pointer_cast<RemoteConnectorAsyncContext>(read_context); ASSERT_NE(nullptr, context);
//     ASSERT_EQ(RemoteConnectorAsyncContext::State::RCS_ERROR, context->state());
// }
