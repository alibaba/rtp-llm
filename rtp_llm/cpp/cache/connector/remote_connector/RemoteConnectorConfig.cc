...
void SdkWrapperConfig::Jsonize(Jsonizable::JsonWrapper& json) {
    if (json.GetMode() == FastJsonizableBase::Mode::TO_JSON) {
        ...
        parsed_backend_configs.reserve(sdk_backend_configs.size());
        for (const auto& config_any : sdk_backend_configs) {
            const auto* config_json_map = AnyCast<json::JsonMap>(&config_any);
            if (config_json_map == nullptr) {
                AUTIL_LEGACY_THROW(NotJsonizableException, "sdk_backend_configs element must be a JSON object");
            }
            const auto type_it = config_json_map->find("type");
            if (type_it == config_json_map->end()) {
                AUTIL_LEGACY_THROW(NotJsonizableException, "sdk_backend config is missing string field 'type'");
            }
            const auto* type_str = AnyCast<std::string>(&type_it->second);
            if (type_str == nullptr) {
                AUTIL_LEGACY_THROW(NotJsonizableException, "sdk_backend config field 'type' must be a string");
            }
            ...
            parsed_backend_configs.push_back(std::move(sdk_backend_config));
        }
