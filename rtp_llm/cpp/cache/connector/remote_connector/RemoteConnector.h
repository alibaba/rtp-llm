...
private:
    ...
    std::unique_ptr<CPSlotMapper>                  cp_slot_mapper_;
    std::string                                    remote_group_tag_;
    size_t                                         logical_key_span_ = 1;
...
public:
    explicit RemoteAsyncMatchContext(size_t prev_reuse_blocks_num, size_t logical_span):
        prev_reuse_blocks_num_(prev_reuse_blocks_num), logical_span_(logical_span) {}
    ...
    size_t matchedTokenCount() const override {
        return matched_block_count_ * logical_span_;
    }
    size_t logicalKeySpan() const {
