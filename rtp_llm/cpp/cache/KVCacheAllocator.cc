...
uint32_t KVCacheAllocator::convertToGlobalLayerId(size_t model_id, int local_layer_id) const {
    ...
    const int expected_global_layer_id = static_cast<int>(config_.layer_num)
                                         + static_cast<int>(model_id - 1) * static_cast<int>(sub->layer_num)
                                         + local_layer_id;
    for (int gid = 0; gid < sub->groupNums(); ++gid) {
        const auto& group_layers = sub->layerIdsForGroup(static_cast<size_t>(gid));
        if (std::find(group_layers.begin(), group_layers.end(), expected_global_layer_id) != group_layers.end()) {
            return static_cast<uint32_t>(expected_global_layer_id);
        }
    }
    RTP_LLM_LOG_ERROR("convertToGlobalLayerId: local_layer_id=%d is not mapped in mtp_sub_configs[%zu]",
                      local_layer_id,
                      model_id - 1);
    return std::numeric_limits<uint32_t>::max();
}
