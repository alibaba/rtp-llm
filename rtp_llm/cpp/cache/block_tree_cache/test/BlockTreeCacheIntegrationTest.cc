...
TEST_F(BlockTreeCacheIntegrationTest, CacheShutdownWaitsForCommittedLoadBackSettlement) {
    ...
    for (CopyStatus copy_result : {CopyStatus::OK, CopyStatus::DEVICE_IO_ERROR}) {
        ...
        ASSERT_NE(target_block, NULL_BLOCK_IDX);
...
TEST_P(BlockTreeCacheLowerTierTest, FullSWA_MatchLowerTierOnlyReturnsTicketWithoutPublishing) {
    ...
    std::vector<GroupBlockSet> request_targets;
    for (PendingLoadBackItem& item : result.load_back_ticket->items()) {
        ...
        GroupBlockSet request_target{item.group_id, Tier::DEVICE, {item.target_device_blocks}};
        request_target.nodes = {item.node};
        request_targets.push_back(std::move(request_target));
    }
    ...
    environment->reclaimAll();
    environment->cache->releaseMatchedBlocks(request_targets);
    environment->reclaimAll();
    environment->expectFullyReclaimed();
}
...
TEST_F(BlockTreeCacheIntegrationTest, DeviceLoadBackPreflightRejectionImmediatelyRestoresCandidates) {
    ...
    std::vector<std::pair<int, BlockIdxType>> device_sources;
    for (PendingLoadBackItem& item : result.load_back_ticket->items()) {
        if (item.source_tier != Tier::DEVICE) {
            continue;
        }
        ASSERT_EQ(item.device_group_ids.size(), item.source_blocks.size());
        item.node                 = nullptr;
        ...
        for (size_t local_pool_index = 0; local_pool_index < item.source_blocks.size(); ++local_pool_index) {
            device_sources.emplace_back(item.device_group_ids[local_pool_index], item.source_blocks[local_pool_index]);
        }
    }
    ...
    for (const auto& [gid, block] : device_sources) {
        EXPECT_TRUE(environment->device_pools[static_cast<size_t>(gid)]->isAllocated(block));
        EXPECT_EQ(environment->device_pools[static_cast<size_t>(gid)]->refCount(block), 2u);
    }
    environment->expectPayloads();
    ...
    for (const auto& [gid, block] : device_sources) {
        EXPECT_EQ(environment->device_pools[static_cast<size_t>(gid)]->refCount(block), 1u);
    }
    environment->expectPayloads();
    ...
    for (const auto& [gid, block] : device_sources) {
        EXPECT_FALSE(environment->device_pools[static_cast<size_t>(gid)]->isAllocated(block));
    }
...
TEST_F(BlockTreeCacheIntegrationTest, DeviceLoadBackAsyncCompletionRefreshesBeforeTerminalPublication) {
    ...
    std::vector<std::pair<int, BlockIdxType>> device_sources;
    std::vector<GroupBlockSet>                request_target_blocks;
    for (PendingLoadBackItem& item : result.load_back_ticket->items()) {
        item.target_device_blocks.clear();
        if (item.source_tier == Tier::DEVICE) {
            ASSERT_EQ(item.device_group_ids.size(), item.source_blocks.size());
            ...
            item.target_device_blocks = item.source_blocks;
            for (size_t local_pool_index = 0; local_pool_index < item.source_blocks.size(); ++local_pool_index) {
                device_sources.emplace_back(item.device_group_ids[local_pool_index],
                                            item.source_blocks[local_pool_index]);
            }
            continue;
        }
        ...
        ASSERT_EQ(item.target_device_blocks.size(), item.device_group_ids.size());
        ASSERT_NE(item.node, nullptr);
        GroupBlockSet request_targets{item.group_id, Tier::DEVICE, {item.target_device_blocks}};
        request_targets.nodes = {item.node};
        request_target_blocks.push_back(std::move(request_targets));
    }
    ASSERT_EQ(device_sources.size(), 4u);
    ...
    context->waitDone();
    EXPECT_TRUE(context->success());
    environment->cache->releaseMatchedBlocks(request_target_blocks);
    environment->reclaimAll();
    environment->expectFullyReclaimed();
}
