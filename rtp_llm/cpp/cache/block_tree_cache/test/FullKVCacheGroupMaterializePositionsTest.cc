...
class FullKVCacheGroupMaterializePositionsTest: public ::testing::Test {
protected:
    void SetUp() override {}
...
TEST_F(FullKVCacheGroupMaterializePositionsTest, MaterializePositionsDeduplicatesMissingSlotsAndOnlyTakesUnifiedRefs) {
...
TEST_F(FullKVCacheGroupMaterializePositionsTest, MaterializePositionsRejectsOutOfRangeWithoutMutation) {
    auto block_pool = createDeviceBlockPool();
    ASSERT_TRUE(block_pool->init());
...
TEST_F(FullKVCacheGroupMaterializePositionsTest, MaterializePositionsRollsBackWhenPoolCannotSatisfyWholeRequest) {
    auto block_pool = createDeviceBlockPool();
    ASSERT_TRUE(block_pool->init());
    const size_t initial_free = block_pool->freeBlocksNum();
