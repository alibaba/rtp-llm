...
bool MultiRankBlockTransferEngine::execute(const std::vector<TransferDescriptor>& descriptors,
    ...
    if (broadcast_manager_ == nullptr) {
        RTP_LLM_LOG_WARNING("MultiRankBlockTransferEngine: broadcast manager is not initialized");
        return false;
    }
    if (descriptors.empty() || timeout_ms <= 0) {
        RTP_LLM_LOG_WARNING("MultiRankBlockTransferEngine: invalid batch, item_count=%zu, timeout_ms=%d",
                            descriptors.size(),
                            timeout_ms);
        return false;
    }
    ...
    for (const TransferDescriptor& descriptor : descriptors) {
        if (!BlockTransferRequestConverter::appendTransfer(descriptor, component_groups_, request)) {
            RTP_LLM_LOG_WARNING("MultiRankBlockTransferEngine: failed to encode transfer, "
    ...
    if (worker_count == 0) {
        RTP_LLM_LOG_WARNING("MultiRankBlockTransferEngine: no worker configured");
        return false;
    }
    ...
    if (memory_request == nullptr) {
        RTP_LLM_LOG_WARNING("MultiRankBlockTransferEngine: failed to create memory request");
        return false;
    }
    ...
    if (broadcast_result == nullptr) {
        RTP_LLM_LOG_WARNING("MultiRankBlockTransferEngine: failed to start broadcast");
        return false;
    }

    broadcast_result->waitDone();
    if (!broadcast_result->success()) {
        RTP_LLM_LOG_WARNING("MultiRankBlockTransferEngine: worker RPC failed");
        return false;
    }

    const std::vector<FunctionResponsePB> responses = broadcast_result->responses();
    if (responses.size() != worker_count) {
        RTP_LLM_LOG_WARNING("MultiRankBlockTransferEngine: response count mismatch, expected=%zu, actual=%zu",
                            worker_count,
                            responses.size());
        return false;
    }
    for (size_t rank = 0; rank < responses.size(); ++rank) {
        ...
        if (!response.has_mem_response() || !response.mem_response().success()) {
            RTP_LLM_LOG_WARNING("MultiRankBlockTransferEngine: worker transfer failed, rank=%zu", rank);
            return false;
        }
    }
    return true;
