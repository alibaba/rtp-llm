...
BlockTreePosixDiskBlockIO::~BlockTreePosixDiskBlockIO() {
    close();
}

DiskBlockIOStatus
BlockTreePosixDiskBlockIO::openAndPreallocate(const std::string& file_path, size_t bytes, bool buffered_io) {
    close();
    ...
    if (rc == EOPNOTSUPP || rc == ENOSYS) {
        RTP_LLM_LOG_WARNING("posix_fallocate unsupported, falling back to ftruncate, file=%s, bytes=%zu, error=%s",
                            file_path.c_str(),
                            bytes,
                            std::strerror(rc));
        if (::ftruncate(fd_, static_cast<off_t>(bytes)) != 0) {
            RTP_LLM_LOG_ERROR("ftruncate disk block file failed, file=%s, bytes=%zu, error=%s",
                              file_path.c_str(),
                              bytes,
                              std::strerror(errno));
            close();
...
DiskBlockIOStatus BlockTreePosixDiskBlockIO::validate(uint64_t offset, const void* buffer, size_t bytes) const {
...
DiskBlockIOStatus BlockTreePosixDiskBlockIO::read(uint64_t offset, void* dst, size_t bytes) {
...
DiskBlockIOStatus BlockTreePosixDiskBlockIO::write(uint64_t offset, const void* src, size_t bytes) {
...
DiskBlockIOStatus BlockTreePosixDiskBlockIO::read(const std::vector<DiskRead>& reads) {
...
DiskBlockIOStatus BlockTreePosixDiskBlockIO::write(const std::vector<DiskWrite>& writes) {
...
void BlockTreePosixDiskBlockIO::close() {
    if (fd_ >= 0) {
        ::close(fd_);
        fd_ = -1;
    }
...
std::string BlockTreePosixDiskBlockIO::debugString() const {
    std::ostringstream oss;
    oss << "BlockTreePosixDiskBlockIO{file=" << file_path_ << ", bytes=" << bytes_
