...
TEST(GroupSetTest, RejectsNullPoolAndInvalidLogicalPayload) {
    ...
    auto zero_payload_spec                  = makeGroupBase({0});
    ...
    auto overflow_spec                  = makeGroupBase({0});
...
TEST(GroupSetTest, RejectsNonReusableMembership) {
    const auto topology = makeTestTopology({makeGroupBase({0}, false)});
...
TEST(GroupSetTest, RejectsIncompatibleMemberPolicy) {
    auto first               = makeGroupBase({0});
    auto second              = makeGroupBase({0});
...
TEST(GroupSetTest, KeepsTopologyAliveAfterCallerReleasesOwnership) {
    ...
    EXPECT_EQ(&group->groupAt(0), &group->topology()->groupById(0));
}
