...
size_t SWAGroupSet::computeReuseBlockCount(size_t matched_block_count, const std::vector<TreeNode*>& path) const {
    ...
    for (size_t i = matched_block_count; i > 0; --i) {
        ...
        if (group_set_id < node->group_set_resources.size() && node->group_set_resources[group_set_id].isMatchUsable()
            && !node->group_set_resources[group_set_id].is_empty()) {
            count++;
...
bool SWAMatchValidator::validate(const TreeNode* node, const GroupSetResource& resource) {
