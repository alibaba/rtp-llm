...
CopyStatus HostDiskTransferExecutor::hostToDisk(const TransferDescriptor& desc, const ResolvedGroupLayout& layout) const {
    ...
    if (host_pool.payloadBytes() != disk_pool.payloadBytes()) {
        RTP_LLM_LOG_WARNING("host payload bytes %zu != disk payload bytes %zu",
...
CopyStatus HostDiskTransferExecutor::diskToHost(const TransferDescriptor& desc, const ResolvedGroupLayout& layout) const {
    const auto disk_block = desc.disk_block;
    const auto host_block = desc.host_block;
    auto&      host_pool  = *layout.host_pool;
    auto&      disk_pool  = *layout.disk_pool;
