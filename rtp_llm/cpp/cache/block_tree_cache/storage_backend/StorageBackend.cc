    ...
    void finish() {
        std::call_once(finish_once, [this] {
            ...
            for (const Target& target : targets) {
                const auto transitions = target.pool->decRefWithResult({target.block}, BlockRefType::STORAGE_BACKEND);
                for (size_t group_id : target.group_ids) {
                    releases.append(group_id, transitions);
                }
            }
            targets.clear();
...
bool StorageBackend::init(std::shared_ptr<const CacheTopology>       topology,
    ...
    RTP_LLM_CHECK(topology && group_pools.size() == topology->groups().size() && buffer_resolver && release_callback);
    for (const auto& pool : group_pools) {
        RTP_LLM_CHECK(pool != nullptr);
    }
    topology_        = std::move(topology);
    group_pools_     = std::move(group_pools);
...
std::shared_ptr<storage_backend_detail::StorageTaskState> StorageBackend::prepare(StorageRequest request) {
    ...
    std::unordered_map<BlockKey, size_t, BlockKeyHash> target_indexes;
    for (const auto& key_handles : state->request.handles) {
        for (const StorageBlockHandle& handle : key_handles) {
            RTP_LLM_CHECK(handle.group_id < group_pools_.size() && !isNullBlockIdx(handle.block));
            const auto&    pool = group_pools_[handle.group_id];
            const BlockKey key{pool.get(), handle.block};
            auto [it, inserted] = target_indexes.emplace(key, state->targets.size());
            if (inserted) {
                pool->incRef(handle.block, BlockRefType::STORAGE_BACKEND);
                state->targets.push_back({pool, handle.block, {}});
            }
            auto& group_ids = state->targets[it->second].group_ids;
            if (std::find(group_ids.begin(), group_ids.end(), handle.group_id) == group_ids.end()) {
...
StorageBackend::convertIndexToBuffer(int layer_id, int group_id, int block_id) const {
    RTP_LLM_CHECK(buffer_resolver_ && group_id >= 0 && static_cast<size_t>(group_id) < group_pools_.size());
...
void StorageBackend::read(StorageRequest request,
    ...
    readImpl(std::move(prepared_request),
             std::move(match_meta),
             [state = std::move(state), done = std::move(done)] {
                 state->finish();
                 done();
             });
}
