    ...
    if (!pool_snapshot.storage.empty()) {
        ...
        if (should_log) {
            RTP_LLM_LOG_INFO("kvc tier[%s]: total=%zu free=%zu available=%zu capacity_bytes=%zu "
                             ...
                             pool_snapshot.storage.c_str(),
                             pool_snapshot.total_blocks,
                             pool_snapshot.free_blocks,
                             pool_snapshot.available_blocks,
                             pool_snapshot.capacity_bytes,
                             pool_snapshot.occupied_bytes,
                             pool_snapshot.indexer_bytes,
                             pool_snapshot.working_set_bytes);
        }
    }
    ...
    for (int layer_id = 0; layer_id < static_cast<int>(config_.layer_num); ++layer_id) {
        if (static_cast<size_t>(layer_id) < all_layer_tensors.size()) {
            ...
            if (!all_layout.mla_hbm_cache_by_layer.empty()) {
                ...
                if (!all_layout.block_generations_by_layer.empty()) {
                    layout.block_generations_by_layer.push_back(all_layout.block_generations_by_layer[layer_id]);
                }
            }
        } else {
            RTP_LLM_CHECK(false);
        ...
        if (local_layer_id < mtp_sub_config->local_to_global_layer_ids.size()) {
            ...
            if (global_layer_id >= 0 && static_cast<size_t>(global_layer_id) < all_layer_tensors.size()) {
                ...
                if (!all_layout.mla_hbm_cache_by_layer.empty()) {
                    ...
                    if (!all_layout.block_generations_by_layer.empty()) {
                        layout.block_generations_by_layer.push_back(
                            all_layout.block_generations_by_layer[global_layer_id]);
                    }
                }
            } else {
                RTP_LLM_CHECK(false);
