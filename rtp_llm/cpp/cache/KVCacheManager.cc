...
CacheLayerLayout KVCacheManager::getMTPModuleCacheLayerLayout(int mtp_module_id) const {
    ...
    std::vector<int> mtp_global_layer_ids(mtp_layer_num, -1);
    for (size_t gid = 0; gid < static_cast<size_t>(mtp_sub_config->groupNums()); ++gid) {
        for (int global_layer_id : mtp_sub_config->layerIdsForGroup(gid)) {
            ...
            mtp_global_layer_ids[static_cast<size_t>(local_layer_id)] = global_layer_id;
        }
    }
