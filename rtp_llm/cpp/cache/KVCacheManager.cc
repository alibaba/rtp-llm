...
void KVCacheManager::buildCacheLayerLayout(CacheLayerLayout&                 layout,
    ...
    for (size_t layer_id = 0; layer_id < layer_count; ++layer_id) {
        if (layer_id < all_layer_tensors.size() && all_layer_tensors[layer_id].defined()
            && all_layer_tensors[layer_id].numel() > 0) {
            layout.layers_to_buffer_ptrs[layer_id] = torchTensor2Buffer(all_layer_tensors[layer_id]);
        } else {
            layout.layers_to_buffer_ptrs[layer_id] = nullptr;
        }

        if (!all_scale_tensors.empty() && layer_id < all_scale_tensors.size() && all_scale_tensors[layer_id].defined()
            && all_scale_tensors[layer_id].numel() > 0) {
...
void KVCacheManager::buildCacheLayerLayout(CacheLayerLayout&                 layout,
                                           size_t                            layer_count,
                                           const std::vector<torch::Tensor>& all_layer_tensors,
                                           const std::vector<torch::Tensor>& all_scale_tensors,
                                           const std::vector<int>&           global_layer_ids) const {
    layout.layers_to_buffer_ptrs.clear();
    layout.layers_to_buffer_ptrs.resize(layer_count);
    layout.layers_to_scale_buffer_ptrs.clear();
    layout.layers_to_scale_buffer_ptrs.resize(layer_count);

    for (size_t local_layer_id = 0; local_layer_id < layer_count; ++local_layer_id) {
        int global_layer_id = -1;
        if (local_layer_id < global_layer_ids.size()) {
            global_layer_id = global_layer_ids[local_layer_id];
        }
        ...
        if (global_layer_id >= 0 && static_cast<size_t>(global_layer_id) < all_layer_tensors.size()
            ...
            layout.layers_to_buffer_ptrs[local_layer_id] = torchTensor2Buffer(all_layer_tensors[global_layer_id]);
        } else {
            layout.layers_to_buffer_ptrs[local_layer_id] = nullptr;
        }
        ...
        if (!all_scale_tensors.empty() && global_layer_id >= 0
            ...
            layout.layers_to_scale_buffer_ptrs[local_layer_id] = torchTensor2Buffer(all_scale_tensors[global_layer_id]);
        } else {
...
CacheLayerLayout KVCacheManager::getMainModelCacheLayerLayout() const {
    ...
    buildCacheLayerLayout(layout, config_.layer_num, all_layer_tensors, all_scale_tensors);
...
CacheLayerLayout KVCacheManager::getMTPModuleCacheLayerLayout(int mtp_module_id) const {
    ...
    buildCacheLayerLayout(layout, mtp_layer_num, all_layer_tensors, all_scale_tensors, mtp_global_layer_ids);
