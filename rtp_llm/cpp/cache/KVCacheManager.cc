...
void KVCacheManager::allocateAndSync() {
    ...
    if (world_size > 1) {
        ...
        if (parallelism_config_.ffn_disaggregate_config.is_ffn_service()) {
            ...
            // expected to be identical on every rank but nothing validates that, and the warmup
            // measurement is per-rank (deliberately asymmetric under MoE skew, where the hot
            // rank legitimately sets the cluster minimum). INFO rather than WARNING because
            // with the default skew every non-hot rank is reduced on every startup; the knob
            // values are printed so a suspected drift can be compared across ranks.
