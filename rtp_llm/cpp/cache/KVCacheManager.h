...
public:
    ...
    std::vector<BufferPtr> convertIndexToBuffer(int layer_id, int block_id) const;
    ...
    std::shared_ptr<AsyncContext> asyncLoadCache(const KVCacheResource&                       resource,
    ...
    std::shared_ptr<AsyncContext> asyncStoreCache(const KVCacheResource&                       resource,
                                                  const std::shared_ptr<KVCacheConnectorMeta>& meta,
                                                  const KVCacheConnectorControlParams&         control_params);
