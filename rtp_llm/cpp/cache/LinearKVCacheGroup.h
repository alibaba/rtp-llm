...
public:
    ...
    MatchResult match(const CacheKeysType& cache_keys,
                      int64_t              current_batch_epoch = BlockCache::NO_EPOCH_FILTER) override;
    // Match a single cache key (used by Hybrid allocator to do right-to-left joint matching).
    MatchResult matchSingleKey(CacheKeyType cache_key, int64_t current_batch_epoch = BlockCache::NO_EPOCH_FILTER) const;
