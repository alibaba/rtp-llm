...
struct KVCacheSpec {
    ...
    virtual size_t scale_size_per_block() const { return 0; }  // Default: no scale
    virtual size_t scale_size_bytes_per_block() const { return scale_size_per_block() * sizeof(float); }
    virtual size_t scale_stride_bytes() const { return scale_size_bytes_per_block(); }
...
struct MHAKVCacheSpec: public KVCacheSpec {
    ...
    MHAKVCacheSpec(const AttentionConfigs& attn_config, const ParallelismConfig& parallelism_config) {
        ...
        local_head_num_kv = static_cast<uint32_t>(std::max(1, 
            (attn_config.kv_head_num > 1) ? 
                static_cast<int>(attn_config.kv_head_num / parallelism_config.tp_size) : 
...
struct LinearKVCacheSpec: public KVCacheSpec {
    ...
    LinearKVCacheSpec(const AttentionConfigs& attn_config, const ParallelismConfig& parallelism_config, 
        ...
        RTP_LLM_CHECK_WITH_INFO(
            linear_config.linear_conv_kernel_dim > 1, 
        ...
        RTP_LLM_CHECK_WITH_INFO(
            local_num_k_heads > 0 && local_num_v_heads > 0,
            "invalid local heads for linear attention: k=%d v=%d tp=%d",
            local_num_k_heads,
            local_num_v_heads,
            tp);
        RTP_LLM_CHECK_WITH_INFO(
            linear_config.linear_key_head_dim == linear_config.linear_value_head_dim,
