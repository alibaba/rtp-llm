...
void HybridKVCacheAllocator::referenceBlocksInGroup(int                     group_id,
                                                    const BlockIndicesType& blocks,
                                                    BlockRefType            ref_type) const {
    kv_cache_groups_[static_cast<size_t>(group_id)]->reference(blocks, ref_type);
}
...
const std::vector<size_t>* HybridKVCacheAllocator::groupIdsForGroupSet(size_t group_set_id) const {
    ...
    const auto& group_set = block_tree_cache_->groupSets()[group_set_id];
    return group_set == nullptr ? nullptr : &group_set->groupIds();
}

bool HybridKVCacheAllocator::preflightLoadMappings(const std::shared_ptr<LoadTicket>& ticket) const {
    if (ticket == nullptr || ticket->empty()) {
        return true;
    }
    for (size_t item_index = 0; item_index < ticket->itemCount(); ++item_index) {
        const auto* group_ids = groupIdsForGroupSet(ticket->groupSetId(item_index));
...
int HybridKVCacheAllocator::reuseCache(const CacheKeysType&                 cache_keys,
    ...
    ticket                 = match_result.load_ticket;
    auto fail_match        = [&]() {
        ticket.reset();
    ...
    if (!preflightLoadMappings(ticket)) {
        return fail_match();
    }
    ...
    for (int group_id : full_group_ids_) {
        ...
        BlockIndicesType blocks = groupBlocks(group_id);
        blocks.resize(static_cast<size_t>(logical_reuse_blocks), NULL_BLOCK_IDX);
        kv_resource.mutableBlockIds(0, group_id).assign(std::move(blocks));
    }
    ...
    for (int group_id : linear_group_ids_) {
        ...
        kv_resource.mutableBlockIds(0, group_id)
            .assign(BlockIndicesType(static_cast<size_t>(logical_reuse_len), NULL_BLOCK_IDX));
        const auto& blocks = groupBlocks(group_id);
        if (!blocks.empty() && ready_reuse_len > 0) {
            kv_resource.mutableBlockIds(0, group_id).setAt(static_cast<size_t>(ready_reuse_len - 1), blocks.back());
        }
    }
    for (int group_id : swa_group_ids_) {
        ...
        const int group_reuse_len = cpCompactSwaGroup(group_id, cp_mapper) ? logical_reuse_blocks : logical_reuse_len;
        kv_resource.mutableBlockIds(0, group_id)
        ...
        for (size_t i = 0; i < blocks.size(); ++i) {
            ...
            const size_t target_position    = cpCompactSwaGroup(group_id, cp_mapper) ?
                                                  canonical_position :
                                                  (canonical_position + 1) * static_cast<size_t>(cp_scale) - 1;
            kv_resource.mutableBlockIds(0, group_id).setAt(target_position, blocks[i]);
        }
    }

    if (ticket != nullptr) {
        for (size_t item_index = 0; item_index < ticket->itemCount(); ++item_index) {
            if (ticket->sourceTier(item_index) != Tier::DEVICE && !ticket->joinedLoad(item_index)) {
                continue;
            }
            const auto&             source_blocks = ticket->sourceBlocks(item_index);
            const auto* group_ids = groupIdsForGroupSet(ticket->groupSetId(item_index));
            const BlockIndicesType& reusable_blocks =
                ticket->joinedLoad(item_index) ? ticket->targetDeviceBlocks(item_index) : source_blocks;
    ...
    for (int group_id = 0; group_id < kv_resource.groupNums(); ++group_id) {
        ...
        if (!valid.empty()) {
            referenceBlocksInGroup(group_id, valid, BlockRefType::REQUEST);
...
MallocResult HybridKVCacheAllocator::initMallocForCommonLen(const MallocInfo& malloc_info) {
    ...
    auto                          rollback = [&](const std::vector<size_t>& original_sizes) -> MallocResult {
        load_ticket.reset();
        rollbackInitMalloc(*kv_resource, referenced_blocks, original_sizes, releases);
    ...
    if (malloc_info.enable_device_cache) {
        ...
        reuse_blocks = reuseCache(match_keys, *kv_resource, cp_mapper, load_ticket, referenced_blocks);
    ...
    if (load_ticket != nullptr && !load_ticket->empty()) {
        for (size_t item_index = 0; item_index < load_ticket->itemCount(); ++item_index) {
            if (load_ticket->sourceTier(item_index) == Tier::DEVICE) {
                continue;
            }
            if (load_ticket->joinedLoad(item_index)) {
                continue;
            }
            const auto* group_ids = groupIdsForGroupSet(load_ticket->groupSetId(item_index));
            ...
            for (const size_t raw_group_id : *group_ids) {
                ...
                const size_t target_position =
                    type == CacheGroupType::LINEAR
                            || (type == CacheGroupType::SWA && !cpCompactSwaGroup(group_id, cp_mapper)) ?
                        (load_ticket->pathIndex(item_index) + 1) * static_cast<size_t>(cp_scale) - 1 :
                        load_ticket->pathIndex(item_index);
    ...
    if (load_ticket != nullptr && !load_ticket->empty()) {
        for (size_t item_index = 0; item_index < load_ticket->itemCount(); ++item_index) {
            ...
            if (load_ticket->joinedLoad(item_index)) {
                if (target_device_blocks != load_ticket->targetDeviceBlocks(item_index)) {
                    return rollback(original_sizes);
                }
                continue;
            }
            if ((load_ticket->sourceTier(item_index) == Tier::DEVICE
                ...
                || !load_ticket->bindTargetDeviceBlocks(item_index, std::move(target_device_blocks))) {
                return rollback(original_sizes);
            }
        }
    }
    ...
    MallocResult result{true, reuse_blocks * reuse_unit_tokens, match_cost_time_us, nullptr, load_ticket};
    if (load_ticket != nullptr) {
        result.memory_reuse_len = static_cast<int>(load_ticket->logicalMatchedBlocks(Tier::HOST)) * reuse_unit_tokens;
        result.disk_reuse_len   = static_cast<int>(load_ticket->logicalMatchedBlocks(Tier::DISK)) * reuse_unit_tokens;
    }
    return result;
}
    ...
    for (int b = 0; b <= failed_batch && b < batch_size; ++b) {
        for (int group_id = 0; group_id < kv_resource->groupNums(); ++group_id) {
            ...
            if (!blocks_to_free.empty()) {
                (void)freeBlocksInGroup(group_id, blocks_to_free, BlockRefType::REQUEST);
            }
            for (size_t pos : filled_positions) {
...
std::shared_ptr<KVCacheResource> HybridKVCacheAllocator::incrKVCacheRef(const KVCacheResource& kvcache_resource,
    ...
    for (int group_id = 0; group_id < kvcache_resource.groupNums(); ++group_id) {
        ...
        if (!valid.empty()) {
            referenceBlocksInGroup(group_id, valid, ref_type);
        }
...
void HybridKVCacheAllocator::decrKVCacheRef(const KVCacheResource& kvcache_resource, bool is_connector) {
    ...
    for (int group_id = 0; group_id < kvcache_resource.groupNums(); ++group_id) {
        BlockIndicesType valid;
        for (auto b : kvcache_resource.blocks(group_id)) {
            if (!isNullBlockIdx(b) && b > 0) {
                valid.push_back(b);
            }
        }
        if (!valid.empty()) {
            releases.append(static_cast<size_t>(group_id), freeBlocksInGroup(group_id, valid, ref_type));
        }
    }
    submitBlockReleases(releases);
}
...
bool HybridKVCacheAllocator::updateKVBlock(const BatchKVCacheResourcePtr&  batch_kv_cache_resource,
    ...
    auto rollback_replacements = [&]() {
        for (int group_id = 0; group_id < group_nums; ++group_id) {
            ...
            if (!blocks.empty()) {
                (void)kv_cache_groups_[static_cast<size_t>(group_id)]->release(blocks, BlockRefType::REQUEST);
                blocks.clear();
            }
        }
    };
    ...
    for (int old_batch_idx = 0; old_batch_idx < old_batch_size; ++old_batch_idx) {
        ...
        for (int group_id = 0; group_id < group_nums; ++group_id) {
            ...
            if (!to_free.empty()) {
                releases.append(
                    static_cast<size_t>(group_id),
                    kv_cache_groups_[static_cast<size_t>(group_id)]->release(to_free, BlockRefType::REQUEST));
            }
        }
    }
...
void HybridKVCacheAllocator::rollbackInitMalloc(BatchKVCacheResource&                kv_resource,
    ...
    for (int group_id = 0; group_id < kv_resource.groupNums(); ++group_id) {
        auto& block_ids = kv_resource.mutableBlockIds(0, group_id);
        if (!original_sizes.empty() && static_cast<size_t>(group_id) < original_sizes.size()
            && block_ids.blocksNum() > original_sizes[static_cast<size_t>(group_id)]) {
            rollbackBlockIdsToSize(group_id, block_ids, original_sizes[static_cast<size_t>(group_id)]);
        }
