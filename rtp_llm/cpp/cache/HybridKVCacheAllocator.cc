...
bool HybridLayerKVCacheAllocator::init() {
    ...
    // NOTE: current BlockPoolConfigHelper only supports a single spec (plus MTP sub models).
    // For hybrid attention models in this repo, we assume all groups share the same underlying BlockPool layout,
    // and group-specific semantics are handled by block_map and cache groups.
    // If the upper layer provides multiple specs/groups, extend BlockPoolConfigHelper/BlockPool accordingly.
    ...
    // Build cache groups.
    // - Prefer config_.global_layer_ids (blockpool's "global layer" indexing).
    // - Fall back to config_.layer_ids for backward compatibility.
    const auto& layer_groups = !config_.global_layer_ids.empty() ? config_.global_layer_ids : config_.layer_ids;
    if (layer_groups.empty()) {
        RTP_LLM_LOG_ERROR("CacheConfig has no layer_ids/global_layer_ids");
        return false;
    }
    ...
    for (int gid = 0; gid < group_nums; ++gid) {
        KVCacheSpecPtr spec = config_.cache_specs[std::min(static_cast<size_t>(gid), config_.cache_specs.size() - 1)];
        const auto&    ids  = layer_groups[static_cast<size_t>(gid)];
        if (ids.empty()) {
            RTP_LLM_LOG_WARNING("Hybrid allocator group %d has empty layer ids", gid);
        }
        ...
        for (int layer_id : ids) {
            if (layer_id < 0 || layer_id >= static_cast<int>(config_.layer_num)) {
                RTP_LLM_LOG_ERROR("layer_id out of range in hybrid group mapping: layer_id=%d layer_num=%u",
                                  layer_id,
                                  config_.layer_num);
                return false;
            }
...
int HybridLayerKVCacheAllocator::reuseCache(const CacheKeysType& cache_keys, BatchKVCacheResource& kv_resource) {
    if (cache_keys.empty() || kv_cache_groups_.empty()) {
        return 0;
    }
    ...
    if (!linear_group_ids_.empty() && pos >= 0) {
        for (; pos >= 0; --pos) {
            bool all_linear_matched = true;
            for (size_t i = 0; i < linear_group_ids_.size(); ++i) {
                const int gid    = linear_group_ids_[i];
                auto*     linear = dynamic_cast<LinearKVCacheGroup*>(kv_cache_groups_[static_cast<size_t>(gid)].get());
                if (!linear) {
                    all_linear_matched = false;
                    break;
                }
                auto single = linear->matchSingleKey(cache_keys[static_cast<size_t>(pos)]);
                if (single.block_indices.empty()) {
                    all_linear_matched = false;
                    break;
                }
                linear_tail_blocks[i] = single.block_indices[0];
            }
    ...
    const int reuse_blocks = std::max(pos + 1, 0);
    if (reuse_blocks <= 0) {
        return 0;
    }
    ...
    for (int gid = 0; gid < static_cast<int>(kv_cache_groups_.size()); ++gid) {
        ...
        blocks.clear();
        blocks.resize(static_cast<size_t>(reuse_blocks), NULL_BLOCK_IDX);
    }

    for (int gid : full_group_ids_) {
        auto& blocks = kv_resource.mutableBlocks(0, gid);
        blocks       = full_matched_blocks[static_cast<size_t>(gid)];
        if (static_cast<int>(blocks.size()) > reuse_blocks) {
            blocks.resize(static_cast<size_t>(reuse_blocks));
        }
    }
    ...
    for (size_t i = 0; i < linear_group_ids_.size(); ++i) {
        const int gid                                 = linear_group_ids_[i];
        auto&     blocks                              = kv_resource.mutableBlocks(0, gid);
        blocks[static_cast<size_t>(reuse_blocks - 1)] = linear_tail_blocks[i];
    }
...
MallocResult HybridLayerKVCacheAllocator::incrMalloc(const MallocInfo& malloc_info) {
    ...
    for (int b = 0; b < batch_size; ++b) {
        for (int gid = 0; gid < kv_resource->groupNums(); ++gid) {
            ...
            if (!kv_cache_groups_[static_cast<size_t>(gid)]->malloc(blocks, seq_len)) {
                all_success  = false;
                failed_batch = b;
                failed_group = gid;
                break;
...
MallocResult HybridLayerKVCacheAllocator::initMallocForCommonLen(const MallocInfo& malloc_info) {
    ...
    if (reserve_blocks > 0 && estimated_blocks > 0) {
        ...
        const int    actual_blocks    = std::max(estimated_blocks - reuse_block_cnt, 0);
        if (actual_blocks > 0 && available_blocks < static_cast<size_t>(actual_blocks) + reserve_blocks) {
            if (malloc_info.verbose) {
                RTP_LLM_LOG_INFO("Hybrid initMalloc rejected by reserve blocks: request_id=%ld "
                                 "need_blocks=%d reuse_block_cnt=%d adjusted_need_blocks=%d available_blocks=%zu "
                                 "reserve_blocks=%zu",
...
std::shared_ptr<KVCacheResource> HybridLayerKVCacheAllocator::incrKVCacheRef(KVCacheResource&     kvcache_resource,
    ...
    for (auto key : cache_keys) {
        ...
        for (int gid = 0; gid < group_nums; ++gid) {
            ...
            const auto b = src_blocks[pos];
            if (!isNullBlockIdx(b) && b > 0) {
                selected_blocks[static_cast<size_t>(gid)].push_back(b);
                any_added = true;
            }
        }
        if (any_added) {
    ...
    // Merge and reference all selected blocks (dedup to avoid over-ref).
    std::unordered_set<int> uniq;
    for (const auto& v : selected_blocks) {
        for (auto b : v) {
            if (!isNullBlockIdx(b) && b > 0) {
                uniq.insert(b);
            }
    ...
    BlockIndicesType uniq_blocks;
    uniq_blocks.reserve(uniq.size());
    for (auto b : uniq) {
        uniq_blocks.push_back(b);
    }
    block_pool_->blockCacheReference(uniq_blocks);
...
void HybridLayerKVCacheAllocator::decrKVCacheRef(KVCacheResource& kvcache_resource) {
    ...
    block_pool_->blockCacheFree(uniq_blocks);
}
...
int HybridLayerKVCacheAllocator::getNeedBlocks(const MallocInfo& malloc_info) const {
    ...
    for (int gid = 0; gid < malloc_info.batch_kv_cache_resource->groupNums(); ++gid) {
        const int total_blocks_per_batch =
            kv_cache_groups_[static_cast<size_t>(gid)]->needBlocksNum(total_seq_len, /*current_blocks=*/0);
        const int common_blocks =
            kv_cache_groups_[static_cast<size_t>(gid)]->needBlocksNum(common_seq_len, /*current_blocks=*/0);
        const int extra_blocks_per_batch = std::max(total_blocks_per_batch - common_blocks, 0);
        common_blocks_total += common_blocks;
        extra_blocks_total += extra_blocks_per_batch;
    }
    return (batch_size <= 0) ? 0 : (common_blocks_total + batch_size * extra_blocks_total);
}
...
int HybridLayerKVCacheAllocator::singleBatchNeedBlocks(const BatchKVCacheResourcePtr& batch_kv_cache_resource,
    ...
    for (int gid = 0; gid < batch_kv_cache_resource->groupNums(); ++gid) {
        const int cur_blocks = batch_kv_cache_resource->blocksNum(/*batch_id=*/0, gid);
        need += kv_cache_groups_[static_cast<size_t>(gid)]->needBlocksNum(seq_len, cur_blocks);
    }
    return need;
}
