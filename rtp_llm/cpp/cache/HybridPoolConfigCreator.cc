...
void applyDsv4HcaStatePoolBlocks(CacheConfig& config, uint32_t block_num) {
    ...
    for (size_t gid = 0; gid < policies.size(); ++gid) {
        if (config.tagForGroup(gid) == "hca_state") {
            // The spec descriptor is the owner of explicit sizing. Keep the
            // config-level value only as a fallback for callers that do not
            // provide one, rather than overwriting an explicit descriptor.
            if (policies[gid].explicit_block_num == 0) {
