...
KVCacheResource CPSlotMapper::projectConnectorResource(const KVCacheResource& source,
    ...
    // This is a transfer-only view over the request timeline. Reinitialize the copy's tagged physical payloads, then
    // select existing raw keys; never rewrite the source resource or create a group-local key identity.
    ...
    // contract discards the dummy, not the usable selected key. The dummy exists only in this temporary copy.
    ...
    for (const auto& [tag, block_ids] : source.groupBlocks()) {
