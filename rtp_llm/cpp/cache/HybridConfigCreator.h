...
private:
    ...
    static std::pair<std::vector<int>, std::vector<int>> splitLayersByAttentionType(
        const ModelConfig& model_config);
    static CacheConfig initializeConfig(
        const ModelConfig& model_config,
        const std::vector<int>& linear_layers,
        const std::vector<int>& full_layers,
        rtp_llm::DataType dtype);
    static KVCacheSpecPtr createFullAttentionSpec(
        const ModelConfig& model_config, 
        const ParallelismConfig& parallelism_config,
        rtp_llm::DataType dtype);
    static KVCacheSpecPtr createLinearAttentionSpec(
        const ModelConfig& model_config,
        const ParallelismConfig& parallelism_config,
        rtp_llm::DataType dtype);
    static std::pair<std::vector<std::vector<int>>, std::vector<std::vector<int>>> createLayerGroups(
        const std::vector<int>& linear_layers,
        const std::vector<int>& full_layers,
        int& group_layer_num);
    static void setupCacheConfigSpecs(
        CacheConfig& config,
    ...
    static void setupPhysicalSizes(
        CacheConfig& config,
        const KVCacheSpecPtr& full_spec,
        const KVCacheSpecPtr& linear_spec);
    static void setupLayerToGroupMapping(
        CacheConfig& config);
};
