...
RuntimeMemorySizingResult calculateRuntimeMemorySizing(const RuntimeMemorySizingInput& input) {
    ...
    if (!input.has_warmup) {
        // Pre-warmup-feature semantics, kept bit-for-bit: the ratio term is a floor inside the
        // max() (formerly the hardcoded max(2048 MiB, 5% * total) minimum), not additive
        // headroom, so deployments that never run a traced warmup see no sizing change.
        RuntimeMemorySizingResult result;
        result.safety_ratio_bytes     = safety_ratio_bytes;
    ...
    RuntimeMemorySizingResult result;
    result.safety_ratio_bytes     = safety_ratio_bytes;
    result.runtime_required_bytes = base_required_bytes + safety_ratio_bytes;
    // The floor is deliberately not applied on this path, so flag the case where that
    // makes this deployment reserve less than it did before the warmup feature. Only a
    // flag: this layer has no logger, and MemoryEvaluationHelper owns the message that
    // names the knob to restore the old behaviour.
    result.warmup_below_no_warmup_floor =
        input.no_warmup_floor_bytes > 0 && result.runtime_required_bytes < input.no_warmup_floor_bytes;
    return result;
}
