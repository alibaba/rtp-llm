...
public:
    ...
    int curBlocksNum() const {
        ...
        for (const auto& [tag, block_ids] : resource.groupBlocks()) {
            (void)tag;
            max_blocks_num = std::max(max_blocks_num, static_cast<int>(block_ids->blocksNum()));
        }
    ...
    void check() const {
        RTP_LLM_CHECK(!batch_resource.empty());
        const auto& groups = batch_resource[0].groupBlocks();
        for (const auto& resource : batch_resource) {
            for (const auto& [tag, block_ids] : groups) {
                RTP_LLM_CHECK(resource.blocksNum(tag) == static_cast<int>(block_ids->blocksNum()));
            }
        }
    }
