...
void DSV4CacheConfigHelper::applyConfig(CacheConfig&             config,
    ...
    // restore-window recompute would overwrite shared cached blocks). The Python trim path is
    // suffix-slice based and explicitly CP-off only; keep the C++ FULL extension gated the same
    // way so CP page-RR does not read cached FULL blocks while Python runs the untrimmed body.
    // No separate layer_num gate is needed here: caching is itself gated to multi-layer models,
    // and the SWA-only one-layer MTP propose config owns no FULL group so every trim stage is a
    // structural no-op there regardless. Default off => restore_blocks contributes 0 in
    // reuseCache => byte-identical.
