...
CacheConfig CacheConfigCreator::createBasicConfig(const rtp_llm::GptInitParameter& param,
                                                  bool is_mtp)
{
        ...
        const std::optional<WarmUpResult>& warm_up_result)
{
    ...
    if (params.kv_cache_mem_mb_ > 0) {
        RTP_LLM_LOG_INFO("GptInitParameter explicitly specified kv cache memory size %ld MiB",
    ...
    if (warm_up_result) {
        ...
        RTP_LLM_LOG_INFO("devices reserved %ld MiB memory, warm up consumed %ld MiB max memory, env runtime memory %ld MiB, final runtime memory %ld MiB",
    ...
    RTP_LLM_LOG_INFO("cache config final decided kv cache memory size %ld MiB",
