...
struct MemoryCacheValue {
    int64_t            cache_key;           // 对应的cache_key
    ...
    MemoryCacheValue(const int64_t&            cache_key,
                     const int&                block_id,
...
public:
    ...
    BlockLRUMatchResult match(const std::vector<int64_t>& cache_keys);

    // 将数据放入缓存, 放入block会减少ref counter, 这里假设用了match的请求一定会在最后调用put
    // 返回没有缓存住的block_ids
    std::vector<int> put(const std::vector<int64_t>& cache_keys,
...
private:
    mutable LRUCache<int64_t, std::shared_ptr<MemoryCacheValue>> lru_cache_;
