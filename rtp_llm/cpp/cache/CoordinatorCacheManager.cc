...
CoordinatorCacheManager::incrKVCacheRefWithReleaseCallback(const KVCacheResource& kvcache_resource,
...
void CoordinatorCacheManager::blockBatchCopyByGroup(const std::vector<TaggedBlockIdPair>& copy_mapping) {
    ...
    for (const auto& mapping : copy_mapping) {
        ...
        const auto  layer_ids = config_.layerIdsForGroup(mapping.tag);
        const auto  main_layer_num = std::count_if(layer_ids.begin(), layer_ids.end(), [this](int layer_id) {
            return layer_id >= 0 && static_cast<uint32_t>(layer_id) < config_.layer_num;
        });
        copy_nums[copy_type] += main_layer_num * (group.kvScaleStrideBytes() > 0 ? 2 : 1);
        for (const auto& mtp_config : config_.mtp_sub_configs) {
            const auto& mtp_group = mtp_config->group(mapping.tag);
            copy_nums[copy_type] +=
                mtp_config->layerIdsForGroup(mapping.tag).size() * (mtp_group.kvScaleStrideBytes() > 0 ? 2 : 1);
        }
    }
    ...
    for (const auto& mapping : copy_mapping) {
        ...
        for (int layer_id : config_.layerIdsForGroup(static_cast<size_t>(gid))) {
            const auto src_buffers =
                kv_cache_groups_[static_cast<size_t>(gid)]->convertIndexToBuffer(layer_id, mapping.src);
            const auto dst_buffers =
                kv_cache_groups_[static_cast<size_t>(gid)]->convertIndexToBuffer(layer_id, mapping.dst);
            ...
            for (size_t buffer_id = 0; buffer_id < src_buffers.size(); ++buffer_id) {
                ...
                RTP_LLM_CHECK_WITH_INFO(src.addr != nullptr && dst.addr != nullptr,
                                        "cache block copy has null buffer for tag=%s layer=%d buffer=%zu src=%d dst=%d",
                                        mapping.tag.c_str(),
                                        layer_id,
                                        buffer_id,
                                        mapping.src,
...
size_t CoordinatorCacheManager::totalReservableBlocks() const {
    size_t total = 0;
    for (size_t gid = 0; gid < group_block_pools_.size(); ++gid) {
        if (!group_block_pools_[gid] || config_.usesExplicitIndependentBlocks(gid)) {
            continue;
...
size_t
CoordinatorCacheManager::reserveBlocksForPool(size_t gid, size_t reserve_blocks, size_t total_reservable_blocks) const {
