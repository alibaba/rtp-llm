...
struct MemoryLayoutConfig {
    ...
    // ---- Offsets within the device pool global buffer ----
    ...
    // Number of kernel blocks packed inside one device-pool block. When > 1,
    // the pool allocates physical blocks (each = bpk × kernel block bytes), but
