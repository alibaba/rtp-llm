...
public:
    ...
    static BlockPoolConfig createConfig(const CacheConfig& cache_config) {
        ...
        MemoryLayoutConfig main_layout =
            createMemoryLayoutConfig(is_hybrid, layer_num, cache_config.block_num, cache_config.cache_specs[0]);
        ...
        for (size_t i = 0; i < cache_config.mtp_sub_configs.size(); ++i) {
            ...
            MemoryLayoutConfig mtp_layout =
                createMemoryLayoutConfig(false, mtp_layer_num, cache_config.block_num, mtp_spec);
...
private:
    static MemoryLayoutConfig createMemoryLayoutConfig(bool                                enable_hybrid_attention,
        ...
        cfg.kv_scale_stride_bytes = spec->scale_block_size_bytes();
        ...
        // KV pool is still byte-addressed internally, but expose to upper layer with spec[0].dtype
