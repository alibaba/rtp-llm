...
public:
        ...
        if (config.mla_tiered_cache) {
            ...
            for (auto& layout : config.memory_layouts) {
                ...
                layout.kv_block_pool_size_bytes =
                    layout.layer_num * layout.kv_block_stride_bytes * (layout.block_num - layout.mla_hbm_blocks);
                ...
                layout.total_size_bytes =
                    layout.kv_block_pool_size_bytes + layout.kv_scale_pool_size_bytes + layout.mla_hbm_size_bytes;
            }
        }
    ...
    static BlockPoolConfig createConfigForGroup(const CacheConfig& cache_config, size_t group_id) {
        ...
        config.mla_tiered_cache =
            cache_config.dsa_mla_resident_tokens > 0 && std::dynamic_pointer_cast<MLAKVCacheSpec>(spec) != nullptr;
        if (config.mla_tiered_cache) {
            layout.mla_hbm_blocks      = cache_config.dsa_mla_hbm_blocks;
            layout.mla_resident_tokens = cache_config.dsa_mla_resident_tokens;
            layout.mla_hbm_size_bytes =
                layer_num * kv_stride
                * (layout.mla_hbm_blocks + layout.mla_resident_tokens / layout.seq_size_per_block);
            layout.kv_block_pool_size_bytes = layer_num * kv_stride * (config.block_num - layout.mla_hbm_blocks);
            layout.kv_scale_offset_bytes    = layout.kv_block_pool_size_bytes;
            layout.total_size_bytes =
                layout.kv_block_pool_size_bytes + layout.kv_scale_pool_size_bytes + layout.mla_hbm_size_bytes;
        }
        config.memory_layouts.push_back(layout);
        config.total_size_bytes =
            layout.kv_block_pool_size_bytes + (config.mla_tiered_cache ? 0 : layout.kv_scale_pool_size_bytes);
        return config;
    }
