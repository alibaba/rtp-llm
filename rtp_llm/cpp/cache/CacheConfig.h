...
struct CacheConfig {
    ...
    // Opaque model-extension payload (DSV41ModelIdentity) populated by the
    // model's cache config helper; the generic config never reads its content.
    std::shared_ptr<const void>    dsv41_model_identity;
