...
struct CacheConfig {
    ...
    uint32_t                       auto_linear_pool_blocks                 = 0;
    ...
    uint32_t groupBlockNumForGlobal(size_t group_id, uint32_t global_block_num) const {
        ...
        const bool use_auto_linear_blocks = use_independent_block_pools && !use_typed_cache_regions && is_linear
                                            && auto_linear_pool_blocks > 0;

        if (use_auto_linear_blocks) {
            return auto_linear_pool_blocks;
        }
...
#define OUTPUT_FIELD_EXPR(name, expr) os << indent1 << name << "=" << expr << "\n"
        ...
        OUTPUT_FIELD(auto_linear_pool_blocks);
