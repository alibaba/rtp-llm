...
public:
    CudaGraphRunner(const GraphParams& graph_params, py::object py_instance):
        ...
        kv_cache_group_specs_(graph_params.kv_cache_group_specs),
...
private:
    ...
    std::map<std::string, GroupGraphSpec> kv_cache_group_specs_;
    ...
    mutable std::atomic<uint64_t>         cache_shape_fallback_count_{0};
