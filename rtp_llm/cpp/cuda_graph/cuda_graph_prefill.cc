...
void CudaGraphRunner::capturePrefill() {
    ...
    for (int i = capture_range_size - 1; i >= 0; i--) {
        ...
        if (is_prefill_cuda_graph_mode_ && num_tokens_per_bs_ == max_seq_len_) {
            ...
            // Draft-prefill capture executes a fixed Bmax*q rectangle. The
            // graph key can be smaller than that physical shape, but every
            // capture-time metadata view must still describe the full shape.
            const int capture_tokens = inputs.attention_inputs.total_tokens;
            ...
            inputs.attention_inputs.input_lengths_host.fill_(0);
            inputs.attention_inputs.prefix_lengths.fill_(prefix_len);
            inputs.attention_inputs.prefix_lengths_host.fill_(prefix_len);
            auto& input_lengths      = inputs.attention_inputs.input_lengths;
            auto& input_lengths_host = inputs.attention_inputs.input_lengths_host;
            for (int b = 0; b < active_bs; b++) {
                int tokens           = (b < active_bs - 1) ? num_tokens_per_bs_ :
                                                              (capture_tokens - b * num_tokens_per_bs_);
                input_lengths[b]      = tokens;
                input_lengths_host[b] = tokens;
            }
            ...
            for (int b = 0; b < max_bs_; b++) {
                ...
                cu_kv_seqlens_host[b + 1] = cu_kv_seqlens_host[b].item<int>()
                                              + input_lengths[b].item<int>()
                                              + prefix_lengths[b].item<int>();
            }
