...
void CudaGraphRunner::prepareAttentionInputs(const PyModelInputs& inputs,
    ...
    // Per-group device block tables are already mirrored by the device strided copies above
    // (see lines ~365-371). The host-side per-group counterpart was removed in
    // feat: support async schedule, so no extra H2H pass is needed here.
...
#endif
        // feat: support async schedule unified prefix_lengths to a single CUDA tensor;
        // the dual host/prefix_lengths_d mirror was dropped.
        inputs.attention_inputs.prefix_lengths =
            torch::full({int(max_bs_)}, prefix_init, options_cuda_int32_);
    } else {
...
void CudaGraphRunner::prepareCaptureInputs(PyModelInputs& inputs, int batch_size, int seq_len_or_tokens) {
    ...
    // prefix_lengths_d was removed by feat: support async schedule; only prefix_lengths
    // (now a single CUDA tensor) needs to be sliced.
    if (capture_mem_hold_.py_model_inputs_.attention_inputs.prefix_lengths.defined()
        && capture_mem_hold_.py_model_inputs_.attention_inputs.prefix_lengths.numel() > 0) {
        inputs.attention_inputs.prefix_lengths =
