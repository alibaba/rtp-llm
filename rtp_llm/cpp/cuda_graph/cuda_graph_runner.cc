...
void CudaGraphRunner::prepareAttentionInputs(const PyModelInputs& inputs,
    ...
    addD2DCopy(d2d_copies,
               ...
               state.current_batch_size * sizeof(int));
    addD2DCopy(d2d_copies,
               inputs.attention_inputs.nan_diag_batch_id,
               py_model_inputs_.attention_inputs.nan_diag_batch_id,
               sizeof(int64_t));
...
PyModelOutputs CudaGraphRunner::forward(const PyModelInputs& inputs, CudaGraphState& state) {
    ...
    if (is_prefill_cuda_graph_mode_) {
        ...
        outputs.nan_diag_event_counters =
            graph_instances_[state.current_real_graph_bs].mem_hold_.nan_diag_event_counters_;
        outputs.nan_diag_events = graph_instances_[state.current_real_graph_bs].mem_hold_.nan_diag_events_;
    }
...
void CudaGraphRunner::initCaptureAttentionInputs(PyModelInputs& inputs, int max_bs, int num_tokens_per_bs) {
    ...
    inputs.attention_inputs.nan_diag_batch_id =
        torch::zeros({1}, torch::TensorOptions(torch::kInt64).device(torch::kCUDA));
...
void CudaGraphRunner::prepareCaptureInputs(PyModelInputs& inputs, int batch_size, int seq_len_or_tokens) {
    ...
    inputs.attention_inputs.nan_diag_batch_id = capture_mem_hold_.py_model_inputs_.attention_inputs.nan_diag_batch_id;
