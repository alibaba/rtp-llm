...
private:
    static void bindCacheGroups(GraphParams&                                            params,
        ...
        for (const auto& tag : group_tags) {
            ...
            const auto shape    = shape_it == group_shapes.end() ?
                                      std::pair<size_t, size_t>{static_cast<size_t>(params.tokens_per_block),
                                                                static_cast<size_t>(params.kernel_tokens_per_block)} :
                                      shape_it->second;
            const auto [it, inserted] =
                params.kv_cache_group_specs.emplace(tag, GroupGraphSpec{shape.first, shape.second});
            (void)it;
