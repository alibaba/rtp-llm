...
class RejectUnexpectedBertCaptureModel(TaggedBlockTableModel):
    """Fail capture if a single Bert table incorrectly enables Bert buffers."""

    def forward(self, inputs: PyModelInputs, fmha_impl=None) -> PyModelOutputs:
        bert_position_ids = inputs.bert_embedding_inputs.combo_position_ids
        if bert_position_ids is not None and bert_position_ids.numel() != 0:
            raise RuntimeError("unexpected Bert position ids")
        bert_token_type_ids = inputs.bert_embedding_inputs.combo_tokens_type_ids
        if bert_token_type_ids is not None and bert_token_type_ids.numel() != 0:
            raise RuntimeError("unexpected Bert token type ids")
...
class BertEmbeddingWithSequenceLengthsModel(BertEmbeddingModel):
    """Expose a two-request ragged prefix while running the real Bert op."""

    def forward(self, inputs: PyModelInputs, fmha_impl=None) -> PyModelOutputs:
        output = super().forward(inputs, fmha_impl).hidden_states
        attention_inputs = inputs.attention_inputs[GROUP_TAGS[0]]
        length_signature = torch.stack(
            (
                attention_inputs.input_lengths_device[0],
                attention_inputs.input_lengths_device[1],
                attention_inputs.cu_seqlens_device[1],
                attention_inputs.cu_seqlens_device[2],
            )
        ).to(output.dtype)
        return PyModelOutputs(output + length_signature)
...
def _build_prefill_inputs(
    ...
    if input_lengths is not None:
        if not input_lengths or any(length <= 0 for length in input_lengths):
            raise ValueError("input_lengths must contain positive lengths")
        if sum(input_lengths) != seq_len:
            raise ValueError("input_lengths must sum to seq_len")
    ...
    if input_lengths is None:
        ...
        input_lengths = [seq_len // batch_size] * batch_size
    cu_seqlens = [0]
...
def _create_bert_replay_fixture(
    ...
    *,
    expose_sequence_lengths: bool = False,
    max_context_batch_size: int = 2,
) -> BertReplayFixture:
    ...
    runner = CudaGraphRunner()
    model = (
        BertEmbeddingWithSequenceLengthsModel(word_embedding)
        if expose_sequence_lengths
        else BertEmbeddingModel(word_embedding)
    )
    runner.init_prefill(
        model,
...
class TestCudaGraphTaggedCache(unittest.TestCase):
    ...
    def test_multimodal_prefill_falls_back_from_cuda_graph(self) -> None:
        ...
        with self.subTest(contract="prepared_state_cleared_after_multimodal_fallback"):
            ...
            self.assertTrue(runner.prepareAttentionInputs(prepared_inputs))
    ...
    def test_canrun_rejects_multimodal_prefill_before_prepare(self) -> None:
        runner = CudaGraphRunner()
        runner.init_prefill(
            TaggedBlockTableModel(),
            2,
    ...
    def _assert_bert_replay(
        ...
        *,
        ...
        if prepare:
            self.assertTrue(fixture.runner.prepareAttentionInputs(inputs))
        else:
    ...
    def test_prefill_bert_replay_copies_ids_for_ragged_batch(self) -> None:
        fixture = _create_bert_replay_fixture(
            (4, 8),
            expose_sequence_lengths=True,
            max_context_batch_size=4,
        )
        input_ids = [1, 2, 3, 4, 5, 6]
        position_ids = [6, 5, 4, 3, 2, 1]
        token_type_ids = [1, 2, 3, 4, 5, 6]
        inputs = _build_bert_replay_inputs(
            fixture,
            input_ids,
            position_ids,
        ...
        torch.testing.assert_close(
            ...
            + torch.tensor([1, 5, 1, 6], dtype=torch.bfloat16, device="cuda"),
        )
    ...
    def test_prefill_capture_with_only_one_bert_table_uses_non_bert_inputs(
        ...
        for name, position_encoding, token_type_embedding in (
            ...
            with self.subTest(name=name):
                runner = CudaGraphRunner()
                runner.init_prefill(
                    RejectUnexpectedBertCaptureModel(),
                    2,
                    TOKENS_PER_BLOCK,
                    TOKENS_PER_BLOCK,
                    TOKENS_PER_BLOCK,
