...
TEST(CudaGraphReplayContractsTest, ValidatesBertIdBuffersAsAPair) {
    ...
    EXPECT_FALSE(validateBertReplayIdBuffersForCopy(two_dimensional, valid, valid, valid, 4));
    EXPECT_FALSE(validateBertReplayIdBuffersForCopy(valid, valid, two_dimensional, valid, 4));
}
