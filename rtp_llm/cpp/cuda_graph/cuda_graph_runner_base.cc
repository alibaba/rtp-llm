...
CudaGraphRunnerBase::~CudaGraphRunnerBase() {
...
CudaGraphRunnerBase::CudaGraphRunnerBase(py::object  py_instance,
...
void CudaGraphRunnerBase::setPositionEncoding(torch::Tensor position_encoding) {
    position_encoding_ = std::move(position_encoding);
}

void CudaGraphRunnerBase::setTokenTypeEmbedding(torch::Tensor token_type_embedding) {
    token_type_embedding_ = std::move(token_type_embedding);
}

void CudaGraphRunnerBase::setInputEmbeddingScalar(float input_embedding_scalar) {
    input_embedding_scalar_ = input_embedding_scalar;
}

void CudaGraphRunnerBase::initCapturePreamble() {
...
void CudaGraphRunnerBase::logCudaGraphPoolMemory(const char* phase) {
    size_t free_bytes  = 0;
...
void CudaGraphRunnerBase::captureOneGraphInstance(int key, const char* key_type) {
...
void CudaGraphRunnerBase::replayGraph(int key) {
    graph_instances_[key].graph_.replay();
}

void CudaGraphRunnerBase::replayAndSyncCheck(int key, const char* key_type) {
...
void CudaGraphRunnerBase::prepareCaptureInputs(PyModelInputs& inputs, int batch_size, int seq_len_or_tokens) {
...
CaptureMemoryHold CudaGraphRunnerBase::createCaptureMemoryHold(PyModelInputs& inputs, int tokens_count) {
