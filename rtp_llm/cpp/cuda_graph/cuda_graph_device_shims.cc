...
py::module_ getGraphCaptureModule() {
    ...
    // Never retain a process-lifetime py::object here. Function-local statics
    // are destroyed after Py_Finalize during Python extension teardown, and
    // their late Py_DECREF can segfault an otherwise clean ROCm shutdown.
    // import() is cheap after the first call because Python caches the module
    // in sys.modules, while the returned reference is released under the GIL.
