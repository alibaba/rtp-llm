...
py::module_ getGraphCaptureModule() {
    ...
    // Keep the owned reference inside the GIL-protected call. A function-static
    // module is destroyed after CPython finalization has started and can crash
    // while pybind11 decrements its reference from a process exit handler.
