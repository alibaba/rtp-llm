...
struct StreamThinkInfo {
    ...
    StreamThinkInfo(
        ...
        int32_t input_length, int32_t output_length, bool is_beam_search,
        std::shared_ptr<StringContainDFA<size_t, int>> dfa_ptr) : 
        in_think_mode(think_mode), max_thinking_tokens(max_thinking_tokens), 
        end_think_token_ids(end_think_token_ids), 
        input_length(input_length), current_output_length(output_length), is_beam_search(is_beam_search),
        dfa_ptr(dfa_ptr) {}
...
public:
    static std::shared_ptr<ThinkModeLogitsProcessor> fromGenerateInput(rtp_llm::DeviceBase* device, std::shared_ptr<GenerateInput> generate_input, int32_t num);
