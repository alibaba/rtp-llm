...
bool LookupPrefillEstimator::init(const std::vector<LookupConfig>& configs) {
    for (auto& config : configs) {
        ...
        if (!impl_map_[config.machine_info].init(config.config_path)) {
            RTP_LLM_LOG_ERROR(
                "failed to init LookupMapImpl with machine:[%s], config:[%s], init LookupPrefillEstimator failed");
            return false;
        }
    }
    return true;
...
absl::StatusOr<int64_t> LookupPrefillEstimator::estimate(const std::string& machine_info,
