    ...
    RequestBlockBuffer::WatchFunc      watchFunc =
        [this, request_id, weak_task](bool ok, const std::vector<std::shared_ptr<BlockBuffer>>& blocks) {
            ...
            if (transfer_request == nullptr) {
                // Watchers see all blocks for a request, including unrelated
                // keys and duplicates already being transferred. No new work
                // does not mean the remote store has failed.
                return;
            }
