...
TEST(CacheStoreTensorUtilsTest, BlockReadUsesCpuTensorForCpuBlocks) {
    ...
    TestTcpCacheStoreServiceImpl service(
        memory_util, request_block_buffer_store, nullptr, timer_manager, locked_block_manager, tcp_client);
...
TEST(CacheStoreTensorUtilsTest, AllocatePinnedByteTensorDirectly) {
    auto tensor = cacheStoreAllocatePinnedByteTensor(4096);

    ASSERT_TRUE(tensor.defined());
    EXPECT_EQ(tensor.device(), torch::Device(torch::kCPU));
    EXPECT_EQ(tensor.scalar_type(), torch::kUInt8);
    EXPECT_EQ(tensor.numel(), 4096);
