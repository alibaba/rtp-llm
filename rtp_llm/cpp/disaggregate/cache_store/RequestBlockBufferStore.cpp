...
std::shared_ptr<BlockBuffer> RequestBlockBufferStore::makeValidBlock(const std::shared_ptr<BlockBuffer>& block) {
    ...
    // Allocate pinned storage directly. `torch::empty(...).pin_memory()` first
    // creates a pageable tensor and then copies it with the CPU intra-op pool.
    // CacheStore runs up to 15 layer copies concurrently, so that path creates
    // an OpenMP team per worker and can exhaust the host before D2H starts.
    auto tensor = cacheStoreAllocatePinnedByteTensor(static_cast<int64_t>(block->len));
    if (!tensor.defined()) {
