...
inline std::pair<int, int> getCachedTokenBlockSwapIdx(int cur_cached_len, int nxt_cached_len, int seq_size_per_block) {
    ...
    int base_block_idx       = cur_cached_len / seq_size_per_block;
    int cached_token_offset  = seq_size_per_block - cur_cached_len % seq_size_per_block - 1;
    int cached_src_block_idx = base_block_idx + cached_token_offset;
    int cached_des_block_idx = nxt_cached_len / seq_size_per_block - 1;

    return {cached_src_block_idx, cached_des_block_idx};
}

inline std::pair<int, int> getFinalTokenBlockSwapIdx(int cur_cached_len, int nxt_cached_len, int seq_size_per_block) {
    int base_block_idx   = cur_cached_len / seq_size_per_block;
    int accept_token_num = nxt_cached_len - cur_cached_len;
    int src_block_idx    = base_block_idx + accept_token_num - 1;
    int des_block_idx    = (nxt_cached_len - 1) / seq_size_per_block;
