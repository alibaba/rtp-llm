...
absl::Status EagleExecutor::propose(const std::list<GenerateStreamPtr>& streams, bool skip_check) {
    ...
    for (auto& stream : streams) {
        ...
        if (stream->containProposeStream()) {
            ...
            propose_stream = std::make_shared<EagleStream>(*stream, propose_step_);
    ...
    for (size_t i = 0; i < propose_step_; i++) {
        ...
        RETURN_IF_STATUS_ERROR(mtp_executors_[0]->process(propose_streams));
    }
