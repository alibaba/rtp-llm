...
std::shared_ptr<GenerateStream> SpeculativeEngine::createMinFakeStream(int32_t max_new_tokens,
    ...
    if (fake_hidden_states) {
        auto      dtype = score_model_params_.model_config_.data_type_;
        BufferPtr fake_hidden_states;
        if (sp_type_ == "eagle3") {
            fake_hidden_states =
                device_->allocateBuffer({dtype,
                                         {1, (size_t)score_model_params_.model_config_.hidden_size_ * 3},
                                         rtp_llm::AllocationType::DEVICE});
        } else {
            fake_hidden_states =
                device_->allocateBuffer({dtype,
                                         {1, (size_t)score_model_params_.model_config_.hidden_size_},
                                         rtp_llm::AllocationType::DEVICE});
        }
        // avoid logits nan
...
absl::Status SpeculativeEngine::init() {
    ...
    if (score_model_params_.runtime_config.warm_up) {
        ...
        RTP_LLM_LOG_INFO("warm up (max_context_batch_size %d, max_seq_len %d calculate_loss %d) query begin",
                         score_model_params_.runtime_config.max_context_batch_size,
                         score_model_params_.model_config_.max_seq_len_,
...
WarmUpResult SpeculativeEngine::warmUp() {
    ...
    fake_input->input_ids                             = device_->allocateBuffer(
        {rtp_llm::DataType::TYPE_INT32, {(size_t)score_model_params_.model_config_.max_seq_len_ - 1}, rtp_llm::AllocationType::HOST});
    std::memset(fake_input->input_ids->data(), 0, fake_input->input_ids->sizeBytes());
    fake_input->generate_config                       = make_shared<GenerateConfig>();
    fake_input->generate_config->num_return_sequences = score_model_params_.runtime_config.max_context_batch_size;
...
absl::Status SpeculativeEngine::initSystemPrompt() {
    resource_context_.reuse_cache = score_model_params_.runtime_config.reuse_cache;
...
absl::Status SpeculativeEngine::step() {
    ...
    if (device_->getDeviceProperties().tp_rank == 0) {
        ...
        if (streams.empty()) {
            if (score_model_params_.gpt_init_parameter.parallelism_config.dp_size > 1) {
                CHECK_AND_ASSIGN(streams, scheduler_->schedule(reserve_step));
                if (streams.empty()) {
                    if (score_model_params_.gpt_init_parameter.pd_sep_config.role_type == RoleType::PREFILL) {
        ...
        if (score_model_params_.gpt_init_parameter.parallelism_config.dp_size > 1
            && score_model_params_.gpt_init_parameter.pd_sep_config.role_type != RoleType::PREFILL) {
...
absl::Status SpeculativeEngine::mtpStep(std::list<GenerateStreamPtr>& streams) {
...

    {
        ...
        RTP_LLM_CHECK_WITH_INFO(score_model_params_.gpt_init_parameter.parallelism_config.dp_size <= 1 || !skip_propose,
