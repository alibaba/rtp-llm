...
int get_bits_in_quant_type(QuantType quant_type) {
    switch (quant_type) {
        ...
        case QuantType::PACKED_INT4_WEIGHT_ONLY:
            return 4;
        default:
            RTP_LLM_CHECK_WITH_INFO(false, "Invalid quant_type");
            return -1;
    }
}
...
LayoutDetails getLayoutDetailsForArch(QuantType quant_type)
{
    ...
    {
        RTP_LLM_CHECK_WITH_INFO(false, "Unsupported quantization type");
    }
    return details;
}
...
LayoutDetails getLayoutDetailsForTransform(QuantType quant_type, const int arch)
{
    ...
    else if (arch >= 80 && arch <= 90)
    {
        ...
        RTP_LLM_CHECK_WITH_INFO(false, "Unsupported Arch: %d", arch);
...
void permute_B_rows_for_mixed_gemm(int8_t*                    permuted_quantized_tensor,
    ...
    RTP_LLM_CHECK(quant_type == QuantType::PACKED_INT4_WEIGHT_ONLY || quant_type == QuantType::INT8_WEIGHT_ONLY);
    ...
    RTP_LLM_CHECK_WITH_INFO(arch_version >= 75,
...
void subbyte_transpose(int8_t*                    transposed_quantized_tensor,
    ...
    if (quant_type == QuantType::INT8_WEIGHT_ONLY) {
        ...
        RTP_LLM_CHECK_WITH_INFO(false, "Invalid quant_tye");
    }
}
...
void add_bias_and_interleave_int8s_inplace(int8_t* int8_tensor, const size_t num_elts) {
    ...
    /*RTP_LLM_CHECK_WITH_INFO(num_elts % 4 == 0, "Dimensions of int8 tensor must be a multiple of 4 for register
    relayout"); for (size_t base = 0; base < num_elts; base += 4)
    {
...
void add_bias_and_interleave_int4s_inplace(int8_t* packed_int4_tensor, const size_t num_elts) {
    ...
    /*for (size_t ii = 0; ii < num_bytes; ++ii)
    {
        ...
        RTP_LLM_CHECK_WITH_INFO(transformed_second_elt >= 0 && transformed_second_elt <= 15,
    ...
    /*RTP_LLM_CHECK_WITH_INFO(num_bytes % 4 == 0, "Dimensions of int4 tensor must be a multiple of 8 for register
    relayout"); const size_t num_registers = num_bytes / 4;
...
void add_bias_and_interleave_quantized_tensor_inplace(int8_t* tensor, const size_t num_elts, QuantType quant_type) {
    if (quant_type == QuantType::INT8_WEIGHT_ONLY) {
        ...
        RTP_LLM_CHECK_WITH_INFO(false, "Invalid quantization type for interleaving.");
    }
}
...
void symmetric_quantize(int8_t*                    processed_quantized_weight,
                        ...
                        QuantType                  quant_type,
                        const int                  arch) {
    RTP_LLM_CHECK_WITH_INFO(processed_quantized_weight, "Processed quantized tensor is NULL");
    RTP_LLM_CHECK_WITH_INFO(scale_ptr, "Scale output pointer is NULL");
    RTP_LLM_CHECK_WITH_INFO(input_weight_ptr, "Input weight pointer is NULL");
    ...
    preprocess_weights_for_mixed_gemm(
        processed_quantized_weight, unprocessed_quantized_weight, shape, quant_type, arch);
}

template void symmetric_quantize<half, float>(
    int8_t*, int8_t*, half*, const float*, const std::vector<size_t>&, QuantType, const int);
...
template void
symmetric_quantize<float, float>(int8_t*, float*, const float*, const std::vector<size_t>&, QuantType, const int);

template void
symmetric_quantize<half, float>(int8_t*, half*, const float*, const std::vector<size_t>&, QuantType, const int);

template void
symmetric_quantize<half, half>(int8_t*, half*, const half*, const std::vector<size_t>&, QuantType, const int);
...
template void symmetric_quantize<__nv_bfloat16, half>(
    int8_t*, __nv_bfloat16*, const half*, const std::vector<size_t>&, QuantType, const int);

template void symmetric_quantize<half, __nv_bfloat16>(
    int8_t*, half*, const __nv_bfloat16*, const std::vector<size_t>&, QuantType, const int);

template void symmetric_quantize<__nv_bfloat16, float>(
    int8_t*, __nv_bfloat16*, const float*, const std::vector<size_t>&, QuantType, const int);
#endif
}  // namespace rocm
}  // namespace rtp_llm
