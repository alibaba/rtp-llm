...
void hipblasMMWrapper::FP8_Gemm(hipblasOperation_t        transa,
    ...
    ROCM_CHECK(hipblasLtMatmulAlgoGetHeuristic(hipblaslt_handle_,
                                               ...
                                               heuristicResult,
                                               &returnedAlgoCount));
    RTP_LLM_CHECK_WITH_INFO(returnedAlgoCount > 0, "No valid solution found for FP8 GEMM, %d, %d, %d", m, n, k);
...
void hipblasMMWrapper::Gemm(hipblasOperation_t transa,
    ...
    if (info) {
        ...
        auto no_quant = [](hipDataType type) { return type == HIP_R_16BF || type == HIP_R_16F; };
        // if (!(no_quant(Atype_) && no_quant(Btype_)) && (use_swizzleA_ || test_swizzleA_) && transa==HIPBLAS_OP_N &&
