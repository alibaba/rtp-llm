...
class FrontendServer(object):
    ...
    async def inference(self, req: Union[str, Dict[Any, Any]], raw_request: RawRequest):
        ...
        try:
            ...
            logging.info("request_arrival: trace_id=%s request_id=%s model=%s prompt_len=%d",
                         req.get("trace_id", "-"), req.get(request_id_field_name),
                         self.py_env_configs.model_args.model_type, len(req.get("prompt", "")))
        except Exception as e:
    ...
    async def _collect_complete_response_and_record_access_log(
        ...
        logging.info("request_completion: trace_id=%s request_id=%s status=success",
                     req.get("trace_id", "-"), req.get(request_id_field_name, "unknown"))
