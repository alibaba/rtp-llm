...
class GracefulShutdownServer(Server):
    ...
    async def shutdown(self, sockets: Optional[List[socket.socket]] = None) -> None:
        active_requests = self.frontend_server.active_requests
    ...
    def create_app(self):
        ...
        async def startup():
            ...
            RunVar("_default_thread_limiter").set(
                CapacityLimiter(
                    self.frontend_server.global_controller.max_concurrency * 2
                )
            )
        ...
        async def list_models():
            assert self.frontend_server.openai_endpoint != None
            return await self.frontend_server.openai_endpoint.list_models()
