...
class TokenDecoder:
    """Token解码处理器，负责增量和非增量解码逻辑"""

    def __init__(self, tokenizer: BaseTokenizer, model_config):
        ...
        if generate_config.has_num_beams():
            ...
            if not generate_config.ignore_eos:
                processed_tokens_np_list = batch_remove_padding_eos(
                    all_output_ids_np, self.model_config.special_tokens.eos_token_id
                )
                tokens_lists_for_decode_input = [
            ...
            for i, generate_output in enumerate(generate_outputs.generate_outputs):
                ...
                if not generate_config.ignore_eos:
                    tokens = remove_padding_eos_with_numpy(
                        tokens, self.model_config.special_tokens.eos_token_id
                    )
                else:
        ...
        for i, generate_output in enumerate(generate_outputs.generate_outputs):
            ...
            if not ignore_eos:
                tokens_list = remove_padding_eos_with_numpy(
                    tokens_np, self.model_config.special_tokens.eos_token_id
                ).tolist()
            else:
