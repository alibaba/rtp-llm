"""Process-local HTTP and gRPC admission and lifetime observations.

The ASGI call or inference RPC owns the token. Routing tasks may update it,
but only the request owner closes it. No concurrency-controller resources live here.
"""
...
_registries = weakref.WeakValueDictionary()
_registry_lock = threading.Lock()


def get_frontend_request_registry(rank_id, server_id, is_embedding=False):
    """Share one sampler across HTTP/gRPC owners with the same process identity."""
    key = (str(rank_id), str(server_id))
    with _registry_lock:
        registry = _registries.get(key)
        if registry is None:
            registry = FrontendRequestRegistry(rank_id, server_id, is_embedding)
...
class FrontendRequestRegistry:
    def __init__(self, rank_id, server_id, is_embedding=False, reporter=kmonitor):
        ...
        self.counts = {
            (protocol, route, stage): 0
            for protocol, routes in (
                ("http", set(self.routes.values())),
                ("grpc", {"inference"}),
            )
            for route in routes
            for stage in STAGES
    ...
    def event(self, route, event, protocol="http"):
        _report(
            self.reporter,
    ...
    def sample(self):
        for (protocol, route, stage), value in self.snapshot().items():
            if value <= 0:
                continue
            _report(
                self.reporter,
...
class FrontendRequestToken:
    def __init__(self, registry, route, protocol="http"):
        ...
        with registry.lock:
            registry.counts[protocol, route, self.stage] += 1
        registry.event(route, "received", protocol)
    ...
    def outcome_once(self, event):
        with self.registry.lock:
            if self.closed or self.outcome is not None:
                return
            self.outcome = event
        self.registry.event(self.route, event, self.protocol)
    ...
    def _update_stage_locked(self):
        ...
        if stage != self.stage:
            self.registry.counts[self.protocol, self.route, self.stage] -= 1
            self.registry.counts[self.protocol, self.route, stage] += 1
            self.stage = stage
    ...
    def close_once(self):
        with self.registry.lock:
            ...
            self.registry.counts[self.protocol, self.route, self.stage] -= 1
        if missing_outcome:
            self.registry.event(self.route, "reject_other", self.protocol)
