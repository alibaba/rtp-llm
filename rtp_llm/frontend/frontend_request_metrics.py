...
class _RequestSnapshot:
    ...
    private_input_len: Optional[int] = None
    private_output_len: Optional[int] = None
...
def _tps_counters(snapshot: _RequestSnapshot) -> _TpsCounters:
    ...
    if input_tokens_with_cache is None:
        # A new backend always sends frontend_input_len. Missing context
        # counters on such a frame mean this stream is not the batch metric
        # owner, not that its request length should be charged again.
        input_tokens_with_cache = (
            snapshot.input_len if snapshot.private_input_len is None else 0
        ) or 0

    input_tokens = snapshot.context_token_num
    if input_tokens is None:
        if (
            snapshot.private_input_len is None
...
def frontend_metrics_enabled() -> bool:
    """Return whether the additive frontend metric pipeline is enabled."""
    raw_value = os.environ.get("RTP_LLM_FRONTEND_METRICS_ENABLE", "").strip().lower()
    if raw_value in {"1", "true", "yes", "on"}:
        return True
    if raw_value not in {"", "0", "false", "no", "off"}:
        logging.warning(
            "unrecognized RTP_LLM_FRONTEND_METRICS_ENABLE value %r; "
            "frontend metrics remain disabled",
            raw_value,
        )
    return False
...
def _request_snapshot(response: Any) -> Optional[_RequestSnapshot]:
    ...
    has_private_envelope = any(
        _field(response, name) is not None
        for name in (
            "frontend_input_len",
            "frontend_output_len",
            "context_token_num",
            "context_token_num_with_cache",
    ...
    if not aux_infos and not has_private_envelope:
    ...
    input_len = (
        _as_nonnegative_int(_field(aux_infos[0], "input_len")) if aux_infos else None
    )
    reuse_len = (
        _as_nonnegative_int(_field(aux_infos[0], "reuse_len")) if aux_infos else None
    )
    output_len = 0
    ...
    speculative_verify_rounds = (
        _as_nonnegative_int(_field(response, "speculative_verify_rounds")) or 0
    )
    speculative_accepted_token_num = (
        _as_nonnegative_int(_field(response, "speculative_accepted_token_num")) or 0
    )
    speculative_proposed_draft_tokens = (
        _as_nonnegative_int(_field(response, "speculative_proposed_draft_tokens")) or 0
    )
    ...
    frontend_input_len = _as_nonnegative_int(_field(response, "frontend_input_len"))
    frontend_output_len = _as_nonnegative_int(_field(response, "frontend_output_len"))
    request_input_len = (
        frontend_input_len
        if frontend_input_len is not None
        else (
    ...
    request_output_len = (
        frontend_output_len
        if frontend_output_len is not None
        else (
    ...
    return _RequestSnapshot(
        ...
        private_input_len=frontend_input_len,
        private_output_len=frontend_output_len,
        generate_token_num=(
...
def _merge_snapshots(
    ...
    return _RequestSnapshot(
        ...
        private_input_len=optional_sum("private_input_len"),
        private_output_len=optional_sum("private_output_len"),
...
def _latest_attempt_snapshots(
    snapshots: Dict[Any, _RequestSnapshot],
) -> Iterable[_RequestSnapshot]:
    """Select the newest backend attempt per logical request unit."""
    latest: Dict[Any, Tuple[int, _RequestSnapshot]] = {}
    for unit_key, snapshot in snapshots.items():
        if isinstance(unit_key, tuple) and len(unit_key) == 2:
            logical_unit, attempt = unit_key
            attempt = _as_nonnegative_int(attempt) or 0
        else:
            logical_unit, attempt = unit_key, 0
        previous = latest.get(logical_unit)
        if previous is None or attempt >= previous[0]:
            latest[logical_unit] = (attempt, snapshot)
...
class FrontendRequestMetricState:
    def __init__(
        ...
        self._nonstream_first_outputs: Dict[Any, Tuple[int, float, int]] = {}
    ...
    def _log_parse_error_once(self, message: str) -> None:
        if self._parse_error_logged:
            return
    ...
    def observe(self, response: Any, now_ms: Optional[float] = None) -> None:
        ...
        try:
            ...
            self._log_parse_error_once("failed to observe frontend request metrics")
    ...
    def observe_tps(self, response: Any) -> None:
        ...
        try:
            ...
            self._log_parse_error_once("failed to observe frontend TPS metrics")
    ...
    def _observe_tps(self, response: Any) -> None:
        ...
        for unit_id, unit_response in _request_units(response):
            ...
            if snapshot is not None:
                self._observe_nonstream_first_token(
                    unit_id,
                    unit_response,
                    snapshot,
                )
    ...
    def _observe_nonstream_first_token(
        self,
        unit_id: Any,
        response: Any,
        snapshot: _RequestSnapshot,
    ) -> None:
        """Derive non-streaming TTFT/TPOT origin from the backend side-channel."""
        if self._streaming:
            return
        if snapshot.output_len is None or snapshot.output_len <= 0:
            return
        attempt = (
            (_as_nonnegative_int(unit_id[1]) or 0)
            if isinstance(unit_id, tuple) and len(unit_id) == 2
            else 0
        )
        logical_unit = unit_id[0] if isinstance(unit_id, tuple) else unit_id
        previous = self._nonstream_first_outputs.get(logical_unit)
        if previous is not None and attempt <= previous[0]:
            return

        aux_infos = list(_iter_aux_info(response))
        # Anchor the estimate to side-channel arrival (the frontend clock),
        # then subtract only the backend time elapsed after its first token.
        # This also handles a one-step non-streaming request whose only raw
        # frame is the final frame, without mixing frontend/backend origins.
        arrival_ms = self._owner.now_ms()
        first_output_ms = arrival_ms
        backend_first_token_estimates = []
        for aux in aux_infos:
            cost_us = _as_nonnegative_int(_field(aux, "cost_time_us"))
            first_token_us = _as_nonnegative_int(
                _field(aux, "first_token_cost_time_us")
            )
            if (
                cost_us is not None
                and first_token_us is not None
                and cost_us >= first_token_us > 0
            ):
                backend_first_token_estimates.append(
                    arrival_ms - (cost_us - first_token_us) / 1000.0
                )
        if backend_first_token_estimates:
            first_output_ms = min(backend_first_token_estimates)
        first_output_ms = min(
            max(first_output_ms, self._start_ms),
            arrival_ms,
        )
        ...
        first_token_count = sum(
            1
            for aux in aux_infos
            if (_as_nonnegative_int(_field(aux, "output_len")) or 0) > 0
        )
        if first_token_count <= 0:
            first_token_count = max(
                _as_nonnegative_int(_field(response, "_frontend_output_batch_size"))
                or 1,
                1,
            )
        first_token_count = min(first_token_count, snapshot.output_len)
        self._nonstream_first_outputs[logical_unit] = (
            attempt,
            first_output_ms,
            first_token_count,
        )
        self._first_output_ms = min(
            candidate[1] for candidate in self._nonstream_first_outputs.values()
        )
        self._first_output_len = sum(
            candidate[2] for candidate in self._nonstream_first_outputs.values()
        )
    ...
    def _backend_tps_snapshot(
        ...
        if context_batch_size is not None:
            ...
            if (
                ...
                and snapshot.private_input_len is None
                and snapshot.input_len is not None
            ):
    ...
    def _report(self, end_ms: float) -> None:
        ...
        if snapshot is None:
            return
        backend_snapshot = _merge_snapshots(
            _latest_attempt_snapshots(self._tps_unit_snapshots)
        )
        if backend_snapshot is not None:
            private_lengths: Dict[str, int] = {}
            if backend_snapshot.private_input_len is not None:
                private_lengths["input_len"] = backend_snapshot.private_input_len
            if backend_snapshot.private_output_len is not None:
                private_lengths["output_len"] = backend_snapshot.private_output_len
            if backend_snapshot.reuse_len is not None:
                private_lengths["reuse_len"] = backend_snapshot.reuse_len
            private_lengths["speculative_verify_rounds"] = (
                backend_snapshot.speculative_verify_rounds
            )
            private_lengths["speculative_accepted_token_num"] = (
                backend_snapshot.speculative_accepted_token_num
            )
            private_lengths["speculative_proposed_draft_tokens"] = (
                backend_snapshot.speculative_proposed_draft_tokens
            )
            if private_lengths:
                # Keep outward usage/AuxInfo untouched while frontend-only
                # metrics use aggregate input, output, and cache-reuse totals.
                snapshot = replace(snapshot, **private_lengths)
        ...
        if snapshot.input_len is not None and snapshot.reuse_len is not None:
            if snapshot.input_len > 0:
                report(
                    ...
                    min(snapshot.reuse_len / snapshot.input_len, 1.0),
                    tags,
                )
...
class FrontendRequestMetrics:
    def __init__(
        ...
        self._closed = False
        self._report_error_logged = threading.Event()
        self._heartbeat_stop = threading.Event()
        self._heartbeat_thread: Optional[threading.Thread] = None
        self._heartbeat_interval_s = concurrency_report_interval_s
        self._enabled = frontend_metrics_enabled() if enabled is None else enabled
        self._last_tps_report_ms = self.now_ms()
    ...
    def start(self) -> None:
        """Start periodic reporting after the owning service starts successfully."""
    ...
    def report(self, metric: Any, value: float, tags: Dict[str, str]) -> None:
        ...
        try:
            ...
            if not self._report_error_logged.is_set():
                self._report_error_logged.set()
        ...
        *,
        ...
        if not self._enabled:
            return FrontendRequestMetricState(
                self, tags, container_tags, streaming, self.now_ms()
            )
    ...
    def _report_tps_window(self, *, emit_idle_zero: bool = True) -> None:
        with self._active_lock:
            report_time_ms = self.now_ms()
            report_window_ms = max(report_time_ms - self._last_tps_report_ms, 0.0)
            self._last_tps_report_ms = report_time_ms
            ...
            report_values["output_tokens"] = counters.output_tokens
            # Output TPS follows the legacy one-second token-count metric, so
            # engine execution time is deliberately not carried across windows.
            report_counters = _TpsCounters(**report_values)
            self._pending_tps = _TpsCounters(**pending_values)
            pending_is_empty = self._pending_tps == _TpsCounters()
        ...
        # A zero interval is possible only for an immediate/manual flush (and
        # deterministic tests). Treat it as the legacy one-second window;
        # periodic production reports use their measured elapsed interval.
        effective_report_window_ms = (
            report_window_ms if report_window_ms > 0 else 1000.0
        )
        has_generate = report_counters.output_tokens > 0
        ...
        if has_generate:
            output_tps = (
                report_counters.output_tokens * 1000.0 / effective_report_window_ms
            )
            self.report(
    ...
    def close(self) -> None:
        with self._close_lock:
            ...
            if heartbeat_thread is not None:
                interval_s = self._heartbeat_interval_s or 1.0
                heartbeat_thread.join(timeout=max(interval_s * 2.0, 1.0))
                if heartbeat_thread.is_alive():
                    logging.warning("frontend metric heartbeat did not stop in time")
                    self._closed = True
                    return
            with self._active_lock:
