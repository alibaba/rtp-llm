...
class AsyncRotatingFileHandler(logging.Handler):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._start_worker()
        logging.info(
            f"AsyncRotatingFileHandler init complete, worker thread alive: {self._worker_thread.is_alive() if self._worker_thread else 'None'}"
        )
    ...
    def _start_worker(self) -> None:
        ...
        if self._worker_thread is None or not self._worker_thread.is_alive():
            self._worker_thread = threading.Thread(
                target=self._worker_loop, name=f"AsyncLogWorker-{id(self)}", daemon=True
            )
    ...
    def emit(self, record: logging.LogRecord) -> None:
        ...
        try:
            ...
            if dropped % 10 == 1:  # Reduce logging frequency
                logging.warning(
                    f"AsyncLogHandler: dropped {dropped} log records (queue full)"
                )
