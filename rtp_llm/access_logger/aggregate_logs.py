...
def extract_process_info(filename: str) -> Dict[str, int]:
    ...
    if match:
        return {"rank_id": int(match.group(1)), "server_id": int(match.group(2))}
...
def main():
    ...
    parser.add_argument("--log-path", default=".", help="日志文件目录 (default: .)")
    ...
    parser.add_argument("--rank-id", type=int, help="指定rank ID (查看特定进程)")
    ...
    parser.add_argument(
        "--sort-by-time", action="store_true", help="按时间排序显示日志"
    )

    parser.add_argument("--follow", "-f", action="store_true", help="实时跟踪日志文件")

    parser.add_argument(
        "--no-process-info", action="store_true", help="不显示进程信息前缀"
    )
    ...
    log_files = get_log_files(
        args.log_path, args.log_type, args.rank_id, args.server_id
    )

    if not log_files:
        print(
            f"在 {args.log_path} 中没有找到匹配的 {args.log_type} 日志文件",
            file=sys.stderr,
        )
        sys.exit(1)
