...
def _convert_tokens_to_string_with_added_encoders(
    ...
    legacy_added_tokens = set(tokenizer._added_tokens_encoder.keys()) - set(
...
class IncrementDecodingUtils(object):
    ...
    def detokenize_incrementally(
        ...
        output_tokens, prefix_offset, read_offset = (
            IncrementDecodingUtils._get_new_tokens(
                tokenizer, all_input_ids, state, skip_special_tokens
            )
        )
        prefix_text, new_text = IncrementDecodingUtils._convert_token_to_string(
            tokenizer,
            output_tokens,
            prefix_offset,
            read_offset,
        ...
        if len(new_text) > len(prefix_text) and not new_text.endswith("�"):
            ...
            state.update(
                len(output_tokens), output_tokens, read_offset, len(output_tokens)
            )
            return new_text
        else:
    ...
    def _get_new_tokens(
        ...
        if state.prev_tokens is None:
            ...
            new_tokens = tokenizer.convert_ids_to_tokens(
                all_input_ids[state.last_input_id_index :],
