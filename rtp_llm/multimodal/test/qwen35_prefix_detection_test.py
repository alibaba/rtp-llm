...
class Qwen35PrefixDetectionTest(unittest.TestCase):
    ...
    def test_language_model_only_skips_multimodal_config(self):
        for language_model_only in (True, 1, "true", "1"):
            with self.subTest(language_model_only=language_model_only):
                config = self._new_model_config()

                Qwen35Moe._parse_mm_config(
                    {"language_model_only": language_model_only}, config
                )
    ...
    def test_false_or_missing_language_model_only_keeps_vlm_config(self):
        for language_model_only in (False, 0, "false", "0", None):
            with self.subTest(language_model_only=language_model_only):
                config = self._new_model_config()
                config_json = {
    ...
    def test_invalid_language_model_only_is_rejected(self):
        config = self._new_model_config()

        with self.assertRaisesRegex(ValueError, "language_model_only"):
            Qwen35Moe._parse_mm_config(
                {
                    "language_model_only": "maybe",
                    "vision_start_token_id": 151652,
                    "vision_end_token_id": 151653,
                },
                config,
