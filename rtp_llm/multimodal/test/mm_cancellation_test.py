...
class MMCancellationTest(TestCase):
    ...
    def test_greennet_rpc_wait_uses_full_budget_and_caller_deadline(self):
        server = MultimodalRpcServer.__new__(MultimodalRpcServer)
        server.engine = mock.Mock()
        server.engine.wait_greennet_verdict.return_value = GreenNetVerdict(passed=True)
        for remaining, expected in (
            ...
            with self.subTest(remaining=remaining):
                ...
                context.abort.side_effect = AssertionError("unexpected RPC abort")
                server.engine.wait_greennet_verdict.reset_mock()
                server.WaitGreenNetVerdict(MultimodalInputsPB(request_id=42), context)
                call = server.engine.wait_greennet_verdict.call_args
                self.assertEqual(call.kwargs["timeout_ms"], expected)
                self.assertEqual(call.kwargs["request_id"], 42)
