...
class CompleteVideoTransportTest(TestCase):
    def test_grpc_and_rdma_preserve_complete_video_spans(self):
        from rtp_llm.utils.grpc_util import trans_from_tensor

        embeddings = [_rows(2), _rows(14)]
        positions = [
            torch.arange(2, dtype=torch.int32)[:, None].expand(-1, 3),
            torch.arange(14, dtype=torch.int32)[:, None].expand(-1, 3),
        ]
        res = MMEmbeddingRes(embeddings, position_ids=positions)
        inline = GrpcInlineOutputBackend().transfer(MultimodalInputsPB(), res).receipt
        self.assertEqual(list(inline.split_size), [2, 14])
        self.assertEqual(
            inline.multimodal_embedding, trans_from_tensor(torch.cat(embeddings))
        )
        self.assertEqual(
            inline.multimodal_pos_id, trans_from_tensor(torch.cat(positions))
        )
        exporter = MagicMock()
        exporter.export_embedding.return_value = [_serialized_desc("video")]
        with _tensors_look_cuda():
            receipt = RdmaOutputBackend(exporter).transfer(_rdma_request(), res).receipt
        self.assertEqual(list(receipt.split_size), [2, 14])
        args = exporter.export_embedding.call_args.args
        torch.testing.assert_close(args[0], torch.cat(embeddings))
        torch.testing.assert_close(args[1], torch.cat(positions))
