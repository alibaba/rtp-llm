...
try:
    from rtp_llm.multimodal.multimodal_mixins.minimax_m3_vl import (
        minimax_m3_vl_mixin as m3,
    )
except ImportError:
    # The gate and cache coordination are framework features. The concrete
    # MiniMax graph-cache integration is only present on MiniMax model branches.
    m3 = None
...
@unittest.skipUnless(
    torch.cuda.is_available() and m3 is not None,
    "MiniMax M3 CUDA graph cache required",
)
