...
class _FakeWorkItem:
    ...
    def complete_cache(self, result):
        pass
...
class MMSchedulerTest(TestCase):
    ...
    def test_cost_aware_oom_fails_whole_batch_and_executor_continues(self):
        ...
        try:
            ...
            self.assertTrue(
                all(
                    isinstance(e, MMSchedulerExecutionError) and e.is_oom
                    for e in errors
                ),
                errors,
            )
            ...
            with self.subTest(budget=budget):
                ...
                try:
                    with self.assertRaisesRegex(
                        ...
                        sched.submit_and_wait(items)
                    self.assertIsInstance(raised.exception, MMSchedulerExecutionError)
                    self.assertTrue(raised.exception.is_oom)
                    self.assertTrue(
