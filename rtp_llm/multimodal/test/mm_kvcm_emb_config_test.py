...
def _kv_meta_model_deployment(user_data="deployment-data"):
    return {
...
class RtpKvMetaExplicitConfigTest(unittest.TestCase):
    ...
    def test_rejects_non_object_primary_entries(self):
        cases = (
            {"": []},
            {"only-entry": "not-an-object"},
            {"": None, "ignored": _primary_config()},
        )
        for config_map in cases:
            with (
                self.subTest(config_map=config_map),
                self.assertRaisesRegex(
                    RtpKvMetaObjectConfigError, "primary.*JSON object"
                ),
            ):
                RtpKvMetaObjectClientConfig.from_kv_cache_config(
                    _split_reco_config(reco_client_config=_serialized(config_map))
                )
    ...
    def test_rejects_invalid_utf8_and_oversized_user_data(self):
        oversized = "界" * 21_846
        explicit_invalid_utf8 = _primary_config()
        explicit_invalid_utf8["model_deployment"]["user_data"] = "bad\ud800"
        explicit_oversized = _primary_config()
        explicit_oversized["model_deployment"]["user_data"] = oversized
        cases = (
            (
                _split_reco_config(reco_model_user_data="bad\ud800"),
                "valid UTF-8",
            ),
            (
                _split_reco_config(reco_model_user_data=oversized),
                "exceeds 65536",
            ),
            (
                _split_reco_config(
                    reco_client_config=_serialized({"": explicit_invalid_utf8})
                ),
                "valid UTF-8",
            ),
            (
                _split_reco_config(
                    reco_client_config=_serialized({"": explicit_oversized})
                ),
                "exceeds 65536",
            ),
        )
        for config, message in cases:
            with (
                self.subTest(message=message, explicit=bool(config.reco_client_config)),
                self.assertRaisesRegex(RtpKvMetaObjectConfigError, message),
            ):
                RtpKvMetaObjectClientConfig.from_kv_cache_config(config)

    def test_accepts_hostname_ipv4_and_bracketed_ipv6_static_addresses(self):
        addresses = [
            "kvcm.example:1",
            "127.0.0.1:19001",
            "[2001:db8::1]:65535",
        ...
        config = _split_reco_config(
            reco_client_config=_serialized({"": _primary_config(address=addresses)})
        )

        derived = RtpKvMetaObjectClientConfig.from_kv_cache_config(config)

        self.assertEqual(derived.addresses, tuple(addresses))
    ...
    def test_resolves_vipserver_once_and_freezes_static_snapshot(self):
        ...
        def resolve(domain):
            ...
            return [
                ...
                SimpleNamespace(ip="2001:0db8::2", port=18002),
            ]
        ...
        self.assertEqual(
            derived.addresses,
            ("10.0.0.1:18001", "[2001:db8::2]:18002"),
        )
        self.assertEqual(transfer["address"], list(derived.addresses))
    ...
    def test_rejects_malformed_vipserver_results_without_leaking_domain(self):
        ...
        duplicate_ipv6 = [
            SimpleNamespace(ip="2001:0db8::1", port=80),
            SimpleNamespace(ip="2001:db8::1", port=80),
        ]
        with self.assertRaisesRegex(RtpKvMetaObjectConfigError, "duplicate"):
            RtpKvMetaObjectClientConfig.from_kv_cache_config(
                config, vipserver_resolver=lambda _domain: duplicate_ipv6
            )
    ...
    def test_vipserver_provider_exceptions_are_redacted(self):
        primary = _primary_config(
            enable_vipserver=True,
            vipserver_domain="secret-domain",
            address=[],
        ...
        def fail_directly(_domain):
            raise RuntimeError("private-provider-detail")

        def fail_during_iteration(_domain):
            yield SimpleNamespace(ip="10.0.0.1", port=19001)
            raise RuntimeError("private-iterator-detail")

        for resolver in (fail_directly, fail_during_iteration):
            with self.subTest(resolver=resolver.__name__):
                with self.assertRaisesRegex(
                    RtpKvMetaObjectConfigError, "failed to resolve"
                ) as caught:
                    RtpKvMetaObjectClientConfig.from_kv_cache_config(
                        config, vipserver_resolver=resolver
                    )
                message = str(caught.exception)
                self.assertNotIn("secret-domain", message)
                self.assertNotIn("private-", message)

    def test_vipserver_resolution_is_bounded_and_invalid_identity_precedes_resolution(
        self,
    ):
        primary = _primary_config(
            enable_vipserver=True,
        ...
        with self.assertRaisesRegex(
            RtpKvMetaObjectConfigError, "invalid endpoint count"
        ):
            RtpKvMetaObjectClientConfig.from_kv_cache_config(
                config, vipserver_resolver=endless
            )
        self.assertEqual(len(yielded), 65)

        invalid_primary = _primary_config(
            instance_id="",
            enable_vipserver=True,
            vipserver_domain="must-not-resolve",
            address=[],
        )
        invalid_config = _split_reco_config(
            reco_client_config=_serialized({"": invalid_primary})
        )
        resolver_calls = []
        with self.assertRaisesRegex(RtpKvMetaObjectConfigError, "instance_id"):
            RtpKvMetaObjectClientConfig.from_kv_cache_config(
                invalid_config,
...
class RtpKvMetaSplitConfigTest(unittest.TestCase):
    def test_online_environment_reuses_existing_reco_settings(self):
        ...
        self.assertEqual(
            transfer["model_deployment"],
            _kv_meta_model_deployment(user_data=""),
        )
    ...
    def test_late_config_attribute_failure_is_redacted_before_resolution(self):
        class BrokenConfig:
            reco_client_config = ""
            reco_enable_vipserver = True
            reco_vipserver_domain = "secret-domain"

            @property
            def reco_server_address(self):
                raise RuntimeError("private-late-property-detail")

        resolver_calls = []
        with self.assertRaisesRegex(
            RtpKvMetaObjectConfigError, "reco_server_address"
        ) as caught:
            RtpKvMetaObjectClientConfig.from_kv_cache_config(
                BrokenConfig(),
                vipserver_resolver=lambda domain: resolver_calls.append(domain),
        ...
        self.assertNotIn("private-late-property-detail", str(caught.exception))
        self.assertNotIn("secret-domain", str(caught.exception))
        self.assertEqual(resolver_calls, [])
