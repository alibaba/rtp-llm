...
class EmptyEmbeddingResultTest(TestCase):
    def test_empty_tensor_fails_before_any_hash_is_computed(self):
        outputs = (
            torch.empty((0, 4)),
            torch.empty((0, 2, 4)),
            torch.empty((2, 0, 4)),
            [torch.ones((2, 4)), torch.empty((0, 4))],
        )
...
class AsyncSubmitGetEmbeddingTest(TestCase):
    def test_empty_embedding_fails_without_publishing_cache_or_hashes(self):
        for cache_enabled in (False, True):
            for async_mode in (False, True):
                with self.subTest(cache_enabled=cache_enabled, async_mode=async_mode):
                    engine = self._make_engine()
                    if not cache_enabled:
                        engine._embedding_cache.resize(0, 0)
                        engine._hash_key_cache.resize(0)
                    inp = self._make_input("fake://empty-embedding")
                    try:
                        with patch.object(
                            engine.mm_part,
                            "batched_embedding",
                            return_value=[(torch.empty((0, 4)), None)],
                        ), patch(
                            "rtp_llm.multimodal.mm_process_engine.get_multimodal_feature_hash"
                        ) as hash_op:
                            with self.assertRaisesRegex(
                                Exception, "ViT returned empty embedding"
                            ):
                                if async_mode:
                                    engine.async_submit([inp])
                                    engine.get_embedding_result([inp], timeout_ms=2000)
                                else:
                                    engine.mm_embedding_impl([inp])
                            hash_op.assert_not_called()
                            self.assertIsNone(
                                engine._embedding_cache.peek(inp.cache_key())
                            )
                            self.assertEqual(engine._hash_key_cache.keys(), [])
                    finally:
                        engine.stop()
