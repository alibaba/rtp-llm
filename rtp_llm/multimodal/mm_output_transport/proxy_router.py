...
class MMOutputProxyRouter:
    ...
    def record_receipt(
        ...
        handles = [desc.handle for desc in receipt.output_rdma_slots if desc.handle]
        if not handles:
            return
    ...
    def release(self, request: ReleaseEmbeddingPB, context) -> None:
        ...
        for group_index, (worker_address, handles) in enumerate(release_groups):
            try:
                ...
                stub.ReleaseMultimodalEmbedding(
                    ReleaseEmbeddingPB(handle=handles), timeout=timeout
                )
