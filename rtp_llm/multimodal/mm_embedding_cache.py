...
class _TensorMemoryPool:
    ...
    def reserve(
        self, requested_bytes: int, alignment: int = 1
    ) -> Optional[_PoolBlock]:
        ...
        while True:
            ...
            with self._lock:
                ...
                for index, (offset, free_size) in enumerate(self._free):
                    aligned_offset = self._align(offset, alignment)
        ...
        with self._lock:
            ...
            return {
                ...
                "largest_free_block_bytes": max(
                    (size for _, size in self._free), default=0
                ),
            }
    ...
    try:
        for pool, identities in grouped.items():
            ...
            for identity in identities:
                ...
                total += nbytes
            alignment = max(
                max(1, specs[identity][0].element_size()) for identity in identities
            )
            block = pool.reserve(total, alignment)
