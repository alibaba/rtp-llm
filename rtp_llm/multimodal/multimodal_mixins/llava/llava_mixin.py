...
try:
    ...
    cpu = None
    logging.warning(
        "decord is not installed; video processing in LLaVA will fail until "
        "decord is available. Install it with `pip install decord`."
    )
...
class LlavaImageEmbedding(MultiModalEmbeddingInterface):
    ...
    def load_video(data, config):
        if VideoReader is None:
            raise ImportError(
                "decord is required for video processing in LLaVA. "
    ...
    def load_from_bytes(data, mm_type, config):
        ...
        if mm_type == MMUrlType.DEFAULT:
            # copy.copy(BytesIO) is shallow — both copies share the underlying
            # buffer and load_image consumes it, leaving the file pointer at
            # the end (or closed by PIL). Save/restore the read position so the
            # video fallback can re-read the original bytes.
            initial_pos = data.tell() if hasattr(data, "tell") else None
            try:
                load_data = LlavaImageEmbedding.load_image(data, config)
            except Exception as image_error:
                try:
                    if initial_pos is not None and hasattr(data, "seek"):
                        data.seek(initial_pos)
                    load_data = LlavaImageEmbedding.load_video(data, config)
                except Exception as video_error:
                    # Surface both failures so callers can tell which path was
                    # attempted; `from` preserves the inner traceback chain.
                    raise RuntimeError(
                        f"Failed to load as image ({image_error}) "
                        f"or video ({video_error})"
                    ) from video_error
            ...
            for image_idx, image_feature in enumerate(image_features):
                if mm_type == MMUrlType.VIDEO:  # video operations
                    ...
                    if (
                        ...
                        # Narrow exception set so genuine bugs (NameError on missing
                        # imports, AttributeError, etc.) surface instead of silently
                        # falling back to a 2x2 grid that corrupts vision embeddings.
                        except (ValueError, SyntaxError, AssertionError) as e:
                            logging.error(
                                f"failed to parse image_grid_pinpoints "
                                f"{config.get('image_grid_pinpoints')!r}: {e}; "
                                f"falling back to 2x2 grid"
                            )
