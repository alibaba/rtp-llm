...
class DeepSeekVLV2Mixin(BaseMultiModalMixin):
    ...
    def _init_multimodal(self):
        ...
        # tile_indicators) so load_mm_weight picks them up from the checkpoint;
        ...
        if hasattr(self.mm_part, "image_newline"):
            vit_parts["image_newline"] = self.mm_part.image_newline
        if hasattr(self.mm_part, "view_seperator"):
            vit_parts["view_seperator"] = self.mm_part.view_seperator
        if hasattr(self.mm_part, "tile_indicators"):
            vit_parts["tile_indicators"] = self.mm_part.tile_indicators
