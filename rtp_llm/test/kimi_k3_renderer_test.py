...
class KimiK3RendererTest(unittest.TestCase):
    ...
    def test_pretokenized_tool_choice_none_selects_no_tools_grammar(self) -> None:
        ...
        self.assertEqual(source, "pretokenized_request_tool_choice_none")
        tag = json.loads(config.structural_tag)
    ...
    def test_pretokenized_chat_constraints_declare_safe_completion_boundary(
        self,
    ) -> None:
        ...
        for phase in ("reasoning", "response"):
            self.assertEqual(
                constraints[phase]["completion_boundary"],
                "<|close|>message<|sep|>",
            )
