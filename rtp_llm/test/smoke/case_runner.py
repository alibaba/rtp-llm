...
class CaseRunner(object):
    ...
    def _start_profile(self, server_manager: MagaServerManager) -> None:
        ...
        url = f"http://0.0.0.0:{port}/start_profile"
        config = {
            "gen_timeline": True,
            "trace_name": "normal_profiler",
            "start_step": 0,
            "num_steps": 3,
        }
        profile_config_str = os.environ.get("PROFILE_CONFIG")
        if profile_config_str:
            try:
                config = json.loads(profile_config_str)
            except json.JSONDecodeError:
                logging.warning("[PROFILE] Invalid PROFILE_CONFIG JSON, using defaults")
        try:
            logging.info(f"[PROFILE] Starting profile: POST {url} config={config}")
            response = requests.post(url, json=config, timeout=10)
            logging.info(f"[PROFILE] Response: {response.status_code} {response.text}")
    ...
    def curl_server(
        ...
        profile_server_managers: Optional[List[MagaServerManager]] = None,
    ) -> TaskStates:
        targets = (
            profile_server_managers
            if profile_server_managers is not None
            else [server_manager]
        )
        for mgr in targets:
