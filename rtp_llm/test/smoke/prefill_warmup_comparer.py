...
class PrefillWarmupComparer(BaseComparer):
    ...
    def _evidence_lines(logs: str) -> str:
        markers = ("[PREFILL_WARMUP]", "[MOE_WARMUP]", "warm up done", "[KV_ALLOC]")
        lines = [line for line in logs.splitlines() if any(marker in line for marker in markers)]
    ...
    def _assert_prefill_warmup_logs(self) -> None:
        ...
        missing = [name for name, pattern in required.items() if re.search(pattern, logs) is None]
        if missing:
        ...
        memory_match = re.search(r"warm up done, max runtime used memory: (\d+) bytes", logs)
        ...
        if moe_match is None:
            raise AssertionError(
                "prefill warmup did not emit a structured MoE skew line\n"
                f"actual evidence:\n{self._evidence_lines(logs)}"
            )
        ...
        expected_fraction = (
            1.0
            if expected_ep_size <= 1 or expert_num <= top_k
            else max(
