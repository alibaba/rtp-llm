...
class SmokeResponse(BaseModel):
    ...
    prompt_logits: Optional[PromptLogitsGolden] = None
...
class NormalComparer(BaseComparer):
    ...
    def _compare(
        ...
        # prompt_logits
        if expect.prompt_logits is not None and actual.prompt_logits is None:
            diffs.append(f"{prefix}prompt_logits: expected but missing in actual")
        elif expect.prompt_logits is not None and actual.prompt_logits is not None:
            epl = expect.prompt_logits
            apl = actual.prompt_logits
            ...
            if exp_ids_head is not None:
                if exp_ids_head != act_ids_head:
                    diffs.append(
                        f"{prefix}prompt_logits topk_token_ids_head mismatch:\n"
                        f"    expect: {exp_ids_head}\n"
                        f"    actual:  {act_ids_head}"
                    )
            if exp_lp_head is not None and act_lp_head is not None:
                cmp = torch.isclose(
                ...
                if not all(cmp.reshape(-1)):
                    diffs.append(f"{prefix}prompt_logits topk_logprobs_head not close")
            if epl.target_logprobs is not None and apl.target_logprobs is not None:
                cmp = torch.isclose(
