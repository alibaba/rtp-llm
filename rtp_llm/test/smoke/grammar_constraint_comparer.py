...
class GrammarConstraintComparer(OpenaiComparer):
    ...
    def _validate_json_schema(self, expect_result, actual_result, rf: dict) -> None:
        ...
        # When the query enables thinking, reasoning_content should be
        # free-form natural text — proves the grammar mask is OFF before
        # </think>. Paired with the schema check on `content`, this pins
        # both halves of the reasoner invariant (think = unconstrained,
        # post-think = schema-constrained) without needing a per-case flag.
        check_reasoning_unconstrained = bool(
            self.qr_info.get("query", {})
            .get("extra_configs", {})
        ...
        for idx, choice in enumerate(self._choices(actual_result)):
            ...
            if check_reasoning_unconstrained:
                ...
                # Heuristic: a free-form reasoning trace has spaces and many
                # alpha chars. If the mask leaked into the thinking phase, we'd
                # see mostly schema tokens (braces, quotes, digits) with no
                # spaces / very few letters.
                if (
                    len(reasoning) < 50
                    or " " not in reasoning
                    or sum(c.isalpha() for c in reasoning) < 20
                ):
                    diffs.append(
                        f"choice[{idx}]: reasoning_content looks mask-constrained "
                        f"(expected free-form text during thinking)\n"
                        f"  len         : {len(reasoning)}\n"
                        f"  has_space   : {' ' in reasoning}\n"
                        f"  alpha_count : {sum(c.isalpha() for c in reasoning)}\n"
