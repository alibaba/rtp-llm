...
class KimiK3WorkEstimateTest(TestCase):
    def setUp(self):
        ...
        self.embedding._rdma_max_slot_bytes = None
    ...
    def test_image_work_estimate_matches_processor_geometry(self):
        ...
        expected_intermediate_elements = (
            288
            * (
            ...
            + 72 * config.text_hidden_size
        )
        output_embedding_bytes = 72 * config.text_hidden_size * 2
        self.assertEqual(
            estimate.estimated_workspace_bytes,
            2 * expected_intermediate_elements + output_embedding_bytes,
        )

    def test_rdma_slot_adds_to_output_embedding_estimate(self):
        image = Image.new("RGB", (225, 223))
        without_rdma = self.embedding.estimate_work(image, MMUrlType.IMAGE)
        self.embedding._rdma_max_slot_bytes = 1024 * 1024 * 1024
        with_rdma = self.embedding.estimate_work(image, MMUrlType.IMAGE)

        self.assertEqual(
            with_rdma.estimated_workspace_bytes
            - without_rdma.estimated_workspace_bytes,
            64 * 1024,
        )
        self.embedding.vision_config.text_hidden_size = 7168
        self.assertEqual(
            self.embedding._estimated_rdma_slot_bytes(16384, 2),
            448 * 1024 * 1024,
        )

    def test_rdma_slot_estimate_covers_cross_image_bucket_rounding(self):
        self.embedding._rdma_max_slot_bytes = 1024 * 1024 * 1024
        self.embedding.vision_config.text_hidden_size = 32768
        estimate = self.embedding._estimated_rdma_slot_bytes(8192, 2)
        estimate += self.embedding._estimated_rdma_slot_bytes(1, 2)

        self.assertGreaterEqual(estimate, 1024 * 1024 * 1024)
    ...
    def test_mm_process_engine_chunks_k3_multi_image_request(self):
        ...
        vit_config = _vit_config(
            ...
            gpu_max_batch_images=5,
            gpu_max_batch_patches=8,
        )
...
class KimiK3VisionConfigWiringTest(TestCase):
    ...
    def test_load_vision_config_propagates_multimodal_fields(self):
        ...
        checkpoint_config = {
            ...
            "image_placeholder": "checkpoint-image-placeholder",
    ...
    def test_vision_config_without_image_placeholder_is_rejected(self):
        from rtp_llm.config.model_config import ModelConfig
        from rtp_llm.models.kimi_k3.kimi_k3 import KimiK3
