...
def _token_roundtrips(tokenizer: Any, token_id: int) -> bool:
    ...
    return _encode_plain(tokenizer, text) == repeated
...
def _stable_single_token_id(
    ...
    extras = [f" {c}" for c in "abcdefghijklmnopqrstuvwxyz0123456789"]
    extras += list("abcdefghijklmnopqrstuvwxyz0123456789")
    extras += [f"{c}{c}" for c in "abcdefghijklmnopqrstuvwxyz"]
    for candidate in list(candidates) + extras:
        ids = _encode_plain(tokenizer, candidate)
        if len(ids) != 1 or ids[0] in forbidden:
            continue
        if _token_roundtrips(tokenizer, ids[0]):
            return ids[0]
    ...
    for token_id in range(vocab_size):
        if token_id in forbidden:
            continue
        if _token_roundtrips(tokenizer, token_id):
            return token_id
...
def _decode_exact_token_ids(tokenizer: Any, token_ids: List[int]) -> str:
    ...
    encoded = _encode_plain(tokenizer, text)
