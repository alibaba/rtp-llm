...
class BatchDecodeTest(unittest.TestCase):
    ...
    def test_main_stops_server_when_runner_raises(self):
        ...
        with (
            patch("rtp_llm.config.log_config.setup_logging"),
            patch.object(batch_decode_test, "parse_args", return_value=(args, [])),
            patch.object(
                batch_decode_test, "resolve_perf_engine_paths", side_effect=lambda x: x
            ),
            patch.object(batch_decode_test, "prepare_config", return_value=config),
            patch.object(batch_decode_test, "EngineServer") as engine_server,
            patch.object(batch_decode_test, "query_engine_status", return_value={}),
            patch.object(batch_decode_test, "print_config_table"),
            patch.object(batch_decode_test, "create_query", return_value={4: "query"}),
            patch.object(
                batch_decode_test,
                "_run_decode",
                side_effect=RuntimeError("runner failed"),
            ),
            patch.object(batch_decode_test, "summarize_and_cleanup_coredumps"),
            patch.object(batch_decode_test.os, "makedirs"),
        ):
...
class PipelinePrefillPerfTest(unittest.TestCase):
    ...
    def test_pp_rejects_request_and_prearm_profilers(self):
        for env, gen in [
            ...
            with (
                self.subTest(env=env, gen=gen),
                self.assertRaisesRegex(ValueError, "profiler"),
            ):
    ...
    def test_grid_threads_native_scheduler_to_warmup_and_measurement(self):
        ...
        with (
            patch("rtp_llm.test.perf_test.grid_runner.BatchPerfImpl") as batch,
            patch("rtp_llm.test.perf_test.grid_runner.create_metrics_table"),
    ...
    def test_server_respects_routing_override_and_trace_output(self):
        ...
        with (
            patch("rtp_llm.test.perf_test.server.MagaServerManager") as manager,
            patch.dict(
                os.environ,
    ...
    def test_required_trace_positive_and_negative_gates(self):
        ...
        for case in (
            ...
            with (
                ...
                patch.dict(os.environ, {"PERF_REQUIRED_TRACE": "fwd_rank0_idx24.json"}),
                patch("rtp_llm.test.perf_test.perf_utils.time.sleep"),
            ):
