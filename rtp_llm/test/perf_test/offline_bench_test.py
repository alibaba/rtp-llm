...
class EstimatedCapacity:
    ...
    def print_summary(self):
        lines = [
            ...
            f"Weight Memory:         {self.weight_mem_bytes / 1024**3:.2f} GiB",
            f"KV Cache Memory:       {self.kv_cache_mem_bytes / 1024**3:.2f} GiB",
            f"Full Attn Layers:      {self.full_attn_layers}",
            f"Linear Layers:         {self.linear_layers}",
            f"KV Bytes/Token:        {self.kv_bytes_per_token:,} (full attn only)",
            f"Per-Request Overhead:  {self.per_request_overhead:,} bytes (linear state)",
            f"Avg Tokens/Request:    {self.avg_tokens_per_request:.0f}",
            f"KV Capacity:           {self.kv_capacity} concurrent requests",
...
def estimate_capacity(
    ...
    KV memory model:
      - Full attention layers: per-token KV growth (2 × kv_heads × head_dim × dtype)
      - Linear attention layers: fixed per-request state (SSM + conv)
      - capacity = kv_mem / (per_token × avg_tokens + per_request_overhead)
    """
    import torch
    ...
    est = EstimatedCapacity(overload_ratio=overload_ratio)
    ...
    # Per-token cost (full attention layers only)
    ...
    # Per-request overhead (linear attention SSM + conv state)
    ...
    if denom > 0:
        est.kv_capacity = int(est.kv_cache_mem_bytes / denom)
...
def main() -> str:
    ...
    if args.auto_concurrency and checkpoint_path:
        est = estimate_capacity(
            checkpoint_path, config, tp_size=tp_size, overload_ratio=args.overload_ratio,
        )
        est.print_summary()
        args.concurrency_limit = est.concurrency_limit
        config.concurrency_limit = est.concurrency_limit

    max_seq_len = args.input_len_max + args.output_len_max
