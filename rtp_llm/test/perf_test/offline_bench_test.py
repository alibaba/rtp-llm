...
def parse_offline_args():
    ...
    bench.add_argument("--total_requests", type=int, default=-1,
                       help="Fixed-workload mode: submit exactly N requests then drain. "
                            "-1 = auto (3 * concurrency_limit), 0 = duration mode.")
...
def _extract_last_arg(args_list, key: str, default: str = "") -> str:
    flag = f"--{key}"
    prefix = f"--{key}="
    value = default
...
def main() -> str:
    ...
    model_type = _extract_last_arg(remaining, "model_type")
    ...
    if config.total_requests == -1:
        config.total_requests = config.concurrency_limit * 2
        logging.info(f"Auto total_requests: {config.total_requests} (2 * concurrency_limit={config.concurrency_limit})")
