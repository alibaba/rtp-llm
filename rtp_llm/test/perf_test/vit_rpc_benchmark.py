...
def make_request(urls, padding, padding_size=4):
    request = MultimodalInputsPB()
    for index, url in enumerate(urls):
        item = request.multimodal_inputs.add(multimodal_url=url, multimodal_type=1)
        item.mm_preprocess_config.mm_padding_size = (padding + index) % max(
            padding_size, 1
        )
    return request
...
def _client_process(
    address, urls, references, count, barrier, timeout, atol, rtol, padding_size
):
...
def benchmark(
    ...
    with grpc.insecure_channel(
        ...
        stub = MultimodalRpcServiceStub(channel)
        references = []
        for padding in range(max(padding_size, 1)):
...
def main():
    ...
    parser.add_argument("--mm-padding-size", type=int, default=4)
