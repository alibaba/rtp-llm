...
class GridRunnerTest(unittest.TestCase):
    ...
    def test_failure_skip_is_opt_in(self):
        ...
        with patch.object(runner, "warmup"):
            with self.assertRaisesRegex(RuntimeError, "every request to succeed"):
                runner.run()
        self.assertEqual(len(self.cells_sent()), 1)

    def test_prefill_does_not_skip_failed_measurements(self):
        self.BatchPerfImpl.return_value.run.return_value = TestResultMetrics(8, 0, 8)
        runner = grid_runner.GridRunner(
            8088, 8, [1], [32768], {32768: "query"}, is_decode=False
        )
        with patch.dict(os.environ, {"PERF_SKIP_ON_FAIL": "1"}), patch.object(
            runner, "warmup"
        ):
            with self.assertRaisesRegex(RuntimeError, "every request to succeed"):
                runner.run()

    def test_partial_decode_failure_remains_an_error(self):
        self.BatchPerfImpl.return_value.run.return_value = TestResultMetrics(8, 7, 1)
        runner = self.runner([1], [32768])
        with patch.dict(os.environ, {"PERF_SKIP_ON_FAIL": "1"}), patch.object(
            runner, "warmup"
        ):
            with self.assertRaisesRegex(RuntimeError, "every request to succeed"):
...
class PerfConfigurationTest(unittest.TestCase):
    ...
    def test_all_sm120_targets_accept_supplied_checkpoint_paths(self):
        ...
        for node in build.body:
            ...
            if "name" in kwargs and ast.literal_eval(kwargs["name"]) in {
                "v4_flash_sm120_dp4ep4_decode",
                "v4_flash_sm120_dp8ep8_decode",
                "v4_flash_sm120_dp8ep8_decode_mtp",
                "v4_flash_sm120_cp4ep4_prefill",
            }:
