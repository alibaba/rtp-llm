...
module_config = {
    "qkv_gemm": ModuleConf(
        ...
        num_flop_calc_func=lambda x: calc_gemm_num_flop(
            qkv_size, hidden_size, x.batch_size * x.seq_len
        ),
        mem_io_calc_func=lambda x: calc_gemm_mem_io(
            qkv_size, hidden_size, x.batch_size * x.seq_len, 1, 2
        ),
    ),
    "fmha": ModuleConf(
        name_pattern="fmha",
        ...
        * x.seq_len
        * head_dim
        * (2 * num_q_heads + 2 * num_kv_heads),
    ),
    "o_gemm": ModuleConf(
        name_pattern="6144u, 12288u",
        num_flop_calc_func=lambda x: calc_gemm_num_flop(
            hidden_size, o_size, x.batch_size * x.seq_len
        ),
        mem_io_calc_func=lambda x: calc_gemm_mem_io(
            hidden_size, o_size, x.batch_size * x.seq_len, 1, 2
        ),
    ),
    "dispatch": ModuleConf(
        ...
        * int(num_experts_per_tok * x.batch_size * x.seq_len),
    ),
    "moe_up": ModuleConf(
        name_pattern="5120u, 6144u,",
        num_flop_calc_func=lambda x: calc_gemm_num_flop(
            5120, 6144, int(x.batch_size * x.seq_len * num_experts_per_tok)
        ),
        mem_io_calc_func=lambda x: int(num_experts / x.dp_size) * (5120 * 6144)
        + int(x.batch_size * x.seq_len * num_experts_per_tok) * (5120 + 6144),
    ),
    "moe_down": ModuleConf(
        name_pattern="6144u, 2560u,",
        num_flop_calc_func=lambda x: calc_gemm_num_flop(
            6144, 2560, int(x.batch_size * x.seq_len * num_experts_per_tok)
        ),
        mem_io_calc_func=lambda x: int(num_experts / x.dp_size) * (6144 * 2560)
        + int(x.batch_size * x.seq_len * num_experts_per_tok) * (6144 + 2560),
    ),
    "combine": ModuleConf(
