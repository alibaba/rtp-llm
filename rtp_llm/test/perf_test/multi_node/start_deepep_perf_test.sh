...
# DeepEP normal-mode configuration (NOT low-latency)
export USE_DEEPEP_MOE=1
...
# DeepEP runtime env
export AITER_ASM_DIR=/opt/conda310/lib/python3.10/site-packages/aiter_meta/hsa/
export ACCL_FP8_CAST_LEVEL=2
...
# DeepEP frees more VRAM than mori; cap KV blocks to avoid ck_tile FMHA int32 page-offset overflow
export TEST_BLOCK_NUM=3000

# KV cache / attention
export SEQ_SIZE_PER_BLOCK=1024
export KERNEL_SEQ_SIZE_PER_BLOCK=16

# Disable features not needed for DeepEP normal mode
...
# Decode perf test parameters (same conditions as start_mori_perf_test.sh)
export IS_DECODE=1
...
echo "=== Starting DeepEP (normal) Decode Perf Test ==="
