...
from rtp_llm.test.perf_test.test_util import create_query, create_reuse_query
...
def main() -> str:
    ...
    if distribution_mode:
        ...
        reuse_query_dict = None
        if reuse_ratio > 0:
            logging.info(f"Reuse cache enabled: ratio={reuse_ratio}")
            os.environ["REUSE_CACHE"] = "1"
            os.environ["ENABLE_DEVICE_CACHE"] = "1"
            if "--reuse_cache" not in remaining:
                remaining.extend(["--reuse_cache", "1"])
            if "--enable_device_cache" not in remaining:
                remaining.extend(["--enable_device_cache", "1"])
        ...
        if reuse_ratio > 0:
            reuse_query_dict = create_reuse_query(
        ...
        if args.partial in (0, 1):
            GridRunner(
                ...
                decode_test_length=args.decode_test_length,
                generate_config=generate_config,
                reuse_query_dict=reuse_query_dict,
            ).run()
