...
class OfflineBenchConfig:
    ...
    total_requests: int = -1  # -1 = auto (2*concurrency_limit), 0 = duration mode, >0 = fixed
    seed: int = 42
    dump_workload: str = ""
...
class WorkloadGenerator:
    ...
    def __init__(
        ...
        for g in range(config.prefix_groups):
            ...
            if config.prefix_len > 0:
                ...
                prefix_ids = self._token_slice(offset, config.prefix_len)
    ...
    def next(self) -> Tuple[str, int]:
        ...
        token_ids = self._token_slice(offset, input_len)
...
class OfflineRunner:
    ...
    def run(self) -> OfflineMetrics:
        ...
        try:
            ...
            gen = WorkloadGenerator(
                self._config, self._tokenizer, seed=self._config.seed, dump_file=dump_file
            )
    ...
    def _run(self, gen: WorkloadGenerator) -> OfflineMetrics:
        ...
        if cfg.total_requests > 0:
            records, cancelled_count, submitted_count = asyncio.run(
                self._dispatch_fixed(gen, concurrency_limit, cfg.total_requests, cfg.drain_timeout_s)
            )
        else:
            records, cancelled_count, submitted_count = asyncio.run(
    ...
    async def _dispatch(
    ...
    ) -> Tuple[List[_RequestRecord], int, int]:
        ...
        Returns (records, cancelled_count, submitted_count).
        """
        ...
        async with aiohttp.ClientSession(
            ...
            while time.perf_counter() < deadline:
                ...
                if self._profile and submitted == concurrency_limit:
                    loop.run_in_executor(None, self._trigger_profile)
        ...
        return records, cancelled_count, submitted
    ...
    async def _dispatch_fixed(
        ...
        Returns (records, cancelled_count, total_requests).
        """
        sem = asyncio.Semaphore(concurrency_limit)
        records: List[_RequestRecord] = []
        lock = asyncio.Lock()
        ...
        async with aiohttp.ClientSession(
            ...
            acquire_timeout_s = drain_timeout_s if drain_timeout_s > 0 else timeout.total
            while submitted < total_requests:
                try:
                    await asyncio.wait_for(sem.acquire(), timeout=acquire_timeout_s)
                except asyncio.TimeoutError:
                    logging.warning(
                        f"Timed out waiting for an available request slot after "
                        f"{acquire_timeout_s}s; stopping submission at {submitted}/{total_requests}"
                    )
                    break
            ...
            logging.info(
                f"All {total_requests} requests submitted. "
        ...
        while time.perf_counter() < deadline:
            try:
                ...
                if self._status_sample_errors <= 3:
                    logging.warning(f"Status sample failed ({self._status_sample_errors}): {e}")
    ...
    def _analyze(
        ...
        metrics.fail_requests = max(submitted_count - metrics.success_requests - cancelled_count, 0)
        if cancelled_count > 0:
        ...
        metrics.kv_cache_total_blocks = engine_status.get("total_kv_cache", 0)
        metrics.kv_cache_block_size = engine_status.get("block_size", 1)
