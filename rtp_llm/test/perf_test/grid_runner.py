...
def _request_timeout_s() -> int:
    # A timeout shorter than the server's actual work is not just a slow test:
    # the client gives up while the server keeps the requests, so their
    # concurrency slots are never released and every later request is rejected
    # with 409. Long-context batches (JIT warmup at bs26 x 76800) can exceed the
    # old fixed 500s, so allow raising it.
    return int(os.environ.get("PERF_REQUEST_TIMEOUT_S", "500"))
...
class GridRunner:
    ...
    def warmup(self) -> None:
        if self._reuse_cache_query_dict:
            logging.info("skip global grid warmup for reuse-cache mode")
            return
    ...
    def run(self) -> List[MetricState]:
        ...
        with tqdm(
            ...
            for batch_size, input_len in grid_cases:
                ...
                reuse_query = self._reuse_cache_query_dict.get(input_len)
                query = (
                    reuse_query.hit_queries[0]
                    if reuse_query
                ...
                metric = BatchPerfImpl(
                    ...
                    self._generate_config,
                    trace_name,
                    reuse_cache_seed_query=(
                        reuse_query.seed_query if reuse_query else None
                    ),
                    query_variants=(reuse_query.hit_queries if reuse_query else None),
                    target_reuse_len=(
                        reuse_query.target_reuse_len if reuse_query else 0
                    ),
