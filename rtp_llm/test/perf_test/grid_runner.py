...
class GridRunner:
    ...
    def __init__(
        ...
        *,
        ...
        reuse_query_dict: Optional[Dict[int, tuple]] = None,
    ):
        self._port = port
        ...
        self._reuse_query_dict = reuse_query_dict
        reuse_ratio = float(os.environ.get("PERF_REUSE_RATIO", "0"))
        suffix = f" (reuse={reuse_ratio:.0%})" if reuse_ratio > 0 else ""
        self._title = ("Decode Result" if is_decode else "Prefill Result") + suffix
        ...
        with tqdm(
            ...
            for batch_size in self._batch_size_list:
                for input_len in self._input_len_list:
                    ...
                    reuse_pairs = None
                    test_query = self._input_query_dict[input_len]
                    if self._reuse_query_dict and input_len in self._reuse_query_dict:
                        reuse_pairs = self._reuse_query_dict[input_len]
                        test_query = reuse_pairs[1][1]
                        logging.info(
                            f"Reuse cache: input_len={input_len}, "
                            f"phases={len(reuse_pairs)}"
                        )
