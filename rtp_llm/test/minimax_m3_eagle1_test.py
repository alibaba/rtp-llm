...
from rtp_llm.models_py.modules.hybrid import CausalAttention
...
class DecoderAttentionHookTest(unittest.TestCase):
    def test_generic_decoder_keeps_causal_attention_call_contract(self):
        ...
        causal_parameters = inspect.signature(CausalAttention.forward).parameters
        self.assertIn("x_fp8", causal_parameters)
        self.assertIn("x_scale", causal_parameters)
    ...
    def test_generic_decoder_forward_accepts_topk_routing_arguments(self):
        parameters = inspect.signature(GenericMoeDecoderLayer.forward).parameters
        self.assertIn("prev_topk_indices", parameters)
        self.assertIsNone(parameters["prev_topk_indices"].default)
        self.assertIn("force_reuse_topk_indices", parameters)
        self.assertFalse(parameters["force_reuse_topk_indices"].default)
