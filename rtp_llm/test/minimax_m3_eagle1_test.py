...
class SparseDecodeRoutingTest(unittest.TestCase):
    ...
    def test_static_topk_is_target_verify_only(self, mock_score, mock_sparse):
        for decode_query_len in (1, 2, 3, 4):
            with self.subTest(decode_query_len=decode_query_len):
                ...
                self.assertEqual(
                    sparse_kwargs["target_verify_static_topk"],
                    decode_query_len > 1,
                )
                self.assertEqual(
...
class SparseDecodeMergeTest(unittest.TestCase):
    ...
    def test_static_topk_matches_general_paged_decode(self):
        torch.manual_seed(7)
        num_q_heads = 64
        num_kv_heads = 4
        head_dim = 128
        ...
        for token_rows in (4, 6, 8):
            ...
            for kv_dtype in (torch.bfloat16, torch.float8_e4m3fn):
                ...
                for sink in (
                    ...
                    expected = flash_decode_with_gqa_share_sparse_paged(**common)
                    actual = flash_decode_with_gqa_share_sparse_paged(
                        **common, target_verify_static_topk=True
                    )
                    torch.testing.assert_close(actual, expected)
                ...
                expected_empty = flash_decode_with_gqa_share_sparse_paged(**empty_topk)
                actual_empty = flash_decode_with_gqa_share_sparse_paged(
                    **empty_topk, target_verify_static_topk=True
                )
                torch.testing.assert_close(actual_empty, expected_empty)
                ...
                expected_fallback = flash_decode_with_gqa_share_sparse_paged(
                    **non_divisible_topk
                )
                actual_fallback = flash_decode_with_gqa_share_sparse_paged(
                    **non_divisible_topk, target_verify_static_topk=True
                )
                torch.testing.assert_close(actual_fallback, expected_fallback)
