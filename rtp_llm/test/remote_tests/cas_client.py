...
class CASClient:
    ...
    def upload_directory(
        ...
        all_digests = [
            re_pb2.Digest(hash=h, size_bytes=s) for h, s in result.sizes.items()
        ]
        if progress:
        ...
        if not missing:
            ...
            log.info(
                "[CAS_SUMMARY] blobs=%d total_mb=%.1f missing=0 uploaded_mb=0.0",
                total_count,
                total_mb,
            )
            if progress:
        ...
        log.info(
            "Uploading %d missing blobs (%.1f MB)",
            len(missing),
        ...
        def _send_batch_tracked(
            batch: List[re_pb2.BatchUpdateBlobsRequest.Request],
        ) -> None:
        ...
        if large_files:
            ...
            log.info(
                "ByteStream uploading %d large files in parallel (workers=%d)",
                len(large_files),
                n_workers,
            )
        ...
        try:
            resp = self.stub.BatchReadBlobs(
                re_pb2.BatchReadBlobsRequest(
                    instance_name=self.instance_name, digests=[digest]
                ),
    ...
    def _find_missing(self, digests: List[re_pb2.Digest]) -> Set[str]:
        ...
        for i in range(0, len(digests), 1000):
            ...
            resp = self._grpc_call_with_retry(
                "FindMissingBlobs",
                lambda: self.stub.FindMissingBlobs(
                    re_pb2.FindMissingBlobsRequest(
                        instance_name=self.instance_name, blob_digests=batch
                    ),
                    metadata=self.metadata,
                ),
            )
    ...
    def _grpc_call_with_retry(self, op_name: str, call):
        for attempt in range(1, CAS_RPC_MAX_ATTEMPTS + 1):
            try:
                return call()
            except grpc.RpcError as e:
                code = e.code() if hasattr(e, "code") else None
                if code not in CAS_RPC_RETRY_CODES or attempt >= CAS_RPC_MAX_ATTEMPTS:
                    raise
                wait_s = min(30, 2**attempt)
                log.warning(
                    "%s failed with retryable CAS RPC error %s; retrying in %ss "
                    "(attempt %d/%d): %s",
                    op_name,
                    code,
                    wait_s,
                    attempt + 1,
                    CAS_RPC_MAX_ATTEMPTS,
                    e,
                )
                time.sleep(wait_s)
    ...
    def _send_batch(self, requests):
        resp = self.stub.BatchUpdateBlobs(
            re_pb2.BatchUpdateBlobsRequest(
                instance_name=self.instance_name, requests=requests
            ),
            metadata=self.metadata,
        )
        failed = []
