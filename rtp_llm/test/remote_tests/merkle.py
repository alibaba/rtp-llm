...
def build_merkle_tree(root: Path, files: List[str]) -> MerkleResult:
    ...
    def _build(dir_path: str) -> re_pb2.Digest:
        ...
        d = re_pb2.Directory()
        for name, is_file, digest, is_exec in sorted(
            dir_children.get(dir_path, []), key=lambda e: e[0]
        ):
            if is_file:
                d.files.append(
                    re_pb2.FileNode(name=name, digest=digest, is_executable=is_exec)
                )
            else:
                child = f"{dir_path}/{name}" if dir_path else name
                child_digest = _build(child)
                d.directories.append(
                    re_pb2.DirectoryNode(name=name, digest=child_digest)
                )
    ...
    root_digest = _build("")
    return MerkleResult(
        root_digest=root_digest, file_map=file_map, dir_blobs=dir_blobs, sizes=sizes
    )
