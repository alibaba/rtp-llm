...
def make_tar_postscript() -> str:
    ...
    return (
        ...
        "if grep -E -q 'NVRM: Xid|nvAssertFailedNoLog|GPU has fallen off the bus' "
        f"{WORKER_OUTPUTS_DIR}/gpu_state_*.log 2>/dev/null; then "
        'echo ">>>RTP_GPU_INFRA_FAILURE"; '
        "grep -E 'NVRM: Xid|nvAssertFailedNoLog|GPU has fallen off the bus' "
        f"{WORKER_OUTPUTS_DIR}/gpu_state_*.log 2>/dev/null | tail -20; "
        "fi; "
