...
class CacheEntry:
    ...
    def to_dict(self) -> dict:
        ...
        if self.output_files:
            d["output_files"] = {
                k: v.to_dict() for k, v in self.output_files.items()
            }
        return d
...
class RemoteTestCache:
    ...
    def load_manifest(self, input_root_digest: re_pb2.Digest) -> dict:
        ...
        if manifest and manifest.get("tests"):
            log.info(
    ...
    def get_cached_nodeids(
        self, manifest: dict, items_with_gpu: list
    ) -> List[str]:
