...
class RemoteREAPIPlugin:
    ...
    def __init__(self, config, mode: RemoteDispatchMode):
        ...
        self._outputs_dir = (
            Path(_odir)
            if _odir
            else (self.rootdir / ".pytest_cache" / "remote_outputs")
        )
    ...
    def _build_session_deselect_args(self, manifest: dict) -> List[str]:
        ...
        for _key, entry_dict in manifest["tests"].items():
            ...
            if nodeid:
                deselect_nodeids.append(nodeid)
    ...
    def _build_session_command(
        ...
        if ci_profile:
            try:
                from rtp_llm.test.ci_profile_plugin import _get_pytest_ci_section
        ...
        for tier, n_workers, phase_mark in phases:
            ...
            lines.append(
                ...
                f"python rtp_llm/test/utils/device_resource.py "
                f"python -m pytest -p no:remote-gpu -p no:rtp-ci-profile {common} "
                f"{mark_arg}"
                f"-n {n_workers} "
