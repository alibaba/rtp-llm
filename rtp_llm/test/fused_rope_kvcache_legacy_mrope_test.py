...
class LegacyMropeDecodeTest(unittest.TestCase):
    def make_case(self, host_lengths=False):
        ...
        params = ops.FusedRopeAttnParams(
            kv_cache_offset=ops._get_fused_rope_kvcache().convert_offset_to_block_array(table),
    ...
    def test_bf16_cache_uses_physical_slots_with_three_axis_rope(self):
        ...
        self.check_result(ops.FusedRopeKVCacheDecodeOp(cfg).forward(qkv, kv_cache, params), case)
    ...
    def make_prefill_case(self, lengths, prefixes, explicit_positions=True):
        ...
        max_len = max(lengths)
        padding = torch.cat([
            torch.full((length,), batch * max_len - cu_host[batch],
        ...
        positions = (
            ...
            if explicit_positions else physical[:, None].expand(-1, 3).contiguous()
        )
        block_counts = [(prefix + length + 63) // 64
                        for prefix, length in zip(prefixes, lengths)]
    ...
    def test_long_image_prefill_qkv_matches_independent_rope(self):
        ...
        expected = torch.cat((reference_mrope(q, positions).flatten(1),
    ...
    def test_new_api_keeps_independent_position_and_length_inputs(self):
        ...
        op = ops.FusedRopeKVCacheDecodeOp(cfg)
        with mock.patch.object(ops, "_get_fused_rope_kvcache", return_value=kernel), mock.patch.object(
            op, "_legacy_mrope_decode", side_effect=AssertionError("legacy fallback used for new API")
        ):
