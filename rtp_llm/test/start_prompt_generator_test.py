...
class StartPromptGeneratorTest(unittest.TestCase):
    def setUp(self):
        ...
        self.cfg.parallelism_config.local_world_size = 4
    ...
    def _mock_startup_boundaries(self):
        ...
        self.stack.enter_context(patch("rtp_llm.telemetry.resolve_pod_ip"))
        return mocks
    ...
    def test_rank_topology_process_arguments_and_health_registration(self):
        for world_rank, expected_ranks in ((0, (0, 2)), (1, (0, 1, 3))):
            with self.subTest(world_rank=world_rank):
                ...
                # Each rank's workers share a listener port; server_index must
                # not add an offset or require an additional health probe.
                worker_ports = set()
                for rank in expected_ranks:
                    for server_index in range(2):
                        server_config = copy.copy(self.cfg.server_config)
                        server_config.set_local_rank(
                            rank % self.cfg.parallelism_config.local_world_size
                        )
                        server_config.frontend_server_id = server_index
                        self.assertEqual(
                            server_config.server_port,
                            self.cfg.server_config.start_port
                            + rank * self.cfg.server_config.worker_info_port_num,
                        )
                        worker_ports.add(server_config.server_port)
                self.assertEqual(
                    check_health.call_args_list,
                    [call(port) for port in sorted(worker_ports)],
                )

    def test_partial_start_failure_reaps_started_processes(self):
        first, second, failed = MagicMock(), MagicMock(), MagicMock()
        ...
        with (
            ...
            patch.object(
                start_server.multiprocessing,
                "Process",
                side_effect=[first, second, failed],
            ),
        ...
        for process in (first, second):
            ...
            process.join.assert_called_with(timeout=5)
    ...
    def test_all_rank_ports_must_be_ready(self):
        with (
            patch.dict(os.environ, {"LOCAL_WORLD_SIZE": "4"}),
            patch.object(start_server.multiprocessing, "Process"),
        ):
            start_server.start_prompt_generator_impl(
                self.controller, self.cfg, self.manager
            )
        check_ready = self.manager.register_health_check.call_args.kwargs[
            "check_ready_fn"
        ]
        with patch.object(
            start_server, "check_server_health", side_effect=[True, False]
        ) as health:
            self.assertFalse(check_ready())
            self.assertEqual(health.call_count, 2)
        with patch.object(start_server, "check_server_health", return_value=True):
            self.assertTrue(check_ready())
