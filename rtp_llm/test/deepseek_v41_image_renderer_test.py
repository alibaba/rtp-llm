...
from rtp_llm.models.deepseek_v41 import DeepSeekV41Weight
...
class V41ImageRendererTest(TestCase):
    ...
    def test_weight_descriptor_preserves_norm_overrides_and_three_delimiters(self):
        ...
        names = {weight.name.removeprefix("v41.") for weight in weights}
        self.assertEqual(names, expected)
        self.assertEqual(len(weights), 266)
        fp32_norms = {
            weight.name.removeprefix("v41.")
            for weight in weights
            if weight.data_type == torch.float32
        }
