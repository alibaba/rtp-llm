...
class Eagle3TargetVerifyBackendTest(unittest.TestCase):
    def test_fp8_kv_uses_fp8_query(self):
        self.assertEqual(
            _target_verify_query_dtype(KvCacheDataType.FP8, torch.bfloat16),
            torch.float8_e4m3fn,
        )

    def test_base_kv_preserves_model_query_dtype(self):
        self.assertEqual(
            _target_verify_query_dtype(KvCacheDataType.BASE, torch.bfloat16),
            torch.bfloat16,
        )
    ...
    def test_defaults_to_auto(self):
        with patch.dict(os.environ, {}, clear=True):
            self.assertEqual(_requested_target_verify_backend(), "auto")
