...
class TestGenerateConfigUpdates(unittest.TestCase):
    ...
    def test_type_validation_failure_is_atomic(self):
        values = {"reuse_cache": "false", "num_beams": "not-an-integer"}

        for update_method in ("update", "update_and_pop"):
            with self.subTest(update_method=update_method):
                config = GenerateConfig(reuse_cache=True, num_beams=1)
                ...
                self.assertEqual(
                    values,
                    {"reuse_cache": "false", "num_beams": "not-an-integer"},
                )
