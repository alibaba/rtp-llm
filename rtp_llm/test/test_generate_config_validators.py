    ...
    def test_negative_max_new_tokens_is_rejected(self):
        for values in (
            ...
            with self.subTest(values=values):
                with self.assertRaisesRegex(ValidationError, "max_new_tokens") as ctx:
                    GenerateConfig(**values)
                self.assertEqual(ctx.exception.errors()[0]["loc"], ("max_new_tokens",))
...
class TestGenerateConfigUpdates(unittest.TestCase):
    ...
    def test_checkpoint_token_limit_null_preserves_default_without_mutating_input(self):
        for values, expected in (
            ({}, 16),
            ({"max_new_tokens": None}, 16),
            ({"max_new_tokens": 0}, 0),
            ({"max_new_tokens": 8}, 8),
            ({"max_new_tokens": None, "aux_info": None}, None),
        ):
            with self.subTest(values=values):
                values = {"top_k": 7, **values}
                original = values.copy()
                config = GenerateConfig(max_new_tokens=16)
                ...
                with patch("rtp_llm.models.base_model.open", mock_open()), patch(
                    ...
                    if expected is None:
                        ...
                        self.assertEqual(config, GenerateConfig(max_new_tokens=16))
                    else:
                        BaseModel.load_default_generate_config(model, env)
