...
def layer0_per_dir_collect():
    ...
    if crashed:
        ...
        print(
            ">>>L0_ALL_OK (crash may be from testpath interaction, not individual dirs)"
        )
...
def layer_exit_trap():
    ...
    cmd = [
        ...
        "-t",
        "-e",
        "trace=exit_group,exit,kill,tgkill,rt_sigaction",
        "-e",
        "signal=all",
        "-o",
        strace_log,
    ...
    try:
        ...
        exit_lines = [
            l for l in lines if "exit_group" in l or "exit(" in l or "+++ exited" in l
        ]
        ...
        for i, l in enumerate(lines):
            ...
            print(
                f"(no exit_group(-1/255) found, showing all {len(exit_lines)} exit calls)"
            )
...
def layer4_system_tools():
    ...
    subprocess.run(
        [
            "bash",
            "-c",
            f'echo "{cores_dir}/core.%e.%p" > /proc/sys/kernel/core_pattern',
        ],
        check=False,
    )
    ...
    if has_strace:
        ...
        try:
            ...
            sig_lines = [
                l for l in lines if any(k in l for k in ["SIG", "kill", "exit_group"])
            ]
        ...
        with open(stdout_log, "w") as fout, open(stderr_log, "w") as ferr:
            result = subprocess.run(
                base_cmd, env=env, timeout=180, stdout=fout, stderr=ferr
            )
