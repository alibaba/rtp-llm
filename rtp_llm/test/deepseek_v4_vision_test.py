...
class DeepSeekV4VisionTest(TestCase):
    def test_image_block_alignment(self):
        for padding_size in (0, 2, 4):
            for start_pos in (0, 1, 2, 3, 18, 24, 405):
                padding = (
                    padding_size - 1 - start_pos % padding_size if padding_size else 0
                )
                types, permutation = build_image_block(7, 12, padding, padding_size)
                image_start = int((types == IMAGE_START).nonzero().item())
                self.assertEqual(image_start, padding)
                if padding_size:
                    self.assertEqual((start_pos + image_start + 1) % padding_size, 0)
    ...
    def test_tiny_encoder_uses_multimodal_config(self):
        ...
        with mock.patch(
            ...
            output = encoder.image_embedding(
                [Image.new("RGB", (4, 4))], mm_padding_size=0
            )[0]
    ...
    def test_embedding_cache_is_scoped_by_image_padding(self):
        vision_config = {
            "hidden_size": 8,
            "vision_n_layers": 1,
            "vision_dim": 16,
        ...
        phase1 = SimpleNamespace(mm_padding_size=1)
        phase2 = SimpleNamespace(mm_padding_size=2)
    ...
    def test_mm_process_engine_preserves_image_padding(self):
        class FakeMMPart:
            def __init__(self):
                self.starts = []

            def mm_embedding(self, url, mm_type, download_headers, configs):
                self.starts.append(configs.mm_padding_size)
