...
def check_server_health(server_port):
    try:
        ...
        logging.debug(f"health check is not ready: {e}")
        return False
...
def start_vit_server_impl(
    ...
    if is_vit_role:
        ...
        time_module.sleep(2)  # 给工作进程一些启动时间

        # 计算外部端口（主进程使用，代理模式只在 VIT 角色时启用）
...
def start_server(py_env_configs: PyEnvConfigs):
    ...
    try:
        ...
        if need_vit_server:
            logging.info(
                f"[START_SERVER] Starting VIT server, current managed processes: "
                f"{len(process_manager.processes)}, role_type={py_env_configs.role_config.role_type}"
            )
        ...
        if (
            ...
            logging.info(
                f"[START_SERVER] Starting backend server, current managed processes: "
                f"{len(process_manager.processes)}"
            )
            ...
            logging.info(
                f"[START_SERVER] Backend server process started: pid={backend_process.pid if backend_process else None}, "
                f"before_add: {len(process_manager.processes)} managed processes"
            )
            process_manager.add_process(backend_process)
            logging.info(
                f"[START_SERVER] After adding backend process: {len(process_manager.processes)} managed processes"
            )

        if py_env_configs.role_config.role_type != RoleType.VIT:
            # vit has its own frontend server
            logging.info(
                f"[START_SERVER] Starting frontend server, current managed processes: "
                f"{len(process_manager.processes)}"
            )
            ...
            logging.info(
                f"[START_SERVER] Frontend server processes started: {len(frontend_process) if frontend_process else 0} processes, "
                f"before_add: {len(process_manager.processes)} managed processes"
            )
            process_manager.add_processes(frontend_process)
            logging.info(
                f"[START_SERVER] After adding frontend processes: {len(process_manager.processes)} managed processes"
            )
