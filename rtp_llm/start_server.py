...
def start_vit_server_impl(
    ...
    if vit_server_count > 1:
        ...
        external_grpc_port = base_grpc_port
...
def _role_is_prefill(py_env_configs: PyEnvConfigs) -> bool:
    ...
    prefill_value = RoleType.PREFILL.value
...
def _get_startup_real_warmup_max_len(py_env_configs: PyEnvConfigs):
    model_max_len = int(py_env_configs.model_args.max_seq_len or 0)
    if model_max_len <= 0:
        raise ValueError(
...
def _get_startup_real_warmup_speculative_reserve_step(
    ...
    if sp_config is None:
        return 0
    sp_type = sp_config.type
    if sp_type in (None, "", SpeculativeType.NONE):
        return 0
    return int(sp_config.gen_num_per_cycle or 0) + 1
