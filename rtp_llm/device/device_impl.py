...
class GpuImpl(DeviceBase):
    ...
    def unpack_packed_int4_to_int8(self, tensor: torch.Tensor):
        assert tensor.dtype == torch.int8
        packed_uint8 = tensor.view(torch.uint8)
        low_nibbles = packed_uint8 & 0x0F
        high_nibbles = (packed_uint8 >> 4) & 0x0F
        unpacked = torch.stack([low_nibbles, high_nibbles], dim=-1)
        unpacked = unpacked.reshape(tensor.shape[:-1] + (-1,))
        return unpacked.view(torch.int8)
...
class CudaImpl(GpuImpl):
    ...
    def preprocess_weights_for_mixed_gemm(
        ...
        if self.arch < 75:
            return super().preprocess_weights_for_mixed_gemm(
                tensor, quant_mode, act_dtype
            )
