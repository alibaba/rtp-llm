...
class CudaImpl(GpuImpl):
    ...
    def maybe_prepare_static_weights_for_fp4_moe(
        ...
        if kernel_name not in [W.moe_down, W.moe_gate_up]:
            return kernel, scale

        if self.py_env_configs.moe_config.fp4_moe_op == "cutedsl":
            # cutedsl moe needs gate+up format for gate_up
            if kernel_name == W.moe_gate_up:
                kernel = torch.cat(
                    [
...
class RocmImpl(GpuImpl):
    ...
    def shuffle_moe_weight(
        ...
        is_gate = name in [W.moe_gate_up, W.moe_gate_up_s]
        do_shuffle = name in [W.moe_gate_up, W.moe_down]
        if x.dim() == 2:
            x = x.unsqueeze(-1)
        x_ = (
