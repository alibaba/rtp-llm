...
class GpuImpl(DeviceBase):
    ...
    def preprocess_weights_for_mixed_gemm(
        ...
        elif quant_mode == torch.quint4x2:
    ...
    def symmetric_quantize_last_axis_of_batched_matrix(self, weight, quant_mode):
        ...
        qweight = self.preprocess_weights_for_mixed_gemm(qweight, quant_mode)
    ...
    def apply_int8(self, tensor: torch.Tensor, device: str):
        ...
        int8_weight, int8_scale = self.symmetric_quantize_last_axis_of_batched_matrix(  # type: ignore
            tensor.reshape([shape[0], -1]), torch.int8,
        )
    ...
    def moe_apply_int8(self, tensor: torch.Tensor, device: str):
        ...
        for t in tensor_list:
            ...
            weight, scale = self.symmetric_quantize_last_axis_of_batched_matrix(  # type: ignore
                t.reshape([shape[0], -1]), torch.int8,
            )
    ...
    def preprocess_groupwise_weight_params(
        ...
        qweight_interleaved = self.preprocess_weights_for_mixed_gemm(qweight, quant_type)
...
class CudaImpl(GpuImpl):
    ...
    def preprocess_weights_for_mixed_gemm(
        ...
        if self.arch < 75:
            return super().preprocess_weights_for_mixed_gemm(tensor, quant_mode, act_dtype)
