...
public class WorkerStatus {
    ...
    /** One-way lifecycle of this exact service-discovery generation. */
    public enum GenerationLifecycle {
        ACTIVE,
        RETIRING
    }

    /** Mutated only while {@link #lock} is held; read lock-free by poll gates. */
    private volatile GenerationLifecycle generationLifecycle =
    ...
    private final AtomicBoolean statusCheckInProgress = new AtomicBoolean(false);
    private final AtomicBoolean cacheCheckInProgress = new AtomicBoolean(false);
    ...
    public void requireActiveGeneration() {
        requireGenerationLock();
        if (generationLifecycle != GenerationLifecycle.ACTIVE) {
            throw new IllegalStateException(
    ...
    public void requireRetiringGeneration() {
        requireGenerationLock();
        if (generationLifecycle != GenerationLifecycle.RETIRING) {
            throw new IllegalStateException(
     ...
     */
    public boolean beginRetirement() {
        requireGenerationLock();
        if (generationLifecycle == GenerationLifecycle.RETIRING) {
            return false;
        }
        generationLifecycle = GenerationLifecycle.RETIRING;
        return true;
    }

    @JsonIgnore
    public GenerationLifecycle generationLifecycle() {
        return generationLifecycle;
    }

    @JsonIgnore
    public boolean isActiveGeneration() {
        return generationLifecycle == GenerationLifecycle.ACTIVE;
    }
    ...
    public void recordSuccessfulCachePoll() {
        cacheLastUpdateTime.set(System.nanoTime() / 1000);
    }
    ...
    public boolean tryBeginStatusPoll() {
        lock.lock();
        try {
            return generationLifecycle == GenerationLifecycle.ACTIVE
                    && statusCheckInProgress.compareAndSet(false, true);
        } finally {
            lock.unlock();
        }
    }

    public void endStatusPoll() {
        statusCheckInProgress.set(false);
    }

    /** Acquire the independent cache-poll slot for this generation. */
    public boolean tryBeginCachePoll() {
        lock.lock();
        try {
            return generationLifecycle == GenerationLifecycle.ACTIVE
                    && cacheCheckInProgress.compareAndSet(false, true);
        } finally {
            lock.unlock();
        }
    }

    public void endCachePoll() {
        cacheCheckInProgress.set(false);
    }
