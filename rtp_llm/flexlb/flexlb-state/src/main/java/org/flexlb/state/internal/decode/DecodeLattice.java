...
/**
 * Decode 侧相位格纯函数集（I2 蕴含闭包 / L9 越级闭包补记 / S4 裁决矩阵 D 分支）。
 *
...
final class DecodeLattice {
    ...
    /**
     * I2 蕴含闭包（D 侧）：D 侧格为严格单调链，格内蕴含即前缀关系——
    ...
    /**
     * S4 裁决矩阵（D 分支）：与 P 侧同构，签名与分支优先级完全一致（见
     * {@code PrefillLattice#arbitrate} 的分支 javadoc）——世代屏障（S8）→
     * 版本屏障（L2，DROP_DUP）→ 迟到中间态（L9，DROP_LATE）→ 越级推进
