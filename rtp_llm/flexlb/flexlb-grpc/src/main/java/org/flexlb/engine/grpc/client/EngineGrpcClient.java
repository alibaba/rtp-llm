 ...
 */
@Component("pooledEngineGrpcClient")
public class EngineGrpcClient implements EngineAddressResolver.Listener {
    ...
    private <R> R executeGrpcCall(
        ...
        try {
            return invoke(pooledChannel, grpcCall, requestTimeoutMs, serviceType, false);
            ...
            if (isConnectionBrokenError(e)) {
                ...
                return invoke(replacement, grpcCall, requestTimeoutMs, serviceType, true);
            }
    ...
    private <R> R invoke(
        ...
        grpcReporter.reportCallMetrics(
                serviceType.getOperationName(), duration, responseSize, retry);
        return response;
    }
