...
physical and shared. Each logical worker is published and retired independently, so an unhealthy
index removes only that logical worker from routing; resource capacity, queue state, and cache
ownership remain independent per index. The schedule response omits `engine_index` when
`multi_engine_num` is `1` and returns the selected index when it is greater than `1`; internal
cache and rollback identities remain index-qualified in both cases. Metrics derived from a worker
status use `ip:http_port` for N=1 and `ip:http_port@index` for N>1.
