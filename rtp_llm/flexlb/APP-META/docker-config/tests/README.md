...
./mvnw -B -P 'opensource,!internal' -pl flexlb-api,flexlb-mock-engine -am clean package \
  -Dtest=ConfigServiceTest,ApplicationLifecycleTest,FlexlbGrpcDrainTest,FlexlbServiceImplTest,AppStateHookServerTest,FlexlbServiceCancelTest,FlexlbForwardHopGuardNettyTest,FollowerAsyncForwardingNettyTest \
...
and `protobuf`, using the existing `online_eval` protocol helpers. Build jars first using
...
- `queued`: start a long decode, hold three more Schedule RPCs in a 12-second
  batch collection window (batch size four), and TERM the entry. Pending RPCs must survive beyond six seconds
...
The branch-local fixture uses schema 1, FIFO queueing, BATCH dispatch and a
24-second mock decode (2000 tokens at 12 ms/token). The late/engine cases use
singleton batches; the queued case uses a collection window because this branch
rejects unavailable decode capacity during routing instead of queueing it.
The long decode also exceeds Python gRPC's idle-connection
close delay after server shutdown. Mock output metadata is
checked; real model token contents are not generated.
