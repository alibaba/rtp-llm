...
from term_test_support import (  # noqa: E402
    API_JAR,
    MOCK_JAR,
    JAVA_MODULE_OPTS,
    MockEngine,
...
from term_test_support import EngineOps  # noqa: E402
...
def main():
    ...
    manager = MockEngine(root / "engine")
    proc = None
    ops = None
    ...
    try:
        ...
        env = manager.start()
        config = test_config()
        # This branch rejects exhausted decode capacity during routing. Use a
        # real batch collection window to hold accepted Schedule RPCs instead.
        if args.scenario == "queued":
            config["router"]["roles"]["decode"]["availability"]["maxEngineRequests"] = 128
            config["dispatcher"].update(maxRequests=4, maxCollectionWaitMs=12000)
        config.setdefault("grpcServer", {})[
            "shutdownQuietPeriodMs"
        ] = args.quiet_period_ms
        save("flexlb-config.json", config)
        launch_env = dict(os.environ, **manager.master_env())
        ...
        if pending:
            ...
            check(proc.wait(timeout=25) == 0, "entry did not exit successfully")
            event(
