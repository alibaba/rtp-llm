...
final class RequestSlot extends RequestLifecycle {
    ...
    private final ExternalCompletionSink externalCompletion;
    ...
    RequestSlot(
            ...
            ExternalCompletionSink externalCompletion,
            AdmissionMutationSink admissionMutationSink,
            long requestId) {
        super(requestId);
        ...
        this.admissionMutationSink = Objects.requireNonNull(
                admissionMutationSink, "admissionMutationSink");
        this.requestId = requestId;
        this.future = new RequestFuture(externalCompletion, this);
    }
    ...
    void configureDeadlineError(StrategyErrorType errorType) {
        ...
        if (item != null || !admissionOpen || phase != Phase.ACTIVE) {
    ...
    boolean isLiveGeneration() {
        ...
        return isCurrentGeneration() && phase != Phase.TOMBSTONE;
    }

    boolean isTombstone() {
        requireSlotLock("tombstone lookup");
        return phase == Phase.TOMBSTONE;
    }
    ...
    private final class SlotAdmissionMutation implements AdmissionMutation {
        ...
        public void terminate(Response failure) {
            ...
            admissionMutationSink.terminate(
        ...
        public void close() {
            ...
            admissionMutationSink.complete(RequestSlot.this, this);
        }
    }
    ...
    PreemptionClaim tryInstallPreemption(
    ...
    PreemptionReduction applyPreemptionFact(PreemptionFact fact) {
        ...
        if (fact instanceof PreemptionFact.CancelStarted started) {
            PreemptionRegistration exact = exactPreemption(started.exact());
        ...
        if (fact instanceof PreemptionFact.CancelAccepted accepted) {
            PreemptionRegistration exact = exactPreemption(accepted.exact());
            if (!ownsActiveGeneration()
                    || exact == null
                    || preemption != exact
                    || !exact.acceptCancel()) {
        ...
        if (fact instanceof PreemptionFact.CancelNotFound notFound) {
            PreemptionRegistration exact = exactPreemption(notFound.exact());
            if (!ownsActiveGeneration()
                    || exact == null
                    || preemption != exact
                    || !exact.markNotFound()) {
        ...
        if (fact instanceof PreemptionFact.CancelUnknown unknown) {
            PreemptionRegistration exact = exactPreemption(unknown.exact());
            if (!ownsActiveGeneration()
                    || exact == null
                    || preemption != exact
                    || !exact.markUnknown()) {
        ...
        if (fact instanceof PreemptionFact.Released released) {
            PreemptionRegistration exact = exactPreemption(released.exact());
            if (!ownsActiveGeneration()
                    || exact == null
                    || preemption != exact
                    || !exact.isReleasable()) {
        ...
        if (fact instanceof PreemptionFact.Tombstoned tombstoned) {
            PreemptionRegistration exact = exactPreemption(
                    tombstoned.exact());
            if (!ownsActiveGeneration()
                    || exact == null
            ...
            DeferredTerminal terminal = DeferredTerminal.priority(
                    tombstoned.detail(), cancellationReason);
            exact.retainTerminal(terminal);
            // DecodePreemptionCoordinator has already consumed the exact
            // endpoint claim before publishing TOMBSTONED. Reconciliation is
            // therefore neither required nor legal on this authoritative path.
            detachPreemptionOwner(exact);
            assertInvariant();
            return new PreemptionReduction.Replay(
                    new ReplayPayload.Terminal(terminal, false),
                    exact,
                    null);
        }
        ...
        if (fact instanceof PreemptionFact.WorkerTerminal worker) {
            ...
            if (exact == null) {
                return new PreemptionReduction.Replay(
                        new ReplayPayload.Terminal(
                                worker.terminal(),
                                worker.terminal().endpointAlreadyRetired()),
                        null,
                        null);
            }
            if (exact.isSettled()) {
        ...
        if (fact instanceof PreemptionFact.DecodeGenerationRetired retired) {
            ...
            assertInvariant();
            return new PreemptionReduction.Replay(
                    new ReplayPayload.Terminal(terminal, true),
                    signal,
                    prefillCleanup);
        }
    ...
    FenceReduction requestDeliveryFence(String detail) {
        requireSlotLock("delivery fence request");
        if (preemption != null) {
            PreemptionRegistration exact = preemption;
            exact.postDeliveryFenceDetail = detail;
            if (!exact.isNotFound()) {
                assertInvariant();
    ...
    FenceReduction requestCancellationFence(String detail) {
        requireSlotLock("cancellation fence request");
        if (preemption != null) {
            PreemptionRegistration exact = preemption;
            exact.postDeliveryFenceDetail = detail;
            if (!exact.isNotFound()) {
    ...
    /** Reduce one exact Engine-fence fact to an opaque terminal capability. */
    FenceReduction applyFenceFact(FenceFact fact) {
        requireSlotLock("Engine fence fact reduction");
        if (fact instanceof FenceFact.CancelStarted started) {
            EngineFenceRegistration exact = exactFence(started.exact());
            if (!ownsActiveGeneration()
                    || exact == null
                    || !exact.beginCancel()) {
                return FenceReduction.Stale.INSTANCE;
        ...
        if (fact instanceof FenceFact.AwaitTerminal awaiting) {
            EngineFenceRegistration exact = exactFence(awaiting.exact());
            if (!ownsActiveGeneration()
                    || exact == null
                    || !exact.awaitTerminal()) {
                return FenceReduction.Stale.INSTANCE;
        ...
        FenceFact.Tombstoned tombstoned = (FenceFact.Tombstoned) fact;
        EngineFenceRegistration exact = exactFence(tombstoned.exact());
        if (!ownsActiveGeneration() || exact == null) {
            return FenceReduction.Stale.INSTANCE;
        }
    ...
    private PreemptionReduction materializePendingReplay(
        ...
        if (terminal != null
            ...
            boolean ordinaryWon = decode == null
                    || decode.reconcilePriorityVictimFinished(
                            exact.attemptToken,
            ...
            return new PreemptionReduction.Replay(
                    new ReplayPayload.Terminal(
                            terminal, terminal.endpointAlreadyRetired()),
                    signal,
                    null);
        }
        if (transportUnknown || !exact.pendingDeliveryConfirmation) {
            return PreemptionReduction.None.INSTANCE;
        }
    ...
    private PublicationLease requirePublicationLease() {
        PublicationLease lease =
                externalCompletion.tryReservePublication(this);
    ...
    private boolean isCurrentGeneration() {
        return currentGeneration.isCurrent(this);
    }
    ...
    sealed interface PreemptionFact {
        record CancelStarted(PreemptionClaim exact)
                implements PreemptionFact {
        }

        record CancelAccepted(PreemptionClaim exact)
                implements PreemptionFact {
        }

        record CancelNotFound(PreemptionClaim exact)
                implements PreemptionFact {
        }

        record CancelUnknown(PreemptionClaim exact)
                implements PreemptionFact {
        }

        record Released(PreemptionClaim exact)
                implements PreemptionFact {
        }

        record Tombstoned(PreemptionClaim exact, String detail)
                implements PreemptionFact {
        }
        ...
        record DispatchRejected(
            ...
            public DispatchRejected {
                if (terminal.kind() != DeferredTerminalKind.DELIVERY_REJECTED) {
                    throw new IllegalArgumentException(
    ...
    /** External facts which advance one exact Engine fence. */
    sealed interface FenceFact {
        record CancelStarted(FenceHandle exact) implements FenceFact {
        }

        record AwaitTerminal(FenceHandle exact) implements FenceFact {
        }

        record Tombstoned(FenceHandle exact) implements FenceFact {
        }
    }
    ...
    static final class AdmissionCleanup {
        ...
        synchronized void release(ExpirationTimer<RequestSlot> timer) {
            if (released) {
                return;
            }
            released = true;
    ...
    /** Exact priority owner. Its mutable state is visible only to RequestSlot. */
    private static final class PreemptionRegistration
            implements PreemptionClaim {
        private final long requestId;
        ...
        private PreemptionRegistration(
                long requestId,
                long attemptToken,
                String detail) {
            this.requestId = requestId;
        ...
        @Override
        public CompletableFuture<VictimTerminal> terminal() {
            return terminal;
        }
        ...
        private boolean markNotFound() {
            if (state != State.CANCEL_IN_FLIGHT) {
                return false;
            }
            state = State.NOT_FOUND_STALE;
            return true;
        }
        ...
        private boolean isFenceTransferable() {
            return state == State.CLAIMED
                    || state == State.CANCEL_IN_FLIGHT
                    || state == State.NOT_FOUND_STALE;
        }
        ...
        private boolean isNotFound() {
            return state == State.NOT_FOUND_STALE;
        }

        private boolean isUnknown() {
            return state == State.CANCEL_UNKNOWN;
        }
        ...
        private boolean canSettleTombstone() {
            return state == State.CANCEL_IN_FLIGHT
                    || state == State.NOT_FOUND_STALE
                    || state == State.CANCEL_UNKNOWN;
        }
        ...
        private void recordDeliveryConfirmation(long batchId) {
    ...
    static final class PublicationPermit {
        ...
        Publication claimCancellation(boolean mayInterruptIfRunning) {
        ...
        private Publication terminalPublication(Publication publication) {
            return () -> {
                try {
...
/** Exact-generation predicate supplied by the canonical slot directory. */
@FunctionalInterface
interface CurrentGeneration {
    boolean isCurrent(RequestSlot exactSlot);
}

/** Narrow boundary used only by the public CompletableFuture adapter. */
interface ExternalCompletionSink {
    PublicationLease tryReservePublication(RequestSlot exactSlot);

    boolean publishResponse(RequestSlot slot, Response response);

    boolean publishFailure(RequestSlot slot, Throwable error);

    boolean publishCancellation(
            RequestSlot slot, boolean mayInterruptIfRunning);
}
...
record DeferredTerminal(
    ...
    static DeferredTerminal admissionCleanup(String detail) {
        return new DeferredTerminal(
                DeferredTerminalKind.ADMISSION_CLEANUP,
                null,
                detail,
                false,
                null,
    ...
    static DeferredTerminal deliveryRejected(String detail) {
        return new DeferredTerminal(
                DeferredTerminalKind.DELIVERY_REJECTED,
    ...
    static DeferredTerminal priority(
            String detail,
            CancelReason cancellationReason) {
        return new DeferredTerminal(
                DeferredTerminalKind.PRIORITY,
    ...
    static DeferredTerminal decodeGenerationRetired(String detail) {
        return new DeferredTerminal(
                DeferredTerminalKind.DECODE_GENERATION_RETIRED,
                StrategyErrorType.BATCH_DISPATCH_FAILED,
                detail,
                false,
                null,
    ...
    boolean authoritativeWorker() {
        return kind == DeferredTerminalKind.WORKER
                || kind == DeferredTerminalKind.DECODE_GENERATION_RETIRED;
    }

    boolean endpointAlreadyRetired() {
        return kind == DeferredTerminalKind.DECODE_GENERATION_RETIRED;
    }

    boolean deliveryFailure() {
        return (kind == DeferredTerminalKind.FAILURE
                    && releasePrefillAccounting)
...
interface TerminalTransition {
    RequestLifecycleSnapshot apply(RequestSlot slot);
}
...
final class RequestFuture extends CompletableFuture<Response> {
    private final ExternalCompletionSink sink;
    private final RequestSlot slot;
    ...
    public boolean complete(Response response) {
        return sink.publishResponse(slot, response);
    }

    @Override
    public boolean completeExceptionally(Throwable error) {
        return sink.publishFailure(slot, error);
    }

    @Override
    public boolean cancel(boolean mayInterruptIfRunning) {
        return sink.publishCancellation(slot, mayInterruptIfRunning);
    }
