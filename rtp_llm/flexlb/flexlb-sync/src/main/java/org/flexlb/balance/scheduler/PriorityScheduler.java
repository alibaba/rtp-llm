...
public class PriorityScheduler implements DecisionGroupHandler, DecisionDelivery.Callback, InflightRegistrar {
    ...
    private RequestLifecycleSnapshot finishLocalCancellation(InflightEntry entry) {
        Response response = entry.cancellationReason == CancelReason.DEADLINE_EXCEEDED
                ? buildExpirationResponse(entry, cancelDetail(entry.cancellationReason))
                : buildErrorResponse(StrategyErrorType.REQUEST_CANCELLED, cancelDetail(entry.cancellationReason));
    ...
    private ResponseCompletion settleCancellationLocked(InflightEntry entry,
                                                        String proof) {
        Response response = entry.cancellationReason == CancelReason.DEADLINE_EXCEEDED
                ? buildExpirationResponse(entry, cancelDetail(entry.cancellationReason) + "; " + proof)
                : buildErrorResponse(StrategyErrorType.REQUEST_CANCELLED, cancelDetail(entry.cancellationReason));
    ...
    private ResponseCompletion timeoutEntry(InflightEntry entry, String detail) {
        PrefillEndpoint prefill = entry.item.prefillEp();
        Response response = buildExpirationResponse(entry, detail);
        if (prefill != null) {
            prefill.getBatcher().queueManager().tryRemove(
    ...
    private Response buildExpirationResponse(InflightEntry entry, String trigger) {
        BatchItem item = entry.item;
        RequestLifecycleSnapshot lifecycle = entry.lifecycle.snapshot();
        if (lifecycle.deliveryClaimKind().isClaimed()) {
            recordFailureDiagnostics(item, "delivery did not complete; claim="
                    + lifecycle.deliveryClaimKind() + "; trigger=" + trigger);
            return buildErrorResponse(StrategyErrorType.BATCH_SLO_EXPIRED,
                    "delivery did not complete; claim=" + lifecycle.deliveryClaimKind() + "; trigger=" + trigger);
        }
        WorkerBatcher batcher = item.prefillEp() == null ? null : item.prefillEp().getBatcher();
