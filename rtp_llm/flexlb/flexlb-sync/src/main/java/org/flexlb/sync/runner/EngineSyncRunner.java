...
public class EngineSyncRunner implements Runnable {
    ...
    public void run() {
        ...
        try {
            ...
            for (WorkerHost host : latestEngineWorkerList) {
                ...
                if (workerStatus.tryBeginStatusPoll()) {
                    try {
                ...
                if (workerStatus.tryBeginCachePoll()) {
                    try {
    ...
    private WorkerStatus getOrCreateWorkerStatus(
        ...
        while (true) {
            WorkerStatus workerStatus = workerStatuses.computeIfAbsent(
                    workerIpPort,
                    ignored -> createAndActivateWorkerStatus(
    ...
    private WorkerStatus createAndActivateWorkerStatus(
            String workerIpPort,
            String site,
            String group) {
        ...
        if (!cacheAwareService.activateEngineGeneration(
                workerIpPort,
                roleType,
                discovered.getGenerationId())) {
            throw new IllegalStateException(
                    "Failed to activate cache generation for "
                            + workerIpPort + "#"
                            + discovered.getGenerationId());
        }
