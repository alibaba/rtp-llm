 ...
 * metadata and is never an ownership token.  The capability is deliberately
 * thread-confined: routing, admission and close all run on the capture thread,
 * so it needs no second monitor or state machine.</p>
 */
