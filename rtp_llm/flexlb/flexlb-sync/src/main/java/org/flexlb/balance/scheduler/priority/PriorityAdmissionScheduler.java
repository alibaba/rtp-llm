...
public class PriorityAdmissionScheduler {
        ...
        while (registrar.isAdmissionOpen(ctx.getRequestId(), future)) {
            ...
            if (outcome.plan == null) {
                ...
                if (outcome.failureResponse == null) {
                    onInfeasible(ctx, future, null);
                    return;
                }
                Response failure = outcome.failureResponse;
                if (!isDecodeCapacityFailure(failure)
                        || (!preemptionAllows(config, VictimStage.DECODE_RESERVED)
                            && !preemptionAllows(config, VictimStage.DECODE_ENGINE_OWNED))) {
                    onInfeasible(ctx, future, failure);
                    return;
                }
                Long limit = config.getRouter().getRoles().getDecode().getAvailability().getMaxEngineRequests();
                ClusterSnapshot snapshot = ClusterSnapshot.captureDecode(
                        endpointRegistry, failure.getFailedGroup(), limit == null ? 0 : limit);
                eviction = tryDecodeEviction(ctx, future, snapshot, config, registrar, permit, failure);
            } else {
                try {
     ...
     * Captures a snapshot after placement fails; commit checks the selected
    ...
    private EvictionOutcome tryDecodeEviction(BalanceContext ctx,
        ...
        if (proposal == null) {
            ...
            future.complete(placementFailure);
            return EvictionOutcome.FINISHED;
        }
        reportEvictionPlan(ctx.getPriority(), ctx.getRequestId(),
        ...
        synchronized (future) {
          ...
          if (!presence.success()) {
              ...
              Map<String, Object> diagnostics = new HashMap<>(ctx.getSchedulingDiagnostics());
              diagnostics.put("cause", "decode victim reservations changed");
              diagnostics.put("endpoint", proposal.endpointId());
              diagnostics.put("plannedVictims", reservedVictimIds.size());
              diagnostics.put("freedVictims", presence.freedVictimIds().size());
              ctx.setSchedulingDiagnostics(Map.copyOf(diagnostics));
    ...
    private EvictionOutcome placeAfterDecodeEviction(BalanceContext ctx,
        ...
        PrefillEndpoint prefillEp = prefill != null && prefill.isSuccess()
                ? endpointRegistry.getPrefill(prefill.getServerIp() + ":" + prefill.getHttpPort()) : null;
        if (prefill != null && prefill.isSuccess() && prefillEp == null) {
            prefill = ServerStatus.code(StrategyErrorType.NO_PREFILL_WORKER,
                    "prefill endpoint not registered after decode eviction");
        }
        if (prefill == null || !prefill.isSuccess()) {
            if (prefill == null) {
                prefill = ServerStatus.code(StrategyErrorType.BATCH_DISPATCH_FAILED,
                        "Prefill selection returned no decision after decode eviction");
            }
            prefill.setRole(RoleType.PREFILL);
            prefill.setGroup(decodeEp.getStatus().getGroup());
            Response failure;
            try {
                failure = DefaultRouter.buildFailureResponse(endpointRegistry, ctx, prefill);
            } finally {
                decodeEp.release(ctx.getRequestId());
            }
            future.complete(failure);
    ...
    private static boolean isDecodeCapacityFailure(Response response) {
        return response != null && !response.isSuccess() && response.getFailedRole() == RoleType.DECODE
                && StrategyErrorType.fromErrorCode(response.getCode()).isCapacityRejection();
    }
