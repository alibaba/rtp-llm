...
public class BatchSchedulerReporter {
     ...
     * ledger by the TTL cleanup task (plus the orphan decode reservation
     * reclaim) via {@code app.flexlb.inflight.ttl.expired.qps}.
     ...
     * scheduler-level evictions as an endpoint series — plus a reason
     * bucket, keeping one tag schema with the endpoint series so a single
     * role='*' grouping compares scheduler and endpoint ledgers.
     *
     * @param reason eviction reason bucket ({@code ttl} for the scheduler
     *               inflight TTL cleanup, {@code orphan_reservation} for the
     *               shadow decode reservation reclaim)
     ...
     * (PrefillEndpoint/DecodeEndpoint/PDFusion TTL eviction, plus the
     * scheduler-side orphan decode reservation reclaim) via the same
     ...
     * bucket is {@code ttl}; the orphan decode reservation reclaim reports
     * {@code orphan_reservation} against the owning decode worker; only
     * non-zero counts are reported.
     */
