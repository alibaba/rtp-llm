 ...
 * identity, normalized priority, SLO-derived deadline and KV demand.
 *
 ...
 * @param requestSloMs     per-request SLO in ms (seqLen bucket x priority multiplier)
 * @param deadlineMs       latest admission deadline (epoch ms):
 *                         arrival + SLO - predicted prefill time on the target endpoint
