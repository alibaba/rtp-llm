...
public class GrpcWorkerStatusRunner implements Runnable {
                    ...
                    .whenCompleteAsync((response, ex) -> {
                        try {
                            if (ex != null) {
                                ...
                                recordStatusCheckFailure(throwable);
                            } else {
                            ...
                            workerStatus.endStatusPoll();
                        }
                    }, callbackExecutor)
                    .exceptionally(callbackSchedulingFailure -> {
                        // whenCompleteAsync may be rejected before its finally
                        // block runs; release the generation-local poll slot.
                        workerStatus.endStatusPoll();
                        logger.error("Worker status callback was not scheduled for {}",
    ...
    private void handleStatusResponse(WorkerStatusResponse newWorkerStatus, long startTime) {
        try {
            ...
            try {
                ...
                if (newWorkerStatus.getRole() != roleType) {
                    endpointToRetire = WorkerGenerationRetirement.begin(
                            workerStatus, endpointRegistry, roleType, ipPort);
                    generationRetiring = true;
                    throw new IllegalStateException(
                            "Worker status role does not match discovery role: expected="
                                    + roleType + ", actual="
                                    + newWorkerStatus.getRole());
                }
                WorkerStatus.StatusObservation observation =
                        workerStatus.freezeStatusResponse(newWorkerStatus);
    ...
    private void projectScheduler(EndpointStatusReduction reduction) {
        endpointEventSink.onEndpointEvent(
