...
final class PrefillWorkRegistry implements PrefillWorkLedger {
    ...
    /**
     * Thread-confined rollback right for one provisional DIRECT registration.
     * The capability is bound to the canonical RequestEntry identity, so its
     * close can never remove a later request which reused the same id.
     */
    private final class DirectRegistrationImpl
        ...
        private DirectRegistrationImpl(RequestEntry rollbackEntry) {
            this.rollbackEntry = Objects.requireNonNull(
        ...
        public void close() {
            requireOwnerThread();
            RequestEntry entry = rollbackEntry;
            if (entry == null) {
                return;
            }
            rollbackEntry = null;
        ...
        private void requireOwnerThread() {
            if (Thread.currentThread() != ownerThread) {
                throw new IllegalStateException(
                        "DIRECT registration must remain on its routing thread");
            }
        }
    }
    ...
    private final class RouteLease extends CapacityLease
        ...
        public List<CommittedHandoff> commitGroupUnderLock(
            ...
            List<RouteReservation> reservations = List.copyOf(exactReservations);
            if (reservations.isEmpty() || reservations.get(0) != this) {
    ...
    private static final class BatchWork {
        ...
        private BatchWork(BatchLease lease,
            ...
            this.originalFeatures = Objects.requireNonNull(
    ...
    /** Read-only outcome fully materialized before status ownership mutates. */
    private static final class PreparedStatusReconciliation
            implements StatusReconciliation {
        private final List<WorkerStatusFact> schedulerFacts;
        private final List<BatchCompletion> batchCompletions;
        private Throwable publicationFailure;

        private PreparedStatusReconciliation(
                List<WorkerStatusFact> schedulerFacts,
                List<BatchCompletion> batchCompletions) {
            this.schedulerFacts = List.copyOf(schedulerFacts);
            this.batchCompletions = List.copyOf(batchCompletions);
        }

        @Override
        public List<WorkerStatusFact> schedulerFacts() {
            return schedulerFacts;
        }

        @Override
        public List<BatchCompletion> batchCompletions() {
        ...
        private void publicationFailed(Throwable failure) {
    ...
    private static final class RequestEntry {
        ...
        private void commitBatch(BatchWork work) {
            ...
            batchWork = Objects.requireNonNull(work, "batchWork");
            committedItem = activeItem;
            reservation = null;
            activeItem = null;
        }
    ...
    private List<CommittedHandoff> commitRoutesUnderLock(
        ...
        List<RouteLease> exactLeases = List.copyOf(leases);
        if (members.size() != exactLeases.size()) {
            throw new IllegalArgumentException(
        ...
        for (int index = 0; index < members.size(); index++) {
            ...
            RouteLease lease = Objects.requireNonNull(
                    exactLeases.get(index), "route lease");
        ...
        for (int index = 0; index < members.size(); index++) {
            RouteLease lease = exactLeases.get(index);
    ...
    private List<BatchItem> validateActiveGroup(List<BatchItem> items) {
        List<BatchItem> members = List.copyOf(items);
        assert !members.isEmpty() : "committed group requires members";
        for (BatchItem item : members) {
            RequestEntry entry = requests.get(item.requestId());
            assert entry != null && entry.activeIdentity(item)
            ...
            if (entry == null || entry.isActive()
                ...
                if (entry.protection != null) {
                    ...
                    BatchReduction reduction = entry.batchWork == null
                            ? null
                            : batchReductionsUnderLock().get(entry.batchWork);
    ...
    public List<BatchCompletion> releaseProtection(
        ...
        if (!(Objects.requireNonNull(
                exactProtection, "exactProtection")
        ...
        Objects.requireNonNull(repredictor, "repredictor");
     ...
     */
    private PreparedStatusReconciliation prepareStatusReconciliationUnderLock(
            RoleType role,
        ...
        for (WorkerStatus.TaskObservation task : activeTasks.values()) {
            ...
            if (entry != null && !entry.isActive()
                ...
                schedulerFacts.add(new ActiveWorkerStatusFact(
                        entry.committedItem));
            }
        }
        ...
        return new PreparedStatusReconciliation(
                schedulerFacts, completions);
    }
    ...
    private List<BatchPredictionUpdate> prepareBatchPredictionsUnderLock(
        ...
        for (BatchReduction reduction : changedBatches) {
            ...
            OptionalLong prediction;
            try {
                prediction = Objects.requireNonNull(
                        repredictor.apply(deliveryItems(survivors)),
                        "batch repredictor result");
    ...
    private StatusReconciliation reconcileEngineStatus(
        ...
        Objects.requireNonNull(finishedTasks, "finishedTasks");
        Objects.requireNonNull(repredictor, "repredictor");
        Objects.requireNonNull(
                committedPublication, "committedPublication");
        Objects.requireNonNull(failedReduction, "failedReduction");
        boolean capacityReleased = false;
        boolean canonicalMutationStarted = false;
        PreparedStatusReconciliation outcome = null;
        lock.lock();
        try {
            long nowMs = clock.getAsLong();
            IdentityHashMap<BatchWork, BatchReduction> reductions =
    ...
    public int evictExpiredIndividuals(
        ...
        Objects.requireNonNull(schedulerOwnsRequest, "schedulerOwnsRequest");
        int evicted = 0;
        boolean capacityReleased = false;
        lock.lock();
        try {
    ...
    private void moveGenerationHandoffToOwnedUnderLock(
        ...
        requireLock();
        Objects.requireNonNull(lease, "lease");
        Objects.requireNonNull(committedHandoff, "committedHandoff");
        if (lease.state != LeaseState.OPEN
                || lease.generationHandoff == null
