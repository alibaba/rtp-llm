...
public class ExpirationCleaner {
    ...
    public ExpirationCleaner(EndpointRegistry endpointRegistry, ConfigService configService) {
        this(endpointRegistry, resolveWorkerTimeoutUs(configService), VitStatusConfig.WORKER_TIMEOUT_US);
    }
    ...
    ExpirationCleaner(EndpointRegistry endpointRegistry, long workerTimeoutUs, long vitWorkerTimeoutUs) {
        this.endpointRegistry = endpointRegistry;
        this.workerTimeoutUs = workerTimeoutUs;
        this.vitWorkerTimeoutUs = vitWorkerTimeoutUs;
        logger.info("ExpirationCleaner config: workerTimeoutUs={}, vitWorkerTimeoutUs={}",
                workerTimeoutUs, vitWorkerTimeoutUs);
    }
    ...
    public void doClean(Map<String, WorkerStatus> workerStatusMap, RoleType role) {
        ...
        for (Iterator<Map.Entry<String, WorkerStatus>> it = workerStatusMap.entrySet().iterator(); it.hasNext(); ) {
            ...
            // Use the VIT-specific stale-status window for VIT workers so a transient
            // status timeout does not evict the aggregate proxy endpoint prematurely.
