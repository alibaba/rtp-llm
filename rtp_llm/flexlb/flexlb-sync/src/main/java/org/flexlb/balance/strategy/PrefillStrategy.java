...
/**
 * Shared candidate discovery, policy evaluation, and result materialization for cost-based
 * Prefill placement. The public contract supports single-request and batch call shapes while
 * keeping the candidate and planning representations package-scoped.
 */
public abstract class PrefillStrategy {
    ...
    private final AtomicLong equalTtftCursor;
    private final AtomicLong equalCacheLeaderCursor;

    protected PrefillStrategy(WorkerDirectory workerDirectory,
                              CacheAwareService cacheAwareService,
                              EngineHealthReporter engineHealthReporter) {
        this.workerDirectory = workerDirectory;
        this.cacheAwareService = cacheAwareService;
        this.engineHealthReporter = engineHealthReporter;
        this.equalTtftCursor = new AtomicLong();
        this.equalCacheLeaderCursor = new AtomicLong();
    }

    /**
     * One Prefill request and its already resolved routing group.
     */
    public record BatchRequest(BalanceContext context,
                               RoleType roleType,
                               String group) {
        public BatchRequest {
    ...
    /**
     * Select one request with the shared cost-based candidate flow.
     */
    public final PlacementResult<SelectedRole, RoleType> select(BalanceContext balanceContext,
                                                                RoleType roleType,
                                                                String group) {
        return selectOne(balanceContext, roleType, group);
    }

    /**
     * Select a list independently. The batched strategy overrides this method with joint
     * planning; this implementation defines the single-strategy behavior behind the same API.
     */
    public List<PlacementResult<SelectedRole, RoleType>> selectBatch(List<BatchRequest> requests) {
        Objects.requireNonNull(requests, "requests");
        List<PlacementResult<SelectedRole, RoleType>> results =
        ...
        try {
            ...
            for (int i = results.size() - 1; i >= 0; i--) {
                SelectedRole selection = results.get(i).value();
                if (selection == null) {
                    continue;
                }
                try {
    ...
    /**
     * Execute the common single-request candidate evaluation and materialization flow.
     */
    protected final PlacementResult<SelectedRole, RoleType> selectOne(BalanceContext balanceContext,
                                                                      RoleType roleType,
                                                                      String group) {
        ...
        PrefillCandidateSet survivors = evaluateCandidates(
                ...
                cacheMatchResult,
                rejections,
                poolWideBlockers, CANDIDATE_SCRATCH.get());
        if (survivors.size() == 0) {
            Logger.debug(
        ...
        return materialize(balanceContext, roleType, group, discovery, survivors,
                cacheMatchResult, rejections, selectedIndex, modeledSelection
                        ? OptionalLong.of(survivors.projectedTtftMs(selectedIndex))
                        : OptionalLong.empty());
    }

    final PlacementResult<SelectedRole, RoleType> materialize(
            BalanceContext balanceContext, RoleType roleType, String group,
            EndpointDiscovery discovery, PrefillCandidateSet survivors,
            CacheMatchResult cacheMatchResult, Map<String, Integer> rejections,
            int selectedIndex, OptionalLong selectedTtft) {
        String requestId = balanceContext.getRequestId();
        long seqLen = balanceContext.getRequest().getSeqLen();
        FlexlbConfig config = balanceContext.getConfig();
    ...
    final void recordDecision(BalanceContext context,
                              RoleType role,
                              String group,
                              int totalWorkers,
     ...
     */
    static int selectUnmodeledCandidate(PrefillCandidateSet candidates) {
        int selectedIndex = -1;
    ...
    static double normalizedHitRate(double configuredRate) {
    ...
    final PrefillCandidateSet evaluateCandidates(
            ...
            Map<String, Integer> rejections,
            Map<RoleType, Integer> poolWideBlockers,
            PrefillCandidateSet.Scratch scratch) {
        org.flexlb.dao.loadbalance.Request request = balanceContext.getRequest();
    ...
    private static RoutingConfig.OutlierRejectionConfig outlierRejection(
        ...
        return switch (candidateChoice.getType()) {
            case RANDOM_WITHIN_TOLERANCE, BEST_ONLY -> candidateChoice.getOutlierRejection();
            case LEAST_RECENTLY_USED_IN_POOL -> null;
        };
    }

    final EndpointDiscovery discoverAliveEndpoints(
            RoleType roleType,
            String group) {
    ...
    final CacheMatchResult getCacheMatchResult(
            BalanceContext balanceContext, RoleType roleType, String group) {
        org.flexlb.dao.loadbalance.Request request = balanceContext.getRequest();
    ...
    private CacheTokenMatch calculateCacheMatch(
            PrefillEndpoint ep,
            CacheMatchResult cacheMatchResult,
            org.flexlb.dao.loadbalance.Request request,
            FlexlbConfig config) {
