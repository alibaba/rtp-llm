...
final class PlacementWaitRegistry implements AutoCloseable {
    ...
    interface Work {
        ...
        /** Empty when finished; otherwise the exact resource still blocking. */
        Optional<PlacementKey> attempt();
    ...
    Handle park(
        ...
        synchronized (monitor) {
            ...
            if (rejected || work.done()) {
                ...
                size++;
                waitOn(entry, blocker, observedAvailabilitySequence);
            }
        }
        if (rejected) {
    ...
    private void runLane(PlacementKey key, WaitLane lane) {
        while (true) {
            ...
            Optional<PlacementKey> result = execute(candidate.entry());
    ...
    private Optional<PlacementKey> execute(Entry entry) {
    ...
    private boolean complete(
            Candidate candidate, Optional<PlacementKey> result) {
        synchronized (monitor) {
            ...
            if (entry.closed || entry.work.done() || result.isEmpty()) {
                discardUnderLock(entry);
                return !closed;
            }

            PlacementKey blocker = result.orElseThrow();
            waitOn(
                    entry,
                    blocker,
