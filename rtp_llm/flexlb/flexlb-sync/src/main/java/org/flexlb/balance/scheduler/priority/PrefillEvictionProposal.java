...
public record PrefillEvictionProposal(
    ...
    /**
     * Cross-endpoint plan preference (design doc 9.4, smaller = better):
     * netCost asc → rawCost asc → victimCount asc → cacheHitTokens desc →
     * minVictimPriority asc → maxVictimDeadline desc → endpointId asc.
     */
    public static final Comparator<PrefillEvictionProposal> ORDER = Comparator
            .comparingLong(PrefillEvictionProposal::netCost)
            ...
            .thenComparingInt(p -> p.cost().minVictimPriority())
