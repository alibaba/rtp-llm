...
public final class GroupPlanner {
    ...
    /** Synchronous prediction over the exact candidate list being considered. */
    @FunctionalInterface
    public interface GroupPredictor<T> {
        double predict(List<T> items);
    }
    ...
    private static <T> double validatedPrediction(
            GroupPredictor<T> predictor, List<T> items) {
        return requireValidPrediction(predictor.predict(items));
    }
