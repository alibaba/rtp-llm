...
public final class WorkerGenerationRetirement {
    ...
    public static EndpointRegistry.DetachedGeneration begin(
        ...
        if (endpointRegistry != null) {
            WorkerEndpoint expected = endpointRegistry.get(
                    role, ipPort, status);
            detached = endpointRegistry.detach(
                    role, ipPort, status);
    ...
    private static void finalizeRetirement(
        ...
        try {
            if (!cacheAwareService.retireEngineGeneration(
                    ipPort, role, status.getGenerationId())) {
                logger.debug(
                        "Exact cache generation already absent or superseded for {}#{}",
                        ipPort, status.getGenerationId());
            }
        } catch (Throwable cacheFailure) {
            logger.warn(
                    "Exact cache cleanup failed after retiring generation {} for {}",
                    status.getGenerationId(), ipPort, cacheFailure);
        }
    }
}
