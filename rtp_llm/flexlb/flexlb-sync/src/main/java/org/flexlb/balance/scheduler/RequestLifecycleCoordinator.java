...
import org.flexlb.balance.eviction.AdmissionFailureClassifier;
import org.flexlb.balance.eviction.EngineCancelChannel;
import org.flexlb.balance.eviction.VictimLifecyclePort;
 ...
 */
@Component
final class RequestLifecycleCoordinator implements EndpointRequestRuntime,
        AdmissionLifecyclePort,
        PreemptionLifecyclePort,
        VictimLifecyclePort,
        InflightCommitPort,
        RequestExpirationController.Lifecycle,
        RequestExpirationOrchestrator.Lifecycle,
        RequestMetricsOrchestrator.Lifecycle,
        RequestShutdownOrchestrator.Lifecycle {
    ...
    private final AdmissionMutationSink admissionMutationSink =
            new AdmissionMutationSink() {
                ...
                public void terminate(
                        ...
                        Response failure) {
                    terminateAdmissionMutation(slot, exact, failure);
                }
            };
    /** Stateless translation from endpoint-owned facts to exact RequestSlot actions. */
    private final EndpointEventProjector endpointEvents;

    @Autowired
    RequestLifecycleCoordinator(ConfigService configService,
        ...
        this.expirationController = new RequestExpirationController(
                this,
        ...
        this.completionPublisher = new RequestCompletionPublisher(
                new RequestCompletionPublisher.Terminalizer() {
                    @Override
                    public RequestSlot.PublicationPermit terminalizeResponse(
                            RequestSlot exactSlot, Response response) {
                        return publishExternalResponse(exactSlot, response);
                    }

                    @Override
                    public RequestSlot.PublicationPermit terminalizeFailure(
                            RequestSlot exactSlot, Throwable error) {
                        return publishExternalFailure(exactSlot, error);
                    }

                    @Override
                    public RequestSlot.PublicationPermit terminalizeCancellation(
                            RequestSlot exactSlot) {
                        return publishExternalCancellation(exactSlot);
                    }
                });
        this.engineFenceCoordinator = new EngineFenceCoordinator(
                engineCancelChannel, this::onEngineFenceTombstoned);
        this.endpointEvents = new EndpointEventProjector(this);
    }

    private RequestExpirationController expiration() {
    ...
    void attachRequestExpiration(
        ...
        expiration().attachRequestDeadline(
    ...
    private void completeAdmissionMutation(
            RequestSlot slot,
            AdmissionMutation exact) {
        try {
    ...
    private void terminateAdmissionMutation(
            RequestSlot slot,
            AdmissionMutation exact,
            Response failure) {
        try {
    ...
    @Override
    public PreemptionClaim claimForPreemption(
        ...
        synchronized (entry) {
            return entry.tryInstallPreemption(
                    reservationToken, attemptToken, detail);
        }
    }

    @Override
    public boolean releasePreemptionClaim(PreemptionClaim claim) {
        return reducePreemptionClaim(
                claim,
                claim == null ? null
                        : new RequestSlot.PreemptionFact.Released(claim),
                false);
    }

    @Override
    public boolean markPreemptionCancelInFlight(PreemptionClaim claim) {
        return reducePreemptionClaim(
                claim,
                claim == null ? null
                        : new RequestSlot.PreemptionFact.CancelStarted(claim),
                false);
    }

    @Override
    public boolean markPreemptionCancelAccepted(PreemptionClaim claim) {
        return reducePreemptionClaim(
                claim,
                claim == null ? null
                        : new RequestSlot.PreemptionFact.CancelAccepted(claim),
                false);
    }

    @Override
    public boolean markPreemptionNotFound(PreemptionClaim claim) {
        return reducePreemptionClaim(
                claim,
                claim == null ? null
                        : new RequestSlot.PreemptionFact.CancelNotFound(claim),
                false);
    }

    @Override
    public boolean markPreemptionUnknown(PreemptionClaim claim) {
        return reducePreemptionClaim(
                claim,
                claim == null ? null
                        : new RequestSlot.PreemptionFact.CancelUnknown(claim),
                false);
    }

    @Override
    public boolean finishTombstoned(PreemptionClaim claim, String detail) {
        return reducePreemptionClaim(
                claim,
                claim == null ? null
                        : new RequestSlot.PreemptionFact.Tombstoned(
                                claim, detail),
                true);
    }

    private boolean reducePreemptionClaim(
            PreemptionClaim claim,
            RequestSlot.PreemptionFact fact,
            boolean releasePrefillCounterpart) {
        if (claim == null || fact == null) {
            return false;
        }
        ...
        PreemptionWork work;
        synchronized (entry) {
            Runnable counterpartCleanup = releasePrefillCounterpart
                    ? exactPrefillCounterpartCleanup(entry.activeItem())
                    : null;
            work = reducePreemptionFactLocked(
                    entry, fact, counterpartCleanup);
        }
        consumePreemptionWork(entry, work);
    ...
    private PublicationWork materializeReplayLocked(
        ...
        if (payload instanceof RequestSlot.ReplayPayload.Terminal terminal) {
            ...
            if (exact.kind() == DeferredTerminalKind.PRIORITY) {
            ...
            return applyOrdinaryTerminalLocked(
                    entry,
                    exact,
                    terminal.endpointAlreadyRetired()
                            || exact.endpointAlreadyRetired());
        }
        ...
        if (work.publication() != null) {
            try {
                ...
                if (work.terminalSignal() != null) {
                    work.terminalSignal().terminal().complete(
    ...
    record PreemptionWork(
            ...
            CancelTarget target,
            PreemptionClaim terminalSignal,
    ...
    @Override
    public CancelTarget resolveCancelTarget(
    ...
    private void startFence(
        ...
        if (target == null) {
            synchronized (entry) {
                RequestSlot.FenceReduction reduction = entry.applyFenceFact(
                        new RequestSlot.FenceFact.AwaitTerminal(fence));
    ...
    private FenceEndpointSettlement settleFenceEndpointTerminalLocked(
        ...
        PreemptionClaim transferred = proof.transferred();
    ...
    // ==================== Completion from endpoint-owned facts ====================

    /**
     * Synchronously project an already-committed endpoint event. The projector
     * contains every stale/missing race and never retains an event for replay.
     */
    ...
    private PublicationWork applyOrdinaryTerminalLocked(
        ...
        if (endpointAlreadyRetired || terminal.endpointAlreadyRetired()) {
            return applyDecodeRetirementTerminalLocked(entry, terminal);
        }
        switch (terminal.kind()) {
            case ADMISSION_CLEANUP -> {
                String detail = terminal.detail() == null
                        ? "external future terminal" : terminal.detail();
                return PublicationWork.terminal(beginTerminalLocked(
                        entry, true, true,
                        owner -> owner.fail(detail), null));
            }
            case FAILURE -> {
                return PublicationWork.terminal(beginTerminalLocked(
                        entry, true, terminal.releasePrefillAccounting(),
                        owner -> owner.fail(terminal.detail()),
                        buildErrorResponse(
                                terminal.errorType(), terminal.detail())));
            }
            ...
            case TIMEOUT -> {
                return PublicationWork.terminal(
                        timeoutEntry(entry, terminal.detail()));
            }
            case WORKER -> {
                return applyWorkerTerminalLocked(entry, terminal.workerObservation());
            }
            case PRIORITY, DECODE_GENERATION_RETIRED ->
                    throw new IllegalStateException(
                            "terminal kind requires its typed reducer: "
                                    + terminal.kind());
        }
        throw new IllegalStateException("Unhandled terminal kind " + terminal.kind());
    }

    private PublicationWork applyDecodeRetirementTerminalLocked(
            RequestSlot entry,
            DeferredTerminal terminal) {
        return applyDecodeSettledTerminalLocked(entry, terminal);
    }
    ...
    private PublicationWork applyDecodeSettledTerminalLocked(
        ...
        return PublicationWork.terminal(beginTerminalLocked(
                ...
                buildErrorResponse(terminal.errorType(), detail)));
    }
    ...
    private boolean reduceStaleSlot(
        ...
        synchronized (exactSlot) {
            if (!isCurrentSlot(exactSlot)
                    ...
                    || nowMs - exactSlot.createdAtMs() <= staleTtlMs
                    || exactSlot.hasCancellationFirstCause()) {
                return false;
            }
        }
        RequestLifecycleSnapshot reduced = cancelRequest(
                exactSlot.requestId(), 0L,
                CancelReason.DEADLINE_EXCEEDED);
        return reduced != null
                && (reduced.state()
                        == RequestLifecycleState.CANCEL_REQUESTED
                    || reduced.state().isTerminal());
    }
    ...
    @Override
    public void onOfferFailure(DeliveryItem exactItem, Throwable error) {
        ...
        if (entry != null) {
            PreemptionWork work;
            synchronized (entry) {
                work = reduceDeferredTerminalFactLocked(entry, DeferredTerminal.failure(
                        errorType,
    ...
    @Override
    public void onDeliveryFailure(DeliveryItem exactItem, Throwable error) {
    ...
    @Override
    public SlotDeliveryPort.Claim tryCommit(
            DeliveryItem exactItem,
        ...
        synchronized (exact.slot) {
            ...
            if (completion == SlotDeliveryPort.Completion.Delivered.INSTANCE) {
                work = switch (exact.identity.boundary()) {
                    ...
                    instanceof SlotDeliveryPort.Completion.Failed failed) {
                String detail = "Delivery failed: "
                        + detailOf(failed.cause());
                ...
                String detail = switch (completion) {
                    case SlotDeliveryPort.Completion.TimedOut timedOut ->
                            "Delivery timed out: "
                                    + detailOf(timedOut.cause());
                    case SlotDeliveryPort.Completion.Uncertain uncertain ->
                            "Delivery outcome uncertain: "
                                    + detailOf(uncertain.cause());
                    case SlotDeliveryPort.Completion.Delivered ignored ->
                            throw new IllegalStateException(
                                    "delivered outcome was already handled");
                    case SlotDeliveryPort.Completion.Failed ignored ->
                            throw new IllegalStateException(
    ...
    private void armAcceptanceDeadline(RequestSlot entry) {
        ...
        if (delay.isPresent()) {
            expiration().registerAcceptanceDeadline(
    ...
    private TerminalAction timeoutEntry(RequestSlot entry, String detail) {
        AdmissionFailure admissionFailure = null;
        BatchItem item = entry.activeItem();
        PrefillEndpoint prefill = item == null ? null : item.prefillEp();
        if (entry.wasPriorityAdmission() && item != null) {
            admissionFailure = classifyAdmissionTimeout(item, prefill);
        }
        Response response;
        if (admissionFailure != null) {
            ...
            response = buildAdmissionErrorResponse(admissionFailure, detail);
        } else {
            response = buildErrorResponse(entry.timeoutErrorType(), detail);
        }
        return beginTerminalLocked(
                entry, true, true,
                owner -> owner.timeout(detail), response);
    }
    ...
    private static AdmissionFailure classifyAdmissionTimeout(BatchItem item,
        ...
        List<DeliveryItem> ahead = new ArrayList<>();
        for (DeliveryItem queued
                : prefill.captureQueueSnapshot().items()) {
            if (queued == item) {
    ...
    private RequestSlot.PublicationPermit publishExternalResponse(
        ...
        TerminalTransition transition = response != null && !response.isSuccess()
                ? owner -> owner.fail(detail)
    ...
    private RequestSlot.PublicationPermit publishExternalFailure(
    ...
    private RequestSlot.PublicationPermit publishExternalCancellation(
            RequestSlot slot) {
    ...
    private TerminalAction claimExternalLocalTerminal(
            RequestSlot slot, TerminalTransition transition) {
        synchronized (slot) {
    ...
    public void maintainExpiration(
        ...
        expirationController.maintain(exactSweeper);
    }
