...
public interface BatchSubmissionPort {
    ...
    CapacityBoundary.Attempt<PreparedSubmission> prepare();

    /**
     * One prepared dispatcher admission.
     *
     * <p>A successful {@link #submit(Command, Observer)} moves ownership to the
     * dispatcher. The subsequent try-with-resources {@link #close()} must then
     * be a no-op. A repeated or otherwise illegal submit must throw.
    ...
    interface PreparedSubmission extends AutoCloseable {
        ...
        void submit(Command command, Observer observer);
    ...
    /** One terminal transport observation for one exact submitted item. */
    @FunctionalInterface
    interface Observer {
