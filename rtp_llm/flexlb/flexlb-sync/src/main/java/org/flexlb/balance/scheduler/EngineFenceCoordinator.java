...
final class EngineFenceCoordinator {
    ...
    TerminalDisposition resumeTombstoned(
        ...
        synchronized (slot) {
            reduction = slot.applyFenceFact(
                    new RequestSlot.FenceFact.Tombstoned(exactFence));
        }
    ...
    private static void validateDisposition(
        ...
        synchronized (slot) {
            RequestSlot.FenceReduction actual = slot.applyFenceFact(
                    new RequestSlot.FenceFact.Tombstoned(exactFence));
    ...
    private static void awaitAuthoritativeTerminal(
        ...
        synchronized (slot) {
            RequestSlot.FenceReduction reduction = slot.applyFenceFact(
                    new RequestSlot.FenceFact.AwaitTerminal(exactFence));
            if (!(reduction instanceof RequestSlot.FenceReduction.Stale)) {
