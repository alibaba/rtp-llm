    ...
    private static RouteProjection.Result projectWithPredictions(
        ...
        if (committed.hasUnknownWork()) {
            ...
            if (queue.queueScheduling()) {
                ...
                if (!incomingShape.fitsCompute(
                        queue.constraints().batchTokenCapacity())) {
                    return unavailable(
                            "INCOMING_BATCH_TOKEN_CAPACITY_EXCEEDED");
                }
        ...
        while (!ordered.isEmpty()) {
            ...
            if (!headShape.fitsCompute(queue.constraints().batchTokenCapacity())) {
                // A strict compute-shape violation is terminal in the real
                // batcher. Existing invalid work is rejected and cannot block
                // a later probe; an invalid probe itself is not selectable.
                if (head == probe) {
                    return unavailable(
                            "INCOMING_BATCH_TOKEN_CAPACITY_EXCEEDED");
                }
                ordered.removeHead();
                if (head == initialActiveHead) {
                    initialHeadDisposition =
                            RouteProjection.Result.InitialHeadDisposition.TERMINAL_PRUNED;
                }
                continue;
