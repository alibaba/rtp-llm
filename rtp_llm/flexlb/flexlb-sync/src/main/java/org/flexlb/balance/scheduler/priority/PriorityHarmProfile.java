...
public final class PriorityHarmProfile implements Comparable<PriorityHarmProfile> {
    ...
    /** Return the exact aggregated harm for one priority bucket. */
    public BigInteger harmAt(int priority) {
        requireValidPriority(priority);
        BigInteger harm = harmByPriority[priority];
        return harm == null ? BigInteger.ZERO : harm;
    }
