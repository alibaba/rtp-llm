...
class GrpcCacheStatusCheckRunnerTest {
    ...
    void usesWorkerStatusPortForLogicalEngineCacheStatus() {
        ...
        when(engineGrpcService.getCacheStatusAsync(
                ...
                .thenReturn(CompletableFuture.completedFuture(EngineRpcService.CacheStatusPB.newBuilder()
                        .setVersion(1)
                        .build()));

        new GrpcCacheStatusCheckRunner(
                "test-model", host, RoleType.PREFILL, workerStatus, engineHealthReporter,
                engineGrpcService, cacheAwareService, 20, new LongAdder(), 50L, true, Runnable::run)
                .run();
