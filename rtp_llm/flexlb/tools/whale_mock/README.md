...
部署专用标定 JSON 由运行环境提供，不存放在仓库或镜像构建上下文中。
...
Use an external calibration JSON's `master_config` as the complete `FLEXLB_CONFIG`,
...
`MOCK_BUNDLE_LEGACY_MASTER=1` 选择外部指定的生产源码版本；
schema 1 原文由外部配置提供。CI 在临时目录构建该版本，
...
Inner 部署使用外部标定文件的独立规模、容量和性能初值，
`FLEXLB_CONFIG` 则完整读取外部 master 配置。不要使用上面的旧 WLCB
