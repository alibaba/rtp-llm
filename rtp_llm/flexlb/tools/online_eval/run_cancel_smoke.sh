...
AUTO_TPM_PREFILL_MAX_INFLIGHT_REQUESTS_PER_WORKER="${AUTO_TPM_PREFILL_MAX_INFLIGHT_REQUESTS_PER_WORKER:-0}"
# Priority preemption deliberately holds the victim behind a delayed typed
# CANCELED fence. Give that control-plane exercise a stable admission budget;
# production SLO defaults are intentionally much tighter and may change.
AUTO_TPM_SLO_LENGTH_BUCKETS="${AUTO_TPM_SLO_LENGTH_BUCKETS:-*:30000}"
...
if [[ "${START_FLEXLB}" == "1" ]]; then
  ...
  env "${FLEXLB_ENV_ARGS[@]}" \
    ...
    "AUTO_TPM_PREFILL_MAX_INFLIGHT_REQUESTS_PER_WORKER=${AUTO_TPM_PREFILL_MAX_INFLIGHT_REQUESTS_PER_WORKER}" \
    "AUTO_TPM_SLO_LENGTH_BUCKETS=${AUTO_TPM_SLO_LENGTH_BUCKETS}" \
...
PYTHONDONTWRITEBYTECODE=1 python3 "${SCRIPT_DIR}/priority_preemption_smoke.py" \
  ...
  --prefill-request-cap "${AUTO_TPM_PREFILL_MAX_INFLIGHT_REQUESTS_PER_WORKER}" \
