...
DSv4 mock 测试刻度由 `data/performance/dsv4_l20_mock_calibration.json` 保存。`default` 直接读取该文件，`fault_env` 在测试专用的 100ms prefill 覆盖上引用同一份刻度；其状态为 `legacy_unverified`，不是已验证的线上测量。预设注册表通过相对路径选择性能文件或引用刻度，加载时将 decode 数值、模型身份及刻度 SHA 写入运行用性能 JSON；master 默认 prefill 表达式读取同一文件。Java mock jar 将该文件作为资源打包，供无显式 FORMULA 的独立启动使用；LEARNING 在 mock 中使用这份静态近似。采集档案保持自己的记录、哈希与 block 口径，不与该测试刻度共用身份。
