...
本目录集中保存性能压测、采集、分析和报告实现。功能 case 的当前入口见 [工具导航](../README.md)。
...
默认不额外压缩，以免增加压测收尾耗时。`compare_ab.py --html` 输出 `reports/comparison/stress-ab/report.html`，同页包含指标表与时序曲线，图表所需 Chart.js 已固定并内嵌，离线可打开；`--archive`
可把 A、B、比较 JSON 和报告包装进一个档案。
...
as well: its coverage now lives in the `flexlb_test_framework/` functional-test
framework. The retained Python tools (the online_eval aggregation / report /
consolidation scripts and the `flexlb_test_framework/` framework itself) run on the
...
coverage now lives in the `flexlb_test_framework/` functional-test framework
(`../parallel_runner.py --profile batch-window|single-nonbatch|single-batch|window-nonbatch`).
...
- A Linux host or container whose system `python3` provides `grpcio`,
  ...
  layer and the `flexlb_test_framework/` framework are Python).
...
`flexlb_test_framework/README.md` (Quick start); the script only self-builds the
...
```bash
FLEXLB_PROFILE=stress-na130 \
FLEXLB_CONFIG_OVERRIDE=max_collection_wait_ms=200,decode_max_engine_requests=64 \
DURATION_S=300 \
LIMIT=5000 \
REPLAY_SPEED=20 \
N_PREFILL=4 \
...
`FLEXLB_CONFIG` is generated by `flexlb_cfg.py` (the single source of
truth): `FLEXLB_PROFILE` picks the base document — `stress-na130` (the
default; the retired `data/config/master_fixed_window.json` semantics,
field for field) or one of the four functional profiles (`batch-window`,
`single-nonbatch`, `single-batch`, `window-nonbatch`) — and
`FLEXLB_CONFIG_OVERRIDE="k=v,..."` layers per-field overrides on top (the
vocabulary is `flexlb_cfg.ConfigOverride`; the legacy `STRIP_PREEMPTION=1`
call shape is accepted as a `strip_preemption` alias). The script renders
BOTH the master env string and `run/<id>/master_config.json` (the mock
engine's `--master-config` envelope) from the same render, so the two
channels cannot diverge; `FLEXLB_JVM_HEAP_SIZE` (default `32g`) rides the
envelope. An explicitly exported `FLEXLB_CONFIG` still wins for the master
env (escape hatch) — the envelope keeps the rendered document and the
script warns about the possible two-source divergence.

The prefill performance formula is
`router.roles.prefill.executionTimeEstimator.expression`; there is no separate
formula environment variable. The generator injects the production DSv4
prefill fit explicitly on every render (see `flexlb_cfg.DSV4_PREFILL_EXPRESSION`).
...
- `run/<id>/master_config.json`: the per-run `--master-config` envelope,
  ...
  `flexlb_cfg.py`'s stress-na130 profile).
...
their scenarios are now covered by the `flexlb_test_framework/` framework and the upstream
...
First render the master-config envelope (the mock engine requires the
file):

```bash
python3 -c 'import sys; sys.path.insert(0, "rtp_llm/flexlb/tools/online_eval"); from flexlb_cfg import render_process_config; open("rtp_llm/flexlb/tools/online_eval/run/master_config.json", "w").write(render_process_config("stress-na130", None))'
```
...
- **engine key-level (theoretical)** — "matched keys / requested keys":
  ...
  requests contribute 0/0 by construction). This is a mock key counter;
  real `recent_cache_key_hit_count / total_count` are request-level token
...
`parallel_runner.py`, `flexlb_test_framework/{instance_runner,instance_plan,resource_plan}.py`
