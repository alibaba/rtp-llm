...
# Chart.js 引用方式：
#   * 默认走 jsDelivr CDN（未找到本地库文件时，保持旧行为）；
#   * 脚本同目录（或 aggregate 同目录）存在 chart.umd.min.js 时整体内联进
#     <script>，产物离线双击即可打开（真 self-contained）。
CHARTJS_CDN_TAG = (
...
def resolve_chartjs_tag(extra_candidates=()):
    """返回 (<script> 标签, 库文件路径或 None)：内联优先，否则 CDN。"""
    here = os.path.dirname(os.path.abspath(__file__))
    candidates = list(extra_candidates) + [
        os.path.join(here, "chart.umd.min.js"),
    ]
    for cand in candidates:
        if cand and os.path.isfile(cand):
            with open(cand, encoding="utf-8") as f:
                src = f.read()
            if len(src) > 1000 and "Chart" in src:  # 防 404 页面误当库文件
                return "<script>" + src + "</script>", cand
    return CHARTJS_CDN_TAG, None
...
def render(spec, chartjs_tag=None):
    """spec: 见模块 docstring。返回完整 HTML 字符串。

    chartjs_tag: 完整 <script> 标签；None 时自动解析（本地库内联优先，
    否则 CDN，见 resolve_chartjs_tag）。
    """
    ...
    if chartjs_tag is None:
        chartjs_tag, _src = resolve_chartjs_tag()
    return (
