...
from flexlb_eval.runtime.historical_master import MANIFEST as HISTORICAL_MASTER, adapt_env

# ---------------------------------------------------------------------------
# Paths
# ---------------------------------------------------------------------------

TOOL_DIR = Path(__file__).resolve().parents[3]  # rtp_llm/flexlb/tools/online_eval
FLEXLB_DIR = Path(__file__).resolve().parents[5]  # rtp_llm/flexlb (maven root)
REPO_ROOT = Path(__file__).resolve().parents[7]  # repo root
...
# import robust regardless of how flexlb_eval.runtime.harness itself was imported.
#
...
from flexlb_eval.runtime.resource_plan import VICTIM_OFFSETS
...
def port_accepting(port: int, host: str = "127.0.0.1") -> bool:
    """Post-start liveness probe; SO_REUSEADDR bind probes can miss listeners on macOS."""
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
        sock.settimeout(0.5)
...
def _http_get_text(url: str, timeout: float = 5.0) -> Optional[str]:
    from flexlb_eval.monitoring.telemetry import http_text
...
class BalanceSampler:
    ...
    def _poll_mock(self, t_rel: float) -> None:
        from flexlb_eval.monitoring.telemetry import shared_samples_since
...
# ---------------------------------------------------------------------------
# Proto bootstrap (translated from flexlb_eval/runtime/proto_utils.py)
# ---------------------------------------------------------------------------
