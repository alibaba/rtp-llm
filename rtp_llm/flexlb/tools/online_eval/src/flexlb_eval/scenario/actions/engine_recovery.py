...
from flexlb_eval.scenario.actions import cancel, elastic
from flexlb_eval.scenario.actions import status_protocol as status
from flexlb_eval.scenario.actions.engine_control import ENGINE_NAME
...
def validate_observe(params, plan):
    ...
    if "keys" in p:
        from flexlb_eval.scenario.actions.kv import _keys
...
def execute_cache_control(ctx, params, deadline):
    from flexlb_eval.scenario.actions.engine_control import _http
...
def validate_apply(params, plan):
    ...
    plan.reference(p["selection"], "snapshot")
    from flexlb_eval.scenario.actions.engine_control import validate
...
def execute_apply(ctx, params, deadline):
    from flexlb_eval.scenario.actions.engine_control import execute
...
def execute_arm(ctx, params, deadline):
    from flexlb_eval.scenario.actions.engine_control import _http
    from flexlb_eval.scenario.actions.engine_fault import inject as execute_inject
