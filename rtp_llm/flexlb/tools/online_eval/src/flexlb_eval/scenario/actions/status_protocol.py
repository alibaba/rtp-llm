...
from flexlb_eval.scenario.actions.observation import FrozenSnapshot
...
def metric(frame, name):
    if name == "scheduler_tombstones":
        from flexlb_eval.runtime.debug_client import check_scheduler_tombstone
...
def _debug_directory(capture, n_prefill, n_decode):
    from flexlb_eval.runtime.debug_client import DebugUnavailable
...
def execute_outcomes(ctx, params, deadline):
    ...
    if "per_execution_batch" in params:
        from flexlb_eval.scenario.actions.execution_evidence import partial_outcome_bounds, read_events
