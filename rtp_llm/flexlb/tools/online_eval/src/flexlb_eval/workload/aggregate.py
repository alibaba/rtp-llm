...
import gzip
from flexlb_eval.reporting import bundle_path
...
from flexlb_eval.runtime.requests import request_success
from flexlb_eval.analysis.consolidate import parse_grouped_prometheus_timeseries
...
def run_canvas(out, timeout_s):
    with (out / "canvas.json").open("w") as output, (out / "aggregate.stderr").open(
        ...
        run = subprocess.run(
            [sys.executable, str(ROOT / "src/flexlb_eval/analysis/aggregate.py")],
            cwd=out,
            stdout=output,
            stderr=errors,
            timeout=timeout_s,
    ...
    with (out / "render.log").open("w") as log:
        render = subprocess.run(
            [
                sys.executable,
                str(ROOT / "src/flexlb_eval/reporting/stress_report.py"),
                "--aggregate",
                str(out / "canvas.json"),
                "--out",
                str(out),
