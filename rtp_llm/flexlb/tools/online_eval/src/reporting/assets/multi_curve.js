...
(function(root) {
  ...
  function mount(parent, panel, context) {
    ...
    function refresh() {Object.keys(panel.axes).forEach(axis=>{chart.options.scales[axis].display=datasets.some((d,i)=>d.yAxisID===axis&&chart.isDatasetVisible(i));});inputs.forEach((input,i)=>{input.checked=chart.isDatasetVisible(i);plottedLabels[i].classList.toggle('is-hidden',!input.checked);plottedLabels[i].style.order=input.checked?'0':'1';plottedLabels[i].setAttribute('aria-pressed',String(input.checked));});pickerButton.textContent='指标 '+inputs.filter(input=>input.checked).length+'/'+inputs.length+' ▾';if(focused!==null) focus(focused);else chart.update('none');}
