...
def observability_integrity(case):
    ...
    case.step(
        ...
        params=case.params(
            "observability_integrity.observe",
            {
                "placeholder": output("placeholder", "requests"),
                "wave": output("wave", "requests"),
            },
...
def cancel_tombstoned(case):
    ...
    # Fallback contract alignment (degraded coverage — retirement-aligned
    # expectations; see the cancel_tombstoned_fix report): "incoming" feeds
    # the rejected (victim_inflight_gone) verdict instead of the unreachable
    # preempt-and-serve completion, and the cancel census feeds
    # delta_zero / census_stable instead of delta_seen / reached.
    case.step(
        "ts_final",
        "preemption_ts_final",
        params={
            "cut": output("victim_cut", "cut"),
            "incoming": output("incoming_completed", "rejected"),
            "delta_zero": output("cancel_arrival", "delta_zero"),
            "census_stable": output("cancel_arrival", "census_stable"),
...
def cancel_absent_fence(case):
    """Direct engine contract guard; no crash and no master-originated Cancel."""
    case.step(
        "setup", "setup", timeout_s=case.value("cancel_absent_fence.setup_timeout_s")
    )
    case.step(
        "completed",
        "priority_start",
        params=case.value("cancel_absent_fence.completed"),
    )
    case.step(
        "settled",
        "preemption_settled",
    ...
    case.step(
        ...
        "preemption_live_drain",
        params=case.params(
            "cancel_absent_fence.drain", {"requests": output("completed", "requests")}
        ),
    )
    case.step(
        "contract",
