...
from flexlb_ft.scenario.loader import load_document
    ...
    def test_four_profiles_render_ssot_and_no_import_of_process_framework(self):
        ...
        proc = subprocess.run(
            [
                sys.executable,
                "-c",
                "import sys; from flexlb_ft.scenario import compile_scenarios; assert not any(x in sys.modules for x in ['flexlb_ft.harness','flexlb_ft.engine_ops','grpc'])",
            ],
            cwd=TOOLS,
            capture_output=True,
            text=True,
...
class LoaderTest(unittest.TestCase):
    def test_json_yaml_same_compile_plan(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            a.write_text(json.dumps(scenario()))
            b.write_text(yaml.safe_dump(scenario()))
    ...
    def test_cli_reports_compile_mode_and_zero_selection_is_error(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = subprocess.run(
                [sys.executable, "-m", "flexlb_ft.scenario", str(p)],
                cwd=TOOLS,
                capture_output=True,
                text=True,
            )
