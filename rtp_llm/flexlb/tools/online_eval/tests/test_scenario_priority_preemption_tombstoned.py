...
from flexlb_ft.support.priority import _ts_spec
...
class TombstonedPrograms(unittest.TestCase):
    ...
    def run_program(self, profile="single-batch", **kwargs):
        ...
        ), patch("flexlb_ft.scenario.actions.engine_fault._http", backend.http), patch(
            "flexlb_ft.scenario.actions.engine_control._http", backend.http
        ), patch(
            "flexlb_ft.scenario.actions.status_protocol._http", backend.status_http
        ):
    ...
    def test_both_batch_profiles_keep_single_decision_crash_and_fence_contract(self):
        for profile in ("single-batch", "batch-window"):
            with self.subTest(profile=profile):
                ...
                old = _ts_spec(NS(profile=profile))
    ...
    def test_reached_poll_and_final_delta_are_separate_legacy_facts(self):
