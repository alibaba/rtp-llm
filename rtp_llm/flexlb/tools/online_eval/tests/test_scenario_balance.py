...
from flexlb_ft.scenario.actions import balance as b
...
from flexlb_ft.scenario.runtime import Deadline, StageTimeout, execute_instance
...
class BalanceTests(unittest.TestCase):
    ...
    def test_shipped_catalog_six_contracts_all_profiles(self):
        ...
        self.assertEqual(
            len({legacy for p in plans for legacy in p["legacy_case_ids"]}), 6
        )
        for plan in plans:
