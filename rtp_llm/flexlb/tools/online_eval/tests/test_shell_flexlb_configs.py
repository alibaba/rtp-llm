...
class ShellFlexlbConfigTest(unittest.TestCase):
    ...
    def test_shell_and_shipped_master_use_complete_v3_contract(self):
        ...
        for path in SCRIPT_DIR.glob("*.sh"):
            for match in re.finditer(
                r"(?:DEFAULT_FLEXLB_CONFIG|default_flexlb_config|FLEXLB_CONFIG)='(\{[^']+\})'",
                path.read_text(),
            ):
                with self.subTest(path=path.name, config=count):
                    self.assert_v3(json.loads(match.group(1)))
                count += 1
        self.assertGreaterEqual(count, 10)
        for path in (SCRIPT_DIR / "data/config").glob("master*.json"):
    ...
    def test_experiment_config_generators_use_v3_and_preserve_parameters(self):
        import run_inflight_experiment

        self.assert_v3(json.loads(run_inflight_experiment.DEFAULT_FLEXLB_CONFIG))
        profile = str(SCRIPT_DIR / "data/config/master_fixed_window_4g.json")
        for name, args in (
            ("flexlb_behavior_test.sh", ["12345", "7", "1000"]),
            ("run_batcher_experiments.sh", [profile, "7", "160"]),
            ("run_burst_test.sh", [profile, "7", "160", "550", "132"]),
        ):
            with self.subTest(script=name):
                ...
                config = json.loads(
                    subprocess.check_output(
                        [sys.executable, "-", *args], input=code.encode()
                    )
                )
                self.assert_v3(config)
                self.assertEqual(7, config["dispatcher"]["maxInflightPerPrefillWorker"])
                if name == "flexlb_behavior_test.sh":
                    self.assertEqual(
                        12345, config["requestLifecycle"]["request"]["timeoutMs"]
                    )
                else:
                    self.assertEqual(
                        160, config["scheduler"]["decision"]["maxCollectionWaitMs"]
                    )
    ...
    def test_master_env_contains_config_documents_and_role_hosts(self):
        with TemporaryDirectory() as tmp:
            ...
            self.assertEqual(
                {"FLEXLB_CONFIG", "MODEL_SERVICE_CONFIG", "RTP_LLM_TRACE_CONFIG"},
                set(result),
            )
            self.assertEqual(
                {"enabled": False}, json.loads(result["RTP_LLM_TRACE_CONFIG"])
            )
