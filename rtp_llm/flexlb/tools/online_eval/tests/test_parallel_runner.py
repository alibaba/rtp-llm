...
class PlanCaseLanesTest(unittest.TestCase):
    ...
    def test_empty_case_lane_does_not_spawn_full_suite(self):
        args = argparse.Namespace(case_pairs=[], mock_stride=500)
        with mock.patch.object(parallel_runner, "_spawn") as spawn:
            result = parallel_runner._run_case_lane(
                0, [], args, Path("/unused"), "stamp"
            )
        spawn.assert_not_called()
        self.assertEqual([], result.runs)
        self.assertEqual([], result.case_names)
