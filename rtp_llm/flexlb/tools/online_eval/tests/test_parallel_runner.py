...
class BaselineWriteWiringTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        def _drop():
            ...
            parallel_runner._WINDOW_LOCK_FILES = None
            parallel_runner._close_window_locks(holders)
            if parallel_runner._RUN_LOCK_FILE is not None:
                parallel_runner._RUN_LOCK_FILE.close()
                parallel_runner._RUN_LOCK_FILE = None
    ...
    def test_window_then_output_lock_are_held_before_planning(self):
        with tempfile.TemporaryDirectory() as tmp:
            out = Path(tmp)
            argv = ["parallel_runner.py", "--parallel", "1", "--out-dir", str(out)]

            def plan(args):
                self.assertIsNotNone(parallel_runner._WINDOW_LOCK_FILES)
                self.assertIsNone(
                    parallel_runner._try_window_lock(18080, 55151, 500, 1)
                )
                with self.assertRaisesRegex(SystemExit, "held by another"):
                    parallel_runner._acquire_run_lock(out)
                raise SystemExit("plan reached with both locks")

            def acquire(path):
                self.assertIsNotNone(parallel_runner._WINDOW_LOCK_FILES)
                original_acquire(path)

            original_acquire = parallel_runner._acquire_run_lock
            with mock.patch.dict(os.environ, {}, clear=True), mock.patch.object(
                sys, "argv", argv
            ), mock.patch.object(
                parallel_runner, "_plan", side_effect=plan
            ), mock.patch.object(
                parallel_runner, "_acquire_run_lock", side_effect=acquire
            ), mock.patch.object(
                parallel_runner, "run_lane"
            ) as lane:
                with self.assertRaisesRegex(SystemExit, "plan reached with both locks"):
                    parallel_runner.main()
                lane.assert_not_called()

    def test_existing_output_lock_stops_disjoint_window_before_plan(self):
        with tempfile.TemporaryDirectory() as tmp:
            out = Path(tmp)
            sentinel = out / "cases.json"
            sentinel.write_text("existing artifacts")
            parallel_runner._acquire_run_lock(out)
            argv = ["parallel_runner.py", "--parallel", "1", "--out-dir", str(out)]
            with mock.patch.dict(os.environ, {}, clear=True), mock.patch.object(
                sys, "argv", argv
            ), mock.patch.object(parallel_runner, "_plan") as plan:
                with self.assertRaisesRegex(SystemExit, "held by another"):
                    parallel_runner.main()
                plan.assert_not_called()
            self.assertEqual("existing artifacts", sentinel.read_text())
