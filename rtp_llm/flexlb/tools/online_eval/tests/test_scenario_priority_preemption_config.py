...
class ConfigRejectionProgram(unittest.TestCase):
    ...
    def test_three_raw_payloads_match_old_and_all_probes_cleanup(self):
        result, backend, evidence = self.run_program()
        self.assertEqual("PASS", result["status"], result)
        configs = [
            _prio_config(),
            _prio_config(ordering="fifo"),
            _prio_config(preemption=_PREEMPT_DECODE),
        ]
        expected = [json.loads(render_env("single-nonbatch", c)) for c in configs]
