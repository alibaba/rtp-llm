...
from flexlb_ft.cases.elastic import _pending_drain_spec
...
class SingleBatchTests(unittest.TestCase):
    ...
    def test_profile_registration_and_entire_old_config_are_preserved(self):
        plans, _ = self.plans()
        self.assertEqual(
        ...
        for p in plans:
            old = _pending_drain_spec(NS(profile=p["profile"]))
            ...
            self.assertEqual(
                ...
                (
                    []
                    if p["variant_id"] == "zero_errors"
                    else ["elastic_remove_pending_drain"]
                ),
            )
