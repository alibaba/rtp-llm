...
from flexlb_ft.support.priority import _dr_live_spec
...
class ReservedPrograms(unittest.TestCase):
    ...
    def test_both_profiles_preserve_four_block_pool_shapes_and_real_fetch(self):
        for variant in ("decode_reserved_live_single", "decode_reserved_live_window"):
            with self.subTest(variant=variant):
                ...
                old = _dr_live_spec(NS(profile=plan["profile"]))
                ...
                self.assertEqual(4, actual.decode_cache_blocks)
                self.assertEqual(
                ...
                self.assertEqual(
                    [3, 3], [r["schedule_count"] for r in backend.fetches[:2]]
                )
