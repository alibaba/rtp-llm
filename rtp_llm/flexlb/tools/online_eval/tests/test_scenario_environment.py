...
from flexlb_ft.scenario.actions.environment import (
    _validate,
    mutate_config,
    reconfigure,
    startup_probe,
...
from flexlb_ft.scenario.runtime import Deadline, RuntimeContext, _core_action
from flexlb_ft.support.priority import _PREEMPT_DECODE, _prio_config
...
class EnvironmentTest(unittest.TestCase):
    ...
    def test_three_raw_mutations_match_old_payloads(self):
        cases = [
            ("removed_auto_tpm", _prio_config()),
            ("fifo_default_priority", _prio_config(ordering="fifo")),
            ("owned_without_cancellation", _prio_config(preemption=_PREEMPT_DECODE)),
        ]
        for name, overrides in cases:
            base = json.loads(render_env(PROFILE, overrides))
    ...
    def test_real_harness_startup_failure_tail_uses_isolated_log(self):
        from flexlb_ft.harness import EnvManager, EnvSpec
        ...
        with patch("flexlb_ft.harness.API_JAR", jar), patch(
            "flexlb_ft.harness.resolve_java21", return_value="unused-java"
        ), patch("flexlb_ft.harness.port_in_use", return_value=False), patch.object(
            manager, "_master_env", return_value={}
        ), patch(
            "flexlb_ft.harness.ProcessOps.start", side_effect=launch
        ):
