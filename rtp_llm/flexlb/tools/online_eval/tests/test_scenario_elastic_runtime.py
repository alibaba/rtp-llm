...
class ElasticRuntimeTests(unittest.TestCase):
            ...
            if action == "elastic_scale":
                ...
                return StageOutput(
                    output=dict(
                        scale=register(
                            "snapshot",
                            dict(started_s=clock(), response=dict(drained=True)),
                        ),
                        drained=True,
                    ),
    ...
    def test_high_hit_runs_all_stages_for_both_victims(self):
        ...
        for result in results:
            ...
            self.assertEqual(
                [c["id"] for c in rows["baseline"]["checks"]], ["high_hit"]
            )
            self.assertEqual(rows["transient"]["checks"], [])
            self.assertEqual(rows["steady"]["checks"], [])
            self.assertEqual(
                {c["id"] for c in rows["verdict"]["checks"]},
                {"drained", "PC", "PQ", "PK", "P6", "P2"},
            )

    def test_low_hit_blocks_mutation_and_verdict(self):
        results, mutated = self.run_pilot(rate=0.5)
        self.assertEqual(mutated, [])
        for result in results:
            self.assertEqual(result["status"], "FAIL")
            rows = {row["id"]: row for row in result["stages"]}
            self.assertEqual(rows["baseline"]["checks"][0]["status"], "FAIL")
            self.assertEqual(rows["scale"]["status"], "BLOCKED")
            self.assertEqual(rows["verdict"]["status"], "BLOCKED")
