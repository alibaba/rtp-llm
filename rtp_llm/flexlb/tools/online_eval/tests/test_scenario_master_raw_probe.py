...
from flexlb_ft.harness import _write_master_config
from flexlb_ft.scenario import compile_scenarios, load_scenarios
from flexlb_ft.scenario.actions.environment import mutate_config
...
class RawMasterProbeTest(unittest.TestCase):
    ...
    def test_each_bad_document_reaches_master_while_mock_uses_valid_base(self):
        ...
        for stage in probes:
            with self.subTest(stage=stage["id"]), tempfile.TemporaryDirectory() as tmp:
                ...
                with patch("flexlb_ft.harness.EnvManager.ensure", ensure), patch(
                    "flexlb_ft.harness.EnvManager._start_mock", start_mock
                ), patch("flexlb_ft.harness.EnvManager.start_master", start_master):
    ...
    def test_mock_failure_keeps_ownership_and_restores_target_spec_without_master_claim(
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("flexlb_ft.harness.EnvManager.ensure", ensure), patch(
                "flexlb_ft.harness.EnvManager._start_mock", fail_mock
            ), patch("flexlb_ft.harness.EnvManager.start_master") as master:
