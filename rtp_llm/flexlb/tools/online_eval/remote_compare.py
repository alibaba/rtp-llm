...
from online_eval.reporting import write_bundle, run_meta, compare_controls, table
...
def load_export(path: Path) -> dict:
    ...
    if (
        any(
            ...
            for key in ("start_ms", "end_ms", "granularity_ms")
        )
    ...
    for alias, series in data["series"].items():
        if not isinstance(series, dict) or any(
            series.get(key) is None for key in SERIES_FIELDS
        ):
        ...
        for point in points:
            ...
            if (
                ...
                or t in seen
                or t < provenance["start_ms"]
                or t > provenance["end_ms"]
            ):
...
def compare_exports(
    a: dict, b: dict, *, steady_lo_ms: int, steady_hi_ms: int, min_common: int = 3
) -> tuple[dict, dict]:
    pa, pb = a["provenance"], b["provenance"]
    alignment = compare_controls(
        *[{k: p[k] for k in MATCHING_CONDITIONS} for p in (pa, pb)],
        required=tuple("/" + k for k in MATCHING_CONDITIONS),
    )
    mismatches = alignment["differences"] + alignment["missing"]
    if mismatches:
        raise Incomparable(f"experiment conditions differ: {mismatches}")
    if not (
        max(pa["start_ms"], pb["start_ms"])
        <= steady_lo_ms
        < steady_hi_ms
        <= min(pa["end_ms"], pb["end_ms"])
    ):
    ...
    for alias in common_aliases:
        ...
        if len(common) < min_common:
            raise Incomparable(
                f"{alias}: only {len(common)} paired samples; need {min_common}"
            )
        ...
        rows.append(
            {
                ...
                "role": sa["role"],
                "unit": sa["unit"],
                "spatial_aggregation": sa["spatial_aggregation"],
                "temporal_aggregation": sa["temporal_aggregation"],
                "paired_samples": len(common),
                "a_samples": len(series_maps[0]),
                "b_samples": len(series_maps[1]),
                "a_mean": mean_a,
                "b_mean": mean_b,
                "delta_mean": mean_b - mean_a,
                "relative_delta_pct": (
                    (mean_b - mean_a) / mean_a * 100 if mean_a else None
                ),
            }
        )
        ...
        panels.append(
            {
                "id": f"remote_{len(panels)}",
                "title": f"{alias} · {sa['role']}",
                "caption": (
                    f"{sa['metric']} · {sa['spatial_aggregation']} / "
    ...
    chart = {
        "run_id": "remote-compare",
        "title": "远端 real / mock 对比",
...
def main(argv=None) -> int:
    ...
    try:
        ...
        report, chart = compare_exports(
            a, b, steady_lo_ms=args.steady_lo_ms, steady_hi_ms=args.steady_hi_ms
        )
    ...
    chart["sections"] = [
        table(
            "Paired metric means",
            ["metric", "A", "B", "delta", "paired samples"],
            [
                [
                    r["alias"],
                    r["a_mean"],
    ...
    bundle = write_bundle(
        ...
        meta=run_meta(
            dict(id="remote"),
            environment=[a["provenance"], b["provenance"]],
            evidence=[str(args.real), str(args.mock)],
        ),
        producer="remote",
    )
    result, page = bundle / "analysis.json", bundle / "report.html"
    if args.archive:
        create_archive(
            args.archive,
            {"real": args.real, "mock": args.mock, "comparison": args.out_dir},
            kind="ab",
