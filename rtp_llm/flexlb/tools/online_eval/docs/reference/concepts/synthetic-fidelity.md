...
接受“必须保留长度与共享深度联合结构”的问题判断。没有照搬交接文档的 V4：原型虽抽取 `(blocks, shared)`，
却只使用 `blocks`，深度仍从另一个独立的家族池取得；V0/V4 评估器还把家族首次出现算作已热，
因此其表格不代表生产输出的实际复用。联合对 bootstrap 不会保留事件顺序、原标签和时序，不能等同 trace 回放。

生产 `realistic` 现有两种显式采样语义：

- `sampling: independent`：默认兼容路径，长度经验 ICDF 加固定家族深度。保留旧 seed 的请求字节；只有这一份兼容实现，没有报告侧的 V0 复刻。
...
本次替换的 7 个未跟踪分析原型已在仓库外归档；旧 V0/V4/V5 对照生成器和重复统计入口不进入正式代码。
