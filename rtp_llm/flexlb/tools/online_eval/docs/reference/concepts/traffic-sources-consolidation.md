...
```sh
python3 -m online_eval.fit_frontend_prefix --source CAPTURE_DIR --out FIT_DIR \
  --expected-pods 20 --output-tokens 420
# 旧模型的一次性迁移入口；运行时注册表不支持 v1：
python3 -m online_eval.prefix_lineage old-model.json.gz \
...
默认 profile `frontend_20260921` 由随库模型及其 fit-report 导出，包含稠密
经验长度分布、族集中度拟合、冷比例、共享深度。可通过如下命令重建参数：

```sh
python3 -m online_eval.calibrate_traffic --model lineage-model.xz \
...
小型 cache_scale_in 保留原有显式实验形状和判据；两个规模场景改用校准
profile。`config/scale_cases/cache_scale_in_lineage.yaml` 为 v2 实测模型入口。
