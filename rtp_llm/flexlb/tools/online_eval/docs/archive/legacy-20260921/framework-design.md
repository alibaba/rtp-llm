...
| 使用方式 | 请求发送 | 编排 |
|---|---|---|
| 压测 | JavaLoadClient | 标准压测计划与采集流程 |
| 功能测试 | Python 协议客户端 | Python 精确请求、操作与断言 |
| 场景测试 | 同一个 JavaLoadClient | Python 流量阶段、系统操作与 checkpoint |

场景请求不经过 Python 逐笔转发。YAML 给出可复现 trace 的种子、请求数、节奏、token 组成、输出长度与优先级；Python 可在启动前离线生成 trace，Java 自行加载和发送。`trace_scale_out::prepared_background` 是这条接口的完整示例。

- **流量组（flow group）**：一份 trace 与一个 Java 客户端执行实例。准备流量、背景流量和正式流量各有身份，可同时运行；初版一组对应一个现有客户端进程。
- **发起阶段（phase_id）**：流量组在用例中的用途，随请求记录保留。跨阶段完成不会改写它。
- **checkpoint**：Python 有截止时间的前提检查，保存实际观测。检查本身不暂停后台发送。
- **停止发送**：停止接纳新的 trace 请求；已经接纳的请求继续消费响应。`DRAINING` 是发送端确认，包含命令 ID、已接纳数、实际启动数和终态数。
- **排空**：已接纳请求全部具有终态记录，客户端完成产物落盘并退出。`DRAINED` 仍须与进程退出码、JSONL 总数交叉核对；有失败响应也可能成功排空，业务成功率另行断言。

每个流量组使用自己运行目录下的 `control/stop.json` 和原子替换的 `control/status.json`。命令带 run/group/command 身份，Python 等待实际状态，不把写入命令等同于完成。控制通道不承载请求正文，没有逐请求闸门或内部阶段挂起协议。停止/排空异常走框架失败清理，不能用杀进程的退出码当作完成证据。

合成 trace 明确展开 token；Java 校验长度、优先级、1024-token block size 和请求身份，缺 token、重复身份、错误 key、截断、强制优先级或循环改 key 都拒绝。未提供 key 时由同一 Java 实现从 token 计算。原有压测 trace 的兼容读取保持独立，严格读取由流量组模式启用。

请求 JSONL 与阶段事件共用运行身份。报告直接读取成熟聚合产物的 `per_second` 派生序列，沿实际请求时间原点接入场景图和阶段 A/B 排行。每秒 P99 的均值明确标为“每秒分位值的均值”，不能当作整个阶段所有请求的 P99；缺数据和无效证据不排名，也不自动给出性能退化结论。

现有功能用例的精确请求能力保持不变。历史 workload 的旧发压适配逐家族迁移时，应保持原有前提、故障行为和断言；不能仅通过分类标签宣称它已使用新流量组接口。
...
报告公共层拥有原始证据、统计口径、时间窗口、单位、配置哈希和图形能力。`workload.views` 读取已有 `reports/run/<instance-id>/analysis.json`（旧归档仍支持读取），由独立 YAML 选择指标、事件和布局，不重新采集。当前提供 timeline 与 sweep 两类视图，分别服务版本对比与参数实验。
