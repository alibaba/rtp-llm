...
该显式实验放在 `config/experiments/`，不进入默认场景目录的数据完备性校验和 CI 枚举。
数据不提交仓库；运行前须提供该 YAML 指定 SHA256 的 `data/traffic_models/local_flash_20260922_v3.xz`。
回放客户端显式指定每条请求的输出长度，会绕过引擎 EOS 采样。生成 trace 时以原始事件序号和固定 seed，
使用 SplitMix64 采样 geometric(mean=400)，上限8192，再作为显式长度回放。A/B 逐条输出长度相同；
过滤、截取或 run namespace 不影响同一事件的采样结果。这是独立指定的输出模型，不是采集到的真实输出。
