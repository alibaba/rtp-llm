...
cleanup() {
  ...
  if [[ -n "${EXPERIMENT_ARCHIVE_PATH}" && -d "${RUN_DIR}" ]]; then
    ...
    if PYTHONPATH="${ONLINE_EVAL_DIR}/src:${ONLINE_EVAL_DIR}${PYTHONPATH:+:${PYTHONPATH}}" python3 -m online_eval.archive create \
...
start_monitoring() {
  ...
  PYTHONPATH="${ONLINE_EVAL_DIR}/src:${ONLINE_EVAL_DIR}" python3 -m online_eval.monitoring serve \
...
PYTHONPATH="${ONLINE_EVAL_DIR}/src:${ONLINE_EVAL_DIR}" python3 -m online_eval.monitoring report --run-dir "${RUN_DIR}"
