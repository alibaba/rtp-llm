...
from .resource_plan import JavaMockBudget, ResourcePlanError, plan_lane_leases
        ...
        rows = [
            {
                ...
                "tags": [],
                "requires": [],
                "legacy_case_ids": [name],
                "estimated_duration_s": legacy.CATEGORY_WEIGHTS[category],
                "resource_budget": {"backend": "legacy_fixed_window", "bounded": False},
            }
            for name, category in legacy.list_case_pairs(args.profile)
        ]
        instances.extend(
            parse_catalog(
                {"schema_version": 1, "instances": rows},
                source="legacy",
                profile=args.profile,
            )
        )
...
def _read_results(path, group, source):
    ...
    try:
        ...
        rows = data["instances"] if source == "yaml" else data["cases"]
        if not isinstance(rows, list):
            raise ValueError("result rows must be an array")
        by_id = {}
        legacy_ids = (
            {instance.metadata["legacy_case_ids"][0]: instance.id for instance in group}
            if source == "legacy"
            else {}
        )
        for row in rows:
            ...
            identity = (
                row.get("id") if source == "yaml" else legacy_ids.get(row.get("name"))
            )
...
def _run_lane(index, lane, lease, args, out, stamp, legacy, children):
    ...
    results = []
    segments = []
    # Preserve list order within a lane, including interleaved sources.
    for part, (source, values) in enumerate(
        itertools.groupby(lane, key=lambda instance: instance.source)
    ):
        group = list(values)
        segment = directory / f"part{part}-{source}"
        ...
        if source == "yaml":
            ...
            result_path = segment / "cases.json"
            command = [
                sys.executable,
                str(legacy.RUNNER),
                "--profile",
                ...
                args.grade,
                "--cases",
                ",".join(instance.metadata["legacy_case_ids"][0] for instance in group),
                "--run-root",
                str(segment / f"run-{stamp}"),
                "--json",
                str(result_path),
            ]
        try:
            execution = (
                [instance.metadata["execution"] for instance in group]
                if source == "yaml"
                else []
            )
            timeout = (
            ...
            rows = _read_results(result_path, group, source)
        except Exception as exc:
            rc = 1
            rows = [
                _error(instance, f"child launch failed: {exc}") for instance in group
            ]
...
def run_structured(args: argparse.Namespace, legacy) -> int:
    """Keep window -> output lock -> child list -> lane execution ordering."""
    legacy._resolve_port_bases(args)
    ...
    if not args.dry_run:
        ...
        legacy._acquire_run_lock(out)
    try:
        instances = select_instances(
            _catalog(args, legacy), exact_ids=args.instances, categories=args.categories
        )
        ...
        leases = plan_lane_leases(
            [
                [instance.budget or JavaMockBudget(149, 0) for instance in lane]
                for lane in lanes
            ],
            master_base=legacy._master_base(),
            mock_base=legacy._mock_base(),
            mock_stride=args.mock_stride,
        )
        for i, lease in enumerate(leases):
            if list(lease.ports()) != legacy._lane_ports(
                i, args.mock_stride, legacy._master_base(), legacy._mock_base()
            ):
                raise ResourcePlanError("resource planner and port preflight disagree")
            if lease.child_env() != legacy.lane_env(i, args.mock_stride):
    ...
    futures = [
        pool.submit(_run_lane, i, lane, leases[i], args, out, stamp, legacy, children)
        for i, lane in enumerate(lanes)
    ]
    interrupted = False
    try:
