...
| Schema 3 测试配置 | 取值来源 |
| --- | --- |
| 请求 `timeoutMs` / 决策 `lifetime` | 请求 timeoutMs 必填；决策系数默认 2.0，期限为 `2 × Prefill剩余预测 + 10s`。harness 显式使用 60000 ms / 2.0。 |
| `dispatcher.maxInflightPerPrefillWorker` | 正整数，所有模式默认 2；BATCH 单位是批，NON_BATCH / DIRECT 单位是请求。与组批大小独立。 |
| `DECODE_ENGINE_OWNED` 抢占 | `scheduler.ordering.preemption.timeoutMs` 默认 1000 ms；ACK 阶段结束后等待真实 Engine 终态。超时则 incoming 失败、victim 资源继续保护；ACK 内部默认 50 ms。 |
| 旧数量门槛测试 | 已改为Engine 能力倍数、全局排队与 TTL 到期测试。 |
...
用 `--list` 查看当前用例。默认的 `--shard case` 按实测耗时把每个用例逐例摊到 N 路；每条 lane 是一个独立 runner 子进程树，端口与 run 目录显式分段，互不相碰。
...
| `cancel_stream_break_decode_autonomous` | 客户端直接断流，decode 侧自主提前终态（仅 NON_BATCH 投递） | decode 主动上报终态；账目无残渣 |

### status（固定 batch-window）
...
| `status_prefill_suppress_all` | 所有 prefill 状态消息全静默 | 由正常 Decode 终态及时释放 Prefill 账目；master 存活；恢复 |
| `status_prefill_suppress_finished` | 请求终态被抑制但持续上报 RUNNING | RUNNING 不延长完整请求 lifetime；恢复终态上报后账目收敛 |
...
| `status_decode_running_before_prefill` | decode 只报 RUNNING 不报终态，prefill 事实被抑制 | D 中间态零驱动：prefill 账目不被提前清理、请求不被结算；恢复终态上报后收敛 |
| `status_decode_waiting_before_prefill` | decode 只报早期 RECEIVED 相位（合成事实），prefill 事实被抑制 | D 中间态零驱动：RECEIVED 不结算请求、不提前清 prefill 账目；恢复终态上报后收敛 |
| `status_unknown_rid_running` | 一次性幽灵 RUNNING 条目（未知 rid） | 不得创建 Master 请求；恢复正常状态后收敛 |
...
| `kv_pe_evict_zero_match` | 前缀族在引擎A上 prime 后被 /cache_evict 整族强制逐出 | 首次回放完成；Master 候选命中 tokens 与 Engine 实际命中块增量均为 0 |
| `kv_pe_prefix_continuity` | 共享族在引擎A挖出缺口、引擎B保留前 8 块连续 | 命中按连续前缀计算（缺口截断）：批次落在引擎B而非块数更多的引擎A |
| `kv_g_shared_block_both_match` | 同一前缀族被双投共享给两个引擎 | 两端完整持有前缀；回放落在持有者集合，Master 与 Engine 均确认完整前缀命中 |
| `kv_g_partial_release_redirect` | 共享族只从引擎A逐出 | 收敛后引擎B成为唯一持有者；同前缀请求全部重定向到引擎B |
| `kv_g_full_release_no_ghost` | 共享族从两个持有者全部逐出 | 两端快照不再含该族；首次回放的 Master 候选命中与 Engine 实际命中均为 0 |
| `kv_g_sync_convergence` | 交错 admit/evict 事件流后静默 ≥3.5s | 无持有者族首次回放零命中；唯一持有者族继续路由到持有者，结果与快照一致 |
...
| `kv_prefix_stickiness` | 多前缀族复用流量 + 自由流量混合 | family 续连保持亲和；自由流量确认零缓存命中；全部完成 |
| `kv_hot_prefix_tension` | 70% 流量集中于单一热前缀族 | 热族续连保持亲和；新键请求确认零缓存命中；不设置持有者流量份额上限 |
| `kv_match_mixed` | 全命中 / 半命中 / 零命中三档流量 | Master 与 Engine 确认各档命中量；全、半命中保持持有者亲和；全部完成 |
...
验证并发请求完成、Prefill token 成本分流、Decode 准入与轮转。BEST_ONLY 不承诺等成本 Prefill 的均匀份额。

| 用例 | 场景 | 期望 |
| --- | --- | --- |
| `balance_concurrent_mix` | 20 请求 20 路并发混合突发 | 全部完成；batch credits 不造成 QUEUE 发布拒绝 |
...
| `balance_decode_rotation` | 每个请求结算后再路由，健康 Decode 均可准入 | 每个健康 Decode 都接收请求；完成总数准确 |
| `balance_len_mixed` | 双峰长度混合（每波 2 长 + 6 短） | 两个长请求按 TTFT 分别选择 worker；短请求全部完成；按 token 统计负载 |

### elastic（固定 batch-window）
...
| `master_quota_block` | 1P+1D 小集群配额阻塞 | batch credits 随请求结算归还，后续恢复 ≥90% |
...
准入测试覆盖 Engine 的并发、KV、等待队列门槛，以及 Master 的单机 uncached-token 容量、Decode 准入、batch credits 与排队 TTL。Engine 的队列容量是独立控制。
...
| `admission_prefill_request_capacity_park` | SINGLE 组批、1 个 batch credit、Prefill 模拟 3s，6 请求突发 | 请求留在 Master 待发队列，容量释放后全部完成；账目干净并恢复 |
| `admission_prefill_request_capacity_deadline` | 请求容量不足，queueTimeout=1.5s | Master 排队者以 8511 到期；已投递者完成，账目清零 |
| `admission_nonbatch_request_capacity_wait` | NON_BATCH 显式上限1，仅容纳1请求 | 请求留在全局队列，释放后继续投递，全部完成 |
...
| `atpm_preempt_decode_engine_owned` | DECODE_RESERVED/DECODE_ENGINE_OWNED 抢占（Engine-owned 终态等待默认 1000 ms）、decode 满载波 | [EV-2] decode 驱逐不可达：零 8429、占用者完成、incoming 走 EV2 拒绝族；AT5 闭环预算 band |
...
| `atpm_config_strict_reject` | 3 个非法 FLEXLB_CONFIG 原始 JSON 变体（removed 字段 / FIFO+defaultPriority / 缺失 decision.lifetime） | master 启动失败 + 严格解析器报文族命中；rejected 后 current=None（AT1） |
