...
rework — gates are functional, injections are chaos):
    ...
    gate_slo_queue_deadline    (G11a) tiny scheduler.queueTimeoutMs +
                                      ...
                                      SLO deadline fires.  (main note:
                                      the queue-deadline admission gate
                                      has no flat equivalent — see
                                      _gate_config.)
    gate_master_capacity_reject(G11b) capacity.maxOutstandingRequestsGlobal=2
                                      ...
                                      capacity exhausted".  (main note:
                                      maps to flexlbBatchMaxInflight=2; the
                                      reject surfaces as QUEUE_FULL 8502 /
                                      "TooManyRequests" with autoTpm off,
                                      or RESOURCE_EXHAUSTED "master inflight
                                      capacity exhausted" with autoTpm on —
                                      see _gate_config.)
...
def _gate_config(
    ...
    """FLEXLB_CONFIG for the gate cases: the chaos template (BATCH +
    fixed_window + FIFO; PRIORITY degrades to FIFO — no main equivalent)
    via the unified harness.build_flexlb_config template, with the
    admission knobs parameterised.  queue_timeout_ms has NO main
    equivalent and is dropped by the generator — gate_slo_queue_deadline
    (G11a) relies on it and needs a main-side redesign (see the UNMAPPED
    list in build_flexlb_config); max_outstanding maps to the flat
    flexlbBatchMaxInflight submit-path admission gate."""
