...
class EnvSpec:
    ...
    # the FLEXLB_CONFIG document is generated from the flat main-branch
    # config template unless master_env overrides it (chaos/gate suites
...
# ---------------------------------------------------------------------------
# Scheduling profiles (flat main-branch FLEXLB_CONFIG)
# ---------------------------------------------------------------------------
#
# The main-branch master consumes a FLAT FLEXLB_CONFIG document: a single
# camelCase-key JSON object parsed directly into FlexlbConfig (see
# flexlb-common ConfigService); every simple-typed field is additionally
# overridable through an UPPER_SNAKE env var (camelCase field →
# UPPER_SNAKE env, e.g. defaultScheduleMode → DEFAULT_SCHEDULE_MODE — the
# key set mirrored by run_online_eval.sh OVERRIDE_ENV_KEYS).  The
# intake3-rebuild test line was authored against the schemaVersion:2
# nested document (scheduler/dispatcher/router/workerRegistry); the
# generator below re-derives the equivalent flat keys from the same
# behaviour axes so the --profile switch keeps working on the main
# master.
#
# Axis mapping (v2 nested → main flat):
#   dispatcher BATCH     → defaultScheduleMode=BATCH (master batches +
#                          EnqueueBatch; clients consume FetchResponse)
#   dispatcher NON_BATCH → defaultScheduleMode=DIRECT (approximation: v2
#                          still routed through the scheduler queue while
#                          frontend-sending; main DIRECT bypasses batching
#                          and the client sends GenerateStreamCall itself)
#   decision fixed_window → flexlbBatchAlgorithm=fixed_window with the
#                          FLEXLB_BATCH_FIXED_WAIT_MS / PREDICT_THRESHOLD_MS
#                          / SIZE_MAX parameters
#   decision single      → fixed_window with wait=0 + sizeMax=1
#                          (approximation: every request dispatches alone
#                          immediately; main has no SINGLE decision type)
#   ordering fifo|priority → no main equivalent (main queue order is fixed
#                          by the autoTpm switch); PRIORITY degrades to
#                          the plain FIFO order
#
# window-nonbatch keeps the fixed_window declaration for template
# uniformity, but under DIRECT the flexlbBatch* keys have no consumer —
# the v2 "batched window + frontend-sent delivery" combination does not
# exist on main and the profile approximates the frontend-send side only
# (see the UNMAPPED list in build_flexlb_config).
#
# Phase-1 profile set (user ruling 2026-08): all profiles carry the FIFO
# ordering intent; PRIORITY ordering / preemption / selector variants are
# left for a dedicated later phase.
...
# decision × dispatcher axes per profile (rendered onto the flat config
# template above; the v2 scheduler was QUEUE, ordering FIFO).
PROFILE_SPECS = {
...
# constant the intake3 test line calibrated. The harness injects it
# EXPLICITLY into every generated FLEXLB_CONFIG as the flat costFormula
# key instead of relying on the Java code default: the main-branch default
# is the legacy token-sum "sum(computeTokens) + 0.3*sum(hitCacheTokens)",
# which overpredicts a 32k all-miss prefill by ~96x (32.8 s vs the fitted
# ~342 ms) and would poison every ledger-driven routing decision in these
# suites. The expression syntax is fully supported by the main formula
# engine (flexlb-sync PrefillTimeFormula): max/sum functions, arithmetic,
# and trailing-dot decimals such as "1024." all parse (costFormula is a
# CRITICAL_CONFIG_FIELDS member — a parse failure aborts master startup).
...
def build_flexlb_config(
    *,
    ordering: str = "fifo",  # legacy v2 axis; no main equivalent (ignored)
    decision: str = "fixed_window",  # fixed_window | single
    dispatcher: str = "batch",  # batch | non_batch
    # decision → fixed_window parameters (ignored when decision == "single")
    max_requests: int = 32,
    max_collection_wait_ms: int = 10,
    max_predicted_execution_ms: int = 550,
    # scheduler knobs (None → omit the key, keep the Java default)
    queue_timeout_ms: Optional[int] = None,  # NO main equivalent (ignored)
    max_outstanding: int = 5_000,
    stale_inflight_ms: int = 30_000,
    delivered_not_accepted_timeout_ms: int = 30_000,  # NO equivalent (ignored)
    max_delivered_not_accepted: int = 200,  # NO main equivalent (ignored)
    # dispatcher knobs
    ...
    max_inflight_requests_per_worker: Optional[
        int
    ] = None,  # NON_BATCH; NO main equivalent (ignored)
    # workerRegistry.health — NO main equivalent (ignored)
    status_rpc_ms: int = 1_000,
) -> str:
    """Unified flat main-branch FLEXLB_CONFIG generator.
    ...
    (injection_gate_cases._gate_config) all delegate here.  The signature
    keeps the legacy schema-v2 knobs so no call site changes; the body
    maps every knob with a main equivalent onto the flat camelCase keys
    (ConfigService parses the FLEXLB_CONFIG JSON document directly into
    FlexlbConfig) and drops the ones without one.

    Mapped (v2 nested → main flat):
      dispatcher.type=BATCH/NON_BATCH → defaultScheduleMode=BATCH/DIRECT
      decision FIXED_WINDOW           → flexlbBatchFixedWaitMs /
                                        flexlbBatchSizeMax /
                                        flexlbBatchPredictThresholdMs
      decision SINGLE                 → fixed_window wait=0 + sizeMax=1
                                        (approximation, see module header)
      scheduler.capacity.maxOutstandingRequestsGlobal
                                      → flexlbBatchMaxInflight (the BATCH
                                        submit-path admission gate)
      scheduler.lifecycle.staleInflightTimeoutMs → flexlbInflightTtlMs
      dispatcher.maxInflightBatchesPerPrefillWorker
                                      → flexlbBatchFixedMaxInflightBatches
      dispatcher.enqueueRpcTimeoutMs  → flexlbBatchEnqueueDeadlineMs
      router.availabilityHysteresisPercent → hysteresisBiasPercent
      ...
      router.roles.decode.availability.maxEngineRequests
                                      → decodeConcurrencyLimit
      router.roles.prefill.executionTimeEstimator (FORMULA + DSv4 fit)
                                      → costFormula

    UNMAPPED (no main-branch equivalent; parameter accepted but ignored):
      ordering        — main has no ordering axis (autoTpm decides queue
                        order); PRIORITY degrades to FIFO
      queue_timeout_ms — no SLO queue-deadline admission gate on the main
                        submit path; gate_slo_queue_deadline (G11a) relies
                        on it and needs a main-side redesign
      delivered_not_accepted_* — no delivered-not-accepted lifecycle on
                        main (flexlbInflightTtlMs only covers stale inflight)
      max_inflight_requests_per_worker — DIRECT has no per-worker inflight
                        ceiling
      status_rpc_ms   — no workerRegistry.health group; main status polling
                        is governed by the statusCheck executor sizing only
      router cacheAffinity (maxExtraTtftMs / minPrefixHitPercent) and the
      ESTIMATED_TTFT selector (RANDOM_WITHIN_TOLERANCE candidate choice,
      outlier rejection) — main routing has no affinity gate, random
      within-tolerance choice, or outlier rejection (only the scoreTie*
      tie-break window)

    The DSv4 fit is injected EXPLICITLY as the flat costFormula key
    (:data:`DSV4_PREFILL_EXPRESSION`): the test line must not depend on the
    Java code default, which is the legacy token-sum expression.
    """
    cfg: dict = {
        # dispatcher axis → schedule mode
        "defaultScheduleMode": "BATCH" if dispatcher == "batch" else "DIRECT",
        # decision axis → the fixed_window algorithm (main's only
        # algorithm besides the SLO-budget fallback)
        "flexlbBatchAlgorithm": "fixed_window",
        # router axis (flat keys, aligned with the main eval template in
        # run_online_eval.sh DEFAULT_FLEXLB_CONFIG)
        "loadBalanceStrategy": "COST_BASED_PREFILL",
        "decodeLoadBalanceStrategy": "COST_BASED_DECODE",
        "hysteresisBiasPercent": 0,  # v2 availabilityHysteresisPercent=0
        "prefillQueueSizeThreshold": 100000,  # v2 prefill maxPendingRequests
        "decodeConcurrencyLimit": 132,  # v2 decode maxEngineRequests
        "costFormula": DSV4_PREFILL_EXPRESSION,
        # scheduler capacity / lifecycle
        "flexlbInflightTtlMs": stale_inflight_ms,
        "flexlbBatchMaxInflight": max_outstanding,
    }
    if decision == "single":
        # main has no SINGLE decision type; a zero-length window plus a
        # one-request batch approximates "dispatch every request alone
        # immediately" (queueSize >= 1 → batch_full dispatch and
        # elapsed >= 0 → window timeout fires on the same tick).
        cfg["flexlbBatchFixedWaitMs"] = 0
        cfg["flexlbBatchSizeMax"] = 1
    else:
        cfg["flexlbBatchFixedWaitMs"] = max_collection_wait_ms
        cfg["flexlbBatchSizeMax"] = max_requests
        cfg["flexlbBatchPredictThresholdMs"] = max_predicted_execution_ms
    if dispatcher == "batch":
        cfg["flexlbBatchFixedMaxInflightBatches"] = max_inflight_batches
        if enqueue_rpc_timeout_ms is not None:
            cfg["flexlbBatchEnqueueDeadlineMs"] = enqueue_rpc_timeout_ms
    # else: DIRECT leaves the flexlbBatch* keys above unconsumed (template
    # uniformity); max_inflight_requests_per_worker has no DIRECT-side
    # equivalent — see the UNMAPPED list in the docstring.
...
def flexlb_config_for_profile(profile: str, **overrides) -> str:
    ...
    kwargs = {
        ...
        # No queue_timeout_ms on main: the v2 SLO queue-deadline admission
        # gate has no flat equivalent (see the UNMAPPED list in
        # build_flexlb_config).
    }
...
# Master env that is actually consumed by the main-branch code:
#   FLEXLB_CONFIG          — set per spec from the flat-config generator
#                            above (ConfigService parses the JSON document
#                            into FlexlbConfig; every simple-typed field is
#                            additionally env-overridable, camelCase field
#                            → UPPER_SNAKE env)
