...
need a short deadline bottom line.  Cases whose contract is a CLEAN inflight
baseline or bit-identical ledger fingerprints (unknown_rid_running,
unknown_batchid, special_ids, unbatched_single_request, foreign_batchid)
take the same shape with isolated=True: they never share an environment
with the rest of the family, so an earlier case's leak residue cannot fail
their baselines — the leak belongs to (and fails) the case that produced
it.
...
def _status_spec(ctx: CaseContext, isolated: bool = False) -> EnvSpec:
    ...
    """
    # isolated=True (EnvSpec flag, fingerprint-visible): cases whose
    # contract is a CLEAN inflight baseline / bit-identical ledger
    # fingerprints never share an env with the rest of the family — an
    # earlier case's leak residue on the shared env would fail their
    # baseline without the case under test having done anything wrong
    # (the leak stays owned by the case that produced it).
    return EnvSpec(
        label=f"status_fault_{ctx.profile}" + ("_iso" if isolated else ""),
        n_prefill=2,
        n_decode=2,
        perf=default_perf(),
...
def _ttl_eviction_events(
    ...
    Absent-channel degradation (old-stack adaptation): the counter family
    only exists on masters that implement the TTL-eviction metric.  When
    the role's series is absent BOTH at the before-baseline and after the
    full observation window, the family is simply not implemented on this
    master (v1 old stack) — the event assertion then degrades to the
    BEHAVIOR assertions each caller already applies independently (the
    ledger drains to zero inside the TTL window), and the degradation is
    recorded in the detail.  When the series exists the >= min_delta
    assertion stays hard, so a stack that implements the channel can
    ...
    if reached:
        return True, f"{role}_ttl_eviction_delta={final_delta} (need>={min_delta})"
    # Absent-channel check: series never appeared across the whole window
    # (before-baseline AND the post-window read both miss the role).
    after = ops.master_ttl_eviction_counts()
    if after is not None and before.get(role) is None and after.get(role) is None:
        return True, (
            f"{role}_ttl_eviction=COUNTER_ABSENT (delta=0, need>={min_delta}) — "
            f"the TTL-eviction counter family is not implemented on this "
            f"master; the behavior assertions (ledger drain to zero within "
            f"the TTL window) carry the contract"
        )
...
def status_unknown_rid_running(ctx: CaseContext):
    ...
    Grade: P1."""
    ops = ctx.engine_ops(ctx.env_manager.ensure(_status_spec(ctx, isolated=True)))
    base = rid_base(ctx, "status")
    names = _prefill_names(ops)
    if not names:
        return False, "no prefill engines found"
