...
category-reorg migrations:
  ...
  storm / capacity
    ...
    kv_decode_capacity_park        every decode KV-exhausted -> the
                                   ...
                                   family, E4) — SKIPPED on dsv4 v1:
                                   v2-only park contract (the v1 stack
                                   fail-fasts instead of parking)
...
# B-side diversion geometry (kv_pe_admit_isolation), dsv4 v1 semantics:
# the master's routing wait is pure FORMULA predict (no engine-side
# feedback), and the mock cluster shares ONE performance model, so a
# set_perf override leaks onto every engine — engine timings in this
# case must come from the FORMULA itself.  A 200-block blocker input
# carrying the 48-block fam5 prefix (24% hit >= the 20% affinity gate)
# prices ~2.56s of predict on B (vs ~3.47s miss-priced on A, so the hit
# discount lands the blocker on B deterministically) and its engine-side
# execution is the same ~2.56s, holding the window for the whole family
# sequence.  The four ~0.24s families land on A one at a time, spaced
# FAMILY_SPACING_S so each settles before the next decision — A's
# pending count stays <= 1, below the prefill hotspot filter's
# pending > 1.5 x (floor-divided) average rejection (avg = 1 once B
# holds the blocker; two stacked in-flight families on A would divert
# the next decision onto B).
FAM5_BLOCKS = 48
BLOCKER_BLOCKS = 200
FAM5_PREFIX_TOKENS = FAM5_BLOCKS * BLOCK_TOKENS  # 49152
BLOCKER_INPUT_LEN = BLOCKER_BLOCKS * BLOCK_TOKENS  # 204800
# Family-admit spacing (kv_pe_admit_isolation): covers one family's
# ~0.24s FORMULA execution plus the ~50ms status settle.
FAMILY_SPACING_S = 0.45
# Seed A geometry (kv_pe_admit_isolation): a 60-block miss (~625ms of
# FORMULA predict) keeps A in flight while seed B's decision runs, so
# the ledger-separated seeding splits the two seed families across
# engines deterministically.
SEED_A_INPUT_TOKENS = 60 * BLOCK_TOKENS  # 61440
...
def case(
    ...
    (a contract the dsv4 v1 stack does not implement).
...
def _kv_spec(
    ...
    *,
    ...
    Decode capacity stays at DEFAULT_DECODE_CACHE_BLOCKS (3000 blocks x
    1024 tokens): the dsv4 v1 master hard-reserves decode KV against
    seqLen (CostBasedDecodeStrategy rejects when available < seqLen), so
    a tiny decode pool fail-fasts every seed request (10240..147456
    tokens) with NO_DECODE_WORKER before any prefill-side KV behaviour
    can be observed.  The v2-only park contract that tolerated the tiny
    pool does not exist on this stack."""
    return EnvSpec(
        ...
        decode_cache_blocks=DEFAULT_DECODE_CACHE_BLOCKS,
        discovery=discovery,
    )
...
def _lru_spec(ctx: CaseContext) -> EnvSpec:
    ...
    LRU (4 blocks).  The eviction exercise lives in the prefill LRU; the
    decode pool keeps DEFAULT_DECODE_CACHE_BLOCKS because the dsv4 v1
    master hard-reserves decode KV against seqLen (see _kv_spec)."""
    return EnvSpec(
...
def kv_pe_admit_isolation(ctx: CaseContext):
    ...
    technique, seed B scheduled while seed A is in flight) pins family-0
    on engine A and family-5 (a 48-block prefix) on engine B; a
    big-input fam5 continuation is then held IN FLIGHT on B — the dsv4
    v1 master's routing wait is pure FORMULA predict, so the diversion
    signal is bought with computeTokens (~2.56s of predict window on B)
    — and the four zero-hit families are admitted onto A one at a time
    inside that window, each settling before the next decision (the
    prefill hotspot filter rejects an endpoint holding pending >
    1.5x the pool average, so stacking two in-flight families on A
    would divert the next decision onto B).  Behaviour: per-engine
    admit accounting in the master's cache-status index.
    Expected (contract): only A's cache_key_set grows with the four
    families — B's stays exactly its own seed family — and subsequent
    same-prefix continuations stick to A (P9); a global broadcast of
    A's admits would equalize the hit and dissolve the affinity into
    spread.  Prediction: passes.
    """
    ...
    try:
        ...
        # Seed A carries an enlarged 60-block miss (~625ms of FORMULA
        # predict) and seed B is scheduled while seed A is still in
        # flight, so the live predict window on A's engine prices it
        # outside the tie window and B deterministically lands on the
        # other engine (the kv_prefix_stickiness technique — a plain
        # serial seeding puts both families on one engine half the time).
        # No set_perf here: the mock cluster shares ONE performance
        # model, so any override would leak onto the diversion phase.
        ...
        if err is None:
            # Seed B carries a 32-block fam5 prefix (32768 tokens): the
            # diversion blocker later re-uses it as its hit discount.
            fam5 = _fam_keys(base, 5, blocks=FAM5_BLOCKS)
            rid_b = ops.next_request_id(base)
            addr_b, err_b = ops.run_one_request(
                rid_b,
                input_len=FAM5_PREFIX_TOKENS,
                output_len=2,
                block_keys=fam5,
                stream_timeout_s=STREAM_TIMEOUT_S,
            )
            b_name = ops.addr_to_name().get(addr_b, addr_b)
        # Drain AFTER seed B so the overlap holds the deterministic
        # separation (draining first would free A's engine and turn seed B
        # into a plain tie-window coin flip).
        _drain_fired(ops, fired, fired_handles)
        fired, fired_handles = [], {}
        if err or err_b:
        ...
        # The dsv4 v1 master's routing wait is pure FORMULA predict, so
        # the diversion window is bought with computeTokens: a 200-block
        # fam5 continuation (24% prefix hit >= the 20% gate) prices
        # ~2.56s of FORMULA batchWait on B while the hit discount keeps
        # its select-time score ~900ms below A's miss price, so the
        # blocker lands on B deterministically and stays engine-side in
        # flight for the whole family sequence (its execution is the
        # same ~2.56s FORMULA value).  The four zero-hit families land
        # on A one at a time, spaced FAMILY_SPACING_S: each settles on
        # A (~0.24s execution + ~50ms status settle) before the next
        # decision, so A's pending count stays <= 1 — a tighter burst
        # would stack two in-flight families on A and trip the prefill
        # hotspot filter (pending > 1.5x floor-divided average rejects
        # A, leaving B the only candidate).
        if not _wait_cache_sync(ops, names):
            return False, "cache sync never converged after seeding"
        blocker_keys = _fam_keys(base, 5, blocks=FAM5_BLOCKS)
        rid_blk = ops.next_request_id(base)
        blk_name, blk_err = _fire_request(
            ops,
            rid_blk,
            fired,
            fired_handles,
            input_len=BLOCKER_INPUT_LEN,
            output_len=2,
            block_keys=blocker_keys,
        )
        if blk_err is not None:
            return False, f"diversion blocker failed: {blk_err}"
        if blk_name != b_name:
            _drain_fired(ops, fired, fired_handles)
            return False, (
                f"diversion blocker landed on {blk_name} instead of "
                f"{b_name} (hit-discount diversion premise broken)"
            )
        if not _poll_engine_pending(ops, b_name, 1):
            _drain_fired(ops, fired, fired_handles)
            return False, (
                f"diversion blocker never appeared on {b_name} "
                f"(affinity diversion premise broken)"
            )
        ...
        for keys in fams:
            ...
            if name != a_name:
                _drain_fired(ops, fired, fired_handles)
                return False, (
                    f"zero-hit family landed on {name} instead of {a_name} "
                    f"(B-heavy diversion failed)"
                )
            # Let this family settle on A before the next decision so
            # A's pending count stays <= 1 (hotspot-filter discipline,
            # see the diversion comment above).
            time.sleep(FAMILY_SPACING_S)
        _drain_fired(ops, fired, fired_handles)
        fired, fired_handles = [], {}
        if not _wait_cache_sync(ops, names):
...
def kv_g_full_release_no_ghost(ctx: CaseContext):
    ...
    try:
        ...
        #    zero-hit tie-window spread — no ghost stickiness.  The 0.05s
        #    cadence keeps all 10 scheduling decisions inside one master
        #    cache-status poll period, BEFORE the wave's own re-admits can
        #    propagate (a slower cadence lets the first re-admit re-affinitize
        #    the wave tail — a correct-behaviour bias the spread assertion
        #    must not pay for).
        wave_names = []
        for _ in range(10):
            rid = ops.next_request_id(base)
        ...
        if wave_names:
            ...
            for n in wave_names:
                dist[n] = dist.get(n, 0) + 1
            max_share = max(dist.values()) / len(wave_names)
            # Case-local P1 bands calibrated for this 10-sample wave (the
            # global P1 bands are calibrated at 20 samples, where the binomial
            # tail is tighter): under a correct no-ghost spread with n=10,
            # P(X >= 8 same engine) ~= 10.9% (normal-grade noise, not a
            # contract breach) and P(X >= 9) ~= 2.1%; a ghost entry pins the
            # whole wave at share 1.0, far above every tier.
            report.check(
                "P1",
                max_share,
                context="full_release",
                bands={"strict": 0.70, "normal": 0.80, "loose": 0.90},
                detail=(
...
def kv_lru(ctx: CaseContext):
    ...
    3. R3 extends the prefix within capacity ([k1,k2]+k3 -> three admits,
       still inside the capacity-4 pool with its ~5% reserve watermark):
       zero evictions, and the k1,k2 prefix keeps R3 on X.
    4. R4 applies pressure ([k3,k4,k5]: k3 held by X pins R4 on X via
       affinity; the pool is then 3 pure-LRU keys + 1 free, so admitting
       the two net-new blocks evicts the eldest, k1 — snapshot proves
       evictions >= 1 with cache_keys capped at 4).

    KV v2 note: the legacy "5 admits into a capacity-4 LRU" shape was v1
    admit-then-evict churn; the v2 block pool instead REJECTS demand that
    exceeds available-minus-reserve at the admission gate (LACK_MEM), so
    eviction is asserted on the acquire-coupled LRU tail instead (step 4).
    """
    ...
    try:
        ...
        # Within-capacity extension: prefix [k1,k2] + k3 -> 3 admits into
        # the capacity-4 pool (gate: need 3 <= avail 4, avail-need >= the
        # ~5% reserve watermark) — no eviction yet; the k1,k2 prefix keeps
        # R3 on X.
        keys_ext = keys_a + [base + 3]
        rid3 = ops.next_request_id(base)
        addr3, err3 = run(rid3, keys_ext, 3072)
        if err3:
            return False, f"R3 (extend) failed: {err3}"
        time.sleep(KV_CACHE_SYNC_WAIT_S)  # R3's k3 admit must sync for R4's affinity
        snap = ops.snapshot_by_name()
        engine_z = addr_map.get(addr3, "?")
        keys_after_ext = snap.get(engine_z, {}).get("cache_keys", 0)
        evictions_after_ext = snap.get(engine_z, {}).get("cache_evictions", 0)
        prefix_affinity = addr3 == addr1

        # Pressure (KV v2 acquire-coupled eviction): [k3,k4,k5] — k3 is
        # held by X (R3's admit), so the affinity gate pins R4 on X; the
        # pool then holds 3 pure-LRU keys with 1 free block, and admitting
        # R4's two net-new blocks (k4,k5) evicts the eldest (k1).
        keys_pr = [base + 3, base + 4, base + 5]
        rid4 = ops.next_request_id(base)
        addr4, err4 = run(rid4, keys_pr, 3072)
        if err4:
            return False, f"R4 (pressure) failed: {err4}"
        time.sleep(0.5)  # admit lands at prefill completion
        snap = ops.snapshot_by_name()
        engine_w = addr_map.get(addr4, "?")
        keys_after_pressure = snap.get(engine_w, {}).get("cache_keys", 0)
        evictions_after_pressure = snap.get(engine_w, {}).get("cache_evictions", 0)
        pressure_affinity = addr4 == addr1

        prime_ok = keys_after_prime >= 2 and evictions_after_prime == 0
        extend_ok = keys_after_ext == 3 and evictions_after_ext == 0
        eviction_ok = (
            evictions_after_pressure >= 1
            and keys_after_pressure <= 4
            and pressure_affinity
        )
        passed = affinity and prime_ok and extend_ok and eviction_ok and prefix_affinity
        return passed, (
            f"engine_x={engine_x}, affinity_r2={affinity}, "
            f"after_prime: keys={keys_after_prime}, evictions={evictions_after_prime}, "
            f"extend_landed_on={engine_z}, prefix_affinity_r3={prefix_affinity}, "
            f"after_extend: keys={keys_after_ext} (==3), evictions={evictions_after_ext} (==0), "
            f"pressure_landed_on={engine_w}, affinity_r4={pressure_affinity}, "
...
@case(
    ...
    skip_reason=(
        "v2-only: the decode capacity park contract (queue-and-wait under KV "
        "exhaustion, Cancel releases the parked entry) is not present in the "
        "dsv4 v1 stack, which fail-fasts with 8403/NO_DECODE_WORKER instead"
    ),
