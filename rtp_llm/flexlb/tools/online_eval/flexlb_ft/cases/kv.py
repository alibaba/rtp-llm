...
path.  Grouped per the researcher spec plus the category-reorg
migrations:
  ...
  storm / capacity
    ...
    kv_decode_capacity_park        every decode KV-exhausted -> the
                                   ...
                                   family, E4)
    kv_leader_saturation_spill     a saturated cache leader spills its
                                   family onto the survivor — the LRU
                                   ping-pong collapse (expected-fail
                                   probe, production observation)
  ...
  block-pool saturation (KV v2 extremes, 2026-09 follow-up)
...
MOCK CAPABILITY NOTE (category-reorg wiring): the module IS registered
...
def kv_leader_saturation_spill(ctx: CaseContext):
    ...
    try:
        ...
        def snapshot_digest() -> dict:
            ...
            return {
                n: {
                    f: _contiguous_prefix_len(key_sets[n], fams[f])
                    for f in range(2)
                }
                for n in names
            }
        ...
        recovery_str = (
            str(recovery_at) if recovery_at else f">{SPILL_RECOVERY_WINDOWS}"
        )
