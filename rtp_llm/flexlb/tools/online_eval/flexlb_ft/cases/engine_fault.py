...
def _created_generation_count(env, ip_port: str, offset: int = 0) -> int:
    ...
    (v2) / "Created new WorkerStatus for worker: {ip}" (dsv4 v1, dynamic
    discovery only).  Under dsv4 static file discovery neither line fires
    across an engine stop/start (the status object never leaves the sync
    map) — see _generation_turnover_bumped for the old-stack equivalent."""
...
def _generation_turnover_bumped(
    created_before: int, created_after: int, retired: bool, alive_back: bool
) -> bool:
    """True when the endpoint's generation observably turned over across
    the outage.  Primary signal: the master created a NEW WorkerStatus
    for the endpoint (v2 generation bump / dsv4 dynamic-discovery
    re-create, both matched by _created_generation_count).  dsv4 (v1)
    equivalent: the old stack retires by removing the endpoint from the
    registry directly (GrpcWorkerStatusRunner marked-dead →
    endpointRegistry.remove) and re-ensures it on the next discovery tick
    (EngineSyncRunner.getOrCreateWorkerStatus → ensureEndpoint); under
    static file discovery no "Created ... WorkerStatus" log line fires,
    so the equivalent full-resync signal (the E1 contract wording: "a NEW
    WorkerStatus generation (or an equivalent full resync signal)") is the
    retire → re-registration cycle itself: retired AND alive restored."""
    if created_after > created_before:
        return True
    return retired and alive_back
...
def recovery_generation_bump(ctx: CaseContext):
    ...
    try:
        ...
        generation_bumped = _generation_turnover_bumped(
            created_before, created_after, retired, alive_back
        )
