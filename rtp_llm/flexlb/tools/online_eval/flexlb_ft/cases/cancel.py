...
def cancel_engine_notfound_settle(ctx: CaseContext):
    ...
    the engine branch is the mock's production-faithful three-branch
    cancel semantics: ACCEPTED (live or active-cancel tombstone) /
    NOT_FOUND (seen but already terminal — this case) / TOMBSTONED
    (never-seen rid, absent fence installed). The production 10-minute
    recently-seen TTL is simplified away in the mock: every cancel in
    these cases is a sub-second race, far inside that window).
    """
    ops = ctx.ops()
    rid = ops.next_request_id(rid_base(ctx, "cancel"))
    try:
