...
def _prio_config(
    ...
    * ``maxWaitingRequestsPerPrefillWorker`` rides the native
      ...
      v1 wiring caveat (wave-3): the generator only emits the key under
      the BATCH dispatcher (NonBatchDispatcherConfig has no such field —
      the strict mapper fails master startup on the unknown property),
      so on this NON_BATCH base config the waiting-cap value is accepted
      but NOT emitted; the v1 NON_BATCH queue-full path keeps the Java
      default.
    """
...
def _d1_spec(ctx: CaseContext) -> EnvSpec:
    ...
    FLEXLB_MONITOR_METRIC_WHITELIST=flexlb_ (behaviour-neutral for
    scheduling): the default critical-only metrics filter hides
    auto_tpm.*; atpm_decode_reservation_priority asserts the
    auto_tpm.victim.count priority tags, which need the metrics filter
    opened.  v1 wiring (wave-3): the v2-only FLEXLB_MONITOR_MODE=all
    switch does not exist on the dsv4 v1 stack — the v1 knob is the
    metric-name whitelist env (prefix match; flexlb_ covers every
    flexlb/auto-tpm series the master exposes).
    """
    return _spec(
        ctx,
        "atpm_d1",
        ...
        extra_env={"FLEXLB_MONITOR_METRIC_WHITELIST": "flexlb_"},
    )
...
def _o1_spec(ctx: CaseContext) -> EnvSpec:
    ...
    so an env-level metrics switch is required (v1 wiring, wave-3: the
    v2-only FLEXLB_MONITOR_MODE=all is not a property of the dsv4 v1
    stack — the v1 knob is the FLEXLB_MONITOR_METRIC_WHITELIST prefix);
...
def _q3_spec(ctx: CaseContext) -> EnvSpec:
    ...
    config plus FLEXLB_MONITOR_METRIC_WHITELIST=flexlb_ (v1 wiring,
    wave-3: the v2-only FLEXLB_MONITOR_MODE=all is not a property of the
    dsv4 v1 stack).  auto_tpm.request.count{priority=..}
...
def prio_normalize(ctx: CaseContext):
    ...
    try:
        ...
        # Needs its own env (FLEXLB_MONITOR_METRIC_WHITELIST=flexlb_): the
        # shared env / Q1 specs run the default critical-only metrics filter
        # that hides auto_tpm.*.  Fresh env means the counters start at zero,
...
def atpm_decode_reservation_priority(ctx: CaseContext):
    ...
    FLEXLB_MONITOR_METRIC_WHITELIST=flexlb_ so auto_tpm.victim.count is
    exposed — the default critical-only filter hides auto_tpm.*).  Every
    wave follows
...
def atpm_observability_integrity(ctx: CaseContext):
    ...
    deterministic) + master debug log + the metrics whitelist
