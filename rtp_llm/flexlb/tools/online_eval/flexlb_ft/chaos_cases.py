...
Profile semantics (main-branch flat config, 2026-08 port): every
environment in this file except chaos_coldstart_burst pins the chaos
template (BATCH + fixed_window + FIFO — the v2 chaos axes with PRIORITY
degraded to the main FIFO order, see harness.build_flexlb_config) via
FLEXLB_CONFIG in its EnvSpec — chaos_flexlb_config / ttl_spec /
quota_spec and their injection-gate counterparts — so re-running those
cases under another --profile would execute the identical configuration.
They are declared profiles=["batch-window"] for label honesty and
regression efficiency (new profile variants are dedicated-phase
material).  chaos_coldstart_burst runs on a real per-profile config
(coldstart_spec has no config override) but stays scoped to batch-window
this round — its intake-probe LB assertion is dispatcher-independent,
and spreading it is deliberate later-phase work, not a semantic
restriction.
"""
...
def chaos_flexlb_config(
    ...
    """Chaos-environment FLEXLB_CONFIG (BATCH + fixed_window + FIFO — the
    v2 chaos axes rendered onto the main flat config; PRIORITY ordering
    has no main equivalent and degrades to FIFO), generated through the
    unified harness.build_flexlb_config template.

    Shorter flexlbInflightTtlMs (30s vs the main Java default 300s) so the
    TTL cleanup cases finish within their 90s caps.
    """
...
def quota_spec(ctx: CaseContext) -> EnvSpec:
    ...
    (the flat flexlbBatchFixedMaxInflightBatches key — under main it is
    also env-overridable as FLEXLB_BATCH_FIXED_MAX_INFLIGHT_BATCHES)."""
    return EnvSpec(
