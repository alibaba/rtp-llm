...
def main():
    ...
    if per_second:
        ...
        if (
                ...
                five_series.append(
                    (
                        ...
                        const(
                            ...
                            num_arr(
                                [
                                    (ps_by_t.get(t) or {}).get("e2e_p95", 0)
                                    for t in tsec_vals
                                ]
                            ),
                        ),
            ...
            _five_title = (
                "六延迟：full_e2e / e2e / ttft / schedule / prefill exec / decode exec"
                if has_full_e2e
                else "五延迟：e2e / ttft / schedule / prefill exec / decode exec"
            )
            latency_containers.append(
                emit_container(
                    _five_title,
    ...
    if has_birth_pe or has_birth_de or used_completion_pe or used_completion_de:
        print(
            ...
            + " (birth = rid-joined per-second buckets, same axis as e2e/full_e2e)"
        )
    print(
        TAG
        + " gini scope: "
