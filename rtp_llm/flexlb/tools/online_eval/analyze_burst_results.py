...
def generate_report(runs: List[SpeedRunData], sla_ms: float, speeds: List[int]) -> str:
    ...
    L.append("  - FLEXLB_CONFIG.scheduler=QUEUE / ordering=PRIORITY")
    L.append("  - FLEXLB_CONFIG.dispatcher=BATCH")
    L.append("  - Prefill selector=ESTIMATED_TTFT / RANDOM_WITHIN_TOLERANCE")
    L.append("  - Decode selector=KV_USAGE_WEIGHTED_RANDOM")
