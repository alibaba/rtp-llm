...
axis no longer exists — all three v1 modes mapped to the same configuration,
...
SINGLE/FIXED_WINDOW decision × BATCH/NON_BATCH dispatcher); the harness renders
it as the FLAT main-branch `FLEXLB_CONFIG` document (camelCase keys parsed
directly into FlexlbConfig — `defaultScheduleMode`,
`flexlbBatchAlgorithm`/`flexlbBatchFixedWaitMs`/`flexlbBatchSizeMax`,
`flexlbInflightTtlMs`, `costFormula` …). BATCH dispatcher →
`defaultScheduleMode=BATCH`; NON_BATCH → `DIRECT`; SINGLE decision →
fixed_window with wait 0 + sizeMax 1. Profile fields without a flat equivalent
(PRIORITY ordering, the SLO queue-deadline admission gate,
delivered-not-accepted lifecycle) are dropped — see the UNMAPPED list in
`flexlb_ft/harness.py` `build_flexlb_config`. The v1 "direct" mode name was
doubly misleading: v1 direct still routed through the master (only the
delivery leg was frontend-sent), and frontend-sending is now approximated by
the `DIRECT` schedule mode, not a dispatcher axis.
