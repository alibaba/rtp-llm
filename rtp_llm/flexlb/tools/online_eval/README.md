...
`run_online_eval.sh` sources `lib_load_client.sh` for the JavaLoadClient
launcher `run_java_load_client` (env-var mapping in one place) and the
JDK 21 detection helpers; the mock cluster itself is started with the
plain `java -jar` invocation shown above.
