...
#
# Flow:
#   1. Start the Java mock engine cluster once (reused across all groups)
...
STRATEGY_CONFIGS='{}'
# Score-tie randomization disabled for scheduling smoke tests.  When
# enabled, COST_BASED_PREFILL uses reservoir sampling among workers
# whose scores fall within the tie threshold — this introduces
# non-deterministic routing that breaks determinism assertions (e.g. S6).
# With this flag off the tie threshold collapses to 0, so only workers
# with the exact minimum score enter the candidate pool.
