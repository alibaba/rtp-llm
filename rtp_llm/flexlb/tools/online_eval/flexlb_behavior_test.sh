...
#
# FlexLB engine disconnect/reconnect behavior test (pure Java stack).
#
# Uses the single-JVM Java mock engine cluster's HTTP control API
# (/stop_engine, /start_engine, /set_perf — driven through lib_load_client.sh's
# start_java_mock_cluster / wait_mock_cluster_ready / mock_http /
# stop_java_mock_cluster helpers) to test FlexLB's behavior when an engine's
# gRPC server stops and restarts. Load is generated by JavaLoadClient via
# run_java_load_client (env-var configuration).
#
# Scenarios:
...
#
# Exit code: 0 when every executed scenario PASSes (WARN tolerated), 1 when
# any scenario FAILs. Verdicts are read back from each scenario's result.json
# after the final report is generated, so a FAIL still leaves the report on
# disk.
# ===========================================================================
...
# send_requests calls); keep the heap small so startup stays fast. These must
# be set before sourcing the lib (it applies the defaults at source time).
...
# run_java_load_client, require_java21, ...). Requires FLEXLB_DIR.
export FLEXLB_DIR
source "${SCRIPT_DIR}/lib_load_client.sh"

# Make sure a JDK >= 21 is active for the mock cluster, the load client and
# the FlexLB master. When JAVA_HOME is not exported, derive it from the java
# binary on PATH (the lib's own detection has no macOS fallback); then let
# require_java21 validate the result (replaces the hard-coded JAVA_HOME of
# the old layout).
if [[ -z "${JAVA_HOME:-}" ]] && command -v java >/dev/null 2>&1 \
   && [[ "$(java_major java)" -ge 21 ]]; then
  export JAVA_HOME="$(dirname "$(dirname "$(command -v java)")")"
fi
require_java21

# -- Module opens for the FlexLB master JVM --------------------------------
JAVA_MODULE_OPTS=(
...
# Java mock engine cluster tuning, consumed by start_java_mock_cluster
# (env-var contract). The mock cluster for these behavior scenarios is small
# (1P+1D .. 2P+2D), so a compact heap and few threads are enough.
...
# The mock cluster only reads the prefill execution-time formula out of this
# zone-process config (MockPerformanceModel.loadPrefillExpression); the FlexLB
# master itself is configured exclusively via FLEXLB_CONFIG below. Same file
# run_online_eval.sh passes as --master-config.
MOCK_MASTER_CONFIG="${MOCK_MASTER_CONFIG:-${SCRIPT_DIR}/data/config/master_fixed_window.json}"
export MOCK_JVM_XMS MOCK_JVM_XMX MOCK_EVENT_LOOP_THREADS MOCK_COMPLETION_THREADS
...
# -- State -----------------------------------------------------------------
# CURRENT_MOCK_DIR is the run_dir passed to start_java_mock_cluster (it owns
# <dir>/mock_engine.pid, endpoints.json, flexlb_env.txt, mock_engine.log).
CURRENT_MOCK_DIR=""
FLEXLB_PID=""
LOAD_PID=""
CURRENT_SCENARIO=""
...
cleanup() {
  ...
  if [[ -n "${CURRENT_MOCK_DIR}" ]]; then
    stop_java_mock_cluster "${CURRENT_MOCK_DIR}"
    CURRENT_MOCK_DIR=""
  fi
  echo "[cleanup] done."
}
...
setup_environment() {
  ...
  # Pre-flight: free all ports (control port = base-1, then gRPC ports)
  local ports=("${FLEXLB_HTTP_PORT}" "${FLEXLB_MANAGEMENT_PORT}" "$((MOCK_BASE_GRPC_PORT - 1))")
  ...
  # Start the Java mock engine cluster (single JVM). Env knobs come from the
  # lib contract: MOCK_N_PREFILL/MOCK_N_DECODE/MOCK_BASE_GRPC_PORT plus the
  # exported MOCK_PERFORMANCE_FILE / MOCK_MASTER_CONFIG. The cluster writes
  # endpoints.json / flexlb_env.txt / mock_engine.{log,pid} into scenario_dir.
  log "  starting Java mock cluster (${n_prefill}P + ${n_decode}D) ..."
  MOCK_N_PREFILL="${n_prefill}"
  MOCK_N_DECODE="${n_decode}"
  if ! start_java_mock_cluster "${scenario_dir}"; then
    echo "ERROR: failed to start Java mock cluster" >&2
    tail -50 "${scenario_dir}/mock_engine.log" >&2 || true
    exit 1
  fi
  CURRENT_MOCK_DIR="${scenario_dir}"
  if ! wait_mock_cluster_ready "${MOCK_BASE_GRPC_PORT}" "$((n_prefill + n_decode))" 60; then
    echo "ERROR: Java mock cluster not ready within 60s" >&2
    tail -50 "${scenario_dir}/mock_engine.log" >&2 || true
    exit 1
  fi
  log "  mock cluster ready (control=${MOCK_CONTROL_PORT}, endpoints=${MOCK_ENDPOINT_FILE})"

  # Start FlexLB Master (config injected via the strict FLEXLB_CONFIG JSON)
  log "  starting FlexLB Master ..."
  # Parse service-discovery env vars from the endpoint file the cluster wrote
  local env_args=()
  while IFS= read -r line; do
    env_args+=("${line}")
  done < <(python3 - "${MOCK_ENDPOINT_FILE}" <<'PY'
import json, sys
...
PY
  ...
  if ! wait_for_port "127.0.0.1" "${FLEXLB_HTTP_PORT}" 60; then
    echo "ERROR: master port ${FLEXLB_HTTP_PORT} not reachable within 60s" >&2
    tail -50 "${scenario_dir}/flexlb_master.log" >&2 || true
    exit 1
  fi
...
# Get a field from a specific mock engine by name (control API /snapshot)
get_mock_field() {
  local engine_name="$1"
  local field="$2"
  curl -s "http://127.0.0.1:${MOCK_CONTROL_PORT}/snapshot" 2>/dev/null | \
    python3 -c "
import json, sys
try:
    data = json.load(sys.stdin)
...
# Stop a mock engine via the cluster control API
stop_engine() {
  local engine_name="$1"
  log "  POST /stop_engine {\"engine\": \"${engine_name}\"}"
  mock_http POST "${MOCK_CONTROL_PORT}" "/stop_engine" \
    "{\"engine\": \"${engine_name}\"}" 2>/dev/null || true
  echo ""
}

# Start a mock engine via the cluster control API
start_engine() {
  local engine_name="$1"
  log "  POST /start_engine {\"engine\": \"${engine_name}\"}"
  mock_http POST "${MOCK_CONTROL_PORT}" "/start_engine" \
    "{\"engine\": \"${engine_name}\"}" 2>/dev/null || true
  echo ""
}

# Set performance parameter for a mock engine via /set_perf (Python-compatible
# field names: prefill_fixed_ms, decode_scale — verified against the Java
# MockControlServer handler)
set_perf() {
  local engine_name="$1"
  local perf_key="$2"   # prefill_fixed_ms or decode_scale
  local perf_val="$3"
  mock_http POST "${MOCK_CONTROL_PORT}" "/set_perf" \
    "{\"engine\": \"${engine_name}\", \"${perf_key}\": ${perf_val}}" \
    >/dev/null 2>&1 || true
...
# Send requests synchronously via JavaLoadClient. Runs in a subshell so
# run_java_load_client's exec only replaces the subshell; summary.json /
# per_request.jsonl land in <output_dir> exactly like the old client.
send_requests() {
  local trace="$1"
  local limit="$2"
  local concurrency="$3"
...
w(f"**Stack**: Java mock engine cluster (JavaMockEngineCluster) + JavaLoadClient")
...
# ===========================================================================
# Verdict aggregation: propagate scenario FAILs to the exit code
# ===========================================================================
# Reads every scenario*/result.json written under the run dir. Any "pass":
# "FAIL" (WARN tolerated) makes the whole run exit 1.
check_verdicts() {
  python3 - "${RUN_DIR}" <<'PY'
import json
import sys
...
failed = []
for path in sorted(Path(sys.argv[1]).glob("scenario*/result.json")):
    try:
        result = json.loads(path.read_text(encoding="utf-8"))
    except Exception:
        continue
    if result.get("pass") == "FAIL":
        reasons = result.get("reasons", "").strip()
        failed.append(f"{path.parent.name}: {reasons}" if reasons else path.parent.name)
if failed:
    print("Verdict: FAIL", file=sys.stderr)
    for item in failed:
        print(f"  - {item}", file=sys.stderr)
    sys.exit(1)
print("Verdict: PASS (all executed scenarios passed; WARN tolerated)")
PY
}
...
if [[ ! -f "${FLEXLB_JAR}" ]]; then
  ...
  echo "Build it with: (cd \"${FLEXLB_DIR}\" && ./mvnw -P\"${MAVEN_PROFILES}\" -pl flexlb-api -am package -DskipTests)" >&2
  exit 1
fi
...
if [[ ! -f "${MOCK_MASTER_CONFIG}" ]]; then
  echo "ERROR: master config for the mock cluster not found: ${MOCK_MASTER_CONFIG}" >&2
  exit 1
fi
...
main() {
  ...
  # Verdict propagation: any executed scenario FAIL -> exit 1 (report is
  # already on disk at this point).
  local overall=0
  if ! check_verdicts; then
    overall=1
  fi
  echo "=========================================="
  exit "${overall}"
