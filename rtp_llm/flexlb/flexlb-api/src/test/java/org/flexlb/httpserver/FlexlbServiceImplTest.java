...
class FlexlbServiceImplTest {
    ...
    static Stream<Arguments> forwardFailureCases() {
        ...
        return Stream.of(
                ...
                Arguments.of(
                        CompletableFuture.completedFuture(FlexlbGrpcForwarder.MasterForwardResult.failed(
                                Status.UNAVAILABLE.asRuntimeException(), host)), 8510),
                Arguments.of(
                        CompletableFuture.completedFuture(FlexlbGrpcForwarder.MasterForwardResult.failed(
                                new IllegalStateException("DEADLINE_EXCEEDED"), host)), 8510),
                Arguments.of(
                        CompletableFuture.completedFuture(FlexlbGrpcForwarder.MasterForwardResult.failed(
                                "DEADLINE_EXCEEDED", host)), 8510));
    }

    @ParameterizedTest
    @MethodSource("forwardFailureCases")
    void forwardFailureUsesExceptionTypeAndNeverDiagnosticText(
    ...
    void ambiguousForwardFailureReturnsImmediatelyWithoutCancelOrLocalSchedule() {
        ...
        for (var status : java.util.List.of(Status.UNAVAILABLE, Status.DEADLINE_EXCEEDED)) {
            ...
            verify(observer).onNext(response.capture());
            verify(observer).onCompleted();
            assertEquals(status.getCode() == Status.Code.DEADLINE_EXCEEDED
                    ? StrategyErrorType.BATCH_SLO_EXPIRED.getErrorCode()
                    : StrategyErrorType.BATCH_DISPATCH_FAILED.getErrorCode(), response.getValue().getCode());
    ...
    void formerMasterDoesNotClaimAnOwnedRequestWasNeverAccepted() {
        when(lbStatusConsistencyService.isNeedConsistency()).thenReturn(true);
        when(lbStatusConsistencyService.isMaster()).thenReturn(false);
        when(routeService.getRequestState(88L, 0L)).thenReturn(new RequestLifecycleSnapshot(88L,
                RequestLifecycleState.ACKNOWLEDGED, DeliveryClaimKind.BATCH_ENQUEUE,
        ...
        assertEquals(StrategyErrorType.BATCH_DISPATCH_FAILED.getErrorCode(), result.getValue().getCode());
    ...
    void followerQueriesAndCancelsItsLocalOwnerBeforeConsultingTheLeader() {
        when(lbStatusConsistencyService.isNeedConsistency()).thenReturn(true);
        when(lbStatusConsistencyService.isMaster()).thenReturn(false);
        RequestLifecycleSnapshot owned = new RequestLifecycleSnapshot(86L, RequestLifecycleState.ACKNOWLEDGED,
