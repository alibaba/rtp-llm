...
public class RequestCancelTest {
     ...
     *   4. 触发服务端的 doOnCancel() → RouteService.cancelRequest()
    ...
    public void run() {
        ...
        try {
            ...
            Thread.sleep(1000);
            // 验证 routeService.cancelRequest() 被调用一次
            verify(routeService).cancelRequest(ArgumentMatchers.any());
            log.info("routeService.cancelRequest() 被调用一次");
