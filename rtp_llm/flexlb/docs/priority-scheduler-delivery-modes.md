...
DIRECT must omit `queueTimeoutMs`, ordering and decision. FIFO must not contain priority settings.
SINGLE must not contain window settings. NON_BATCH must not contain batch credits.
All modes use the same request registry, per-Prefill concurrency admission and request/decision lifetimes.
