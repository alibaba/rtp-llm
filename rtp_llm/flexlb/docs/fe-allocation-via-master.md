...
`GET /dispatcher/_snapshot` returns FE URLs, liveness/failure counts and the split/allocation settings, without routing credentials. It reads local state without probing, calling master or advancing cursors, and is excluded from graceful-drain accounting. `_dryrun` is not provided.
