...
```
Set these two variables in the platform's advanced environment settings to use the defaults. Defaults apply to unset settings; explicit empty or invalid values go through the same startup validation as native Spring properties. Dispatcher exposes only the following settings:
