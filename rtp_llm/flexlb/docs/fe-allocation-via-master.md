...
```
Followers forward once; unknown leaders, self-forwarding, repeated hops and transport failures fail without local allocation. Upgrade every potential master to the same batch-allocation protocol, including `allocation_type` and `frontend_urls`, before enabling Dispatcher. Responses from older masters lacking the requested FE list fail validation; worker-only RTP callers retain their existing address schema.
