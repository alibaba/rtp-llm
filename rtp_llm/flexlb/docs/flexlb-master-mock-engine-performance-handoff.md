...
- 必须使用 `SCHEDULE_MODE=batch`；load client 的测试开关是
  `FETCH_OUTPUT_STREAM=0`：客户端不读输出流，engine 侧仍完整执行
  prefill+decode。10 ms base case 使用 `FLEXLB_BATCH_FIXED_WAIT_MS=10`；SLO
  case 使用独立配置文件中的 500 ms 预测阈值和 160 ms 固定等待。
