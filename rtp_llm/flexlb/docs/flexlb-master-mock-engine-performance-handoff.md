...
- Master 进程配置必须在 `FLEXLB_CONFIG` 中选择
  ...
  `dispatcher.maxCollectionWaitMs=10`（本线为 schema v1 平铺
  dispatcher 字段，对应上游 v2 的 scheduler.decision.*）；当前仓库的
  ...
  `dispatcher.maxCollectionWaitMs=160`；fixture 在本线保持 v1
  dispatcher 平铺字段。
...
python3 -m py_compile tools/online_eval/flexlb_functional_tests.py tools/online_eval/parallel_runner.py
```
...
```text
-Dflexlb.perf.e2e.requests=8192
-Dflexlb.perf.min-e2e-qps=5000
-Dflexlb.perf.max-e2e-server-p99-ms=250
...
| Prefill engine | Decode engine | 目标发送 QPS |
|---:|---:|---:|
| 1 | 2 | 1000 / 2000 / 5000 / 10000 |
| 2 | 4 | 2000 |
| 4 | 8 | 1000 / 2000 / 5000 / 10000 |
| 8 | 16 | 2000 |
| 16 | 32 | 1000 / 2000 / 5000 / 10000 |
...
```text
-Dflexlb.perf.engine-matrix-duration-ms=500
-Dflexlb.perf.engine-matrix-min-requests=1024
-Dflexlb.perf.engine-matrix-min-qps-ratio=0.85
-Dflexlb.perf.engine-matrix-max-server-p99-ms=250
-Dflexlb.perf.engine-matrix-max-batch-wait-p99-ms=50
```
...
`fast_ab` 模型当前使用 `sleep_scale=1.0` 和显式 decode step 曲线。它用于可重复的 Master/Mock 对比，但仍不代表真实 LLM engine 的绝对 TTFT 或端到端容量。做 Master 容量测试前必须确认 mock 仍有余量；需要评估真实 engine 容量时，应换成实测 performance model。
...
PERFORMANCE_FILE="$PWD/data/performance/dsv4_flash_performance.sample.json" \
