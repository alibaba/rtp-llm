...
over its gRPC and HTTP control planes. Use `--category` to select a scenario
family and `--profile batch-window|single-nonbatch|single-batch|window-nonbatch`
to select the scheduling shape, or `parallel_runner.py` to execute isolated
lanes in parallel.
