...
class LocalStandbyCacheManagerTest {
    ...
    void appliesWorkerReportedCacheMatchRollback() {
        WorkerStatusProvider workerStatusProvider = mock(WorkerStatusProvider.class);
        WorkerStatus worker = worker("10.0.0.1", 8080, RoleType.PDFUSION, 1);
    ...
    private WorkerStatus worker(String ip, int port) {
        return worker(ip, port, RoleType.PREFILL, 0);
    }

    private WorkerStatus worker(String ip, int port, RoleType role, int cacheMatchRollbackBlocks) {
        return workerStatus(ip, port, role, false, null, cacheMatchRollbackBlocks);
    }
