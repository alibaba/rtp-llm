...
public class BlockHashExecutor {
    ...
    public BlockHashExecutor(
        ...
        if (maxThreadCount < coreThreadCount) {
            throw new IllegalArgumentException("flexlb.block-hash.max-thread-count must be at least core-thread-count");
        }
    ...
    private <T> Mono<TimedTaskResult<T>> submitTimed(Callable<T> task) {
        return Mono.defer(() -> {
            ...
            return Mono.fromCallable(() -> {
                        ...
                        monitor.report(BLOCK_HASH_QUEUE_WAIT_TIME_US, queueWaitTimeUs);
                        try {
                            T value = task.call();
