...
def init_moe_group_args(parser, moe_config, eplb_config, deep_ep_config):
    ...
    moe_group.add_argument(
        "--enable_elastic_ep",
        env_name="ENABLE_ELASTIC_EP",
        bind_to=(moe_config, "enable_elastic_ep"),
        type=str2bool,
        default=False,
        help="是否使用弹性EP扩缩容，目前仅支持纯DP decode deepep_low_latency mode下的缩容，并且需要配置--redundant_expert支持专家重新分布",
    )
