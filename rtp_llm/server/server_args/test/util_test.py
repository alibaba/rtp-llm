...
class BoundedConverterTest(unittest.TestCase):
    ...
    def test_bounded_int_boundaries_and_invalid_inputs(self):
        ...
        for value in ("-1", "4"):
            with self.subTest(value=value):
                self.assertArgumentError(
                    lambda raw: bounded_int(raw, max_value=3),
                    value,
                    "[0, 3]",
                    repr(value),
                )
        for value in ("", "x"):
            with self.subTest(value=value):
                self.assertArgumentError(
                    bounded_int, value, "must be an integer", repr(value)
                )
    ...
    def test_bounded_float_boundaries_and_invalid_inputs(self):
        ...
        exclusive = partial(bounded_float, max_value=1.0, max_value_exclusive=True)
