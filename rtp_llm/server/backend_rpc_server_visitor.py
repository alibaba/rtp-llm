...
class BackendRPCServerVisitor:
    def __init__(
        ...
        self.mm_padding_size = mm_padding_size
    ...
    async def get_master_route_addrs(
        ...
        try:
            ...
            prepared = getattr(input, "v41_inputs", None)
            prepared_remote_vit = bool(
                getattr(self, "remote_vit", False)
                and prepared is not None
                and prepared.images
            )
            if prepared_remote_vit or (image_token_id is not None and input.mm_inputs):
                ...
                if not prepared_remote_vit and len(image_locs) != len(input.mm_inputs):
                    raise FtRuntimeException(
                        ExceptionType.INVALID_PARAMS,
                        "DeepSeek image placeholders and inputs must match",
                    )
                if self.mm_padding_size:
                    for i, (loc, mm_input) in enumerate(
                        zip(image_locs, input.mm_inputs)
                    ):
                        start = loc if i == 0 else loc - image_locs[i - 1]
                        mm_input.config = replace(
                            mm_input.config,
                            mm_padding_size=self.mm_padding_size
                            - 1
                            - start % self.mm_padding_size,
                        )
                ...
                routing_input = raw_input
                if not prepared_remote_vit:
                    ...
                    # V4 routes with remote metadata; V4.1 already carries canonical IDs.
    ...
    async def _get_vit_token_ids(self, vit: RoleAddr, input_pb, timeout_s: float):
        ...
        try:
            response = await MultimodalRpcServiceStub(
                channel
            ).RemoteMultimodalEmbedding(request, timeout=timeout_s)
            ...
            }.get(error.code(), ExceptionType.MM_PROCESS_ERROR)
            route_logger.warning(
                "ViT metadata RPC failed: %s, %s", error.code(), error.details()
            )
            raise FtRuntimeException(
                exception_type, f"ViT metadata RPC failed: {error.code().name}"
            ) from error
...
def create_backend_rpc_server_visitor(
    ...
    return BackendRPCServerVisitor(
        ...
        mm_padding_size=model_config.mm_model_config.mm_padding_size,
    )
