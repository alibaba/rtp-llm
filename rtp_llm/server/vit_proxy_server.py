...
class LoadBalancer:
    ...
    def apply_worker_observation(
        self,
        worker_address: str,
        alive: bool,
        expected_version: Optional[int] = None,
    ) -> tuple[bool, bool]:
        """Apply a current observation; return ``(applied, resulting_alive)``.

        ``applied`` is False when the expected version does not match, i.e. a
        newer event already superseded this observation.
        """
        with self.lock:
            ...
            if expected_version is not None and version != expected_version:
                return False, self.worker_alive[worker_address]
    ...
    def set_worker_alive(
        ...
        return self.apply_worker_observation(
            worker_address, alive, expected_version
        )[1]
...
class VitProxyRpcServer(MultimodalRpcServiceServicer):
    ...
    def _complete_status_probe(
        ...
        with self._status_probes_lock:
            ...
            applied = False
            if active:
                applied, current_alive = self.load_balancer.apply_worker_observation(
                    worker_address,
                    worker_status is not None,
                    expected_version=probe.health_version,
                )
                self._status_probes.pop(worker_address, None)
        if not current_alive or not applied:
            # Unapplied observations are stale: a newer event (for example a
            # successful forwarding) already updated the load balancer, so a
            # stale probe result must never override it for subscribers.
            worker_status = None
        probe.complete(
    ...
    def _subscribe_status_probe(
        ...
        new_probe = False
        subscribed = False
        # Note: this version read and the probe creation below are not atomic.
        # A forwarding failure landing in between bumps the version, so this
        # probe's result may be discarded as stale in _complete_status_probe.
        # Recovery is bounded by the background recovery loop
        # (WORKER_RECOVERY_INTERVAL_SEC), so the extra window is acceptable.
    ...
    def refresh_worker_health(
        ...
        *,
        ...
        while pending:
            ...
            pending -= 1
        # The load balancer holds the versioned, latest observation of every
        # worker (probe callbacks apply theirs through apply_worker_observation).
        # Reporting the stale per-probe result instead would under-report health
        # when a forwarding success superseded a late dead probe.
        alive = set(self.load_balancer.get_alive_worker_addresses())
        return {addr: addr in alive for addr in results}
