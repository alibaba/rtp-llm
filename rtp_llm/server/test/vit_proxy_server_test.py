...
class VitProxyRpcServerStatusTest(TestCase):
    ...
    def test_late_probe_failure_does_not_erase_new_forwarding_success(self):
        ...
        # The stale dead probe result must not leak to subscribers either: the
        # newer forwarding success supersedes it.
        self.assertIsNone(results.get_nowait().worker_status)
        # Health reporting follows the load balancer's versioned verdict
        # instead of under-reporting from the stale probe result.
        self.assertEqual(server.refresh_worker_health(timeout_s=0.05), {"worker": True})

    def test_apply_worker_observation_reports_whether_the_write_applied(self):
        balancer = LoadBalancer(["worker"])
        self.assertEqual(
            balancer.apply_worker_observation("worker", False), (True, False)
        )
        # Unknown workers never apply.
        self.assertEqual(
            balancer.apply_worker_observation("missing", True), (False, False)
        )
        # A superseded expected_version write does not apply and reports the
        # current alive state instead.
        self.assertEqual(
            balancer.apply_worker_observation("worker", True, expected_version=0),
            (False, False),
        )
        # A fresh versioned write applies.
        self.assertEqual(
            balancer.apply_worker_observation("worker", True, expected_version=1),
            (True, True),
        )
        self.assertEqual(balancer.get_alive_worker_addresses(), ["worker"])
