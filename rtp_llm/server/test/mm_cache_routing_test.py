...
class MMCacheRoutingIntegrationTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_master_cannot_change_the_selected_vit(self):
        self.visitor.master_client.get_backend_role_addrs.side_effect = [
            FlexlbResponse(role_addrs=[self.vit], server_status=[self.status]),
    ...
    async def test_vit_route_uses_short_rpc_timeout_without_changing_request_budget(self):
        client = MasterClient(
        ...
        client._send_schedule_request = AsyncMock(return_value=response)
        ...
        request_pb = call.args[1]
        self.assertEqual(request_pb.generate_timeout, 30000)
        self.assertEqual(list(request_pb.media_keys), ["image"])
        self.assertTrue(request_pb.vit_route_only)
...
class MMCacheApiTest(unittest.TestCase):
    ...
    def test_rdma_response_keeps_inline_feature_hashes(self):
        ...
        exporter = Mock()
        slot = MMRdmaSlotPB(
            rdma_descriptor=RdmaDescriptorPB(lease_id="handle"),
            roles=[MMRdmaSlotPB.EMBEDDING],
        )
        ...
        response = RdmaOutputBackend(exporter).transfer(
            MultimodalInputsPB(support_rdma=True), result
        ).receipt
        self.assertEqual(response.output_rdma_slots[0].rdma_descriptor.lease_id, "handle")
