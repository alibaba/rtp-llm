...
class MMCacheRoutingTest(unittest.TestCase):
    def test_shared_key_resolves_request_overrides_and_ignores_timeout(self):
        item = MultimodalInput(
            ...
            1,
            torch.empty(0),
            MMPreprocessConfig(-1, -1, 10, 2000, 1, 1, 8, [0.1, 0.2], 10, 100),
        )
        cfg = GenerateConfig(
            min_pixels=20,
            max_pixels=4000,
    ...
    def test_shared_key_normalizes_disabled_long_side_limit(self):
        for media_limit in (-1, 0, -2, 100):
            for request_limit in (-1, 0, -2, 200):
                with self.subTest(media_limit=media_limit, request_limit=request_limit):
                    item = MultimodalInput(
                        "https://example/image",
                        1,
                        torch.empty(0),
                        MMPreprocessConfig(
                            -1, -1, -1, -1, -1, -1, -1, [], -1, media_limit
                        ),
                    )
                    cfg = GenerateConfig(max_long_side_pixel=request_limit)
                    request = GenerateInput(1, torch.tensor([1]), [item], cfg)
                    wire = GenerateInputPB()
                    trans_multimodal_input(request, wire, cfg)
                    worker_config = trans_config(
                        wire.multimodal_inputs[0].mm_preprocess_config
                    )
                    resolved_limit = (
                        request_limit if request_limit != -1 else media_limit
                    )
                    self.assertEqual(
                        worker_config.max_long_side_pixel,
                        resolved_limit if resolved_limit > 0 else -1,
                    )
                    worker_key = MultimodalInput(
                        item.url, item.mm_type, torch.empty(0), worker_config
                    ).cache_key()
                    self.assertEqual(multimodal_cache_keys(request), [worker_key])
...
class MMCacheRoutingIntegrationTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_old_master_falls_back_to_ordinary_schedule(self):
        ...
        # Ordinary Schedule may already own/admit this request on main.
        # Do not issue a second admission after retrieving optional hashes.
    ...
    async def test_short_route_deadline_preserves_request_timeout(self):
        client = MasterClient(
        ...
        self.assertTrue(call.args[1].vit_route_only)
        self.assertEqual(call.args[1].generate_input, b"")
        await client.close()
