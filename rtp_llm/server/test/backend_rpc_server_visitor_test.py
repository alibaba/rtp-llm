...
class BackendRPCServerVisitorEnqueueRetryTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_enqueue_retry_reuses_role_candidate_when_no_alternative_exists(self):
        visitor = BackendRPCServerVisitor.__new__(BackendRPCServerVisitor)
        visitor.max_seq_len = 10
        visitor.sp_config = None
        visitor.pd_route_retry_on_unavailable = 1
        ...
        only_decode = RoleAddr(
            role=RoleType.DECODE, ip="10.0.0.3", http_port=300, grpc_port=301
        )

        async def route_ips(input):
            input.generate_config.role_addrs = [stale_prefill, only_decode]
        ...
        self.assertEqual(
            client.seen_role_addrs,
            [[stale_prefill, only_decode], [fresh_prefill, only_decode]],
        )
        self.assertEqual(input.generate_config.role_addrs, [fresh_prefill, only_decode])
    ...
    async def test_enqueue_retry_reuses_only_candidate_before_exhaustion(self):
        visitor = BackendRPCServerVisitor.__new__(BackendRPCServerVisitor)
        visitor.max_seq_len = 10
        visitor.sp_config = None
        visitor.pd_route_retry_on_unavailable = 1
        ...
        class _AlwaysFailModelRpcClient:
            def __init__(self):
                self.calls = 0
                self.seen_role_addrs = []
        ...
        client = _AlwaysFailModelRpcClient()
        visitor.model_rpc_client = client
        ...
        self.assertEqual(client.calls, 2)
        self.assertEqual(client.seen_role_addrs, [[stale_addr], [stale_addr]])

    async def test_enqueue_retry_falls_back_to_route_ips_when_candidates_missing(self):
        visitor = BackendRPCServerVisitor.__new__(BackendRPCServerVisitor)
        visitor.max_seq_len = 10
        visitor.sp_config = None
        visitor.pd_route_retry_on_unavailable = 1
        ...
        master_prefill = RoleAddr(
            role=RoleType.PREFILL, ip="10.0.0.2", http_port=200, grpc_port=201
        )
        master_decode = RoleAddr(
        ...
        async def route_ips(input):
            ...
            if len(route_calls) == 1:
                ...
                input.generate_config.role_addrs = [master_prefill, master_decode]
        ...
        self.assertEqual(
            client.seen_role_addrs,
            [[stale_prefill, stale_decode], [master_prefill, master_decode]],
        )

    async def test_enqueue_retry_falls_back_to_route_ips_for_incomplete_domain_route(self):
        visitor = BackendRPCServerVisitor.__new__(BackendRPCServerVisitor)
        visitor.max_seq_len = 10
        visitor.sp_config = None
        visitor.pd_route_retry_on_unavailable = 1
        ...
        async def route_ips(input):
            ...
            if len(route_calls) == 1:
                ...
                input.generate_config.role_addrs = [stale_prefill, fresh_decode]
        ...
        self.assertEqual(
            client.seen_role_addrs,
            [[stale_prefill, stale_decode], [stale_prefill, fresh_decode]],
        )
