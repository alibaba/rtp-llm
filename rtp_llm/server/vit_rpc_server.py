...
def vit_start_server(on_ready=None, on_prebind=None):
    """Start the VIT RPC server with optional pre/post-bind hooks.

    ``on_prebind`` is called after model initialization but before the socket
    is bound; this is the SCR template safe point. ``on_ready`` remains a
    post-bind lifecycle hook for existing callers.
    """
