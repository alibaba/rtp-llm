...
def _format_tokens(content_tokens: List[int], role_special_tokens: Any) -> List[int]:
    content_tokens = (
        role_special_tokens.token_ids
        + content_tokens
        + role_special_tokens.eos_token_ids
    )
...
def encode_chatapi(
    ...
    max_input_tokens = (
        2**32
    )  # int max, maybe support max_history_len in generate_config
    ...
    for i, message in enumerate(messages[::-1]):
        ...
        if message["role"] == "user":
            ...
            round_input = (
                _format_tokens(content_tokens, special_tokens.assistant) + round_input
            )
    ...
    if system_input:
        total_input = (
            _format_tokens(tokenizer.encode(system_input), special_tokens.system)
            + total_input
        )
