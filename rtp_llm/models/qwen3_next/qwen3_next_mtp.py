...
class Qwen35DenseMTPWeight(Qwen3NextMTPWeight):
    def __init__(self, *args: List[Any], **kwargs: Dict[str, Any]):
        super().__init__(*args, **kwargs)
        self.model_prefix = "model.language_model."
...
class Qwen35DenseMTP(Qwen35Dense):
    ...
    def _create_config(cls, ckpt_path: str) -> ModelConfig:
        config = super()._create_config(ckpt_path)
        # The draft layer is standard MHA even though the target model uses a
        # hybrid linear/full-attention layout.
        config.hybrid_attention_config.hybrid_attention_types = [
            HybridAttentionType.NONE
        ]
        ...
        config.num_layers = 1
        config.is_mtp = True
        # Draft MTP consumes text tokens only, so plain RoPE is sufficient.
        config.attn_config.rope_config.style = 1
        return config
