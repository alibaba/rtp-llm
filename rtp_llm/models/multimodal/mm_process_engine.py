...
class MMProcessEngine:
    ...
    def __init__(self, model: Any, py_env_configs: PyEnvConfigs):
        """Initialize the multimodal process engine."""
        self.model = model
        self.vit_config = py_env_configs.vit_config
        self.contains_pos: bool = (
            self.model.model_config.mm_model_config.mm_position_ids_style != 0
        )
        self.mm_preprocess_batch_size: int = (
            self.model.model_config.mm_related_params.preprocess_batch_size
        )
        ...
        for attempt in range(max_retries):
            try:
                work_item.may_submit_preprocess(
                    self.model.mm_part, self.vit_config, self.mm_preprocess_executor
                )
            except concurrent.futures.process.BrokenProcessPool:
