...
class MultiModalMixin:
    ...
    def multimodal_modify_prompt_plugin(
        ...
        **kwargs: Any,
        ...
        if (
            kwargs.get("generate_config", {})["request_format"]
                ...
                if isinstance(message["content"], str):
                    ...
                    for x in message["content"]:
                        if x["type"] == "text":
                            ...
                            raise FtRuntimeException(
                                ExceptionType.ERROR_INPUT_FORMAT_ERROR,
                                "content type can only be text or image_url, but get: "
                                + x["type"],
                            )
                    new_prompt += "\n"
            return new_prompt + "ASSISTANT :", [
                MultimodalInput(new_url) for new_url in new_urls
            ]
        elif isinstance(prompt, List):
            raise FtRuntimeException(
                ExceptionType.ERROR_INPUT_FORMAT_ERROR,
                "raw request format cannot accept dict prompt",
            )
