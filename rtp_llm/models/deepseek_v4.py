...
class DeepSeekV4(DeepSeekV2, MultiModalMixin):
    ...
    @staticmethod
    def eval_vit_param_count(mm_related_params):
        cfg = mm_related_params.config
        dim = int(cfg["vision_dim"])
        inter = int(cfg["vision_inter_dim"])
        layers = int(cfg["vision_n_layers"])
        patch = int(cfg["vision_patch_size"])
        ratio = int(cfg["vision_downsample_ratio"])
        hidden = int(cfg["hidden_size"])
        patch_embed = 3 * patch * patch * dim + dim
        vision_blocks = layers * (4 * dim * dim + 3 * dim * inter + 6 * dim)
        aligner = dim * ratio * ratio * hidden + hidden + hidden * hidden + hidden
        return patch_embed + vision_blocks + dim + aligner + 4 * hidden

    @classmethod
    def eval_vit_model_size(cls, mm_related_params):
        return cls.eval_vit_param_count(mm_related_params) * 2
    ...
    def _from_hf(config: ModelConfig, ckpt_path: str):  # noqa: C901  (acceptably long)
        ...
        if vision_n_layers > 0:
            padding_size = int(config_json.get("mm_padding_size", 4))
            if padding_size < 0:
                raise ValueError("mm_padding_size must be non-negative")
            config.mm_model_config.mm_padding_size = padding_size
            vision_keys = (
                "vision_n_layers",
                "vision_dim",
                "vision_n_heads",
            ...
            config.mm_related_params.eval_param_count = DeepSeekV4.eval_vit_param_count
            config.mm_related_params.eval_model_size = DeepSeekV4.eval_vit_model_size
