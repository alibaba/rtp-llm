...
class KimiK3LoadFp8Weight(LoadQuantPerBlockFp8Weight):
    ...
    def _split(self, tensor, load_config):
        ...
        supported = (8, 16) if projection_ktp else (1, 2, 4, 8)
    ...
    def _postprocess(self, tensor, device, load_config):
        ...
        logging.info(
            ...
            "scale_dtype=%s TP=%d rank=%d KTP=%d ktp_rank=%d "
            "derived_mla=%s weight_padding=0 tp_ranges=%s",
            self.layer_id,
            self.kernel.name,
            ...
            load_config.tp_size,
            load_config.tp_rank,
            getattr(load_config, "ktp_size", 1),
            getattr(load_config, "ktp_rank", 0),
            self.derive_mla,
            self.tp_ranges,
        )
        return result
