...
class KimiK3LoadFp8Weight(LoadQuantPerBlockFp8Weight):
    ...
    w8a8_weight_list: Dict[str, str] = {
        ...
        W.linear_attn_f_b_w: K3W.KDA_FORGET_S,
    ...
    def _quantize_matrix(matrix, *, use_ue8m0):
        ...
        for begin in range(0, rows, 1024):
            ...
            block_weight, block_scale = per_block_cast_to_fp8(
                matrix[begin:end], use_ue8m0=use_ue8m0
            )
    ...
    def _split(self, tensor, load_config):
        ...
        if name == K3W.KDA_INPUT:
            ...
            fa_width = weight.shape[0] - offset - heads
            if fa_width <= 0 or fa_width % 128 or heads % parallel:
                raise ValueError("KDA F_a and beta dimensions cannot be FP8-sharded")
            beta_begin = offset + fa_width
            if beta_begin % 128:
                raise ValueError("KDA beta must start on an FP8 scale block")
            self.tp_ranges["replicated"].append((offset, beta_begin))
            rows.append(weight[offset:beta_begin])
            blocks.append(scale[offset // 128 : beta_begin // 128])
            local_heads = heads // parallel
            start = beta_begin + rank * local_heads
            self.tp_ranges["sharded"].append((start, start + local_heads))
            rows.append(weight[start : start + local_heads])
            blocks.append(scale[start // 128 : (start + local_heads + 127) // 128])
            weight, scale = torch.cat(rows), torch.cat(blocks)
            # DeepGEMM requires the output row count to fill FP8 blocks. The
            # sharded beta tail is shorter than one block, so leave the
            # remaining lanes at zero and trim them after the projection.
            padded_rows = ((weight.shape[0] + 127) // 128) * 128
            if padded_rows != weight.shape[0]:
                padding = torch.zeros(
                    (padded_rows - weight.shape[0], weight.shape[1]),
                    dtype=weight.dtype,
