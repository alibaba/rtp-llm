...
class KimiK3WeightNames:
    ...
    # KDA cache/kernel tensors use W.linear_attn_*; the packed K3 input
    # projection has its own key because its component sharding differs.
...
class _KimiK3InputWeight(LinearAttnAtomicWeight):
    def _split(self, tensor, load_config):
        if isinstance(tensor, dict):
            tensor = tensor[self.name]
        cfg = self.config
        if cfg.linear_num_value_heads != cfg.linear_num_key_heads:
            raise ValueError("K3 fused input requires equal query/key/value heads")
        return {
            self.name: split_kda_input(
                tensor,
...
class _KimiExpertByteWeight(MoeAtomicWeight):
    ...
    def _get_split_func(self):
        ...
        # Each EP owner keeps complete packed matrices for its local experts.
        return sp_id
...
class KimiK3Weight(ModelDeployWeightInfo):
    ...
    def _mla_weights(self) -> List[WeightModule]:
        ...
        * ``mla.rope_head_dim`` is a *physical* 64-d suffix but carries no
          positional signal (NoPE). Main skips RoPE when its cache is absent.
    ...
    def _create_rope_w(self) -> Optional[AtomicWeight]:
        # Main's MLA implementations already support absent RoPE weights.
        return None
