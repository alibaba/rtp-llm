...
from rtp_llm.models.whisper_weight import WhisperWeightInfo
...
class WhisperAudioEmbedding(AudioEmbeddingInterface):
    ...
    def audio_embedding(self, audio: Tuple[torch.Tensor, int], device):
        ...
        features = self.processor(
            np.array(audio_data), sampling_rate=self.sampling_rate, return_tensors="pt"
        ).input_features
        ...
        if res.shape[1] != self.embedding_length:
            raise Exception(
                f"Wrong shape embedding for audio input dim 1, expect {self.embedding_length}, but get {res.shape[1]}"
            )
        return res
    ...
    def __init__(self, config: GptInitModelParameters):
        if config.tp_rank == 0:
            with torch.device(g_parallel_info.device):
                ...
                self.mm_part = WhisperAudioEmbedding(
                    WhisperProcessor.from_pretrained(ckpt_path),
                    WhisperEncoder.from_pretrained(ckpt_path),
                    config.cross_attn_input_len,
                )
