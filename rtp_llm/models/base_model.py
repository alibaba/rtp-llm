...
class BaseModel(object):
    ...
    def load(self, skip_python_model: bool = False):
        ...
        # GraphFX install gate — the single switch for all model_py fused
        # kernels.
        #
        # Eager model code is the pure unfused baseline; GraphFX (torch.compile
        # FX passes + install-time fused-path enablement) is the SOLE way to
        # recover fused-kernel performance.  ``ENABLE_GRAPHFX_FUSION`` (default
        # off) is the only switch — when off, the model runs pure baseline.
