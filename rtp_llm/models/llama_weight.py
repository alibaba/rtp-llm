...
def permute(w, head_num, dim1, dim2):
    return (
        w.view(head_num, dim1 // head_num // 2, 2, dim2)
        .transpose(1, 2)
        .reshape(dim1, dim2)
    )
...
class LlamaWeightInfo(ModelDeployWeightInfo):
    ...
    def _process_meta(self, meta_dicts, weight_keys):
        if (
            ...
            and SQWeightNames.W_QKV.format(i="0") in weight_keys
        ):
    ...
    def _get_weight_info(self):
        weights = [
            AtomicWeight(
                W.embedding,
                [CkptWeightInfo(self._prefix + self._names.TOKEN_EMBEDDING, concat_1)],
                identity,
            ),
            AtomicWeight(
                W.final_ln_gamma,
                [CkptWeightInfo(self._prefix + self._names.NORM, identity)],
                identity,
            ),
            AtomicWeight(
        ...
        if self._names == CohereWeightNames:
            weights.append(
                AtomicWeight(
                    ...
                    [
                        CkptWeightInfo(
                            self._prefix + self._names.TOKEN_EMBEDDING, identity
                        )
                    ],
                    identity,
                )
            ...
            weights.append(
                AtomicWeight(
                    W.lm_head,
                    [CkptWeightInfo(self._prefix + self._names.OUTPUT, concat_0)],
                    identity,
                )
            )
        ...
        layer_weights: list[WeightModule] = [
            AtomicWeight(
                W.pre_ln_gamma,
                [CkptWeightInfo(self._prefix + self._names.ATTEN_NORM, identity)],
                identity,
            ),
            AtomicWeight(
                W.post_ln_gamma,
                [CkptWeightInfo(self._prefix + self._names.FFN_NORM, identity)],
                identity,
            ),
        ]
        if self.weight_style == WeightStyle.RTP_SMOOTH_LLM_STYLE:
            layer_weights.extend(
                [
                    AttnAtomicWeight(
                        ...
                        [
                            CkptWeightInfo(
                                self._prefix + self._names.WO.removesuffix(".int8.col"),
                                identity,
                            )
                        ],
                        transpose,
                        ...
                        sub_weights=[
                            FfnAtomicWeight(
                                ...
                                [
                                    CkptWeightInfo(
                                        self._prefix
                                        + self._names.FFW1.removesuffix(".int8.col"),
                                        identity,
                                    )
                                ],
                                transpose,
                            ...
                            FfnAtomicWeight(
                                ...
                                [
                                    CkptWeightInfo(
                                        self._prefix
                                        + self._names.FFW3.removesuffix(".int8.col"),
                                        identity,
                                    )
                                ],
                                transpose,
                            ...
                            FfnAtomicWeight(
                                ...
                                [
                                    CkptWeightInfo(
                                        self._prefix
                                        + self._names.FFW2.removesuffix(".int8.col"),
                                        identity,
                                    )
                                ],
                                transpose,
                ...
                layer_weights.append(
                    AtomicWeight(
                        ...
                        [
                            ...
                            CkptWeightInfo(self._prefix + self._names.K_NORM, identity),
                        ],
                        concat_0,
                    )
                )
            ...
            if self._names == InternlmWeightNames:
                ...
                layer_weights.append(
                    AttnAtomicWeight(
                        W.attn_o_b,
                        [CkptWeightInfo(self._prefix + self._names.BO, identity)],
                        identity,
                        config=attn_config,
                    )
                )
            ...
            if self._merge_qkv is not None:
                if hasattr(self._names, "W_QKV"):
                    ...
                    infos = [
                        CkptWeightInfo(self._prefix + self._names.WQ, concat_0),
...
class GemmaWeightInfo(LlamaWeightInfo):
    def __init__(self, model_config, prefix="", **kwargs):
        super().__init__(model_config=model_config, prefix=prefix, **kwargs)
