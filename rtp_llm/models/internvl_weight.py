...
from rtp_llm.models.llama_weight import merge_qkv_hf, qkv_rerange
...
class InternVLWeightInfo(ModelDeployWeightInfo, BaseMultiModalWeightInfo):
    ...
    def _get_weight_info(self):
        weights = [
            AtomicWeight(
                W.embedding,
                [CkptWeightInfo(self._names.TOKEN_EMBEDDING, concat_1)],
                identity,
            ),
            AtomicWeight(
                W.final_ln_gamma, [CkptWeightInfo(self._names.NORM, identity)], identity
            ),
            AtomicWeight(
                W.final_ln_beta, [], functools.partial(zeros, shape=[self._hidden_size])
            ),
            AtomicWeight(
                W.lm_head, [CkptWeightInfo(self._names.OUTPUT, concat_0)], identity
            ),
        ]
        ...
        for _ in range(self._num_layers):
            ...
            layer_weight = [
                AtomicWeight(
                    W.pre_ln_gamma,
                    [CkptWeightInfo(self._names.ATTEN_NORM, identity)],
                    identity,
                ),
                AtomicWeight(
                    W.post_ln_gamma,
                    [CkptWeightInfo(self._names.FFN_NORM, identity)],
                    identity,
                ),
                AttnAtomicWeight(
