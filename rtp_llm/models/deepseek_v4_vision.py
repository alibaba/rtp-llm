...
def grid_tokens(best_height, best_width, patch_size, downsample_ratio, padding_size=4):
    n_llm_h = math.ceil((best_height // patch_size) / downsample_ratio)
    n_llm_w = math.ceil((best_width // patch_size) / downsample_ratio)
    body_tokens = (n_llm_h + n_llm_h % 2) * (n_llm_w + 1)
    tail_padding = -body_tokens % padding_size if padding_size else 0
    num_tokens = body_tokens + tail_padding + 2
    return n_llm_h, n_llm_w, num_tokens


def solve_resize_ratio(
    height, width, patch_size, downsample_ratio, max_n_token, padding_size=4
):
    ratio = height / width
    ...
    n_llm_h, n_llm_w, num_tokens = grid_tokens(
        best_height, best_width, patch_size, downsample_ratio, padding_size
    )
...
def safe_resize(
    ...
    max_n_token -= max(padding_size - 1, 0)
    n_llm_h, n_llm_w, num_tokens = grid_tokens(
        best_height, best_width, patch_size, downsample_ratio, padding_size
    )
    budget = max_n_token
    while num_tokens > max_n_token:
        n_llm_h, n_llm_w, best_height, best_width, num_tokens = solve_resize_ratio(
            height, width, patch_size, downsample_ratio, budget, padding_size
        )
        budget -= 1
...
def build_image_block(
    n_llm_h: int, n_llm_w: int, mm_padding_size: int, padding_size: int = 4
):
    pad_h = n_llm_h % 2
    rows = n_llm_h + pad_h
    row_len = n_llm_w + 1
    pad_last = -(rows * row_len) % padding_size if padding_size else 0
    types = torch.tensor(
    ...
    types = torch.cat(
        [
            torch.full((mm_padding_size,), IMAGE_PAD, dtype=torch.int64),
...
class PreparedImage:
    ...
    def output_tokens(self) -> int:
        ...
        body_tokens = rows * row_len
        tail_padding = -body_tokens % self.padding_size if self.padding_size else 0
        return self.mm_padding_size + 2 + body_tokens + tail_padding
...
class DeepSeekV4VisionEmbedding(nn.Module):
    def __init__(self, mm_related_params, model_config):
        ...
        self.padding_size = int(cfg.get("mm_padding_size", 4))
        self.vision = ViT(cfg)
    ...
    def _get_mm_padding_size(self, configs):
        if self.padding_size == 0:
            return 0
        padding = configs.mm_padding_size if configs is not None else 0
        if not 0 <= padding < self.padding_size:
            raise ValueError(f"mm_padding_size must be in [0, {self.padding_size})")
        return padding
    ...
    def preprocess_embedding(
        ...
        mm_padding_size = self._get_mm_padding_size(configs)
        ...
        return PreparedImage(
            *preprocess_image(image, self.mm_related_params.config),
            mm_padding_size,
            self.padding_size,
        )

    def _build_embedding_block(self, embeds, n_llm_h, n_llm_w, mm_padding_size):
        types, perm = build_image_block(
            n_llm_h, n_llm_w, mm_padding_size, self.padding_size
        )
        ...
        return [
            (
                self._build_embedding_block(
                    embeds, image.n_llm_h, image.n_llm_w, image.mm_padding_size
                ),
                None,
            )
    ...
    def mm_embedding(
        ...
        mm_padding_size = self._get_mm_padding_size(configs)
        cache_key = ("deepseek_v4", url, mm_padding_size)
    ...
    def mm_process(self, mm_input, **kwargs):
        configs = kwargs.get("configs")
        mm_padding_size = self._get_mm_padding_size(configs)
        return self.image_embedding([mm_input], mm_padding_size=mm_padding_size)[0]

    @torch.inference_mode()
    def image_embedding(
        self, images: List[Image.Image], mm_padding_size: int = 0
    ) -> List[torch.Tensor]:
        outputs = []
        ...
        for image in images:
            ...
            block = self._build_embedding_block(
                embeds, n_llm_h, n_llm_w, mm_padding_size
            )
            outputs.append(block)
            mm_padding_size = max(self.padding_size - 2, 0)
        return outputs
