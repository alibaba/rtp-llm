...
class DeepSeekV4VisionEmbedding(nn.Module):
    ...
    def mm_embedding(
        ...
        # The leading image-pad count depends on token_start, so URL-only
        # embedding caching would be incorrect. Byte downloads remain cached.
        ...
        return features.to(self._data_type).contiguous(), None

    @torch.inference_mode()
    def mm_process(self, mm_input, **kwargs):
        configs = kwargs.get("configs")
        start_pos = int(getattr(configs, "token_start", -1))
        if start_pos < 0:
            raise ValueError("DeepSeek-V4 image input is missing token_start")
        return self.image_embedding([mm_input], start_pos=start_pos)[0]
