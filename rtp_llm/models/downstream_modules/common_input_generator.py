...
from rtp_llm.models.downstream_modules.embedding.api_datatype import ChatCompletionRequest, ChatMessage
import threading
...
class CommonInputGenerator(object):
    def __init__(self, tokenizer: PreTrainedTokenizerBase, config: GptInitModelParameters):
        ...
        self.openai_render_info:OpenAIRenderBasicInfo = OpenAIRenderBasicInfo(tokenizer, config)
        self.lock = threading.Lock()

    @torch.inference_mode()
    def generate( # type: ignore
        self,
        prompt: Union[str, List[str], List[Tuple[str, str]],List[ChatMessage], List[List[ChatMessage]]],
        truncate: bool = True,
        ...
        with self.lock:
            ...
            multimodal_inputs = []
            if isinstance(prompt, str) or (isinstance(prompt, list) and isinstance(prompt[0], str)) or (isinstance(prompt, list) and isinstance(prompt[0], Tuple)):
                ...
                rendered_input = self.openai_render_info.chat_renderer.render_chat(chat_request)
                encoded = rendered_input.input_ids
                combo_tokens = torch.from_numpy(np.array(encoded)).to(torch.int32)
                input_lengths = torch.from_numpy(np.array([len(encoded)])).to(torch.int32)
                multimodal_inputs = rendered_input.multimodal_inputs
            else:
                logging.error(f"Unsupported prompt type: {type(prompt)}")
                raise FtRuntimeException(ExceptionType.ERROR_INPUT_FORMAT_ERROR, f"Unsupported prompt type: {type(prompt)}")
            ...
            return EngineInputs(token_ids=combo_tokens, token_type_ids=combo_token_types, input_lengths=input_lengths, multimodal_inputs=multimodal_inputs)
