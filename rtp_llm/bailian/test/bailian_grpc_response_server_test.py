"""Tests for ``bailian_grpc_response_real`` (stream response build) and ``bailian_grpc_server`` (servicer)."""
...
class BuildStreamResponseFromGenerateOutputsTest(TestCase):
    ...
    def test_not_finished_finish_reason_zero(self) -> None:
        out = GenerateOutput(
...
class BailianGrpcInferenceServicerTest(TestCase):
    ...
    @patch.object(bgs, "_run_enqueue_sync")
    def test_real_mode_uses_enqueue(self, mock_sync: MagicMock) -> None:
        out = GenerateOutput(
            output_ids=torch.tensor([9], dtype=torch.int32),
            finished=True,
            aux_info=AuxInfo(input_len=1, reuse_len=0),
        )
        mock_sync.return_value = [GenerateOutputs(generate_outputs=[out])]
