...
class Level2WeightsRegionIsolationTest(unittest.TestCase):
    ...
    def test_gpu_split_and_postprocess_weights_are_discarded_after_load_and_update(
        self,
    ):
        raw = torch.full((512, 512), 7.0, device="cuda")
        ...
        # Exercise the real AtomicWeight._split clone and _postprocess rewrite.
        # Both already return CUDA tensors, so the final .to("cuda") is a no-op.
        for rewrite in (False, True):
            for operation in ("load", "update"):
                with self.subTest(rewrite=rewrite, operation=operation):
                    config.exported_device = SimpleNamespace(
                        maybe_rewrite_weight_by_key=lambda name, tensor: (
                            tensor.clone() if rewrite else tensor
                        )
                    )
                    weight = AtomicWeight("test", [])
                    with (
                        patch.object(
                            weight, "_get_split_func", return_value=lambda t, **kw: t
                        ),
                        patch.object(weight, "_load_raw_tensor", return_value=raw),
                    ):
                        result = (
                            weight.load(None, None, "cuda", config)
                            if operation == "load"
                            else weight.update(raw, "cuda", config)
                        )["test"]
                    ptr = result.data_ptr()
                    ...
                    self.assertTrue(torch.all(raw == 7.0).item())
