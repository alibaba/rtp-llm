...
class _EmptyWeightsInfo(ModelDeployWeightInfo):
    """Only checkpoint descriptors are fake; create_load_config stays real."""
...
class _LoaderOnlyModel(BaseModel):
    ...
    def load(self, skip_python_model=False):
        # Stop before checkpoint I/O, GPU materialization and model construction.
        # Do not replace from_config/__init__/create_model_loader or any of the
        # get_model_loader -> ModelLoader -> create_load_config handoffs.
        ...
        for reserve in (0, 137):
            with self.subTest(reserve=reserve):
                ...
                with (
                    ...
                    # Device selection is outside the configuration handoff contract.
                    # Avoid importing CUDA compute libraries in this CPU test.
                    patch.dict(
                        sys.modules,
                        {
