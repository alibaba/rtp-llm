...
class TestDirectModelOptMxfp4Weight(unittest.TestCase):
    def test_mixed_quant_config_detects_offline_fp4_without_database(self):
        quant_config = Fp8MxBlockWiseQuantConfig(
            quantized_layers={
                "model.layers.0.mlp.experts": {"quant_algo": "MXFP4"}
            }
        )

        self.assertTrue(is_offline_mega_moe_fp4_ckpt(None, quant_config))
...
class TestOfflineFp4SharedExpertWeight(unittest.TestCase):
    ...
    def test_modelopt_mxfp8_down_proj_split_uses_ffn_tp(self):
        ...
        self.assertNotIn("_get_split_func", mxfp8.kernel.__dict__)
        self.assertNotIn("_get_split_func", mxfp8.scale.__dict__)
    ...
    def test_modelopt_mxfp8_stacked_up_gate_split_uses_ffn_tp(self):
        ffn = self._make_shared_ffn()
        mxfp8 = Mxfp8Weight(
            ffn.w13,
            Fp8MxBlockWiseQuantConfig(
        ...
        kernel = torch.empty((16, 64), dtype=torch.float8_e4m3fn)
        scale = torch.empty((16, 2), dtype=torch.float32)
        load_config = MagicMock(
            tp_size=8,
            tp_rank=3,
            ffn_tp_size=1,
        ...
        self.assertEqual(tuple(split[mxfp8.kernel.name].shape), (16, 64))
        self.assertEqual(tuple(split[mxfp8.scale.name].shape), (16, 2))
