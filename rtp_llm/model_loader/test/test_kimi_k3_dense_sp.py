"""Dense TP partials must sum to one MLP even when SP sets ffn_tp_size=1."""
import unittest
...
class KimiK3DenseSPTest(unittest.TestCase):
    def test_sp_loader_matches_unsharded_mlp(self):
        ...
        for tp in (1, 2, 8):
            with self.subTest(tp=tp):
                ...
                for rank in range(tp):
                    ...
                    self.assertEqual(tuple(loaded[W.ffn_w1].shape), (8, 32 // tp))
                    self.assertEqual(tuple(loaded[W.ffn_w2].shape), (32 // tp, 8))
                    partials.append(activation(x @ loaded[W.ffn_w1], x @ loaded[W.ffn_w3]) @ loaded[W.ffn_w2])
                actual = torch.stack(partials).sum(0)
