...
from rtp_llm.utils.model_weight import W, CkptWeightInfo, identity, concat_0, pad, pad_w13, sp_neg1, \
    sp_id, sp_head_gemm_a8, sp_head_s_gemm_a8_channel, sp_0, stack_, stack_moe_w1, sp_0_w13
...
class W8A8Fp8PerChannelAttnAtomicWeight(AttnAtomicWeight, W8A8Fp8PerChannelAtomicWeight):
    def __init__(self, *args: Any, **kwargs: Any):
        super().__init__(*args, **kwargs)
...
def create_w8a8_fp8_per_channel_weight(src_weight_info: WeightModule, *args: Any, **kwargs: Any) -> W8A8Fp8PerChannelAtomicWeight :
...
class PerChannelFp8Weight(CompositeWeight, QuantWeight):
    ...
    def _get_qkv_quant_weight(self, src_weight_info: AttnAtomicWeight):
        ...
        kernel = create_w8a8_fp8_per_channel_weight(src_weight_info, W.attn_qkv_w,
                                                    qkv_w_list, concat_0, data_type=torch.float8_e4m3fn, config=src_weight_info.config)

        scale = create_w8a8_fp8_per_channel_weight(src_weight_info, W.attn_qkv_s,
    ...
    def _get_mha_attn_out_quant_weight(self, src_weight_info: AttnAtomicWeight):
        ...
        kernel = create_w8a8_fp8_per_channel_weight(src_weight_info, W.attn_o_w, [CkptWeightInfo(w_name + QW_SUFFIX)],
                                 data_type=torch.float8_e4m3fn, config=src_weight_info.config)
        scale = create_w8a8_fp8_per_channel_weight(src_weight_info, W.attn_o_s,
    ...
    def _get_ffn_quant_weight(self, src_weight_info: FfnAtomicWeight):
        ...
        if src_weight_info.name == W.ffn_w13:
            ...
            return [
                create_w8a8_fp8_per_channel_weight(src_weight_info, 
                ...
                create_w8a8_fp8_per_channel_weight(src_weight_info, 
                    ...
                    functools.partial(pad_w13, inter_padding_size=src_weight_info.config.inter_padding_size, dim=0), data_type=torch.float32,
                    config=src_weight_info.config)
            ]
            ...
            kernel = create_w8a8_fp8_per_channel_weight(src_weight_info, w, [CkptWeightInfo(w_name + QW_SUFFIX, identity)],
                                     functools.partial(pad, inter_padding_size=src_weight_info.config.inter_padding_size, dim=0),
                                     data_type=torch.float8_e4m3fn, config=src_weight_info.config)
            scale = create_w8a8_fp8_per_channel_weight(src_weight_info, s, [CkptWeightInfo(w_name + QS_SUFFIX, identity)],
                                    functools.partial(pad, inter_padding_size=src_weight_info.config.inter_padding_size, dim=0),
                                    data_type=torch.float32, config=src_weight_info.config)
            return [kernel, scale]
        else:
            kernel = create_w8a8_fp8_per_channel_weight(src_weight_info, W.ffn_w2, [CkptWeightInfo(w_name + QW_SUFFIX, identity)],
                                     functools.partial(pad, inter_padding_size=src_weight_info.config.inter_padding_size, dim=1),
                                     data_type=torch.float8_e4m3fn, config=src_weight_info.config)
            scale = create_w8a8_fp8_per_channel_weight(src_weight_info, W.ffn_s2, [CkptWeightInfo(w_name + QS_SUFFIX, identity)],
                                     data_type=torch.float32, config=src_weight_info.config)
            return [kernel, scale]
    ...
    def _get_moe_w2_quant_weight(self, src_weight_info: MoeAtomicWeight):
        ...
        w_name = src_weight_info.weights[0].name[:-len(W_SUFFIX)]
        kernel = create_w8a8_fp8_per_channel_weight(src_weight_info, W.moe_w2, [CkptWeightInfo(w_name + QW_SUFFIX, identity)], stack_,
                                 data_type=torch.float8_e4m3fn, config=src_weight_info.config)
        scale = create_w8a8_fp8_per_channel_weight(src_weight_info, W.moe_s2, [CkptWeightInfo(w_name + QS_SUFFIX, identity)], stack_,
                                data_type=torch.float32, config=src_weight_info.config)
        return [kernel, scale]

    def _get_moe_w1_quant_weight(self, src_weight_info: MoeAtomicWeight):
        assert src_weight_info.name in [W.moe_w1]
        kernel = create_w8a8_fp8_per_channel_weight(src_weight_info, W.moe_w1, [CkptWeightInfo(w.name[:-len(W_SUFFIX)] + QW_SUFFIX, identity) for w in src_weight_info.weights],
                                stack_moe_w1, data_type=torch.float8_e4m3fn, config=src_weight_info.config)
        scale = create_w8a8_fp8_per_channel_weight(src_weight_info, W.moe_s1, [CkptWeightInfo(w.name[:-len(W_SUFFIX)] + QS_SUFFIX, identity) for w in src_weight_info.weights],
