...
class ModelLoader:
    ...
    def _is_memory_enough_for_fastsafetensor(self):
        # Calculate actual model size considering both TP and EP splitting
        model_size = self._weights_info.config.eval_model_weight_size_with_parallelism(
            tp_size=self._load_config.tp_size, ep_size=self._load_config.ep_size
        )
        ...
        model_mem = model_size / (1024.0**2)  # Already considers TP and EP splitting
        max_file_mem = max_file_size / (1024.0**2)
        logging.debug(
            f"FastSafetensor memory check: free={free_mem:.2f}MB, model={model_mem:.2f}MB, max_file={max_file_mem:.2f}MB, tp_size={self._load_config.tp_size}, ep_size={self._load_config.ep_size}"
        )
    ...
    def _choose_weight_convert_device(self, current_device):
        ...
        model_mem = model_size / (1024.0**2)  # Already considers TP and EP splitting
        logging.debug(
            f"Memory check: free={free_mem:.2f}MB, model={model_mem:.2f}MB, tp_size={self._load_config.tp_size}, ep_size={self._load_config.ep_size}"
        )
