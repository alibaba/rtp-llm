...
class WeightModule(ABC):
    ...
    def load(
        ...
        # GPU split/postprocess may allocate the resident weight before .to(),
        # which is a no-op for tensors already on the target device. Keep the
        # entire load pipeline in the region, not just the final copy.
    ...
    def update(
        ...
        # As in load(), GPU split/postprocess allocations must be tagged too.
