...
def _dequantize_mxfp8(weight: torch.Tensor, scale_exponents: torch.Tensor) -> torch.Tensor:
    ...
    This is used for ``mla_kc``/``mla_vc`` BMM weights, which cannot retain the
    checkpoint's MXFP8 representation because they are consumed by torch.bmm.
    """
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if getattr(self, "scale", None) is not None and getattr(
            ...
            # FFN tensors follow FFN TP, which can differ from attention TP
            # when sequence parallelism replicates the shared expert on every
            # EP rank. The stacked w13 splitter already follows FFN TP.
        ...
        for ckpt in getattr(src_weight_info, "weights", ()):
            ...
            if quantized_layers:
                ...
                if (
                    ...
                    or str(info.get("quant_algo", "")).upper() != "MXFP8"
                ):
                    return True
        return False
