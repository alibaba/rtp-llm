...
def convert_fp8_moe_to_fp4_ue8m0(
    ...
    Handles two scale layouts:
      ...
      - 2D-block: [E, ceil(N/128), K//128] — one scale per 128×128 block
    ...
    if K % block_size:
        raise ValueError(f"K={K} not divisible by block_size={block_size}")
    if K % FP8_PER_BLOCK:
        raise ValueError(f"FP8 source K={K} must be divisible by {FP8_PER_BLOCK}")
    ...
    elif weight_scale.shape == (
        E,
        (N + FP8_PER_BLOCK - 1) // FP8_PER_BLOCK,
        K // FP8_PER_BLOCK,
    ):
...
class OnlineMegaMoeFp4Weight(CompositeWeight, QuantWeight):
    ...
    (``moe_w1`` = up||gate, ``moe_w2`` = down) starting from BF16.
