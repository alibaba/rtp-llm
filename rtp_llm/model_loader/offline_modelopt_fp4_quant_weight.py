...
def _is_direct_modelopt_mxfp4_name(name: str) -> bool:
    return name.endswith((".w13_weight", ".w2_weight"))
...
def _fp4_scale_name(weight_name: str, scale_dtype: torch.dtype) -> str:
    # ModelOpt DeepSeek-style fused MXFP4 tensors retain ``weight`` in the
    # base name: w13_weight -> w13_weight_scale.
    if _is_direct_modelopt_mxfp4_name(weight_name):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        direct_modelopt_mxfp4 = any(
            _is_direct_modelopt_mxfp4_name(w.name) for w in src_weight_info.weights
        )
        ...
        scale = MoeAtomicWeight(
            ...
            process_fun=src_weight_info.process_fun,
            # The checkpoint serializes UE8M0 exponent bits as U8. Keep the
            # bytes intact here; a numeric U8 -> float8 cast changes them.
...
def is_offline_mega_moe_fp4_ckpt(
    database: Optional[BaseDatabase],
    ...
    1. **Primary**: ``quantization_config.expert_dtype == "fp4"``
       (emitted by ``glm5_fp4_moe_fp8_quant.py`` when not ``all_fp8``), or an
       MXFP4 entry in ModelOpt ``quantized_layers``.
    2. **Fallback**: legacy per-expert or fused ModelOpt
       ``experts.*_weight_scale`` tensor names.
    """
    if isinstance(quant_config, Fp8MxBlockWiseQuantConfig) and quant_config.has_quant_algo(
        "MXFP4"
    ):
        return True
    if database is None:
        ...
        if _os.path.exists(cfg_path):
            try:
                ...
                if qc.get("expert_dtype") == "fp4":
                    return True
            except (OSError, TypeError, AttributeError, _json.JSONDecodeError):
...
def wrap_shared_expert_for_offline_fp4(
    ...
    # ModelOpt mixed checkpoints already use the native MXFP8 loader for the
    # shared expert. Preserve its .weight_scale naming and UE8M0 conversion.
