...
@contextmanager
def feature_weights_region() -> Iterator[None]:
    """Own allocations made while a feature derives resident model weights.

    Feature modules often repack checkpoint tensors into a kernel-specific
    layout (for example, FP4/FP8 MoE weights).  The normal fastsafetensors
    loader already owns the tensors it materializes directly; this boundary is
    for the additional resident tensors created after loading.  A feature
    should put *only* that resident-weight build phase in this context and put
    staging buffers, workspaces, and JIT warmups in its runtime setup phase.

    Keeping this as a named abstraction (rather than making every feature
    import ``weights_region`` directly) gives future features one ownership
    contract and keeps the VMM tag/pause policy in this module.
    """
    with weights_region():
        yield
