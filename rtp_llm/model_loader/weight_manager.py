...
from rtp_llm.utils.sleep_timing import timed_sleep_method
...
class WeightManager:
    ...
    @timed_sleep_method("sleep", "suspend_collectives")
    ...
    @timed_sleep_method("wake", "resume_collectives")
    ...
    def reload_weights_from_loader(self) -> None:
        ...
        try:
            ...
            for strat in iter_mega_strategies() + iter_mega_se_strategies():
                if _owned(strat):
                    mega_by_layer[strat.cfg.layer_id] = strat
                    extra_keys = getattr(
                        strat, "sleep_reload_extra_weight_names", set
                    )()
                    mega_keys_by_layer[strat.cfg.layer_id] = (
                        mega_routed_keys | extra_keys
                    )
            ...
            with torch.cuda.stream(self._working_stream), torch.inference_mode():
                for layer_id, name, tensor in source:
                    ...
                    if ori is None:
                        ...
                        if (
                            layer_id in mega_by_layer
                            and name in mega_keys_by_layer[layer_id]
                        ):
                            if (
                            ...
                            acc[name] = tensor
                            del tensor
                            if set(acc) == mega_keys_by_layer[layer_id]:
    ...
    def _rederive_dsv4_computed_weights(
        ...
        These resident tensors are ``weights``-tagged (blank-remapped by
        ...
        * Mega-MoE kernel weights (``_mega_l1_w`` / ``_l1_sf`` / ``_l2_w`` /
          ...
          reload stream (``mega_acc``). The Mega-SE path additionally restores
          its four fused shared-expert tensors from the popped shared weights.
        ...
        * Attention FP8 scale layouts, wo_a stacks, RoPE/cos-sin tables, and
          indexer folded projections. These are copied into their original
          storage so CUDA-graph-captured pointers remain valid.

        Transform scratch is untagged; results are copied into the original
        region-backed buffers before KV-cache resume, while headroom is ample
        for the held raws + transform transients. Raises if any registered
        consumer was not fully covered (surfaces to the C++ wake hook as an ERROR
        rather than leaving blank-page garbage)."""
        if not mega_by_layer and not compressors and not attentions and not fp8_linears:
            return
        import gc
        ...
        with self._lock:
            with torch.cuda.stream(self._working_stream), torch.inference_mode():
                for layer_id, strat in mega_by_layer.items():
                    ...
                    required = mega_keys_by_layer[layer_id]
                    if have != required:
                        missing = sorted(str(k) for k in (required - have))
                        raise RuntimeError(
                ...
                seen_freqs = set()
                seen_cos_sin = set()
                # MTP fusion projections live on the model, not an attention
                # layer. Restore every factory-built FP8 linear in this scope.
                if fp8_linears:
                    from rtp_llm.models_py.modules.dsv4.fp8.attention import (
                        AttentionFP8,
                    )

                    for linear in fp8_linears:
                        AttentionFP8._reload_linear_scale(linear)
                for attn in attentions:
                    attn.reload_sleep_computed_weights(seen_freqs, seen_cos_sin)
                self._working_stream.synchronize()
                _rd_probe("after_attentions")
        ...
        logging.info(
            ...
            "(%d Mega-MoE layer(s) + %d compressor(s) + %d attention(s))",
            len(mega_by_layer),
            len(compressors),
            len(attentions),
        )
