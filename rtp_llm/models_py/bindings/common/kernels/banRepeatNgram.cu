...
__global__ void ban_repeat_ngram(T*                   logits,
                                 TokenIdType const**  output_ids_buf,
                                 FinishedState const* finished_buf,
                                 SizeType32 const**   parent_ids_buf,
                                 SizeType32 const*    batch_slots,
                                 SizeType32           batch_size,
                                 SizeType32           beam_width,
                                 SizeType32           max_seq_len,
                                 SizeType32 const*    no_repeat_ngram_size_buf,
                                 SizeType32           vocab_size_padded,
    ...
    // NOTE(wangyin): In RTP-LLM framework, sequence_lengths is always real value - 1,
    //                see rtp_llm/cpp/normal_engine/NormalBatchStreamProcessor.cc:92
    //                ```
    //                sequence_lengths[batch_idx] = stream->seqLength() - 1; // need remove
    //                ```
    //                This is because of original fmha kernel from fastertransformer.
    //                Here the seq lengths are modified to add 1.
...
void invokeBanRepeatNgram(T*                   logits,
    ...
    ban_repeat_ngram<<<grid, block, (block.x + 2 * (max_no_repeat_ngram_size - 1)) * sizeof(int), stream>>>(
        logits,
        output_ids_buf,
        finished_buf,
        parent_ids_buf,
...
#define INVOKE_BAN_REPEAT_NGRAM(T)                                                                                     \
    template void invokeBanRepeatNgram(T*                   logits,                                                    \
                                       TokenIdType const**  output_ids_buf,                                            \
                                       const FinishedState* finished_buf,                                              \
                                       SizeType32 const**   parent_ids_buf,                                            \
                                       SizeType32 const*    batch_slot,                                                \
                                       SizeType32 const*    sequence_lengths,                                          \
                                       SizeType32           batch_size,                                                \
