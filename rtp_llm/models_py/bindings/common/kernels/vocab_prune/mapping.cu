...
__global__ void mapping_draft2target_kernel(
    IdType* tokens, int batch_size, int token_offset, int token_stride, int64_t* d2t_map, int d2t_map_size) {
    ...
    IdType token_id = tokens[idx];
    if (token_id >= 0 && token_id < static_cast<IdType>(d2t_map_size)) {
        tokens[idx] = d2t_map[token_id];
    }
}
...
void invokeMappingDraft2Target(IdType*      tokens,
    ...
    mapping_draft2target_kernel<<<grid_size, block_size, 0, stream>>>(
        tokens, batch_size, token_offset, token_stride, d2t_map, d2t_map_size);
}

#define INSTANTIATE_MAPPING_DRAFT2TARGET(IdType)                                                                       \
    template void invokeMappingDraft2Target(IdType*      tokens,                                                       \
                                            int          batch_size,                                                   \
                                            int          token_offset,                                                 \
                                            int          token_stride,                                                 \
                                            int64_t*     d2t_map,                                                      \
