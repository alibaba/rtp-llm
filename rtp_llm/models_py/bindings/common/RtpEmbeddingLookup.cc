...
const int* optionalInt32TensorPtr(const std::optional<at::Tensor>& tensor,
                                  ...
                                  const bool                       require_exact_length) {
...
void embedding(at::Tensor&               output,
    ...
    auto*      mask_ptr =
        optionalInt32TensorPtr(text_tokens_mask, "text_tokens_mask", tokens, device, /*require_exact_length=*/false);
...
void embeddingBert(at::Tensor&               output,
    ...
    CHECK_EQ(output.size(1), hidden_size);
    const int* mask_ptr =
        optionalInt32TensorPtr(text_tokens_mask, "text_tokens_mask", tokens, device, /*require_exact_length=*/true);
    StreamType stream = GET_CURRENT_STREAM();
