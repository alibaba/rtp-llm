...
const int* optionalInt32TensorPtr(const std::optional<at::Tensor>& tensor,
                                  ...
                                  const MaskLengthPolicy           length_policy) {
    ...
    if (length_policy == MaskLengthPolicy::kExact) {
        TORCH_CHECK(
            value.size(0) == tokens, name, " must have exactly one id per token, got ", value.size(0), " vs ", tokens);
    } else {
        TORCH_CHECK(
            value.size(0) >= tokens, name, " must have at least one id per token, got ", value.size(0), " vs ", tokens);
    }
...
void embedding(at::Tensor&               output,
    ...
    // Preserve the generic op's legacy capacity-buffer contract: only the
    // active token prefix is consumed, so a longer mask remains valid.
    auto* mask_ptr =
        optionalInt32TensorPtr(text_tokens_mask, "text_tokens_mask", tokens, device, MaskLengthPolicy::kActivePrefix);
...
void embeddingBert(at::Tensor&               output,
    ...
    CHECK_EQ(output.size(1), hidden_size);
    const int* mask_ptr =
        optionalInt32TensorPtr(text_tokens_mask, "text_tokens_mask", tokens, device, MaskLengthPolicy::kExact);
    StreamType stream = GET_CURRENT_STREAM();
