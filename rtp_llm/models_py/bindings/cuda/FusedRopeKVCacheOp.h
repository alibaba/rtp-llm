...
public:
    FusedRopeKVCachePrefillOp(const ModelConfig& model_config, const ParallelismConfig& parallelism_config);
...
public:
    FusedRopeKVCacheDecodeOp(const ModelConfig& model_config, const ParallelismConfig& parallelism_config);
    TRTAttnPtr    prepare(torch_ext::PyAttentionInputs attn_inputs);
    torch::Tensor forward(const torch::Tensor&              qkv,
                          FMHAType                          fmha_type,
                          std::optional<torch_ext::KVCache> kv_cache,
