/// \file topk_impl.cuh
/// \brief GLM5 (DSA indexer) top-k implementation classes.
///
...
///  - the standalone launcher instantiates 2/4/8-CTA cluster variants.
///
...
/// main kernel runs Register or Streaming implementations) declares one
...
SGL_DEVICE bool is_nan(float x) {
...
SGL_DEVICE uint32_t extract_coarse_bin(float x) {
  ...
  if ((bits & 0x7fffu) > 0x7c00u) return (1u << kBits) - 1u;
...
struct TopKConfig {
  ...
  /// Exact fallback when the coarse threshold bin contains more candidates
  ...
  SGL_DEVICE static void exact_boundary_scan_topk(
    ...
    if (tx == 0) {
      ...
      const uint32_t hi_key =
          extract_exact_bin(nextafterf(v_hi, -FLT_MAX));
      uint32_t prefix = 0;
...
#pragma unroll
      for (uint32_t round = 0; round < 4; ++round) {
        ...
        if (lo_byte != hi_byte) break;
        prefix |= lo_byte << shift;
        first_round = round + 1;
      }
      smem->match.bin = prefix;
      smem->match.above_count = problem.topk - output_base;
      smem->match.equal_count = first_round;
    }
    __syncthreads();

#pragma unroll 1
    for (uint32_t round = smem->match.equal_count; round < 4; ++round) {
      if (tx < kRadixSize) smem->histogram[0][tx] = 0;
      __syncthreads();

      const uint32_t prefix = smem->match.bin;
      const uint32_t mask =
          round == 0 ? 0u : (~0u << (32u - round * 8u));
      const uint32_t shift = 24u - round * 8u;
...
#pragma unroll 1
      ...
      const uint32_t remaining = smem->match.above_count;
      if (tx < kRadixSize) {
        const uint32_t count_ge = smem->histogram[0][tx];
        const uint32_t count_gt =
            tx + 1u < kRadixSize ? smem->histogram[0][tx + 1u] : 0u;
        if (count_ge >= remaining && count_gt < remaining) {
          smem->match.equal_count = tx;
        }
      }
      __syncthreads();
      if (tx == 0) {
        const uint32_t threshold = smem->match.equal_count;
        const uint32_t count_gt = threshold + 1u < kRadixSize
            ? smem->histogram[0][threshold + 1u]
            : 0u;
        smem->match.bin |= threshold << shift;
        smem->match.above_count -= count_gt;
      }
      __syncthreads();
    }

    const uint32_t pivot = smem->match.bin;
    if (tx == 0) smem->counter = 0;
    __syncthreads();
    for (uint32_t idx = tx; idx < problem.seq_len; idx += kBlockSize) {
      const float value = problem.in[idx];
...
struct TopKCluster : TopKRadixBase<10> {
 public:
  ...
  // Process ONE batch element (one cluster). There is no trailing barrier, so
  // the kernel performs cluster.sync() after
  ...
  SGL_DEVICE static void forward(TopKProblem problem, void* _smem) {
    forward_impl<kAlignedInput>(problem, _smem);
  }
