...
void concat_and_cache_mla(torch::Tensor&     kv_c,          // [num_tokens, kv_lora_rank]
    ...
    // Whitelist of supported kv_cache dtypes — must match DISPATCH_BY_KV_CACHE_DTYPE
    // arms below.  Anything else is rejected up front so callers get a clear error
    // (e.g. "fp8_model1_mla" was retired in favour of the dsv4 Triton writer; see
    // rtp_llm/models_py/modules/dsv4/fp8/_swa_kv_insert_triton.py).
    TORCH_CHECK(kv_cache_dtype == "auto" || kv_cache_dtype == "fp8" ||
                ...
                kv_cache_dtype,
                "'. Supported: auto, fp8, fp8_e4m3, fp8_e5m2, fp8_ds_mla.");
