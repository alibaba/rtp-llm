...
template <bool SINGLE_CTA, typename OrderedType>
__device__ __forceinline__ float RadixTopKDeterministicGroupSum(
    float block_sum, float* shared_sum, RadixRowState* state, int& barrier_phase,
    uint32_t ctas_per_group, uint32_t cta_in_group, uint32_t iter, uint32_t tx) {
  if constexpr (SINGLE_CTA) {
    ...
    // Reuse the slot reserved for the final radix round. The following
    // iteration starts from the next ring slot and clears this slot before it
    // is needed again.
    constexpr uint32_t NUM_ROUNDS = sizeof(OrderedType) * 8 / 8;
    const uint32_t scratch_idx = (iter * NUM_ROUNDS + NUM_ROUNDS - 1) % 3;

    // Read-complete barrier for the radix path: no CTA may overwrite
    // current_hist while another CTA is still copying it to shared memory.
    // On the k >= vocab_size path it also provides the iteration handoff.
    AdvanceRadixGroupBarrier(state, barrier_phase, ctas_per_group, tx);

    if (tx == 0) {
      state->histogram[scratch_idx][cta_in_group] = __float_as_uint(block_sum);
    }

    // Write-complete barrier: CTA 0 may reduce only after every block sum is
    // visible in its fixed CTA-index slot.
    AdvanceRadixGroupBarrier(state, barrier_phase, ctas_per_group, tx);

    if (tx == 0 && cta_in_group == 0) {
      float group_sum = 0.0f;
      for (uint32_t i = 0; i < ctas_per_group; ++i) {
        group_sum += __uint_as_float(state->histogram[scratch_idx][i]);
      }
      state->sum_topk = group_sum;
    }

    // Publish barrier: every CTA observes the completed ordered reduction before
    // any CTA advances to a later persistent iteration and reuses global state.
    AdvanceRadixGroupBarrier(state, barrier_phase, ctas_per_group, tx);
    return state->sum_topk;
  }
}
...
#pragma unroll
      ...
      const float group_sum = RadixTopKDeterministicGroupSum<SINGLE_CTA, OrderedType>(
          block_sum, shared_sum, state, barrier_phase, ctas_per_group, cta_in_group, iter, tx);
...
cudaError_t RadixTopKRenormProbMultiCTA(DType* probs, DType* renormed_prob, IdType* top_k_arr,
  ...
  chunk_size = round_up(chunk_size, vec_size);
  chunk_size = std::min(chunk_size, max_chunk_elements);
  if (ctas_per_group > RADIX_TOPK_MAX_DETERMINISTIC_CTAS_PER_GROUP) {
