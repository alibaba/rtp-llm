...
__global__ void SamplingFromProbKernel(DType*    probs,
    ...
    // A per-row seed/offset pair already identifies the RNG stream. Mixing the
    // launch row (bx) into the Philox subsequence makes a seeded request change
    // its samples when its position in a dynamic batch changes. Keep bx only for
    // the scalar-seed API, where it is still needed to decorrelate rows.
    curand_init(philox_seed, seed_arr ? 0 : bx, philox_offset, &state);
    const uint32_t row_idx = indices == nullptr ? bx : indices[bx];
