...
void topk_v3_from_histogram(
    ...
    TORCH_CHECK(
        lengths.device() == scores.device() &&
            histograms.device() == scores.device() &&
            output.device() == scores.device() &&
            workspace.device() == scores.device(),
        "scores, lengths, histograms, output, and workspace must be on the same CUDA device");
    TORCH_CHECK(
    ...
    TORCH_CHECK(
        k == 512 || k == 1024 || k == 2048,
        "topk_v3_from_histogram supports K=512, 1024, or 2048");
    TORCH_CHECK(
        max_seq_len > 0 && max_seq_len <= scores.size(1),
        "invalid max_seq_len");
    TORCH_CHECK(
        max_seq_len <= std::numeric_limits<int32_t>::max(),
        "max_seq_len exceeds int32 output-index range");
    TORCH_CHECK(
        scores.size(0) <= std::numeric_limits<uint32_t>::max(),
        "too many score rows for the CUDA dispatcher");
    TORCH_CHECK(
