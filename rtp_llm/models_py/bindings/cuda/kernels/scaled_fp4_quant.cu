...
// Get SM version (compute capability major * 10 + minor)
inline int getSMVersion() {
    int major = getComputeCapabilityMajor();
    int minor = getComputeCapabilityMinor();
...
void quant_impl(void*        output,
    ...
    dim3      grid(
        std::min(static_cast<int>((totalWorkSize + block.x - 1) / block.x), multiProcessorCount * numBlocksPerSM));
    ...
    if (mask != nullptr) {
        ...
        cvt_fp16_to_fp4_expert<T, false><<<grid, block, 0, stream>>>(m_topk,
                                                                     k,
    ...
    if (blockRepeat > 1) {
        ...
        if (n_experts >= 4) {
            cvt_fp16_to_fp4<T, false, false>
                <<<grid, block, shared_mem_size, stream>>>(m_topk,
                                                           k,
                                                           reinterpret_cast<T*>(input),
                                                           reinterpret_cast<float*>(input_global_scale),
                                                           reinterpret_cast<uint32_t*>(output),
...
void scaled_fp4_experts_quant_sm100a(torch::Tensor&       output,
    ...
    if (in_dtype == at::ScalarType::Half) {
        quant_impl<half>(output.data_ptr(),
        ...
        quant_impl<__nv_bfloat16>(output.data_ptr(),
                                  output_scale.data_ptr(),
                                  input.data_ptr(),
                                  input_global_scale.data_ptr(),
                                  input_offset_by_experts.data_ptr(),
...
void silu_and_mul_scaled_fp4_experts_quant_sm100a(torch::Tensor&       output,
                                                  torch::Tensor&       output_scale,
                                                  torch::Tensor const& input,
                                                  torch::Tensor const& input_global_scale,
