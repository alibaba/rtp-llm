// Adapt from https://github.com/sgl-project/sglang/blob/main/sgl-kernel/csrc/gemm/per_token_group_quant_8bit.cu
// Licensed under the Apache License, Version 2.0
#include "per_token_group_quant_8bit_checked.h"
...
__global__ void per_token_group_quant_8bit_checked_kernel(const T* __restrict__ input,
                                                  ...
                                                  int32_t* __restrict__ status   = nullptr) {
...
#pragma unroll
    ...
    const unsigned group_mask = 0xffffu << (((threadIdx.x % 32) / threads_per_group) * threads_per_group);
    local_absmax              = GroupReduceMax(local_absmax, group_mask);
    const bool     group_bad  = (__ballot_sync(group_mask, local_nonfinite != 0) & group_mask) != 0;
...
void per_token_group_quant_8bit_checked(torch::Tensor input,
...
#define LAUNCH_KERNEL(T, DST_DTYPE)                                                                                    \
    do {                                                                                                               \
        ...
        if (is_column_major) {                                                                                         \
            if (scale_ue8m0) {                                                                                         \
                per_token_group_quant_8bit_checked_kernel<T, DST_DTYPE, true, true>                                    \
                ...
                per_token_group_quant_8bit_checked_kernel<T, DST_DTYPE, true, false>                                   \
                    <<<grid, block, 0, stream>>>(static_cast<T*>(input.data_ptr()),                                    \
                                                 output_q.data_ptr(),                                                  \
            ...
            assert(!scale_ue8m0);                                                                                      \
            per_token_group_quant_8bit_checked_kernel<T, DST_DTYPE, false>                                             \
                <<<grid, block, 0, stream>>>(static_cast<T*>(input.data_ptr()),                                        \
                                             output_q.data_ptr(),                                                      \
                                             static_cast<float*>(output_s.data_ptr()),                                 \
                                             group_size,                                                               \
