...
struct MmaComputer {
 ...
 public:
  ...
  #pragma unroll
      ...
      ::rtp_llm::arrive_barrier(smem_barrier + 1 + stage_idx * 2);
      stage_idx += 1;
...
void kimi_k3_fused_a_gemm(at::Tensor& output,
                       at::Tensor const& mat_a,
                       at::Tensor const& mat_b, bool enable_pdl) {
  TORCH_CHECK(mat_a.is_cuda() && mat_b.is_cuda() && output.is_cuda(), "expected CUDA tensors");
  ...
  TORCH_CHECK(mat_b.stride(0) == 1 && mat_b.stride(1) == hd_in,
                  "mat_b must be a packed column-major [hd_in, hd_out] tensor");

  TORCH_CHECK(
      mat_a.scalar_type() == at::kBFloat16 &&
  ...
  const c10::cuda::CUDAGuard device_guard(mat_a.device());
...
#undef DISPATCH_DSV3_SHAPE
  ...
  TORCH_CHECK(false, "unsupported DSV3 fused-A GEMM shape");
}
