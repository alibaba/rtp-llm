...
#include "per_token_group_quant_8bit_v2_checked.h"
...
__device__ __forceinline__ float GroupReduceMax(float val, const unsigned mask) {
...
__global__ void per_token_group_quant_8bit_v2_checked_kernel(const T* __restrict__ input,
...
#pragma unroll
            for (uint32_t j = 0; j < INPUT_PRIMARY_VEC_SIZE; ++j) {
                const float primary = static_cast<float>(input_primary_vec[j]);
                local_nonfinite |= !isfinite(primary);
                float val;
                if constexpr (FUSE_SILU_AND_MUL) {
                    const float secondary = static_cast<float>(input_secondary_vec[j]);
                    local_nonfinite |= !isfinite(secondary);
                    // TODO maybe vectorize
                    T val_lowprec = static_cast<T>(silu(primary)) * input_secondary_vec[j];
                    val                  = static_cast<float>(val_lowprec);
                    input_primary_vec[j] = val_lowprec;
                } else {
                    val = primary;
                }
                local_nonfinite |= !isfinite(val);

                float abs_val = fabsf(val);
                local_absmax  = fmaxf(local_absmax, abs_val);
            }

            constexpr unsigned lane_bits = (1u << THREADS_PER_SUBWARP) - 1u;
            const unsigned subwarp_mask =
                lane_bits << (((threadIdx.x % 32) / THREADS_PER_SUBWARP) * THREADS_PER_SUBWARP);
            local_absmax        = GroupReduceMax<THREADS_PER_SUBWARP>(local_absmax, subwarp_mask);
            const bool group_bad = (__ballot_sync(subwarp_mask, local_nonfinite != 0) & subwarp_mask) != 0;
            if (lane_id == 0 && group_bad) {
                atomicOr(status, 1);
            }
...
void sgl_per_token_group_quant_8bit_v2_checked(
...
#define LAUNCH_KERNEL_INNER(SCHEDULER, GROUP_SIZE, THREADS_PER_SUBWARP, T, DST_DTYPE, output_s_dtype, ...)             \
    do {                                                                                                               \
        ...
        SCHEDULER::compute_exec_config(THREADS_PER_SUBWARP,                                                            \
                                       ...
                                       block);                                                                         \
                                                                                                                       \
        per_token_group_quant_8bit_v2_checked_kernel<                                                                 \
            SCHEDULER, GROUP_SIZE, THREADS_PER_SUBWARP, T, DST_DTYPE, __VA_ARGS__>                                    \
