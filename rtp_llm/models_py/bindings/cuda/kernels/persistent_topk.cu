...
void launch_persistent_topk(const torch::Tensor& logits,
  ...
  TORCH_CHECK(cudaDeviceGetAttribute(
                  &num_sms, cudaDevAttrMultiProcessorCount, device) == cudaSuccess,
              "failed to query CUDA SM count for device ", device);
  TORCH_CHECK(cudaDeviceGetAttribute(&max_smem_per_block,
                                     cudaDevAttrMaxSharedMemoryPerBlockOptin,
                                     device) == cudaSuccess,
              "failed to query CUDA shared-memory limit for device ", device);
...
void persistent_topk(const torch::Tensor& logits, const torch::Tensor& lengths,
  ...
  TORCH_CHECK(workspace.is_cuda(), "workspace must be CUDA tensor");
  TORCH_CHECK(lengths.device() == logits.device() &&
                  output.device() == logits.device() &&
