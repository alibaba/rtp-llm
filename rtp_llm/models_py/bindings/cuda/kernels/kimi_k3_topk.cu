 ...
 */
#include "rtp_llm/models_py/bindings/cuda/kernels/kimi_k3_topk_native.cuh"
...
// Kernel body pinned to vLLM c3b48446349569512749db7f6e2164aa8a33437d.
// Only namespace and the RTP Tensor ABI differ; scoring/reduction order is native.
std::tuple<at::Tensor, at::Tensor> kimi_k3_grouped_topk(
  ...
  TORCH_CHECK(scores.is_cuda() && bias.is_cuda(), "K3 routing requires CUDA tensors");
  TORCH_CHECK(scores.scalar_type() == at::kFloat && bias.scalar_type() == at::kFloat,
              "K3 routing requires FP32 logits and correction bias");
  TORCH_CHECK(scores.device() == bias.device(), "K3 routing requires one device");
  TORCH_CHECK(scores.dim() == 2 && bias.dim() == 1 && scores.size(1) == bias.size(0),
              "K3 routing expects [tokens, experts] and [experts]");
  TORCH_CHECK(scores.is_contiguous() && bias.is_contiguous(), "K3 routing requires contiguous inputs");
  const auto tokens = scores.size(0), experts = scores.size(1);
  TORCH_CHECK(n_group > 0 && n_group <= 32 && topk_group > 0 && topk_group <= n_group,
              "Invalid K3 routing groups");
  TORCH_CHECK(experts > 0 && experts % n_group == 0 && topk > 0 && topk <= 32 &&
              topk <= topk_group * (experts / n_group), "Invalid K3 routing top-k");
  TORCH_CHECK(tokens <= std::numeric_limits<int32_t>::max(), "K3 routing token count exceeds int32");
  const c10::cuda::CUDAGuard guard(scores.device());
  auto weights = at::empty({tokens, topk}, scores.options());
  auto ids = at::empty({tokens, topk}, scores.options().dtype(at::kInt));
  if (tokens == 0) return {weights, ids};
  const auto stream = at::cuda::getCurrentCUDAStream(scores.get_device());
  rtp_k3_native::moe::invokeNoAuxTc<float, float, int32_t, rtp_k3_native::moe::SCORING_SIGMOID>(
      scores.data_ptr<float>(), weights.data_ptr<float>(), ids.data_ptr<int32_t>(),
      bias.data_ptr<float>(), tokens, experts, n_group, topk_group, topk,
      renormalize, scale, tokens <= rtp_k3_native::moe::PDLEnableTokens, stream);
