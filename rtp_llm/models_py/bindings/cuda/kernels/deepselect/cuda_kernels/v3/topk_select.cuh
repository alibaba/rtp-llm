...
#include "rtp_llm/models_py/bindings/cuda/kernels/deepselect/cuda_kernels/utils.cuh"
#include "rtp_llm/models_py/bindings/cuda/kernels/deepselect/cuda_kernels/common_parts.cuh"
...
public:
    ...
    static __device__ __forceinline__ void topk_select_kernel_devfunc(const TopkSelectArgs& args,
        ...
        // RTP: clamp signed bounds before conversion; the sparse plan normally
        // already produces valid bounds, but the native API is safe for any int32.
        int32_t requested_end =
            args.end_ptr == nullptr ? static_cast<int32_t>(args.vocab_size) : __ldg(args.end_ptr + batch_idx);
        uint32_t end_vocab_idx = requested_end <= 0 ? 0u : min(static_cast<uint32_t>(requested_end), args.vocab_size);
        ...
        uint32_t num_perm_segs =
            num_input_segs <= NUM_SEGS_IN_INIT_WINDOW ?
                0u :
                (num_input_segs - 1) / NUM_TAIL_SEGS
                    * NUM_TAIL_SEGS;  // Skip permutation if the total number of segs is small enough
        uint32_t num_perm_elems = num_perm_segs * NUM_ELEMS_PER_SEG;
        uint32_t num_tail_elems_padded =
            num_perm_segs != 0 ? (uint32_t)NUM_TAIL_ELEMS : num_input_segs * NUM_ELEMS_PER_SEG;
        ...
        BF16Base::template scan_segs<false>(tma_params,
                                            smem,
                                            batch_idx,
                                            end_vocab_idx,
                                            args.topk,
                                            ...
                                            [&](uint32_t m) {
                                                if constexpr (HAS_PARTIAL_ROUNDS) {
                                                    return (num_init_rounds + m) * NUM_SEGS_PER_ROUND + warp_idx
                                                           < NUM_TAIL_SEGS + num_perm_segs;
                                                } else {
                                                    return true;
                                                }
                                            });
...
__launch_bounds__(Kernel::NUM_THREADS, Kernel::TARGET_OCCUPANCY, 1) __global__
    void topk_kernel(__grid_constant__ const TopkSelectArgs             args,
...
void run_topk_select_kernel(const TopkSelectArgs& args) {
    ...
    ku::launch_kernel(ku::KernelLaunchConfig{dim3(args.batch_size), dim3(Kernel::NUM_THREADS), smem_size, args.stream},
                      kernel,
                      args,
                      tma_params);
}
