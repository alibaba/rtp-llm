...
__device__ __forceinline__ uint32_t bf16x2_to_u32(const nv_bfloat162& v) {
    static_assert(sizeof(nv_bfloat162) == sizeof(uint32_t));
    return *reinterpret_cast<const uint32_t*>(&v);
}

__device__ __forceinline__ nv_bfloat162 u32_to_bf16x2(uint32_t u) {
    static_assert(sizeof(nv_bfloat162) == sizeof(uint32_t));
    nv_bfloat162 v;
...
__device__ __forceinline__ UIntValueT distort(const UIntValueT& x) {
...
template<typename UIntValueT>
__device__ __forceinline__ void distort_x2(UIntValueT result[2], const UIntValueT input[2]) {
    static_assert(sizeof(UIntValueT) == 2 || sizeof(UIntValueT) == 4);
    if constexpr (sizeof(UIntValueT) == 2) {
        asm volatile("{"
                     ".reg .u32 mask;"
                     "prmt.b32 mask, %1, 0, 0xbb99;"  // mask[0:16] will be 0xFF if cur_value_packed[0]'s MSB is 1, 0x00
                                                      // otherwise
...
template<typename UIntValueT>
__device__ __forceinline__ UIntValueT un_distort(const UIntValueT& x) {
    static_assert(sizeof(UIntValueT) == 2 || sizeof(UIntValueT) == 4);
    if constexpr (sizeof(UIntValueT) == 2) {
