...
#include "utils.cuh"
#include "rtp_llm/models_py/bindings/cuda/kernels/deepselect/cuda_kernels/config.h"
#include "rtp_llm/models_py/bindings/cuda/kernels/deepselect/cuda_kernels/bit_utils.cuh"
...
__device__ __forceinline__ void ld_shared(ValueT res[NUM_VALUES], const ValueT* ptr) {
    constexpr uint32_t NUM_BYTES_TO_LOAD = NUM_VALUES * sizeof(ValueT);
    static_assert(NUM_BYTES_TO_LOAD == 16 || NUM_BYTES_TO_LOAD == 8 || NUM_BYTES_TO_LOAD == 4
                  || NUM_BYTES_TO_LOAD == 2);
...
template<uint32_t NUM_VALUES, typename ValueT>
__device__ __forceinline__ void ld_shared_with_loop(ValueT res[NUM_VALUES], const ValueT* ptr) {
    constexpr uint32_t NUM_BYTES_TO_LOAD = NUM_VALUES * sizeof(ValueT);
    if constexpr (NUM_BYTES_TO_LOAD <= 16) {
        ...
        ld_shared_with_loop<NUM_VALUES - NUM_LOAD_VALUES_THIS_ROUND>(res + NUM_LOAD_VALUES_THIS_ROUND,
                                                                     ptr + NUM_LOAD_VALUES_THIS_ROUND);
    }
}

template<uint32_t NUM_VALUES, typename ValueT>
__device__ __forceinline__ void st_global(ValueT* ptr, ValueT src[NUM_VALUES]) {
    constexpr uint32_t NUM_BYTES_TO_STORE = NUM_VALUES * sizeof(ValueT);
    static_assert((IS_LDG_STG_256_AVAILABLE && NUM_BYTES_TO_STORE == 32) || NUM_BYTES_TO_STORE == 16
                  || NUM_BYTES_TO_STORE == 8 || NUM_BYTES_TO_STORE == 4 || NUM_BYTES_TO_STORE == 2);
...
template<typename Config, uint32_t MAX_TOPK, uint32_t NUM_THREADS, uint32_t TARGET_OCCUPANCY>
struct EpilogueRunner {
    ...
    using BlockRadixSortT                         = cub::BlockRadixSort<UIntValueT,
                                                                        NUM_THREADS,
                                                                        NUM_VALUES_PER_THREAD_FOR_SORT,
                                                                        uint32_t,
                                                                        4,
                                                                        !(NUM_REGS_PER_THREAD <= 80
                                                  && NUM_VALUES_PER_THREAD_FOR_SORT
                                                         >= 16)  // MemoizeOuterScan. Setting this to `false` can reduce
                                                                                         // register pressure. We use a simple heuristic here
                                                                        >;
    ...
    using BlockRadixSortTempStorageT =
        cute::conditional_t<Config::sorted_value, typename BlockRadixSortT::TempStorage, char>;

    template<bool IS_SHORTCUT>  // "shortcut" means `end_vocab_idx` <= `topk`
    static __device__ __forceinline__ void topk_select_epilogue(typename Config::ValueT* smem_value_buf,  // [MAX_TOPK]
        ...
        //  - Assume topk indices locate at smem_index_buf[:topk] and topk values locate at smem_value_buf[:topk] (an
        //  extra `__syncthreads()` might be necessary)
        //  - Perform sorting, if `Config::sorted_value` is `true`
        //  - Save indices & values to global memory (values are un-distorted if `UNDISTORT_VALUES` is `true`)

        ValueT* input_values =
            Config::return_value ? (ValueT*)args.input + (uint64_t)batch_idx * args.stride_input_batch : nullptr;
        OutIdxT* result_indices    = (OutIdxT*)args.output_index + (uint64_t)batch_idx * args.stride_output_index_batch;
        ValueT*  result_values     = Config::return_value ?
                                         (ValueT*)args.output_value + (uint64_t)batch_idx * args.stride_output_value_batch :
                                         nullptr;
        ...
        if constexpr (!Config::sorted_value) {
            if constexpr (IS_SHORTCUT) {
                // Saves indices 0...end_vocab_idx (plus `output_idx_offset`) followed by (args.topk-end_vocab_idx)
                // `args.idx_oob_fill_value` into `result_indices`
                constexpr uint32_t NUM_OUTPUT_IDXS_PER_STORE = NUM_BYTES_PER_GMEM_STORE / sizeof(OutIdxT);
#pragma unroll 2
                for (uint32_t i = threadIdx.x * NUM_OUTPUT_IDXS_PER_STORE; i < args.topk;
                     i += NUM_THREADS * NUM_OUTPUT_IDXS_PER_STORE) {
...
#pragma unroll 2
                    for (uint32_t i = threadIdx.x * NUM_VALUES_PER_LOAD_STORE; i < args.topk;
                         i += NUM_THREADS * NUM_VALUES_PER_LOAD_STORE) {
                ...
                constexpr uint32_t NUM_OUTPUT_IDX_PER_ROUND =
                    cute::min(NUM_BYTES_PER_SMEM_STORE / sizeof(uint32_t), NUM_BYTES_PER_GMEM_STORE / sizeof(OutIdxT));
#pragma unroll 2
                for (uint32_t i = threadIdx.x * NUM_OUTPUT_IDX_PER_ROUND; i < args.topk;
                     i += NUM_THREADS * NUM_OUTPUT_IDX_PER_ROUND) {
                ...
                if constexpr (Config::return_value) {
                    // Load & store values
                    constexpr uint32_t NUM_VALUES_PER_ROUND =
                        cute::min(NUM_BYTES_PER_SMEM_STORE, NUM_BYTES_PER_GMEM_STORE) / sizeof(ValueT);
#pragma unroll 2
                    for (uint32_t i = threadIdx.x * NUM_VALUES_PER_ROUND; i < args.topk;
                         i += NUM_THREADS * NUM_VALUES_PER_ROUND) {
...
#pragma unroll 2
                    for (uint32_t i = (threadIdx.x - NUM_THREADS / 2) * NUM_VALUES_PER_LDG128; i < end_vocab_idx;
                         i += (NUM_THREADS / 2) * NUM_VALUES_PER_LDG128) {
            ...
            ld_shared_with_loop<NUM_VALUES_PER_THREAD_FOR_SORT>(
                local_values, (UIntValueT*)smem_value_buf + threadIdx.x * NUM_VALUES_PER_THREAD_FOR_SORT);
            ld_shared_with_loop<NUM_VALUES_PER_THREAD_FOR_SORT>(
                local_indices, smem_index_buf + threadIdx.x * NUM_VALUES_PER_THREAD_FOR_SORT);
            CUTE_UNROLL
            ...
            // For common cases, our MAX_TOPK <= 1024 and NUM_THREADS >= 256, so we have NUM_VALUES_PER_THREAD_FOR_SORT
            // <= 8, which means each thread only need to call STG once. So we don't use SMEM to buffer here
            if (thread_offset < args.topk) {
                CUTE_UNROLL
                for (uint32_t i = 0; i < NUM_VALUES_PER_THREAD_FOR_SORT; ++i)
                    local_values[i] = !IS_SHORTCUT || threadIdx.x * NUM_VALUES_PER_THREAD_FOR_SORT + i < end_vocab_idx ?
                                          un_distort(local_values[i]) :
                                          *(UIntValueT*)&oob_fill_value;
                ...
                auto store = [&](auto* dst, auto* src) {
                    using T                               = std::remove_pointer_t<decltype(dst)>;
                    constexpr uint32_t NUM_VALUES         = NUM_VALUES_PER_THREAD_FOR_SORT;
                    constexpr uint32_t NUM_BYTES_TO_STORE = NUM_VALUES * sizeof(T);
                    if constexpr (NUM_BYTES_TO_STORE <= NUM_BYTES_PER_GMEM_STORE) {
                        // Don't need to do OOB check because the output array is at least padded to the maximum width
                        // of global store.
                ...
                store((UIntValueT*)result_values + thread_offset, local_values);
                store(result_indices + thread_offset, local_indices_new);
            }
        }
    }
};
...
public:
    ...
    static constexpr uint64_t PLACEHOLDER_B64 =
        std::is_same_v<ValueT, nv_bfloat16> ? 0xff80ff80ff80ff80 : 0xff800000ff800000;
    ...
    static constexpr uint32_t NUM_ELEMS_PER_THREAD_PER_ROUND =
        NUM_ELEMS_PER_ROUND / NUM_THREADS;  // TODO Unify "values", "elements", and "elems" to "elems"
    ...
    static constexpr uint32_t NUM_UINT32_IN_INIT_WINDOW_PER_THREAD =
        NUM_ELEMS_IN_INIT_WINDOW * sizeof(ValueT) / sizeof(uint32_t) / NUM_THREADS;
    ...
    static constexpr uint32_t NUM_BYTES_PER_TMA_ROW = 128;  // Limit the innermost box dim of TMA to 1) prevent OOB 2)
                                                            // swizzling has limitations on the innermost box dim
    ...
    static CUtensorMap make_topk_tensor_map(const TopkSelectArgs& args) {
        ...
        constexpr CUtensorMapDataType dtype =
            std::is_same_v<ValueT, nv_bfloat16> ? CU_TENSOR_MAP_DATA_TYPE_BFLOAT16 : CU_TENSOR_MAP_DATA_TYPE_FLOAT32;
        return ku::make_tensor_map(
            ...
            {NUM_ELEMS_PER_TMA_ROW, NUM_TMA_ROWS_PER_SEG, 1},
            args.input,
            dtype,
    ...
    static __device__ __forceinline__ uint32_t get_permuted_seg_idx(uint32_t linear_id,
                                                                    uint32_t perm_len,
                                                                    uint32_t perm_mul) {
        return ((linear_id % perm_len) * perm_mul + PERM_ADD_BASE) % perm_len;
    }

    static __device__ __forceinline__ void advance_perm_state(uint32_t& state, uint32_t stride, uint32_t perm_len) {
        state += stride;
        if (state >= perm_len)
            state -= perm_len;
    }

    static __device__ __forceinline__ uint32_t sw_msk(uint32_t e) {
        return (e >> SWIZZLE_SHIFT) & SWIZZLE_MASK;
    }
    static __device__ __forceinline__ uint32_t sw_elem(uint32_t e) {
        return e ^ sw_msk(e);
    }
    static __device__ __forceinline__ uint32_t sw_b128(uint32_t u) {
        return u ^ ((u >> 3) & (7));
    }
    ...
    // Given local > pivot and == pivot counts, compute this thread's output prefix (start position in the final output
    // buffer) and how many equal elements it may still append (eq_quota). Should be called by all threads cnt_gt and
    // cnt_eq must be < 65536.
    static __device__ __forceinline__ EqGtPrefix compute_equal_quota_and_prefix(
        uint32_t cnt_gt, uint32_t cnt_eq, uint32_t topk, uint32_t warp_idx, uint32_t lane_idx, uint32_t* warp_cnt) {
    ...
    static __device__ __forceinline__ void load_swizzled_slice(
    ...
    // Find the bucket id that the topk-th element resides in, and get how many elements we should select from that
    // bucket Save result to `smem.reconstruct_pivot_bucket` and `smem.reconstruct_num_should_select` If
    // `CHECK_IF_SHOULD_SELECT_WHOLE_BUCKET`, return whether we should select all elements in that bucket. Should be
    // called by a whole warp
    template<bool CHECK_IF_SHOULD_SELECT_WHOLE_BUCKET>
    static __device__ __forceinline__ bool find_pivot_in_histogram(SharedMemoryPlanBase& smem,
                                                                   const uint32_t*       bucket_counter,
                                                                   uint32_t              topk,
                                                                   uint32_t              lane_idx) {
        ...
        if (suffix_count[8] < topk && topk <= suffix_count[0]) {
            ...
            if constexpr (CHECK_IF_SHOULD_SELECT_WHOLE_BUCKET) {
                ...
                uint32_t d0                = (j & 1) ? suffix_count[1] : suffix_count[0],
                         d1                = (j & 1) ? suffix_count[3] : suffix_count[2];
                uint32_t d2                = (j & 1) ? suffix_count[5] : suffix_count[4],
                         d3                = (j & 1) ? suffix_count[7] : suffix_count[6];
                d0                         = (j & 2) ? d1 : d0;
                d2                         = (j & 2) ? d3 : d2;
    ...
    static __device__ __forceinline__ void clear_reconstruct_histograms(SharedMemoryPlanBase& smem,
                                                                        uint32_t              thread_idx) {
    ...
    init_shared_memory(SharedMemoryPlanBase& smem, uint32_t warp_idx, ExtraBarInitF&& init_extra_bar_func) {
    ...
    static __device__ __forceinline__ void init_shared_memory(SharedMemoryPlanBase& smem, uint32_t warp_idx) {
        init_shared_memory(smem, warp_idx, [] {});
    }

    // Issue TMA loads for one round
    // Use IS_INIT to control whether is the init round.
    // For init round: copy the "tail" [num_perm_segs * NUM_ELEMS_PER_SEG, end_vocab_idx), if HAVE_TAIL is true, and
    // also the first NUM_SEGS_PER_ROUND - NUM_TAIL_SEGS segments according to the perm generator For non-init round:
    // copy the next NUM_SEGS_PER_ROUND segs according to the perm generator
    template<bool IS_INIT, bool HAVE_TAIL>
    static __device__ __forceinline__ void issue_tma_loads_for_round(SharedMemoryPlanBase& smem,
                                                                     const CUtensorMap&    tensor_map,
                                                                     uint32_t              batch_idx,
                                                                     uint32_t              end_vocab_idx,
                                                                     uint32_t              num_perm_segs,
                                                                     uint32_t              round_idx,
                                                                     uint32_t&             next_tma_permuted_segment,
                                                                     uint32_t              tma_permuted_segment_stride,
                                                                     uint32_t              warp_idx) {
        ...
        auto               copy_one_seg      = [&](uint32_t dst_seg_idx, uint32_t global_seg_idx) {
            cute::SM90_TMA_LOAD_3D::copy(&tensor_map,
                                         ...
                                         0,
                                         global_seg_idx * NUM_TMA_ROWS_PER_SEG,
                                         batch_idx);
        };
        ...
        if (IS_INIT && num_perm_segs == 0) {
            ...
            if constexpr (HAVE_TAIL) {
                // Copy tail segs
                // To save memory BW, only copy segs that have at least one valid element. The other segs are handled by
                // `fill_padded_tail_segments`
                ...
                bar.arrive_and_expect_tx(
                    (num_real_tail_segs_issued + NUM_SEGS_PER_ISSUE_WARP - NUM_TAIL_SEGS_PER_ISSUE_WARP)
                    * NUM_BYTES_PER_SEG);
            } else {
    ...
    static __device__ __forceinline__ void fill_padded_tail_segments(ValueT* init_buf, uint32_t num_tail_elems) {
        ...
        for (uint32_t i = tail_covered_by_tma + threadIdx.x * NUM_ELEMS_PER_128b; i < NUM_TAIL_ELEMS;
             i += NUM_THREADS * NUM_ELEMS_PER_128b) {
    ...
    static __device__ __forceinline__ void
    stage_output_and_epilogue(SharedMemoryPlanBase&                           smem,
                              typename EpilogueT::BlockRadixSortTempStorageT& sort_temp_storage,
                              const TopkSelectArgs&                           args,
                              uint32_t                                        batch_idx,
                              uint32_t                                        end_vocab_idx,
        ...
        uint32_t* smem_index_buf =
            (uint32_t*)(smem.surviving_topk_pairs[staging_buf_idx] + MAX_TOPK * (uint32_t)sizeof(ValueT) / 8);
        ...
        if constexpr (SORT_BY_INDEX)
            if (!is_already_sorted_in_index) {
                ...
                static_assert(NUM_WARPS <= 32);
                uint32_t cur_prefix_sum =
                    __reduce_add_sync(0xFFFFFFFF, lane_idx < warp_idx ? smem.warp_cnt[lane_idx] : 0u)
                    + warp_level_exclusive_prefix_sum(my_cnt, lane_idx);
                for (uint32_t i = w_lo; i < w_hi; ++i) {
        ...
        EpilogueT::template topk_select_epilogue<false>(
            smem_value_buf, smem_index_buf, args, batch_idx, end_vocab_idx, warp_idx, sort_temp_storage);
    }

    // RTP: NaN rows must initialize every output slot; the upstream sentinel
    // only writes slot zero and leaves the remaining indices undefined.
    static __device__ __forceinline__ void take_action_when_have_nan(const TopkSelectArgs& args, uint32_t batch_idx) {
        if (args.abort_when_nan_found) {
            if (threadIdx.x == 0) {
                printf(
                    "[topk_select] NaN detected. Calling `trap;` which will result in \"unspecified launch failure\"\n");
            }
            __syncthreads();
            ku::trap();
        } else {
            OutIdxT* row_output = (OutIdxT*)args.output_index + batch_idx * args.stride_output_index_batch;
            for (uint32_t i = threadIdx.x; i < args.topk; i += blockDim.x) {
                row_output[i] = args.idx_oob_fill_value;
            }
        }
    }
};
...
public:
    ...
    // Histogram pass 1 (MSB radix): for each distorted bf16x2 value, retrieve the upper 8 bits and performs atomicAdd
    // on the bucket counter `num_packed_values` should be guaranteed to be aligned by `NUM_PACKED_VALUES_ALIGNMENT`
    template<uint32_t N, uint32_t NUM_PACKED_VALUES_ALIGNMENT = 1>
    static __device__ __forceinline__ void
    histogram_radix_msb(uint32_t* bucket_counter, const nv_bfloat162 (&packed_values)[N], uint32_t num_packed_values) {
        uint32_t bucket_base = cute::cast_smem_ptr_to_uint(bucket_counter);
        CUTE_UNROLL
        for (uint32_t i = 0; i < N; i++) {
            if (i % NUM_PACKED_VALUES_ALIGNMENT == 0 && i == num_packed_values)
                break;
    ...
    // Histogram pass 2 (LSB radix refinement): among values whose distorted MSB equals pivot_hi8, retrieve the lower 8
    // bits and performs atomicAdd on the bucket counter `num_packed_values` should be guaranteed to be aligned by
    // `NUM_PACKED_VALUES_ALIGNMENT` `pivot_hi8` is the highest 8 bit of the DISTORTED pivot
    template<bool USE_CLUSTER_ADDRESSING, uint32_t N, uint32_t NUM_PACKED_VALUES_ALIGNMENT = 1>
    static __device__ __forceinline__ void histogram_radix_lsb_for_pivot_msb(uint32_t* bucket_counter,
                                                                             uint32_t  pivot_hi8,
                                                                             const nv_bfloat162 (&packed_values)[N],
                                                                             uint32_t num_packed_values) {
        uint32_t bucket1_base = cute::cast_smem_ptr_to_uint(bucket_counter);
        // Every elements in the pivot bucket must have the same sign, so we can use this information to optimize
        // instead of
        ...
        // Elements outside the bucket are counted in this row's sink slot (index NUM_RECONSTRUCT_BUCKETS) and never
        // read.
        ...
        for (uint32_t i = 0; i < N; i++) {
            ...
            asm volatile(
                ...
                "}\n"
                :
                : "r"(bf16x2_to_u32(packed_values[i])),
                  "r"(raw_hi8_x2),
                  "r"(bucket1_base),
                  "r"(scratch_bucket_idx_x2),
                  "r"(low_byte_xor)
                : "memory");
        }
    }
    ...
    template<uint32_t N, uint32_t NUM_PACKED_VALUES_ALIGNMENT = 1>
    static __device__ __forceinline__ CensusCounts get_census_counts(const nv_bfloat162 (&values)[N],
                                                                     uint32_t num_packed_values,
    ...
    static __device__ __forceinline__ void copy_selected_pairs_to_survivor(
        uint32_t& dst_ptr,      // The pointer to the new survivor buffer. May increase. Marked as uint32 since it's
                                // obtained from cast_smem_ptr_to_uint
...
#undef MOVE_SELECTED_PAIR2
    ...
    template<bool USE_CLUSTER_ADDRESSING>
    static __device__ __forceinline__ void append_selected_pairs_from_registers(uint32_t&    dst_ptr,
                                                                                uint32_t&    eq_quota,
                                                                                nv_bfloat162 packed_values,
                                                                                uint32_t     pivot_value_x2_bits,
                                                                                uint32_t     index0,
...
#define APPEND_SELECTED_PAIR2(ADD)                                                                                     \
    asm volatile("{\n"                                                                                                 \
                 ...
                 "}\n"                                                                                                 \
                 : "+r"(dst_ptr), "+r"(eq_quota)                                                                       \
                 : "r"(values_raw), "r"(pivot_value_x2_bits), "r"(index0), "r"(index1), "r"(val_word1)                 \
                 : "memory")
        if constexpr (USE_CLUSTER_ADDRESSING) {
...
#undef APPEND_SELECTED_PAIR2
    ...
    // The MSB histogram should be ready and all threads should be able to see it (e.g. via a `__syncthreads()`) before
    // we call the function We don't absorb MSB histogram's logic inside since sometimes we'd like to overlap TMA load
    // and MSB histogram's modification
    ...
    template<bool USE_CLUSTER_ADDRESSING, uint32_t N, uint32_t NUM_PACKED_VALUES_ALIGNMENT = 1>
    static __device__ __forceinline__ PivotAndQuota compute_pivot_and_quota(uint32_t topk,
                                                                            const nv_bfloat162 (&values)[N],
                                                                            uint32_t              num_values,
                                                                            uint32_t              warp_idx,
                                                                            uint32_t              lane_idx,
                                                                            SharedMemoryPlanBase& smem) {
        if (warp_idx == 0) {
        ...
        histogram_radix_lsb_for_pivot_msb<USE_CLUSTER_ADDRESSING, N, NUM_PACKED_VALUES_ALIGNMENT>(
            smem.reconstruct_bucket_counter[1], pivot_hi8, values, num_values);
        __syncthreads();

        if (warp_idx == 0) {
            Base::template find_pivot_in_histogram<false>(
                smem, smem.reconstruct_bucket_counter[1], num_elem_should_select_in_pivot_bucket, lane_idx);
        }
        __syncthreads();
        ...
        auto eqgt = Base::compute_equal_quota_and_prefix(
            census.cnt_gt, census.cnt_eq, topk, warp_idx, lane_idx, smem.reconstruct_bucket_counter[0]);
    ...
    scan_segs(const TmaParams&      tma_params,
              SharedMemoryPlanBase& smem,
              uint32_t              batch_idx,
              uint32_t              end_vocab_idx,
              uint32_t              topk,
              ...
              uint32_t local_start_seg_idx,  // The index of the first "local" segment ("local" means "belong to this
                                             // CTA"). 0 if not the cluster-based implementation
              uint32_t num_local_perm_segs,  // The number of permuted segments to be processed, locally
              uint32_t num_local_tail_elems_padded,  // num_local_tail_elems, padded to NUM_TAIL_ELEMS if num_perm_segs
                                                     // > 0, otherwise padded to NUM_ELEMS_PER_SEG
        ...
        auto issue_tma_copy = [&](auto is_init, auto have_tail, uint32_t round_idx) {
            constexpr bool IS_INIT   = decltype(is_init)::value;
            constexpr bool HAVE_TAIL = decltype(have_tail)::value;
            Base::template issue_tma_loads_for_round<IS_INIT, HAVE_TAIL>(smem,
                                                                         tma_params.tensor_map,
                                                                         batch_idx,
                                                                         end_vocab_idx,
                                                                         num_perm_segs,
                                                                         round_idx,
                                                                         next_tma_permuted_segment,
                                                                         tma_permuted_segment_stride,
                                                                         warp_idx);
        };
        auto issue_tma_copy_for_main_rounds = [&](uint32_t round_idx) {
            issue_tma_copy(std::bool_constant<false>{}, std::bool_constant<false>{}, round_idx);
        };
        ...
        if (num_local_elems_padded != 0) {
            // Issue copy: Copy the tail and the first few segments, result in init_buf[0: NUM_ELEMS_IN_INIT_WINDOW] (or
            // init_buf[0: ])
            ValueT* init_buf = (ValueT*)smem.incoming_topk_pairs;
            if (cute::elect_one_sync()) {
                if (num_local_tail_elems_padded != 0) {
                    issue_tma_copy(std::bool_constant<true>{}, std::bool_constant<true>{}, 0);
                } else {
                    issue_tma_copy(std::bool_constant<true>{}, std::bool_constant<false>{}, 0);
                }

                CUTE_UNROLL
                for (uint32_t i = 1; i < NUM_INIT_ROUNDS_MAX; i++) {
                    if (i < num_init_rounds) {
                        issue_tma_copy(std::bool_constant<true>{}, std::bool_constant<false>{}, i);
                    }
                }
            ...
            uint32_t num_elems_in_init_window_padded =
                num_local_tail_elems_padded
                + min(num_local_perm_segs, (uint32_t)NUM_SEGS_IN_INIT_WINDOW - num_local_tail_segs_padded)
                      * NUM_ELEMS_PER_SEG;  // Init window size, padded to NUM_ELEMS_PER_SEG
            ...
            for (uint32_t i = 0; i < NUM_INIT_ROUNDS_MAX; i++) {
                ...
                if (num_local_tail_elems % NUM_ELEMS_PER_SEG != 0 && i == num_local_tail_elems / NUM_ELEMS_PER_ROUND) {
                    uint32_t box_end =
                        ku::ceil_div(num_local_tail_elems, (uint32_t)NUM_ELEMS_PER_SEG) * NUM_ELEMS_PER_SEG;
                ...
                for (uint32_t k = 0; k < NUM_128b_PER_THREAD_PER_ROUND; ++k) {
                    ...
                    if (pos < num_128b_in_init_window_padded) {
                        ld_shared<4>(
                            my_values + k * 4,
                            reinterpret_cast<const nv_bfloat162*>(init_buf + Base::sw_b128(pos) * NUM_ELEMS_PER_128b));
                        num_my_values = 4 * (k + 1);
                    }
                }
                histogram_radix_msb<4 * NUM_128b_PER_THREAD_PER_ROUND, 4>(
                    smem.reconstruct_bucket_counter[0], my_values, num_my_values);
            }

            nv_bfloat162 init_values[NUM_UINT32_IN_INIT_WINDOW_PER_THREAD];
            CUTE_UNROLL
            for (uint32_t j = 0; j < NUM_128b_INIT_PER_THREAD; ++j) {
                if (j * NUM_ELEMS_PER_128b == num_my_elems)
                    break;
                uint32_t u0 = my_elem_start_idx / NUM_ELEMS_PER_128b + j;
                ld_shared<4>(init_values + j * NUM_UINT32_PER_128b,
                             reinterpret_cast<const nv_bfloat162*>(init_buf + Base::sw_b128(u0) * NUM_ELEMS_PER_128b));
            }
            ...
            auto [pivot_value_x2_bits, start_pos_in_collector, eq_quota, cnt_nan] =
                compute_pivot_and_quota<USE_CLUSTER_ADDRESSING, NUM_UINT32_IN_INIT_WINDOW_PER_THREAD, 4>(
                    effective_topk, init_values, num_my_elems / 2, warp_idx, lane_idx, smem);
...

            {
                ...
                uint32_t s0        = local_start_seg_idx
                              + (max(my_elem_start_idx, num_local_tail_elems_padded) - num_local_tail_elems_padded)
                                    / NUM_ELEMS_PER_SEG;
                ...
                for (uint32_t i = 0; i < NUM_UINT32_IN_INIT_WINDOW_PER_THREAD; i++) {
                    ...
                    append_selected_pairs_from_registers<USE_CLUSTER_ADDRESSING>(
                        out_ptr, eq_quota, cur_value, pivot_value_x2_bits, index0, index1);
                }
            }
        ...
        uint32_t linear_segment_start =
            local_start_seg_idx + num_init_rounds * NUM_SEGS_PER_ROUND - num_local_tail_segs_padded + warp_idx;
        ...
        // Re-select the top-k elements from the smem.surviving_topk_pairs[survivor_buf_idx][:num_survivors] +
        // smem.incoming_topk_pairs[:num_incomers], Write the selected pairs into the other survivor buffer, and return
        // the new pivot
        auto reconstruct = [&](uint32_t cur_extra_len) -> uint32_t {
            ...
            uint32_t num_uint32_per_thread =
                ku::ceil_div(num_uint32, (uint32_t)NUM_THREADS) | 1u;  // force odd => bank-conflict-free smem gathers
            ...
            {  // write back
                uint32_t out_ptr = cute::cast_smem_ptr_to_uint(smem.surviving_topk_pairs[survivor_buf_idx ^ 1])
                                   + out_prefix * (uint32_t)sizeof(uint64_t);
                CUTE_UNROLL
                for (uint32_t m = 0; m < NUM_UINT32_RECONSTRUCT_PER_THREAD; m++) {
                    if (m == num_my_units)
                        break;
                    copy_selected_pairs_to_survivor<USE_CLUSTER_ADDRESSING>(
                        out_ptr, eq_quota, values[m], pivot_value_x2_bits, unit_to_pair_addr(unit_base + m));
                }
            }
        ...
        for (uint32_t main_round_idx = 0; main_round_idx < num_main_rounds; ++main_round_idx) {
            ...
            // Hits are rare, so appending through the set bits of this mask is faster than testing + storing every
            // element again.
            ...
            if (is_warp_active) {
                ...
                Base::template load_swizzled_slice<NUM_128b_PER_THREAD_PER_ROUND>(
                    values, buf, smem_read_offset, chunk_swizzle_mask);
                CUTE_UNROLL
                for (int32_t j = NUM_ELEMS_PER_THREAD_PER_ROUND - 4; j >= 0; j -= 4) {
                    asm volatile(
                        "{\n"
                        ".reg .b32 r0, r1, ind, nib;\n"
                        "set.gtu.s32.bf16x2 r0, %1, %3;\n"  // .gtu: NaN always counts as a hit, so every NaN reaches
                                                            // the incoming buffer and the census below reports it.
                        "set.gtu.s32.bf16x2 r1, %2, %3;\n"  // `set` with .s32 fills the corresponding 16bit to 0xFFFF
                                                            // (-1) when the condition holds. We use `prmt` to extract
                                                            // one bit from each
                        "prmt.b32 ind, r0, r1, 0x7531;\n"
                        "dp4a.s32.s32 nib, ind, 0xF8FCFEFF, 0;\n"  // ind's bytes are 0x00/0xFF, so with signed byte
                                                                   // weights each hit contributes 1/2/4/8
            ...
            if (is_warp_active && warp_total_hits != 0) {
                asm volatile("ld.shared.u16 %0, [%1];"
                             : "=r"(first_hit_val)
                             : "r"(elem_addr_base + ((first_hit_e ^ chunk_swizzle_mask) << 1)));
            }
            ...
            uint32_t dst_slot =
                num_incomers + __reduce_add_sync(0xFFFFFFFF, lane_idx < warp_idx ? stored_warp_hits : 0u) + lane_prefix;

            if (is_warp_active && warp_total_hits != 0) {
                uint32_t dst_ptr =
                    cute::cast_smem_ptr_to_uint(smem.incoming_topk_pairs) + dst_slot * (uint32_t)sizeof(uint64_t);
                if (hit_mask != 0) {
                    asm volatile("st.shared.v2.u32 [%0], {%1, %2};" ::"r"(dst_ptr),
                                 "r"(seg_elem_base + first_hit_e),
                                 "r"(first_hit_val)
                                 : "memory");
                ...
                while (mask != 0) {
                    ...
                    asm volatile("ld.shared.u16 %0, [%1];"
                                 : "=r"(val_word)
                                 : "r"(elem_addr_base + ((e ^ chunk_swizzle_mask) << 1)));
                    asm volatile(
                        "st.shared.v2.u32 [%0], {%1, %2};" ::"r"(dst_ptr), "r"(seg_elem_base + e), "r"(val_word)
                        : "memory");
                    dst_ptr += (uint32_t)sizeof(uint64_t);
                }
            }
