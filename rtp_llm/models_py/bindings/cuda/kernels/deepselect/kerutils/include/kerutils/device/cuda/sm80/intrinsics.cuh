...
#include "rtp_llm/models_py/bindings/cuda/kernels/deepselect/kerutils/include/kerutils/device/cuda/common.h"
...
CUTE_DEVICE void cp_async_cacheglobal(const void* src, void* dst, bool pred = true) {
    ...
    if constexpr (PREFETCH_SIZE == PrefetchSize::B64) {
        ...
        static_assert(PREFETCH_SIZE == PrefetchSize::B64 || PREFETCH_SIZE == PrefetchSize::B128
...
#define EMIT2(PRIMARY_PRIORITY_STR)                                                                                    \
    ...
    if constexpr (PRIMARY_PRIORITY == CacheHint::EVICT_FIRST) {
        ...
        static_assert(PRIMARY_PRIORITY == CacheHint::EVICT_FIRST || PRIMARY_PRIORITY == CacheHint::EVICT_NORMAL
...
CUTE_DEVICE constexpr int64_t create_simple_cache_policy() {
    if constexpr (CACHE_HINT == CacheHint::EVICT_FIRST) {
        ...
        static_assert(CACHE_HINT == CacheHint::EVICT_FIRST || CACHE_HINT == CacheHint::EVICT_NORMAL
...
CUTE_DEVICE
void atomicadd_f32_with_policy_and_pred(void*        global_addr,
                                        const float& data,
...
// be larger than the physical number of SMs in the device.". However, result shows that, at least for sm90 and sm100,
...
#define KU_LDG_128(global_addr, result, NC_STR, L1_CACHE_HINT_STR, L2_PREFETCH_SIZE_STR)                               \
    {                                                                                                                  \
        ...
        asm volatile("ld.global" NC_STR ".L1::" L1_CACHE_HINT_STR ".L2::" L2_PREFETCH_SIZE_STR                         \
                     ".v2.u64 {%0, %1}, [%2];\n"                                                                       \
...
#define KU_STG_128(global_addr, src, L1_CACHE_HINT_STR, L2_CACHE_HINT_STR)                                             \
    {                                                                                                                  \
        ...
        asm volatile("st.global.L1::" L1_CACHE_HINT_STR L2_CACHE_HINT_STR ".v2.u64 [%0], {%1, %2};\n"                  \
                     :                                                                                                 \
