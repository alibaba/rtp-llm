...
#define KU_ASSERT(cond, ...)                                                                                           \
    do {                                                                                                               \
        if (not(cond)) {                                                                                               \
            char _ku_buf[1024];                                                                                        \
            int  _ku_len =                                                                                             \
                snprintf(_ku_buf, sizeof(_ku_buf), "Assertion `%s` failed (%s:%d)", #cond, __FILE__, __LINE__);        \
            __VA_OPT__(_ku_len += snprintf(_ku_buf + _ku_len, sizeof(_ku_buf) - _ku_len, ": " __VA_ARGS__);)           \
            fprintf(stderr, "%s\n", _ku_buf);                                                                          \
...
#define KU_CUDA_CHECK(call)                                                                                            \
    do {                                                                                                               \
        ...
        if (status_ != cudaSuccess) {                                                                                  \
            char _ku_buf[1024];                                                                                        \
            snprintf(                                                                                                  \
                _ku_buf, sizeof(_ku_buf), "CUDA error (%s:%d): %s", __FILE__, __LINE__, cudaGetErrorString(status_));  \
            fprintf(stderr, "%s\n", _ku_buf);                                                                          \
...
#define KU_CUTLASS_CHECK(call)                                                                                         \
    do {                                                                                                               \
        ...
        if (status_ != cutlass::Status::kSuccess) {                                                                    \
            char _ku_buf[1024];                                                                                        \
            snprintf(                                                                                                  \
                _ku_buf, sizeof(_ku_buf), "CUTLASS error (%s:%d): %d", __FILE__, __LINE__, static_cast<int>(status_)); \
            fprintf(stderr, "%s\n", _ku_buf);                                                                          \
...
make_tensor_map(const std::vector<uint64_t>& size,
    ...
    KU_ASSERT(strides.size() == (uint32_t)dim - 1 && box_size.size() == (uint32_t)dim
              && element_strides.size() == (uint32_t)dim);

    auto call_cuTensorMapEncodeTiled = [&](auto... args) {
        cudaDriverEntryPointQueryResult cuda_status;
        void*                           pfn = nullptr;
#if (__CUDACC_VER_MAJOR__ > 12)
        KU_CUDA_CHECK(
            cudaGetDriverEntryPointByVersion("cuTensorMapEncodeTiled", &pfn, 12000, cudaEnableDefault, &cuda_status));
#else
        KU_CUDA_CHECK(cudaGetDriverEntryPoint("cuTensorMapEncodeTiled", &pfn, cudaEnableDefault, &cuda_status));
#endif
    ...
    CUtensorMap result;
    CUresult    ret_code = call_cuTensorMapEncodeTiled(&result,
                                                    data_type,
                                                    dim,
                                                    global_ptr,
                                                    size.data(),
...
void launch_kernel(const KernelLaunchConfig& cfg, KernelFunc kernel, Args&&... args) {
    if (cfg.dynamic_smem > 0) {
        KU_CUDA_CHECK(cudaFuncSetAttribute(reinterpret_cast<const void*>(kernel),
    ...
    if (cfg.cooperative) {
        ...
        KU_CUDA_CHECK(cudaLaunchCooperativeKernel(kernel,
                                                  cfg.grid,
                                                  cfg.block,
                                                  sizeof...(Args) > 0 ? kernel_args : nullptr,
                                                  static_cast<unsigned int>(cfg.dynamic_smem),
                                                  cfg.stream));
    } else if (need_kernel_ex) {
        if (is_cluster) {
            KU_ASSERT(cfg.grid.x % cfg.cluster.x == 0 && cfg.grid.y % cfg.cluster.y == 0
                      && cfg.grid.z % cfg.cluster.z == 0);
            if (cfg.cluster.x * cfg.cluster.y * cfg.cluster.z > 8) {
                KU_CUDA_CHECK(cudaFuncSetAttribute(
                    reinterpret_cast<const void*>(kernel), cudaFuncAttributeNonPortableClusterSizeAllowed, 1));
            }
        }
        ...
        cudaLaunchConfig_t config = {{cfg.grid.x, cfg.grid.y, cfg.grid.z},
                                     {cfg.block.x, cfg.block.y, cfg.block.z},
                                     cfg.dynamic_smem,
                                     cfg.stream,
                                     attrs,
                                     num_attrs};
        KU_CUDA_CHECK(cudaLaunchKernelEx(&config, kernel, std::forward<Args>(args)...));
    } else {
        kernel<<<cfg.grid, cfg.block, cfg.dynamic_smem, cfg.stream>>>(std::forward<Args>(args)...);
    }
