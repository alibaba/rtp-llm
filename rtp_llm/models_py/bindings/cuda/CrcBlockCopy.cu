...
CrcBlockCopyResult CrcBlockCopy::store(
    void* host, size_t bytes, int64_t request_id, bool host_is_pinned, const std::function<bool()>& capture_failure) {
    auto& work = *impl_;
    ...
    checkCuda(cudaStreamSynchronize(work.stream));
    if (!host_is_pinned)
        std::memcpy(host, work.host_staging, transferBytes(bytes));
    // Provenance is CPU-owned and never used as CRC input or transferred to GPU.
    auto* metadata = static_cast<unsigned char*>(host) + transferBytes(bytes);
    std::memset(metadata, 0, storageBytes(bytes) - transferBytes(bytes));
    std::memcpy(metadata, &request_id, sizeof(request_id));
...
CrcBlockCopyResult CrcBlockCopy::loadAndValidate(const void*                  host,
    ...
    if (!host_is_pinned) {
        std::memcpy(work.host_staging, host, transferBytes(bytes));
        input = work.host_staging;
    }
    checkCuda(cudaMemcpyAsync(work.staging, input, transferBytes(bytes), cudaMemcpyHostToDevice, work.stream));
