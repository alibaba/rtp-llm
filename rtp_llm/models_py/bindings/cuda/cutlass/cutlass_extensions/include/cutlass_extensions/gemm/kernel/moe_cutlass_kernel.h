...
public:
    ...
    struct Arguments {
        ...
        Arguments(int                               problem_count,
                  ...
                  ElementScale const*               weight_scales,
                  ElementScale const*               weight_zeros,
    ...
    struct Params {
        ...
        Params(Arguments const& args, void* workspace = nullptr, int tile_count = 0):
            problem_visitor(args.total_tokens_including_expert,
                            args.gemm_n,
                            args.gemm_k,
                            args.problem_count,
                            workspace,
        ...
        void update(Arguments const& args, void* workspace = nullptr, int tile_count = 0) {
            ...
            problem_visitor   = typename ProblemVisitor::Params(args.total_tokens_including_expert,
                                                              args.gemm_n,
                                                              args.gemm_k,
                                                              args.problem_count,
                                                              workspace,
        ...
        while (problem_visitor.next_tile()) {
            ...
            if constexpr (use_dq_gemm<Mma>::value) {
                ElementScale* weight_scale_ptr =
                    isFinegrained(Mma::QuantOp) ?
                        params.weight_scales + problem_idx * gemm_k * gemm_n / params.group_size :
                        params.weight_scales + problem_idx * gemm_n;
                ElementScale* weight_zero_ptr =
                    hasZero(Mma::QuantOp) ? params.weight_zeros + problem_idx * gemm_n * gemm_k / params.group_size :
                                            nullptr;
                ...
                typename Mma::IteratorScale iterator_scale =
                    initialize_scale<typename Mma::IteratorScale, Mma::QuantOp>(
                        Mma::IteratorScale::Layout(scale_col_extent),
                        weight_scale_ptr,
                        weight_zero_ptr,
                        {scale_row_extent, scale_col_extent},
                        thread_idx,
                        tb_offset_scale,
                        params.group_size);
