...
TileConfig get_tile_config_from_config(CutlassTileConfig tile_config) {
    switch (tile_config) {
...
std::vector<CutlassTileConfig> get_candidate_tiles(const int  sm,
                                                   const bool is_weight_only,
...
get_valid_config_from_occupancies(const std::vector<CutlassGemmConfig>& candidate_configs,
                                  const std::vector<int>&               occupancies) {
    if (occupancies.size() != candidate_configs.size()) {
        throw std::runtime_error("[TensorRT-LLm Error][estimate_best_config_from_occupancies] occpancies and "
        ...
        if (current_score < config_score
            ...
            current_m_tile = tile_config.block_m;
                   ...
                   && (best_config.stages < candidate_config.stages
                       || candidate_config.split_k_factor < best_config.split_k_factor
                       || current_m_tile < tile_config.block_m)) {
...
get_candidate_tiles(int const sm, CutlassGemmConfig::CandidateConfigTypeParam const config_type_param) {
    enum class CutlassGemmType : char {
        Default,
        WeightOnly,
        Simt,
...
CutlassGemmConfig estimate_best_config_from_occupancies(std::vector<CutlassGemmConfig> const& candidate_configs,
    ...
    if (occupancies.size() != candidate_configs.size()) {
        TLLM_THROW("[estimate_best_config_from_occupancies] occpancies and "
                   "candidate configs vectors must have equal length.");
    }
