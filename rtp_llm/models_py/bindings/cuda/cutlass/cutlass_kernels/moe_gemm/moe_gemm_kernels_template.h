...
#include "rtp_llm/models_py/bindings/cuda/cutlass/cutlass_kernels/moe_gemm/launchers/fused_moe_gemm_launcher_sm80.h"
...
template<typename T,
         ...
         typename GemmOutputType,
         typename arch,
         typename EpilogueTag,
         typename ThreadblockShape,
         typename WarpShape,
         int Stages>
void genericMoeGemmKernelLauncher(T const*                              A,
                                  WeightType const*                     B,
                                  GemmOutputType const*                 weight_scales,
                                  GemmOutputType const*                 weight_zeros,
                                  int                                   group_size,
                                  GemmOutputType const*                 biases,
    ...
    if (!use_fused_moe) {
        ...
        using TaggedOperator =
            typename std::conditional<cutlass::platform::is_same<T, WeightType>::value,
        ...
        typename GemmGrouped::Arguments args(num_experts,
                                             ...
                                             group_size,
                                             epilogue_op,
                                             reinterpret_cast<ElementType const*>(A),
                                             ...
                                             reinterpret_cast<CutlassGemmOutputType const*>(biases),
                                             bias_is_broadcast,
                                             reinterpret_cast<CutlassGemmOutputType*>(C),
                         ...
                         && (std::is_same_v<EpilogueTag, cutlass_extensions::EpilogueOpDefaultSilu>
                             || std::is_same_v<EpilogueTag,
                                               cutlass_extensions::EpilogueOpDefaultFtGelu>))  // use fused moe gemm
                                                                                               ...
                                                                                               // fp16 or bf16)
    {
        sm80_generic_fused_moe_gemm_kernelLauncher<ElementType,
                                                   CutlassWeightType,
                                                   ThreadblockShape::kM,
                                                   ThreadblockShape::kN,
                                                   ThreadblockShape::kK,
                                                   Stages,
                                                   EpilogueTag>(reinterpret_cast<ElementType const*>(A),
                                                                reinterpret_cast<CutlassWeightType const*>(B),
                                                                reinterpret_cast<ElementType const*>(biases),
                                                                bias_is_broadcast,
...
template<typename T,
         ...
         typename WarpShape,
         int Stages>
static void dispatch(T const*                              A,
                     WeightType const*                     B,
                     GemmOutputType const*                 weight_scales,
                     GemmOutputType const*                 weight_zeros,
                     int                                   group_size,
    ...
    if constexpr ((Stages == 2 || Arch::kMinComputeCapability >= 80)
        ...
        kernels::cutlass_kernels::genericMoeGemmKernelLauncher<T,
                                                               WeightType,
                                                               QuantOp,
                                                               GemmOutputType,
                                                               Arch,
...
template<typename T,
         ...
         typename ThreadblockShape,
         typename WarpShape>
void dispatchGemmConfig(T const*                              A,
                        WeightType const*                     B,
                        GemmOutputType const*                 weight_scales,
                        GemmOutputType const*                 weight_zeros,
                        int                                   group_size,
    ...
    switch (gemm_config.stages) {
        case 2:
            dispatch<T, WeightType, QuantOp, GemmOutputType, arch, EpilogueTag, ThreadblockShape, WarpShape, 2>(
                A,
                B,
                weight_scales,
                weight_zeros,
            ...
            break;
        case 3:
            dispatch<T, WeightType, QuantOp, GemmOutputType, arch, EpilogueTag, ThreadblockShape, WarpShape, 3>(
                A,
                B,
                weight_scales,
                weight_zeros,
            ...
            break;
        case 4:
            dispatch<T, WeightType, QuantOp, GemmOutputType, arch, EpilogueTag, ThreadblockShape, WarpShape, 4>(
                A,
                B,
                weight_scales,
                weight_zeros,
...
void dispatchMoeGemmToCutlass(T const*                              A,
                              WeightType const*                     B,
                              GemmOutputType const*                 weight_scales,
                              GemmOutputType const*                 weight_zeros,
                              int                                   group_size,
    ...
    switch (gemm_config.tile_config) {
        case cutlass_extensions::CutlassTileConfig::CtaShape16x256x128_WarpShape16x64x128:
            dispatchGemmConfig<T,
                               ...
                               cutlass::gemm::GemmShape<16, 64, 128>>(A,
                                                                      B,
                                                                      weight_scales,
                                                                      weight_zeros,
                                                                      group_size,
...
std::vector<cutlass_extensions::CutlassGemmConfig>
MoeGemmRunner<T, WeightType, QuantOp, OutputType, ScaleBiasType>::getValidConfigs(
    T const*               A,
    WeightType const*      B,
    ...
    for (int i = 0; i < candidate_configs.size(); i++) {
        if (candidate_configs[i].tile_config_sm90 != CutlassTileConfigSM90::ChooseWithHeuristic
            || tensorrt_llm::kernels::cutlass_kernels::is_valid_split_k_factor(
...
template<typename T,
         ...
         typename OutputType,
         typename ScaleBiasType>
cutlass_extensions::CutlassGemmConfig
MoeGemmRunner<T, WeightType, QuantOp, OutputType, ScaleBiasType>::getChosenConfig(int64_t        total_rows,
                                                                                  int64_t        gemm_n,
                                                                                  int64_t        gemm_k,
                                                                                  int            num_experts,
                                                                                  bool           use_fused_moe,
    ...
    switch (activation_type) {
        case ActivationType::Relu:
            return dispatchChosenConfig<cutlass_extensions::EpilogueOpDefaultReLU>(
                total_rows, gemm_n, gemm_k, num_experts, use_fused_moe, stream);
        case ActivationType::Gelu:
            return dispatchChosenConfig<cutlass_extensions::EpilogueOpDefaultFtGelu>(
                total_rows, gemm_n, gemm_k, num_experts, use_fused_moe, stream);
        case ActivationType::Silu:
            return dispatchChosenConfig<cutlass_extensions::EpilogueOpDefaultSilu>(
                total_rows, gemm_n, gemm_k, num_experts, use_fused_moe, stream);
        case ActivationType::Identity:
            return dispatchChosenConfig<cutlass_extensions::EpilogueOpDefault>(
                total_rows, gemm_n, gemm_k, num_experts, use_fused_moe, stream);
...
template<typename EpilogueTag>
cutlass_extensions::CutlassGemmConfig
MoeGemmRunner<T, WeightType, QuantOp, OutputType, ScaleBiasType>::dispatchChosenConfig(
    ...
    if (sm_ == 90 && supportsHopperSpecialisation() && total_rows / num_experts > 32) {
        CutlassGemmConfig config(CutlassTileConfigSM90::CtaShape128x128x128B,
    ...
    HopperGroupedGemmInput hopper_input{};
    for (size_t ii = 0; ii < valid_splitk_configs.size(); ++ii) {
        dispatchToArch<EpilogueTag>(nullptr,
                                    nullptr,
                                    nullptr,
                                    nullptr,
                                    gemm_k,
...
template<typename T,
         ...
         typename OutputType,
         typename ScaleBiasType>
bool MoeGemmRunner<T, WeightType, QuantOp, OutputType, ScaleBiasType>::isFusedGatedActivation(bool is_gated_activation,
                                                                                              int  gemm_n,
                                                                                              int  gemm_k) const {
...
template<typename T,
         ...
         typename ScaleBiasType>
template<typename EpilogueTag>
void MoeGemmRunner<T, WeightType, QuantOp, OutputType, ScaleBiasType>::dispatchToArch<EpilogueTag>(
    T const*                              A,
    WeightType const*                     B,
    ScaleBiasType const*                  weight_scales,
    ScaleBiasType const*                  weight_zeros,
    ...
    TLLM_CHECK_WITH_INFO(sm_ >= 89 || !hopper_input.isValid(),
                         "Hopper input information is set for non specialised implementation");
    TLLM_CHECK_WITH_INFO(sm_ == 90 || !gemm_config.is_sm90,
    ...
    if (sm_ >= 70 && sm_ < 75) {
        ...
        if constexpr (use_fp8) {
            ...
            dispatchMoeGemmToCutlass<T, WeightType, QuantOp, ScaleBiasType, cutlass::arch::Sm80, EpilogueTag>(
                A,
                B,
                weight_scales,
                weight_zeros,
        ...
        if constexpr (kernels::cutlass_kernels::isValidHopperMOESpecialisation<T, WeightType, EpilogueTag>()) {
            ...
            if (gemm_config.is_sm90) {
                ...
                TLLM_CHECK_WITH_INFO(occupancy || hopper_input.isValid(),
                ...
                auto select_function = [&]() {
                    switch (hopper_input.fusion) {
                        case HopperGroupedGemmInput::EpilogueFusion::FINALIZE:
                            return &dispatchMoeGemmSelectTileShapeSM90<
                                T,
                                WeightType,
                                OutputType,
                                EpilogueTag,
                        ...
                        case HopperGroupedGemmInput::EpilogueFusion::NONE:
                            return &dispatchMoeGemmSelectTileShapeSM90<T,
                                                                       WeightType,
                                                                       OutputType,
                                                                       EpilogueTag,
                        ...
                        default:
                            TLLM_THROW("Unimplemented fusion %d requested", (int)hopper_input.fusion);
                    };
                };
...
template<typename T,
         ...
         typename ScaleBiasType>
template<typename EpilogueTag>
void MoeGemmRunner<T, WeightType, QuantOp, OutputType, ScaleBiasType>::runGemm(
    T const*                              A,
    WeightType const*                     B,
    ScaleBiasType const*                  weight_scales,
    ScaleBiasType const*                  weight_zeros,
...
template<typename T,
         ...
         typename OutputType,
         typename ScaleBiasType>
void MoeGemmRunner<T, WeightType, QuantOp, OutputType, ScaleBiasType>::moeGemmBiasAct(
    T const*                              A,
    WeightType const*                     B,
    ScaleBiasType const*                  weight_scales,
    ScaleBiasType const*                  weight_zeros,
    ...
    switch (activation_type) {
        case ActivationType::Relu:
            runGemm<cutlass_extensions::EpilogueOpDefaultReLU>(A,
                                                               B,
                                                               weight_scales,
                                                               weight_zeros,
                                                               group_size,
            ...
            break;
        case ActivationType::Gelu:
            runGemm<cutlass_extensions::EpilogueOpDefaultFtGelu>(A,
                                                                 B,
                                                                 weight_scales,
                                                                 weight_zeros,
                                                                 group_size,
            ...
            break;
        case ActivationType::Silu:
            runGemm<cutlass_extensions::EpilogueOpDefaultSilu>(A,
                                                               B,
                                                               weight_scales,
                                                               weight_zeros,
                                                               group_size,
            ...
            break;
        case ActivationType::Identity:
            runGemm<cutlass_extensions::EpilogueOpDefault>(A,
                                                           B,
                                                           weight_scales,
                                                           weight_zeros,
                                                           group_size,
...
template<typename T,
         ...
         typename OutputType,
         typename ScaleBiasType>
void MoeGemmRunner<T, WeightType, QuantOp, OutputType, ScaleBiasType>::moeGemm(
    T const*                              A,
    WeightType const*                     B,
    ScaleBiasType const*                  weight_scales,
    ScaleBiasType const*                  weight_zeros,
