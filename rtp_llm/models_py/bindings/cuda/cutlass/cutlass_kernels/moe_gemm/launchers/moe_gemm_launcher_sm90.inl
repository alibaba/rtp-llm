...
#include "rtp_llm/models_py/bindings/cuda/cutlass/cutlass_kernels/cutlass_type_conversion.h"
#include "rtp_llm/models_py/bindings/cuda/cutlass/cutlass_kernels/moe_gemm/launchers/moe_gemm_launcher_sm90.h"
...
struct HopperGroupedGemmInfo {
    ...
    using CollectiveEpilogueFinalize = typename cutlass::epilogue::collective::EpilogueMoeFusedFinalizeBuilder<  //
        ...
        StrideC*,  //
        ElementFinalOutput,
        HopperGroupedGemmInput::FusedFinalizeEpilogue::StrideFinalOutput,  //
        ElementAccumulator,                                                //
        ElementAccumulator,                                                //
        ElementBias,
        HopperGroupedGemmInput::FusedFinalizeEpilogue::StrideBias,  //
        ElementRouterScales,
        HopperGroupedGemmInput::FusedFinalizeEpilogue::StrideRouterScales  //
        >::CollectiveOp;
    ...
    using KernelSchedule = std::conditional_t<IsFP8,
    ...
    using GemmKernel = cutlass::gemm::kernel::
        GemmUniversal<HopperGroupedGemmInput::ProblemShape, CollectiveMainloop, CollectiveEpilogue>;
...
template<typename T,
         ...
         typename ClusterShape,
         bool BIAS>
void sm90_generic_moe_gemm_kernelLauncher(HopperGroupedGemmInput hopper_input,
                                          int                    num_experts,
    ...
    if constexpr (!should_filter_sm90_gemm_problem_shape_v<TileShape, ClusterShape, T>) {
        ...
        MainloopArguments const mainloop_params = {reinterpret_cast<ElementB const**>(hopper_input.ptr_b),
                                                   hopper_input.stride_b,
                                                   reinterpret_cast<ElementA const**>(hopper_input.ptr_a),
                                                   hopper_input.stride_a};
        ...
        auto make_epi_args = [&]() {
            if constexpr (FUSION == EpilogueFusion::NONE) {
                ...
                return EpilogueArguments{epilogue_scalars,
                                         reinterpret_cast<ElementCNoVoid const**>(hopper_input.ptr_c),
                                         hopper_input.stride_c,
                ...
                return EpilogueArguments{
                    ...
                    reinterpret_cast<ElementCNoVoid const**>(hopper_input.ptr_c),
                    hopper_input.stride_c,  // C params
                ...
                static_assert(sizeof(EpilogueArguments) == 0,
                              "Unimplemented fusion provided to SM90+ MoE gemm launcher");
            }
        };
        ...
        typename GemmGrouped::Arguments args{cutlass::gemm::GemmUniversalMode::kGrouped,
                                             hopper_input.shape_info,
                                             mainloop_params,
                                             epilogue_params,
                                             hw_info,
                                             scheduler_args};
        ...
        TLLM_CHECK_WITH_INFO(calculated_ws_size <= hopper_input.gemm_workspace_size,
                             ...
                             calculated_ws_size,
                             hopper_input.gemm_workspace_size);
