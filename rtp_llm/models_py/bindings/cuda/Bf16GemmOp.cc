...
at::Tensor cublas_gemm_fp16_fp16_fp32(const at::Tensor& input, const at::Tensor& weight) {
    TORCH_CHECK(input.is_cuda(), "cublas_gemm_fp16_fp16_fp32: input must be a CUDA tensor");
    TORCH_CHECK(weight.is_cuda(), "cublas_gemm_fp16_fp16_fp32: weight must be a CUDA tensor");
    TORCH_CHECK(input.scalar_type() == at::kHalf, "cublas_gemm_fp16_fp16_fp32: input must be float16");
    TORCH_CHECK(weight.scalar_type() == at::kHalf, "cublas_gemm_fp16_fp16_fp32: weight must be float16");
    TORCH_CHECK(input.dim() == 2 && weight.dim() == 2, "cublas_gemm_fp16_fp16_fp32: input and weight must be 2-D");
    TORCH_CHECK(input.get_device() == weight.get_device(),
                "cublas_gemm_fp16_fp16_fp32: input and weight must be on the same CUDA device");
    TORCH_CHECK(input.size(1) == weight.size(1), "cublas_gemm_fp16_fp16_fp32: inner dimensions must match");
    ...
    const int64_t int_max = static_cast<int64_t>(std::numeric_limits<int>::max());
    TORCH_CHECK(M <= int_max && N <= int_max && K <= int_max,
                "cublas_gemm_fp16_fp16_fp32: dimensions exceed cuBLAS int32 limit");
