...
nvinfer1::DataType nvinfer1DtypeConvert(at::ScalarType dtype) {
    switch (dtype) {
        case at::kHalf : return nvinfer1::DataType::kHALF;
        case at::kBFloat16 : return nvinfer1::DataType::kBF16;
        case at::kFloat : return nvinfer1::DataType::kFLOAT;
        case at::kChar : return nvinfer1::DataType::kINT8;
        default: throw std::runtime_error("Unimplemented dtype for nvinfer1DtypeConvert");
    }
    return nvinfer1::DataType::kFLOAT;
}

FusedMoEOp::FusedMoEOp(const GptInitParameter& gpt_init_parameter): 
    configs_(gpt_init_parameter),
    moe_plugin_(std::make_unique<trt_plugins::MixtureOfExpertsPlugin>()) {}
void FusedMoEOp::forward(
    torch::Tensor hidden_states, torch::Tensor up_proj, torch::Tensor down_proj, 
    torch::Tensor expert_scales, torch::Tensor expert_ids, torch::Tensor outputs)
{
    ...
    const auto new_worksapce = torch::zeros({static_cast<int64_t>(new_ws_size)}, hidden_states.options().dtype(torch::kUInt8));
    auto       fc2_result    = torch::zeros({token_num, top_k, hidden_dim}, hidden_states.options().dtype(hidden_states.dtype()));
    const auto new_expanded_source_row_to_dest = torch::zeros({top_k, token_num}, hidden_states.options().dtype(torch::kInt32));
