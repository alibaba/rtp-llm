...
public:
    FlashInferPrefillOp(const ModelConfig& model_config, const ParallelismConfig& parallelism_config);
...
public:
    FlashInferDecodeOp(const ModelConfig& model_config, const ParallelismConfig& parallelism_config);
    bool          support(torch_ext::PyAttentionInputs attn_inputs);
    ParamsBasePtr prepare(torch_ext::PyAttentionInputs attn_inputs);
