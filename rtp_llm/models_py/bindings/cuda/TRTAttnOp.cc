...
torch::Tensor TRTPagedPrefillOp::forward(const torch::Tensor&                   input,
    ...
    // The paged FMHA runner is built with dataType == dataTypeOut == E4M3 when
    // KV cache is FP8, so it reads FP8 query and writes FP8 output. Mirror the
    // normal path conversion; otherwise the kernel reinterprets BF16/FP16 bytes.
    torch::Tensor tmp_fmha_input;
    torch::Tensor tmp_fmha_output;
