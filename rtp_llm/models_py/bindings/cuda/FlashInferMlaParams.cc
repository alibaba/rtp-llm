...
void FlashInferMlaAttnParams::fillPrefillParamsInternal(torch::Tensor t_input_lengths,
    ...
    for (int i = 0; i < batch_size; i++) {
        ...
        int prefix_length = prefix_lengths ? prefix_lengths[i] : 0;
        ...
        int seq_len = input_length + prefix_length;
        max_q_len   = std::max(max_q_len, input_length);
        accu_q_len += input_length;
        accu_kv_len += seq_len;

        int reuse_page_num_for_batch = (prefix_length + seq_size_per_block - 1) / seq_size_per_block;
        if (kv_cache_block_id) {
            RTP_LLM_CHECK_WITH_INFO(reuse_page_idx + reuse_page_num_for_batch <= max_reuse_page_num_,
                                    "reuse_page_num exceed reserved %d > %d",
                                    reuse_page_idx + reuse_page_num_for_batch,
                                    max_reuse_page_num_);
            for (int j = 0; j < reuse_page_num_for_batch; j++) {
        ...
        if (prefix_length) {
            RTP_LLM_CHECK_WITH_INFO(i < max_batch_size_, "batch_index exceed reserved %d >= %d", i, max_batch_size_);
            ...
            batch_reuse_info_vec_ptr[i * 4 + 3] = reuse_page_num_for_batch;
            batch_start_idx += reuse_page_num_for_batch;
        } else {
            RTP_LLM_CHECK_WITH_INFO(i < max_batch_size_, "batch_index exceed reserved %d >= %d", i, max_batch_size_);
            // batch_reuse_info_vec is 2D: [batch_size, 4]
            batch_reuse_info_vec_ptr[i * 4]     = i;
...
void FlashInferMlaAttnParams::fillDecodeParamsInternal(torch::Tensor t_sequence_lengths,
    ...
    for (int i = 0; i < batch_size; i++) {
        ...
        int seq_len         = sequence_lengths[i] + 1;
        accu_q_len += 1;
        accu_kv_len += 1;
...
void FlashInferMlaAttnParams::refreshBuffer(
    ...
    // Update shape for batch_reuse_info_vec (only for prefill mode)
    if (batch_reuse_info_size > 0) {
        shape = {batch_size, 4};
...
void FlashInferMlaAttnParams::fillPrefillParams(torch::Tensor t_input_lengths,
                                                torch::Tensor t_prefix_lengths,
                                                torch::Tensor t_kv_cache_block_id_host,
                                                int           batch_size,
                                                int           seq_size_per_block) {
    const int actual_batch_size = t_input_lengths.size(0);
    ...
    int batch_reuse_info_size = actual_batch_size * 4;  // 4 ints per batch entry

    for (int i = 0; i < actual_batch_size; i++) {
        int input_length  = input_lengths_ptr[i];
        int prefix_length = prefix_lengths_ptr ? prefix_lengths_ptr[i] : 0;
    ...
    ensureTensorSize(actual_batch_size, input_token_num, page_num, reuse_page_num, batch_reuse_info_size);

    // Fill params directly into HOST tensors
    fillPrefillParamsInternal(t_input_lengths,
                              t_prefix_lengths,
    ...
    refreshBuffer(actual_batch_size, input_token_num, page_num, reuse_page_num, batch_reuse_info_size);
}

void FlashInferMlaAttnParams::fillDecodeParams(torch::Tensor t_sequence_lengths,
                                               torch::Tensor t_kv_cache_block_id_host,
                                               int           batch_size,
                                               int           seq_size_per_block) {
    const int actual_batch_size = batch_size;
    ...
    int input_token_num       = actual_batch_size;  // Decode mode: 1 token per batch
    int page_num              = 0;
    int reuse_page_num        = 0;
    int batch_reuse_info_size = 0;  // Not used in decode mode

    for (int i = 0; i < actual_batch_size; i++) {
        int seq_len = sequence_lengths_ptr[i] + 1;
        page_num += (seq_len + seq_size_per_block - 1) / seq_size_per_block;
    }
    ...
    // Fill params directly into HOST tensors
    fillDecodeParamsInternal(t_sequence_lengths,
                             t_kv_cache_block_id_host,
                             actual_batch_size,
                             seq_size_per_block,
                             input_token_num,
...
void FlashInferMlaAttnParams::fillParams(torch::Tensor sequence_lengths,
                                         torch::Tensor input_lengths,
                                         torch::Tensor kv_cache_block_id_host,
                                         int           batch_size,
                                         int           seq_size_per_block,
                                         torch::Tensor prefix_lengths) {
    fillDecodeParams(sequence_lengths, kv_cache_block_id_host, batch_size, seq_size_per_block);
}
...
void registerPyFlashInferMlaParams(pybind11::module& m) {
    ...
    // Prefill mode function
    m.def(
        "fill_prefill_mla_params",
        [](torch::Tensor t_input_lengths,
           ...
           int           seq_size_per_block) {
            ...
            params->fillPrefillParams(
                t_input_lengths, t_prefix_lengths, t_kv_cache_block_id_host, batch_size, seq_size_per_block);
            return params;
        },
    ...
    m.def(
        ...
        [](torch::Tensor t_sequence_lengths,
           ...
           int           seq_size_per_block) {
            auto params = std::make_shared<rtp_llm::FlashInferMlaAttnParams>();
            params->fillDecodeParams(t_sequence_lengths, t_kv_cache_block_id_host, batch_size, seq_size_per_block);
            return params;
        },
