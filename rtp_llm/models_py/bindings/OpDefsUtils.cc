...
void getPaddingOffset(
    int32_t* padding_offset, int32_t* input_lengths, int32_t batch_size, int32_t max_seq_len) {
    // do cumulated sum
...
void calculatePaddingOffset(torch_ext::PyAttentionInputs& py_attn_inputs) {
    ...
    // decode not need padding offset
    if (py_attn_inputs.sequence_lengths.defined() && py_attn_inputs.sequence_lengths.size(0) > 0) {
        return ;
    }
