...
// Stored after the byte-exact CPU payload. Exactly one CRC32C protects the payload.
struct CrcBlockFooter {
    uint32_t crc32c{0};
};
static_assert(sizeof(CrcBlockFooter) == 4);

// CPU-only provenance, written after D2H completes and before cache publication.
// This is outside the CRC/transfer range; block starts remain 16-byte aligned.
struct CrcBlockHostMetadata {
    int64_t writer_request_id{0};
};
...
public:
    ...
    static size_t transferBytes(size_t payload_bytes);
