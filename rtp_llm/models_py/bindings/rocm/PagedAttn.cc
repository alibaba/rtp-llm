...
#include "rtp_llm/cpp/devices/rocm_impl/ROCmDevice.h"
namespace rtp_llm{
PagedAttnDecodeOp::PagedAttnDecodeOp(
    const GptInitParameter& gpt_init_parameter):
     FMHARocmBase(gpt_init_parameter), kv_block_offset_(gpt_init_parameter.num_layers_ * gpt_init_parameter.block_nums_)
{
...
forward_param PagedAttnDecodeOp::forward(
    ...
    if (bool(autil::EnvUtil::getEnv("USE_AITER_PA", 0L))) {
        ...
        if (params->prefix_lengths.defined() && params->prefix_lengths.numel() > 0) {
            prefix_prompt_param.d_prefix_prompt_lengths =
                params->prefix_lengths.data_ptr<int>();
            prefix_prompt_param.max_prefix_prompt_length =
                params->prefix_lengths.max().item<int>();
...
// token_num, size_per_head
        ...
        BufferPtr exp_sums_buffer = device_->allocateBuffer({rtp_llm::DataType::TYPE_FP32,
                                        {num_seqs, num_heads, max_num_partitions},
                                        AllocationType::DEVICE}, {"exp_sums"});
        auto exp_sums = Buffer2torchTensor(exp_sums_buffer,false);

        BufferPtr max_logits_buffer = device_->allocateBuffer({rtp_llm::DataType::TYPE_FP32,
                                        {num_seqs, num_heads, max_num_partitions},
                                        AllocationType::DEVICE}, {"max_logits"});
        auto max_logits = Buffer2torchTensor(max_logits_buffer,false);

        BufferPtr tmp_out_buffer = device_->allocateBuffer({datatype,
        ...
        return {out, exp_sums, max_logits,
                tmp_out, query,
                num_kv_heads, scale,
                context_lens,
...
void registerPagedAttnDecodeOp(py::module& m) {
    py::class_<PagedAttnDecodeOp>(m, "PagedAttnDecodeOp")
        .def(py::init<GptInitParameter>(),
             py::arg("device_init_params"))
    ...
    // // Register CKAttnParams struct
    // py::class_<CKAttn, std::shared_ptr<CKAttn>>(m, "CKAttnParams")
    //     .def_readwrite("local_head_num", &CKAttnParams::local_head_num)
    //     .def_readwrite("size_per_head", &CKAttnParams::size_per_head)
    //     .def_readwrite("kv_head_num", &CKAttnParams::kv_head_num)
    //     .def_readwrite("max_blocks_per_batch", &CKAttnParams::max_blocks_per_batch)
    //     .def_readwrite("decoder_batch_size", &CKAttnParams::decoder_batch_size)
    //     .def_readwrite("max_seq_len", &CKAttnParams::max_seq_len)
    //     .def_readwrite("max_prefix_length", &CKAttnParams::max_prefix_length)
    //     .def_readwrite("stream_", &CKAttnParams::stream_)
    //     .def_readwrite("sequence_lengths", &CKAttnParams::sequence_lengths)
    //     .def_readwrite("prefix_prompt_lengths", &CKAttnParams::prefix_prompt_lengths)
    //     .def_readwrite("output", &CKAttnParams::output)
    //     .def_readwrite("cu_seqlens", &CKAttnParams::cu_seqlens)
    //     .def_readwrite("cu_kv_seqlens", &CKAttnParams::cu_kv_seqlens)
    //     .def_readwrite("decode_plan", &CKAttnParams::decode_plan)
    //     .def_readwrite("attn_type", &CKAttnParams::attn_type)
    //     .def_readwrite("kv_block_array", &CKAttnParams::kv_block_array);
    }
}