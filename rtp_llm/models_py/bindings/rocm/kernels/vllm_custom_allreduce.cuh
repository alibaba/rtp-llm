...
public:
     ...
     */
    CustomAllreduce(
        Signal** signals, void* rank_data, size_t rank_data_sz, int rank, int world_size, bool fully_connected = true):
        rank_(rank),
    ...
    std::pair<std::string, std::vector<int64_t>> get_graph_buffer_ipc_meta() {
        ...
        for (int i = 0; i < num_buffers; i++) {
            ...
            // address
#if defined(USE_ROCM)
            if (hipPointerGetAttribute(&base_ptr, rangeStartAddrAttr, reinterpret_cast<hipDeviceptr_t>(ptr))
                != hipSuccess)
#else
    ...
    template<typename T>
    void allreduce(
        cudaStream_t stream, T* input, T* output, int size, int threads = 512, int block_limit = defaultBlockLimit) {
        ...
        if (env_algo != nullptr) {
            if (std::strcmp(env_algo, "1stage") == 0 || std::strcmp(env_algo, "oneshot") == 0) {
                ...
                throw std::runtime_error("Invalid VLLM_CUSTOM_ALLREDUCE_ALGO: " + std::string(env_algo)
...
#define REDUCE_CASE(ngpus)                                                                                             \
    case ngpus: {                                                                                                      \
        ...
        switch (world_size_) {
            ...
            REDUCE_CASE(8)
            default:
                throw std::runtime_error("custom allreduce only supports num gpus in (2,4,6,8). Actual "
                                         "num "
                                         "gpus = "
