...
__global__ __quickreduce_launch_bounds_two_shot__ static void allreduce_prototype_twoshot(T const*  A,
                                                                                          T*        B,
                                                                                          uint32_t  N,
                                                                                          uint32_t  num_blocks,
                                                                                          int       rank,
...
struct DeviceComms {
        ...
        for (int i = 0; i < world_size; i++) {
            if (i != rank) {
                HIP_CHECK(hipIpcOpenMemHandle(
                    (void**)&buffer_list[i], all_buffer_ipc_handles[i], hipIpcMemLazyEnablePeerAccess));
            } else {
    ...
    void allreduce(T const* A, T* B, uint32_t N, int quant_level, hipStream_t stream) {
        ...
        uint32_t grid         = std::min<uint32_t>(kMaxNumBlocks, num_blocks);
