...
#pragma unroll
    ...
    if (threadIdx.x < WARP_SIZE) {
        float warp_exclusive_prefix_sum = (threadIdx.x < BLOCK_THREADS / WARP_SIZE) ? smem_prefix_sum[threadIdx.x] : 0;
...
#pragma unroll
    ...
    if (aggregate + aggregate_local > u) {
        if constexpr (DETERMINISTIC) {
            ...
            // Finish all warp-prefix reads before adjacent difference reuses block_prim.
            __syncthreads();
        } else {
    ...
    DISPATCH_COMPUTE_CAP_NUM_THREADS(compute_capacity, BLOCK_THREADS, {
        ...
        DISPATCH_ALIGNED_VEC_SIZE(vec_size, VEC_SIZE, {DISPATCH_DETERMINISTIC(deterministic, DETERMINISTIC, {
                                      auto kernel = TopKSamplingFromProbKernel<BLOCK_THREADS,
                                                                               ...
                                                                               T,
                                                                               IdType>;
                                      CUDA_CALL(hipFuncSetAttribute(reinterpret_cast<const void*>(kernel),
...
template<typename T, typename IdType>
hipError_t TopPSamplingFromProb(T*          probs,
                                IdType*     output,
                                IdType*     indices,
                                T*          top_p_arr,
                                uint32_t    batch_size,
...
template<typename T, typename IdType>
hipError_t TopKTopPSamplingFromProb(T*          probs,
                                    IdType*     top_k_arr,
                                    T*          top_p_arr,
                                    IdType*     output,
                                    IdType*     indices,
...
template<uint32_t             BLOCK_THREADS,
         ...
         typename IdType>
__global__ void
TopKRenormProbKernel(DType* probs, DType* renormed_prob, IdType* top_k_arr, uint32_t top_k_val, uint32_t d) {
    const uint32_t         bx = blockIdx.x, tx = threadIdx.x;
    const uint32_t         row_idx = bx;
...
hipError_t TopKRenormProb(DType*      probs,
                          DType*      renormed_prob,
                          IdType*     top_k_arr,
                          uint32_t    batch_size,
                          uint32_t    top_k_val,
