...
#define FLASHINFER_CUDA_CALL(func, ...)                                                                                \
    {                                                                                                                  \
        hipError_t e = (func);                                                                                         \
        if (e != hipSuccess) {                                                                                         \
            std::cerr << "CUDA Error: " << hipGetErrorString(e) << " (" << e << ") " << __FILE__ << ": line "          \
...
__device__ __forceinline__ uint32_t dim2_offset(const uint32_t& dim_a, const uint32_t& idx_b, const uint32_t& idx_a) {
    return idx_b * dim_a + idx_a;
}

__device__ __forceinline__ uint32_t dim3_offset(
