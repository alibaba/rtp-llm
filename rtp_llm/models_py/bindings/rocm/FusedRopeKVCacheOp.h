...
public:
    ...
    torch::Tensor
    forward(const torch::Tensor& qkv, FMHAType fmha_type, std::optional<torch_ext::KVCache> kv_cache_base, const CKAttnPtr& params);
};
...
public:
    ...
    CKAttnPtr prepare(torch_ext::PyAttentionInputs attn_inputs);
    torch::Tensor
    forward(const torch::Tensor& qkv, FMHAType fmha_type, const torch::Tensor& kv_cache_base, const CKAttnPtr& params);
};
