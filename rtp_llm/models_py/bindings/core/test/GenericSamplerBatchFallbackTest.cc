...
TEST(GenericSamplerBatchFallbackTest, AppliesNoRepeatBigramBeforeTopKOneShortcut) {
    ...
    auto sequence_lengths = torch::tensor({3}, torch::kInt32);
    ...
    EXPECT_EQ(logits[0][2].item<float>(), -std::numeric_limits<float>::infinity());
}

TEST(GenericSamplerBatchFallbackTest, NoRepeatNgramUsesEachRowsValidHistory) {
    constexpr int64_t batch_size = 4;
    auto              logits     = torch::tensor({{0.0f, 1.0f, 4.0f, 3.0f}}, torch::kFloat32).repeat({batch_size, 1});
    // Padding deliberately contains valid tokens that would change the bans if included in the history.
    auto token_ids        = torch::tensor({{1, 2, 1, 0}, {1, 2, 1, 0}, {2, 2, 2, 0}, {2, 2, 2, 0}}, torch::kInt32);
    auto original_history = token_ids.slice(1, 0, 3).clone();

    runSample(logits,
              token_ids,
              torch::zeros({batch_size}, torch::kInt32),
              torch::tensor({3, 2, 1, 0}, torch::kInt32),
              torch::ones({batch_size}, torch::kInt32),
              torch::ones({batch_size}, torch::kFloat32),
              torch::ones({batch_size}, torch::kFloat32),
              torch::tensor({2, 2, 2, 1}, torch::kInt32));

    EXPECT_TRUE(torch::equal(token_ids.select(1, 3), torch::tensor({3, 2, 2, 2}, torch::kInt32)));
    EXPECT_TRUE(torch::equal(token_ids.slice(1, 0, 3), original_history));
    EXPECT_EQ(logits[0][2].item<float>(), -std::numeric_limits<float>::infinity());
    for (int64_t row = 1; row < batch_size; ++row) {
        EXPECT_FLOAT_EQ(logits[row][2].item<float>(), 4.0f);
    }
}

TEST(GenericSamplerBatchFallbackTest, NoRepeatNgramRejectsLengthsOutsideHistory) {
    // A width of four reserves the last column for output, so at most three history tokens are valid.
    for (int32_t length : {-1, 4, 5}) {
        SCOPED_TRACE(length);
        auto logits    = torch::tensor({{0.0f, 1.0f, 4.0f, 3.0f}}, torch::kFloat32);
        auto token_ids = torch::tensor({{1, 2, 1, 0}}, torch::kInt32);
        EXPECT_THROW(runSample(logits,
                               token_ids,
                               torch::zeros({1}, torch::kInt32),
                               torch::tensor({length}, torch::kInt32),
                               torch::ones({1}, torch::kInt32),
                               torch::ones({1}, torch::kFloat32),
                               torch::ones({1}, torch::kFloat32),
                               torch::tensor({2}, torch::kInt32)),
