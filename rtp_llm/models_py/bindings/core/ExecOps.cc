...
void runtimeWriteCacheStore(const CacheStoreInputs&     cache_store_inputs,
    ...
    for (size_t batch_id = 0; batch_id < param.context_batch_size; batch_id++) {
        ...
        if (pdDebugEnabled()) {
            ...
            const int         first_index = block_positions.empty() ? -1 : static_cast<int>(block_positions.front());
            const int         last_index  = block_positions.empty() ? -1 : static_cast<int>(block_positions.back());
            const std::string first_cache_key =
                first_index >= 0 ? param.cache_keys[batch_id * max_blocks_per_batch + first_index] : "";
            const std::string last_cache_key =
                last_index >= 0 ? param.cache_keys[batch_id * max_blocks_per_batch + last_index] : "";
            RTP_LLM_LOG_INFO(
                ...
                "group_type=%d first_index=%d last_index=%d first_cache_key=%s last_cache_key=%s "
                "first_block_id=%ld last_block_id=%ld opaque_or_mla=%d",
                ...
                static_cast<long>(blockIdAt(first_index)),
                static_cast<long>(blockIdAt(last_index)),
