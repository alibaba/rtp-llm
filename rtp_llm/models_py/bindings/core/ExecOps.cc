...
void runtimeWriteCacheStore(const torch_ext::PyCacheStoreInputs& cache_store_inputs,
    ...
    for (size_t batch_id = 0; batch_id < context_batch_size; ++batch_id) {
        ...
        if (request_blocks->getBlocksCount() > 0) {
            // Direct callers retain the synchronous publication contract. The
            // async writer supplies a registrar and drains at the consumer boundary.
            if (!register_store_completion) {
                auto store_context = cache_store->storeBuffers({request_blocks}, 5000);
                if (store_context) {
