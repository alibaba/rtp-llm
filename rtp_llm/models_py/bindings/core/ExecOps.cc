    ...
    for (size_t batch_id = 0; batch_id < param.context_batch_size; batch_id++) {
        ...
        if (request_blocks->getBlocksCount() > 0) {
            if (param.wait_cache_store_done) {
                RTP_LLM_CHECK_WITH_INFO(param.cache_store_wait_timeout_ms > 0,
                                        "chunk Prefill cache-store wait timeout must be positive: request=%ld layer=%d "
                                        "timeout_ms=%ld",
                                        request_id,
                                        param.layer_id,
                                        param.cache_store_wait_timeout_ms);
                RTP_LLM_LOG_DEBUG("chunk Prefill cache-store staging wait: request=%ld layer=%d timeout_ms=%ld",
                                  request_id,
                                  param.layer_id,
                                  param.cache_store_wait_timeout_ms);
                auto store_context =
                    cache_store->storeBuffers({request_blocks}, param.cache_store_wait_timeout_ms);
