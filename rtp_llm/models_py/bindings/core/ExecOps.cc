...
void runtimeWriteCacheStore(const torch_ext::PyCacheStoreInputs& cache_store_inputs,
    ...
    RTP_LLM_CHECK_WITH_INFO(context_batch_size > 0, "cache-store write requires at least one request_id");
    ...
    for (size_t batch_id = 0; batch_id < context_batch_size; ++batch_id) {
        ...
        auto addBlock = [&](int key_index, int offset_index) {
            ...
            // KVCacheSpecType is the shared source of truth for prefill key prefixes and
            // decode-side block registration. New cache spec types must update both sides.
