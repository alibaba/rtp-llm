...
// One request's token span inside a Python-planned Prefill round, mirrored
// into C++ by the MTP chunk-prefill hook so the executor can materialize round inputs.
struct KimiK3ChunkSlice {
...
struct KimiK3ChunkRound {
    std::vector<KimiK3ChunkSlice> slices;
...
struct GptModelInputs {
    ...
    // Optional block-aligned transfer span for cache store. Chunked MTP
    // Prefill can execute attention for a terminal singleton while publishing
    // the whole physical block containing that token.
    torch::Tensor         cache_store_input_lengths_host;
    torch::Tensor         cache_store_prefix_lengths_host;
