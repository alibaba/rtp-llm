...
def declared_indexer_cache_mode(config):
    ...
    if impl_id is not None:
        state_format = (
            get_module_registry()
            .implementation("rtp.dsv4.model", impl_id)
            .state_format_id
        )
    modes = {
        STATE_FORMAT: Dsv4IndexerCacheMode.FP8,
        STATE_FORMAT_FP4: Dsv4IndexerCacheMode.FP4,
    }
    if state_format not in modes:
        raise ValueError(
            f"Undeclared DeepSeek-V4 allocator state format: {state_format}"
        )
    return modes[state_format]
    ...
    for kind, contract in CONTRACTS.items():
        ...
        registry.register_module(
            ModuleSpec(
                ...
                contract,
                methods,
                "rtp_llm.models_py.pluggable.dsv4_builders:validate_instance",
            )
        )
        ...
        registry.register_implementation(
            ModuleImplSpec(
                ...
                api_version=1,
                builder=f"rtp_llm.models_py.pluggable.dsv4_builders:build_{kind}",
                supported_devices={DeviceType.Cuda},
                predicate="rtp_llm.models_py.pluggable.dsv4_specs:supports_baseline",
                priority=0,
