...
def _build_fused_qk_rope_quant_custom_op():
    ...
    try:
        ...
        logger.warning("GraphFX fused_qk_rope_quant custom op build failed: %s", exc)
        return None
    ...
    try:
        ...
        def _compile_friendly(hidden_states, residual, weight, eps, stream_id):
            if _is_fake_or_meta_tensor(hidden_states) or _is_fake_or_meta_tensor(
                ...
                return None
            # Always resolve stream internally — callers pass 0 to avoid
            # Dynamo graph-breaks from torch.cuda.current_stream().cuda_stream.
            real_stream = torch.cuda.current_stream().cuda_stream
            return custom_op(
                hidden_states, residual, weight, float(eps), int(real_stream)
            )
        ...
        # Patch RMSResNorm.forward to call the custom_op directly with
        # stream_id=0.  The custom_op has proper mutates_args registration
        # so Dynamo tracks in-place mutations correctly (unlike allow_in_graph
        # on _compile_friendly which doesn't communicate mutation semantics).
        # stream_id=0 is resolved inside _mutating_impl at runtime.
        try:
            from rtp_llm.models_py.modules.base.cuda.norm import RMSResNorm

            _fused_add_rmsnorm_op = custom_op

            def _dynamo_safe_rms_res_norm_forward(self, hidden_states, residual):
                _fused_add_rmsnorm_op(
                    hidden_states,
                    residual,
                    self.weight.data,
            ...
            RMSResNorm.forward = _dynamo_safe_rms_res_norm_forward
        except Exception:
            pass
...
def install_compile_friendly_rmsnorm_wrapper() -> None:
    ...
    try:
        ...
        def _compile_friendly(output, hidden_states, weight, eps, stream_id):
            ...
            real_stream = torch.cuda.current_stream().cuda_stream
            return custom_op(
                output, hidden_states, weight, float(eps), int(real_stream)
            )
        ...
        # Patch RMSNorm.forward to pass stream_id=0 (same reasoning as
        # RMSResNorm above — avoid graph-breaking stream_id computation).
        try:
            from rtp_llm.models_py.modules.base.cuda.norm import RMSNorm

            def _dynamo_safe_rmsnorm_forward(self, hidden_states, output=None):
                if output is None:
            ...
            RMSNorm.forward = _dynamo_safe_rmsnorm_forward
        except Exception:
            pass
...
def install_compile_friendly_silu_and_mul_wrapper() -> None:
    ...
    try:
        ...
        if original is None:
            logger.info("GraphFX: rtp_llm_ops.silu_and_mul not present, skip wrapper")
            return
        if getattr(original, "_graphfx_wrapped", False):
        ...
        try:
            ...
            logger.info("GraphFX: could not patch FusedSiluAndMul.forward: %s", exc)

        _dynamo_allow_in_graph(custom_op)
        _SILU_AND_MUL_WRAPPER_INSTALLED = True
        logger.info("GraphFX installed compile-friendly silu_and_mul wrapper")
    except Exception as exc:
        logger.warning("GraphFX: compile-friendly silu_and_mul install failed: %s", exc)
...
def install_graphfx_compile_disable_wrappers() -> None:
    ...
    # ---- FP8 DeepGEMM: compile-disable the GEMM call only, NOT the full forward.
    # The forward contains sgl_per_token_group_quant_fp8 which must stay visible
    # in the FX graph so add_rmsnorm_fp8_quant_fx can match the
    # fused_add_rmsnorm_mutating → quant pattern.
    installed += _install_compile_disable_targets(
    ...
    # Also patch the imported reference in the linear module itself (the
    # from-import copies the original function object into the module namespace).
    installed += _install_compile_disable_targets(
...
def _allow_graphfx_fusion_candidates_in_graph() -> None:
    ...
    try:
        ...
        from rtp_llm.models_py.triton_kernels.common.fused_add_rmsnorm_fp8_quant import (
            ...
            fused_add_rmsnorm_fp8_quant_inplace,
...
_MUTATING_LEAF_TARGETS = {
    ...
    "rtp_llm_graphfx::fused_add_rmsnorm_mutating",
    "fused_add_rmsnorm_fp8_quant_producer",
...
def eliminate_dead_code_preserving_graphfx_side_effects(gm: object) -> None:
    ...
    def is_impure_node(node: object) -> bool:
        ...
        # In-place methods (copy_, add_, etc.) are side-effectful mutations.
        if op == "call_method" and target_name.endswith("_"):
            return True
