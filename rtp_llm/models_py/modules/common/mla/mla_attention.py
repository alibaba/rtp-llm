...
class MlaAttention(nn.Module):
    ...
    def __init__(
        ...
        self.num_heads = config.attn_config.head_num // parallelism_config.tp_size
        self.qk_nope_head_dim = config.attn_config.nope_head_dim
        self.qk_rope_head_dim = config.attn_config.rope_head_dim
        self.q_head_dim = self.qk_nope_head_dim + self.qk_rope_head_dim
        self.kv_lora_rank = config.attn_config.kv_lora_rank
        self.v_head_dim = config.attn_config.v_head_dim
        self.q_lora_rank = config.attn_config.q_lora_rank
        self.softmax_scale = self.q_head_dim ** (-0.5)
        self.layer_idx = layer_idx
        self.token_per_block = config.attn_config.tokens_per_block
    ...
    def forward(
        ...
        if self.q_lora_rank > 0:
            ...
            kv_offset = self.num_heads * self.config.attn_config.size_per_head
            q, compressed_kv = torch.split(
                fused_qkv,
                [
                    kv_offset,
