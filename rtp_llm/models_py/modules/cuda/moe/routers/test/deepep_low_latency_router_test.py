...
from rtp_llm.distribute.collective import get_ep_group
...
def _init_router(rank: int, use_fp8: bool, parallelism_config: ParallelismConfig, nccl_port: int):
    ...
    init_distributed_environment(parallelism_config=config.parallelism_config, backend="nccl", timeout=60)
...
def _spawn_wrapper(rank: int, use_fp8: bool, world_size: int, test_tp_size: int, nccl_port: int):
    ...
    parallelism_config.tp_rank = 0  # tp_rank is 0 for low latency router
