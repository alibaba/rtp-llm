...
class TrtllmDistEnv:
    ...
    def __init__(
        ...
        # Phase 2: IPC handle exchange + consensus
        phase2_ok = False
        try:
            self._barrier()
            bh = [None] * self.world_size
            dh = [None] * self.world_size
            dist.all_gather_object(bh, self.handle.get_barrier_handle(), group=self.group)
            dist.all_gather_object(dh, self.handle.get_data_handle(), group=self.group)
