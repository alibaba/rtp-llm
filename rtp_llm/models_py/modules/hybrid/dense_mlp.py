    ...
    def forward(
        ...
        *,
        ...
        if sequence_parallel:
            raise RuntimeError(
                "DenseMLP sequence-parallel collectives must be orchestrated "
                "outside the MLP"
            )

        output = self._forward_local(x)
        if reduce_output and self.parallelism_config.get_ffn_tp_size() > 1:
            output = all_reduce(output, group=Group.TP)
        return output

    def _forward_local(self, x: torch.Tensor) -> torch.Tensor:
        """Return this rank's down-projection partial without a collective."""

        gate = self.gate_proj(x) if self.is_gated and not self.merge_gate_up else None
        return self.down_proj(self._activate(gate, self.up_proj(x)))
