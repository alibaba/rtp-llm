...
class KimiK3Fp8WeightTest(unittest.TestCase):
    ...
    def test_kvb_derived_weights_use_final_quantized_values(self):
        ...
        for rank in range(8):
            load = SimpleNamespace(tp_size=8, tp_rank=rank)
            ...
            local = dense[rank * 256 : (rank + 1) * 256]
            torch.testing.assert_close(result[W.mla_kc][0], local[:128], rtol=0, atol=0)
            torch.testing.assert_close(
                result[W.mla_vc][0], local[128:].T, rtol=0, atol=0
            )
