...
Adapted for the released GLM-5.3-Flash shapes:
  - hidden_size: 4096
...
from .mega_buf import get_or_create_mega_buf, get_or_create_mega_output
...
class GLM5MegaMoE(nn.Module):
    ...
    def from_params(
        ...
        if dim <= 0 or dim % 128:
            raise ValueError(f"dim must be positive and divisible by 128, got {dim}")
        if moe_inter_dim <= 0 or moe_inter_dim % FP4_BLOCK:
            raise ValueError(
                "moe_inter_dim must be positive and divisible by "
                f"{FP4_BLOCK}, got {moe_inter_dim}"
            )
        if ep_size <= 0:
            raise ValueError(f"ep_size must be positive, got {ep_size}")
        if n_routed_experts <= 0 or n_routed_experts % ep_size:
            raise ValueError(
                f"n_routed_experts {n_routed_experts} must be positive and "
                f"divisible by ep_size {ep_size}"
            )
        if ep_rank < 0 or ep_rank >= ep_size:
            raise ValueError(f"ep_rank must be in [0, {ep_size}), got {ep_rank}")
        if n_activated_experts <= 0 or n_activated_experts > n_routed_experts:
            raise ValueError(
                "n_activated_experts must be in "
                f"[1, {n_routed_experts}], got {n_activated_experts}"
            )
    ...
    def setup_weights_from_fp4(
        ...
        expected_shapes = {
            "w1_w": (E, 2 * inter, D // 2),
            "w1_s": (E, 2 * inter, D // FP4_BLOCK),
            "w2_w": (E, D, inter // 2),
            "w2_s": (E, D, inter // FP4_BLOCK),
        }
        tensors = {"w1_w": w1_w, "w1_s": w1_s, "w2_w": w2_w, "w2_s": w2_s}
        for name, expected_shape in expected_shapes.items():
            actual_shape = tuple(tensors[name].shape)
            if actual_shape != expected_shape:
                raise ValueError(
                    f"{name} shape {actual_shape} does not match {expected_shape}"
                )
        if w1_w.dtype != torch.int8 or w2_w.dtype != torch.int8:
            raise TypeError(
                f"FP4 packed weights must be int8, got {w1_w.dtype} and {w2_w.dtype}"
            )
    ...
    def setup_weights_from_fp8(
        ...
        tensors = {"w1_fp8": w1_fp8, "w2_fp8": w2_fp8, "w3_fp8": w3_fp8}
        for name, expected_shape in expected_shapes.items():
            actual_shape = tuple(tensors[name].shape)
            if actual_shape != expected_shape:
                raise ValueError(
        ...
        if any(tensor.dtype != expected_dtype for tensor in tensors.values()):
            raise TypeError(
                "FP8 MegaMoE weights must use float8_e4m3fn, got "
                + ", ".join(
                    f"{name}={tensor.dtype}" for name, tensor in tensors.items()
                )
            )
