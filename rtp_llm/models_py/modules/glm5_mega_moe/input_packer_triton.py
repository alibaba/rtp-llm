...
def fused_pack_mega_moe_inputs(
    ...
    if weights.dim() != 2 or indices.dim() != 2:
        raise ValueError(
            "weights and indices must both be [T,topk], got "
            f"weights={tuple(weights.shape)} indices={tuple(indices.shape)}"
        )
    if weights.shape != indices.shape or weights.shape[0] != x.shape[0]:
        raise ValueError(
            "router tensors must share [T,topk] and match x rows, got "
            f"x={tuple(x.shape)} weights={tuple(weights.shape)} "
            f"indices={tuple(indices.shape)}"
        )
    if weights.shape[1] <= 0:
        raise ValueError("router topk must be positive")
    outputs = (out_fp8, out_sf, out_indices, out_weights)
    if any(output.dim() != 2 for output in outputs):
        raise ValueError(
            "all fused MegaMoE output buffers must be 2D, got "
            + ", ".join(str(tuple(output.shape)) for output in outputs)
        )
    if triton is None:
        raise RuntimeError("triton is unavailable")
    if not x.is_cuda:
        raise RuntimeError("fused MegaMoE input packer requires CUDA tensors")
    tensors = (x, weights, indices, out_fp8, out_sf, out_indices, out_weights)
    if any(not tensor.is_cuda for tensor in tensors):
        raise RuntimeError("all fused MegaMoE packer tensors must be on CUDA")
    if any(tensor.stride(-1) != 1 for tensor in tensors):
        raise ValueError("all fused MegaMoE packer tensors must be contiguous by row")
    ...
    if out_fp8.shape[0] < T or out_fp8.shape[1] != D:
        raise ValueError(
            f"out_fp8 capacity/width mismatch: {tuple(out_fp8.shape)} for x={tuple(x.shape)}"
        )
    if out_indices.shape[0] < T or out_indices.shape[1] != topk:
        raise ValueError(
            "out_indices capacity/width mismatch: "
            f"{tuple(out_indices.shape)} for router={tuple(indices.shape)}"
        )
    if out_weights.shape[0] < T or out_weights.shape[1] != topk:
        raise ValueError(
            "out_weights capacity/width mismatch: "
            f"{tuple(out_weights.shape)} for router={tuple(weights.shape)}"
        )
    expected_sf_width = D // 128
    if out_sf.shape[0] < T or out_sf.shape[1] != expected_sf_width:
        raise ValueError(
            f"out_sf capacity/width mismatch: {tuple(out_sf.shape)}, "
            f"expected width {expected_sf_width}"
        )
