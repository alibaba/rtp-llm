...
class TestMoeConfigResolver(unittest.TestCase):
    ...
    def test_get_quant_method(self):
        ...
        config = MagicMock()
        config.model_config.quant_config = MagicMock()
        config.model_config.quant_config.get_method.return_value = "FP8_PER_BLOCK"
        self.assertEqual(self.resolver.get_quant_method(config), "FP8_PER_BLOCK")

    def test_is_ep_enabled_false(self):
        """Test EP not enabled"""
        config = MagicMock()
        config.model_config.ep_size = 1
        self.assertFalse(self.resolver.is_ep_enabled(config))

    def test_is_ep_enabled_true(self):
        """Test EP enabled"""
        config = MagicMock()
        config.model_config.ep_size = 4
        self.assertTrue(self.resolver.is_ep_enabled(config))

    def test_use_low_latency_true(self):
        """Test low latency mode"""
        config = MagicMock()
        config.moe_config.use_deepep_low_latency = True
        runtime_config = RuntimeConfig(config, disable_low_latency=False)
        self.assertTrue(self.resolver.use_low_latency(runtime_config))

    def test_use_low_latency_false_by_disable_low_latency(self):
        """Test low latency mode"""
        config = MagicMock()
        config.moe_config.use_deepep_low_latency = True
        runtime_config = RuntimeConfig(config, disable_low_latency=True)
        self.assertFalse(self.resolver.use_low_latency(runtime_config))

    def test_use_low_latency_false_by_use_deepep_low_latency(self):
        """Test low latency mode"""
        config = MagicMock()
