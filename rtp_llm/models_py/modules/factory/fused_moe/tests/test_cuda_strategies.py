...
class TestFusedMoEQuantConfig(unittest.TestCase):
    def test_uint8_without_block_shape_is_not_per_group_fp4(self) -> None:
        config = FusedMoEQuantConfig(quant_dtype=torch.uint8, block_shape=None)
        self.assertFalse(config.is_per_group_fp4)

    def test_nvfp4_block_size_is_per_group_fp4(self) -> None:
        config = FusedMoEQuantConfig(
            quant_dtype=torch.uint8,
            block_shape=[16, 16],
        )
        self.assertTrue(config.is_per_group_fp4)
...
class TestCudaNoQuantSingleGpuStrategy(unittest.TestCase):
    ...
    def test_can_handle_false_explicit_strategy(self) -> None:
        """Generic no-quant fallback must not override an explicit strategy."""
        config = create_moe_config_adapter(
            model_config=create_model_config_without_quant(),
            parallelism_config=create_parallelism_config(
                ep_size=1, tp_size=1, dp_size=1
            ),
            moe_config=create_moe_config(moe_strategy="fp4_b12x"),
        )
...
class TestCudaFp8PerBlockNoDPStrategy(_Sm9xStrategyTestCase):
    ...
    def test_can_handle_cuda_graph(
...
class TestCudaFp4StrategySelection(unittest.TestCase):
    ...
    def _make_config(
        ...
        *,
        ...
        elif topology == "tp_only":
            moe_config = create_moe_config(use_all_gather=True)
            parallelism_config = create_parallelism_config(
                ep_size=1, tp_size=2, dp_size=1
            )
        elif topology == "b12x_cp":
            moe_config = create_moe_config(use_all_gather=True)
            parallelism_config = create_parallelism_config(
                ep_size=1, tp_size=2, dp_size=1, enable_cp=True
            )
        elif topology == "dp_only":
            moe_config = create_moe_config(use_all_gather=True)
            parallelism_config = create_parallelism_config(
                ep_size=1, tp_size=1, dp_size=2
            )
        else:
    ...
    def test_sm12x_physical_tp_with_prefill_cp_selects_b12x(self) -> None:
        with self._arch(sm12x=True):
            config = self._make_config("b12x_cp", "b12x")
            self.assertEqual(
                self._candidates(config), {**self.NONE_SELECTED, "b12x": True}
            )
    ...
    def test_sm12x_dp_only_rejects_all(self) -> None:
        with self._arch(sm12x=True):
            for fp4_moe_op in ("auto", "b12x", "cutedsl", "trtllm"):
                with self.subTest(fp4_moe_op=fp4_moe_op):
                    config = self._make_config("dp_only", fp4_moe_op)
                    self.assertEqual(self._candidates(config), self.NONE_SELECTED)
    ...
    def test_sm100_no_dp_explicit_trtllm_selects_trtllm(self) -> None:
        with self._arch(sm12x=False):
            config = self._make_config("no_dp", "trtllm")
            self.assertEqual(
                self._candidates(config), {**self.NONE_SELECTED, "no_dp": True}
    ...
    def test_fp4_pure_tp_skips_topk_recompute_without_ep(self) -> None:
        from rtp_llm.models_py.modules.factory.fused_moe.impl.cuda.strategy import (
            CudaFp4NoDPStrategy,
        )
        ...
        with self._arch(sm12x=False):
            config = self._make_config("no_dp", "trtllm")
            attrs = CudaFp4NoDPStrategy().get_attributes()
            router = attrs.router_class(config, attrs.quant_config)
            self.assertFalse(router.do_recompute_topk)
    ...
    def test_sm100_ep_low_latency_explicit_cutedsl_selects_cutedsl(self) -> None:
        with self._arch(sm12x=False):
            config = self._make_config("ep_low_latency", "cutedsl")
            self.assertEqual(
                self._candidates(config), {**self.NONE_SELECTED, "ep_low_latency": True}
            )
