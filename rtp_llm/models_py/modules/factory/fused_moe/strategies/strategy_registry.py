...
class StrategyRegistry:
    ...
    def get_strategy(self, config: RuntimeConfig) -> MoeStrategy:
        ...
        if not candidates:
            logger.error(
                ...
                f"ep_size={config.model_config.ep_size}, "
                f"world_size={config.model_config.world_size}, "
                f"tp_size={config.model_config.tp_size}, "
                f"use_deepep_low_latency={config.use_low_latency}"
            )
            raise ValueError(
