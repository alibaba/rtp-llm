...
class DeepEpLowLatencyPeoRouter(FusedMoeDataRouter):
    """
    ...
    def __init__(
        ...
        # DeepEP low-latency common initialization
    ...
    def _prepare_pre_tp_slice(
        ...
        tp_dispatch_input = torch.narrow(
            a1, 0, slice_begin, slice_size
        )  # [slice_size, hidden_size]
        tp_topk_ids = torch.narrow(
            topk_ids, 0, slice_begin, slice_size
        )  # [slice_size, num_topk]
        tp_topk_weights = torch.narrow(
            topk_weights, 0, slice_begin, slice_size
        )  # [slice_size, num_topk]
    ...
    def prepare(
        ...
        """Prepare dispatch tokens to experts with Per-Expert Overlap (PEO) level 1, 2, 3, 4.
        ...
        if self._enable_peo_level > 0 and self._enable_peo_level < 4:
            return self._peo_prepare(dispatch_args, tp_topk_weights)
        else:
            # Dispatch tokens with PEO level 4: normal dispatch, but combine runs in PEO mode
            tp_num_tokens = dispatch_args["x"].size(0)
            # Calculate expected_m
            expected_m = max(
                1,
    ...
    def _finalize_post_tp_gather(
        ...
        if tp_size > 1:
            ...
            combined_x = gathered_output[:original_num_tokens, :]
        return combined_x
