...
pattern designed for Data Parallel (DP) MoE:
  ...
  prepare:  allgather(scattered_tokens, STAGE) -> full_tokens
  execute:  MoE on full_tokens (each rank computes partial sums)
  finalize: reduce_scatter(partial_output, STAGE) -> scattered_output

This uses standard NCCL collectives within the current stage's EP group.
STAGE is the same WORLD communicator when pipeline parallelism is disabled.
...
class PureDpRouterBase(FusedMoeDataRouter):
    ...
    - In prepare(): allgathers scattered tokens across the current stage
    ...
    def _pad_to_max(
        ...
        all_n = all_gather(n_tensor, group=Group.STAGE)
    ...
    def prepare(
        ...
        a1_full = all_gather(a1, group=Group.STAGE)
        topk_weights_full = all_gather(topk_weights, group=Group.STAGE)
        topk_ids_full = all_gather(topk_ids, group=Group.STAGE)
    ...
    def finalize(
        ...
        output = reduce_scatter(payload.fused_expert_output, group=Group.STAGE)
