...
DeepepNormalRouterBase = pytest.importorskip("rtp_llm.models_py.modules.factory.fused_moe.impl.cuda.routers.deepep_normal_router").DeepepNormalRouterBase
...
def _spawn_wrapper(
    ...
    """Wrapper for mp.spawn that builds ParallelismConfig in child.

    This avoids CUDA-in-fork issues (spawn is required) and avoids pickling
    complex objects by constructing them inside each spawned worker.
    """
    ...
    worker_function(rank, use_fp8, token_num_per_rank, parallelism_config, nccl_port)


def _test_single(world_size: int, test_tp_size: int, use_fp8: bool):
    ...
    # NOTE: torch.cuda cannot be safely used with forked subprocesses.
    # Always use spawn here (mp.spawn uses spawn).
    try:
        mp.spawn(  # pyright: ignore[reportPrivateImportUsage]
            _spawn_wrapper,
            args=(use_fp8, world_size, test_tp_size, token_num_per_rank, nccl_port),
            nprocs=world_size,
            join=True,
        )
    finally:
        # Release locks after spawn completes (success or failure)
        for lock in locks:
...
@mark.cuda
@mark.H20
def test_deepep_normal_router():
    world_size = 2

    # 为每个world_size运行test_single函数
    for use_fp8 in [True]:
        for test_tp_size in [2]:
            _test_single(world_size, test_tp_size, use_fp8)
