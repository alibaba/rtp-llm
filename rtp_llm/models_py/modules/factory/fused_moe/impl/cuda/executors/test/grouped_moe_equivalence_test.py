"""Real-GPU numerical coverage for grouped FP8/FP4 MoE execution.

The test builds the production ``MoE`` layer from canonical MoE tensor descriptors. It compares the automatically available grouped-FP4
...
def _make_layer_weights(
    ...
    *,
    ...
    weights = {
        ...
        W.moe_w1: packed_fp4(inter_dim, dim),
        W.moe_s1: fp4_scale(inter_dim, dim),
        W.moe_w2: packed_fp4(dim, inter_dim),
        W.moe_s2: fp4_scale(dim, inter_dim),
        "routed_up": packed_fp4(inter_dim, dim),
        "routed_up_scale": fp4_scale(inter_dim, dim),
    }
    if stable_routing:
        ...
        weights[W.moe_gate_tid2eid] = (
            token_ids * 4 + topk_slots + routing_offset
        ).remainder(experts)
    weights.update(
        {
            W.ffn_w13: torch.randn(
                2 * inter_dim,
                dim,
                dtype=torch.bfloat16,
                device=device,
            ).to(torch.float8_e4m3fn),
            W.ffn_s13: torch.randint(
                124,
                127,
