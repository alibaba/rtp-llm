...
def _generate_payload_and_weights(
    ...
    hidden_states_3d, masked_m, topk_idx, routing_weights = prepare_inputs(
        hidden_states_expanded, routing_logits, config.expert_num, TOP_K
    )
...
def _filter_valid_tokens(
    output: torch.Tensor, expert_num_tokens: torch.Tensor
) -> torch.Tensor:
...
def test_cutedsl_fp4_executor():
    ...
    if torch.cuda.get_device_capability() < (10, 0):
        print("Skipping test: Nvfp4 Requires compute capability of 10 or above.")
        return
    ...
    output = _filter_valid_tokens(output, payload.expert_tokens_meta.expert_num_tokens)
    
    num_local_experts = NUM_EXPERTS // EP_SIZE
    num_tokens = BATCH_SIZE
    output_aggregated = torch.zeros(
        num_tokens, K, device=output.device, dtype=output.dtype
    )
