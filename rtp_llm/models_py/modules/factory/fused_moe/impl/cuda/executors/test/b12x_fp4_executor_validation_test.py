...
class B12xWeightPreparationTest(unittest.TestCase):
    ...
    def test_w1_distinct_up_gate_outer_scales_are_folded_independently(self):
        ...
        scale_2_pair = stack_moe_w1_s2_pair([torch.tensor(2.0), torch.tensor(4.0)])
        ...
        physical_scale = (
            prepared_scale.permute(5, 2, 4, 0, 1, 3).contiguous().reshape(1, 256, 8)
        )
        folded = torch.cat(
            [
        ...
        self.assertTrue(torch.equal(physical_scale, self._reference_swizzle(folded)))
...
class B12xFlashInferCompatibilityTest(unittest.TestCase):
    def test_cutlass_python_path_is_bootstrapped_without_pth_processing(self):
        import nvidia_cutlass_dsl

        python_packages = next(
            Path(root) / "python_packages"
            for root in nvidia_cutlass_dsl.__path__
            if (Path(root) / "python_packages").is_dir()
        )
        ...
        try:
            sys.path[:] = [
                ...
                if Path(entry).resolve() != python_packages.resolve()
            ]
            _ensure_cutlass_python_path()
            self.assertEqual(Path(sys.path[0]).resolve(), python_packages.resolve())
        finally:
            sys.path[:] = original_path

    def test_required_flashinfer_apis_exist(self):
        self.assertGreater(get_b12x_kernel_tile_n(), 0)

    def test_flashinfer_version_is_not_restricted(self):
        import flashinfer

        with patch.object(flashinfer, "__version__", "unrestricted-test"):
            _load_b12x_symbols.cache_clear()
            try:
                self.assertGreater(get_b12x_kernel_tile_n(), 0)
            finally:
                _load_b12x_symbols.cache_clear()

    def test_available_tile_width_api_is_used(self):
        from flashinfer.fused_moe.cute_dsl.blackwell_sm12x import moe_dispatch

        with patch.object(moe_dispatch, "_level_tile_n", None), patch.object(
            moe_dispatch, "_LEVEL_TILE_N", 64, create=True
        ):
            _load_b12x_symbols.cache_clear()
            try:
                self.assertEqual(get_b12x_kernel_tile_n(), 64)
            finally:
                _load_b12x_symbols.cache_clear()
    ...
    def test_pinned_dependency_closure_is_available(self):
        ...
        self.assertEqual(metadata.version("nvidia-cutlass-dsl-libs-base"), "4.5.3")

    def test_cutlass_ir_extension_is_loaded_from_pinned_payload(self):
        module = importlib.import_module("cutlass._mlir._mlir_libs._cutlass_ir")
        origin = Path(module.__file__).resolve()
        libs_dist = metadata.distribution("nvidia-cutlass-dsl-libs-base")
        payload_root = Path(libs_dist.locate_file(""))
        expected = payload_root.joinpath(
            "nvidia_cutlass_dsl/python_packages",
            *module.__name__.split(".")[:-1],
            origin.name,
        )
        self.assertTrue(
            expected.samefile(origin),
            f"CUTLASS IR extension {origin} did not load from {expected}",
        )
