...
class B12xFp4ExecutorEnvironmentTest(unittest.TestCase):
    """Fail the SM120 target when its requested GPU was not assigned."""

    def test_requires_sm12x_cuda(self):
        self.assertTrue(
            torch.cuda.is_available(),
            "b12x_fp4_executor_test requires a CUDA device",
        )
        major, minor = torch.cuda.get_device_capability()
        self.assertEqual(
            major,
            12,
            f"b12x_fp4_executor_test requires sm_120/sm_121, got sm_{major}{minor}",
        )
...
class B12xFp4ExecutorCudaGraphTest(B12xFp4ExecutorTestBase, unittest.TestCase):
    ...
    def test_b12x_fp4_executor_cuda_graph_capture(self):
        ...
        topk_ids = topk_ids.to(executor.topk_ids_dtype)
        initial_topk_ids = topk_ids.clone()
        initial_topk_weights = topk_weights.clone()
        payload = ExpertForwardPayload(
            expert_x=expert_x,
            expert_x_origin_dtype=torch.bfloat16,
            expert_topk_ids=topk_ids,
        ...
        # Replay must consume the same mutable input buffers with new values;
        # compare against the executor's independent eager wrapper so a stale
        # capture or output buffer would be observable.
        self.assertIsNotNone(executor._b12x_moe_eager)
        expert_x.copy_(torch.randn_like(expert_x))
        topk_ids.copy_(torch.roll(topk_ids, shifts=1, dims=0))
        topk_weights.copy_(torch.roll(topk_weights, shifts=1, dims=1))
        self.assertFalse(torch.equal(topk_ids, initial_topk_ids))
        self.assertFalse(torch.equal(topk_weights, initial_topk_weights))
        expected_dynamic = executor._b12x_moe_eager.run(
            x=expert_x,
            w1_weight=executor.w1,
            w1_weight_sf=executor.w1_sf_mma,
            w2_weight=executor.w2,
            w2_weight_sf=executor.w2_sf_mma,
            token_selected_experts=topk_ids,
            token_final_scales=topk_weights,
            w1_alpha=executor.w1_alpha,
            w2_alpha=executor.w2_alpha,
            fc2_input_scale=executor.fc2_input_scale,
        ).clone()
        ...
        dynamic_relative_l2 = (
            torch.linalg.vector_norm(
                output.to(torch.float32) - expected_dynamic.to(torch.float32)
            )
            / torch.linalg.vector_norm(expected_dynamic.to(torch.float32))
        ).item()
        self.assertLessEqual(
            dynamic_relative_l2,
            0.02,
            f"CUDA Graph dynamic replay relative L2 {dynamic_relative_l2:.6f} > 0.02",
        )
