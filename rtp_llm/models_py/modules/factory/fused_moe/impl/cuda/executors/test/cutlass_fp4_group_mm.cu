...
// Inline SM version (replaces cutlass_extensions/common.hpp)
...
void run_get_group_gemm_starts(const torch::Tensor& a_starts,
  ...
  auto stream = at::cuda::getCurrentCUDAStream(a_tensors.get_device()).stream();
  ...
  __CALL_GET_STARTS_KERNEL_BLOCKSCALE(
      ...
      torch::kBFloat16, cutlass::bfloat16_t, LayoutSFA,
      LayoutSFB, ScaleConfig)
...
void run_fp4_blockwise_scaled_group_mm_sm100(
  ...
  auto workspace =
      torch::empty({static_cast<long>(workspace_size)}, torch::TensorOptions().dtype(torch::kByte).device(a.device()));
  const cudaStream_t stream = at::cuda::getCurrentCUDAStream(a.get_device()).stream();
...
void cutlass_fp4_group_mm(torch::Tensor& output,
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("cutlass_fp4_group_mm", &cutlass_fp4_group_mm, "CUTLASS FP4 Group GEMM SM100");
}
