...
from rtp_llm.config.moe_config import B12X_ZEROED_ENERGY_LIMIT_ENV
...
def _log_runtime_config_once(zeroed_energy_limit: float) -> None:
    global _runtime_config_logged
    with _runtime_config_lock:
        if _runtime_config_logged:
            return
        logger.info(
            "b12x FP4 runtime config: %s=%s; checkpoint input_scale "
...
class B12xFp4Executor(FusedMoeExpertExecutor):
    ...
    def __init__(
        ...
        _log_runtime_config_once(config.b12x_zeroed_energy_limit)
