...
class Sm120Fp8GroupedGemmExecutor(FusedMoeExpertExecutor):
    ...
    def __init__(
        ...
        if self.E != self.num_experts_per_partition:
            raise ValueError(
                "SM120 grouped MoE weight expert count does not match the EP "
                f"partition: weights contain {self.E} experts, expected "
                f"{self.num_experts_per_partition} for expert_num={self.num_experts}, "
                f"ep_size={self.ep_size}"
            )
        # ep_gather uses a 512-wide hidden-dimension block. Validate that
        # contract while loading the model instead of failing on first execute.
        if self.K % 512 != 0:
            raise ValueError(
                f"SM120 grouped MoE hidden size K={self.K} must be divisible by 512"
            )
    ...
    def execute(
        ...
        if a2_scale is not None:
            raise ValueError("SM120 grouped MoE does not support a2_scale")
        ...
        if payload.expert_tokens_meta.expert_num_tokens_cpu is not None:
            max_expert_tokens = int(
                max(payload.expert_tokens_meta.expert_num_tokens_cpu, default=0)
            )
        else:
            # CUDA graph is rejected by this strategy, so this synchronization is
            # safe. It prevents allocating E * token_num rows when routing is
            # distributed across experts.
            max_expert_tokens = int(num_recv_tokens_per_expert.max().item())
        if max_expert_tokens > self.masked_max_token_num:
            raise ValueError(
                "SM120 grouped MoE routed too many tokens to one expert: "
                f"{max_expert_tokens} > masked_max_token_num="
                f"{self.masked_max_token_num}. Reduce prefill tokens or increase "
                "--masked_max_token_num after checking GPU memory."
            )
        alignment = align(max(1, max_expert_tokens), self.EXPERT_ALIGNMENT)
        expected_m = min(alignment, token_num_mean_per_expert)
        tuning_token_count = alignment
