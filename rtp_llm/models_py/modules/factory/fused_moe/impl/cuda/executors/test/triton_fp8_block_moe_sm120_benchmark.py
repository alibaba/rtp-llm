...
def main() -> None:
    ...
    # sizes. Tune mode additionally specializes the compiled routing helper for
    # every candidate BLOCK_SIZE_M, so budget for the full Cartesian product.
    tokens = parse_int_list(args.tokens)
    block_m_variants = (
        len({config["BLOCK_SIZE_M"] for config in candidate_configs()})
        if args.mode == "tune"
        else 1
    )
    torch._dynamo.config.recompile_limit = max(
        torch._dynamo.config.recompile_limit,
        len(tokens) * block_m_variants + 4,
    )
    print(
        json.dumps(
            {
