"""DCU Pure-TP router for fused MoE — no quantization path."""
...
class PureTpRouterBase(FusedMoeDataRouter):
    """Pure-TP/TP=EP router for DCU: no token dispatch, all-reduce after experts.

    Accepts both single-GPU (ep_size==1) and TP==EP configurations.
    When ep_size > 1, each rank only owns a contiguous slice of experts;
    the executor builds an expert_map to filter non-local experts.
    """
