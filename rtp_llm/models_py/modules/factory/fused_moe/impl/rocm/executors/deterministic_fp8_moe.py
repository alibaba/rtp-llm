...
def _max_tokens() -> int:
    ...
    try:
        ...
        _log_once(
            logging.WARNING,
            f"bad_max_tokens:{raw}",
            f"{_MAX_TOKENS_ENV}={raw!r} is not an integer; "
            f"using {_DEFAULT_MAX_TOKENS}",
        )
        return _DEFAULT_MAX_TOKENS
...
def _shape_unsupported_reason(
    ...
    """Workload-level reasons (device, shape, dtype, layout) to skip the path."""
...
def _unsupported_reason(
    ...
    shape_reason = _shape_unsupported_reason(
        hidden_states,
        w1,
        w2,
        topk_weights,
    ...
    if shape_reason is not None:
        return shape_reason
    return _runtime_unsupported_reason(str(hidden_states.device))
...
def _route_local_stage2(
    ...
    **kwargs,
    ...
    # aiter's fused_moe_2stages allocates the stage1 intermediate and hands it
    # to stage2 as [token_num, topk, inter]: shape[0] is the token count, not
    # the route count.  Fail loudly if that contract ever changes.
    if inter_states.dim() != 3:
        raise RuntimeError(
            "deterministic stage2 expects inter_states shaped "
            "[token_num, topk, inter], got "
            f"{tuple(inter_states.shape)}"
        )
    token_num, topk_dim, _ = inter_states.shape
    if topk_dim != topk:
        raise RuntimeError(
            f"inter_states topk dim {topk_dim} does not match topk {topk}"
        )
    ...
    # The per-call allocation is intentional: the caching allocator (eager) or
    # the CUDA/HIP-graph private pool (capture) manages the scratch, and the
    # recorded zero-fill replays on every graph replay.  Do not cache this
    # buffer across calls: a shared buffer breaks multi-stream use and
    # cross-graph pool reuse.
...
def try_deterministic_fp8_moe(
    ...
    reason = _shape_unsupported_reason(
        hidden_states,
        w1,
        w2,
        topk_weights,
    ...
    if runtime_reason is not None:
        # The workload matches the supported shapes, so this flag requested
        # determinism for exactly this path.  Fail closed instead of silently
        # falling back to non-deterministic kernels (shape mismatches above
        # keep the graceful fallback: the flag is global while the path only
        # targets specific workload shapes).
        raise RuntimeError(
            f"{ROCM_FP8_MOE_DETERMINISTIC_REDUCE_ENV}=1 requires the "
            f"deterministic ROCm FP8 MoE path for this workload, but the "
            f"runtime is unsupported: {runtime_reason}"
        )
