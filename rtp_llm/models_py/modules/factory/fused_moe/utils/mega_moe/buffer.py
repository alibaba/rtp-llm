"""Symmetric-memory buffer cache and capability gates for MegaMoE.
...
across all MoE layers via a module-level cache, avoiding one large persistent
    ...
    if buf is None:
        ...
        if actual_bytes is not None:
            if estimated_bytes is not None:
                logging.info(
                    "[MegaMoE] allocated symm buffer: group_size=%d "
...
def _get_or_create_mega_fp8_buf(
    group,
    num_experts,
    num_max_tokens_per_rank,
    num_topk,
