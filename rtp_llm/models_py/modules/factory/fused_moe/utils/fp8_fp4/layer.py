...
class Fp8Fp4MoeRuntimeConfig:
    ...
    def __init__(
        ...
        *,
        ...
        if moe_w1_layout not in {"gate_up", "up_gate"}:
            raise ValueError(
                "moe_w1_layout must be 'gate_up' or 'up_gate', "
                f"got {moe_w1_layout!r}"
            )
        self.moe_w1_layout = moe_w1_layout
        self.expert_num = int(expert_num)
        self.physical_expert_num = int(
            self.expert_num if physical_expert_num is None else physical_expert_num
        )
        if self.physical_expert_num < self.expert_num:
            raise ValueError(
                "physical_expert_num must be at least expert_num, got "
                f"{self.physical_expert_num} < {self.expert_num}"
            )
        if self.physical_expert_num % max(ep_size, 1) != 0:
            raise ValueError(
    ...
    def __init__(
        ...
        *,
        ...
        runtime_config = Fp8Fp4MoeRuntimeConfig(
            ...
            physical_expert_num=physical_expert_num,
        )
