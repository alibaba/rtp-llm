...
class TestPCPAll2AllAttnOp(unittest.TestCase):
    ...
    def test_user_buffer_accept_path_for_mha_exchange(self):
        op = object.__new__(PCPAll2AllAttnOp)
        op.use_ub = True
        op.prefill_cp_rank = 0
        calls = []
        ...
        kv = torch.zeros(4, 3, device=self.device)
        recv = torch.empty_like(kv)
        with patch(f"{_A2A_MODULE}.send_to_group_rank") as send, patch(
            f"{_A2A_MODULE}.recv_from_group_rank"
        ) as receive:
            op._exchange_kv(kv, recv, next_rank_id=1, prev_rank_id=3)

        self.assertEqual(len(calls), 1)
        send.assert_not_called()
        receive.assert_not_called()

    def test_user_buffer_reject_path_uses_ordered_collectives(self):
        op = object.__new__(PCPAll2AllAttnOp)
        op.use_ub = True
        op.prefill_cp_rank = 1
        ...
        with patch(f"{_A2A_MODULE}.send_to_group_rank") as send, patch(
            ...
            op._exchange_kv(kv, recv, next_rank_id=0, prev_rank_id=2)

        receive.assert_called_once_with(recv, src=2, group=Group.TP)
        send.assert_called_once_with(kv, dst=0, group=Group.TP)
    ...
    def _build_shuffle_indices(
        ...
        for padded_length in padded_lengths:
            indices.extend(zigzag_positions_for_rank(padded_length, cp_size, rank))
    ...
    def test_cache_append_offsets_prefix_and_filters_production_padding(self):
        ...
        prefix_lengths = [64, 128]
        sequence_lengths = [
            prefix + actual for prefix, actual in zip(prefix_lengths, actual_lengths)
        ]
        ...
        attn_inputs.context_parallel_info.prefill_shuffle_indices = all_shuffle[0]

        with patch(
            f"{_A2A_MODULE}.all_gather", return_value=torch.cat(all_shuffle)
        ...
        values = -keys
        for rank, expected_count in enumerate((65, 64, 64, 65)):
            ...
            self.assertTrue(torch.equal(kwargs["append_key"], keys[expected_indices]))
            self.assertTrue(
    ...
    def test_warmup_without_kv_cache(self):
        self.run_no_prefix(
            batch_size=1, sequence_lengths=[32], cp_size=4, cp_rank=0, warmup=True
        )
