...
class TestPCPAll2AllAttnOp(unittest.TestCase):
    ...
    def test_chunk_append_positions_exclude_padding(self):
        ...
        with patch.multiple(
            _A2A_MODULE,
            all_gather=Mock(return_value=torch.cat(all_shuffle)),
            get_user_buffers_communicator=Mock(return_value=None),
            get_py_flashinfer_workspace_buffer=Mock(
                return_value=torch.empty(1, device=self.device)
            ),
            BatchPrefillWithPagedKVCacheWrapper=DEFAULT,
            BatchPrefillWithRaggedKVCacheWrapper=DEFAULT,
            append_paged_kv_cache=DEFAULT,
        ) as mocks:
            append = mocks["append_paged_kv_cache"]
