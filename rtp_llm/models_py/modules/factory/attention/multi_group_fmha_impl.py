...
def modify_attn_inputs_by_gid(
    ...
    if not by_group_device or gid >= len(by_group_device):
        yield
        return
...
class MultiGroupFMHAImpl(FMHAImplBase):
    ...
    group via swtich_group().

    CUDA Graph compatibility:
    - swtich_group() is a pure Python operation and executes normally during graph
    ...
    When swtich_group(gid) is called for a non-full-attention layer (e.g. SSM / linear
    ...
    def __init__(self, impls: Dict[int, FMHAImplBase]) -> None:
        ...
        self._current_gid: int = next(iter(impls))

    def swtich_group(self, gid: int) -> None:
    ...
    def forward(
        ...
        return self._impls[self._current_gid].forward(qkv, kv_cache)

    @property
    def fmha_params(self):
        return self._impls[self._current_gid].fmha_params
    ...
    def support(
        ...
        return all(
            impl.support(attn_configs, attn_inputs) for impl in self._impls.values()
        )
