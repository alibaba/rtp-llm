...
class TokenSpeedMlaDecodeOp:
    ...
    def __init__(
        ...
        projection_weight = next(
            (
                layer_weights[W.mla_kc]
                for layer_weights in weights
                if W.mla_kc in layer_weights and W.mla_vc in layer_weights
            ),
            None,
        )
    ...
    def _absorb_query(
        ...
        if (
            layer_id < 0
            or layer_id >= len(self.weights)
            or W.mla_kc not in self.weights[layer_id]
            or W.mla_vc not in self.weights[layer_id]
        ):
            raise RuntimeError(
                f"TokenSpeed MLA decode layer {layer_id} has no projection weights"
            )
        ...
        if self.fp8_compute:
            if self._q_fp8 is None or self._q_fp8.size(0) < num_tokens:
                if self.use_cuda_graph:
                    raise RuntimeError(
                        "FP8 MLA query buffer cannot grow during graph replay"
                    )
                self._q_fp8 = torch.empty_like(q_absorbed, dtype=self._dtype)
            return quantize_fp8(
                q_absorbed,
                self.q_scale,
                self._q_fp8[:num_tokens],
                name="decode_absorbed_q",
            )
        return q_absorbed
    ...
    def forward(
        ...
        record_model(f"mla.layers.{layer_id}.decode.query", q_absorbed)
        ...
        record_model(
            f"mla.layers.{layer_id}.decode.metadata",
            {
                "block_tables": self._block_tables[
                    : self._batch_size, : self._padded_blocks
                ],
        ...
        projected = self._project_output(attn_output, layer_id)
        record_model(f"mla.layers.{layer_id}.decode.value_projection", projected)
        return projected
