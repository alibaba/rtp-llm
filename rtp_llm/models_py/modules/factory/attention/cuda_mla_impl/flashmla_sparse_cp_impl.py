...
class SparseMlaFp8CPOp(SparseMlaFp8Op):
    ...
    def plan(
        ...
        if n_q > 0:
            ...
            full_rope = torch.zeros(
                positions_d.size(0), dtype=positions_d.dtype, device=positions_d.device,
            )
    ...
    def forward(
        ...
        out = torch.zeros(
            q.size(0), out0.size(1), out0.size(2),
            dtype=out0.dtype, device=out0.device,
        )
