...
def build_working_sets(kv_cache, config):
    from rtp_llm.utils.dsa_indexing import dsa_layer_skips_topk

    # Typed hybrids contain KDA layers and independently allocated MLA pools.
    # Share maps only when both the selected indices and allocator agree.
    groups = []
    generations = getattr(kv_cache, "block_generations_by_layer", [])
    for layer, hbm in enumerate(kv_cache.mla_hbm_cache_by_layer):
        if hbm is None:
            continue
        generation = generations[layer] if generations else kv_cache.block_generations
        if (
            groups
            and dsa_layer_skips_topk(config, layer)
            and groups[-1][1].data_ptr() == generation.data_ptr()
        ):
            groups[-1][0].append(layer)
        else:
            groups.append(([layer], generation))
    result = {}
    page = kv_cache.kernel_seq_size_per_block
    for layers, generation in groups:
        backing = [
            kv_cache.get_layer_cache(layer).kv_cache_base.reshape(
                -1, page, kv_cache.get_layer_cache(layer).kv_cache_base.shape[-1]
            )
            for layer in layers
        ]
        working = PinnedMlaWorkingSet(
            ...
            block_generations=generation,
            allocator_block_size=kv_cache.seq_size_per_block,
            hbm_tokens=kv_cache.dsa_mla_hbm_blocks * kv_cache.seq_size_per_block,
            hbm_cache=[
                kv_cache.mla_hbm_cache_by_layer[layer].reshape(
                    -1, page, kv_cache.mla_hbm_cache_by_layer[layer].shape[-1]
                )
                for layer in layers
            ],
        )
...
def _gather_unique(
    ...
    # One byte-preserving gather for an entire prefill layer, including mixed tiers.
    for row in range(tl.program_id(0), N, tl.num_programs(0)):
        token = (
            tl.load(Ids + row // PAGE_SIZE).to(tl.int64) * PAGE_SIZE + row % PAGE_SIZE
        )
        x = tl.arange(0, B)
        host = tl.load(
            Host + (token - HBM_TOKENS) * WIDTH + x,
            (token >= HBM_TOKENS) & (x < WIDTH),
            other=0,
        )
        hbm = tl.load(
            Hbm + token * WIDTH + x,
            (token >= 0) & (token < HBM_TOKENS) & (x < WIDTH),
            other=0,
        )
        tl.store(
            Out + row * WIDTH + x, tl.where(token < HBM_TOKENS, hbm, host), x < WIDTH
        )
...
def _protect(
    ...
    generation = tl.load(
        Generations + token // BLOCK_SIZE, valid & (slot >= 0), other=-1
    )
...
def _fetch_tiled(
    ...
    for start in range(tl.program_id(0) * ROWS, count, tl.num_programs(0) * ROWS):
        ...
        x = tl.arange(0, B)
        data = tl.load(
            host + (token[:, None] - HBM_TOKENS) * (WIDTH // 4) + x[None, :],
            valid[:, None] & (x[None, :] < WIDTH // 4),
            other=0,
        )
        tl.store(
            device + (slot[:, None] + HBM_TOKENS) * (WIDTH // 4) + x[None, :],
            data,
            valid[:, None] & (x[None, :] < WIDTH // 4),
        )
...
@triton.jit
def _invalidate(
    Ids, Map, Tags, N: tl.constexpr, B: tl.constexpr, HBM_TOKENS: tl.constexpr = 0
):
    i = tl.program_id(0) * B + tl.arange(0, B)
    token = tl.load(Ids + i, i < N, other=-1)
    valid = (i < N) & (token >= HBM_TOKENS)
...
class PinnedMlaWorkingSet:
    ...
    def __init__(
        ...
        for tensor in backing:
            ...
            if tensor.shape != first.shape or tensor.dtype != first.dtype:
                raise ValueError(
                    "shared-index group layers must have identical KV layout"
                )
        ...
        self.resident = (
            tuple(hbm_cache)
            if hbm_cache is not None
            else tuple(
                torch.empty(shape, dtype=first.dtype, device=self.device)
                for _ in backing
            )
        )
        if len(self.resident) != len(backing) or any(
            ...
            raise ValueError(
                "HBM tensors must hold complete blocks followed by the working set"
            )
        ...
        if (
            self.allocator_block_size <= 0
            or self.logical_tokens % self.allocator_block_size
        ):
        ...
        self.fetch_tiled = (
            ...
            and self.width % 4 == 0
            and all(
                tensor.data_ptr() % 4 == 0 for tensor in (*self.backing, *self.resident)
            )
        )
        ...
        # Internal capture events cannot be waited on by a later eager prefill.
        # Keep fork/join events internal, and expose one real completion event
        # per group for capture -> eager transitions.
        self.capture_complete = torch.cuda.Event(external=True)
        self.last_begin_captured = False
        self.started = False
    ...
    def begin(self, logical_indices: torch.Tensor) -> torch.Tensor:
        ...
        self.last_begin_captured = torch.cuda.is_current_stream_capturing()
        if not self.last_begin_captured:
        ...
        with torch.cuda.stream(self.transfer_stream):
            ...
            for layer, (host, resident, ready) in enumerate(
                zip(self.backing, self.resident, self.ready)
            ):
                if n:
                    if self.fetch_tiled:
                ...
                if self.last_begin_captured and layer == len(self.ready) - 1:
                    # Record before the final internal event so the capture's
                    # join also includes this external event node.
                    self.capture_complete.record(self.transfer_stream)
    ...
    def layer_cache(self, layer: int) -> torch.Tensor:
        ...
        self._wait_for_layer(layer, self.compute_stream)
        return self.resident[layer]

    def _wait_for_layer(self, layer: int, stream: torch.cuda.Stream) -> None:
        event = self.ready[layer]
        if self.last_begin_captured and not torch.cuda.is_current_stream_capturing():
            event = self.capture_complete
    ...
    def wait_prefetch_complete(self) -> None:
        ...
        self._wait_for_layer(-1, torch.cuda.current_stream(self.device))

    def write(
        self, layer: int, logical_slots: torch.Tensor, values: torch.Tensor
    ) -> None:
    ...
    def gather_pages(self, layer: int, page_ids: torch.Tensor) -> torch.Tensor:
        """Gather complete pages in table order for the ordinary dense prefill path."""
        self._check_ids(page_ids)
        if self.started:
            self.compute_stream.wait_stream(self.transfer_stream)
        page = self.backing[layer].shape[1]
        cache = torch.empty(
            (page_ids.numel(), *self.backing[layer].shape[1:]),
            dtype=self.backing[layer].dtype,
            device=self.device,
        )
        if page_ids.numel():
            _gather_unique[(min(page_ids.numel() * page, 128),)](
                self.backing[layer].view(torch.uint8),
                self.resident[layer].view(torch.uint8),
                page_ids,
    ...
    def prefill_cache(self, layer: int, logical_indices: torch.Tensor):
        """Gather unique selected rows into a temporary, layer-local GPU workspace.

        Prompt queries share most KV rows. Gathering each unique row once avoids
        a decode-sized fetch per query and keeps the workspace linear in KV length.
        It is released after attention; persistent prefix residency stays on host.
        """
        self._check_ids(logical_indices)
        if self.started:
            self.compute_stream.wait_stream(self.transfer_stream)
        unique, inverse = torch.unique(logical_indices, return_inverse=True)
        page = self.backing[layer].shape[1]
        rows = triton.cdiv(unique.numel(), page) * page
        cache = torch.empty(
            (rows // page, *self.backing[layer].shape[1:]),
            dtype=self.backing[layer].dtype,
            device=self.device,
        )
        _gather_unique[(min(unique.numel(), 128),)](
            self.backing[layer].view(torch.uint8),
            self.resident[layer].view(torch.uint8),
            unique,
            cache.view(torch.uint8),
        ...
        return cache, inverse.to(torch.int32).masked_fill(logical_indices < 0, -1)
