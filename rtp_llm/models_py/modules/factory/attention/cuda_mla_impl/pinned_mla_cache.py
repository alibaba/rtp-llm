...
def build_working_sets(kv_cache):
    """Build only the MLA layers explicitly marked by the native allocator.

    GLM53 computes its own top-k at each MLA layer. The intervening KDA layers
    have no tier metadata and must never be reshaped as token KV rows.
    """
    result = {}
    for layer, info in enumerate(kv_cache.mla_host_cache_by_layer):
        if info.hbm_cache is None:
            continue
        view = kv_cache.get_layer_cache(layer)
        if view.kv_cache_base.is_cuda:
            raise ValueError("MLA host metadata requires CPU backing")
        page = kv_cache.kernel_seq_size_per_block
        hbm = info.hbm_cache
        capacity = hbm.shape[0] * hbm.shape[1] - info.hbm_tokens
        working = PinnedMlaWorkingSet(
            [view.kv_cache_base],
            capacity,
            page,
            hbm.device,
            block_generations=info.block_generations,
            allocator_block_size=kv_cache.seq_size_per_block,
            hbm_tokens=info.hbm_tokens,
            hbm_cache=[hbm],
        )
        result[layer] = (working, 0)
    return result
...
class PinnedMlaWorkingSet:
    ...
    def __init__(
        ...
        if (
            ...
            or self.hbm_tokens % self.allocator_block_size
        ):
