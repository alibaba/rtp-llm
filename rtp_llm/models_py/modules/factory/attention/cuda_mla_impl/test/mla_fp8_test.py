...
from rtp_llm.models_py.modules.factory.attention.cuda_mla_impl.mla_fp8_kernels import (
    gather_fp8_prefix, quantize_fp8,
)
...
class MlaFp8Test(unittest.TestCase):
    ...
    def test_quantizer_saturation_noncontiguous_and_graph(self):
        x = torch.linspace(-1000, 1000, 4096, device="cuda").reshape(32, 128).bfloat16().t()
    ...
    def test_quantizer_preserves_nan_and_saturates_infinity(self):
        x = torch.tensor([float("nan"), float("inf"), -float("inf"), 0.0], device="cuda")
        actual = quantize_fp8(x).float()
        self.assertTrue(torch.isnan(actual[0]).item())
        torch.testing.assert_close(actual[1:], torch.tensor([448., -448., 0.], device="cuda"))
    ...
    def test_paged_prefix_and_suffix_nonunit_scale(self):
        ...
        storage = quantize_fp8(torch.randn(8, page_size + 1, 576, device="cuda").bfloat16())
        cache = storage[:, :page_size]
        pages = torch.tensor([5, 1, 7], device="cuda", dtype=torch.int32)
        info = torch.tensor([[0, 133, 0, 2], [1, 17, 2, 1]], device="cuda", dtype=torch.int32)
        ...
        gather_fp8_prefix(out_c, out_r, c, r, cache, pages, info, q_ind, page_size, scale=0.5)
        expected = torch.cat([
            (cache[5].float() * 0.5).bfloat16(),
            (cache[1, :5].float() * 0.5).bfloat16(), new[:3],
            (cache[7, :17].float() * 0.5).bfloat16(), new[3:]
        ])
    ...
    def test_full_and_bounded_prefix_routes_agree(self):
        ...
        from rtp_llm.models_py.modules.factory.attention.cuda_mla_impl.flashmla_dense_prefill import MlaFlashMLAPrefillOp
        from rtp_llm.models_py.modules.factory.attention.cuda_mla_impl.test.flashmla_forward_test_utils import (
            DeterministicPackedProjection, output_and_lse,
            EXPANDED_KV_BYTES_PER_TOKEN,
        )
        from rtp_llm.models_py.modules.factory.attention.cuda_mla_impl.test.flashmla_dense_prefill_packed_kv_test import FlashMLADensePrefillPackedKVTest
        ...
        for q_lens, prefix_lens in (((2, 3), (0, 300)), ((1, 1), (127, 640))):
            ...
            inputs.kv_cache.kv_cache_base = quantize_fp8(inputs.kv_cache.kv_cache_base, 0.5)
            results = []
            for capacity in (0, 256):
                op = MlaFlashMLAPrefillOp(
                    num_heads=12, kv_lora_rank=512, qk_rope_head_dim=64,
                    qk_nope_head_dim=128, v_head_dim=128, page_size=128,
                    softmax_extra_scale=1.0, use_mla=True, weights=[{}],
                    kv_cache_dtype=KvCacheDataType.FP8, fp8_compute=True,
                    q_scale=0.5, kv_scale=0.5,
                    expanded_kv_budget_bytes=capacity * EXPANDED_KV_BYTES_PER_TOKEN,
                )
                op.plan(inputs.params)
                with mock.patch.object(op, "_create_kv_b_proj", return_value=DeterministicPackedProjection()):
            ...
            torch.testing.assert_close(results[0][0].float(), results[1][0].float(), atol=2e-3, rtol=3e-2)
            torch.testing.assert_close(results[0][1], results[1][1], atol=2e-3, rtol=2e-3)

    def test_cache_writer_scale_page_boundary_and_skipped_slot(self):
        from types import SimpleNamespace
        from rtp_llm.ops import KvCacheDataType
        from rtp_llm.models_py.modules.factory.attention.cuda_mla_impl.mla_kv_cache_write_op import MlaKVCacheWriteOp
        ...
        op.forward(x[:, :512], x[:, 512:], SimpleNamespace(kv_cache_base=cache),
    ...
    def test_prefill_causal_and_noncausal_fp8_compute(self):
        from rtp_llm.models_py.modules.factory.attention.cuda_mla_impl.flashmla_dense_prefill import MlaFlashMLAPrefillOp
        from rtp_llm.ops import KvCacheDataType
        op = MlaFlashMLAPrefillOp(12, 512, 64, 128, 128, 128, 1.0, True,
                                [{}], kv_cache_dtype=KvCacheDataType.FP8,
                                fp8_compute=True, q_scale=0.5, kv_scale=0.25)
        for causal in (False, True):
            ...
            out, lse = op._run_dense_attention(q, k, v, qo_indptr=qi, kv_indptr=ki,
                                               max_q_len=7, max_kv_len=17, causal=causal)
            ...
            for qs, qe, ks, ke in ((0, 4, 0, 12), (4, 11, 12, 29)):
                ...
                if causal:
                    allowed = torch.arange(ke-ks, device="cuda")[None, :] <= (ke-ks-(qe-qs)+torch.arange(qe-qs, device="cuda"))[:, None]
                ...
                ref = torch.einsum('hqk,khd->qhd', p8, vf[ks:ke]) / p.sum(-1).t()[..., None]
                torch.testing.assert_close(out[qs:qe].float(), ref, atol=0.0005, rtol=0.01)
                torch.testing.assert_close(lse[qs:qe], score.logsumexp(-1).t(), atol=0.003, rtol=0.002)

    def test_diagnostic_ranges_and_graph_exclusion(self):
        import json
        from unittest.mock import patch
        from rtp_llm.models_py.modules.factory.attention.cuda_mla_impl import mla_fp8_kernels as kernels
        x = torch.tensor([-1000.0, 200.0, float("nan"), float("inf"), 0.0], device="cuda")
    ...
    def test_decode_verify_fp8_kernel_page128(self):
        from rtp_llm.models_py.modules.factory.attention.cuda_mla_impl.tokenspeed_mla_impl import _load_tokenspeed_mla
        self.assertTrue(_load_tokenspeed_mla())
        from tokenspeed_mla.mla_decode import tokenspeed_mla_decode
        from rtp_llm.models_py.modules.factory.attention.cuda_mla_impl.tokenspeed_mla_impl import _get_tokenspeed_workspace
        for q_len in (1, 2, 3, 4):
            query = quantize_fp8(torch.randn(1, q_len, 12, 576, device="cuda").bfloat16() * 0.1, 0.5)
            cache = quantize_fp8(torch.randn(2, 128, 576, device="cuda").bfloat16() * 0.1, 0.25)
            ...
            workspace = _get_tokenspeed_workspace(torch.device('cuda', 0), 12, 512, q_len)
            tokenspeed_mla_decode(query=query, kv_cache=cache, workspace_buffer=workspace,
                                 kv_lora_rank=512, qk_rope_head_dim=64, block_tables=pages,
                                 seq_lens=lens, max_seq_len=256, softmax_scale=192**-0.5*0.5*0.25,
                                 output_scale=0.25, out=out, is_var_seq=True, causal_mask=True,
                                 enable_pdl=False)
            real_cache = torch.cat([cache[1].float(), cache[0].float()])[:193] * 0.25
            score = torch.einsum('qhd,kd->hqk', query[0].float()*0.5, real_cache) * 192**-0.5
            allowed = torch.arange(193, device='cuda')[None,:] <= (193-q_len+torch.arange(q_len, device='cuda'))[:,None]
            score.masked_fill_(~allowed, float('-inf'))
    ...
    def test_decode_adapter_graph_reuses_fp8_query_buffer(self):
        from types import SimpleNamespace
        from rtp_llm.models_py.modules.factory.attention.cuda_mla_impl.tokenspeed_mla_impl import TokenSpeedMlaDecodeOp
        ...
        kwargs = dict(num_heads=12, kv_lora_rank=512, qk_rope_head_dim=64,
                      qk_nope_head_dim=128, token_per_block=128,
                      softmax_extra_scale=1.0, weights=weights, max_bs=2,
                      max_q_len=4, max_context_len=256,
                      fp8_compute=True, q_scale=0.5, kv_scale=0.25)
        ...
        params = SimpleNamespace(
            ...
            decode_page_indptr_d=torch.tensor([0, 2, 4], dtype=torch.int32, device="cuda"),
            page_indice_d=torch.tensor([2, 0, 3, 1], dtype=torch.int32, device="cuda"),
        )
        eager.plan(params)
        captured.plan(params)
        cache = SimpleNamespace(kv_cache_base=quantize_fp8(torch.randn(4, 128, 576, device="cuda").bfloat16() * .1, .25))
        q = torch.randn(8, 12, 128, device="cuda").bfloat16() * .1
        rope = torch.randn(8, 12, 64, device="cuda").bfloat16() * .1
        for _ in range(3):
        ...
        for _ in range(5):
            ...
            torch.testing.assert_close(output, reference, atol=.0001, rtol=.001)
