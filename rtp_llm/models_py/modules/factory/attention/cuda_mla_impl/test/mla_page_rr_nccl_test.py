...
def _tp2_page_rr_worker(rank: int, init_port: int) -> None:
    ...
    try:
        ...
        MlaKVCacheWriteOp(KvCacheDataType.BASE).forward(
            compressed_kv,
            k_pe,
            SimpleNamespace(kv_cache_base=raw_cache),
        ...
        with patch.object(
            collective_torch, "all_gather_into", wraps=collective_torch.all_gather_into
        ) as gather:
            empty = adapter.read_prefix(raw_cache, block_table, (0, 0))
            assert empty.shape == (0, payload_features)
            assert gather.call_count == 0
            restored = adapter.read_prefix(raw_cache, block_table, _PREFIX_LENS)
...
class MlaPageRRNCCLTest(unittest.TestCase):
    def test_tp2_owner_write_gather_and_canonical_restore(self) -> None:
