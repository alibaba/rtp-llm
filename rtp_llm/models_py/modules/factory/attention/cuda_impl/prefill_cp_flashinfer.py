...
class ContextParallelFlashInferRaggedPrefillOp:
    ...
    def prepare(self, attention_inputs: PyAttentionInputs) -> ParamsBase:
        ...
        if self.rotate_method == CPRotateMethod.ALL_GATHER:
            ...
            self.q_part_0_indices, self.q_part_1_indices = self._generate_q_indices(
                prefill_cp_chunk_lengths
            )
            self.kv_part_0_indices, self.kv_part_1_indices = self._generate_kv_indices(
            ...
            self.half_q_indices = self._generate_half_q_indices(
                prefill_cp_chunk_lengths
            )
            self.half_kv_indices = self._generate_half_kv_indices(
                prefill_cp_chunk_lengths
            )
    ...
    def forward_all_gather(
        ...
        all_keys = all_gather(k, group=Group.CP).reshape(k.shape[0] * self.cp_size, -1)
        all_values = all_gather(v, group=Group.CP).reshape(
            k.shape[0] * self.cp_size, -1
        )
        retore_keys = all_keys[self.kv_restore_indices].reshape(
            -1, self.num_kv_heads, self.head_dim
        )
        retore_values = all_values[self.kv_restore_indices].reshape(
            -1, self.num_kv_heads, self.head_dim
        )
        q_reshaped = q.reshape(-1, self.num_qo_heads, self.head_dim)

        # Use contiguous() to ensure memory layout for FlashInfer
        q0 = q_reshaped[self.q_part_0_indices].contiguous()
        q1 = q_reshaped[self.q_part_1_indices].contiguous()
        k0 = retore_keys[self.kv_part_0_indices].contiguous()
        k1 = retore_keys[self.kv_part_1_indices].contiguous()
        v0 = retore_values[self.kv_part_0_indices].contiguous()
        v1 = retore_values[self.kv_part_1_indices].contiguous()
