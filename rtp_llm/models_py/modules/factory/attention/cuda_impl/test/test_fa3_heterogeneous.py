"""Combined FA3 heterogeneous-batch regression suites.

This module keeps both target-verify and draft-prefill heterogeneous replay
tests in a single file.

Run:
    bazelisk test //rtp_llm/models_py/modules/factory/attention/cuda_impl/test:test_fa3_heterogeneous
"""
...
NUM_TOKENS_PER_BS = NUM_DRAFT_TOKENS
...
def _build_target_verify_attn_inputs(
    *,
    ...
    * ``decode_cu_seqlens_d`` is provided externally so capture and replay
      share the same stable CUDA buffer, even though ``PyFA3PagedAttnOp``
      rebuilds ``cu_seqlens_q`` from ``input_lengths`` rather than reading
      this tensor directly.
    * ``zero_device_lengths``: when True, zero BOTH the host and device
      prefix/input-length tensors.  The current FA3 implementation rebuilds
      ``cache_seqlens`` and ``cu_seqlens_q`` from the host tensors during
      ``prepare()``, so zeroing only ``*_d`` would not reproduce the
      zero-work capture state these CG tests intend to model.
    ...
    if zero_device_lengths:
        inputs.prefix_lengths.zero_()
        inputs.input_lengths.zero_()
...
def _build_draft_prefill_attn_inputs(
    *,
    cache_seqlens: List[int],
    input_lengths: List[int],
    block_id_host: torch.Tensor,
...
def _expected_cu_seqlens(input_lengths: List[int]) -> List[int]:
    cu = [0]
    for il in input_lengths:
        cu.append(cu[-1] + il)
    return cu
...
class TestFA3TargetVerifyHeterogeneous(BaseAttentionTest):
    ...
    def _make_op_eager(self, attn_inputs: Any):
        from rtp_llm.models_py.modules.factory.attention.cuda_impl.py_fa3_paged import (
            PyFA3PagedAttnOp as PyFA3PagedTargetVerifyAttnOp,
        )
    ...
    def _run_scenario(
        ...
        attn_inputs_eager = _build_target_verify_attn_inputs(
        ...
        # Our FA3 path rebuilds runtime buffers from host prefix/input lengths,
        # so the fixture zeros those host tensors too. That means the captured
        # FA3 launch does ZERO actual attention
        ...
        attn_inputs_capture = _build_target_verify_attn_inputs(
        ...
        self.assertEqual(cg_op._cache_seqlens_buf.cpu().tolist(), [0] * bs)
        self.assertEqual(cg_op._cu_seqlens_q_buf.cpu().tolist(), [0] * (bs + 1))
        ...
        attn_inputs_replay = _build_target_verify_attn_inputs(
            cache_seqlens=cache_seqlens,
            input_lengths=input_lengths,
            block_id_host=block_id_host,
            block_id_device=block_id_device,
        ...
        attn_inputs_capture.prefix_lengths.copy_(attn_inputs_replay.prefix_lengths)
        attn_inputs_capture.input_lengths.copy_(attn_inputs_replay.input_lengths)
    ...
    def test_heterogeneous_multi_replay_reproduces_bug(self):
        ...
        # Build capture inputs the production way: zeroed host/device lengths +
        ...
        for it, replay_seqlens in enumerate(replay_patterns):
            ...
            replay_inputs = _build_target_verify_attn_inputs(
            ...
            attn_inputs_capture.prefix_lengths.copy_(replay_inputs.prefix_lengths)
            attn_inputs_capture.input_lengths.copy_(replay_inputs.input_lengths)
            ...
            eager_inputs = _build_target_verify_attn_inputs(
                cache_seqlens=replay_seqlens,
                input_lengths=input_lengths,
                block_id_host=block_id_host,
                block_id_device=block_id_device,
    ...
    def test_padded_batch_capture_smaller_replay_reproduces_bug(self):
        ...
        # Stable CUDA buffer carried through the same capture/replay plumbing
        # as production, even though this FA3 path rebuilds cu_seqlens_q from
        # host input_lengths during prepare().
        decode_cu_seqlens_d = torch.arange(
            0,
        ...
        # Capture-time inputs: production behavior — zero host/device lengths,
        ...
        cg_op.prepare(attn_inputs_capture)
        self.assertEqual(
            cg_op._cache_seqlens_buf.cpu().tolist(),
            replay_cache_seqlens,
        )
        self.assertEqual(
            cg_op._cu_seqlens_q_buf.cpu().tolist(),
            _expected_cu_seqlens(input_lengths_full),
        )
    ...
    def test_replay_reads_cache_seqlens_at_runtime(self):
        ...
        def replay_with(probe_seqlens: List[int]) -> torch.Tensor:
            ...
            probe_inputs = _build_target_verify_attn_inputs(
            ...
            attn_inputs_capture.prefix_lengths.copy_(probe_inputs.prefix_lengths)
            attn_inputs_capture.input_lengths.copy_(probe_inputs.input_lengths)
    ...
    def test_heterogeneous_eager_only_passes(self):
        ...
        attn_inputs = _build_target_verify_attn_inputs(
            cache_seqlens=cache_seqlens,
            input_lengths=input_lengths,
            block_id_host=block_id_host,
            block_id_device=block_id_device,
...
class TestFA3DraftPrefillHeterogeneous(BaseAttentionTest):
    ...
    def test_homogeneous_q_eager_passes(self):
        ...
        attn_inputs = _build_draft_prefill_attn_inputs(
            cache_seqlens=cache_seqlens,
            input_lengths=input_lengths,
    ...
    def test_heterogeneous_q_cg_replay_matches_eager(self):
        ...
        capture_cache_seqlens = [MAX_SEQ_LEN] * captured_bs
        attn_inputs_capture = _build_draft_prefill_attn_inputs(
            cache_seqlens=capture_cache_seqlens,
        ...
        replay_inputs = _build_draft_prefill_attn_inputs(
            cache_seqlens=replay_cache_seqlens,
        ...
        self.assertEqual(
            cg_op._cu_seqlens_q_buf.cpu().tolist(),
            _expected_cu_seqlens(replay_input_lengths),
        )
        ...
        eager_inputs = _build_draft_prefill_attn_inputs(
            cache_seqlens=replay_cache_seqlens,
            input_lengths=replay_input_lengths,
    ...
    def test_multi_replay_varying_q_patterns(self):
        ...
        for it, (input_lengths, cache_seqlens) in enumerate(replay_patterns):
            ...
            self.assertEqual(
                cg_op._cu_seqlens_q_buf.cpu().tolist(),
                _expected_cu_seqlens(input_lengths),
            )
    ...
    def test_cg_replay_reads_input_lengths_at_runtime(self):
        ...
        def replay_with(input_lengths, cache_seqlens):
            ...
            probe_inputs = _build_draft_prefill_attn_inputs(
                cache_seqlens=cache_seqlens,
                input_lengths=input_lengths,
                block_id_host=block_h,
                block_id_device=block_d,
