...
class BaseAttentionTest(unittest.TestCase):
    ...
    def _create_kv_cache_block_ids(
        ...
        # -1, not 0: block id 0 is a real page, so zero padding would make an
        # out-of-range read return plausible KV data instead of failing.
        kv_cache_block_id = torch.full(
            [batch_size, max_blocks], -1, dtype=torch.int32, device="cpu"
        )
