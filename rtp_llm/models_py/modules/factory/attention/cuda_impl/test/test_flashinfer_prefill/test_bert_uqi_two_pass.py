"""BERT two-pass attention parity tests, migrated from qui-two-pass."""
import random
import unittest

import torch

from rtp_llm.models_py.modules.factory.attention import bert_uqi_schedule as bm
...
from rtp_llm.models_py.modules.factory.attention.cuda_impl import bert_uqi_two_pass_core as core
    ...
    def test_vs_custom_mask_path(self):
        ...
        for dtype in (torch.float16, torch.bfloat16):
            for modes in (["mid", "none"], ["start", "mid", "nosep"]):
                ...
                cu = cu_host.tolist()
                cu_dev = cu_host.to(self.dev)
                mask = build_bert_uqi_flashinfer_mask(seg, cu_dev)
...
class TestBertOutputOrder(unittest.TestCase):
    def test_restores_original_token_order_for_heads(self):
        from types import SimpleNamespace
        from rtp_llm.models_py.model_desc.bert import BertModel

        hidden = torch.arange(12, dtype=torch.float32).reshape(6, 2)
        perm = torch.tensor([0, 1, 5, 2, 3, 4])
        seen = []
        ...
        model = SimpleNamespace(
            _reject_input_embeddings=lambda inputs: None,
            embed_tokens=lambda *args: hidden,
            pre_decoder_layernorm=lambda x: x,
            multimodal_embedding_injector=lambda x, *args: x,
            layers=[layer], layer_num=1, kv_cache=None,
        )
        inputs = SimpleNamespace(
            input_ids=torch.tensor([101, 10, 2, 15, 102, 200]),
            bert_embedding_inputs=SimpleNamespace(
                combo_position_ids=None, position_encoding=None,
                combo_tokens_type_ids=None, token_type_embedding=None,
                input_embedding_scalar=1.0,
            ),
            multimodal_inputs=SimpleNamespace(multimodal_features=[], mm_features_locs=None),
            embedding_inputs=SimpleNamespace(text_tokens_mask=None),
        )
        impl = SimpleNamespace(uqi_perm=perm, uqi_inv_perm=perm.argsort())
        result = BertModel.forward(model, inputs, impl)
        torch.testing.assert_close(seen[0], hidden[perm])
