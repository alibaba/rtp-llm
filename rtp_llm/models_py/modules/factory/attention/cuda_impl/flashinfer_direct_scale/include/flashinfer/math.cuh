...
// Cover FP32 logits while keeping empty-tile subtraction finite.
...
__forceinline__ __device__ uint32_t fp32_vec_to_e4m3(float const& f0,
                                                     float const& f1,
                                                     float const& f2,
                                                     float const& f3) {
...
__forceinline__ __device__ uint32_t fp32_vec_to_e2m1(float const& f0,
                                                     float const& f1,
                                                     float const& f2,
                                                     float const& f3,
                                                     float const& f4,
...
__forceinline__ __device__ uint32_t fp32_vec_to_e2m1(float const (&array)[8]) {
    return fp32_vec_to_e2m1(array[0], array[1], array[2], array[3], array[4], array[5], array[6], array[7]);
}

__forceinline__ __device__ uint32_t fp32_vec_to_e2m1(float2 const (&array)[4]) {
    return fp32_vec_to_e2m1(
...
__forceinline__ __device__ float shfl_xor_sync(float x, int lane_mask) {
    float y;
    asm volatile("shfl.sync.bfly.b32 %0, %1, %2, 0x1f, 0xffffffff;" : "=f"(y) : "f"(x), "r"(lane_mask));
    return y;
}
