...
class PyFlashinferPrefillPagedAttnOp(object):
    ...
    def prepare(
        ...
        if self.enable_cuda_graph and self.prefill_wrapper._qo_indptr_buf is None:
            ...
            # Page indices are a flat list addressed through kv_indptr, so their
            # length may vary. Replay forbids storage reallocation, and padded
            # requests have empty page ranges.
    ...
    def _plan_prefill_wrapper(self, qo_indptr: torch.Tensor) -> None:
        ...
        # Both native fill paths expose active tensor shapes separately from
        # reserved storage capacity; qo_indptr may include graph padding.
