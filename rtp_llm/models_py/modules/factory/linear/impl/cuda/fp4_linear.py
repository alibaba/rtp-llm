...
class CudaFp4GEMMLinear(LinearBase):
    ...
    def __init__(
        ...
        if self.backend == "sgl_cutlass" and not is_legacy_cutlass_fp4_available():
            raise RuntimeError(
                "RTP_LLM_FP4_GEMM_BACKEND=sgl_cutlass requires the legacy "
                "CUTLASS FP4 extension, which is compiled only for SM10x. "
                "Select a FlashInfer backend on SM120."
            )
