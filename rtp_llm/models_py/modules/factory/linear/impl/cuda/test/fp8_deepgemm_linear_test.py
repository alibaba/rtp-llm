...
class CudaFp8DeepGEMMLinearTest(unittest.TestCase):
    ...
    def _create_fp8_linear(self, with_bias: bool = False, with_bias_2d: bool = False):
    ...
    def test_module_creation(self):
        ...
        # Test with bias 2D
        fp8_linear_with_bias_2d = self._create_fp8_linear(with_bias_2d=True)
        self.assertEqual(fp8_linear_with_bias_2d.K, self.K)
        self.assertEqual(fp8_linear_with_bias_2d.N, self.N)
        self.assertIsNotNone(fp8_linear_with_bias_2d.weight)
        self.assertEqual(fp8_linear_with_bias_2d.weight.shape, (self.N, self.K))
        self.assertIsNotNone(fp8_linear_with_bias_2d.weight_scale)
        self.assertEqual(
            fp8_linear_with_bias_2d.weight_scale.shape, (self.scale_N, self.scale_K)
        )
        self.assertIsNotNone(fp8_linear_with_bias_2d.bias)
        self.assertEqual(fp8_linear_with_bias_2d.bias.dim(), 2)
        # Test with bias 1D
        fp8_linear_with_bias = self._create_fp8_linear(
            with_bias=True, with_bias_2d=False
        )
        self.assertEqual(fp8_linear_with_bias.K, self.K)
        self.assertEqual(fp8_linear_with_bias.N, self.N)
        self.assertIsNotNone(fp8_linear_with_bias.weight)
        self.assertEqual(fp8_linear_with_bias.weight.shape, (self.N, self.K))
        self.assertIsNotNone(fp8_linear_with_bias.weight_scale)
        self.assertEqual(
            fp8_linear_with_bias.weight_scale.shape, (self.scale_N, self.scale_K)
        )
        self.assertIsNotNone(fp8_linear_with_bias.bias)
        self.assertEqual(fp8_linear_with_bias.bias.dim(), 1)
        # Test without bias
        ...
        self.assertIsNotNone(fp8_linear_no_bias.weight)
        self.assertEqual(fp8_linear_no_bias.weight.shape, (self.N, self.K))
        self.assertIsNotNone(fp8_linear_no_bias.weight_scale)
        self.assertEqual(
            fp8_linear_no_bias.weight_scale.shape, (self.scale_N, self.scale_K)
        )
    ...
    def test_weight_validation(self):
        ...
        self.assertIn(
            f"got weight dim: {weight.dim()} and weight scale dim: {self.weight_scale.dim()}",
    ...
    def test_weight_scale_validation(self):
        ...
        self.assertIn(
            f"got weight dim: {self.weight.dim()} and weight scale dim: {weight_scale.dim()}",
            str(context.exception),
        )
        weight_scale = torch.randn(
        ...
        self.assertIn(f"got {weight_scale.dtype}", str(context.exception))
    ...
    def test_output_correctness(self):
        ...
        weight_fp8, weight_scale = per_block_cast_to_fp8(weight_bf16, use_ue8m0=False)
        weight_fp8 = weight_fp8.reshape(self.K, self.N)
        weight_scale = weight_scale.reshape(self.scale_K, self.scale_N)
        # Initialize FP8 linear layer
        fp8_linear = Fp8PerBlockLinear(weight_fp8, weight_scale)
    ...
    def test_bias_handling(self):
        ...
        # Initialize FP8 linear layer
        fp8_linear = Fp8PerBlockLinear(weight_fp8, weight_scale, bias_bf16)
        # Test some batch sizes
        for test_batch_size in self.test_batch_sizes:
            with self.subTest(batch_size=test_batch_size):
                input_tensor = torch.randn(
