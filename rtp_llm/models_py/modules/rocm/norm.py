...
from typing import Optional, Dict, TypedDict, Tuple, Any, Union
from aiter import rmsnorm2d_fwd as rms_norm
from aiter import layernorm2d_fwd as layernorm2d_fwd
from aiter import layernorm2d_fwd_with_add as fused_add_layer_norm
...
class AddBiasResLayerNorm(BaseAddBiasResLayerNorm):
    ...
    def forward(
        ...
        if hidden_states.shape[0] > 32 and hidden_states.shape[1] <= 768:
            return layernorm2d_fwd(hidden_states, self.weight.data, bias, self.variance_epsilon, x_bias = None)
        else:
...
class FusedQKRMSNorm(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        rtp_llm_ops.fused_qk_rmsnorm(hidden_states, self.q_weight, self.k_weight, self.eps, self.head_num, self.kv_head_num, m, n, self.size_per_head)
        return hidden_states
