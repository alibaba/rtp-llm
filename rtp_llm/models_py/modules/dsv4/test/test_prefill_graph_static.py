...
from rtp_llm.models_py.modules.dsv4.prefill_island_graph import _BridgeState
from rtp_llm.models_py.modules.dsv4.prefill_island_graph import PrefillIslandGraphManager
...
class _FakeCudaTensor:
    def __init__(self, shape, *, dtype=torch.bfloat16, device="cuda:0", ptr=1):
...
class PrefillIslandGraphManagerTest(unittest.TestCase):
    def test_attn_out_direct_returns_existing_static_input(self):
        manager = PrefillIslandGraphManager()
        layer = object()
        attn_out = _FakeCudaTensor((64, 7168), ptr=31)
        residual = _FakeCudaTensor((64, 4, 7168), ptr=32)
        post = _FakeCudaTensor((64, 4, 1), dtype=torch.float32, ptr=33)
        comb = _FakeCudaTensor((64, 4, 4), dtype=torch.float32, ptr=34)
        static_attn_out = _FakeCudaTensor((64, 7168), ptr=301)
        key = manager._attn_ffn_bridge_key(
            layer,
            attn_out,
            residual,
            post,
        ...
        manager._bridge_states[key] = _BridgeState(
            static_inputs=(static_attn_out, residual, post, comb),
        ...
        out = manager.attn_post_ffn_pre_static_attn_out(
            layer=layer,
            attn_out_template=attn_out,
            residual=residual,
            post=post,
            comb=comb,
        )

        self.assertIs(out, static_attn_out)

    def test_attn_out_direct_returns_none_when_state_missing_or_shape_mismatch(self):
        manager = PrefillIslandGraphManager()
        layer = object()
        attn_out = _FakeCudaTensor((64, 7168), ptr=41)
        residual = _FakeCudaTensor((64, 4, 7168), ptr=42)
        post = _FakeCudaTensor((64, 4, 1), dtype=torch.float32, ptr=43)
        comb = _FakeCudaTensor((64, 4, 4), dtype=torch.float32, ptr=44)

        self.assertIsNone(
            manager.attn_post_ffn_pre_static_attn_out(
                layer=layer,
                attn_out_template=attn_out,
                residual=residual,
                post=post,
        ...
        manager._bridge_states[key] = _BridgeState(
            static_inputs=(_FakeCudaTensor((63, 7168), ptr=401), residual, post, comb),
            static_freqs_cis=None,
            static_indexer_freqs_cis=None,
            graph=object(),
            outputs=(),
    ...
    def test_ffn_out_direct_returns_existing_static_input(self):
        manager = PrefillIslandGraphManager()
        current_layer = object()
        next_layer = object()
        ffn_out = _FakeCudaTensor((64, 7168), ptr=11)
        residual = _FakeCudaTensor((64, 4, 7168), ptr=12)
        post = _FakeCudaTensor((64, 4, 1), dtype=torch.float32, ptr=13)
        comb = _FakeCudaTensor((64, 4, 4), dtype=torch.float32, ptr=14)
        static_ffn_out = _FakeCudaTensor((64, 7168), ptr=101)
        key = manager._bridge_key(
            current_layer,
            next_layer,
            ffn_out,
            residual,
        ...
        manager._bridge_states[key] = _BridgeState(
            static_inputs=(static_ffn_out, residual, post, comb),
            static_freqs_cis=None,
            static_indexer_freqs_cis=None,
            graph=object(),
            outputs=(),
        ...
        out = manager.ffn_post_attn_pre_static_ffn_out(
            current_layer=current_layer,
            next_layer=next_layer,
            ffn_out_template=ffn_out,
            residual=residual,
            post=post,
            comb=comb,
        )

        self.assertIs(out, static_ffn_out)

    def test_ffn_out_direct_returns_none_when_state_missing_or_shape_mismatch(self):
        manager = PrefillIslandGraphManager()
        current_layer = object()
        next_layer = object()
        ffn_out = _FakeCudaTensor((64, 7168), ptr=21)
        residual = _FakeCudaTensor((64, 4, 7168), ptr=22)
        post = _FakeCudaTensor((64, 4, 1), dtype=torch.float32, ptr=23)
        comb = _FakeCudaTensor((64, 4, 4), dtype=torch.float32, ptr=24)

        self.assertIsNone(
            manager.ffn_post_attn_pre_static_ffn_out(
                current_layer=current_layer,
                next_layer=next_layer,
                ffn_out_template=ffn_out,
                residual=residual,
        ...
        manager._bridge_states[key] = _BridgeState(
            static_inputs=(_FakeCudaTensor((63, 7168), ptr=201), residual, post, comb),
            static_freqs_cis=None,
            static_indexer_freqs_cis=None,
            graph=object(),
            outputs=(),
