...
from rtp_llm.models_py.modules.dsv4.attention import Attention, WorkspaceMeta
...
def _build_meta_legacy(
    ...
    """Legacy B==1 / DSV4_VARLEN_PREFILL=0 — call without varlen tensors so
    the dispatch falls into the scalar branch even when env=1."""
...
class BuildWorkspaceMetaLegacyTest(unittest.TestCase):
    ...
    def test_env_disable_forces_legacy_even_with_b2(self) -> None:
        """``DSV4_VARLEN_PREFILL=0`` MUST force legacy regardless of B>1.
        Tests the `_use_varlen_prefill()` env gate."""
        stub = _make_stub(win=8, compress_ratio=4, n_reqs=2)
        with mock.patch.dict(os.environ, {"DSV4_VARLEN_PREFILL": "0"}):
            m = _build_meta_varlen(stub, [0, 8], [4, 6], with_dense_cmp_topk=False)
        # Legacy branch runs with sp_int=prefix_lengths[0]=0, seqlen=4+6=10.
        # Expected: N=(0+10)//4=2, P=0, gather=10, M=12.
        self.assertEqual(m.M, 12)
        self.assertEqual(m.N, 2)
        # swa_seq_lens is [seq_total]=[10] (single-element legacy).
        self.assertEqual(m.swa_seq_lens.shape, (1,))
        self.assertEqual(int(m.swa_seq_lens.item()), 10)
