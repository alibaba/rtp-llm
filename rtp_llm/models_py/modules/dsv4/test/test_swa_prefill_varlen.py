...
Phase-2 of the dsv4 prefill varlen migration. Validates:
  ...
  2. ``_build_swa_prefill_meta`` (explicit ``use_varlen`` kwarg) —
     per-request meta fields with bit-equality to the ``use_varlen=False``
     legacy scalar branch on B==1 inputs (where the formulas collapse).
...
class _StubAttention:
    """Minimal stand-in for ``Attention`` exposing only what
    ``_build_swa_prefill_meta`` actually reads:
    ...
    # Bind the unbound methods so the stub quacks correctly. The varlen /
    # legacy splits are private impls dispatched to from the public entry —
    # bind both so both code paths are reachable from the stub.
    _build_swa_prefill_meta = Attention._build_swa_prefill_meta
...
class BuildSwaPrefillMetaVarlenTest(unittest.TestCase):
    ...
    def _build_meta_legacy(
        ...
        """Legacy B==1 scalar branch — explicit ``use_varlen=False``.
        Bit-equality oracle for the varlen path's B==1 collapse."""
        return stub._build_swa_prefill_meta(
            seqlen=S, sp_int=sp, device=self.device, use_varlen=False
        )
