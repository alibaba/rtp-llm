"""CPU regression for post-gather SM120 fused-MoE workspace capacity.

Capacity must cover world * padded_rows, not just the per-rank admission budget.
Cover multi-rank decode, single-rank execution and the 512-row tiling boundary.
"""
...
class CapacityCoversPostGatherTileTest(unittest.TestCase):
    """Compare per-rank budgets with post-gather tiles for DP4, MTP-3."""

    # (admission cap, capture batch, result with per-rank-only sizing)
    CASES = [
        (4, 1, "pass"),    # budget 16 == tile 16
