...
from rtp_llm.models_py.modules.dsv4.hc import mhc_tilelang
...
class PrefillSmallTokenBypassTest(unittest.TestCase):
    ...
    def test_empty_fresh_workspace_gate_only_skips_b1_no_continuation(self):
        from rtp_llm.models_py.modules.dsv4.fp8 import attention

        with patch.dict(
            attention.os.environ,
            {"DSV4_PREFILL_EMPTY_FRESH_WORKSPACE": "1"},
        ):
            cp_fresh = SimpleNamespace(
                any_cont=False,
                cp_ctx=SimpleNamespace(cp_size=8, prefix_length=0),
                prefix_lengths=torch.zeros(1, dtype=torch.int32),
            )
            self.assertFalse(
                attention._prefill_workspace_needs_zero_init(
                    cp_fresh, batch_size=1
                )
            )
            self.assertFalse(
                attention._prefill_workspace_needs_zero_init(
                    SimpleNamespace(any_cont=False), batch_size=1
                )
            )
            prefix_reuse = SimpleNamespace(
                any_cont=True,
                cp_ctx=SimpleNamespace(cp_size=8, prefix_length=17),
                prefix_lengths=torch.tensor([17], dtype=torch.int32),
            )
            self.assertTrue(
                attention._prefill_workspace_needs_zero_init(
                    prefix_reuse, batch_size=1
                )
            )
            self.assertTrue(
                attention._prefill_workspace_needs_zero_init(
                    SimpleNamespace(any_cont=True), batch_size=1
                )
            )
            self.assertTrue(
                attention._prefill_workspace_needs_zero_init(
                    SimpleNamespace(any_cont=False), batch_size=2
                )
            )

        with patch.dict(attention.os.environ, {}, clear=True):
            self.assertTrue(
                attention._prefill_workspace_needs_zero_init(
                    SimpleNamespace(any_cont=False), batch_size=1
                )
    ...
    def test_tk_mhc_post_fwd_out_bypasses_autograd_wrapper(self):
        calls = []

        class FakePostOpsModule:
            @staticmethod
            def mhc_post_fwd(x, residual, post_mix, comb_mix, out=None):
                calls.append((x, residual, post_mix, comb_mix, out))
                return out

        x = torch.ones(1, 2, 8, dtype=torch.bfloat16)
        residual = torch.ones(1, 2, 4, 8, dtype=torch.bfloat16)
        post = torch.ones(1, 2, 4, 1, dtype=torch.float32)
        comb = torch.ones(1, 2, 4, 4, dtype=torch.float32)
        out = torch.empty_like(residual)

        with patch.object(
            mhc_tilelang, "_can_use_tk", lambda residual, hc_mult: True
        ), patch.object(mhc_tilelang, "_tk_mhc_post_ops_mod", FakePostOpsModule):
            result = mhc_tilelang.tk_mhc_post_fwd_out(
                x,
                residual,
                post,
                comb,
        ...
        self.assertEqual(calls, [(x, residual, post, comb, out)])

    def test_tk_mhc_pre_out_reuses_exact_shape_workspace(self):
        class FakePreBigFuseModule:
            @staticmethod
            ...
            def mhc_pre_big_fuse_out(
                ...
                *,
                ...
                residual_flat = residual.view(
                    -1, residual.shape[-2], residual.shape[-1]
                )
                layer_input.copy_(residual_flat[:, 0, :])
                post_mix.fill_(1.0)
                comb_mix.fill_(2.0)
                outer_shape = residual.shape[:-2]
                return (
                    post_mix.view(*outer_shape, residual.shape[-2], 1),
                    comb_mix.view(*outer_shape, residual.shape[-2], residual.shape[-2]),
                    layer_input.view(*outer_shape, residual.shape[-1]),
                )

        residual = torch.ones(1, 2, 4, 8, dtype=torch.bfloat16)
        fn = torch.ones(24, 32, dtype=torch.float32)
        scale = torch.ones(3, dtype=torch.float32)
        base = torch.ones(24, dtype=torch.float32)
        workspace = {}

        with patch.object(
            mhc_tilelang, "_can_use_tk", lambda residual, hc_mult: True
        ), patch.object(mhc_tilelang, "_tk_mhc_pre_big_fuse_mod", FakePreBigFuseModule):
            out1 = mhc_tilelang.tk_mhc_pre_out(
                residual,
                fn,
                scale,
                base,
            ...
            out1_cloned = tuple(t.clone() for t in out1)
            first_layer_input = workspace["layer_input"]
            out2 = mhc_tilelang.tk_mhc_pre_out(
                residual,
                fn,
                scale,
                base,
        ...
        self.assertIs(workspace["layer_input"], first_layer_input)
        self.assertTrue(torch.equal(out1_cloned[0], out2[0]))
        self.assertEqual(tuple(out1[0].shape), (1, 2, 8))
        self.assertEqual(tuple(out1[1].shape), (1, 2, 4, 1))
        self.assertEqual(tuple(out1[2].shape), (1, 2, 4, 4))

    def test_tk_mhc_pre_out_unchecked_reuses_compiled_launcher(self):
        calls = []
        ...
        class FakePreBigFuseModule:
            ...
            def _run_tilelang_single_gemm(
                ...
                calls.append("gemm")
                gemm_out_mul.fill_(3.0)
                gemm_out_sqrsum.fill_(4.0)
                return 1
            ...
            def _mhc_pre_big_fuse(
                ...
                *,
                ...
                calls.append(("compile", hidden_size, n_splits, mhc_mult))
            ...
            @staticmethod
            def mhc_pre_big_fuse_out(*args, **kwargs):
                raise AssertionError("checked mhc_pre_big_fuse_out should be bypassed")
        ...
        with patch.dict(
            mhc_tilelang.os.environ, {"DSV4_MHC_PRE_OUT_UNCHECKED": "1"}
        ), patch.object(
            mhc_tilelang, "_can_use_tk", lambda residual, hc_mult: True
        ), patch.object(mhc_tilelang, "_tk_mhc_pre_big_fuse_mod", FakePreBigFuseModule):
            out1 = mhc_tilelang.tk_mhc_pre_out(
            ...
            launcher = workspace["pre_big_fuse_kernel"]
            out2 = mhc_tilelang.tk_mhc_pre_out(
                residual,
                fn,
                scale,
        ...
        self.assertIs(workspace["pre_big_fuse_kernel"], launcher)
        self.assertEqual(calls.count("gemm"), 2)
        self.assertEqual(calls.count("kernel"), 2)
        self.assertEqual(
        ...
        self.assertTrue(torch.equal(out2[1], torch.full_like(out2[1], 1.0)))
        self.assertTrue(torch.equal(out2[2], torch.full_like(out2[2], 2.0)))

    def test_tk_mhc_pre_out_rebuilds_workspace_on_shape_change(self):
        class FakePreBigFuseModule:
            @staticmethod
            def _requested_backend():
                return "tilelang_single"
            ...
            def mhc_pre_big_fuse_out(
                ...
                *,
                ...
                layer_input.fill_(float(residual.shape[1]))
                post_mix.fill_(1.0)
                comb_mix.fill_(2.0)
                outer_shape = residual.shape[:-2]
                return (
        ...
        with patch.object(
            ...
            out_2 = mhc_tilelang.tk_mhc_pre_out(
                torch.ones(1, 2, 4, 8, dtype=torch.bfloat16),
                fn,
                scale,
                base,
                norm_eps=1e-6,
            ...
            buffer_2 = workspace["layer_input"]
            out_3 = mhc_tilelang.tk_mhc_pre_out(
                torch.ones(1, 3, 4, 8, dtype=torch.bfloat16),
                fn,
                scale,
                base,
                norm_eps=1e-6,
            ...
            buffer_3 = workspace["layer_input"]
            out_2_again = mhc_tilelang.tk_mhc_pre_out(
                torch.ones(1, 2, 4, 8, dtype=torch.bfloat16),
                fn,
                scale,
                base,
        ...
        self.assertIsNot(buffer_2, buffer_3)
        self.assertEqual(tuple(out_2[0].shape), (1, 2, 8))
        self.assertEqual(tuple(out_3[0].shape), (1, 3, 8))
        self.assertEqual(tuple(out_2_again[0].shape), (1, 2, 8))
        self.assertTrue(
            torch.equal(out_2_again[0], torch.full_like(out_2_again[0], 2.0))
        )
