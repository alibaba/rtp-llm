...
class CompressorVarlenRawPathTest(unittest.TestCase):
    ...
    def test_batched_eq_per_request(self) -> None:
        ...
        # directly so we can compare against the per-req reference).
        x32 = x_flat.float()
        out_dim = (1 + cmp_a.overlap) * cmp_a.head_dim
        fused_out = torch.nn.functional.linear(x32, cmp_a._wkv_wgate_fused)
        ...
        for b in range(2):
            x_b = x_per_req[b]
            x32_b = x_b.float()
            fused_b = torch.nn.functional.linear(x32_b, cmp_b._wkv_wgate_fused)
