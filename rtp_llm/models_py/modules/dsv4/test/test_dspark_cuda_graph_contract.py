...
class DSparkCommitOnlyConstructionTest(unittest.TestCase):
    ...
    def test_constructor_matches_prefill_descriptor_role(self):
        ...
        for role, want in (
            (RoleType.PREFILL, True),
            ("PREFILL", True),
            (RoleType.DECODE, False),
            (RoleType.PDFUSION, False),
        ):
            with (
    ...
    def test_pruned_globals_require_no_markov_head(self):
        ...
        weights = SimpleNamespace(
            global_weights={
                ...
                W.v4_dspark_main_proj_w: object(),
                W.v4_dspark_main_proj_s: object(),
            }
        )
        with (
            patch.object(dspark_model_module, "RMSNorm", return_value=object()),
