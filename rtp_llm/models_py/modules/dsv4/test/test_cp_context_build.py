...
def test_b_gt_1_fresh_nonprefix_restore_matches_workspace_out() -> None:
    """Fresh and caller-owned destinations must preserve restore row ordering."""
    ...
    fresh_result = _CP._cp_restore_gathered_full_2d(gathered, ctx, out=None)
    assert fresh_result.data_ptr() != out.data_ptr()
    assert torch.equal(fresh_result, workspace_result)
    assert torch.equal(fresh_result, gathered[ctx.unpad_restore])
