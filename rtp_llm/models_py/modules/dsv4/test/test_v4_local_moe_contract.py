...
class LocalLoopRoutedStorageTest(unittest.TestCase):
    def test_canonical_fp4_weights_are_consumed_without_payload_copies(self):
        ...
        cfg = SimpleNamespace(
            ...
            swiglu_limit=10,
            tp_size=1,
            model_config=SimpleNamespace(model_type="deepseek_v4"),
        )
        ...
        class FakeExpert(nn.Module):
            def __init__(self, *_args, expert_weights, **_kwargs):
                ...
                router_order.append(_kwargs["router_weight_after_w2"])
        ...
        self.assertFalse(strategy._router_weight_after_w2)
        self.assertEqual(router_order, [False, False])
...
class LocalLoopFastPathQuantizationSpyTest(unittest.TestCase):
    def _strategy(self, router_weight_after_w2: bool) -> LocalLoopStrategy:
        strategy = LocalLoopStrategy.__new__(LocalLoopStrategy)
        nn.Module.__init__(strategy)
        strategy._router_weight_after_w2 = router_weight_after_w2
    ...
    def _run(
        ...
        n_tokens: int,
        route_weight: float,
        router_weight_after_w2: bool,
    ) -> tuple[torch.Tensor, list[torch.Tensor]]:
        strategy = self._strategy(router_weight_after_w2)
    ...
    def test_bs1_and_bsn_w2_quant_inputs_ignore_router_weight(self):
        for method_name, n_tokens in (
            ...
            with self.subTest(method=method_name):
                y_a, inputs_a = self._run(method_name, n_tokens, 0.25, True)
                y_b, inputs_b = self._run(method_name, n_tokens, 0.75, True)
    ...
    def test_bs1_and_bsn_router_order_matches_quantized_reference(self):
        gate = torch.tensor(0.8, dtype=torch.bfloat16).float()
        up = torch.tensor(0.9, dtype=torch.bfloat16).float()
        activation = F.silu(gate) * up
        for method_name, n_tokens in (
            ("_forward_topk_bs1", 1),
            ("_forward_topk_bsN", 2),
        ):
            for after_w2 in (False, True):
                for weight in (0.0, 0.25, 0.75, 1.0):
                    with self.subTest(
                        method=method_name, after_w2=after_w2, weight=weight
                    ):
                        y, inputs = self._run(
                            method_name, n_tokens, weight, after_w2
                        )
                        expected_input = (
                            activation if after_w2 else activation * weight
                        ).to(torch.bfloat16)
                        w2_inputs = inputs[1::2]
                        self.assertEqual(len(w2_inputs), n_tokens)
                        for actual in w2_inputs:
                            self.assertTrue(
                                torch.equal(actual, expected_input.expand(1, 3))
                            )
                        quantized = (torch.round(expected_input.float() * 4.0) / 4.0).to(
                            torch.bfloat16
                        )
                        expected = (
                            (quantized.float() * 2.0).to(torch.bfloat16).float()
                        )
                        if after_w2:
                            expected = expected * weight
                        self.assertTrue(torch.equal(y, expected.expand(n_tokens, 2)))
