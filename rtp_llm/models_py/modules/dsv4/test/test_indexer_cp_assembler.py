...
def test_copy_actual_indexer_k_to_padded_supports_cuda_fp8():
    ...
    actual_u8 = (
        torch.arange(17 * 2, dtype=torch.uint8, device="cuda").reshape(17, 2) + 1
    )
    actual_q = actual_u8.view(torch.float8_e4m3fn)
    actual_s = torch.arange(17, dtype=torch.uint8, device="cuda").reshape(17, 1) + 101
    padded_q = torch.zeros((32, 2), dtype=torch.float8_e4m3fn, device="cuda")
    padded_s = torch.zeros((32, 1), dtype=torch.uint8, device="cuda")
    ...
    expected_q = torch.zeros((32, 2), dtype=torch.uint8, device="cuda")
    expected_s = torch.zeros((32, 1), dtype=torch.uint8, device="cuda")
    expected_q[0:1] = actual_u8[0:1]
    expected_s[0:1] = actual_s[0:1]
    expected_q[16:32] = actual_u8[1:17]
    expected_s[16:32] = actual_s[1:17]
    assert torch.equal(padded_q.view(torch.uint8), expected_q)
    assert torch.equal(padded_s, expected_s)
