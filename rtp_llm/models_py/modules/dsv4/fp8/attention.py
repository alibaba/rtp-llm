...
def _prefill_empty_fresh_workspace_enabled() -> bool:
    return os.environ.get("DSV4_PREFILL_EMPTY_FRESH_WORKSPACE", "0") == "1"


def _prefill_workspace_needs_zero_init(common: Any, batch_size: int) -> bool:
    # ``common.any_cont`` is computed once while building prefill meta. Do not
    # re-read prefix tensors here: this helper runs per compressed layer.
    if (
        _prefill_empty_fresh_workspace_enabled()
        and int(batch_size) == 1
        and not bool(getattr(common, "any_cont", True))
    ):
        return False
    return True
...
class AttentionFP8(nn.Module):
    ...
    def _cleanup_pending_prefill_gather(
        ...
        with suppress(Exception):
            wait_fn(pending)
        release_fn = getattr(compressor, "release_prefill_pending_workspace", None)
    ...
    def _attn_via_workspace(
        ...
        with record_function_range("dsv4.fp8.attn.workspace.alloc"):
            workspace_shape = (B, wm.M, D)
            needs_zero_init = _prefill_workspace_needs_zero_init(common, B)
            if needs_zero_init:
                workspace = torch.zeros(
                    workspace_shape, dtype=torch.bfloat16, device=qkv.q.device
                )
            else:
