...
@triton.jit
def _pack_slots_copy_u64(
    src_data,
    dst_data,
    valid,
    n_data_u64: tl.constexpr,
):
    """Copy 72 uint64 without a 128-wide masked load."""
    off0 = tl.arange(0, 64)
    data0 = tl.load(src_data + off0, mask=valid, other=0)
    tl.store(dst_data + off0, data0, mask=valid)
    off1 = 64 + tl.arange(0, 16)
    tail = (off1 < n_data_u64) & valid
    data1 = tl.load(src_data + off1, mask=tail, other=0)
    tl.store(dst_data + off1, data1, mask=tail)


@triton.jit
def _pack_slots_zero_u64(dst_data, n_data_u64: tl.constexpr):
    off0 = tl.arange(0, 64)
    tl.store(dst_data + off0, tl.zeros((64,), dtype=tl.uint64))
    off1 = 64 + tl.arange(0, 16)
    tl.store(
        dst_data + off1,
        tl.zeros((16,), dtype=tl.uint64),
        mask=off1 < n_data_u64,
    )
...
def _pack_slots_to_paged_kernel(
    ...
    Grid-stride over dest slots so CUDA-graph launch size stays bounded
    when capture pads HCA width to 8K. ``HAS_LENS`` skips those tail
    columns without shrinking the captured dest tensor.
    """
    pid = tl.program_id(0).to(tl.int64)
    n_prog = tl.num_programs(0).to(tl.int64)
    row = pid
    while row < n_dst:
        ...
        skip = False
        if HAS_LENS and in_slots:
            col = row - (row // width) * width
            limit = tl.load(lens_ptr + (row // width))
            skip = col >= limit
        if skip:
            ...
            src_scale = (src_block_ptr + src_page * token_data_size + src_pos * 8).to(
                tl.pointer_type(tl.uint64)
            )
            dst_scale = (dst_block_ptr + dst_page * token_data_size + dst_pos * 8).to(
                tl.pointer_type(tl.uint64)
            )

            if valid:
                _pack_slots_copy_u64(src_data, dst_data, valid, n_data_u64)
                tl.store(dst_scale, tl.load(src_scale))
            else:
                _pack_slots_zero_u64(dst_data, n_data_u64)
                tl.store(dst_scale, tl.zeros((), dtype=tl.uint64))
            if in_slots:
                remapped = tl.where(
                    valid, row.to(tl.int32), tl.zeros((), dtype=tl.int32)
                )
...
def pack_slots_to_paged(
    ...
    ``row_lens`` is optional ``[rows]`` — columns ``>= lens[row]`` skip the
    584B copy (CUDA-graph dest size stays the capture width).
    ...
    width = int(slot_indices.shape[-1]) if slot_indices.numel() else 1
    ...
    has_lens = row_lens is not None
    if has_lens:
        n_rows = max(slot_count // max(width, 1), 1)
        lens = row_lens.reshape(-1).to(dtype=torch.int32)
        if not lens.is_contiguous():
            lens = lens.contiguous()
        if int(lens.numel()) != n_rows:
            raise ValueError(
                f"row_lens has {int(lens.numel())} rows; expected {n_rows}"
            )
    else:
        lens = slots
    # Bound the captured grid. Decode graphs pin n_dst to the capture
    # width (HCA 8K); launching one CTA per dest slot spends more time
    # on empty tail CTAs than on the real 584B gathers.
    n_prog = n_dst if n_dst <= 4096 else 4096
    _pack_slots_to_paged_kernel[(n_prog,)](
        k_cache,
        packed,
        slots,
        remapped,
