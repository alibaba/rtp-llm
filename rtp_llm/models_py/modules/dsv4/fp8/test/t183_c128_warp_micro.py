...
from rtp_llm.platforms.ppu.models.dsv4.ppu_provider import register_m890p_dsv4_provider
...
def main() -> None:
    ...
    token_to_req = torch.arange(
        N_REQ, dtype=torch.int32, device="cuda"
    ).repeat_interleave(SEQ)
    state_slots = torch.zeros(N, dtype=torch.int64, device="cuda")
    block_table = torch.zeros(
        (N_REQ, SEQ // STATE_PAGE), dtype=torch.int32, device="cuda"
    )
    ...
    state_cache = torch.zeros(
        (2, STATE_PAGE, 2 * HEAD), dtype=torch.float32, device="cuda"
    )
    kv_raw = (
        torch.randn((N, HEAD), dtype=torch.float32, device="cuda", generator=generator)
        * 0.1
    )
    score_raw = (
        torch.randn((N, HEAD), dtype=torch.float32, device="cuda", generator=generator)
    ...
    ape = (
        torch.randn(
            (RATIO, HEAD), dtype=torch.float32, device="cuda", generator=generator
        )
        * 0.1
    )
    ...
    try:
        for num_warps in (4, 8, 16):
            ...
            byte_equal = (
                True if reference is None else bool(torch.equal(output, reference))
            )
