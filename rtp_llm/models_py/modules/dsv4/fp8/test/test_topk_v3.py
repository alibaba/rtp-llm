...
_HAS_HISTOGRAM_OP = hasattr(
    rtp_llm_ops, "topk_v3_from_histogram"
)
...
def test_special_float_values_exact():
    ...
    if _HAS_HISTOGRAM_OP:
        hist_out = _run_from_histogram(logits, lengths, k, seq_len)
        assert torch.equal(
            hist_out.sort(dim=1).values, expected.sort(dim=1).values
        )
    print("  [special float values] direct/histogram exact index sets OK")
...
def test_length_clamp_and_output_guards():
    ...
    if _HAS_HISTOGRAM_OP:
        clamped_lengths = lengths.clamp(min=0, max=max_seq_len)
        histograms = _ordered_fp16_histogram(logits, clamped_lengths)
        ...
        _assert_clamped_value_equiv(
            ...
            k,
            max_seq_len,
            tag="histogram invalid-length clamp",
        )
        assert (storage[rows:] == sentinel).all()
...
def test_empty_batch_is_a_noop():
    ...
    if _HAS_HISTOGRAM_OP:
        histograms = torch.empty(
            (0, 1024), dtype=torch.int32, device="cuda"
        )
        rtp_llm_ops.topk_v3_from_histogram(
            logits,
            lengths,
