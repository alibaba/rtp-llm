...
from rtp_llm.models_py.modules.dsv4.fp8._kv_cache_utils import (
    ...
    require_physical_pool_tokens_per_block,
...
class PoolSlotMappingSplitTest(unittest.TestCase):
    def test_require_pool_tokens_per_block_uses_semantic_tag_geometry(self) -> None:
        class FakeKVCache:
        ...
        cache = FakeKVCache()
        self.assertEqual(require_pool_tokens_per_block(cache, region=HCA_KV), 128)
        self.assertEqual(require_pool_tokens_per_block(cache, region=SWA_KV), 1024)
        self.assertEqual(
            require_physical_pool_tokens_per_block(cache, region=HCA_KV), 2048
        )

    def test_kernel_block_table_tokens_uses_smallest_tag_geometry(self) -> None:
        class FakeKVCache:
            group_tags = [TAG_BY_ATTN_TYPE[HCA_KV], TAG_BY_ATTN_TYPE[SWA_KV]]
        ...
        self.assertEqual(
            require_kernel_block_table_tokens_per_block(FakeKVCache()), 128
        )

    def test_kernel_block_table_tokens_rejects_non_positive_tag(self) -> None:
        class FakeKVCache:
            group_tags = [TAG_BY_ATTN_TYPE[HCA_KV], TAG_BY_ATTN_TYPE[SWA_KV]]

            def get_kernel_seq_size_per_block(self, tag: str) -> int:
                return 0 if tag == TAG_BY_ATTN_TYPE[SWA_KV] else 128

        with self.assertRaisesRegex(RuntimeError, "positive for every group"):
            require_kernel_block_table_tokens_per_block(FakeKVCache())
    ...
    def test_require_pool_tokens_per_block_rejects_unknown_region(self) -> None:
        ...
        with self.assertRaisesRegex(RuntimeError, "does not contain region tag"):
