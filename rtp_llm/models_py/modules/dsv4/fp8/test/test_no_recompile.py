...
class NoRecompileTest(unittest.TestCase):
    ...
    def test_cp_rank_major_slot_dequant_no_recompile(self):
        """Rank-major direct reads must not specialize on U or slice stride."""
        ...
        for num_unique_blocks, local_slice_bytes in [(2, 4672), (5, 4704)]:
            ...
            compact_slots[0, 2] = -1
            compact_slots[1, 3] = -1
            gather_lens = torch.tensor([width, width - 1], dtype=torch.int32, device=DEVICE)
            out = torch.empty(
                batch_size,
                width + 2,
                512,
        ...
        self.assertEqual(
            cache_after_first,
            cache_after_second,
            "rank-major slot dequant recompiled when U/local slice changed: "
