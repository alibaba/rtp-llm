...
def _pack_slots_to_paged_torch(
    ...
    src_data = src[
        src_block.unsqueeze(1), src_pos.unsqueeze(1) * TOKEN_DATA_SIZE + data_off
    ]
    src_data = torch.where(valid.unsqueeze(1), src_data, torch.zeros_like(src_data))
    dst[dst_block.unsqueeze(1), dst_pos.unsqueeze(1) * TOKEN_DATA_SIZE + data_off] = (
        src_data
    )
    ...
    remapped = torch.where(
        valid,
        rows.to(torch.int32),
        torch.zeros((), dtype=torch.int32, device=k_cache.device),
    )
...
def _fill_unique_footer(cache: torch.Tensor) -> None:
    ...
    scale = ((scale_off.unsqueeze(0) + slots.unsqueeze(1) * 7 + 3) % 251).to(
        torch.uint8
    )
    backing[
        blocks.unsqueeze(1),
...
class PackSlotsToPagedPrecisionTest(unittest.TestCase):
    ...
    def test_row_lens_skips_capture_tail(self) -> None:
        ...
        paged, remapped = pack_slots_to_paged(cache, slots, dst_page, row_lens=lens)
        ref_paged, ref_remap = _pack_slots_to_paged_torch(cache, slots, dst_page)
