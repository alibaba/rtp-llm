...
class MegaMoEStrategy(RoutedExpertsStrategy):
    ...
    def setup_weights(self, layer_weights: Dict) -> None:
        ...
        if device.type == "cuda":
            # Model construction runs in a background thread in RTP-LLM. CUDA's
            # current device is thread-local, while DeepGEMM allocates its
            # symmetric buffer with device="cuda". Reset it here so each rank
            # rendezvous uses its own local device instead of the thread's
            # default cuda:0.
