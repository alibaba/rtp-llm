...
class MegaMoEStrategy(RoutedExpertsStrategy):
    ...
    def setup_weights(self, layer_weights: Dict) -> None:
        ...
        # Runtime state is intentionally deferred until the VMM weight region
        # closes; see ``setup_runtime`` below.
        self._mega_runtime_device = device

    def setup_runtime(self) -> None:
        """Allocate dispatch/output caches and run the optional JIT warmup.

        These are forward workspaces, not resident model weights.  Keeping
        them outside ``feature_weights_region`` lets the normal runtime-cache
        release path reclaim them independently from the weight VMM pool.
        """
    ...
    def _apply_routed_weight_transform(
        ...
        ``feature_weights_region`` NOT suppressed so the fresh ``_mega_*`` buffers are
        ...
        # feature_weights_region: tagging only the transform (as the earlier fix did)
        ...
        # Cold load: w2/l2_w must be allocated INSIDE feature_weights_region (l2_w aliases
        ...
        transform_region = (
            suppress_weights_region if isolate_scratch else feature_weights_region
        )
