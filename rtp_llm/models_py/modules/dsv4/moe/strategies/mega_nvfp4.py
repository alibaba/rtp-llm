...
class MegaNVFP4MoEStrategy(RoutedExpertsStrategy):
    ...
    def setup_weights(self, layer_weights: Dict) -> None:
        ...
        st_w1_s2 = layer_weights.pop(W.v4_routed_w1_s2)
        st_w3_w = layer_weights.pop(W.v4_routed_w3_w)
        st_w3_s = layer_weights.pop(W.v4_routed_w3_s)
        st_w3_s2 = layer_weights.pop(W.v4_routed_w3_s2)
        ...
        # Gate and up keep independent per-expert global scales. The kernel reads
        # them as FP32 [expert, 2] with column 0 = gate (w1), column 1 = up (w3),
        # matching the gate/up split of the interleaved L1 weight below.
        expected_l1_global_scale = (num_local_experts,)
        for name, tensor in (("w1.scale_2", st_w1_s2), ("w3.scale_2", st_w3_s2)):
            if (
                tensor.dtype != torch.float32
                or tuple(tensor.shape) != expected_l1_global_scale
            ):
                raise TypeError(
                    f"NVFP4 {name} must be float32 {expected_l1_global_scale}, got "
                    f"{tensor.dtype} {tuple(tensor.shape)}"
                )
        l1_gsf = torch.stack((st_w1_s2, st_w3_s2), dim=1).to(device)
        if not bool(torch.isfinite(l1_gsf).all()) or bool((l1_gsf <= 0).any()):
            raise ValueError(
                "NVFP4 gate/up weight global scales must be finite and positive, got "
                f"{l1_gsf.flatten().tolist()}"
            )
        del st_w1_s2, st_w3_s2
        ...
        self._mega_nvfp4_l1_gsf = l1_gsf
    ...
    def forward(self, x, weights, indices):  # type: ignore[override]
        ...
        with record_function_range("dsv4.moe.mega_nvfp4"):
            deep_gemm.nvfp4_nvfp4_mega_moe(
                y,
                (
                    self._mega_nvfp4_l1_w,
                    self._mega_nvfp4_l1_sf,
                    self._mega_nvfp4_l1_gsf,
                ),
