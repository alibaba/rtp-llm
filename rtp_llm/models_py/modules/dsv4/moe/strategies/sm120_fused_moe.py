...
class Sm120FusedMoeStrategy(RoutedExpertsStrategy):
    ...
    def _forward_collective(
        ...
        This fallback is WORLD-resolving by construction and has no stage-local
        variant. When a stage context is present the launch has declared itself
        as PP+EP, and WORLD then spans every stage — using it would exchange
        expert payloads with a stage executing a different round at a different
        time. Refuse rather than silently corrupt. The non-PP path is unchanged.
        """
        dist = torch.distributed
        if not dist.is_initialized():
            raise RuntimeError("SM120 collective MoE requires torch.distributed")
