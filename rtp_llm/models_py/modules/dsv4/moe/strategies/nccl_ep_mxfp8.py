"""CP4EP4PP2 expert backend B — stage-local NCCL MXFP8 dispatch/return.

Design §8. This is a NEW strategy adapter written against the algorithm, not a
copy of the donor file: the donor's experimental switches (real DeepEP, token
halves/quarters, deferred cross-layer combine, count caching, DG native wire,
fixed-EP/graph, shared-expert fusing) are deliberately absent, and every
collective resolves through :class:`EpStageContext` instead of ``WORLD``.

Per local layer and one stage round:

    owner BF16 x + router weights + global expert IDs
      -> block32 MXFP8 quantization of the activation
      -> stage-local count agreement (one all_gather per operation)
      -> per-destination route masking, replicated activation payloads
      -> NCCL all_to_all_single dispatch
      -> local expert-ID remap/mask
      -> local routed compute (GroupedFP4 / LocalLoop)
      -> local partial through BF16
      -> block32 MXFP8 quantization of the partial
      -> NCCL all_to_all_single return
      -> ordered FP32 peer accumulation -> BF16 owner output

The shared expert is NOT computed here.  The MoE layer owns it and adds it once
on the owning rank, for both backends; doing it here would double-count.

Numerical identity: this backend rounds its partial through BF16 and ships it
quantized, so it is NOT bit-equivalent to the `purecp_bf16` reference, which
reduces FP32.  They are two declared arithmetic contracts, not one.
"""
...
# Per-token dispatch payload, in bytes (design §8.2):
#   FP8 activation      D
#   UE8M0 scales        D/32
#   router weights      topk * 4
#   global expert IDs   topk * 4
# Return payload is D + D/32.
_WEIGHT_BYTES = 4
_ID_BYTES = 4
...
class NcclEpMxfp8Strategy(RoutedExpertsStrategy):
    ...
    def _stage(self) -> Tuple[object, int, int]:
        """(process_group, world, rank) for THIS stage's expert roster.

        The stage context is built once at model construction and handed in via
        ``MoeCfg.stage_context``.  Without it we accept the legacy identity only
        when ``world == ep_size`` — that is provably a single stage, which is the
        case the 4-GPU donor ran.  Anything else fails closed rather than
        exchanging expert payloads across pipeline stages.
        """
    ...
    def forward(
        ...
        with record_function_range("dsv4.moe.a2a.dispatch"):
            ...
            recv = self._all_to_all(send_payload, [n_local] * world, counts, payload_cols, group)
            del send_payload

        with record_function_range("dsv4.moe.a2a.compute"):
            local_out = self._compute_local(recv, counts, hidden, scale_cols, topk, payload_cols)

        del recv
        with record_function_range("dsv4.moe.a2a.return"):
            ret_payload = self._pack_return(local_out, hidden, scale_cols, n_total)
            del local_out
            ret_recv = self._all_to_all(ret_payload, counts, [n_local] * world, ret_payload.size(1), group)
            del ret_payload
            summed = mxfp8_dequant_peer_sum(
                ret_recv, n_rows=n_local, hidden_size=hidden, world_size=world,
    ...
    def _exchange_counts(self, n_local: int, group, world: int, device) -> list:
        """One stage-local all_gather of the local row count (design §8.3)."""
        if self._count_tensor is None or self._count_tensor.device != device or self._count_tensor.numel() != world:
            self._count_tensor = torch.empty((world, 1), dtype=torch.int64, device=device)
        self._count_tensor.fill_(n_local)
        torch.distributed.all_gather_into_tensor(
            self._count_tensor, torch.tensor([n_local], dtype=torch.int64, device=device), group=group
        )
        return [int(v) for v in self._count_tensor.view(-1).cpu().tolist()]

    @staticmethod
    def _mask_for_peer(weights: torch.Tensor, indices: torch.Tensor, dst: int, experts_per_rank: int):
    ...
    def _pack_dispatch(
        ...
        buf = torch.empty((world, n_local, payload_cols), dtype=torch.uint8, device=x.device)
        if n_local:
    ...
    def _compute_local(self, recv, counts, hidden, scale_cols, topk, payload_cols):
        ...
        order the return splits expect (design §8.3).
        """
    ...
    @staticmethod
    def _dequant_mxfp8(x_q: torch.Tensor, x_s: torch.Tensor, hidden: int) -> torch.Tensor:
        """Block32-MXFP8 -> BF16. The UE8M0 byte is a biased exponent."""
        n = x_q.size(0)
        q = x_q.view(torch.float8_e4m3fn).to(torch.float32)
        scale = torch.exp2(x_s.to(torch.float32) - 127.0).repeat_interleave(SCALE_BLOCK, dim=1)
        return (q * scale[:, :hidden]).to(torch.bfloat16)

    def _pack_return(self, local_out: torch.Tensor, hidden: int, scale_cols: int, n_total: int) -> torch.Tensor:
        from flashinfer import mxfp8_quantize

        ret_cols = hidden + scale_cols
        if n_total == 0:
            return torch.empty((0, ret_cols), dtype=torch.uint8, device=local_out.device)
        # Design §8.1: the partial is rounded through BF16 before quantization.
        ...
        buf = torch.empty((n_total, ret_cols), dtype=torch.uint8, device=local_out.device)
        buf[:, :hidden].copy_(q)
        buf[:, hidden:].copy_(s)
        return buf
