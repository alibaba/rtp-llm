...
class MegaNVFP4InputPacker(ABC):
    ...
    def pack(
        ...
        """Fill ``buf`` with NVFP4 activations plus their per-token global scale.

        ``buf.x_gsf`` carries the activation scale alone: the Mega MoE kernel
        applies each expert's own gate/up weight global scale on top of it.
        """
