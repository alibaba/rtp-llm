...
def forward_layers(
    ...
    **Stage-2 compat shim**: ``Block.forward`` is now flat-native (accepts
    ...
    if prepare_hidden_fn is None:
        h = getattr(v4, "embed_full", v4.embed)(input_ids)  # [T_total, dim]
        if _rt_on:
    ...
    with record_range_ctx():
        uses_fp8_prefill_meta = bool(v4.layers) and hasattr(
            v4.layers[0].attn, "_set_prefill_meta_shared"
        )
        if uses_fp8_prefill_meta:
