...
class Block(nn.Module):
    ...
    def prefill_fast_attn_body(
        ...
        *,
        ...
        if prefill_attn_out is not None:
            kwargs["prefill_attn_out"] = prefill_attn_out
    ...
    def prefill_fast_ffn_gate(
        self, x_pre: torch.Tensor, input_ids: torch.Tensor
    ) -> tuple[torch.Tensor, torch.Tensor]:
        gate_fn = getattr(self.ffn, "prefill_fast_gate", None)
        if gate_fn is None:
            raise RuntimeError("DSV4 prefill_fast_ffn_gate requires MoE fast gate")
        return gate_fn(x_pre, input_ids)

    def prefill_fast_ffn_body(
        self,
        x_pre: torch.Tensor,
        input_ids: torch.Tensor,
        precomputed_gate: tuple[torch.Tensor, torch.Tensor] | None = None,
        prefill_out: torch.Tensor | None = None,
    ) -> torch.Tensor:
        if prefill_out is not None:
            forward_with_out = getattr(self.ffn, "forward_with_output", None)
            if forward_with_out is not None:
                return forward_with_out(
                    x_pre,
                    input_ids,
                    prefill_out,
                    gate_outputs=precomputed_gate,
                )
        if precomputed_gate is not None:
            forward_with_gate = getattr(self.ffn, "forward_with_precomputed_gate", None)
            if forward_with_gate is not None:
                return forward_with_gate(x_pre, input_ids, precomputed_gate)
    ...
    def forward_prefill_fast(
        ...
        """Graph-oriented split prefill path for the FP8 production path.
