"""DSv4 decode Q/KV projection — extracted from ``Attention._forward_decode_body``.

Mirrors :func:`rtp_llm.models_py.modules.dsv4.attention.Attention._prefill_compute_qkv`
for the decode path (per-request batched, ``q_len == 1``).
...
the existing free-function pattern in :mod:`.forward` /
:mod:`.decode_attn_metadata`, and keeps ``attention.py`` thin.
"""
...
def decode_compute_qkv(
    ...
    RoPE table is gathered via ``freqs_cis[start_pos.long()]``; ``fused_rmsnorm_rope``
    applies the partial RoPE in-kernel. KV is a single MQA head.
