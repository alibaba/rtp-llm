...
from rtp_llm.models_py.modules.kimi_k3.kda.prefill import KimiK3KDAPrefill
...
class KimiK3KDA(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        del layer_idx  # Kept in the public constructor for decoder-layer parity.
    ...
    def _validate_request(
        ...
        *,
        ...
        if not self._is_prefill_role:
            if state is not None:
                raise ValueError("Kimi K3 Decode state must be owned by LayerKVCache")
            if kv_cache is None or attention_inputs is None:
                raise RuntimeError(
                    "Kimi K3 Decode and target verify require direct paged cache"
                )
    ...
    def forward(
        ...
        *,
        ...
        if self._is_prefill_role:
            ...
            output, final_state = self.prefill_executor(
                q_projected,
                k_projected,
                v_projected,
                raw_gate,
