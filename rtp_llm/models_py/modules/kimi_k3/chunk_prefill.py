"""Whole-model chunk Prefill execution for Kimi K3.

This module owns round planning, packed metadata, cache publication, optional
MTP hand-off and the lifetime of chunk-scoped workspaces. The target model only
dispatches oversized Prefill inputs here.
"""
...
class KimiK3ChunkCachePublisher:
    ...
    def create(
        ...
        *,
        ...
        if os.environ.get("KIMI_K3_CHUNKWISE_RDMA", "0") != "1":
            return cls()
...
def build_chunk_attention_inputs(
    ...
    *,
    ...
    chunk.kv_cache_block_id_host_by_group = [
        _select_batch_rows(value, batch_indices)
        for value in attention_inputs.kv_cache_block_id_host_by_group
    ]
    chunk.kv_cache_kernel_block_id_host_by_group = [
        _select_batch_rows(value, batch_indices)
        for value in attention_inputs.kv_cache_kernel_block_id_host_by_group
    ]
    chunk.kv_cache_kernel_block_id_device_by_group = [
        _select_batch_rows(value, batch_indices)
        for value in attention_inputs.kv_cache_kernel_block_id_device_by_group
    ]
    return chunk


def reset_kimi_k3_chunk_prefill(model: Any) -> None:
    """Release storage owned by one whole-chunk Prefill invocation."""

    owned_storages = {
        workspace.untyped_storage().data_ptr()
        for workspace in (
            model._prefill_mtp_hidden_workspace,
            model._prefill_mtp_draft_workspace,
        )
        if workspace is not None
    }
    if (
        model._mtp_hidden_buffer is not None
        and model._mtp_hidden_buffer.untyped_storage().data_ptr() in owned_storages
    ):
        model._mtp_hidden_buffer = None
        model._mtp_hidden_valid_tokens = 0
    model._whole_chunk_prefill_active = False
    model._prefill_mtp_hidden_workspace = None
    model._prefill_mtp_draft_workspace = None


def run_kimi_k3_chunk_prefill(
    model: Any,
    ...
    """Execute an oversized packed Prefill through one sequence of rounds."""
    ...
    page_size = int(model.kv_cache.seq_size_per_block)
    if page_size % 64:
        raise RuntimeError(
            "K3 chunk Prefill cache page size must be divisible by 64"
        )
    if chunk_tokens % tp_size or int(model.parallelism_config.ep_size) != tp_size:
        raise RuntimeError("K3 chunk Prefill requires TP-aligned chunks and TP == EP")
    if getattr(attention_inputs, "context_parallel_info", None) is not None:
        raise RuntimeError("K3 chunk Prefill does not support Prefill CP")
    multimodal = inputs.multimodal_inputs
    if multimodal.multimodal_features:
        raise RuntimeError("K3 chunk Prefill does not support multimodal input")

    if model._mtp_runtime_enabled():
        token_capacity = ((int(chunk_tokens) + tp_size - 1) // tp_size) * tp_size
        model._prefill_mtp_hidden_workspace = model.embedding_weight.new_empty(
            token_capacity,
            3 * int(model.config.hidden_size),
        )
        logging.info(
            "[K3_EAGLE3] allocated session-scoped Prefill hidden workspace shape=%s",
            tuple(model._prefill_mtp_hidden_workspace.shape),
        )
    ...
    try:
        ...
        input_lengths = [
            int(value) for value in attention_inputs.input_lengths_host.tolist()
        ]
        prefix_lengths = [
            int(value) for value in attention_inputs.prefix_lengths_host.tolist()
        ]
        logical_request_count = int(
        ...
        cache_publisher = KimiK3ChunkCachePublisher.create(
            attention_inputs,
            model.kv_cache,
            model.layers,
        ...
        barrier(Group.TP)
        logging.info(
            "[K3_WHOLE_CHUNK_PREFILL] enabled total_tokens=%d requests=%d "
            "rounds=%d chunk_tokens=%d page_size=%d TP=%d EP=%d "
            "chunkwise_rdma=%s",
            total_tokens,
            len(input_lengths),
            len(rounds),
        ...
        for round_idx, round_plan in enumerate(rounds):
            ...
            chunk_inputs.attention_inputs = build_chunk_attention_inputs(
                attention_inputs,
                round_plan=round_plan,
                device=input_ids.device,
            )
            chunk_attention = chunk_inputs.attention_inputs
            fmha_impl.prepare(chunk_attention)
            if not model._is_decode_role:
                model._mtp_hidden_buffer = None
                model._mtp_hidden_valid_tokens = 0
            publish_context = cache_publisher.begin_round(logical_round)
            with torch.profiler.record_function(
                ...
                round_output = model._forward_impl_one(
                    ...
                    round_plan=round_plan,
                    chunk_publish_context=publish_context,
                )
            cache_publisher.commit_round(publish_context)

            if round_hook is not None:
                round_mtp_hidden = model.get_mtp_target_hidden_states(-1)
                if round_mtp_hidden is None:
                    raise RuntimeError("K3 chunk Prefill did not publish MTP hidden")
                ...
                for item in logical_round.slices:
                    ...
                    draft_end = packed_end - int(item.terminal)
                ...
                draft_rows = len(draft_row_indices)
                if draft_rows == int(round_mtp_hidden.size(0)):
                    draft_hidden = round_mtp_hidden
                elif not draft_row_indices:
                    draft_hidden = round_mtp_hidden.narrow(0, 0, 0)
                elif draft_row_indices == list(range(draft_rows)):
                    draft_hidden = round_mtp_hidden.narrow(0, 0, draft_rows)
                else:
                    if model._prefill_mtp_draft_workspace is None:
                        model._prefill_mtp_draft_workspace = (
                            round_mtp_hidden.new_empty(
                                model._prefill_mtp_hidden_workspace.size(0),
                                *round_mtp_hidden.shape[1:],
                            )
                        )
                    draft_hidden = model._prefill_mtp_draft_workspace.narrow(
                        0, 0, draft_rows
                    )
                    index = torch.tensor(
                        draft_row_indices,
                ...
                model._mtp_hidden_buffer = draft_hidden
                model._mtp_hidden_valid_tokens = draft_rows
                round_hook(logical_round, round_plan is rounds[-1])
        ...
        if not cache_publisher.enabled:
            writer = create_write_cache_store_impl(attention_inputs, model.kv_cache)
            if writer is not None:
                for layer_idx, layer in enumerate(model.layers):
                    layer_cache = model.kv_cache.get_layer_cache(layer_idx)
                    if layer.is_kda:
                        layer.prepare_kda_cache_store(layer_cache)
                    writer(layer_cache)
        ...
        result = PyModelOutputs(terminal_hidden)
        result.lm_output_already_selected = True
        return result
    finally:
        reset_kimi_k3_chunk_prefill(model)
