...
def plan_state_sequences(cu_seqlens, prefix_lengths, block_table, block_size):
    ...
    for request, prefix in enumerate(prefix_lengths):
        ...
        while start < end:
            ...
            segments.append(StateSegment(start, start + count, table[position // block_size]))
            start += count
            position += count
...
def flash_kda_paged_prefill(q, k, v, g, beta, a_log, dt_bias, lower_bound,
                           cache_states, cu_seqlens, prefix_lengths,
    ...
    try:
        ...
        raise RuntimeError('K3 native prefill requires the pinned FlashKDA backend') from exc
    ...
    if not q.is_cuda or any(x.device != q.device for x in (k,v,g,beta,a_log,dt_bias,cache_states)):
        raise ValueError('FlashKDA tensors must share one CUDA device')
    if torch.cuda.is_current_stream_capturing():
        raise RuntimeError('Paged KDA prefill planning must run outside CUDA Graph capture')
    if cu_seqlens[-1] != q.shape[0]:
        raise ValueError('KDA metadata must cover the physical token shape')
    sequences = plan_state_sequences(cu_seqlens, prefix_lengths, block_table, block_size)
    used_blocks = [s.cache_block for seq in sequences for s in seq.segments]
    used_blocks += [seq.initial_block for seq in sequences if seq.initial_block is not None]
    if used_blocks and max(used_blocks) >= cache_states.shape[0]:
        raise ValueError('KDA state block exceeds cache capacity')
    out = torch.zeros_like(v)
    max_tokens = max((s.end-s.start for seq in sequences for s in seq.segments), default=0)
    if not max_tokens:
        return out
    workspace = torch.empty(flash_kda.get_workspace_size(max_tokens, q.shape[1], 1),
                            dtype=torch.uint8, device=q.device)
    a_log, dt_bias = a_log.contiguous(), dt_bias.reshape(q.shape[1], 128).contiguous()
    for seq in sequences:
        if not seq.segments:
            continue
        state = (torch.zeros((1,q.shape[1],128,128), dtype=torch.float32, device=q.device)
                 if seq.initial_block is None else
                 cache_states[seq.initial_block].transpose(-1,-2).unsqueeze(0).contiguous())
        for segment in seq.segments:
            start, end = segment.start, segment.end
            xs = [x[start:end].unsqueeze(0).contiguous() for x in (q,k,v,g)]
            final = torch.empty_like(state)
            torch.ops.flash_kda.fwd(*xs, beta[start:end].unsqueeze(0).contiguous(),
                                   128 ** -.5, out[start:end].unsqueeze(0), workspace,
                                   a_log, dt_bias, float(lower_bound), state, final)
