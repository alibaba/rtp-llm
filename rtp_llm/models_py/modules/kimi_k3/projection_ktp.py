...
def validate_projection_ktp_sp_type(sp_type: str) -> str:
    ...
    session, so it is true for the Eagle3 score model as well as the draft
    model.  Score versus draft is instead encoded by the model's parallelism:
    the score model keeps KTP while both proposal construction paths force the
    draft model to KTP1.
    """

    normalized = str(sp_type).strip().lower()
    if normalized not in ("", "eagle3"):
        raise RuntimeError(
...
def _apply_projection(
    inputs: torch.Tensor, projection: Any, *, name: str
) -> torch.Tensor:
    ...
    raise TypeError(
        f"KTP {name} projection must be a Tensor or callable Linear, "
...
def pack_ktp_projection_payload(
    ...
    *,
    ...
    projected = _apply_projection(
        gathered_hidden, fused_projection, name="QKVG/F_A/beta"
    )
    ...
    raw_gate = _apply_projection(
        forget_latent, forget_up_projection, name="F_B"
    )
