...
class PyAttentionInputsFake(PyAttentionInputs):
    def __init__(self):
        super().__init__()
        self._cu_seqlens = None
        self._cu_kv_seqlens = None
        self._kv_cache_block_id_device = None
    ...
    def cu_kv_seqlens(self) -> torch.Tensor:
        return self._cu_kv_seqlens

    @property
    def kv_cache_block_id_device(self) -> torch.Tensor:
        return self._kv_cache_block_id_device


def add_setter_to_pybind_property(cls, prop_name, attr_name=None):
    """
    为 pybind 的只读属性添加 setter

    Args:
        cls: pybind 绑定的类
        prop_name: 属性名
        attr_name: 对应的内部属性名（如果存在的话）
    """
    ...
    def getter(self):
        # 调用原始的 getter
        return getattr(type(self), prop_name).fget(self)
...
class FlashInferPythonMHATest(TestCase):
    ...
    def gen_attention_inputs(
    ...
    ) -> PyAttentionInputsFake:
        assert not (input_lengths is None and sequence_lengths is None)
        attention_inputs: PyAttentionInputsFake = PyAttentionInputsFake()
        batch_size: int = 0
        max_seq_len: int = 0
        ...
        if input_lengths is not None:
            ...
            # import pdb
            # pdb.set_trace()
            attention_inputs._cu_seqlens = cu_seqlens
            attention_inputs._cu_kv_seqlens = cu_seqlens
        ...
        attention_inputs._kv_cache_block_id_device = block_tables
    ...
    def test_run_flashinfer_prefill_test(self):
        ...
        kv_cache.k_cache_base = self.k_cache
        kv_cache.v_cache_base = self.v_cache
        op = FlashInferPythonPrefillOp(config)
    ...
    def test_run_flashinfer_decode_test(self):
        ...
        kv_cache.v_cache_base = self.v_cache
        print("gg:", flush=True)
        op = FlashInferPythonDecodeOp(config)
        input_params = op.prepare(attn_inputs)
        # impl = FlashInferPythonPrefillImpl(config, attn_inputs)
        # print('kk:', flush=True)
        out = op.forward(q, kv_cache, input_params)
