...
class SelectTopkOpTest(TestCase):
    ...
    def _run_select_topk_op_test(self, num_tokens: int, num_expert: int, top_k: int, dtype: _dtype):
        torch.manual_seed(1)
        ...
        op_routing_weights = torch.zeros((num_tokens, top_k), dtype=torch.float32, device=router_logits.device)
        op_selected_experts = torch.zeros((num_tokens, top_k), dtype=torch.int32, device=router_logits.device)
        
        for _ in range(5):
            select_topk_op.forward(router_logits_fp32, op_selected_experts, op_routing_weights)

            torch_routing_weights = F.softmax(router_logits, dim=1, dtype=torch.float)
            torch_routing_weights, torch_selected_experts = torch.topk(torch_routing_weights, top_k, dim=-1)
