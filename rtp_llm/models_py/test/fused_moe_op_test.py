...
from libth_transformer.rtp_llm_ops import FusedMoEOp
...
class FusedMoEOpTest(TestCase):
    ...
    def torch_sparse_block_forward(self, hidden_states, up_proj, down_proj, routing_weights, expert_ids):
        ...
        for expert_idx in range(num_experts):
            ...
            current_hidden_states = layer['act_fn'](layer['up_proj_g'](current_state)) * layer['up_proj_x'](current_state)
    ...
    def _run_fused_moe_op_test(self, num_tokens: int, num_expert: int, top_k: int, dtype: _dtype, hidden_dim: int, inter_dim: int):
        torch.manual_seed(0)
        ...
        hidden_states = torch.rand(num_tokens, hidden_dim, dtype=dtype).to('cuda') * 2 - 1
        up_proj = torch.rand(num_expert, inter_dim * 2, hidden_dim, dtype=dtype, device=hidden_states.device) * 2 - 1
        down_proj = torch.rand(num_expert, hidden_dim, inter_dim, dtype=dtype, device=hidden_states.device) * 2 - 1
        expert_scales = torch.rand(num_tokens, top_k, dtype=torch.float32, device=hidden_states.device) * 2 - 1
        expert_scales = torch.softmax(expert_scales, dim=1)
        expert_ids = torch.zeros((num_tokens, top_k), dtype=torch.int32, device=hidden_states.device)
        rtp_op_outputs = torch.zeros((num_tokens, hidden_dim), dtype=dtype, device=hidden_states.device)
        
        for _ in range(5):
            fused_moe_op.forward(hidden_states, up_proj, down_proj, expert_scales, expert_ids, rtp_op_outputs)
            torch_outputs = self.torch_sparse_block_forward(hidden_states, up_proj, down_proj, expert_scales, expert_ids)
