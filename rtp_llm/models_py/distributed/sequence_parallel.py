...
from rtp_llm.models_py.distributed.collective_torch import Group, all_gather_trim
...
def sequence_parallel_output_tokens(
    layout: SequenceParallelLayout,
    *,
    is_fake_stream: bool,
) -> int:
    """Return the rows that must survive the final model-boundary gather.

    A mixed Projection-KTP wave executes one physical dummy request on ranks
    with no local work. The speculative executor still hands that rank dense
    per-request LM indexes and hidden-state bindings until the wave completes,
    so retain exactly one dummy request at the model boundary. Its valid-row
    mask remains zero and the executor discards it; all non-fake ranks expose
    only their logical rows.
    """

    logical_tokens = layout.tokens.logical_tokens
    if logical_tokens or not is_fake_stream or layout.mode == "prefill":
        return logical_tokens
    dummy_tokens = layout.requests.tokens_per_request
    if dummy_tokens <= 0 or dummy_tokens > layout.tokens.physical_tokens:
        raise ValueError(
            "fake decode output requires one complete physical dummy request: "
            f"tokens_per_request={dummy_tokens}, "
            f"physical_tokens={layout.tokens.physical_tokens}"
        )
    return dummy_tokens


def finalize_sequence_parallel_output(
    tensor: torch.Tensor,
    *,
    logical_tokens: int,
    world_size: int,
    ...
    """Restore logical token rows at a sequence-parallel model boundary.

    ``world_size`` is the size of the token-sharding domain, not the global
    distributed world.  This distinction matters for DP/Projection-KTP:
    attention TP is one there, while ``Group.TP`` intentionally aliases the
    global process group for legacy collectives.  Such owner-local tensors
    must only be trimmed locally; gathering them would make every DP owner
    consume rank 0's rows.
    """
    ...
    if world_size > 1:
        return all_gather_trim(tensor, logical_tokens, group=group)
    if tensor.ndim == 0 or logical_tokens > int(tensor.shape[0]):
        raise ValueError(
            "logical token count exceeds owner-local rows: "
            f"logical={logical_tokens}, local_shape={tuple(tensor.shape)}"
        )
