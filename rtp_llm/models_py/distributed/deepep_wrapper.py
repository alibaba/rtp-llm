...
class DeepEPWrapper:
    ...
    def _init_low_latency_buffer(
        ...
        local_rank: int = getattr(self._parallelism_config, 'local_rank',
                                 self._parallelism_config.world_rank % self._parallelism_config.local_world_size if self._parallelism_config.local_world_size > 0 else self._parallelism_config.world_rank)
        if local_rank == 0:
            print(
