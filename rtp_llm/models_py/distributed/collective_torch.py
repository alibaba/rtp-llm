...
def _create_process_groups(
    ...
    if tp_size > 1:
        if world_size == tp_size:
            ...
            logging.info(
                f"[rank: {world_rank}] Creating single TP group (pure TP mode) with all ranks"
            )
            ...
            group_key = Group.TP.name + "0"
            _group_map[group_key] = tp_group

            # Try to initialize symmetric memory (will auto-disable if cross-node)
            logging.info(
                f"[rank: {world_rank}] Calling init_symm_mem_communicator for pure TP group..."
            )
            symm_mem_comm = init_symm_mem_communicator(tp_group)
            if symm_mem_comm is not None and not symm_mem_comm.disabled:
                ...
                logging.info(
                    f"[rank: {world_rank}] ⚠️  Symmetric memory NOT initialized (will use NCCL)"
                )
                ...
                if len(tp_ranks) > 0:
                    ...
                    if world_rank in tp_ranks:
                        ...
                        if symm_mem_comm is not None and not symm_mem_comm.disabled:
                            ...
                            logging.info(
                                f"[rank: {world_rank}] ⚠️  Symmetric memory NOT initialized for TP group {dp_rank_val}"
                            )
...
def all_reduce(tensor: torch.Tensor, group: Group) -> torch.Tensor:
    ...
    if group == Group.TP:
        ...
        if symm_mem_comm is not None and symm_mem_comm.should_torch_symm_mem_allreduce(
            ...
            logging.info(
                f"[AllReduce] Using symm_mem, tensor shape: {tensor.shape}, dtype: {tensor.dtype}"
            )
            return symm_mem_comm.all_reduce(tensor)
        else:
            if symm_mem_comm is None:
                logging.info(
                    f"[AllReduce] Using NCCL (no symm_mem_comm), tensor shape: {tensor.shape}, dtype: {tensor.dtype}"
                )
            elif not symm_mem_comm.should_torch_symm_mem_allreduce(tensor):
                logging.info(
                    f"[AllReduce] Using NCCL (tensor not eligible for symm_mem), tensor shape: {tensor.shape}, dtype: {tensor.dtype}"
                )
