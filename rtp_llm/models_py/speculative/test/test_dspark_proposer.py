...
class SampleSequentialMarkovTest(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.anchors = torch.randint(0, self.vocab, (self.batch,), dtype=torch.int32)
    ...
    def test_matches_reference_chain_exactly(self) -> None:
        ...
        tokens = self.proposer._sample_sequential_markov(self.base_logits, self.anchors)
    ...
    def test_rejects_bad_geometry(self) -> None:
        ...
        with self.assertRaises(ValueError):
            self.proposer._sample_sequential_markov(self.base_logits, self.anchors[:1])
...
class _ProposeProposer(_TinyProposer):
    """Records the query block handed to attention; synthesizes the tail."""
    ...
    def compute_draft_logits(self, hidden):
        rows = hidden.shape[0] // self._dspark_width
        torch.manual_seed(11)
        self.seen_base = torch.randn(rows, self._dspark_width, 23)
        return hidden, self.seen_base
...
class ProposeStepTest(unittest.TestCase):
    def setUp(self) -> None:
        self.width = 5
        self.proposer = _ProposeProposer(width=self.width, vocab=23, rank=4)
