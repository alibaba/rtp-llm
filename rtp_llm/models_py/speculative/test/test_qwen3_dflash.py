...
class Qwen3DFlashConfigTest(unittest.TestCase):
    ...
    def test_setup_accepts_dtypeless_checkpoint_with_resolved_bf16_act_type(
        self,
    ) -> None:
        # TorchSpec DFlash checkpoints may not declare dtype/torch_dtype; the
        # effective dtype resolved through --act_type is authoritative, and the
        # setup must not require the raw json to declare a dtype.
        raw = self._raw_config()
        self.assertNotIn("dtype", raw)
        with tempfile.TemporaryDirectory() as path:
            Path(path, "config.json").write_text(json.dumps(raw))
            draft_config = Qwen3DFlash._create_config(path)
            draft_config.init_precision_config(
        ...
        sp_config = SimpleNamespace(gen_num_per_cycle=7)
        # Must not raise despite config_dtype being unset.
        ModelFactory._setup_dflash_configs(sp_config, target_config, draft_config)
