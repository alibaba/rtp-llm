...
class DSparkProposerMixin:
    ...
    def run_commit_step(
        ...
        input_lengths_device = optional_tensor(
        ...
        prefix_source = prefix_device if prefix_device is not None else prefix
        lengths_source = (
            input_lengths_device
            if input_lengths_device is not None
            else input_lengths
        )
        if prefix_source is None or int(prefix_source.numel()) < batch_size:
            raise RuntimeError(
                "DSpark commit requires prefix_lengths with one value per request"
            )
        prefix_lengths = prefix_source[:batch_size].to(
