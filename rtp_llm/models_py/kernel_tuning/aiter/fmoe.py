...
def configure_aiter_fmoe_overlays() -> KernelTuningStatus:
    ...
    # Double-checked lock: the function is deterministic in process state, but
    # guard the check-then-act sequence so concurrent callers cannot interleave
    # partial validation with the AITER_CONFIG_FMOE environment write.
    with _CONFIG_LOCK:
        if _CONFIG_STATUS is not None:
            return _CONFIG_STATUS
        _CONFIG_STATUS = _configure_aiter_fmoe_overlays()
        return _CONFIG_STATUS


def _configure_aiter_fmoe_overlays() -> KernelTuningStatus:
    global _CONFIG_STATUS
