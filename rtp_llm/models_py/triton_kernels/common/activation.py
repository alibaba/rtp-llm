...
@triton.jit
def _swigluoai_mul_fp8_quant_deep_gemm_masked(
    input_ptr,
    y_q_ptr,
    y_s_ptr,
    counts_ptr,
    ...
    for t in tl.range(0, n_tokens, num_stages=NUM_STAGES):
        ...
        y = gate * (1.0 / (1.0 + tl.exp(-alpha * gate))) * (up + 1.0)
...
def swigluoai_mul_fp8_quant_deep_gemm_masked(
    y: torch.Tensor,
    ...
    """Quantize OpenAI SwiGLU over a [up, gate] routed-expert tensor."""
    assert y.ndim == 3, "y must be (E, T, 2*H)"
    E, T, H2 = y.shape
    assert H2 % 2 == 0, "last dim of y must be even (2*H)"
    H = H2 // 2
    ...
    f_info = torch.finfo(fp8_dtype)
    _swigluoai_mul_fp8_quant_deep_gemm_masked[(E * G,)](
        y,
        y_q,
        y_s,
        tokens_per_expert,
