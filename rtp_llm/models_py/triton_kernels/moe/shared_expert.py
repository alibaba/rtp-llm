"""Triton helpers for shared-expert quantization and output combine."""
...
def quant_bf16_fp8_packed_ue8m0(
    ...
    if triton is None:
        raise RuntimeError("fused FP8 activation quantization requires Triton")
    if not x.is_cuda:
        raise RuntimeError("fused FP8 activation quantization requires CUDA tensors")
    if x.dim() != 2:
...
def fused_moe_epilogue(
    ...
    if triton is None:
        raise RuntimeError("fused shared-expert add requires Triton")
    if not (routed.is_cuda and shared.is_cuda):
        raise RuntimeError(
            "fused shared-expert add requires CUDA tensors; "
