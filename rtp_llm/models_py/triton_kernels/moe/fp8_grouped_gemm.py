...
GemmRole = Literal["generic", "gate_up", "down"]
...
def invoke_sm120_fp8_grouped_gemm(
    ...
    gemm_role: GemmRole = GEMM_ROLE_GENERIC,
    ...
    in the padded allocation are left untouched. The caller must provide token
    counts in [0, max_T]; the asynchronous assertion below enforces this without
    a host synchronization in the hot path.
    """
    E = A.size(0)
    max_T = A.size(1)
    K = A.size(2)
    ...
    torch._assert_async(
        torch.all((expert_num_tokens >= 0) & (expert_num_tokens <= max_T)),
        f"expert_num_tokens must be within [0, max_T={max_T}]",
    )
