...
def test_fp4_int32_scale_packs_into_one_all_gather(monkeypatch):
    from rtp_llm.models_py.triton_kernels.sparse_mla import (
        fused_indexer_cp_assemble as assemble,
    )

    # FP4: k int8 [T, 64], scale int32 [T, 1] (4 UE8M0 bytes packed).
    local_q = torch.arange(192, dtype=torch.uint8).view(3, 64).view(torch.int8)
    local_s = torch.tensor(
    ...
    assert calls[0][2]["role"] == "indexer_k_packed"
    assert gathered_s is None
    assert (q_cols, s_cols) == (64, 4)
    assert tuple(gathered.shape) == (3, 68)
    assert torch.equal(gathered[:, :64], local_q.view(torch.uint8))
    assert torch.equal(gathered[:, 64:], local_s.view(torch.uint8).reshape(3, 4))
...
def test_master_switch_off_uses_two_separate_all_gathers(monkeypatch):
    ...
    local_q = torch.arange(12, dtype=torch.uint8).view(3, 4).view(torch.float8_e4m3fn)
