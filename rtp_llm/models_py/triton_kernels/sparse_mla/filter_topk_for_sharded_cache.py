...
Page-level RR:
   target_rank = (position // block_size) % cp_size
   vblock_idx = (position // block_size) // cp_size
   local_offset = position % block_size
"""
...
def _filter_topk_for_sharded_cache_kernel(
    ...
    global_block_idx = tok_pos // block_size
    target_rank = global_block_idx % cp_size
    not_owned = target_rank != cp_rank

    vblock_idx = global_block_idx // cp_size
    local_offset = tok_pos % block_size
...
def triton_filter_topk_for_sharded_cache(
    ...
    Uses page-level RR (block-granularity distribution).
