...
def store_final_state_to_block_map_kernel(
    ...
    """Store only final_state (no intermediate chunk states h).
    Used with FlashInfer prefill which doesn't produce chunk-boundary states."""
    i_b, i_h, i_v = tl.program_id(0), tl.program_id(1), tl.program_id(2)
    ...
    p_in = tl.make_block_ptr(source_ptr, (V, K), (K, 1), (v_offset, 0), (BLOCK_V, K), (1, 0))
    p_out = tl.make_block_ptr(dest_ptr, (V, K), (K, 1), (v_offset, 0), (BLOCK_V, K), (1, 0))
...
def store_final_state_only_to_block_map(
    ...
    Used when FlashInfer prefill is active, which doesn't produce chunk-boundary states."""
