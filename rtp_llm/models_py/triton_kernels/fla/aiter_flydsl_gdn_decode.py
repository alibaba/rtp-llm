...
_LOGGED_BACKEND_DECISIONS: set[str] = set()
...
class AiterFlydslGdnDecodeStateMetadata:
    ...
    ``block_map_width`` is the logical number of addressable block columns.
    It is explicit because CUDA Graph may pass a ``block_map[:, :1]`` view
    whose row stride still spans the full backing block table.
    """
...
def _backend_decision_category(reason: str | None) -> str:
    """Return a bounded category while retaining distinct fallback causes."""
    if reason is None:
        return "selected"
    return reason.partition(":")[0].partition(", got")[0]


def _log_backend_decision_once(reason: str | None) -> None:
    """Log selection and each distinct fallback category once per process."""
    category = _backend_decision_category(reason)
    if category in _LOGGED_BACKEND_DECISIONS:
        return
    _LOGGED_BACKEND_DECISIONS.add(category)
    if reason is None:
...
def _aiter_flydsl_gdn_decode_unsupported_reason(
    ...
    if batch < 1:
        return f"decode shape requires B>=1, got B={batch}"
    # CUDA/HIP Graph may batch many requests (B > 1), but this adapter resolves
    # exactly one recurrent state transition for each request. Multi-token
    # verification requires B*T state indices and is intentionally unsupported.
    if query_length != 1:
        return f"decode requires T==1, got T={query_length}"
...
def is_aiter_flydsl_gdn_decode_supported(
    ...
    *,
    ...
    """Return whether inputs and complete block-cache metadata are supported."""
    ...
    if reason is None:
        reason = _decode_state_metadata_unsupported_reason(
            ...
            state_metadata.block_map_width,
        ...
        if reason is None and state_metadata.state_pool_size != state.shape[0]:
            reason = (
                "state_pool_size differs from state.shape[0]: "
                f"{state_metadata.state_pool_size} != {state.shape[0]}"
            )
...
def _decode_state_metadata_unsupported_reason(
    ...
    *,
    ...
    if block_map.stride(0) <= 0:
        return f"block_map row stride must be positive, got {block_map.stride()}"
    if not isinstance(block_map_width, int) or isinstance(block_map_width, bool):
        return f"block_map_width must be an integer, got {block_map_width!r}"
    if block_map_width <= 0:
        return f"block_map_width must be positive, got {block_map_width}"
    if block_map_width > block_map.stride(0):
        return (
            "block_map_width exceeds the backing row stride: "
            f"{block_map_width} > {block_map.stride(0)}"
        )
    if block_map.shape[0] > 0:
        storage_elements = (
            block_map.untyped_storage().nbytes() // block_map.element_size()
        )
        required_elements = (
            block_map.storage_offset()
            + (block_map.shape[0] - 1) * block_map.stride(0)
            + block_map_width
        )
        if required_elements > storage_elements:
            return "block_map_width exceeds the backing storage"
    ...
    if not isinstance(host_sequence_lengths, torch.Tensor):
        return "CPU host_sequence_lengths are required"
    if host_sequence_lengths.device.type != "cpu":
        return "host_sequence_lengths must be a CPU tensor"
    if host_sequence_lengths.ndim != 1:
        return "host_sequence_lengths must be 1D"
    if host_sequence_lengths.dtype not in (torch.int32, torch.int64):
        return "host_sequence_lengths must be int32 or int64"
    if host_sequence_lengths.numel() != block_map.shape[0]:
        return "host sequence length count must equal block-map batch size"
    if host_sequence_lengths.numel() == 0:
        return None
    ...
    if max_write_pos >= block_map_width:
...
def prepare_aiter_flydsl_gdn_decode_state_indices(
    ...
    ``block_map_width`` is authoritative for bounds checks. It may exceed the
    ...
    _prepare_decode_state_indices_kernel[(batch,)](
        ...
        block_map_width=state_metadata.block_map_width,
...
def aiter_flydsl_gdn_decode(
    *,
    ...
    flydsl_gdr_decode(
        ...
        dt_bias=dt_bias,
        A_log=A_log,
        # ``indices`` is AITER's required legacy alias. In the integrated API,
        # explicit read/write indices take precedence; keep the alias equal to
        # write_indices for compatibility with older single-index callers.
        indices=write_indices,
        read_indices=read_indices,
        write_indices=write_indices,
        state=state,
        out=output,
        # RTP's original decode path always applies Q/K L2 normalization.
        use_qk_l2norm=True,
