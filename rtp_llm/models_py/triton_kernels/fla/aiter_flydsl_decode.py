...
from rtp_llm.models_py.triton_kernels.fla.utils import env_flag, is_amd_cdna3
...
class AiterFlydslGdnDecodeStateMetadata:
    ...
    def should_copy_state(self, *, is_capturing: bool) -> bool:
        """Derive the boundary-copy policy from complete state metadata."""
        if is_capturing:
            return True
        lengths = self.host_sequence_lengths
        if (
            not isinstance(lengths, torch.Tensor)
            or lengths.device.type != "cpu"
            or lengths.numel() == 0
            or self.seq_size_per_block <= 0
        ):
            return True
        crosses_boundary = (lengths > 0) & (lengths % self.seq_size_per_block == 0)
        return bool(crosses_boundary.any().item())
...
def _aiter_flydsl_gdn_decode_unsupported_reason(
    ...
    if not is_amd_cdna3:
        return "device is not AMD CDNA3/gfx942"
    ...
    if batch < 1 or query_length != 1:
        return (
            f"decode shape requires batch>=1 and T=1, got B={batch}, T={query_length}"
        )
    ...
    if shape not in AITER_FLYDSL_GDN_DECODE_ENABLED_SHAPES:
        return f"shape is not validated: {shape}"
...
def _prepare_decode_state_indices_kernel(
    ...
    # sequence_length < 2 is graph padding and intentionally resolves to block
    # 0. Any other invalid row represents a real request/cache inconsistency.
    tl.store(
        invalid_row_flags + batch,
        ((sequence_length >= 2) & ~valid_ids).to(tl.int32),
    )
    tl.store(read_indices + batch, tl.where(valid_ids, read_id, 0))
    tl.store(write_indices + batch, tl.where(valid_ids, write_id, 0))
...
def _prepare_aiter_flydsl_gdn_decode_state_indices(
    ...
    is non-positive resolve both indices to block 0. A real request known from
    the CPU length mirror to exceed the block-table width fails fast. RTP
    reserves block 0 as a dummy state: the boundary-copy kernel skips it and
    AITER output for padding rows is ignored by the graph scheduler.
...
def copy_aiter_flydsl_gdn_decode_state_at_block_boundary(
    ...
    expected_inner_stride = _expected_state_inner_stride(state)
    ...
    indices_reason = _decode_indices_unsupported_reason(
        read_indices, write_indices, device=state.device
    )
    ...
    # Amortize the graph-safe no-op launch over substantially fewer programs
    # than the original 256-element tiling while keeping heads independent.
    block_elements = 1024
    grid = (
        read_indices.numel(),
        value_heads,
...
def aiter_flydsl_gdn_decode(
    *,
    ...
    """Run fused FlyDSL decode after a graph-safe state-boundary copy.
    ...
    # A warmed signature promises that both the AITER kernel and the boundary
    # copy kernel are compiled. If eager routing skips the real copy, compile a
    # no-op copy with dummy indices before recording the signature; otherwise a
    # later Graph capture could attempt the copy kernel's first JIT compile.
    if (
        not is_capturing
        and not copy_state
        and decode_signature not in _WARMED_DECODE_SIGNATURES
    ):
        dummy_indices = torch.zeros_like(read_indices)
        copy_aiter_flydsl_gdn_decode_state_at_block_boundary(
            state, dummy_indices, dummy_indices
        )

    # Graph capture must always record the real launch because a later replay
    # may cross a block boundary. Eager callers may use their CPU length mirror
    # to skip known non-boundary steps.
    if is_capturing or copy_state:
        copy_aiter_flydsl_gdn_decode_state_at_block_boundary(
    ...
    flydsl_gdr_decode(
        ...
        key=k.contiguous(),
        value=v.contiguous(),
        a=a.reshape(batch, query_length, value_heads).contiguous(),
        b=b.reshape(batch, query_length, value_heads).contiguous(),
        dt_bias=dt_bias,
        A_log=A_log,
