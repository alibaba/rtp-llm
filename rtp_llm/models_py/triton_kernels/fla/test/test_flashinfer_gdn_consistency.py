...
class TestFlashInferGDNDecodeConsistency(unittest.TestCase):
    ...
    def _run_decode_comparison(self, B: int, H: int, HV: int, D: int, seq_size_per_block: int = 128):
        ...
        triton_out, _ = self.fused_recurrent(
            ...
            beta=beta,
            scale=None,
            initial_state=ssm_states_triton,
        ...
        # Only compare the write slots that were actually updated
        for i in range(B):
            wi = write_indices[i].item()
            if wi > 0:
                state_sim = cos_sim(ssm_states_fi[wi], ssm_states_triton[wi])
                self.assertGreater(
                    state_sim, 0.999,
    ...
    def setUpClass(cls):
        ...
        try:
            from rtp_llm.models_py.triton_kernels.fla.blackwell_prefill.gdn import (
                chunk_gated_delta_rule as old_prefill,
            )
