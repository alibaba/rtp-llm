...
def prepare_aiter_flydsl_gdn_decode_state_indices(
    block_map, lengths, block_size, host_sequence_lengths=None, *, state_pool_size
):
...
def _reset_adapter_process_state(test_case: unittest.TestCase) -> None:
    ...
    for item in process_state:
        clear = item.cache_clear if hasattr(item, "cache_clear") else item.clear
        clear()
        test_case.addCleanup(clear)
...
def _make_decode_kwargs(batch=2, **overrides):
    q, k, v, a, b, state = _make_decode_inputs(batch=batch)
...
def _call_mock_decode(kwargs, flydsl_decode=None, *, capturing=False):
...
def _decode_from_block_map(kwargs, block_map, lengths):
    read_indices, write_indices = prepare_aiter_flydsl_gdn_decode_state_indices(
        block_map, lengths, 1024, state_pool_size=kwargs["state"].shape[0]
    )
...
class AiterFlydslGdnDecodeCommonTest(unittest.TestCase):
    ...
    def test_prepare_decode_indices_maps_invalid_rows_to_skip_sentinel(self):
        ...
        for block_map, lengths, pool_size, expected in cases:
            with self.subTest(block_map=block_map):
                ...
                actual = _prepare_decode_state_indices(metadata)
                torch.cuda.synchronize()
    ...
    def test_shape_gate_accepts_production_stride_and_rejects_invalid_inputs(self):
        ...
        with (
            ...
            self.assertTrue(supported(_make_decode_inputs(query_length=2)))
                ...
                self.assertFalse(
                    is_aiter_flydsl_gdn_decode_supported(
                        *valid,
                        ...
                        sequence_lengths_plus_1=torch.ones(
    ...
    def test_decode_entry_forwards_exact_aiter_kwargs(self):
        call_kwargs = _make_decode_kwargs(already_validated=True)
    ...
    def test_decode_entry_fails_fast_if_capture_was_not_warmed(self):
        kwargs = _make_decode_kwargs(already_validated=True)
        with (
...
class AiterFlydslGdnDecodeRocmTest(unittest.TestCase):
    ...
    def test_full_decode_entry_graph_replay_matches_eager_at_boundary(self):
        ...
        _decode_from_block_map(kwargs | {"state": warm_state}, block_map, lengths)
        torch.cuda.synchronize()
        ...
        with torch.cuda.graph(graph):
            graph_output, _, _ = _decode_from_block_map(
                kwargs | {"state": graph_state}, block_map, lengths
            )
        torch.cuda.synchronize()
        ...
        eager_output, _, _ = _decode_from_block_map(
            kwargs | {"state": eager_state}, block_map, lengths
        )
