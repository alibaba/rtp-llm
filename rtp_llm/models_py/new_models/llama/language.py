"""LLaMA-family dense model for the new (vLLM-style) weight loader.

Scope of THIS implementation: standard HuggingFace LLaMA-style checkpoints, i.e.
the `HfWeightNames` layout in the legacy `rtp_llm/models/llama_weight.py`:
  ...
  model.norm.weight
  lm_head.weight   (or tied to embed_tokens when absent)

This covers model_type: llama (LlamaForCausalLM / Yi*), mistral, aquila, xverse.
It is the qwen3 dense template MINUS the per-head q/k RMSNorm, PLUS optional
qkv/o bias (for internlm-style ckpts). Forward reuses the exact same rtp-kernel
primitives as qwen3 (QKV/Row/MergedColumn linear, silu_and_mul, fmha_impl), so
the only thing that differs from the legacy loader is HOW weights are loaded.

Deliberately NOT handled here (legacy LlamaWeightInfo._process_meta variants —
add as follow-ups, each needs a different name map / qkv merge):
  * Meta-original `params.json` layout (tok_embeddings/output + rotary `permute`)
  * Baichuan `W_pack` fused qkv
  * Internlm2 fused `wqkv` (qkv_rerange interleave)
  * Yi `ln1`/`ln2` layernorm naming
  * SmoothQuant (RTP_SMOOTH_LLM_STYLE)
  * Cohere (q_norm/k_norm + tied lm_head) / Gemma (shift_one norm + tied)
"""
...
class LlamaAttention(RtpModule):
    """Standard LLaMA attention: fused QKV -> fmha_impl (rope+attn) -> o_proj.

    No q/k norm (that's Qwen3). `qkv_bias`/`o_proj_bias` default off; flip them
    on for internlm-style checkpoints that ship attention biases.
    """
...
class LlamaDecoderLayer(RtpModule):
...
def _extract_config_values(model_config: Any, load_config: Any):
    if isinstance(model_config, dict):
        ...
        qkv_bias = model_config.get("attention_bias", False)
    else:
...
class LlamaForCausalLM(GptModelBase):
    ...
    def load_weights(self, weights):
        ...
        # LLaMA variants with tie_word_embeddings (e.g. Llama-3.2-1B) ship no
        # lm_head.weight; mirror HF by tying it to the input embeddings.
        if not has_lm_head:
            logging.info(
                "[LlamaForCausalLM] lm_head.weight not found in ckpt; "
