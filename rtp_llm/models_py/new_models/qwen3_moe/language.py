...
class Qwen3Experts(BaseMoEExperts):
    """Qwen3 expert layout implemented by the shared newloader MoE layer."""
...
class Qwen3MoeBlock(RtpModule):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        if self.experts.fused_moe is None:
            raise RuntimeError(
                "Qwen3 MoE executor is not initialized; load and postprocess "
                "all expert weights before forward"
            )
        if hidden_states.dim() != 2:
            raise ValueError(
                "Qwen3 MoE expects a two-dimensional token matrix, got "
...
def _extract_moe_config_values(model_config: Any, load_config: Any):
    ...
    if enable_eplb:
        raise ValueError("EPLB is not supported by the Qwen3 MoE newloader path")
    values.update(
        num_experts=num_experts,
        top_k=top_k,
        moe_intermediate_size=moe_intermediate_size,
...
class Qwen3MoeForCausalLM(GptModelBase):
    ...
    def load_weights(self, weights) -> None:
        ...
        if not has_lm_head and self.tie_word_embeddings:
            logging.info("Qwen3MoeForCausalLM is tying lm_head to embed_tokens")
