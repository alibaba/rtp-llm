...
Sequence-Parallel projection boundary. KDA uses cuLA for packed prefill and a
shape-generic Triton recurrent kernel for token decode. KDA canonical states are
...
from rtp_llm.models_py.modules.kimi_k3.diagnostics.accuracy_trace import (
    accuracy_trace_enabled as _accuracy_trace_enabled,
    accuracy_trace_mode,
    accuracy_trace_requested as _accuracy_trace_requested,
...
from rtp_llm.models_py.modules.kimi_k3.diagnostics.performance_trace import (
    active_performance_profile,
    performance_profile as _perf_profile,
)
...
def _packed_causal_depthwise_conv1d(
    ...
    *,
    ...
    Triton paths matching FLA's forward and cache-update kernels.
    """
    ...
    if not x.is_cuda:
        raise RuntimeError("K3 KDA short convolution supports CUDA execution only")
...
class KimiK3LinearCacheAdapter:
    ...
    packed ``[history,QKV]`` convolution state. cuLA prefill publishes
    ``[H,K,V]`` while recurrent decode converts its V-first register tiles at
    the cache boundary. Cached prefill writes a state at every physical
...
class KimiK3KDA(nn.Module):
    ...
    def __init__(
        ...
        # Prefill 固定走 cuLA，Decode 固定走形状通用的 recurrent Triton kernel。
    ...
    def _kda_core(
        ...
        *,
        ...
        Same ``[1,T,H,*]`` I/O and ``[N,H,K,V]`` fp32 final-state contract for
        the fixed production backends: cuLA chunked prefill and the ported
        Triton fused-recurrent decode kernel.
        """
        ...
        if not q.is_cuda:
            raise RuntimeError("K3 KDA supports CUDA execution only")
        ...
        if mode == "prefill":
            if self._kda_backend == "cula":
                ...
                with (
                    ...
                    active_performance_profile("k3.kda.cula"),
                ):
            ...
            raise RuntimeError("KDA prefill requires the cuLA backend")
        elif mode == "decode":
        ...
        # Normalize to the model's [1,T,H,V] / q.dtype output contract.
...
class KimiK3MLA(MlaAttention):
    ...
    packed Q-A/KV-A/output-gate projection. Accuracy tracing may retain the
    source model's independent projection boundaries.
    """
    ...
    def _apply_output_gate(
        ...
        wise per (head, value) before o_proj.
    ...
    def _forward_impl(
        ...
        if not hidden_states.is_cuda:
            raise RuntimeError("K3 MLA supports CUDA execution only")
        if accuracy_trace_mode() is None:
...
class KimiK3LatentMoE(nn.Module):
    """Production K3 latent MoE backed by DeepGEMM MegaMoE."""
    ...
    def forward(
        ...
        *,
        ...
        with _perf_profile(
            ...
            routed_output = self._distributed_expert_sum(
                routed_input,
                expert_ids,
                routing_weights,
                sequence_parallel=sp_active,
