...
class KimiK3Model(GptModelBase):
    ...
    def initialize(self, init_resource: PyModelInitResources) -> bool:
        ...
        if ktp_size > 1:
            ...
            sp_type = validate_projection_ktp_sp_type(
                os.environ.get("SP_TYPE", "")
            )
            logging.info(
                "[K3_PROJECTION_KTP_SESSION] SP_TYPE=%s speculative_session=%s "
                "model_role=score",
                sp_type or "vanilla",
                bool(init_resource.is_speculative),
            )
