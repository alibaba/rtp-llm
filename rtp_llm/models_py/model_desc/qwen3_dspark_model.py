...
class Qwen3DSparkModel(DSparkProposerMixin, Qwen3Model):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.init_dspark_proposer(
            width=config.gen_num_per_cycle + int(not config.dspark_sample_from_anchor),
            noise_token_id=config.dspark_noise_token_id,
            aux_feature_dim=weights.get_global_weight(W.dspark_fc_w).shape[0],
        ...
        # Concatenate in [N, K] space as in Qwen3Next to preserve ROCm packing.
        ...
        self.context_kv_projection = LinearFactory.create_linear(
            torch.cat([w[W.attn_qkv_w][:, q_cols:].T for w in layer_weights]).T,
            None,
            None,
            None,
    ...
    def _get_fmha_group_tags(self):
        return get_group_tags_for_layers(self.kv_cache, range(self.layer_num))
    ...
    def prepare_forward_commit(self, inputs, is_cuda_graph=False):
        ...
        if isinstance(attention, Mapping):
            return {
                ...
                for tag in self._get_fmha_group_tags()
            }
    ...
    def forward_propose(self, inputs, fmha_impl=None):
        ...
        return self.run_propose_step(inputs, fmha_impl, inputs.input_ids.device)
    ...
    def forward_commit(self, inputs, fmha_impl=None):
        ...
        return self.run_commit_step(inputs, inputs.input_ids.device, fmha_impl)

    def commit_features(self, hidden, inputs, fmha_impl=None):
        if hidden.shape[0] != inputs.input_ids.numel():
            raise ValueError(
