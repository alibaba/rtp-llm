...
class Qwen3DecoderLayer(nn.Module):
    def __init__(
        ...
        self.self_attn = CausalAttention(
            attn_configs, parallelism_config, weights, config.layernorm_eps, quant_config, hw_kernel_config, layer_idx
        )
        self.mlp = DenseMLP(
...
class Qwen3Model(GptModelBase):
    def __init__(
        ...
        self.layers = nn.ModuleList(
            [
                Qwen3DecoderLayer(
                    config, parallelism_config, idx, weights.weights[idx], quant_config, py_hw_kernel_config
                )
