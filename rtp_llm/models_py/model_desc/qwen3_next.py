...
class Qwen3NextGatedDeltaNetPrefill(Qwen3NextGatedDeltaNetBase):
    ...
    def _fla(
        ...
        # Pool is [V, K] layout, Triton expects [K, V]
        initial_states_kv = (
        ...
        attn_out, h, final_state = chunk_gated_delta_rule(
            ...
            initial_state=initial_states_kv,
            output_final_state=True,
            cu_seqlens=cu_seqlens_without_padding,
            use_qk_l2norm_in_kernel=True,
        )
        if ssm_states is not None:
            # Triton outputs in [K, V], transpose back to [V, K] for pool
            h_vk = h.transpose(-1, -2).contiguous() if h is not None else None
            final_state_vk = (
    ...
    def _fla_blackwell(
        ...
        try:
            ...
            # Pool is [V, K], Triton expects [K, V]
            initial_states_kv = (
                initial_states.transpose(-1, -2).contiguous()
                if initial_states is not None else None
            )
...
class Qwen3NextGatedDeltaNetDecode(Qwen3NextGatedDeltaNetBase):
    ...
    def _fla(
        ...
        # Pool is [V, K] layout (FlashInfer native), Triton expects [K, V]
        ...
        # Write back modified states to pool in [V, K] layout
        ssm_states.copy_(ssm_states_kv.transpose(-1, -2))
