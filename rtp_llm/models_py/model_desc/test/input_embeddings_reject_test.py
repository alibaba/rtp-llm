...
class InputEmbeddingsRejectTest(unittest.TestCase):
    def test_model_capability_is_explicit_and_fails_closed(self):
        class UnimplementedModel(GptModelBase):
            pass

        for model in (
            UnimplementedModel,
            BertModel,
            DisaggregateModelBase,
        ):
            with self.subTest(model=model.__name__):
                self.assertFalse(model.supports_input_embeddings)
        self.assertTrue(Qwen3Model.supports_input_embeddings)
        self.assertTrue(MultimodalGenericModel.supports_input_embeddings)
