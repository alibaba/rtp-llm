...
class EmbeddingLookupOpTest(TestCase):
    ...
    def test_generic_embedding_rejects_malformed_mask(self):
        cases = (
            (
                ...
                "text_tokens_mask must have at least one id per token",
            ),
            (
                "mask_dtype",
    ...
    def test_generic_embedding_uses_active_prefix_of_capacity_mask(self):
        mask = torch.tensor([1, 0, 1, 0, 0, 0], dtype=torch.int32, device=self.device)
