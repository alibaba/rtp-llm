...
class BertEmbeddingOpTest(TestCase):
    ...
    def test_embedding_bert_reads_only_active_prefix_of_capacity_ids(self):
        baseline_inputs = _build_inputs()
        baseline_bert_inputs = baseline_inputs.bert_embedding_inputs
        baseline_bert_inputs.position_encoding = torch.arange(
            16, dtype=torch.float16, device="cuda"
        ).reshape(4, 4)
        baseline_bert_inputs.combo_tokens_type_ids = torch.tensor(
            [0, 1, 0, 1], dtype=torch.int32, device="cuda"
        )
        baseline_bert_inputs.token_type_embedding = torch.tensor(
            [[0.25, 0.5, 0.75, 1.0], [1.25, 1.5, 1.75, 2.0]],
            dtype=torch.float16,
            device="cuda",
        )
        baseline = self._embed(
            self.weight,
            baseline_inputs,
            baseline_inputs.embedding_inputs.text_tokens_mask,
        )

        capacity_inputs = _build_inputs()
        capacity_bert_inputs = capacity_inputs.bert_embedding_inputs
        capacity_bert_inputs.position_encoding = baseline_bert_inputs.position_encoding
        capacity_bert_inputs.token_type_embedding = (
            baseline_bert_inputs.token_type_embedding
        )
        capacity_bert_inputs.combo_position_ids = torch.tensor(
            [0, 1, 2, 3, 1000, 1001, 1002, 1003],
            dtype=torch.int32,
            device="cuda",
        )
        capacity_bert_inputs.combo_tokens_type_ids = torch.tensor(
            [0, 1, 0, 1, 1000, 1001, 1002, 1003],
            dtype=torch.int32,
            device="cuda",
        )
        capacity_output = self._embed(
            self.weight,
            capacity_inputs,
            capacity_inputs.embedding_inputs.text_tokens_mask,
        )

        torch.testing.assert_close(capacity_output, baseline)
    ...
    def test_embedding_bert_rejects_invalid_masks(self):
        ...
        cases = [
            ...
            (
                ...
                "text_tokens_mask must have exactly one id per token, got 3 vs 4",
            ),
            (
                "long_length",
                lambda: torch.ones(5, dtype=torch.int32, device="cuda"),
                "text_tokens_mask must have exactly one id per token, got 5 vs 4",
            ),
        ]
