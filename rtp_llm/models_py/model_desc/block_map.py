...
def get_layer_cache_for_group(
    kv_cache: LayeredKVCache | None, local_layer_idx: int, group_tag: str
) -> LayerKVCache:
    if kv_cache is None:
        raise RuntimeError(
            f"KV cache group {group_tag!r} is missing for local layer {local_layer_idx}: "
            "KV cache is not initialized"
        )
    layer_caches = kv_cache.get_layer_cache_groups(local_layer_idx)
    matches = [cache for cache in layer_caches if str(cache.tag) == group_tag]
    if len(matches) > 1:
        raise RuntimeError(
            f"local layer {local_layer_idx} has duplicate KV cache group {group_tag!r}"
        )
    if not matches:
...
def get_layer_caches_for_groups(
    ...
    if (
        ...
        raise RuntimeError(
            f"required KV cache group tags must be unique and non-empty: {group_tags}"
        )
    if kv_cache is None:
        raise RuntimeError(
...
def select_fmha_impl_for_group(fmha_impl: Mapping[str, T], group_tag: str) -> T:
    if not isinstance(fmha_impl, Mapping):
        raise RuntimeError(
            f"sparse MLA requires tagged FMHA implementations, got {type(fmha_impl)!r}"
        )
    try:
        return fmha_impl[group_tag]
    except KeyError as error:
        raise RuntimeError(
            f"FMHA group {group_tag!r} is missing; available tags={list(fmha_impl)}"
        ) from error
