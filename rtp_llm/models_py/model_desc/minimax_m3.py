...
def _requested_target_verify_backend() -> str:
    requested_backend = (
        os.environ.get("RTP_LLM_M3_TARGET_VERIFY_BACKEND", "auto").strip().lower()
    )
...
def _target_verify_query_dtype(
    kv_cache_dtype: KvCacheDataType, model_dtype: torch.dtype
) -> torch.dtype:
...
def _target_verify_impl_class():
    ...
    class MiniMaxM3TargetVerifyAttnOp(PyFlashinferDecodeAttnOp):
        ...
        def prepare(self, attn_inputs):
            ...
            self.decode_wrapper.plan(
                ...
                q_data_type=_target_verify_query_dtype(
                    self.kv_cache_dtype, get_scalar_type(attn_inputs.dtype)
                ),
        ...
        def forward(self, query, kv_cache, params):
            ...
            query_dtype = _target_verify_query_dtype(self.kv_cache_dtype, query.dtype)
