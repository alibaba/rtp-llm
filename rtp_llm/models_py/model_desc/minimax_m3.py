...
def _ensure_target_verify_prefill_seqlens(attn_inputs) -> None:
    """Prefill RoPE reads cu_seqlens_device; CUDA-graph capture may omit it."""
    ...
    if input_lengths is None or input_lengths.numel() == 0:
        return
    device = (
        input_lengths.device if input_lengths.is_cuda else torch.device("cuda")
    )
    lengths = input_lengths.to(device=device, dtype=torch.int32)
    cu_seqlens = getattr(attn_inputs, "cu_seqlens_device", None)
    ...
    if prefix_lengths is None or prefix_lengths.numel() == 0:
        prefix_lengths = attn_inputs.prefix_lengths
    kv_cu = getattr(attn_inputs, "cu_kv_seqlens_device", None)
    if (
        (kv_cu is None or kv_cu.numel() == 0)
        and prefix_lengths is not None
        and prefix_lengths.numel() == lengths.numel()
    ):
        prefix = prefix_lengths.to(device=device, dtype=torch.int32)
        kv_cu = torch.zeros(lengths.numel() + 1, dtype=torch.int32, device=device)
        kv_cu[1:] = (lengths + prefix).cumsum(0)
        attn_inputs.cu_kv_seqlens_device = kv_cu
...
def _target_verify_impl_class():
    ...
    class MiniMaxM3TargetVerifyFlashInferAttnOp(PyFlashinferDecodeAttnOp):
        def __init__(self, attn_configs, attn_inputs=None):
            super().__init__(attn_configs, attn_inputs)
        ...
        def _build_flashinfer_token_rows(
            ...
            # fillDecodeCudaGraphParams requires CUDA sequence_lengths_plus_1.
        ...
        def _kv_dtype(self, attn_inputs):
            # KvCacheDataType only exposes BASE/FP8 on this runtime. Looking up
            # a removed INT8 member raises AttributeError and aborts the C++
            # PyWrappedModel constructor thread (uncaught → SIGABRT).
    ...
    class MiniMaxM3TargetVerifyImpl(FMHAImplBase):
        def _create_fmha_impl(self, attn_configs, attn_inputs=None):
            ...
            return MiniMaxM3TargetVerifyFlashInferAttnOp(attn_configs, attn_inputs)

        def __init__(self, attn_configs, attn_inputs, parallelism_config=None):
            self.need_rope_kv_cache = attn_configs.need_rope_kv_cache
            self.attn_configs = attn_configs
            self.attn_inputs = attn_inputs
            _ensure_target_verify_prefill_seqlens(attn_inputs)
