...
class Qwen3GemmModel(DisaggregateModelBase):
    ...
    def send_to_attention(
        ...
        for idx, size in enumerate(micro_batch_size_list):
            ...
            if (
                captured_hidden_states is not None
                and not self.capture_hidden_states_by_attn_rank[idx]
            ):
                # The final-hidden view has packed-row gaps; copy only this rank's
                # payload. Contiguous single-row and empty slices need no copy.
...
class Qwen3DisaggregateModel(GptModelBase):
    def __init__(
        ...
        ffn_disaggregate_config = parallelism_config.ffn_disaggregate_config
        # forward_micro_batch returns final-normalized hidden states. The C++
        # wrapper uses this capability to avoid applying final norm twice.
        self.micro_batch_final_norm = True
