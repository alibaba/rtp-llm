...
class GptModelBase(nn.Module):
    ...
    def initialize(self, init_resource: PyModelInitResources) -> bool:
        ...
        if self.kv_cache is not None and getattr(self.kv_cache, "dsa_mla_resident_tokens", 0):
            if self.micro_batch_size != 1:
                raise ValueError("Pinned MLA working sets require enable_layer_micro_batch=0")
            ...
            cache_key = (
                self.kv_cache.get_layer_cache(0).kv_cache_base.data_ptr(),
                self.kv_cache.block_generations.data_ptr(),
                self.kv_cache.dsa_mla_resident_tokens,
                self.kv_cache.dsa_mla_hbm_blocks,
                self.kv_cache.mla_hbm_cache_by_layer[0].data_ptr(),
            )
