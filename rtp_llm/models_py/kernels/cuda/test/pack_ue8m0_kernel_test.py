...
requires_sm100 = pytest.mark.skipif(
    not _is_sm100(), reason="Requires SM100 (Blackwell) GPU"
)
...
class TestPackUe8m0KernelLauncher:
    """Tests for pack_ue8m0_kernel_launcher from deepgemm_wrapper.py."""

    @pytest.mark.parametrize("shape", PACK_SHAPES_GRAN1)
    def test_pack_kernel_gran1_matches_reference(self, shape):
        """
        ...
        assert (
            ref_output.shape == our_output.shape
    ...
    def test_pack_kernel_2d_input_gran1(self):
        ...
        diff = (ref_output != our_output).sum().item()
        assert diff == 0, f"Values mismatch: {diff} elements differ"
...
class TestCreatePackedScaleTensor:
    ...
    def test_tensor_shape(self, shape):
        ...
        tensor = create_packed_scale_tensor(E, T, hidden_dim, group_size, "cuda")
        ...
        assert tensor.dtype == torch.int32, f"Expected int32, got {tensor.dtype}"

    @pytest.mark.parametrize("shape", FUSED_SHAPES)
    def test_tensor_strides(self, shape):
        ...
        assert (
            tensor.stride() == expected_strides
...
class TestSiluAndMulPostQuantPackedFwd:
    """Tests for silu_and_mul_masked_post_quant_packed_fwd from activation.py."""

    @pytest.mark.parametrize("shape", FUSED_SHAPES)
    def test_fp8_output_matches_separate_kernel(self, shape):
        """
        ...
        assert output_diff == 0, f"FP8 output mismatch: {output_diff} elements differ"

    @pytest.mark.parametrize("shape", FUSED_SHAPES)
    def test_packed_scale_matches_reference(self, shape):
        """
        ...
        # Shapes should match
        assert (
            ref_scale_packed.shape == our_scale_packed.shape
        ...
        assert scale_diff == 0, f"Packed scale mismatch: {scale_diff} elements differ"
...
class TestDeepGemmIntegration:
    ...
    def test_gemm_with_our_pack_vs_reference_pack(self):
        ...
        assert (
            a_scale_packed_ref.shape == a_scale_packed_ours.shape
        ...
        assert max_diff < 1e-5, f"GEMM output max diff: {max_diff}"
        assert mean_diff < 1e-6, f"GEMM output mean diff: {mean_diff}"
    ...
    def test_fused_kernel_e2e_gemm(self):
        ...
        assert fp8_diff == 0, f"FP8 quant output mismatch: {fp8_diff} elements differ"
