...
skip_condition = not torch.cuda.is_available() or not is_sm10x()
...
def adapt_fp4_quantize_scale(sf: torch.Tensor, m: int, k: int) -> torch.Tensor:
    ...
    if sf_fp8.dim() == 1:
        ...
        sf_fp8 = sf_fp8.reshape(rounded_m, rounded_k)
    elif sf_fp8.dim() == 2:
        current_m, current_k = sf_fp8.shape
...
@pytest.mark.skipif(
    skip_condition, reason="The legacy NVFP4 wrappers are supported only on SM10x."
)
...
def test_nvfp4_gemm(
    ...
    a_fp4, a_scale_interleaved = scaled_fp4_quant_wrapper(a_dtype, a_global_scale)
    b_fp4, b_scale_interleaved = scaled_fp4_quant_wrapper(b_dtype, b_global_scale)
    # # Use flashinfer's fp4_quantize instead of scaled_fp4_quant
    ...
    #     is_sf_8x4_layout=False,
    # )
    # # Adapt scale factors to cutlass_scaled_fp4_mm expected format
    # a_scale_interleaved = adapt_fp4_quantize_scale(a_sf, m, k)
    # b_scale_interleaved = adapt_fp4_quantize_scale(b_sf, n, k)
    ...
    out = cutlass_scaled_fp4_mm_wrapper(
...
if __name__ == "__main__":
    # test_nvfp4_gemm(dtype=torch.bfloat16, shape=(128, 128, 64))
