...
from rtp_llm.ops.compute_ops import silu_and_mul_scaled_fp4_experts_quant


def _grouped_fp4_quant(
    input_tensor: torch.Tensor,
    input_global_scale: torch.Tensor,
    mask: torch.Tensor,
    *,
    use_silu_and_mul: bool,
):
    device = input_tensor.device
    num_experts, max_tokens, input_k = input_tensor.shape
    assert input_tensor.is_contiguous(), (
        "input_tensor must use contiguous (experts, tokens, k) storage."
    )
    assert input_global_scale.dtype == torch.float32, (
        "input_global_scale must have dtype torch.float32."
    )
    assert input_global_scale.shape == (num_experts,), (
        f"input_global_scale must have shape ({num_experts},)."
    )
    assert mask.dtype == torch.int32, "mask must have dtype torch.int32."
    assert mask.shape == (num_experts,), f"mask must have shape ({num_experts},)."
    if use_silu_and_mul:
        assert input_k % 2 == 0, f"input k must be even, but got {input_k}."
        quant_k = input_k // 2
    else:
        quant_k = input_k

    sf_vec_size = 16
    assert quant_k % sf_vec_size == 0, (
        f"quantized k must be a multiple of 16, but got {quant_k}."
    )

    scale_k = quant_k // sf_vec_size
    padded_k = (scale_k + 3) // 4 * 4
    padded_m = (max_tokens + 127) // 128 * 128

    # grouped_gemm_nt_masked excludes rows beyond mask, and the payload has no
    # padded K storage, so masked payload rows do not need initialization.
    output = torch.empty(
        num_experts,
        max_tokens,
        quant_k // 2,
    ...
    # Scale tiles are physically padded in both M and K. The quantization kernel
    # leaves those entries unwritten, so only this smaller metadata buffer is
    # zero-initialized before the Blackwell GEMM reads its tiled layout.
    ...
    silu_and_mul_scaled_fp4_experts_quant(
        output.view(num_experts * max_tokens, quant_k // 2),
        output_scales.view(num_experts * padded_m, padded_k // 4),
        input_tensor.view(num_experts * max_tokens, input_k),
        input_global_scale,
        mask,
        use_silu_and_mul=use_silu_and_mul,
    )
...
def scaled_fp4_grouped_quant(
    ...
    """Quantize contiguous ``(experts, tokens, k)`` inputs for masked FP4 GEMM.

    The scale-then-mask argument order is specific to this RTP-LLM wrapper and
    differs from FlashInfer's similarly named grouped quantization API.
    """
    return _grouped_fp4_quant(
        input_tensor,
        input_global_scale,
        mask,
...
def silu_and_mul_scaled_fp4_grouped_quant(
    ...
    """Apply SiLU-and-mul to contiguous ``(experts, tokens, 2 * k)`` inputs.
