#!/usr/bin/env bash
# After `scripts/rtpcli bazel build --profile <profile> //rtp_llm/dash_sc/proto:predict_v2_py`, create symlinks in this
...
TARGET_DIR="$BAZEL_BIN/rtp_llm/dash_sc/proto"
for f in model_config_pb2.py model_config_pb2_grpc.py predict_v2_pb2.py predict_v2_pb2_grpc.py __init__.py; do
  if [ -f "$TARGET_DIR/$f" ]; then
    ...
    echo "skip $f (run: scripts/rtpcli bazel build --profile <profile> //rtp_llm/dash_sc/proto:predict_v2_py)" >&2
  fi
done
