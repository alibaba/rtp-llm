...
def _configure_xgrammar_library_path() -> None:
    """Make Bazel's xgrammar binding discoverable to the cached tvm_ffi."""
...
def _stop_validator_workers(validator: GrammarValidator) -> None:
    ...
    while True:
        try:
            worker = validator._idle.get_nowait()
        except queue.Empty:
            break
        proc, conn, *resources = worker
        conn.close()
        if proc.is_alive():
            proc.terminate()
...
async def _run(args: argparse.Namespace) -> int:
    _configure_xgrammar_library_path()
