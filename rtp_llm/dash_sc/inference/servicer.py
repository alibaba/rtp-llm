    ...
    *,
    ...
    try:
        ...
        async for go in stream:
            ...
            cumulative_sent_ids.extend(ids_for_accounting)
            finish_reason_override = (
                LLMFinishReason.LENGTH if completion_length_trimmed else None
            )
            if (
            ...
            if (
                ...
                and finish_reason_override == LLMFinishReason.LENGTH
                and getattr(generate_config, "in_think_mode", False)
            ...
            async for request in request_iterator:
                ...
                if sampling is not None and sampling.max_new_tokens <= 0:
                    ...
                    resp = build_dash_error_response(
                        ...
                        request.model_name,
                        error_spec=error_spec,
                        status_message=f"invalid max_new_tokens: {sampling.max_new_tokens}; must be greater than 0",
                    )
                    self._record_and_report_chunk(
                        record,
                        resp,
