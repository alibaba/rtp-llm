...
class GrammarValidator:
    def __init__(
        ...
        *,
        ...
        msg = f"GrammarValidator mode=sandbox compile_backend=on queue_timeout_s={self._queue_timeout_s:g} compile_timeout_s={self._compile_timeout_s:g} compiler_threads={self._compile_threads} compiler_cache_bytes={self._cache_limit_bytes} pool={self._pool_target} worker_memory_limit_mb={worker_limit_mb}"
        logger.debug(msg)
    ...
    def validate_constraint(
        ...
        """Validate the canonical constraint that will be sent to EngineCore."""
    ...
    def _check_grammar(self, kind: str, spec: Any) -> bool:
        ...
        try:
            ...
            logger.warning(
                _with_request_id(
                    f"GrammarValidator: cannot serialize grammar spec ({e}); rejecting it"
                )
            )
            return False
        try:
            result = self._check_grammar_cached(kind, spec_str)
            ...
            logger.warning(
                _with_request_id(
                    f"GrammarValidator: unexpected grammar check failure ({e}); request may be retried"
                )
            )
            raise GrammarCheckUnavailable(
                "unexpected grammar validation failure"
            ) from e

    @functools.lru_cache(maxsize=2048)
    def _check_grammar_cached(self, kind: str, spec_str: str) -> _GrammarCheckResult:
        ...
        Raises GrammarCheckUnavailable for transient worker-pool failures so they are not cached.
        """
