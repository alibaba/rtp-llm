...
class _RpcAggregate:
    ...
    output_repetition: Optional[TailRepetitionResult | OnlineRepetitionResult] = None
    output_repetition_classification: Optional[OutputRepetitionClassification] = None
    output_repetition_check_ms: Optional[float] = None
    output_repetition_impl: Optional[str] = None
    tool_call_loop_result: Optional[ToolCallLoopResult] = None
    tool_call_loop_check_ms: Optional[float] = None
    tool_call_loop_output_span_count: int = 0
    _repetition_monitor: Optional[RequestRepetitionMonitor] = dataclasses.field(
    ...
    # at ``dash_sc/inference/servicer.py`` yields ``ModelStreamInferResponse(error_message=...)``
    ...
    def _ensure_repetition_monitor(self) -> RequestRepetitionMonitor:
        if self._repetition_monitor is None:
    ...
    def capture_request(self, request) -> None:
        ...
        if self.input_ids is None:
            ...
            if ids is not None:
                ...
                if self._repetition_monitor is not None:
        ...
        if infer is not None:
            ...
            if not self.raw_mode:
                if delta:
                    ...
                    self.update_tool_call_loop_monitor(delta)
    ...
    def detect_output_repetition(
        ...
        monitor = self._ensure_repetition_monitor()
        monitor.finalize_output(self.generated_ids)
        self._sync_repetition_fields()
        return self.output_repetition

    def update_tool_call_loop_monitor(self, delta_ids: list[int]) -> None:
        monitor = self._ensure_repetition_monitor()
        monitor.update_output_delta(delta_ids)
        self._sync_repetition_fields()

    def tool_call_loop_marker_name(self) -> Optional[str]:
        return self._ensure_repetition_monitor().tool_call_loop_marker_name()

    def output_repeat_kind(self) -> str:
        return self._ensure_repetition_monitor().output_repeat_kind()

    def repetition_diagnostic_summary(self) -> dict[str, Any]:
        """Compact repetition summary for access log grep and warning logs."""
        self._sync_repetition_fields()
        return self._ensure_repetition_monitor().diagnostic_summary()
    ...
    def build_record(
        ...
        record.update(self._ensure_repetition_monitor().record_fields())
        return record
...
class DashScGrpcAccessLogInterceptor(grpc.ServerInterceptor):
    ...
    def _report_rpc_done(
        ...
        repetition = agg.detect_output_repetition()
        if agg.output_repetition_check_ms is not None:
            kmonitor.report(
                GaugeMetrics.OUTPUT_REPETITION_CHECK_RT_METRIC,
                agg.output_repetition_check_ms,
                tags,
            )
        if repetition is not None:
            repetition_classification = agg.output_repetition_classification
            repetition_tags = self._tags(
                agg.method,
                repeat_kind=agg.output_repeat_kind(),
                implementation=agg.output_repetition_impl or "unknown",
                severity=(
                    repetition_classification.level
                    if repetition_classification is not None
                    else "unknown"
                ),
                classification_source=(
                    repetition_classification.source
                    if repetition_classification is not None
                    else "unknown"
                ),
            ...
            kmonitor.report(
                ...
                repetition.duplicate_token_count,
                repetition_tags,
            )
        tool_loop = agg.tool_call_loop_result
        if agg.tool_call_loop_check_ms is not None:
            kmonitor.report(
                GaugeMetrics.TOOL_CALL_LOOP_CHECK_RT_METRIC,
                agg.tool_call_loop_check_ms,
                tags,
            )
        if tool_loop is not None and tool_loop.hit:
            tool_loop_tags = self._tags(
                agg.method,
                repeat_threshold=str(tool_loop.threshold),
                marker_name=agg.tool_call_loop_marker_name() or "unknown",
                action="metric",
                span_overflow=str(tool_loop.span_overflow).lower(),
            )
            kmonitor.report(AccMetrics.TOOL_CALL_LOOP_QPS_METRIC, 1, tool_loop_tags)
            kmonitor.report(
                GaugeMetrics.TOOL_CALL_LOOP_REPEAT_COUNT_METRIC,
                tool_loop.repeat_count,
                tool_loop_tags,
            )
            kmonitor.report(
    ...
    def _finalize(
        ...
        if record.get("repetition_alert"):
            _LOGGER.warning(
                ...
                "output_repetition_token_len=%s tool_call_loop_token_len=%s "
                "primary_source=%s function_tool_repeated=%s "
                "repetition_monitor_impl=%s repetition_monitor_available=%s "
                "repetition_monitor_unavailable_reason=%s "
                "output_repetition_error=%s tool_call_loop_error=%s",
                record.get("request_id"),
                record.get("status"),
                record.get("output_len"),
                record.get("repetition_reason"),
                record.get("repetition_token_len"),
                record.get("output_repetition_token_len"),
                record.get("tool_call_loop_token_len"),
                record.get("repetition_primary_source"),
                record.get("function_tool_repeated"),
                ...
                record.get("output_repetition_error"),
