...
def contains_sublist(values: list[int], pattern: list[int]) -> bool:
    if not pattern:
        return False
    n = len(pattern)
    if len(values) < n:
        return False
    return any(values[i : i + n] == pattern for i in range(len(values) - n + 1))
...
def plan_dash_sc_thinking(
    ...
    *,
    ...
    if contains_sublist(ids, empty_tokens):
        return DashScThinkPlan.disabled(ids, "empty_think_present")
    ...
    if contains_sublist(ids[-4096:], bos_tokens):
        return DashScThinkPlan(
            thinking=True,
            input_ids=ids,
            max_thinking_tokens=max_thinking_tokens,
            reason="bos_inside_prompt",
        )
