...
DASH_ERROR_DATA_INSPECTION_FAILED = DashErrorSpec(
    error_no=LLMFinishReason.STOP_ENGINE_PARAM,
    finish_reason=LLMFinishReason.USE_PARAMETER_STATUS,
    status_code=400,
...
def parse_other_params(request, ds_attrs: dict[str, Any] | None = None) -> OtherParams:
    ...
    # ModelInferRequest.model_name identifies the Triton serving endpoint and can
    # be the generic literal "model".  Spectrum preserves the user-facing model
    # in ds_header_attributes.model; that is the identifier expected by GreenNet.
    inspection_model_name = _normalize_non_empty_str(ds_attrs.get("model"))
