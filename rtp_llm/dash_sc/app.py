...
class DashScShutdownManager:
    ...
    def start_unavailable(
        ...
        logging.info(
            "[DashScApp] entering pre-stop unavailable state, reason=%s, "
...
class DashScApp:
    ...
    def _install_signal_handlers(self) -> None:
        def _drain_only_handler(signum, frame):
            ...
            self._shutdown_manager.start_unavailable(
                f"signal {signum}",
        ...
        def _handler(signum, frame):
            ...
            if (
                ...
                stale_accept_seconds = max(
                    0.0,
                    self._effective_pre_stop_drain_seconds()
                    - self._shutdown_manager.drain_elapsed_seconds(),
                )
                self._shutdown_manager.start_unavailable(
                    f"signal {signum}",
                    stale_accept_seconds=stale_accept_seconds,
                )
            else:
