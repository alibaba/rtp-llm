...
class GenerateConfigTest(TestCase):
    ...
    def test_qwen_cp_alignment_preserves_user_requirement(self):
        from rtp_llm.models.qwen3_next.qwen3_next import Qwen3NextBase

        configs = self._qwen_cp_configs()
        configs.prefill_cp_config.segment_size_alignment = 128
        with patch(
            "rtp_llm.model_factory.ModelFactory.get_model_cls",
            return_value=Qwen3NextBase,
        ):
        ...
        self.assertEqual(configs.prefill_cp_config.segment_size_alignment, 128)
        self.assertEqual(
            configs.parallelism_config.prefill_cp_config.segment_size_alignment, 128
        )
        ...
        for missing_side in ("source", "target", "both"):
            for enabled in (False, True):
                with self.subTest(missing_side=missing_side, enabled=enabled):
                    ...
                    with patch(
                        "rtp_llm.config.server_config_setup.PREFILL_CP_CONFIG_CAPABILITIES",
                        frozenset(),
                    ):
    ...
    def test_setup_default_args_allows_explicit_frontend_with_old_cp_bindings(self):
        ...
        with patch(
            "rtp_llm.config.server_config_setup.PREFILL_CP_CONFIG_CAPABILITIES",
            frozenset(),
        ), patch("rtp_llm.model_factory.ModelFactory.get_model_cls") as get_model_cls:
    ...
    def test_cp_rejects_layer_micro_batch_for_model_roles(self):
        configs = self._qwen_cp_configs()
        configs.device_resource_config.enable_layer_micro_batch = 1
        with self.assertRaisesRegex(ValueError, "layer micro-batching"):
            _configure_model_prefill_cp(configs)

    def test_cp_skips_layer_micro_batch_validation_for_frontend(self):
        configs = PyEnvConfigs()
        configs.prefill_cp_config.method = CPRotateMethod.ALL_GATHER
        configs.role_config.role_type = RoleType.FRONTEND
        configs.device_resource_config.enable_layer_micro_batch = 1
