...
class AcceptsTheCanonicalProfile(unittest.TestCase):
    ...
    def test_every_decode_rank_of_the_group_is_accepted(self):
        for rank in range(4):
            self.assertEqual(
                [], rp.validate(decode_snapshot(rank_id=rank), PROFILE), f"rank {rank}"
            )
...
class RejectsDecodeMisconfiguration(unittest.TestCase):
    ...
    def test_padding_disabled_is_rejected(self):
        ...
        self.assertRejected(
            decode_snapshot(decode_fixed_bs=0), "fixed decode padding must be enabled"
        )

    def test_padding_malformed_value_is_rejected(self):
        self.assertRejected(
            decode_snapshot(decode_fixed_bs=-1), "fixed decode padding must be enabled"
        )
    ...
    def test_capture_set_missing_the_padded_batch_is_rejected(self):
        self.assertRejected(
            decode_snapshot(decode_capture_batch_sizes=[1]), "does not contain"
        )
    ...
    def test_admission_cap_above_the_padded_batch_is_rejected(self):
        ...
        self.assertRejected(
            decode_snapshot(concurrency_limit=4), "must equal the padded batch"
        )

    def test_admission_cap_below_the_padded_batch_is_rejected(self):
        self.assertRejected(
            decode_snapshot(concurrency_limit=1), "must equal the padded batch"
        )
    ...
    def test_geometry_other_than_256_is_rejected(self):
        self.assertRejected(
            decode_snapshot(kernel_seq_size_per_block=128),
    ...
    def test_a_different_speculative_mechanism_at_the_right_depth_is_rejected(self):
        ...
        self.assertRejected(
            decode_snapshot(sp_type="SpeculativeType.EAGLE"), "speculative mechanism"
        )

    def test_the_expected_speculative_mechanism_is_accepted(self):
        self.assertEqual(
            [], rp.validate(decode_snapshot(sp_type="SpeculativeType.DSPARK"), PROFILE)
        )
...
class RejectsTestOnlyAndNonDefaultOverrides(unittest.TestCase):
    def assertRejected(self, snapshot, needle):
        violations = rp.validate(snapshot, PROFILE)
        self.assertTrue(
            any(needle in v for v in violations), f"expected {needle!r} in {violations}"
        )

    def test_forced_expert_routing_is_rejected(self):
        self.assertRejected(
            decode_snapshot(fake_balance_expert=True), "fake_balance_expert"
        )

    def test_exact_batch_test_scheduler_is_rejected(self):
        self.assertRejected(
            decode_snapshot(use_batch_decode_scheduler=True),
            "use_batch_decode_scheduler",
        )

    def test_forced_transport_override_is_rejected(self):
        self.assertRejected(
            decode_snapshot(nccl_p2p_level="PHB"), "non-default NCCL transport"
        )

    def test_unapproved_experimental_moe_backend_is_rejected(self):
        self.assertRejected(
            decode_snapshot(moe_fp4_backend="deepgemm"),
            "unapproved experimental MoE backend",
        )
...
class RejectsPrefillMisconfiguration(unittest.TestCase):
    def test_admission_bound_above_the_all_to_all_guard_is_rejected(self):
        ...
        violations = rp.validate(
            prefill_snapshot(max_batch_tokens_size=133120), PROFILE
        )
        self.assertTrue(
            any("all-to-all payload guard" in v for v in violations), violations
        )
    ...
    def test_the_profile_bound_is_accepted(self):
        self.assertEqual(
            [], rp.validate(prefill_snapshot(max_batch_tokens_size=64000), PROFILE)
        )
...
class RejectsGraphTopologyRankAndMembershipCases(unittest.TestCase):
    """Reject inconsistent graph settings, topology, membership and artifact identity."""
    ...
    def test_decode_with_cuda_graph_disabled_is_rejected(self):
        ...
        self.assertRejected(
            decode_snapshot(enable_cuda_graph=False), "enable_cuda_graph=False"
        )

    def test_decode_with_unresolved_graph_setting_is_rejected(self):
        self.assertRejected(
            decode_snapshot(enable_cuda_graph=rp.UNSET), "not resolvable"
        )
    ...
    def test_prefill_without_context_parallelism_is_rejected(self):
        ...
        self.assertRejected(
            prefill_snapshot(cp_computes=False), "is_enabled() is false"
        )
    ...
    def test_decode_not_declaring_a_cp_prefill_is_rejected(self):
        ...
        self.assertRejected(
            decode_snapshot(cp_consumes_prefill=False), "is_prefill_enabled() is false"
        )

    def test_sharded_kv_without_an_explicit_cp_size_is_rejected(self):
        self.assertRejected(
            decode_snapshot(prefill_cp_size=1),
            "requires an explicit prefill CP size > 1",
        )

    def test_prefill_with_the_decode_cp_method_is_rejected(self):
        self.assertRejected(
            prefill_snapshot(prefill_cp_method="CPRotateMethod.PREFILL_CP"),
            "prefill leg of this profile expects",
        )

    def test_prefill_geometry_128_is_rejected(self):
        self.assertRejected(
            prefill_snapshot(kernel_seq_size_per_block=128),
            "prefill leg must use the qualified",
        )
    ...
    def test_pd_kv_sharing_mismatch_is_rejected_on_either_leg(self):
        self.assertRejected(
            decode_snapshot(prefill_cp_kv_cache_sharded=False),
            "prefill_cp_kv_cache_sharded=False",
        )
        self.assertRejected(
            prefill_snapshot(prefill_cp_kv_cache_sharded=False),
            "prefill_cp_kv_cache_sharded=False",
        )

    def test_group_with_a_single_rank_is_rejected(self):
        manifests = {
            "decode-0": rp.build_manifest(
                decode_snapshot(rank_id=0), PROFILE, {"git_sha": "abc"}
            ),
            "prefill-0": rp.build_manifest(
                prefill_snapshot(rank_id=0), PROFILE, {"git_sha": "abc"}
            ),
        }
        violations = rp.check_group_consistency(manifests)
        self.assertTrue(
            any("INCOMPLETE MEMBERSHIP" in v for v in violations), violations
        )
    ...
    def test_group_without_artifact_identity_is_rejected(self):
        ...
        for i in range(4):
            manifests[f"decode-{i}"] = rp.build_manifest(
                decode_snapshot(rank_id=i), PROFILE, None
            )
            manifests[f"prefill-{i}"] = rp.build_manifest(
                prefill_snapshot(rank_id=i), PROFILE, None
            )
        violations = rp.check_group_consistency(manifests)
        self.assertTrue(
            any("MISSING ARTIFACT IDENTITY" in v for v in violations), violations
        )
    ...
    def test_mixed_release_profiles_are_rejected(self):
        ...
        violations = rp.check_group_consistency(manifests)
        self.assertTrue(
            any("MIXED RELEASE PROFILES" in v for v in violations), violations
        )
...
class FailsClosedOnUnresolvableConfiguration(unittest.TestCase):
    def test_a_missing_knob_is_a_violation_not_a_default(self):
        ...
        self.assertTrue(
            any("concurrency_limit: not resolvable" in v for v in violations),
            violations,
        )

    def test_a_missing_capture_set_is_a_violation(self):
        violations = rp.validate(
            decode_snapshot(decode_capture_batch_sizes=rp.UNSET), PROFILE
        )
        self.assertTrue(
            any("decode_capture_batch_sizes: not resolvable" in v for v in violations),
            violations,
        )

    def test_an_unknown_role_is_a_violation(self):
        violations = rp.validate(
            decode_snapshot(role_type="RoleType.FRONTEND"), PROFILE
        )
        self.assertTrue(
            any("neither a decode nor a prefill role" in v for v in violations),
            violations,
        )
...
class SnapshotFromResolvedConfig(unittest.TestCase):
    ...
    def stub_config(self):
        return SimpleNamespace(
            ...
            parallelism_config=SimpleNamespace(
                tp_size=1, dp_size=4, ep_size=4, world_size=4
            ),
            concurrency_config=SimpleNamespace(concurrency_limit=2),
            sp_config=SimpleNamespace(
                type="DSpark", gen_num_per_cycle=3, checkpoint_path=None
            ),
            model_args=SimpleNamespace(
                max_seq_len=32832, ckpt_path=None, tokenizer_path=None
            ),
            kv_cache_config=SimpleNamespace(
                kernel_seq_size_per_block=256, reuse_cache=False
            ),
            py_hw_kernel_config=SimpleNamespace(
                decode_capture_batch_sizes=[1, 2], enable_cuda_graph=True
            ),
            ...
            runtime_config=SimpleNamespace(
                fifo_scheduler_config=SimpleNamespace(
                    max_batch_tokens_size=32832, max_context_batch_size=1
                ),
                use_batch_decode_scheduler=False,
            ),
    ...
    def test_padding_padding_absent_from_env_means_disabled(self):
        ...
        self.assertTrue(
            any("padding must be enabled" in v for v in rp.validate(snap, PROFILE))
        )

    def test_malformed_padding_value_is_not_read_as_the_default(self):
        snap = rp.snapshot_from_env_configs(
            self.stub_config(), env={rp.DECODE_FIXED_BS_ENV: "two"}
        )
    ...
    def test_transport_and_backend_overrides_are_captured(self):
        snap = rp.snapshot_from_env_configs(
            self.stub_config(),
            env={
                rp.DECODE_FIXED_BS_ENV: "2",
                rp.NCCL_P2P_LEVEL_ENV: "PHB",
                rp.MOE_FP4_BACKEND_ENV: "deepgemm",
            },
        )
...
class GroupConsistency(unittest.TestCase):
    def group(self, decode_artifact=None, prefill_artifact=None, **decode_overrides):
        ...
        for rank in range(4):
            manifests[f"decode-{rank}"] = rp.build_manifest(
                decode_snapshot(rank_id=rank, **decode_overrides),
                PROFILE,
                decode_artifact,
            )
        for rank in range(4):
    ...
    def test_the_candidate_token_envelope_gap_is_surfaced_but_not_silently_accepted(
        self,
    ):
        # Report differing workspace limits; ingress must enforce request token bounds separately.
        art = {"git_sha": "abc"}
    ...
    def test_the_envelope_rule_rejects_once_enforcement_is_enabled(self):
        ...
        violations = rp.check_group_consistency(
            self.group(art, art), enforce_token_envelope=True
        )
        self.assertTrue(
            any("TOKEN ENVELOPE MISMATCH" in v for v in violations), violations
        )
    ...
    def test_mixed_configuration_within_a_role_is_detected(self):
        ...
        manifests["decode-2"] = rp.build_manifest(
            decode_snapshot(rank_id=2, kernel_seq_size_per_block=128),
            PROFILE,
            {"git_sha": "abc"},
        )
        violations = rp.check_group_consistency(manifests)
    ...
    def test_rank_ids_may_repeat_across_roles_but_not_within_one(self):
        ...
        manifests["decode-3"] = rp.build_manifest(
            decode_snapshot(rank_id=2), PROFILE, art
        )
        violations = rp.check_group_consistency(manifests)
        self.assertTrue(
            any("DUPLICATE rank ids among ranks with role" in v for v in violations),
            violations,
        )
...
class CanonicalSettingsArtifact(unittest.TestCase):
    ...
    def test_the_checked_in_json_matches_the_profile(self):
        ...
        self.assertTrue(
            os.path.exists(path), f"canonical settings artifact is missing: {path}"
        )
        with open(path) as f:
            on_disk = f.read()
        self.assertEqual(
    ...
    def test_the_json_parses_and_carries_both_legs(self):
        ...
        self.assertEqual(
            PROFILE.decode_fixed_bs, payload["per_leg"]["decode"]["fixed_bs"]
        )
        self.assertEqual(
            PROFILE.decode_fixed_bs, payload["per_leg"]["decode"]["concurrency_limit"]
        )
        self.assertIn(
            PROFILE.decode_fixed_bs, payload["per_leg"]["decode"]["capture_batch_sizes"]
        )
        self.assertTrue(payload["per_leg"]["decode"]["enable_cuda_graph"])
        self.assertEqual(
            PROFILE.prefill_max_batch_tokens_size,
            payload["per_leg"]["prefill"]["max_batch_tokens_size"],
        )
    ...
    def test_the_canonical_settings_are_self_consistent_with_the_validator(self):
        ...
        self.assertEqual(PROFILE.expected_decode_ranks, PROFILE.expected_prefill_ranks)
        self.assertLessEqual(
            PROFILE.prefill_max_batch_tokens_size, rp.A2A_PAYLOAD_TOKEN_BOUND
        )
...
class EnforceReleaseProfile(unittest.TestCase):
    ...
    def test_an_unknown_profile_is_rejected(self):
        with self.assertRaises(rp.ReleaseProfileError) as ctx:
            rp.enforce_release_profile(
                self.stub_config(), env={rp.RELEASE_PROFILE_ENV: "nope"}
            )
    ...
    def test_a_valid_decode_rank_returns_its_manifest(self):
        manifest = rp.enforce_release_profile(
            self.stub_config(),
            env={
                rp.RELEASE_PROFILE_ENV: "sm120_dp4ep4_n2",
                rp.DECODE_FIXED_BS_ENV: "2",
            },
...
class GroupRendezvous(unittest.TestCase):
    ...
    def test_records_from_another_attempt_are_ignored_and_counted(self):
        ...
        self.assertEqual(8, len(manifests))
        self.assertEqual(
            8, stale, "the earlier attempt's manifests must be ignored, not merged"
        )

    def test_a_second_claimant_cannot_overwrite_the_incumbent(self):
        # A duplicate claim must preserve the incumbent's manifest.
        self.publish_all()
        with open(os.path.join(self.dir, "a1__RoleType-DECODE__0.json")) as f:
            incumbent = json.load(f)
        impostor = rp.build_manifest(
            decode_snapshot(rank_id=0), PROFILE, {"git_sha": "impostor"}
        )
        with self.assertRaises(rp.ReleaseProfileError) as ctx:
    ...
    def test_a_duplicate_claim_by_a_peer_is_preserved_and_rejected(self):
        ...
        self.assertTrue(
            any("DUPLICATE RANK OWNERSHIP" in v for v in violations), violations
        )
    ...
    def test_join_without_a_configured_directory_is_a_violation(self):
        ...
        violations = rp.join_group_and_verify(
            manifest, env={}, attempt="a1", timeout_s=0
        )
        self.assertTrue(
            any("GROUP VERIFICATION NOT CONFIGURED" in v for v in violations),
            violations,
        )
    ...
    def test_an_incomplete_group_stops_the_rank(self):
        ...
        violations = rp.join_group_and_verify(
            manifest, env=self.join_env(), timeout_s=0
        )
...
class ContentIdentity(unittest.TestCase):
    """Identify loaded artifacts by content, with explicit full/sample coverage."""
    ...
    def test_discovered_libraries_are_compiled_shared_objects(self):
        ...
        self.assertTrue(
            selected, "no library path was discovered in the deployed layout"
        )
    ...
    def test_a_file_that_cannot_be_read_fails_the_tree(self):
        ...
        self.assertTrue(
            any("could not be read" in e for e in broken["errors"]), broken["errors"]
        )
    ...
    def test_a_directory_that_cannot_be_listed_fails_the_tree(self):
        ...
        def walk_with_denied_subdir(top, onerror=None, **kwargs):
            for dirpath, dirnames, filenames in real_walk(top, **kwargs):
                if os.path.basename(dirpath) == "sub" and onerror is not None:
                    onerror(
                        PermissionError(
                            13, "Permission denied", os.path.join(dirpath, "denied")
                        )
                    )
        ...
        self.assertTrue(
            any("could not be listed" in e for e in tree["errors"]), tree["errors"]
        )
    ...
    def test_the_full_inventory_is_kept_when_the_listing_is_capped(self):
        ...
        self.assertEqual(
            rp.MAX_LISTED_FILES + 2, len(tree["files"]), "inventory must be complete"
        )
    ...
    def test_an_empty_manifest_over_a_capped_tree_is_rejected(self):
        # An empty manifest cannot establish full coverage for sampled files.
        for i in range(rp.MAX_LISTED_FILES + 2):
    ...
    def test_declared_digests_are_not_proof_until_verified(self):
        ...
        good = self.write(
            "good.sha256", "%s  shard.safetensors\n" % _h.sha256(payload).hexdigest()
        )
    ...
    def test_strict_mode_rejects_an_unverified_declaration(self):
        ...
        self.assertTrue(
            any("verified full content digest" in e for e in ids.get("errors", [])), ids
        )
    ...
    def test_require_full_weights_rejects_a_sampled_identity(self):
        ...
        env = {
            rp.FULL_HASH_MAX_ENV: "0.001",
            rp.SAMPLE_BYTES_ENV: "0.0005",
            rp.REQUIRE_FULL_WEIGHTS_ENV: "1",
        }
        ids = rp.artifact_identity_from(stub, env)
        self.assertTrue(
            any(
                "requires a verified full content digest" in e
                for e in ids.get("errors", [])
            ),
            ids.get("errors"),
        )
        ...
        self.assertFalse(
            [
                e
                for e in ids2.get("errors", [])
                if "requires a verified full content digest" in e
            ],
            ids2,
        )
