...
class ModelConfig(CppModelConfig):
    ...
    _python_fields = {
        ...
        'inter_size', 'moe_inter_size', 'generate_env_config', 'render_config',
    }
    
    # Known C++ ModelConfig members (from ModelConfig.h)
    # These may not be explicitly exposed via pybind11 but exist in C++
    _cpp_members = {
        'num_layers', 'max_seq_len', 'vocab_size', 'hidden_size',
        'attn_config', 'special_tokens',
        ...
        'mla_ops_type', 'extra_data_path', 'local_extra_data_path',
        'use_float32', 'model_type', 'ptuning_path',
    ...
    def layer_weight_param_count(self) -> int:
        ...
        layer_weight_param_count = (
            ...
            + self.num_layers * hidden_size * (self.attn_config.kv_head_num * self.attn_config.size_per_head) * 2
        )
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.moe_inter_size: int = 0  # MOE intermediate size (for MOE expert FFN layers)
        
        # Renderer configuration fields
        self.generate_env_config: Optional[Any] = None  # GenerateEnvConfig for renderer factory
    ...
    def init_precision_config(self, kv_cache_config: Optional[Any], act_type: Optional[str]):
...
def build_model_config(
    ...
    It copies values from model_args to model_config, then sets up model-specific fields.
        ...
        py_hw_kernel_config: HWKernelConfig for align_size calculation
        profiling_debug_logging_config: ProfilingDebugLoggingConfig for hack_layer_num
        parallelism_config: ParallelismConfig for align_size calculation
    ...
    if model_args.task_type:
        model_config.task_type = model_args.task_type
    ...
    model_config.init_precision_config(kv_cache_config=kv_cache_config, act_type=model_args.act_type)
    ...
    if model_config.hidden_size == 0:
        model_config.hidden_size = model_config.attn_config.size_per_head * model_config.attn_config.head_num
