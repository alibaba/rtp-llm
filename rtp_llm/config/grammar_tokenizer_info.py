...
def build_model_grammar_tokenizer_info_json(
    ...
    """Build the tokenizer serialization shared by admission and EngineCore."""
    ...
    # BaseModel refreshes this field from the loaded tokenizer before building
    # metadata. Admission owns a separately loaded but equivalent tokenizer, so
    # read it directly here to keep both processes on the same path.
    tokenizer_eos_token_id = getattr(tokenizer, "eos_token_id", None)
    add_id(
        tokenizer_eos_token_id
        if tokenizer_eos_token_id is not None
        else special_tokens.eos_token_id
    )
