...
def setup_jit_cache_envs(py_env_configs: PyEnvConfigs) -> None:
    """Use the same local component caches for warmup and normal serving."""
    from rtp_llm.utils.jit_cache_manager import setup_jit_cache_env

    removed_envs = [
        name
        for name in ("REMOTE_JIT_READ_DIR", "WARM_UP_JIT_AND_WRITE_REMOTE")
        if os.environ.get(name)
    ]
    if removed_envs:
        raise ValueError(
            f"{', '.join(removed_envs)} is no longer supported; use REMOTE_JIT_DIR "
            "for startup restore and watchdog snapshot sync"
        )

    local_root = setup_jit_cache_env()
    remote_jit_dir = (py_env_configs.jit_config.remote_jit_dir or "").strip()
    logging.info(
        "JIT cache root=%s; remote snapshot sync=%s",
        local_root,
        remote_jit_dir or "disabled",
    )
