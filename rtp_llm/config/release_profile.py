"""Opt-in release profiles with fail-closed configuration and artifact validation.

Ranks compare manifests before model loading; no per-step collective is added.
Runtime memory capacity and endpoint readiness require separate checks.
"""
...
# An optional packaging label supplements the content-derived artifact identity.
ARTIFACT_LABEL_ENV = "RTP_LLM_RELEASE_ARTIFACT_ID"

# These settings are read directly from the environment, not the resolved config.
DECODE_FIXED_BS_ENV = "RTP_LLM_DECODE_FIXED_BS"
NCCL_P2P_LEVEL_ENV = "NCCL_P2P_LEVEL"
MOE_FP4_BACKEND_ENV = "DSV4_MOE_FP4_BACKEND"

# Match the MoE all-to-all payload guard to avoid the larger fixed-EP fallback buffers.
A2A_PAYLOAD_TOKEN_BOUND = 65536

# Both PD roles must see the same directory for bounded startup rendezvous.
...
# Hash libraries and metadata in full. Large weight shards use explicitly marked
# head/tail samples unless a verified full-digest manifest is supplied.
...
def _resolve(root: Any, path: str) -> Any:
    """Resolve a dotted config path, preserving missing fields as UNSET."""
    cur = root
...
def _call(root: Any, path: str) -> Any:
    """Invoke a config getter, returning UNSET if it is absent or raises."""
    cur = root
    for part in path.split("."):
        if cur is None:
            return UNSET
    ...
    try:
        ...
        Exception
    ):  # noqa: BLE001 - a probe must not be able to fail startup by itself
        return UNSET
    ...
    def resolved_fields(self) -> Dict[str, Any]:
        ...
        for name in (
            ...
            if isinstance(value, _Unset):
                out[name] = (
                    None  # unresolved: visible in the manifest, and a violation under a profile
                )
...
def snapshot_from_env_configs(
    ...
    return ReleaseSnapshot(
        ...
        kernel_seq_size_per_block=_resolve(
            cfg, "kv_cache_config.kernel_seq_size_per_block"
        ),
        decode_capture_batch_sizes=_resolve(
            cfg, "py_hw_kernel_config.decode_capture_batch_sizes"
        ),
        reuse_cache=_resolve(cfg, "kv_cache_config.reuse_cache"),
        max_batch_tokens_size=_resolve(
            cfg, "runtime_config.fifo_scheduler_config.max_batch_tokens_size"
        ),
        max_context_batch_size=_resolve(
            cfg, "runtime_config.fifo_scheduler_config.max_context_batch_size"
        ),
        fake_balance_expert=_resolve(cfg, "moe_config.fake_balance_expert"),
        use_batch_decode_scheduler=_resolve(
            cfg, "runtime_config.use_batch_decode_scheduler"
        ),
...
def _tree_identity(root: str, full_max_bytes: int, sample_bytes: int) -> Dict[str, Any]:
    ...
    for dirpath, dirnames, filenames in os.walk(root, onerror=on_walk_error):
        ...
        for name in sorted(filenames):
            ...
            try:
                files[os.path.relpath(path, root)] = _content_digest(
                    path, full_max_bytes, sample_bytes
                )
            except OSError as e:
    ...
    if unlistable:
        complete = False
        errors.append(
            f"{len(unlistable)} directory(ies) could not be listed: {unlistable[:3]}"
        )
    ident: Dict[str, Any] = {
        "kind": "tree",
        "root": root,
        ...
        "complete": complete,
        "mode": (
            ("full" if all(v["mode"] == "full" for v in files.values()) else "sampled")
...
def _weights_manifest(
    ...
    for line in raw.decode("utf-8", "replace").splitlines():
        ...
        if (
            len(parts) != 2
            or len(parts[0]) != 64
            or any(c not in "0123456789abcdefABCDEF" for c in parts[0])
        ):
    ...
    if verify_full:
        ...
        for name in sorted(files):
            ...
            if want is None:
                continue
            try:
                got = _content_digest(os.path.join(root, name), 0, 0, force_full=True)[
                    "sha256"
                ]
            except OSError as e:
...
def _resolve_libraries() -> Tuple[Dict[str, str], Dict[str, List[str]], List[str]]:
    ...
    try:
        ...
        return (
            ...
            {},
            [
                "rtp_llm could not be imported, so no compiled library could be identified"
            ],
        )
...
def artifact_identity_from(
    ...
    verify_manifest = require_full or _truthy(
        environment.get(VERIFY_WEIGHTS_MANIFEST_ENV)
    )
    ids: Dict[str, Any] = {}
    errors: List[str] = []
    ...
    if libs:
        ...
        errors.append(
            "no compiled library could be identified, so the running binary is not identified"
        )
    ...
    for key, path in (
        ...
        if manifest_path and ident.get("kind") == "tree":
            wm, wm_errors = _weights_manifest(
                manifest_path,
                ident,
                verify_full=verify_manifest,
                full_max_bytes=full_max,
                sample_bytes=sample,
            )
            if wm:
...
def validate(snapshot: ReleaseSnapshot, profile: ReleaseProfile) -> List[str]:
    ...
    def need(name: str) -> Any:
        ...
        if isinstance(value, _Unset):
            v.append(
                f"{name}: not resolvable from the configuration object (fail closed)"
            )
        return value
    ...
    if not (decode or prefill):
        v.append(
            f"role_type: {role!r} is neither a decode nor a prefill role for this profile"
        )

    # ---- test-only knobs must be off in a release profile ----------------------------------------
    if need("fake_balance_expert") is True:
        v.append(
            "fake_balance_expert: test-only forced expert routing must be disabled in release mode"
        )
    ...
    if (
        snapshot.nccl_p2p_level is not None
        and snapshot.nccl_p2p_level not in profile.approved_nccl_p2p_levels
    ):
        v.append(
    ...
    if (
        snapshot.moe_fp4_backend is not None
        and snapshot.moe_fp4_backend not in profile.approved_moe_backends
    ):
        v.append(
    ...
    if (
        not isinstance(max_seq_len, _Unset)
        and int(max_seq_len) < profile.min_max_seq_len
    ):
        v.append(
    ...
    if not isinstance(depth, _Unset) and int(depth) != profile.gen_num_per_cycle:
        v.append(
            f"gen_num_per_cycle={depth}: profile requires {profile.gen_num_per_cycle}"
        )

    sp_type = need("sp_type")
    if (
        not isinstance(sp_type, _Unset)
        and profile.speculative_type not in str(sp_type).upper()
    ):
        v.append(
    ...
    if decode:
        ...
        if (
            not isinstance(graph, _Unset)
            and bool(graph) != profile.decode_enable_cuda_graph
        ):
            v.append(
        ...
        if (
            not isinstance(cp_method, _Unset)
            and profile.decode_cp_rotate_method not in str(cp_method).upper()
        ):
            v.append(
        ...
        if (
            ...
            and int(cp_size) != profile.decode_prefill_cp_size
        ):
            v.append(
                f"prefill_cp_size={cp_size}: profile requires {profile.decode_prefill_cp_size}"
            )
        ...
        if fixed_bs <= 0:
            ...
            v.append(
                f"{DECODE_FIXED_BS_ENV}={fixed_bs}: profile requires {profile.decode_fixed_bs}"
            )
        ...
        if (
            not isinstance(geom, _Unset)
            and int(geom) != profile.decode_kernel_seq_size_per_block
        ):
            v.append(
    ...
    if prefill:
        ...
        for name, expected in (
            ...
            if not isinstance(actual, _Unset) and int(actual) != expected:
                v.append(
                    f"{name}={actual}: profile requires {expected} for the prefill role"
                )

        geom = need("kernel_seq_size_per_block")
        if (
            not isinstance(geom, _Unset)
            and int(geom) != profile.prefill_kernel_seq_size_per_block
        ):
            v.append(
        ...
        if (
            not isinstance(cp_method, _Unset)
            and profile.prefill_cp_rotate_method not in str(cp_method).upper()
        ):
            v.append(
    ...
    if not isinstance(world, _Unset):
        expected_ranks = (
            profile.expected_decode_ranks if decode else profile.expected_prefill_ranks
        )
...
def group_token_envelope(
    manifests: Mapping[Any, Mapping[str, Any]]
) -> Tuple[Optional[int], Optional[int]]:
    """Return the smallest advertised prefill and decode sequence limits.

    A mismatch is reported, not rejected: prefill's limit also sizes its workspace.
    Request admission must independently enforce the decode limit.
    """
...
def check_group_consistency(
    ...
    profile = (
        RELEASE_PROFILES.get(next(iter(profile_names)))
    ...
    if profile is not None and profile.require_artifact_identity:
        no_identity = sorted(
            str(r) for r, m in manifests.items() if not m.get("artifact")
        )
        if no_identity:
            v.append(
    ...
    for rank, m in manifests.items():
        ...
        by_role.setdefault(role, {}).setdefault(
            str(m.get("group_config_digest", "")), []
        ).append(rank)
    ...
    if enforce_token_envelope:
        ...
        if (
            prefill_limit is not None
            and decode_limit is not None
            and prefill_limit > decode_limit
        ):
            v.append(
...
def _manifest_identity(manifest: Mapping[str, Any]) -> Tuple[str, int]:
    ...
    if rank is None:
        raise ReleaseProfileError(
            "a rank manifest without a rank_id cannot be published or compared"
        )
    return role, int(rank)


def publish_rank_manifest(
    manifest: Mapping[str, Any], directory: str, attempt: str
) -> str:
    ...
    try:
        os.link(
            tmp, path
        )  # atomic create-if-absent: fails when the slot is already claimed
...
def join_group_and_verify(
    ...
    if profile is None:
        return [
            f"GROUP VERIFICATION: manifest names unknown profile {manifest.get('profile')!r}"
        ]
    ...
    expected_total = int(profile.expected_decode_ranks) + int(
        profile.expected_prefill_ranks
    )
    try:
...
PROFILE_DIR = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "release_profiles"
)
...
def enforce_release_profile(
    ...
    artifacts = (
        ...
        else artifact_identity_from(cfg, environment)
    )
    ...
    for problem in (
        manifest["artifact"].get("errors", [])
        if isinstance(manifest["artifact"], Mapping)
        else []
    ):
    ...
    logging.info(
        "[RELEASE-PROFILE] resolved manifest: %s",
    ...
    artifact = (
        ...
        if isinstance(manifest.get("artifact"), Mapping)
        else {}
    )
    ...
    logging.info(
        ...
        "identity_errors=%d",
        libs.get("count"),
        str(libs.get("sha256", ""))[:16],
        weights.get("mode"),
        weights.get("file_count"),
        len(artifact.get("errors", []) or []),
    )
