...
# CUTLASS DSL 4.6 removed the explicit result-type argument from NVVM builders.
...
def fmax(
    ...
    a_value = Float32(a).ir_value(loc=loc, ip=ip)
    b_value = Float32(b).ir_value(loc=loc, ip=ip)
    c_value = Float32(c).ir_value(loc=loc, ip=ip) if c is not None else None
    if _NVVM_FMAX_TAKES_RESULT_TYPE:
        result = nvvm.fmax(
            T.f32(), a_value, b_value, c=c_value, loc=loc, ip=ip
        )
    else:
        result = nvvm.fmax(
            a_value,
