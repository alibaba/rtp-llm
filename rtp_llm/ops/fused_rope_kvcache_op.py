...
class FusedRopeKVCachePrefillOpBase:
    ...
    def prepare(self, attn_inputs: PyAttentionInputs) -> FusedRopeAttnParams:
        if attn_inputs.kv_cache_block_id_host is not None and attn_inputs.kv_cache_block_id_host.numel() > 0:
    ...
    def _forward(
        ...
        rope_cache = get_rope_cache_once(rope_config, self.max_seq_len)
        ...
        return prefill_fused_rope_kvcache(
            ...
            rope_cache=rope_cache.data if check_rope_cache(rope_config, rope_cache) else None,
    ...
    def forward(
        self, qkv: torch.Tensor, kv_cache: Optional[KVCache], params: FusedRopeAttnParams
    ) -> torch.Tensor:
...
class FusedRopeKVCacheDecodeOp:
    ...
    def prepare(self, attn_inputs: PyAttentionInputs) -> FusedRopeAttnParams:
        assert attn_inputs.kv_cache_block_id_host is not None and attn_inputs.kv_cache_block_id_host.numel() > 0
        kv_cache_offset = convert_offset_to_block_array(
            attn_inputs.kv_cache_block_id_device
        )
        kv_cache_offset_h = None
