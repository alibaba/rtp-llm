...
class FusedRopeKVCachePrefillOp:
    ...
    def forward(
        ...
        # not support fp8 now
        if (
            fmha_type == FMHAType.TRT_V2
            and params.max_prefix_length > 0
            and kv_cache is not None
            and self.attn_configs.kv_cache_dtype == KvCacheDataType.BASE
        ):
            fmha_type = FMHAType.PAGED_TRT_V2

        store_qkv = fmha_type not in [
            FMHAType.PAGED_TRT_V2,
            FMHAType.NONE,
            FMHAType.FLASH_INFER,
        ]
        store_q_no_transpose = fmha_type in [
            FMHAType.FLASH_INFER,
            FMHAType.PAGED_TRT_V2,
        ]
        store_q = fmha_type == FMHAType.NONE
        store_kv = fmha_type == FMHAType.NONE
        store_cache = kv_cache is not None
        store_qkv_fp8 = False
        use_paged_fmha = fmha_type == FMHAType.PAGED_TRT_V2
