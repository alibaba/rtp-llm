...
TOP_K = 6
NUM_EXPERTS = 256
COLLECTION_GENERATION_TOKENS = 8
...
def validate_decode(root: Path, query_id: str) -> tuple[int, int, str]:
    query_dir = root / f"query_{query_id}"
    actual = sorted(query_dir.rglob("*.pt")) if query_dir.exists() else []
    expected_layer_dirs = [
        query_dir / f"step_{step:03d}" / model / f"layer_{layer:03d}"
        for step in range(3)
        for model, layer in model_layers()
    ]
    if len(actual) != len(expected_layer_dirs):
        raise ValueError(
            f"decode {query_id}: expected 132 files (3 steps x 44 layers), "
            f"got {len(actual)}"
        )
    ranks: set[int] = set()
    ...
    return len(actual), next(iter(ranks)), manifest_sha256
...
def completed_outputs_present(
    ...
    return query_dump_counts(prefill_root, decode_root, query_id) == (44, 132)
...
def validate_complete_query_dump(
    ...
    """Strictly validate an exact 44/132 dump, including transport-error cases."""
    if query_dump_counts(prefill_root, decode_root, query_id) != (44, 132):
        return None
    ...
    decode_files, decode_rank, decode_sha256 = validate_decode(decode_root, query_id)
    decode_query_dir = decode_root / f"query_{query_id}"
    decode_steps = sorted(
        int(path.name.removeprefix("step_"))
        for path in decode_query_dir.glob("step_*")
        if path.is_dir()
    )
    if decode_steps != [0, 1, 2]:
        raise ValueError(
            f"decode {query_id}: expected steps [0, 1, 2], got {decode_steps}"
        )
...
def normalize_generation_limits(
    ...
    """Bound transport generation to exactly what three MTP dump steps need.

    The source request and compatibility-adapted payload remain immutable and
    independently hashed.  The live topology has ``gen_num_per_cycle=3``;
    observed output length seven produced only 89 files, while a floor/cap of
    eight produced all 3 x 44 real decode files.  Both OpenAI caps and the
    underlying GenerateConfig bounds are set so no captured large value wins.
    """
    ...
    normalized["max_tokens"] = COLLECTION_GENERATION_TOKENS
    normalized["max_completion_tokens"] = COLLECTION_GENERATION_TOKENS
    if "max_new_tokens" in normalized:
        normalized["max_new_tokens"] = COLLECTION_GENERATION_TOKENS
    extra_configs["max_new_tokens"] = COLLECTION_GENERATION_TOKENS
    extra_configs["min_new_tokens"] = COLLECTION_GENERATION_TOKENS
    ...
    audit = {
        ...
        "reason": "collect_exactly_3_decode_steps",
        "required_decode_steps": [0, 1, 2],
        "transport_generation_tokens": COLLECTION_GENERATION_TOKENS,
        "validation_basis": (
            "output length 7 observed 89 decode files; min/max 8 is the "
            "smallest observed bound producing 132 files for gen_num_per_cycle=3"
        ),
    }
    ...
    for index, case in enumerate(pending, 1):
        ...
        try:
            for attempt in range(1, args.max_attempts + 1):
                ...
                try:
                    ...
                    decode_files, decode_rank, decode_sha256 = validate_decode(
                        args.decode_dir, query_id
                    )
                    ...
                    if (prefill_count, decode_count) == (44, 132):
                        try:
                    ...
                    print(
                        ...
                        f"prefill={prefill_count}/44 decode={decode_count}/132 "
                        f"error={error_text}",
                        flush=True,
                    )
                    if can_retry:
        ...
        if completed_record is not None:
            ...
            print(
                ...
                f"prefill=44 decode=132 dp_rank={completed_record['decode_rank']} "
