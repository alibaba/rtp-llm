...
class ModelAssistantServer(object):
    ...
    def include_api_from_subpath(sub_path: str):
        ...
        py_files = glob.glob(os.path.join(current_path, sub_path, "*.py"), recursive=True)
        pyc_files = glob.glob(os.path.join(current_path, sub_path, "*.pyc"), recursive=True)
        all_files = py_files + pyc_files
        for filename in all_files:
            if os.path.isfile(filename):
                # convert path to module name
                module_name = "rtp_llm.tools." + filename.replace(current_path, "").replace('/', '.')[:-3].strip(".")
                # add module
