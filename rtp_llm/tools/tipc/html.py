...
class IPCTransportClient:
    ...
    def send(
        self, url: str, name: str, t: torch.Tensor, method: str = "shm"
    ) -> requests.Response:
        """
        ...
        if method == "cuda_ipc":
            ...
            self.transporting_tensor = t

            cu_m: CuIpcTensorMeta = CudaIpcHelper.build_tensor_meta(
                self.transporting_tensor
            )
            payload: dict = {
                "name": name,
                "time": time(),
            ...
            sh_m: SharedMemIpcMeta = self.cpu_ipc.build_tensor_meta(t, self.shm)
            payload: dict = {
                "name": name,
                "time": time(),
                "method": "shm",
            ...
            raise ValueError(
                f"unsupported ipc method: {method}. Choose 'cuda_ipc' or 'shm'."
            )
