...
template<typename TKernelMeta, typename TKernelParam>
void TFusedMultiHeadAttentionXMMAKernel<TKernelMeta, TKernelParam>::run(TKernelParam&  params,
...
TFusedMHAKernelList const*
TFusedMHAKernelFactory<TFusedMHAKernelList>::getXMMAKernels(const typename TFusedMHAKernelList::KernelMeta* pKernelList,
...
template<typename TFusedMHAKernelList>
uint64_t
TFusedMHAKernelFactory<TFusedMHAKernelList>::hashID(Data_type inputType, Data_type outputType, unsigned int sm) const {
...
FusedMultiHeadAttentionXMMAKernelV2::FusedMultiHeadAttentionXMMAKernelV2(
    FusedMultiHeadAttentionKernelMetaInfoV2 const* pMetaStart,
...
uint64_t FusedMultiHeadAttentionXMMAKernelV2::hashID(unsigned int s,
                                                     unsigned int d,
                                                     unsigned int dv,
                                                     bool         interleaved,
                                                     bool         unroll,
                                                     ...
                                                     bool         is_alibi_supported,
                                                     int          attention_mask_type,
                                                     int          input_layout,
                                                     bool         tiled,
                                                     bool         enable_attn_logit_softcapping,
                                                     unsigned int sage_block_size_q,
                                                     unsigned int sage_block_size_k,
                                                     unsigned int sage_block_size_v,
...
uint64_t FusedMultiHeadAttentionXMMAKernelV2::hashID(KernelMeta const& kernelMeta) const {
    return hashID(kernelMeta.mS,
                  ...
                  kernelMeta.mWarpSpecialization,
                  kernelMeta.mAlibiSupported,
                  kernelMeta.mAttentionMaskType,
                  kernelMeta.mAttentionInputLayout,
                  kernelMeta.mTiled,
                  kernelMeta.mEnableAttnLogitSoftcapping,
                  kernelMeta.mSageBlockSizeQ,
                  kernelMeta.mSageBlockSizeK,
                  kernelMeta.mSageBlockSizeV,
                  kernelMeta.mReturnSoftmaxStats);
}

void FusedMultiHeadAttentionXMMAKernelV2::run(Fused_multihead_attention_params_v2& params,
                                              Launch_params&                       launch_params,
                                              cudaStream_t                         stream) const {
    ...
    if (findIter == mFunctions.end()) {
        ...
        RTP_LLM_CHECK_WITH_INFO(false, "%s", ss.str().c_str());
    }
    ...
    if (!forceUnroll) {
        ...
        if (launch_params.dynamic_scheduler) {
            ...
            auto const size_in_bytes =
                2 * static_cast<int64_t>(get_size_in_bytes(mInputDataType)) * params.b * params.h * params.s * params.d;
...
bool FusedMultiHeadAttentionXMMAKernelV2::checkIfKernelExist(MHARunnerFixedParams params) const {
    uint64_t   id       = hashID(0,
                         ...
                         false,
                         false,
                         static_cast<int>(params.attentionMaskType),
                         static_cast<int>(params.attentionInputLayout),
                         false,
                         params.attnLogitSoftcappingScale != 0.f,
                         params.sageBlockSizeQ,
                         params.sageBlockSizeK,
    ...
    if (!found) {
        ...
        RTP_LLM_LOG_WARNING(ss.str().c_str());
    }
    return found;
}

void FusedMultiHeadAttentionXMMAKernelV2::getStepSize(uint32_t&                                  out_step_q,
                                                      uint32_t&                                  out_step_kv,
                                                      Fused_multihead_attention_params_v2 const& params,
...
bool FusedMultiHeadAttentionXMMAKernelV2::useForceUnroll(Fused_multihead_attention_params_v2 const& params,
                                                         Launch_params const& launch_params) const {
...
uint64_t FusedMultiHeadAttentionXMMAKernelV2::hashFromParams(Fused_multihead_attention_params_v2 const& params,
                                                             Launch_params const& launch_params) const {
    bool forceUnroll = useForceUnroll(params, launch_params);
    return hashID(launch_params.kernel_s,
                  params.d,
                  params.dv,
                  ...
                  static_cast<int>(launch_params.attention_input_layout),
                  launch_params.granular_tiling,
                  launch_params.enableAttnLogitSoftcapping,
                  launch_params.sage_block_size_q,
                  launch_params.sage_block_size_k,
                  launch_params.sage_block_size_v,
                  launch_params.supportReturnSoftmaxStats);
}

FusedMultiHeadAttentionXMMAKernelV2 const*
getXMMAKernelsV2(Data_type inputType, Data_type outputType, unsigned int sm) {
    if (sm == kSM_121) {
        sm = kSM_120;
    }
    return FusedMHAKernelFactoryV2::Get().getXMMAKernels(sMhaKernelMetaInfosV2,
                                                         sizeof(sMhaKernelMetaInfosV2)
                                                             / sizeof(sMhaKernelMetaInfosV2[0]),
                                                         inputType,
                                                         outputType,
                                                         sm);
}
