...
static constexpr bool USE_TMA_STORE = true;


using Attention_mask_type = ContextAttentionMaskType;

using Ktraits = fmha::ws::Kernel_traits_Hopper_qgmma_e4m3_fp32<
                64,
                256,
                80,
                0,
...
using Ktraits_causal = fmha::ws::Kernel_traits_Hopper_qgmma_e4m3_fp32<
                       64,
                       256,
                       80,
                       0,
...
using Ktraits_sliding_or_chunked_causal = fmha::ws::Kernel_traits_Hopper_qgmma_e4m3_fp32<
                                      64,
                                      256,
                                      80,
                                      0,
...
using Ktraits_custom_mask = fmha::ws::Kernel_traits_Hopper_qgmma_e4m3_fp32<
                            64,
                            256,
                            80,
                            0,
...
void fmha_v2_flash_attention_e4m3_64_256_S_qkv_80_sage_64_64_256_output_bf16_tma_ws_sm90_kernel(
    ...
    if( warp_group == NUM_COMPUTE_GROUPS ) {  // dma + sched
        ...
        fmha::ws::Compute<fmha::Hopper_qgmma_e4m3_fp32_traits, Ktraits> compute;
...
void fmha_v2_flash_attention_e4m3_64_256_S_qkv_80_causal_sage_64_64_256_output_bf16_tma_ws_sm90_kernel(
    const __grid_constant__ bert::Fused_multihead_attention_params_v2 params){
    ...
    if( warp_group == NUM_COMPUTE_GROUPS ) {  // dma + sched
        ...
        fmha::ws::Compute<fmha::Hopper_qgmma_e4m3_fp32_traits, Ktraits_causal> compute;
        compute.run(warp_group, tidx, shared, params);
    }
}
...
void fmha_v2_flash_attention_e4m3_64_256_S_qkv_80_sliding_or_chunked_causal_sage_64_64_256_output_bf16_tma_ws_sm90_kernel(
    const __grid_constant__ bert::Fused_multihead_attention_params_v2 params){
    ...
    if( warp_group == NUM_COMPUTE_GROUPS ) {  // dma + sched
        ...
        fmha::ws::Compute<fmha::Hopper_qgmma_e4m3_fp32_traits, Ktraits_sliding_or_chunked_causal> compute;
        compute.run(warp_group, tidx, shared, params);
    }
}
...
void fmha_v2_flash_attention_e4m3_64_256_S_qkv_80_custom_mask_sage_64_64_256_output_bf16_tma_ws_sm90_kernel(
    const __grid_constant__ bert::Fused_multihead_attention_params_v2 params){
    ...
    if( warp_group == NUM_COMPUTE_GROUPS ) {  // dma + sched
        ...
        fmha::ws::Compute<fmha::Hopper_qgmma_e4m3_fp32_traits, Ktraits_custom_mask> compute;
        compute.run(warp_group, tidx, shared, params);
    }
}
...
void run_fmha_v2_flash_attention_e4m3_64_256_S_qkv_80_sage_64_64_256_output_bf16_tma_ws_sm90(
    ...
    if( Ktraits::SCHEDULING_MODE == 0 ) {
        ...
        // 2 * 1 stands for kv cache and 1 bytes per element.
        size_t size_in_bytes = block_size.y * params.s * params.d * 2 * 1;
        ...
        if (launch_params.attention_mask_type == Attention_mask_type::CAUSAL) {
            // 2 * 1 stands for kv cache and 1 bytes per element.
            size_t size_in_bytes = params.b * params.h * params.s * params.d * 2 * 1;
...
#if 1 // padding_mask
        FMHA_CHECK_CUDA(cudaFuncSetAttribute(fmha_v2_flash_attention_e4m3_64_256_S_qkv_80_sage_64_64_256_output_bf16_tma_ws_sm90_kernel,
...
#if 0 // causal_mask
        FMHA_CHECK_CUDA(cudaFuncSetAttribute(fmha_v2_flash_attention_e4m3_64_256_S_qkv_80_causal_sage_64_64_256_output_bf16_tma_ws_sm90_kernel,
                                         cudaFuncAttributeMaxDynamicSharedMemorySize,
                                         SMEM_BYTES));

        fmha_v2_flash_attention_e4m3_64_256_S_qkv_80_causal_sage_64_64_256_output_bf16_tma_ws_sm90_kernel
            <<<block_size, Ktraits::THREADS, SMEM_BYTES, stream>>>(convertKernelParmas2BertParams(params));
#endif // causal mask
    } else if( launch_params.attention_mask_type == Attention_mask_type::SLIDING_OR_CHUNKED_CAUSAL ) {
#if 0 // sliding_or_chunked_causal_mask
        FMHA_CHECK_CUDA(cudaFuncSetAttribute(fmha_v2_flash_attention_e4m3_64_256_S_qkv_80_sliding_or_chunked_causal_sage_64_64_256_output_bf16_tma_ws_sm90_kernel,
                                         cudaFuncAttributeMaxDynamicSharedMemorySize,
                                         SMEM_BYTES));

        fmha_v2_flash_attention_e4m3_64_256_S_qkv_80_sliding_or_chunked_causal_sage_64_64_256_output_bf16_tma_ws_sm90_kernel
            <<<block_size, Ktraits::THREADS, SMEM_BYTES, stream>>>(convertKernelParmas2BertParams(params));
#endif // sliding_or_chunked_causal_mask
    } else if( launch_params.attention_mask_type == Attention_mask_type::CUSTOM_MASK ) {
#if 0 // custom_mask
        FMHA_CHECK_CUDA(cudaFuncSetAttribute(fmha_v2_flash_attention_e4m3_64_256_S_qkv_80_custom_mask_sage_64_64_256_output_bf16_tma_ws_sm90_kernel,
                                         cudaFuncAttributeMaxDynamicSharedMemorySize,
                                         SMEM_BYTES));

        fmha_v2_flash_attention_e4m3_64_256_S_qkv_80_custom_mask_sage_64_64_256_output_bf16_tma_ws_sm90_kernel
            <<<block_size, Ktraits::THREADS, SMEM_BYTES, stream>>>(convertKernelParmas2BertParams(params));
#endif // custom mask
    }
