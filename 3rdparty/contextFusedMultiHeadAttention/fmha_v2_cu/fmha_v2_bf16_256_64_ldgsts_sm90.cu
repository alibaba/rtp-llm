...
using Traits_p = fmha::Hopper_hgmma_bf16_traits<64, 256, 16, false, false>;
using Traits_o = fmha::Hopper_hgmma_bf16_traits<64, 64, 16, true, false>;
...
extern "C"
__global__
void fmha_v2_bf16_256_64_ldgsts_sm90_kernel(const __grid_constant__ bert::Fused_multihead_attention_params_v2 params){
...
extern "C"
__global__
void fmha_v2_bf16_256_64_causal_ldgsts_sm90_kernel(const __grid_constant__ bert::Fused_multihead_attention_params_v2 params){
...
extern "C"
__global__
void fmha_v2_bf16_256_64_sliding_or_chunked_causal_ldgsts_sm90_kernel(const __grid_constant__ bert::Fused_multihead_attention_params_v2 params){
...
void run_fmha_v2_bf16_256_64_ldgsts_sm90(Fused_multihead_attention_params_v2 &params,
  ...
  if( launch_params.attention_mask_type == Attention_mask_type::CAUSAL ) {
    ...
    if( smem_size >= 48*1024 ) {
       FMHA_CHECK_CUDA(cudaFuncSetAttribute(fmha_v2_bf16_256_64_causal_ldgsts_sm90_kernel,
                                        cudaFuncAttributeMaxDynamicSharedMemorySize,
                                        smem_size));
    }
    fmha_v2_bf16_256_64_causal_ldgsts_sm90_kernel<<<grid, Kernel_traits::THREADS, Kernel_traits::BYTES_PER_SMEM, stream>>>(convertKernelParmas2BertParams(params));
#endif // causal mask
  } else if( launch_params.attention_mask_type == Attention_mask_type::SLIDING_OR_CHUNKED_CAUSAL ) {
#if 1 // sliding_or_chunked_causal_mask
    if( smem_size >= 48*1024 ) {
       FMHA_CHECK_CUDA(cudaFuncSetAttribute(fmha_v2_bf16_256_64_sliding_or_chunked_causal_ldgsts_sm90_kernel,
                                        cudaFuncAttributeMaxDynamicSharedMemorySize,
                                        smem_size));
    }
    fmha_v2_bf16_256_64_sliding_or_chunked_causal_ldgsts_sm90_kernel<<<grid, Kernel_traits::THREADS, Kernel_traits::BYTES_PER_SMEM, stream>>>(convertKernelParmas2BertParams(params));
#endif // sliding_or_chunked_causal_mask
  } else {
#if 1 // padding_mask
    constexpr int smem_size = Kernel_traits::BYTES_PER_SMEM;
    if( smem_size >= 48*1024 ) {
        FMHA_CHECK_CUDA(cudaFuncSetAttribute(fmha_v2_bf16_256_64_ldgsts_sm90_kernel,
                                         cudaFuncAttributeMaxDynamicSharedMemorySize,
                                         smem_size));
    }
    fmha_v2_bf16_256_64_ldgsts_sm90_kernel<<<grid, Kernel_traits::THREADS, Kernel_traits::BYTES_PER_SMEM, stream>>>(convertKernelParmas2BertParams(params));
#endif // padding_mask
  }
}
...
extern "C"
__global__
void fmha_v2_bf16_256_64_ldgsts_sm90_kernel_nl(bert::Fused_multihead_attention_params_v2 params){
...
extern "C"
__global__
void fmha_v2_bf16_256_64_causal_ldgsts_sm90_kernel_nl(bert::Fused_multihead_attention_params_v2 params){
...
extern "C"
__global__
void fmha_v2_bf16_256_64_sliding_or_chunked_causal_ldgsts_sm90_kernel_nl(bert::Fused_multihead_attention_params_v2 params){
  fused_multihead_attention::device_4x1_hopper_nl<Kernel_traits_sliding_or_chunked_causal_nl>(params);
}

#endif // sliding_or_chunked_causal_mask

void run_fmha_v2_bf16_256_64_ldgsts_sm90_nl(Fused_multihead_attention_params_v2 &params,
    const Launch_params& launch_params, cudaStream_t stream){
...
#if 1 // causal_mask
    if( smem_size >= 48*1024 ) {
        FMHA_CHECK_CUDA(cudaFuncSetAttribute(fmha_v2_bf16_256_64_causal_ldgsts_sm90_kernel_nl,
                                         cudaFuncAttributeMaxDynamicSharedMemorySize,
                                         smem_size));
    }
    fmha_v2_bf16_256_64_causal_ldgsts_sm90_kernel_nl<<<grid, Kernel_traits_nl::THREADS, Kernel_traits_nl::BYTES_PER_SMEM, stream>>>(convertKernelParmas2BertParams(params));
#endif // causal mask
  } else if( launch_params.attention_mask_type == Attention_mask_type::SLIDING_OR_CHUNKED_CAUSAL ) {
#if 1 // sliding_or_chunked_causal_mask
    if( smem_size >= 48*1024 ) {
        FMHA_CHECK_CUDA(cudaFuncSetAttribute(fmha_v2_bf16_256_64_sliding_or_chunked_causal_ldgsts_sm90_kernel_nl,
                                         cudaFuncAttributeMaxDynamicSharedMemorySize,
                                         smem_size));
    }
    fmha_v2_bf16_256_64_sliding_or_chunked_causal_ldgsts_sm90_kernel_nl<<<grid, Kernel_traits_nl::THREADS, Kernel_traits_nl::BYTES_PER_SMEM, stream>>>(convertKernelParmas2BertParams(params));
#endif // sliding_or_chunked_causal_mask
  } else {
#if 1 // padding_mask
    if( smem_size >= 48*1024 ) {
        FMHA_CHECK_CUDA(cudaFuncSetAttribute(fmha_v2_bf16_256_64_ldgsts_sm90_kernel_nl,
                                         cudaFuncAttributeMaxDynamicSharedMemorySize,
                                         smem_size));
    }
    fmha_v2_bf16_256_64_ldgsts_sm90_kernel_nl<<<grid, Kernel_traits_nl::THREADS, Kernel_traits_nl::BYTES_PER_SMEM, stream>>>(convertKernelParmas2BertParams(params));
#endif // padding_mask
  }
}
...
void run_fmha_v2_bf16_256_64_ldgsts_sm90(const bert::Fused_multihead_attention_params_v2 &params, cudaStream_t stream){
    assert(false && "Unsupported CUDA version");
}

#if 1

void run_fmha_v2_bf16_256_64_ldgsts_sm90_nl(const bert::Fused_multihead_attention_params_v2 &params, cudaStream_t stream){
    assert(false && "Unsupported CUDA version");
}
