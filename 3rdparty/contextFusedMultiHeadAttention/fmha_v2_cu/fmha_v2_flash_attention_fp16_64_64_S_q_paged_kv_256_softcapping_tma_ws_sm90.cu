...
using Ktraits_sliding_or_chunked_causal = fmha::ws::Kernel_traits<fmha::Hopper_hgmma_fp16_traits,
                                      64,
                                      64,
                                      256,
                                      0,
...
using Ktraits_custom_mask = fmha::ws::Kernel_traits<fmha::Hopper_hgmma_fp16_traits,
                            64,
                            64,
                            256,
                            0,
...
void fmha_v2_flash_attention_fp16_64_64_S_q_paged_kv_256_softcapping_tma_ws_sm90_kernel(
...
void fmha_v2_flash_attention_fp16_64_64_S_q_paged_kv_256_causal_softcapping_tma_ws_sm90_kernel(
    const __grid_constant__ bert::Fused_multihead_attention_params_v2 params){
...
void fmha_v2_flash_attention_fp16_64_64_S_q_paged_kv_256_sliding_or_chunked_causal_softcapping_tma_ws_sm90_kernel(
    const __grid_constant__ bert::Fused_multihead_attention_params_v2 params){
    ...
    if( warp_group == NUM_COMPUTE_GROUPS ) {  // dma + sched
        ...
        fmha::ws::Compute<fmha::Hopper_hgmma_fp16_traits, Ktraits_sliding_or_chunked_causal> compute;
        compute.run(warp_group, tidx, shared, params);
    }
}
...
void fmha_v2_flash_attention_fp16_64_64_S_q_paged_kv_256_custom_mask_softcapping_tma_ws_sm90_kernel(
    const __grid_constant__ bert::Fused_multihead_attention_params_v2 params){
    ...
    if( warp_group == NUM_COMPUTE_GROUPS ) {  // dma + sched
        ...
        fmha::ws::Compute<fmha::Hopper_hgmma_fp16_traits, Ktraits_custom_mask> compute;
        compute.run(warp_group, tidx, shared, params);
    }
}
...
void run_fmha_v2_flash_attention_fp16_64_64_S_q_paged_kv_256_softcapping_tma_ws_sm90(
...
#if 0 // padding_mask
        FMHA_CHECK_CUDA(cudaFuncSetAttribute(fmha_v2_flash_attention_fp16_64_64_S_q_paged_kv_256_softcapping_tma_ws_sm90_kernel,
                                         cudaFuncAttributeMaxDynamicSharedMemorySize,
                                         SMEM_BYTES));

        fmha_v2_flash_attention_fp16_64_64_S_q_paged_kv_256_softcapping_tma_ws_sm90_kernel
...
#if 1 // causal_mask
        FMHA_CHECK_CUDA(cudaFuncSetAttribute(fmha_v2_flash_attention_fp16_64_64_S_q_paged_kv_256_causal_softcapping_tma_ws_sm90_kernel,
                                         cudaFuncAttributeMaxDynamicSharedMemorySize,
                                         SMEM_BYTES));

        fmha_v2_flash_attention_fp16_64_64_S_q_paged_kv_256_causal_softcapping_tma_ws_sm90_kernel
            <<<block_size, Ktraits::THREADS, SMEM_BYTES, stream>>>(convertKernelParmas2BertParams(params));
#endif // causal mask
    } else if( launch_params.attention_mask_type == Attention_mask_type::SLIDING_OR_CHUNKED_CAUSAL ) {
#if 1 // sliding_or_chunked_causal_mask
        FMHA_CHECK_CUDA(cudaFuncSetAttribute(fmha_v2_flash_attention_fp16_64_64_S_q_paged_kv_256_sliding_or_chunked_causal_softcapping_tma_ws_sm90_kernel,
                                         cudaFuncAttributeMaxDynamicSharedMemorySize,
                                         SMEM_BYTES));

        fmha_v2_flash_attention_fp16_64_64_S_q_paged_kv_256_sliding_or_chunked_causal_softcapping_tma_ws_sm90_kernel
            <<<block_size, Ktraits::THREADS, SMEM_BYTES, stream>>>(convertKernelParmas2BertParams(params));
#endif // sliding_or_chunked_causal_mask
    } else if( launch_params.attention_mask_type == Attention_mask_type::CUSTOM_MASK ) {
#if 0 // custom_mask
        FMHA_CHECK_CUDA(cudaFuncSetAttribute(fmha_v2_flash_attention_fp16_64_64_S_q_paged_kv_256_custom_mask_softcapping_tma_ws_sm90_kernel,
                                         cudaFuncAttributeMaxDynamicSharedMemorySize,
                                         SMEM_BYTES));

        fmha_v2_flash_attention_fp16_64_64_S_q_paged_kv_256_custom_mask_softcapping_tma_ws_sm90_kernel
            <<<block_size, Ktraits::THREADS, SMEM_BYTES, stream>>>(convertKernelParmas2BertParams(params));
#endif // custom mask
    }
