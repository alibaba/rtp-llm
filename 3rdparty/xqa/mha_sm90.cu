...
struct alignas(128) SharedMem {
    ...
    using VBuffer = Vec<Array2D<LdGrain,
                                gemm1CtaTileNbTokens,
                                exactDiv(cacheHeadPartBytes, grainBytes),
...
struct F16QToF8Converter {
    ...
    static __device__ RegData load(uint32_t                     tid,
                                   TinyPtr<IOHead const> const& src,
...
struct KVTilePartLoader {
    ...
    __device__ KVTilePartLoader(bool                                isK,
                                uint32_t                            nbKHeads,
                                KVCacheList<usePagedKVCache> const& cacheList,
                                uint32_t                            idxReq,
                                uint32_t                            idxHeadGrp,
    ...
    __device__ void
    loadData(Array2D<LdGrain, nbTokens, exactDiv(cacheHeadPartBytes, grainBytes), alignedForSwizzle>& dst,
             uint32_t                                                                                 idxTile,
             uint32_t                                                                                 idxPart,
             CtaBarrier&                                                                              bar);
...
struct SpecDec {
    ...
    __device__ inline SpecDec(SpecDecParams const& params, uint32_t idxReq, uint32_t ctaIdxY, uint32_t seqLen):
        params(params), ctaIdxY(ctaIdxY), seqLen(seqLen) {
...
__device__ void
warpGrpApplyMask(Gemm0Acc& acc, SpecDec const& specDec, uint32_t cacheSeqLen, uint32_t idxTile, uint32_t warpRank);
#endif
...
__device__         Vec<RegMatAFrag, gemm1NbGmmaInstM>
loadVTileTransposed(uint32_t warpRank, uint32_t lane, SharedMem::VBuffer const& smemV, uint32_t idxGmmaInstK);
using Gemm1Acc = GmmaAcc<headElems, ctaNbQHeads>;
__device__ void rescaleGemm1AccForNewColMax_sync(uint32_t           warpRank,
                                                 ShmQWiseVec const& shmXColMax,
                                                 ShmQWiseVec const (&shmXColSum)[gemm0NbWarps],
                                                 ShmQWiseVec& shmAccColMax,
                                                 Gemm1Acc&    acc,
...
#else
__device__ void
transposeVTile(uint32_t warpRank, uint32_t lane, SharedMem::VTBuffer& dst, SharedMem::VBuffer const& src);
using Gemm1Acc = GmmaAcc<ctaNbQHeads, headElems>;
__device__ void rescaleGemm1AccForNewRowMax_sync(uint32_t           warpRank,
                                                 ShmQWiseVec const& shmXRowMax,
                                                 ShmQWiseVec const(&shmXRowSum),
                                                 ShmQWiseVec& shmAccRowMax,
                                                 Gemm1Acc&    acc,
                                                 ShmQWiseVec& shmAccRowSum);
template<typename DstHead>
__device__ void
finalizeAndWriteOut_sync(uint32_t                  warpRank,
                         DstHead*                  dst,
                         SharedMem::OutSwizzleBuf& swizzleBuf,
                         Gemm1Acc&                 acc,
                         float                     xvoScale,
...
__device__ Vec<Vec<DstElem, 2>, ropeNbPairsPerThrd<nbThrds>> loadHead(Vec<SrcElem, validElemsPerHead> const& head,
                                                                      uint32_t                               tid);
...
__device__ void
storeRotatedPairsForKV(GMemCacheHead&                                                                 dst,
                       mha::conditional_t<forNeox,
                                          Vec<Vec<CacheElem, ropeNbPairsPerThrd<nbThrds>>, 2>,
                                          Vec<Vec<CacheElem, 2>, ropeNbPairsPerThrd<nbThrds>>> const& src,
                       uint32_t                                                                       tid);
template<bool forNeox, uint32_t nbThrds>
__device__ void
storeRotatedPairsForQ(SharedMem::QBuffer&                                                            dst,
                      mha::conditional_t<forNeox,
                                         Vec<Vec<CacheElem, ropeNbPairsPerThrd<nbThrds>>, 2>,
                                         Vec<Vec<CacheElem, 2>, ropeNbPairsPerThrd<nbThrds>>> const& src,
                      uint32_t                                                                       row,
...
__device__ void
printAcc(CtaBarrier& warpGrpBar, uint32_t warpRank, Array2D<GmmaAccCoreMat, nbGmmaInstM, nbGmmaInstNBase> const& acc) {
...
__launch_bounds__(128 * 3, 1)
    ...
    if (warpIdx.z == 0) {
        ...
        auto const matDescQBase = gmma::makeMatDesc(nullptr,
                                                    0,
                                                    SharedMem::QBuffer::Elem::rowBytes * 8,
...
#pragma unroll
            for (uint32_t idxPart = 0; idxPart < cacheHeadNbParts; idxPart++) {
                ...
                auto const matDescKBase = gmma::makeMatDesc(nullptr,
                                                            0,
                                                            SharedMem::KBuffer::rowBytes * 8,
                                                            &smem.k[0],
                                                            gmma::getSwizzleMode<true>(SharedMem::KBuffer{}))
                                              .raw();
                assert(matDescKBase
                       == gmma::makeMatDesc(nullptr,
                                            0,
                                            SharedMem::KBuffer::rowBytes * 8,
                                            gmma::getSwizzleMode<true>(SharedMem::KBuffer{}))
                              .raw());
...
#pragma unroll
        ...
        for (uint32_t idxIter = 0; idxIter < nbIters; idxIter++) {
            ...
            rescaleGemm1AccForNewColMax_sync(warpRank,
                                             smem.xColMax[idxXBuf],
                                             smem.xColSum[idxXBuf],
                                             smem.gemm1AccColMax,
                                             acc,
                                             smem.gemm1AccColSum,
            ...
            auto const descXBase = gmma::makeMatDesc(nullptr,
                                                     0,
                                                     SharedMem::XBuffer::Elem::rowBytes * 8,
                                                     gmma::getSwizzleMode<true>(SharedMem::XBuffer::Elem{}))
                                       .raw();
#if CACHE_ELEM_ENUM == 0
            auto const descVBase = gmma::makeMatDesc(nullptr,
                                                     0,
                                                     SharedMem::VBuffer::Elem::rowBytes * 8,
...
#if SWAP_AB
                    finalizeAndWriteOut_sync(threadIdx.x,
                                             warpRank,
                                             dst,
                                             smem.outSwizzleBuf(idxXBuf),
                                             acc,
                                             xvoScale,
                                             smem.gemm1WarpGrpBar,
                                             smem.gemm1AccColSum);
#else
                    finalizeAndWriteOut_sync(warpRank,
                                             dst,
                                             smem.outSwizzleBuf(idxXBuf),
                                             acc,
                                             xvoScale,
...
#pragma unroll
                for (uint32_t iter = 0; iter < divUp(ctaNbValidQHeads, warp_size); iter++) {
                    ...
                    ldgsts::copyAsync<sizeof(smem.rowSumMax[idxBuf][i])>(&smem.rowSumMax[idxBuf][i],
                                                                         &scratchMem.rowSumMax()[idxChunk][i]);
                }
...
#pragma unroll
                    ...
                    if (warpElectSync()) {
                        tma::load1DAsync(&smem.tokens[idxBuf],
                                         &scratchMem.tokens()[idxChunk],
                                         sizeof(smem.tokens[idxBuf]),
                                         bar.produced);
...
F16QToF8Converter<nbThrds, beamWidth>::load(uint32_t                     tid,
                                            TinyPtr<IOHead const> const& src,
...
__device__ inline KVTilePartLoader::KVTilePartLoader(bool                                isK,
                                                     uint32_t                            nbKHeads,
                                                     KVCacheList<usePagedKVCache> const& cacheList,
                                                     uint32_t                            idxReq,
                                                     uint32_t                            idxHeadGrp,
...
KVTilePartLoader::loadData(Array2D<LdGrain, nbTokens, exactDiv(cacheHeadPartBytes, grainBytes), alignedForSwizzle>& dst,
                           uint32_t    idxTile,
                           uint32_t    idxPart,
                           CtaBarrier& bar) {
...
#pragma unroll
        for (uint32_t i = 0; i < nbPagesPerTile; i++) {
            tma::loadAsync(&dst(tokensPerPage * i, 0),
                           tensorMap,
                           DimsLE<4>{partElems * idxPart, 0, idxHeadGrp, (uint32_t)pages[i]},
                           bar);
        }
    }
#else
...
__device__ inline RegColWiseVec
computeWarpGrpColMax_sync(CtaBarrier& warpGrpBar, ShmQWiseVec& smemColMax, Gemm0Acc const& src) {
...
__device__ inline RegRowWiseVec
warpRowWiseReduce(RegRowWiseVec const& init, Gemm0Acc const& src, float (*op)(float, float)) {
...
__device__ inline RegRowWiseVec
computeWarpGrpRowMax_sync(uint32_t warpRank, ShmQWiseVec& smemRowMax, Gemm0Acc const& src) {
    assert(warpRank < 4);
...
#if SWAP_AB
__device__ inline Vec<RegMatAFrag, gemm1NbGmmaInstM>
loadVTileTransposed(uint32_t warpRank, uint32_t lane, SharedMem::VBuffer const& smemV, uint32_t idxGmmaInstK) {
...
__device__ inline void storeShmColWiseVecNoDup(ShmQWiseVec&                                           shmVec,
                                               Vec<float, divUp(ShmQWiseVec::size, warp_size)> const& src) {
#pragma unroll
    for (uint32_t i = 0; i < divUp(ShmQWiseVec::size, warp_size); i++) {
        uint32_t const idx     = i * warp_size + laneId();
        bool const     inBound = ((ShmQWiseVec::size % warp_size == 0) || (idx < ShmQWiseVec::size));
...
#if SWAP_AB
__device__ inline void rescaleGemm1AccForNewColMax_sync(uint32_t           warpRank,
                                                        ShmQWiseVec const& shmXColMax,
                                                        ShmQWiseVec const (&shmXColSum)[gemm0NbWarps],
                                                        ShmQWiseVec& shmAccColMax,
                                                        Gemm1Acc&    acc,
...
template<typename DstHead>
__device__ inline void saveTransposedOutput(uint32_t                  threadRank,
                                            uint32_t                  warpRank,
                                            DstHead*                  dst,
                                            SharedMem::OutSwizzleBuf& swizzleBuf,
                                            Gemm1Acc const&           acc,
        ...
        for (uint32_t n = 0; n < Gemm1Acc::cols; n++) {
            ...
            auto const dst_smem =
                idxMat < 2 ?
                    ...
                    nullptr;
            stmatrix<true, 2>(dst_smem, f16Core);
#elif CACHE_ELEM_ENUM == 2
            ...
            swizzleBuf.template at<true>(gmma::instNBase * n + idxQuad,
                                         (gmma::instM * m + exactDiv(gmma::instM, gmmaWarpsPerGrp) * warpRank)
                                             / 16)[idxInQuad] = data;
#endif
        }
    }
...
template<typename DstHead>
__device__ inline void finalizeAndWriteOut_sync(uint32_t                  threadRank,
                                                uint32_t                  warpRank,
                                                DstHead*                  dst,
                                                SharedMem::OutSwizzleBuf& swizzleBuf,
                                                Gemm1Acc&                 acc,
...
#else
template<typename DstHead>
__device__ inline void finalizeAndWriteOut_sync(uint32_t                  warpRank,
                                                DstHead*                  dst,
                                                SharedMem::OutSwizzleBuf& swizzleBuf,
                                                Gemm1Acc&                 acc,
                                                float                     xvoScale,
                                                ShmQWiseVec const&        accRowSum,
                                                uint32_t nbKHeads /* for spec dec. set to 1 for workspace*/,
                                                uint32_t ctaNbValidTokens) {
...
__device__ inline Vec<Vec<DstElem, 2>, ropeNbPairsPerThrd<nbThrds>>
loadHead(Vec<SrcElem, validElemsPerHead> const& head, uint32_t tid) {
...
template<bool forNeox, uint32_t nbPairsPerThrd>
__device__ inline mha::
    conditional_t<forNeox, Vec<Vec<CacheElem, nbPairsPerThrd>, 2>, Vec<Vec<CacheElem, 2>, nbPairsPerThrd>>
    applyRoPE(Vec<Vec<float, 2>, nbPairsPerThrd> const& data, Vec<Vec<float, 2>, nbPairsPerThrd> const& ropeCosSin) {
...
static CUtensorMap makeTensorMapForContiguousKVCache(void const*              addr,
                                                     CUtensorMapDataType_enum dataType,
                                                     uint32_t                 headElems,
                                                     uint32_t                 nbKHeads,
                                                     uint32_t                 maxCacheLen,
                                                     uint32_t                 beamWidth,
                                                     uint32_t                 batchSize,
    ...
    checkCu(cuTensorMapEncodeTiled(&tensorMap,
                                   dataType,
                                   4,
...
static CUtensorMap makeTensorMapForPagedKVCache(void const*              addr,
                                                CUtensorMapDataType_enum dataType,
                                                uint32_t                 headElems,
                                                uint32_t                 nbKHeads,
...
void XQA_FUNC_SM90(cudaDeviceProp const& prop,
                   uint32_t              nbKHeads,
#if SLIDING_WINDOW
    ...
    cudaError_t const err = cudaLaunchKernelEx(&launchCfg,
                                               &XQA_KERNEL_SM90,
                                               nbKHeads,
#if SLIDING_WINDOW
                                               slidingWinSize,
    ...
    auto const tensorMap = makeTensorMapForContiguousKVCache(kvCacheData,
                                                             ...
                                                             beamWidth,
                                                             batchSize,
                                                             gemm0CtaTileNbTokens);
    cudaLaunchKernelEx(&launchCfg,
                       XQA_KERNEL_SM90,
                       nbKHeads,
#if SLIDING_WINDOW
                       slidingWinSize,
