...
#define XQA_SM90(func, a0, a1, b0, b1, c0, c1) \
    func##a0##a1##b0##b1##c0##c1(prop, nbKHeads, qScale, reinterpret_cast<Vec<__nv_bfloat16, c_head_dim>*>(output), reinterpret_cast<Vec<__nv_bfloat16, c_head_dim> const*>(qkv), reinterpret_cast<Vec<float, c_head_dim>*>(ropeCosSin), reinterpret_cast<Vec<__nv_fp8_e4m3, c_head_dim>*>(pool), kvCachePageList, maxSeqLen, seqLen, batchSize, kvCacheScale, semaphores, scratch, stream)
