...
load("@//3rdparty/flashinfer:def_cu13.bzl", "sub_lib")
...
genrule(
    ...
    cmd = "loc=$(locations @flashinfer_cpp_cu13//:aot_build_utils_generate_sm90); loc=$${loc%/*};loc=$${loc%/*}; PYTHONPATH=$$loc /opt/conda310/bin/python -m aot_build_utils.generate_sm90 --enable_f16 true --use_fp16_qk_reductions false --enable_bf16 true --mask_modes 1 --path $(RULEDIR) --head_dims 64,64 128,128 --pos_encoding_modes 0",
    outs = [
...
# Reuse the same shared-library targets for linking and wheel packaging.
_flashinfer_shared_libs = [
...
filegroup(
    name = "flashinfer_shared_libs",
    srcs = _flashinfer_shared_libs,
    visibility = ["//visibility:public"],
)
