...
// NaN safety: NaN == x is always false, so if selected_val is NaN this returns
// idx_b unconditionally. Callers must ensure inputs are NaN-free (beam search
// logits go through log_softmax, which satisfies this).
...
#ifdef ENABLE_BF16
    constexpr bool use_numeric_bound = std::is_same_v<T, __half> || std::is_same_v<T, amd_bfloat16>;
#else
    constexpr bool use_numeric_bound = std::is_same_v<T, __half>;
#endif
    if constexpr (use_numeric_bound) {
        return x ? numeric::get_lower_bound<T>() : numeric::get_upper_bound<T>();
