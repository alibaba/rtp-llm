...
#define CASE_K(PAD_K)                                                                                                  \
    topKSoftMaxKernelLauncher<T, PAD_K>(logits, workspace, bh, stream);                                          \
    break;
...
template void invokeTopkSoftMax<float>(
    float const* logits, void* workspace, BeamHypotheses& bh, cudaStream_t stream);

template void invokeTopkSoftMax<half>(
    half const* logits, void* workspace, BeamHypotheses& bh, cudaStream_t stream);
