    ...
    for (int masking_step = 0; masking_step < n_masking_steps; ++masking_step, --n_block) {
        ...
        if (Is_alibi) {
            flash::apply_alibi(
                ...
                (Element *)params.linear_bias_slopes
            );
        }
        if (!Is_causal) {
...
template<typename Kernel_traits, bool Is_causal, bool Is_alibi, bool Is_even_MN, bool Is_even_K, bool Split, bool Append_KV, typename Params>
inline __device__ void compute_attn_1rowblock_splitkv(const Params &params, const int bidb, const int bidh, const int m_block, const int n_split_idx, const int num_n_splits) {
    ...
    constexpr int n_masking_steps = !Is_causal
        ...
        : (Is_even_MN ? cute::ceil_div(kBlockM, kBlockN) : cute::ceil_div(kBlockM, kBlockN) + 1);
    #pragma unroll
    // printf("n_masking_steps = %d, n_block=%d\n", n_masking_steps, n_block);
    for (int masking_step = 0; masking_step < n_masking_steps; ++masking_step, --n_block) {
        Tensor acc_s = partition_fragment_C(tiled_mma, Shape<Int<kBlockM>, Int<kBlockN>>{});  // (MMA=4, MMA_M, MMA_N)
        clear(acc_s);
        flash::cp_async_wait<0>();
...
inline __device__ void compute_attn_splitkv(const Params &params) {
    ...
    flash::compute_attn_1rowblock_splitkv<Kernel_traits, Is_causal, Is_alibi, Is_even_MN, Is_even_K, Split, Append_KV>(params, bidb, bidh, m_block, n_split_idx, num_n_splits);
}
