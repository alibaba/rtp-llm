...
4. Update `_CUTE_PY` in `BUILD` so every upstream Python file has a declared
   Bazel output. Keep documentation, license, and packaging files in the
   snapshot even though only Python files enter the runtime target.
...
8. Verify that generated sources contain no remaining `flash_attn.cute`
   ...
   `vllm_flash_attention/cute/*.py` without the repository path prefix.
