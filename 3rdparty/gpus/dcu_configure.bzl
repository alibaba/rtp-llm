...
_DEFAULT_ROCM_TOOLKIT_PATH = "/opt/dtk"
...
def _rocm_include_path(repository_ctx, rocm_config):
    ...
    for prefix in clang_include_prefixes:
        ...
        inc_dirs.append(prefix + "/llvm/lib/clang/17.0.0/include")
        inc_dirs.append(prefix + "/llvm/lib/clang/18/include")
...
def enable_rocm(repository_ctx):
    if "TF_NEED_DTK" in repository_ctx.os.environ:
        enable_rocm = repository_ctx.os.environ["TF_NEED_DTK"].strip()
