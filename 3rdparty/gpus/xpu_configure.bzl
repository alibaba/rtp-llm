...
def _xpu_configure_impl(repository_ctx):
    ...
    # Validate Python version: XPU builds use Python 3.10, matching the
    # CUDA/ROCm build infra.  In the XPU container /opt/conda310/bin/python3
    # is a real Python 3.10 conda interpreter; fail early if it is something
    # else.
    ...
    if _py_ver.return_code == 0:
        ...
        if _ver != "3.10":
            auto_configure_fail(
                "XPU build requires Python 3.10 but PYTHON_BIN_PATH (%s) " % python_bin +
                "resolves to Python %s. " % _ver +
                "In the XPU Docker image, /opt/conda310/bin/python3 should be a " +
                "Python 3.10 conda interpreter. Check your container setup.")
    else:
        auto_configure_fail(
...
# Python headers/lib resolved from the XPU python (Python 3.10),
