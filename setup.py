    ...
    try:
        ...
        if major < 64:
            errors.append(
                ...
                "Fix: uv pip install 'setuptools>=64.0,<82'"
            )
        elif major >= 82:
            errors.append(
        ...
        errors.append(
            "setuptools not found. Fix: uv pip install 'setuptools>=64.0,<82'"
        )
...
def ensure_proto_files_generated(project_root: Path = None) -> None:
    ...
    project_root = project_root or get_project_root()
    if has_generated_proto_files(project_root) and not generated_proto_files_stale(project_root):
        print("Using existing generated protobuf Python files.")
        return
    if generated_proto_files_stale(project_root):
        print("Regenerating protobuf Python files (protobuf runtime vs generated code mismatch).")
        for rel in PROTO_OUTPUTS:
            p = project_root / rel
            if p.exists():
                p.unlink()
...
def get_pip_extras() -> str:
    ...
    Used by pip_install.sh to run: uv pip install -e ".[<extras>,dev]"
    """
...
def _resolve_reapi_endpoint(vipserver_host: str, daily_host: str, port: int) -> str:
    ...
    try:
        results = socket.getaddrinfo(vipserver_host, port, socket.AF_INET, socket.SOCK_STREAM)
        if results:
            ip = results[0][4][0]
...
def _get_remote_bazel_args() -> list:
    ...
    if not cfg:
        print("WARNING: RTP_REMOTE=1 but [tool.rtp-llm.remote] missing in pyproject.toml")
        return []
    ...
    args = [
        "--config=cicd",
        "--config=online_aone_bazel_cache", f"--remote_cache={cas_ep}",
        "--config=online_aone_bazel_remote", f"--remote_executor={exec_ep}",
        f"--jobs={jobs}",
    ]
...
def _get_local_jobs_args() -> list:
    ...
    if is_remote_enabled():
        return []
    cpu = int(os.environ.get("AONE_CI_REQUESTED_CPU",
                             os.environ.get("AJDK_MAX_PROCESSORS_LIMIT", "0")))
    if cpu > 0:
...
def copy_core_so_files(bazel_bin: Path, target_dir: Path) -> None:
    ...
    # Copy _solib_local dependencies (e.g. kv_cache_manager_client.so)
    # Skip torch/system .so files — they are already available via pip install
    _SOLIB_SKIP_PREFIXES = (
        "libtorch", "libc10", "libpython", "libcuda.so", "libnvidia-ml",
        "libgomp", "libshm", "libnccl", "libcudnn", "libnvrtc",
        "libcublas", "libcufft", "libcusparse", "libcusolver",
    )
    solib_dir = bazel_bin / "_solib_local"
    if solib_dir.exists():
        for so_file in solib_dir.rglob("*.so"):
            if any(so_file.name.startswith(prefix) for prefix in _SOLIB_SKIP_PREFIXES):
                continue
...
def copy_extensions(project_root: Path, build_config: str) -> None:
    ...
    required = ["libth_transformer.so", "librtp_compute_ops.so", "libth_transformer_config.so"]
