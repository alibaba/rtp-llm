...
def get_version_with_platform() -> str:
    ...
    platform = extract_platform_from_config()
    if platform and platform in PLATFORM_CONFIG_VERSIONS:
        return f"{BASE_VERSION}+{PLATFORM_CONFIG_VERSIONS[platform]}"
    
    # Try to detect platform
    try:
        detected = detect_build_config()
        if detected in PLATFORM_CONFIG_VERSIONS:
...
def generate_proto_files(project_root: Path = None) -> None:
    ...
    for proto_file in proto_files:
        ...
        try:
            ...
            # protoc.main() calls sys.exit() even on success (exit code 0)
            # Only raise error if exit code is non-zero
            if e.code is not None and e.code != 0:
                raise RuntimeError(f"Failed to compile {proto_file}, exit code: {e.code}")
...
def parse_bazel_config() -> list:
    """
    Parse RTP_BAZEL_CONFIG and return list of bazel arguments.
    
    Returns:
        list: Bazel arguments parsed from RTP_BAZEL_CONFIG
...
def build_bazel_extensions(build_config: str) -> None:
    ...
    # Add RTP_BAZEL_CONFIG args directly (native bazel format)
    if bazel_args:
        ...
        # Fallback: use detected platform config
        cmd.append(f"--config={build_config}")
...
def copy_rocm_so_files(project_root: Path, bazel_bin: Path, target_dir: Path) -> int:
    """Copy ROCm-specific .so files (aiter modules and deep_ep_rocm).
    
    Returns:
        int: Number of files copied
    """
    # Category 1: aiter_copy files (without lib prefix)
    # These come from bazel-out/k8-opt/bin/_solib_local/_U@aiter_S_S_Caiter_Uso___Uaiter_Sjit/

    
    print(f"\nCopying ROCm-specific extensions:")
    rocm_copied_count = 0
    
    # Copy aiter_copy files from bazel-out/k8-opt/bin/_solib_local/_U@aiter_S_S_Caiter_Uso___Uaiter_Sjit/
    aiter_solib_dir = project_root / "bazel-out" / "k8-opt" / "bin" / "_solib_local" / "_U@aiter_S_S_Caiter_Uso___Uaiter_Sjit"
    print(f"  Copying aiter_copy files from {aiter_solib_dir}:")
    for so_file in aiter_copy_files:
        src = aiter_solib_dir / so_file
        if not src.exists():
            ...
            if found:
                ...
                print(f"    Warning: {so_file} not found")
                continue
    ...
    # Copy aiter_src_copy and deep_ep_rocm_copy files from bazel-bin/rtp_llm/libs/
    rocm_libs_dir = bazel_bin / "rtp_llm" / "libs"
    print(f"  Copying aiter_src_copy and deep_ep_rocm_copy files from {rocm_libs_dir}:")
    for so_file in aiter_src_copy_files + deep_ep_rocm_files:
        src = rocm_libs_dir / so_file
        if not src.exists():
            # Fallback: search in bazel-bin root and subdirectories
            found = list(bazel_bin.glob(f"**/{so_file}"))
            if found:
                src = found[0]
            else:
                print(f"    Warning: {so_file} not found in bazel-bin")
                continue
    ...
    print(f"  Copied {rocm_copied_count} ROCm-specific .so files")
    return rocm_copied_count
