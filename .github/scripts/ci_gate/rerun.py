...
NO_RUN_COMMENT = (
    ...
    "to create a native build run, then re-approve or post `lgtm ready to ci <head-sha>`."
)
...
EXPIRED_RUN_COMMENT = (
    ...
    "**To fix:** push any commit (even empty: "
    "`git commit --allow-empty -m \"trigger CI\" && git push`) "
    "to create a fresh native build run, then re-approve or post `lgtm ready to ci <head-sha>`."
)
