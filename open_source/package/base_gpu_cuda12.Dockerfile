...
RUN wget http://search-ad.oss-cn-hangzhou-zmf.aliyuncs.com/yp440239%2Fnvshmem_based_nic_libs_47.1-1.3.alios7.x86_64.rpm -O /tmp/nvshmem.rpm && \
    cd / && \
    rpm2cpio /tmp/nvshmem.rpm | cpio -idmv && \
    rm -f /tmp/nvshmem.rpm

RUN rpm -Uvh http://search-ad.oss-cn-hangzhou-zmf.aliyuncs.com/rpms/gdrcpy/gdrcopy-2.4.4-1.x86_64.rpm && rpm -Uvh http://search-ad.oss-cn-hangzhou-zmf.aliyuncs.com/rpms/gdrcpy/gdrcopy-devel-2.4.4-1.noarch.rpm

# RUN wget http://search-ad.oss-cn-hangzhou-zmf.aliyuncs.com/rpms%2Facclep%2FAcclEP-1.0.0.5.alios7.x86_64.rpm -O /tmp/AcclEP.rpm && \
#     cd / && \
#     rpm2cpio /tmp/AcclEP.rpm | cpio -idmv && \
#     rm -f /tmp/AcclEP.rpm

RUN rpm -Uvh http://yum.tbsite.net/taobao/7/x86_64/current/nic-libs-mellanox-rdma/nic-libs-mellanox-rdma-47.1-1.3.alios7.hpn.x86_64.rpm
# torch is using cudnn8
# RUN --mount=type=secret,id=id --mount=type=secret,id=secret cd /tmp/ && \
#     osscmd multiget --thread_num=30 oss://search-ad/pkg/cudnn-local-repo-rhel8-9.6.0-1.0-1.x86_64.rpm 1.rpm --id=$(cat /run/secrets/id) --key=$(cat /run/secrets/secret) --host=oss-cn-hangzhou-zmf-internal.aliyuncs.com && \
#     rpm2cpio 1.rpm | cpio -div && \
#     rpm2cpio ./var/cudnn-local-repo-rhel8-9.6.0/libcudnn9-devel-cuda-12-9.6.0.74-1.x86_64.rpm | cpio -div && \
#     rpm2cpio ./var/cudnn-local-repo-rhel8-9.6.0/libcudnn9-cuda-12-9.6.0.74-1.x86_64.rpm | cpio -div && \
#     mv ./usr/include/* /usr/local/cuda/include/ && \
#     mv ./usr/lib64/* /usr/local/cuda/lib64/ && \
#     rm ./var/ ./etc/ ./usr/ -rf && \
#     rm -f 1.rpm
