...
#   sh dash_sc_start.sh
#   ./dash_sc_start.sh
#   source path/dash_sc_start.sh   # runs maga in child shell; do not use exec on your login shell
#
...
_dash_sc_resolve_checkpoint() {
  _arc_base=$1
  if [ ! -d "$_arc_base" ]; then
    echo "dash_sc_start.sh: CHECKPOINT_PATH is not a directory: $_arc_base" >&2
    return 1
  fi
  if _dash_sc_has_model_weights "$_arc_base"; then
    printf '%s\n' "$_arc_base"
    return 0
  fi
  ...
  _arc_best=""
  _arc_best_len=0
  _arc_tmp=$(mktemp "${TMPDIR:-/tmp}/dash_sc.XXXXXX") || return 1
  ...
  while IFS= read -r _arc_cfg || [ -n "$_arc_cfg" ]; do
    ...
    if ! _dash_sc_has_model_weights "$_arc_dir"; then
      continue
    fi
    _arc_len=${#_arc_dir}
  ...
  if [ -z "$_arc_best" ]; then
    echo "dash_sc_start.sh: no model root (config.json + weights) found under: $_arc_base" >&2
    return 1
  fi
  printf '%s\n' "$_arc_best"
}

RESOLVED=$(_dash_sc_resolve_checkpoint "$BASE") || {
  return 1 2>/dev/null || exit 1
}
export CHECKPOINT_PATH="$RESOLVED"

echo "dash_sc_start.sh: HIPPO_PROC_WORKDIR=$HIPPO_PROC_WORKDIR"
echo "dash_sc_start.sh: CHECKPOINT_PATH=$CHECKPOINT_PATH"

_MAGA_START="/usr/bin/maga_start.sh"
if [ ! -f "$_MAGA_START" ]; then
  echo "dash_sc_start.sh: maga_start.sh not found: $_MAGA_START" >&2
  return 1 2>/dev/null || exit 1
fi

# Direct run (script name is dash_sc_start.sh): exec maga. Sourced ($0 is parent shell): child only.
case "$0" in
  *dash_sc_start.sh)
