...
├── Task 7: BlockInfo.h — 扩展 is_cuda → is_device 或添加 is_npu
...
修改（配合 Task 7 的 BlockInfo 变更）：
```cpp
auto  dst_device = dst_block.is_npu ? torch::Device(torch::kPrivateUse1) :
                   dst_block.is_cuda ? torch::kCUDA : torch::kCPU;
...
```cpp
info.is_cuda = dev.is_cuda();
info.is_npu  = dev.is_privateuseone();
```
...
info.is_npu  = t.is_privateuseone();
info.device_index = t.is_cuda() || t.is_privateuseone() ? static_cast<int32_t>(t.get_device()) : 0;
```

---

## Task 7: BlockInfo.h — 扩展 is_npu 字段

**Files:**
- Modify: `rtp_llm/cpp/cache/BlockInfo.h:13-17`
...
struct BlockInfo {
    bool    is_cuda      = false;
    bool    is_npu       = false;  // Ascend NPU
    int32_t device_index = 0;
    ...
};
```

> **兼容性:** 现有代码中 `is_cuda` 用于判断是否为 GPU 设备。添加 `is_npu` 字段后：
> - CUDA/ROCm 路径：`is_cuda=true, is_npu=false`，行为不变
> - Ascend 路径：`is_cuda=false, is_npu=true`，需要调用方检查两者
> 
> 现有只检查 `is_cuda` 的代码（如 `is_cuda ? kCUDA : kCPU`）在 Ascend 下会回退到 CPU，不会导致崩溃但性能下降。Task 6 已修复关键路径。
