...
#include "src/fastertransformer/cuda/gemm_test/gpt_gemm_func.h"
...
int main(int argc, char* argv[])
{
    if (argc < 10 || argc > 12) {
        ...
        FT_LOG_ERROR("e.g. ./bin/gpt_gemm 8 4 32 32 96 128 49152 51200 1 8 1");
        return 0;
    }
    ...
    const int                head_num_kv   = atoi(argv[5]);
    const int                size_per_head = atoi(argv[6]);
    const int                inter_size    = atoi(argv[7]);
    const int                vocab_size    = atoi(argv[8]);
    const ft::CublasDataType data_type     = static_cast<ft::CublasDataType>(atoi(argv[9]));  // 0 FP32, 1 FP16, 2 BF 16
    const int                tensor_para_size = argc < 10 ? 1 : atoi(argv[10]);
    const bool               is_append        = argc < 11 ? false : (bool)(atoi(argv[11]));
    ...
    FT_LOG_INFO("  head_num_kv: %d", head_num_kv);
