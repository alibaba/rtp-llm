...
__global__ void kuserbuffers_pushrecv(int      myrank,
                                      int      peer,
                                      int*     recv_id,
                                      int*     flagptr,
                                      int      adder,
...
template<typename T>
__global__ void reduceCudaKernel(void *inputs, void *output, const int num_inputs, const int input_size, const size_t aliged_elements_per_input) {
...
void localReduce(void *inputs, void *output, int num_inputs, int input_size, cudaStream_t stream) {
    constexpr uint32_t vec_size = 16 / sizeof(T);
    FT_CHECK(input_size % vec_size == 0);
