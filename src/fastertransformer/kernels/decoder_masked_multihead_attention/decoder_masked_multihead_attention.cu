    ...
    [&] {                                                               \
        if constexpr (!std::is_same<T, float>::value) {                 \
            ...
            if (params.int8_kv_cache || params.fp8_kv_cache) {          \
                FT_FAIL("unsupported float type mmha with quant kvcache"); \
            }                                                           \
            typedef T NAME;                                             \
            return __VA_ARGS__();                                       \
        }                                                               \
        ...
        FT_TCACHE_SWITCH(Tcache, [&]{
            FT_HEADN_SWITCH(params.hidden_size_per_head, [&]{
                FT_ROPE_SWITCH(params.rope_config.style, ROPE_STYLE, [&]{
                    mmha_launch_kernel_ex<T, Tcache, KVCacheBuffer, KernelParamsType, Dh, false, MULTI_BLOCK_MODE, ROPE_STYLE>(
