...
def copy_file(src_num: int, dst_num: int):
    src_files = _gl.glob(f"decoder_masked_multihead_attention_{src_num}_*.cu")
    for src_file in src_files:
        dst_file = src_file.replace(
            f"decoder_masked_multihead_attention_{src_num}",
            f"decoder_masked_multihead_attention_{dst_num}")
    ...
    old_file = _pl.Path(f"decoder_masked_multihead_attention_{dst_num}.cu")
    if old_file.exists():
