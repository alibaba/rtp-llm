                                                  ...
                                                  KVCacheBuffer                                     kvCacheBuffer)
{
    ...
    const int tlength             = DO_CROSS_ATTENTION ? params.memory_length_per_sample[bi] - 1 :
    ...
    const auto ia3_ti_hi =
        do_ia3 ? flat_index2(static_cast<unsigned>(params.ia3_tasks[bi / beam_width]), hi, num_heads) : 0;
    ...
    const int input_len            = (params.input_lengths == nullptr) ? 0 : params.input_lengths[bi];
    int       prefix_prompt_length = (params.prefix_prompt_lengths == nullptr) ? 0 : params.prefix_prompt_lengths[bi];
...
#pragma unroll
                ...
                store_8bits_kv_cache_vec(
                    reinterpret_cast<Tcache*>(k_cache), k, inBlockIdx, float(1 << (8 - 1)) / k_max);
                if (k_idx == 0) {
                    float* k_scale_ptr = reinterpret_cast<float*>(kvCacheBuffer.getKScalePtr(bi, tlength));
...
#pragma unroll
    for (unsigned ii = 0; ii < K_VECS_PER_THREAD; ++ii) {
        q_vec[ii] = *reinterpret_cast<const K_vec_k*>(&q_smem[flat_index2(ii, k_idx.y, K_ELTS_PER_CHUNK)]);
    }
            ...
            for (int k_vec_i = 0; k_vec_i < K_VECS_PER_THREAD; ++k_vec_i) {
                ...
                if constexpr (ENABLE_8BITS_CACHE) {
                    ...
                    convert_from_8bit_kv_cache<K_vec_m, K_vec_k, Tcache, T_scale>(
                        &k_vec[k_vec_i], k_vec_cache[k_loop][k_vec_i], k_scale_ptr[inScaleIdx]);
                }
                else {
                ...
                if (params.relative_attention_bias != nullptr) {
                    if (implicit_rel_attn_bias) {
                        ...
                        qk_ = add(qk_,
                                  params.relative_attention_bias[hi * params.relative_attention_bias_stride
                                                                     * params.relative_attention_bias_stride
                                                                 + tlength * params.relative_attention_bias_stride
                                                                 + local_time_now]);
                    }
                }
...
#pragma unroll
                for (int k_vec_i = 0; k_vec_i < K_VECS_PER_THREAD; ++k_vec_i) {
                    ...
                    if constexpr (ENABLE_8BITS_CACHE) {
                        float* k_scale_ptr =
                            reinterpret_cast<float*>(kvCacheBuffer.getKScalePtr(seqIdx, valid_time_now));
                        int inScaleIdx = kvCacheBuffer.getKVScaleLocalIdx(valid_time_now, hi_kv);
            ...
            for (int v_loop = 0; v_loop < V_LOOP_UNROLL; v_loop++) {
                ...
                if constexpr (ENABLE_8BITS_CACHE) {
                    ...
                    convert_from_8bit_kv_cache<V_vec_m, V_vec_k, Tcache, T_scale>(
                        &v_vec, v_vec_cache[v_loop], v_scale_ptr[inScaleIdx]);
                }
                else {
            ...
            if (!MULTI_BLOCK_FLAG || (c_tile + 1) * timesteps_per_block > input_length) {
                for (int ti = generation_start_ti; ti < generation_v_loop_end; ti += V_PER_ITER) {
                    ...
                    if (ENABLE_8BITS_CACHE) {
                        float* v_scale_ptr = reinterpret_cast<float*>(kvCacheBuffer.getVScalePtr(rowIdx, time_idx));
                        int    inScaleIdx  = kvCacheBuffer.getKVScaleLocalIdx(time_idx, hi_kv);
                        load_8bits_kv_cache_vec(&v, v_cache_batch, inBlockIdx, v_scale_ptr[inScaleIdx]);
                    }
                    else {
...
#pragma unroll
                ...
                if (vi == 0) {
                    float* v_scale_ptr = reinterpret_cast<float*>(kvCacheBuffer.getVScalePtr(bi, tokenIdx));
