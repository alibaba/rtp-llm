...
template<typename T>
ft::NcclParam FtDynamicDecode<T>::tensor_para_;

template<typename T>
ft::NcclParam FtDynamicDecode<T>::pipeline_para_;
...
void FtDynamicDecode<T>::forward(th::Tensor&              logits,  // (batch_size, beam_width, hidden_size)
                                 ...
                                 th::optional<th::Tensor> index_log_probs_opt,
...
#define SAFE_INSERT(map_, name_, type_, optional_)                                                                     \
    ...
    SAFE_INSERT(output_tensors, "index_log_probs", float, index_log_probs_opt);
    SAFE_INSERT(output_tensors, "token_id_for_index_prob", int, token_id_for_index_prob_opt);
    SAFE_INSERT(output_tensors, "tgt_cache_indirection", int, tgt_cache_indirection_opt);
    SAFE_INSERT(output_tensors, "normed_scores", float, normed_scores);
    SAFE_INSERT(output_tensors, "min_normed_scores", float, min_normed_scores);
    SAFE_INSERT(output_tensors, "num_beams", int, num_beams);
    ...
    switch (scalar_type_) {
        case at::ScalarType::Float:
            dynamic_decode_ = std::make_unique<FtDynamicDecode<float>>(
                vocab_size_, vocab_size_padded_, tensor_para_size_, pipeline_para_size_, master_ip_, master_port_);
            break;
...
#endif
        .def(torch::jit::init<int64_t, int64_t, int64_t, int64_t, at::ScalarType, std::string, int64_t>())
