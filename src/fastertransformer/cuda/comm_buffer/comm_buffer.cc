...
CommBuffer::CommBuffer(const NcclParam& nccl_para,
               ...
               bool                       is_ag,
               cudaStream_t               stream) {
    FT_LOG_INFO("Create Userbuffers Communicator\n");
    ...
    FT_LOG_DEBUG("UB_TIMEOUT is set to %d sec, %" PRIu64 " cycles, freq: %dkhz\n",
            sec_timeout,
            comm->ub_timeout,
            device_clock);
    ...
    FT_CHECK_WITH_INFO(buffer_shape.size() == 2, "Userbuffer shape must be 2-dimensional!");
    ...
    FT_LOG_DEBUG("buffer_shape[0] %zu, buffer_shape[1] %zu, getTypeSize(buffer_dtype) %zu, buffer_bytes %zu, tp_size %d", buffer_shape[0], buffer_shape[1], getTypeSize(buffer_dtype), buffer_bytes, tp_size);
...
int register_user_buffer_collective(void** gpubuff, size_t bytes, Communicator* comm, const NcclParam& nccl_para, cudaStream_t stream) {
    ...
    FT_CHECK_WITH_INFO(comm->tp_size <= 8, "CUDA IPC supports only up to 8 GPUs in an NVLink domain.");
    ...
    FT_LOG_DEBUG("nccl_param %s", nccl_para.toString().c_str());
...
std::unique_ptr<CommBuffer>
initCommBuffer(std::vector<size_t>& buffer_shape, DataType buffer_type, const NcclParam& nccl_para, const std::vector<size_t>& tp_ranks,  bool is_ag, cudaStream_t stream) {
    if (!checkP2PAvailable(tp_ranks, nccl_para.rank_)) {
        FT_FAIL("P2P is not available for comm_overlap_type 2");
    }

    auto comm = std::make_unique<CommBuffer>(nccl_para, buffer_shape, buffer_type, tp_ranks, nccl_para.rank_, is_ag, stream);
    return comm;
}

void invokeLocalReduceDispatch(DataType data_type, void *inputs, void *output, int num_inputs, int input_size, cudaStream_t stream) {
