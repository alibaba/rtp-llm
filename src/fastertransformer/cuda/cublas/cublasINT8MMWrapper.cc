...
template<typename T>
int cublasINT8MMWrapper::getFusedINT8QKVType(const int k, const int n, const T* query_weight, const T* key_weight, const T* value_weight) {

    int           fusedINT8QKV_type = 0;
    const int8_t* Q_weight          = (const int8_t*)(query_weight);
    const int8_t* K_weight          = (const int8_t*)(key_weight);
    const int8_t* V_weight          = (const int8_t*)(value_weight);
    // for QKV weight are DataType_ & continue
    if ((query_weight + n * k == key_weight)
        && (key_weight+ n * k == value_weight)) {
        fusedINT8QKV_type = 1;
    }
...
template int
cublasINT8MMWrapper::getFusedINT8QKVType(const int k, const int n,  const float* query_weight, const float* key_weight, const float* value_weight);

template int
cublasINT8MMWrapper::getFusedINT8QKVType(const int k, const int n,  const half* query_weight, const half* key_weight, const half* value_weight);
