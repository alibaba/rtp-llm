...
MemoryTracker::MemoryTracker(void* ptr, const size_t size) {
    total_size_ = size;
...
void* MemoryTracker::allocate(const size_t alloc_size) {
    ...
    for (auto& pair : chunk_map_) {
        ...
        if ((!chunk->used) && (chunk->size >= alloc_size) && \
    ...
    // 2. allocate memory
    if (chunk_to_use) {
        if (chunk_to_use->size == alloc_size) {
            chunk_to_use->used = true;
            return chunk_to_use->ptr;
        } else {
            auto chunk_size = chunk_to_use->size;
            chunk_to_use->used = true;
            chunk_to_use->size = alloc_size;
            auto new_chunk = new MemoryChunk();
            new_chunk->ptr = (void*)((size_t)chunk_to_use->ptr + alloc_size);
            new_chunk->size = chunk_size - alloc_size;
    ...
    FT_LOG_DEBUG("Memory tracker failed to allocate memory of size %lu", alloc_size);
    return nullptr;
}
