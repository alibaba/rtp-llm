...
c10d::TCPStore* createTcpStore(const std::string& master_ip, const int port,  
...
void setUniqueId(ncclUniqueId* id, const std::string& store_key, 
    ...
    try {
        auto vec = std::vector<uint8_t>(
            ...
            reinterpret_cast<uint8_t*>(id) + NCCL_UNIQUE_ID_BYTES);
        tcp_store->set(store_key, vec);
    } catch (const std::exception& e) {
        FT_CHECK_WITH_INFO(false, "failed to set unique id");
    }
}
...
void getUniqueId(ncclUniqueId* id, const std::string& store_key, 
    ...
    try {
        ...
        FT_CHECK_WITH_INFO(false, "failed to get unique id");
    }
}
#endif
