...
__global__ void expandInputRowsContiguousKernel(T const*      unpermuted_input,
    ...
    auto const* source_fp8_scales_row_ptr = reinterpret_cast<ScaleDataElem const*>(fp8_scales + source_row * scale_cols);
    auto*       dest_fp8_scales_row_ptr   = reinterpret_cast<ScaleDataElem*>(permuted_output_fp8_scales + expanded_dest_row * scale_cols);
...
__global__ void doActivationContiguousKernel(T* output, GemmOutputType const* gemm_result, float const* fp8_quant,
    ScaleBiasType const* bias_ptr, bool bias_is_broadcast, int const* src_row_to_dst,
                                             int64_t inter_size, bool gated, int const* permuted_experts)
{
    ...
    int const token = src_row_to_dst[expanded_source_row];
    if (token < 0)
    {
        return;
    }
                            ...
                            ActivationType activation_type, int const* permuted_experts, cudaStream_t stream)
{
    ...
    auto fn_list = std::array{
        &doActivationContiguousKernel<T, GemmOutputType, ScaleBiasType, cutlass::epilogue::thread::GELU>,    // Gelu
        &doActivationContiguousKernel<T, GemmOutputType, ScaleBiasType, cutlass::epilogue::thread::ReLu>,    // Relu
        &doActivationContiguousKernel<T, GemmOutputType, ScaleBiasType, cutlass::epilogue::thread::SiLu>,    // Silu
        &doActivationContiguousKernel<T, GemmOutputType, ScaleBiasType, cutlass::epilogue::thread::SiLu>,    // Swiglu
        &doActivationContiguousKernel<T, GemmOutputType, ScaleBiasType, cutlass::epilogue::thread::GELU>,    // Geglu
        &doActivationContiguousKernel<T, GemmOutputType, ScaleBiasType, cutlass::epilogue::thread::Identity> // Identity
    };
    ...
    fn<<<blocks, threads, 0, stream>>>(output, gemm_result, fp8_quant, bias, bias_is_broadcast,
                                       src_row_to_dst, inter_size, isGatedActivation(activation_type), permuted_experts);
}

template void doActivationContiguous<__nv_bfloat16, __nv_bfloat16, __nv_bfloat16>(__nv_bfloat16* output, __nv_bfloat16 const* gemm_result, float const* fp8_quant, __nv_bfloat16 const* bias,
                                                                                  bool bias_is_broadcast, int const* src_row_to_dst, int num_rows, int64_t inter_size,
                                                                                  ActivationType activation_type, int const* permuted_experts, cudaStream_t stream);
}  // namespace fastertransformer
