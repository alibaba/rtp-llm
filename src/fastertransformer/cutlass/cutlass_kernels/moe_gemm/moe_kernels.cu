...
void topkGatingSoftmaxKernelLauncher(const float* input,
                                     const bool*  finished,
                                     float*       output,
...
__global__ void expandInputRowsKernel(const T*       unpermuted_input,
                                      T*             permuted_output,
...
template<typename T>
void expandInputRowsKernelLauncher(const T*       unpermuted_input,
                                   T*             permuted_output,
                                   const int*     expanded_dest_row_to_expanded_source_row,
                                   int*           expanded_source_row_to_expanded_dest_row,
                                   const int      num_rows,
...
void finalizeMoeRoutingKernelLauncher(const T*                        expanded_permuted_rows,
    ...
    if (skip_1 == nullptr || !is_rank_0) {
        ...
        finalizeMoeRoutingKernelLauncherSelectBias<T, 1>(expanded_permuted_rows,
                                                         reduced_unpermuted_output,
                                                         skip_1,
                                                         skip_2,
                                                         bias,
        ...
        finalizeMoeRoutingKernelLauncherSelectBias<T, 2>(expanded_permuted_rows,
                                                         reduced_unpermuted_output,
                                                         skip_1,
                                                         skip_2,
                                                         bias,
...
__global__ void doGatedActivationKernel(T* output, const T* gemm_result, size_t inter_size) {
    ...
    gemm_result = gemm_result + token * inter_size;
    for (int i = tid; i < inter_size; i += blockDim.x) {
        float fc1_value = output[i];
        T     fc3_value = gemm_result[i];
        T     gate_act  = fn(fc1_value);
        output[i]       = fc3_value * gate_act;
    }
}
...
void doGatedActivation(T*                 output,
    ...
    auto* fn = activation_type == ft::ActivationType::Swiglu ?
...
CutlassMoeFCRunner<T, WeightType, Enable>::getWorkspaceBufferSizes(const int          num_rows,
                                                                   ...
                                                                   const int          k,
                                                                   ft::ActivationType activation_type) const {
    ...
    const size_t glu_inter_elems = isGatedActivation(activation_type) ? (interbuf_elems) : 0;
...
void CutlassMoeFCRunner<T, WeightType, Enable>::runMoe(const void*          input_activations_void,
    ...
    if(isGatedActivation(fc1_activation_type)){
        TLLM_CHECK_WITH_INFO(fc3_expert_weights != nullptr, "Gated Activation needs fc3_expert_weights");
    } else {
        TLLM_CHECK_WITH_INFO(fc3_expert_weights == nullptr, "only Gated Activation needs fc3_expert_weights");
    }
    ...
    topkGatingSoftmaxKernelLauncher(gating_output,
                                    finished,
                                    expert_scales,
                                    softmax_out_,
                                    expert_for_source_row,
    ...
    if (!isGatedActivation(fc1_activation_type)) {
        ...
        doGatedActivation<T>(fc1_result_, glu_inter_result_, inter_size, num_rows * k, fc1_activation_type, stream);
        ft::sync_check_cuda_error();
        ft::print_bsd(0, "moe fn1 * 3", fc1_result_, k, num_rows, inter_size);
    }
    ...
    ft::sync_check_cuda_error();
    ft::print_bsd(0, "moe fn2", fc2_result, k, num_rows, hidden_size);

    finalizeMoeRoutingKernelLauncher<T>(fc2_result,
                                        final_output,
...
void makeLoadBalancedRoutingConfiguration(
    ...
    ft::check_cuda_error(cudaMemsetAsync(data_void, 0x0, num_experts * num_tokens * item_size, stream));

    int stride = ft::ceilDiv(num_experts, k);

    int blockDim = 256;
    int gridDim  = ft::ceilDiv(num_tokens, blockDim);
