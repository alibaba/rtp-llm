                             ...
                             const bool      is_weight_only)
{
    ...
    const size_t ctas_in_m_dim     = (m + tile_shape.m - 1) / tile_shape.m;
    const size_t ctas_in_n_dim     = (n + tile_shape.n - 1) / tile_shape.n;
...
std::vector<CutlassTileConfig> get_candidate_tiles(const bool is_weight_only, const bool simt_configs_only, int sm)
{
    ...
    if (sm >= 75) {
        square_configs.push_back(CutlassTileConfig::CtaShape128x128x64_WarpShape128x32x64);
    }
...
{
    std::vector<CutlassTileConfig> tiles = get_candidate_tiles(is_weight_only, simt_configs_only, sm);
