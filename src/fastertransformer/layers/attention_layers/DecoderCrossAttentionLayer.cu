                                       ...
                                       const T*    ia3_value_weights)
{
    ...
    for (int ite = 0; ite < length; ++ite) {
        ...
        if (step == 1 && tid < size_per_head) {
            ...
            if (do_ia3) {
                key = mul<T, T, T>(key, ia3_key_weights[(ia3_task * head_num + head_id) * size_per_head + tid]);
            }
                              ...
                              cudaStream_t                     stream)
{
    if (!batch_major_cache) {
        ...
        KVLinearBuffer kv_cache_buffer(inference_batch_size, 1, memory_max_len, head_num * size_per_head * sizeof(DataType));
