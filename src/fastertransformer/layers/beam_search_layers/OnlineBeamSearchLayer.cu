...
void OnlineBeamSearchLayer<T>::invokeSoftMax(TensorMap* output_tensors, TensorMap* input_tensors)
{
    ...
    BeamHypotheses beam_hyps;
    beam_hyps.output_ids_tgt       = output_tensors->at("output_ids").getPtr<int>();
    beam_hyps.sequence_lengths_tgt = output_tensors->at("sequence_length").getPtr<int>();
    beam_hyps.cum_log_probs        = output_tensors->at("cum_log_probs").getPtr<float>();
    beam_hyps.normed_scores        = output_tensors->at("normed_scores").getPtr<float>();
    beam_hyps.log_probs            = output_tensors->at("output_log_probs").getPtr<float>();
    beam_hyps.min_normed_scores    = output_tensors->at("min_normed_scores").getPtr<float>();
    beam_hyps.num_beams            = output_tensors->at("num_beams").getPtr<int>();
    beam_hyps.is_done              = output_tensors->at("finished").getPtr<bool>();
    ...
    beam_hyps.log_probs_src        = output_tensors->at("output_log_probs").getPtr<float>();
...
void OnlineBeamSearchLayer<T>::allocateBuffer(size_t batch_size, size_t beam_width)
{
    ...
    topk_softmax_workspace_ = reinterpret_cast<float*>(
        allocator_->reMalloc(topk_softmax_workspace_, sizeof(float) * topk_softmax_workspace_size_));
