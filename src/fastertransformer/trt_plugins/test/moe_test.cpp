...
public:
    void RunMoeTest(
        ...
        if (range_msg == nullptr) {
            range_msg_str = "moe-bs" + std::to_string(token_num)
            + "-dim" + std::to_string(hidden_dim)
            + "-indim" + std::to_string(moe_inter_size)
            + "-exp_num" + std::to_string(num_expert)
            + "-ep" + std::to_string(ep_size)
        ...
        moe_plugin.init(num_expert,
                        ...
                        hidden_dim,
                        moe_inter_size,
                        fastertransformer::ActivationType::Swiglu,
        ...
        const auto expert_scales = device_->allocateBuffer({DataType::TYPE_FP32, {fastertransformer::pad_to_multiple_of_16(token_num * top_k)}});

        const auto expanded_source_row_to_dest = device_->allocateBuffer({DataType::TYPE_INT32, {fastertransformer::pad_to_multiple_of_16(token_num * top_k)}});
        const auto expert_for_source_row = device_->allocateBuffer({DataType::TYPE_INT32, {fastertransformer::pad_to_multiple_of_16(token_num * top_k)}});
        ...
        auto fc1_weight = torch::randn({(int)(num_expert / ep_size), (int)2 * moe_inter_size, (int)hidden_dim}, torch::Device(torch::kCUDA)).to(torch::kFloat16);
        auto fc2_weight = torch::randn({(int)(num_expert / ep_size), (int)hidden_dim, (int)moe_inter_size}, torch::Device(torch::kCUDA)).to(torch::kFloat16);

        const uint64_t round = bench_round + 1;
        for (int i = 0; i < round; ++i) {
            if (i != 0) nvtxRangePushA(range_msg);
            ...
            if (i == 0) {
                printBufferData(*ft::torchTensor2Buffer(input), "input");
                printBufferData(*ft::torchTensor2Buffer(gate), "gate");
