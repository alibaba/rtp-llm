...
MoeDispatchOutput CudaDevice::deepEpLLDispatch(const MoeDispatchParams& params) {
    //FT_LOG_INFO("call deep ep ll dispatch");
    printBufferData(params.input, "params.input");
    printBufferData(params.expert_ids, "params.expert_ids");
    printBufferData(params.expert_scales, "params.expert_scales");
    ...
    printBufferData(*hidden, "hidden", nullptr, false);
    printBufferData(*expert_ids, "expert_ids");
    ...
    printTorchTensorData(input_tensor, "input_tensor");
    printTorchTensorData(topk_idx_tensor, "topk_idx_tensor");

    cudaDeviceSynchronize();
    printTorchTensorData(topk_idx_tensor, "dispatch topk_idx_tensor", nullptr, true);
    ...
    cudaDeviceSynchronize();
    check_cuda_error(cudaGetLastError());
    printTorchTensorData(dispatch_output.packed_recv_count, "dispatch packed_recv_count", nullptr, true);
    printTorchTensorData(dispatch_output.handle.packed_recv_layout_range, "dispatch packed_recv_layout_range", nullptr, true);
    printTorchTensorData(dispatch_output.handle.packed_recv_src_info, "dispatch packed_recv_src_info", nullptr, true);

    BufferPtr packed_recv_x_buffer = torchTensor2Buffer(dispatch_output.packed_recv_x.toType(torch::kHalf));
    printBufferData(*packed_recv_x_buffer, "packed_recv_x_buffer");
    ...
    //FT_LOG_INFO("done call lowLatencyDispatch");
    return out;
}

FfnLayerOutput CudaDevice::deepEpLLCombine(const MoeCombineParams& params) {
    //FT_LOG_INFO("begin call deep ep ll combine");
    ...
    //printBufferData(*params.input, "combine params.input");
    //printBufferData(*params.expert_ids, "params.expert_ids");
    //printBufferData(*params.expert_scales, "params.expert_scales");
    ...
    //printTorchTensorData(topk_idx_tensor, "topk_idx_tensor");
   // printTorchTensorData(topk_weights_tensor, "topk_weights_tensor");
    ...
    if (params.output != nullptr) {
        all_output = torchTensor2Buffer(combine_output.combined_x.toType(dataTypeToTorchType(params.output->type())));
    } else {
        all_output = torchTensor2Buffer(combine_output.combined_x.toType(dataTypeToTorchType(params.input->type())));
    }
    printBufferData(*all_output, "all_output", nullptr, false);

    //FT_LOG_INFO("end call lowLatencyCombine");
...
FfnLayerOutput CudaDevice::deepEpLLMoeFfnLayer(const FfnLayerParams& params, const MoeGateSelectOutput& gate_output) {
    //FT_LOG_INFO("begin call deep ep ll moe ffn layer");
    ...
    for (int i = 0; i < recv_count_cpu->shape()[0]; i++) {
        ...
        //FT_LOG_INFO("start prepare expert %d, token num %d", start_expert_id + i, token_num);
        ...
        expert_contexts.emplace_back(DeepEPLowLatencyExpertContext(i, token_num, expert_all_token_hidden_states, expert_token_hidden_states, expert_ids_gpu_buffer, expert_scales_gpu_buffer));
    }
    cudaDeviceSynchronize();
    //check_cuda_error(cudaGetLastError());
    ...
    for (auto& context : expert_contexts) {
        ...
        //FT_LOG_INFO("start process expert %d, token num %d", start_expert_id + context.index, context.token_num);
        auto moe_ffn_params =
    ...
    for (auto& context : expert_contexts) {
        ...
        printBufferData(*context.hidden_states, "expert_token_hidden_states_input", nullptr, false, true);
        std::string key = "expert_token_hidden_states_output_" + std::to_string(context.index);
        printBufferData(*expert_token_hidden_states, key, nullptr, true, true);
        printBufferData(*context.expert_ids, "expert_ids", nullptr, false);
        printBufferData(*context.expert_scales, "expert_scales", nullptr, false);
    }
    cudaDeviceSynchronize();
    check_cuda_error(cudaGetLastError());
