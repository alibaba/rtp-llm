...
FfnLayerOutput CudaDevice::moeFfnLayer(const FfnLayerParams& params) {
    ...
    const auto expert_hidden_size = params.weights.moe_up_weight->kernel->isQBuffer()
                                  ? params.weights.moe_up_weight->kernel->shape()[2]
                                  : params.weights.moe_up_weight->kernel->shape()[1];
