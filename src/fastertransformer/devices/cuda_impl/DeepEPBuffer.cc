...
bool DeepEPBuffer::init() {
    ...
    if (buffer_->get_num_rdma_ranks() > 1 || low_latency_mode_) {
        ...
        if ((low_latency_mode_ && world_rank_ == 0) || (!low_latency_mode_ && buffer_->get_rdma_rank() == 0)) {
            auto local_nvshmem_unique_id = buffer_->get_local_nvshmem_unique_id();
    ...
    buffer_->sync(device_ids, ipc_handles, root_unique_id);
    return true;
}
...
deep_ep::Config DeepEPBuffer::getDispatchConfig(int num_ranks) {
    ...
    switch (num_ranks) {
        ...
        default:
            FT_CHECK_WITH_INFO(false, "num_ranks is not supported");
    }
...
DeepEPDispatchOutput DeepEPBuffer::dispatch(const torch::Tensor&                       x,
    ...
    if (x_scales.has_value()) {
        FT_CHECK(x.scalar_type() == c10::kFloat8_e4m3fn && x.sizes().size() == 2);  // [num_tokens, hidden // 128]
        FT_CHECK(x_scales->scalar_type() == c10::kFloat);
    } else {
        FT_CHECK(x.scalar_type() == c10::kBFloat16);  // [num_tokens, hidden]
    }
    FT_CHECK(num_tokens_per_rank.has_value() == false || num_tokens_per_expert->scalar_type() == c10::kInt);
    FT_CHECK(num_tokens_per_rdma_rank.has_value() == false || num_tokens_per_rdma_rank->scalar_type() == c10::kInt);
    FT_CHECK(is_token_in_rank.has_value() == false || is_token_in_rank->scalar_type() == c10::kBool);
    FT_CHECK(num_tokens_per_expert.has_value() == false || num_tokens_per_expert->scalar_type() == c10::kInt);
    FT_CHECK(topk_idx.has_value() == false || topk_idx->scalar_type() == c10::kLong);
    FT_CHECK(topk_weights.has_value() == false || topk_weights->scalar_type() == c10::kFloat);
...
DeepEPDispatchOutput DeepEPBuffer::intranodeDispatch(const torch::Tensor&                            x,
    ...
    if (handle.has_value()) {
        ...
        FT_CHECK(topk_idx.has_value());
        FT_CHECK(topk_weights.has_value());
        ...
        FT_CHECK(num_tokens_per_rank.has_value());
        FT_CHECK(num_tokens_per_expert.has_value());
        FT_CHECK(is_token_in_rank.has_value());
        auto [recv_x,
              recv_x_scales,
              recv_topk_idx,
              recv_topk_weights,
...
DeepEPDispatchOutput DeepEPBuffer::internodeDispatch(const torch::Tensor&                            x,
    ...
    if (handle.has_value()) {
        FT_CHECK(topk_idx.has_value());
        FT_CHECK(topk_weights.has_value());
        FT_CHECK(handle->recv_src_meta.has_value());
        FT_CHECK(handle->send_nvl_head.has_value());
