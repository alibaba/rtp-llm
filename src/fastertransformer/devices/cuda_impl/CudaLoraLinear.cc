...
ReduceScatterLoraLinearOutput CudaDevice::loraLinearReduceScatter(const LoraLinearReduceScatterParams& params) {
    ...
    if ((overlap_comm_type == 2) &&
        ...
        if (params.mode == ParallelMode::TP) {
            ...
            FT_CHECK("unavailable");
        }
        Communicator* comm = cb->_comm;

        FT_LOG_DEBUG("m %d, n %d", m, n);
        ...
        if (cudaGetLastError() != cudaSuccess) {
            FT_LOG_INFO(cudaGetErrorString(cudaGetLastError()));
        }
        ...
        for (int i = 0; i < tp_size; i++) {
            ...
            if (i < tp_size - 1) {
                ...
                FT_LOG_DEBUG("comm_bytes %d, output_d->sizeBytes() %d, output_data_type %d i %d send_rank %d", comm_bytes, output_d->sizeBytes(), output_data_type, i, send_rank);
        ...
        FT_CHECK_WITH_INFO(output_d->type() == DataType::TYPE_FP16 || output_d->type() == DataType::TYPE_BF16, "ReduceScatter only supports BF16/FP16 output.");
