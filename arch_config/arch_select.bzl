...
# Wheels not yet rebuilt against torch 2.11+cu130; omitted from the cuda13
# lock files, so requirement() must not reference them on cuda13 configs.
...
def whl_deps():
    return select({
        "@rtp_llm//:using_cuda13_x86": [
            # torch 2.11+cu130 mirrored on OSS (upstream: download.pytorch.org/whl/cu130).
            ...
            "rtp-kernel@http://rtp-maga.oss-cn-zhangjiakou.aliyuncs.com/serina.wzq/whl/06_11/rtp_kernel-0.1.0-cp310-cp310-linux_x86_64.whl",
            ...
            "tilelang==0.1.9",
            # flash_attn / flash_attn_3 / flashinfer_* / fast_hadamard /
            # deep_ep: deferred rebuild for torch2.11+cu130.
        ],
