...
def requirement(names):
    for name in names:
        ...
        generic_deps = [] if name in _GPU_ONLY_REQUIREMENTS else [requirement_cpu(name)]
        arm_deps = [] if name in _GPU_ONLY_REQUIREMENTS else [requirement_arm(name)]
        rocm_deps = [] if name in _GPU_ONLY_REQUIREMENTS else [requirement_gpu_rocm(name)]
        native.py_library(
            name = name,
            deps = select({
                ...
                "//conditions:default": generic_deps,
            }),
...
def whl_deps():
    return select({
        ...
        "@rtp_llm//:using_cuda13_arm": [
            ...
            "deep_gemm@https://rtp-maga.cn-zhangjiakou.oss.aliyuncs.com/0513/arm_pkg/deep_gemm-2.5.0%2B8a4dfba-cp310-cp310-linux_aarch64.whl",
        ...
        "@rtp_llm//:using_cuda12_arm": ["torch==2.6.0+cu126"],
        "@rtp_llm//:using_rocm": [
            "pyrsmi==0.2.0",
